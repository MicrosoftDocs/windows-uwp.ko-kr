...
? 포함 = valuemetadata 쿼리 매개 변수는 경합 트랙에 시간을 얻는 데 자동차의 색 모델 등 사용자 통계 값와 관련 된 메타 데이터를 포함 하도록 응답 합니다.
...
| 매개 변수| 유형| 설명|
| --- | --- | --- | --- | --- | --- |
| statNames| string| 쉼표로 구분 사용자 통계 이름 목록. 예를 들어 다음 URI는 4 개의 통계 URI에 지정 된 사용자 id를 대신 하 여 요청 된 서비스 알립니다. {:: nomakrdown}<br/><br/>`https://userstats.xboxlive.com/users/xuid({xuid})/scids/{scid}/stats/wins,kills,kdratio,headshots?include=valuemetadata`| 
| 포함 = valuemetadata| string| 응답에 uset 시작 값과 관련 된 값 메타 데이터가 포함 되어 있는지를 나타냅니다.|
...
| Xbl 계약 버전 X| string| 사용 하는 API의 버전을 나타냅니다. 이 값은 응답에 메타 데이터 값을 포함 하려면 "3"으로 설정 되어야 합니다.|
