...
항목을 제거 목록에서 인덱스를 통해 표시 하 고 쿼리 문자열 매개 변수 **인덱스**에 식별 됩니다. 한 번만 호출에서 100 개의 인덱스를 전달할 수 하는 인덱스 목록을 쉼표로 구분 되어야 합니다. 그러나 인덱스가 (빈 쿼리 문자열 매개 변수)을 전달 하는 경우 전체 목록의 내용을 삭제 됩니다 (빈 목록을 유지 됩니다 및 버전 번호를 계속 증가 됩니다). 항목이 제거 되 면 목록 "압축 됩니다"를 빈 인덱스 순서에 남아 있습니다. 따라서이 호출이 idempotent 아닙니다.
 
이 호출이 필요는 **경우-일치: versionNumber** 헤더를 **versionNumber** 는 파일의 현재 버전 번호는 요청에 포함 되어야 합니다. 포함 하지 않거나 HTTP 412 (전제 조건 실패) 다음 현재 목록 버전 번호와 일치 하지 않는 경우 상태 코드가 반환 하 고 응답 본문에는 현재 버전 번호를 포함 하는 목록의 최신 메타 데이터를 포함 됩니다. 이 무시 서로 다른 클라이언트에서 업데이트를 수행 됩니다.
