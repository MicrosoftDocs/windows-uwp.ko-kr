 ...
 * `/p` 출력 패키지 이름 설정
 * `/o` 출력 파일이 이미 있을 경우 덮어쓰도록 설정
0. 패키지를 만들었으면 서명해야 합니다. 서명 인증서를 얻는 가장 쉬운 방법은 Visual Studio에서 빈 유니버설 Windows 프로젝트를 만들고 복사입니다 합니다 `.pfx` 있지만 파일을 사용 하 여 수동으로 만들 수는 `MakeCert` 및 `Pvk2Pfx` 유틸리티에 설명 된 대로 [는 **만드는 방법 앱 패키지 서명 인증서를** MSDN의 항목] (https://msdn.microsoft.com/en-us/library/windows/desktop/jj835832(v=vs.85).aspx). 
