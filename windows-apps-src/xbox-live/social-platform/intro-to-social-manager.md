...
가져와서 소셜 그래프에 대 한 정보를 유지 관리 Xbox Live API (XSAPI)에 소셜 Api를 사용 하 여은 복잡 하며,이 정보를 최신 상태로 유지 복잡할 수 있습니다.  이렇게 하지 않으면 올바르게 성능 문제, 오래 된 데이터 또는 Xbox Live 소셜 서비스에 필요한 것 보다 더 자주 호출로 인해 조정 될 수 있습니다.
...
**소셜 그래프**: *소셜 그래프* 장치에 로컬 사용자에 대해 만들어집니다. 이 모든 사용자에 게 친구에 대 한 정보를 최신 상태로 유지 하는 구조를 만듭니다.
...
**Xbox 소셜 사용자 그룹**: 그룹은 UI 채우기와 같은 작업에 사용 되는 사용자의 컬렉션입니다. 두 가지 방법으로 그룹
...
소셜 관리자는 xbox 소셜 사용자 그룹을 최신 상태로 유지, 및 사용자 그룹 또는 사용자에 게 관계를 필터링 할 수 있습니다.  예를 들어, 사용자의 온라인 상태인 친구의 모두 포함 하 고 현재 제목 xbox 소셜 사용자 그룹을 만들 수 있습니다.  이 유지 최신 친구 시작 또는 제목 재생이 중지 합니다.
...
위에서 설명한 대로 특정 조건에 맞는 사용자 그룹. 어떤 유형의 있는 사용자를 추적 하는 이러한 그룹을 노출 하는 Xbox 소셜 사용자 그룹 또는 및 로컬 사용자 그룹에 속한 켜져 필터를 설정 하는 것입니다.
...
보기의 사용자를 호출 하 여 얻을 수 있습니다 합니다 `xbox_social_user_group::get_users()` 의 목록을 반환 하는 함수 `xbox_social_user` 개체입니다.  `xbox_social_user` 게이머 태그, 게이머 사진 uri 등과 같은 소셜 정보를 포함 합니다.
...
### <a name="updating-social-user-group-from-list"></a>목록에서 소셜 사용자 그룹을 업데이트
...
이 예제에서는 제공 추가 컨트롤의 일부를 보여 줍니다.  게임 루프 중 새 사용자 목록을 제공 하는 소셜 사용자 그룹 필터에 의존 하는 대신 소셜 그래프는 게임 루프 외부 초기화 됩니다.  제목에 의존 하는 다음의 `events` 에서 반환 되는 `socialManager->do_work()` 함수.  `events` 목록은 `social_event`, 및 각 `social_event` 마지막 프레임 중에 발생 한 소셜 그래프에 대 한 변경 포함 되어 있습니다.  예를 들어 `profiles_changed`, `users_added`등.  자세한 내용은에서 찾을 수 있습니다는 `social_event` API 설명서.
