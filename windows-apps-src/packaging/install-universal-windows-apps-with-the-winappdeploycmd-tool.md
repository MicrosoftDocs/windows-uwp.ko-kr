...
description: Windows 애플리케이션 배포(WinAppDeployCmd.exe)는 Windows 10 PC에서 Windows 10 디바이스로 UWP(유니버설 Windows 플랫폼) 앱을 배포하는 데 사용할 수 있는 명령줄 도구입니다.
ms.date: 09/30/2018
ms.topic: article
...
Windows 10 SDK를 설치하여 명령 프롬프트나 스크립트 파일에서 WinAppDeployCmd 도구를 실행하기만 하면 됩니다. WinAppDeployCmd.exe로 앱을 설치하면 이 도구는 .appx/.msix 파일 또는 AppxManifest(느슨한 파일의 경우)를 사용하여 앱을 Windows 10 디바이스에 테스트용으로 로드합니다. 이 명령은 앱에 필요한 인증서를 설치하지 않습니다. 앱을 실행하려면 Windows 10 디바이스가 개발자 모드이거나 인증서가 이미 설치되어 있어야 합니다.

모바일 디바이스에 배포하려면 먼저 패키지를 만들어야 합니다. 자세한 내용은 [여기](/windows/msix/package/packaging-uwp-apps)를 참조하세요.

**WinAppDeployCmd.exe** 도구는 Windows 10 PC에서 **C:\\Program Files (x86)\\Windows Kits\\10\\bin\\&lt;SDK Version&gt;\\x86\\WinAppDeployCmd.exe**에 있습니다(SDK 설치 경로에 따라 다름).

> [!NOTE]
> SDK 버전 15063 이상에서는 SDK가 버전별 폴더에 나란히 설치됩니다. 이전 SDK(14393 및 이전 버전)는 부모 폴더에 직접 작성되었습니다.

먼저 Windows 10 디바이스를 동일한 서브넷에 연결하거나 USB 연결로 Windows 10 머신에 직접 연결합니다. 그런 후 다음 구문이나 이 문서의 뒷부분에 있는 이 명령의 예를 사용하여 UWP 앱을 배포합니다.
...
| **명령**  | **설명**                                                     |
|--------------|---------------------------------------------------------------------|
| 디바이스      | 사용 가능한 네트워크 디바이스 목록을 표시합니다.                         |
| 설치      | UWP 앱 패키지를 대상 디바이스에 설치합니다.                     |
| 업데이트       | 대상 디바이스에 이미 설치되어 있는 UWP 앱을 업데이트합니다.    |
...
| -p(-package)    | 앱 패키지를 제거하기 위한 전체 패키지 이름입니다. list 명령을 사용하여 이미 디바이스에 설치된 패키지의 전체 이름을 확인할 수 있습니다. |
| -pin             | 대상 디바이스에 연결을 설정하는 데 필요한 핀입니다. 인증이 필요한 경우 -pin 옵션을 사용하여 다시 시도하라는 메시지가 표시됩니다. |
...
PC의 Downloads 디렉터리에 있는 MyApp.appx 패키지의 앱을 PIN이 A1B2C3이고, IP 주소가 192.168.0.1인 Windows 10 디바이스에 설치하여 디바이스에 연결합니다.
