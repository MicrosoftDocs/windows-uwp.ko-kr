...
동일한 문자열 동일한 결과 항상 제공 됩니다. 실패 결과 받으면 결과 분석 하 고 그에 따라 문자열을 수정 합니다.
 
 

> [!NOTE] 
> 결과 <b>VerifyStringResult</b> 는 문자열에 잘못 된 첫 번째 단어를 보고 합니다. 단어 문자열 내에서 잘못 된 추가 수 있습니다. 문자열을 사용할 수 있도록 잘못 된 단어를 교체 하려는 경우에 잘못 된 단어 또는 하위 문자열을 대체 하 고 잘못 된 문자열을 추가 하는 문자열을 다시 확인 해야 합니다.  
...
| x xbl-계약 버전| 정수 API 계약 버전입니다. 이 API에 대 한 1 또는 2 이어야 합니다.| 
...
요청 본문은 512 자 문자열 배열 크기에 제한이 없는와 문자열의 배열입니다.
