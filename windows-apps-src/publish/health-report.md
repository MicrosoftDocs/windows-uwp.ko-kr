...
**30d** 기간을 선택 하면 원 마커를 볼 수 있습니다. 이러한 크게 증가 개발자나에 대해 알아야 할 생각 하는 특정된 값 감소 합니다. 원이 표시 되는 날짜 크게 증가 또는 감소 하는 이전 주에 비해 감지 하는 주의 끝을 나타냅니다. 변경 된 내용에 대 한 자세한 정보를 보려면 원 위로 가져갑니다.  
...
**크래시 즈 프리 세션 및 장치** 차트 장치나 지난 30 일 이내에 충돌이 발생 하지 않은 사용자 세션의 비율을 표시 합니다. 이 정보를 사용 하면 어떻게 광범위 하 게 충돌에 사용자에 게 영향을 이해 합니다. 예를 들어 앱 10,000 충돌 하루에 있을 수 있습니다. 디바이스의 90%에 영향을 받는 경우 다음 가장 중요 한 것으로 분류 하는 하는 즉시 해결 하기 합니다. 그러나만 앱을 사용 하 여 디바이스의 5%를 나타내는 우선 순위가 낮은 수 있습니다.

이 차트에는 두 개의 탭 합니다.
- **장치 충돌 즈 프리**: (지난 30 일) 동안 각 날짜에 오류가 발생 하지 않은 고유 디바이스의 비율을 표시 합니다.
