...
**참고**  사용 하 여 앱에서 데이터 바인딩 사용에 대 한 일반 정보에 대 한 **{Binding}** (및 사이의 모든 비교 **{0} X:bind}** 고 **{Binding}**)를 참조 하세요 [깊이의 데이터 바인딩](https://msdn.microsoft.com/library/windows/apps/mt210946)합니다.
...
[**경로** ](https://msdn.microsoft.com/library/windows/apps/br209830) (소스 속성)에 바인딩하는 속성에 설명 합니다. 경로는 위치 매개 변수로, 매개 변수 이름을 명시적으로 사용하거나(`{Binding Path=EmployeeID}`), 이름 없는 첫 번째 매개 변수로 지정할 수 있습니다(`{Binding EmployeeID}`).
...
태그 확장에서 설정될 수 있는 [**Binding**](https://msdn.microsoft.com/library/windows/apps/br209820)의 읽기/쓰기 속성이 여러 개이기 때문에 **{Binding}** 은 *bindingProperties* 개체 틀 구문을 통해 설명됩니다. 쉼표로 구분된 *propName*=*value* 쌍으로 순서에 상관없이 속성을 설정할 수 있습니다. 일부 속성에는 형식 변환이 없는 형식이 필요하므로 해당 태그 확장이 **{Binding}** 에 중첩되어야 합니다.
...
**참고**  에서 태그를 변환 하는 경우 **{0} X:bind}** 하 **{Binding}**, 다음에 대 한 기본 값의 차이에 주의 **모드** 속성입니다.

[**변환기**](https://msdn.microsoft.com/library/windows/apps/br209826)를 [ **ConverterLanguage** ](https://msdn.microsoft.com/library/windows/apps/hh701880) 고 **ConverterLanguage** 는 모든 시나리오와 관련 된 값을 변환 하거나에서 입력 합니다 바인딩 소스를 유형 또는 바인딩 대상 속성에 호환 되는 값입니다. 자세한 내용과 예제는 [데이터 바인딩 심층 분석](https://msdn.microsoft.com/library/windows/apps/mt210946)의 "데이터 변환" 섹션을 참조하세요.
...
[**원본**](https://msdn.microsoft.com/library/windows/apps/br209832)하십시오 [ **RelativeSource**](https://msdn.microsoft.com/library/windows/apps/br209831), 및 [ **ElementName** ](https://msdn.microsoft.com/library/windows/apps/br209828) 되므로 바인딩 소스 지정 함께 사용할 수 없습니다.

**팁**  와 같은 값을 단일 중괄호로 지정 하는 경우 [ **경로** ](https://msdn.microsoft.com/library/windows/apps/br209830) 하거나 [ **ConverterParameter** ](https://msdn.microsoft.com/library/windows/apps/br209827), 그런 다음 앞에 백슬래시를: `\{`합니다. 또는 보조 따옴표 집합에서 이스케이프해야 하는 괄호가 포함된 전체 문자열을 다음과 같이 묶습니다. `ConverterParameter='{Mix}'`.
...
두 번째 예제 4 설정 하는 다른 [ **바인딩** ](https://msdn.microsoft.com/library/windows/apps/br209820) 속성: [**ElementName**](https://msdn.microsoft.com/library/windows/apps/br209828), [ **경로**](https://msdn.microsoft.com/library/windows/apps/br209830)하십시오 [ **모드** ](https://msdn.microsoft.com/library/windows/apps/br209829) 하 고 [ **변환기** ](https://msdn.microsoft.com/library/windows/apps/br209826). 이 경우 **Path**는 명시적으로 **Binding** 속성으로 명명되어 표시됩니다. **Path**는 동일한 런타임 개체 트리에 있는 다른 [**Slider**](https://msdn.microsoft.com/library/windows/apps/br209614) 개체(`sliderValueConverter`)인 데이터 바인딩 소스로 평가됩니다.
