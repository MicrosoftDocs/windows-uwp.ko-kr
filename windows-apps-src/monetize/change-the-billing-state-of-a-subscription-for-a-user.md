...
description: "Windows 스토어 구매 API에서 이 메서드를 사용하여 사용자의 구독 청구 상태를 변경할 수 있습니다."
...
ms.prod: windows
ms.technology: uwp
keywords: "windows 10, uwp, Windows 스토어 구매 API, 구독"
...
Windows 스토어 구매 API에서 이 메서드를 사용하여 지정된 사용자의 구독 추가 기능 청구 상태를 변경할 수 있습니다. 구독을 취소, 연장, 환불 요청, 자동 갱신을 중지할 수 있습니다.
...
* 변경할 구독에 대한 권한을 갖고 있는 사용자의 ID를 나타내는 Windows 스토어 ID 키.
...
| 필드      | 유형   | 설명   | 필수 |
|----------------|--------|---------------|----------|
| b2bKey         | 문자열 | 구독을 변경할 사용자의 ID를 나타내는 [Windows 스토어 ID 키](view-and-grant-products-from-a-service.md#step-4)입니다.     | 예      |
...
다음 예제는 이 메서드를 사용하여 구독 기간을 5일 연장하는 방법을 보여 줍니다. *b2bKey* 값을 변경할 구독의 사용자 ID를 나타내는 [Windows 스토어 ID 키](view-and-grant-products-from-a-service.md#step-4)로 바꿉니다.
...
| productId | 문자열 | Windows 스토어 카탈로그에서 구독 추가 기능을 나타내는 [제품](in-app-purchases-and-trials.md#products-skus-and-availabilities)의 [스토어 ID](in-app-purchases-and-trials.md#store-ids)입니다. 제품에 대한 스토어 ID의 예는 9NBLGGH42CFD입니다.     |
| skuId | 문자열 |  Windows 스토어 카탈로그에서 구독 추가 기능을 나타내는 [SKU](in-app-purchases-and-trials.md#products-skus-and-availabilities)의 [스토어 ID](in-app-purchases-and-trials.md#store-ids)입니다. SKU에 대한 스토어 ID의 예는 0010입니다.    |
