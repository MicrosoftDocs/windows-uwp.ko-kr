...
| **시선 추적기** | **gazeInput** 접근 권한 값을 통해 앱에서 호환 시선 추적 디바이스가 연결되어 있을 때 애플리케이션 범위 내에서 사용자가 보는 위치를 검색할 수 있습니다. 이 접근 권한 값은 [**Windows.Devices.Input.Preview**](/uwp/api/windows.devices.input.preview) 네임스페이스의 일부 API를 사용하는 데 필요합니다. |
