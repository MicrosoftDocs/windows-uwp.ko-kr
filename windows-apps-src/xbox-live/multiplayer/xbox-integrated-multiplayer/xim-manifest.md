...
title: XIM 프로젝트 구성
author: KevinAsgari
description: UWP 앱 매니페스트에서 사용 하 여 Xbox 통합 멀티 플레이어 (XIM)에 맞게 구성 하는 방법을 설명 합니다.
...
ms.locfileid: "4616012"
---
# <a name="xim-project-configuration"></a>XIM 프로젝트 구성

## <a name="required-appxmanifest-capability-content"></a>필수 AppXManifest 기능 콘텐츠

기본적으로 XIM을 사용 하 여 응용 프로그램에 연결 하 고 모두 인터넷 및 로컬 네트워크를 통해 네트워크 리소스에서 연결을 수락 해야 합니다. 음성 채팅을 지원 하도록 마이크 장치에 액세스를 해야 합니다. 결과적으로, 앱 "internetClientServer" 및 "privateNetworkClientServer" 기능 및 해당 AppXManifest에서 "마이크" 장치 접근 권한 값을 선언 해야 합니다. 각 부분에 대 한 자세한 내용은 플랫폼 설명서를 참조 하세요. 다음 코드 조각은 패키지/기능 노드 아래에 있어야 하는 노드를 보여 줍니다. 그렇지 않으면 연결 및 채팅 차단 됩니다.
...
Xbox Live 멀티 플레이 응용 프로그램은 게임 초대를 지원 해야 합니다. 초대를 수락 로컬 사용자가 앱의 프로토콜 활성화의 숫자 형태로 도착할 (프로토콜 활성화에 대 한 자세한 정보에 대 한 플랫폼 설명서를 참조). XIM 네트워크 대역의 예약을 통해 관리 호출에 대 한 이외의 XIM을 사용 하는 앱의 `xim::extract_protocol_activation_information()` 런타임, 유니버설 Windows 플랫폼 (UWP) 응용 프로그램에서 프로토콜 활성화 처리에 도움이 되도록 메서드를 통해 정적으로 선언 해야는 이러한 "ms-xbl-멀티 플레이어" 정품 인증 시스템에 의해 지원 AppXManifest "windows.protocol" 확장 합니다. 다음 코드 조각에서와 같이 수행할 수 있습니다.
...
이만 유니버설 Windows 응용 프로그램에 필요 합니다. Xbox Live 제목 ID 확장을 선언 하 여 자동으로 등록 된 대로 Xbox One 단독 리소스 ("XDK") 응용 프로그램 프로토콜 활성화 확장을 명시적으로 선언 하지 않습니다.

## <a name="xbox-one-exclusive-resource-application-network-manifest-templates"></a>Xbox One 단독 리소스 응용 프로그램 네트워크 매니페스트 템플릿

XIM을 사용 하는 모든 Xbox One 단독 리소스 응용 프로그램 특정 소켓 설명 확인 해야 하 고 템플릿 콘텐츠가 AppXManifest의 해당 "네트워크 매니페스트" 확장에 포함 된 (네트워크 매니페스트에 대 한 자세한 내용은 플랫폼 설명서를 참조 하세요.). 앱 다른 템플릿도 있을 수 있지만 다음 코드 조각은 있어야 합니다. 그렇지 실패 XIM 네트워크에 이동:
...
XIM을 사용 하는 모든 유니버설 Windows 응용 프로그램이 특정 소켓 설명 확인 해야 하 고 템플릿 콘텐츠가 해당 "네트워크 매니페스트" 파일에 포함 된 (패키지 루트 디렉터리에 networkmanifest.xml 파일 플랫폼 설명서를 참조 자세히에서 네트워크 매니페스트). 앱 다른 템플릿도 있을 수 있지만 다음 코드 조각은 있어야 합니다. 그렇지 실패 XIM 네트워크에 이동:
