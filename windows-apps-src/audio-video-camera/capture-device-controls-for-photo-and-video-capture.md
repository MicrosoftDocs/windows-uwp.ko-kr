...
값이 설정되었을 때 이벤트가 트리거되지 않도록 [**ValueChanged**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.primitives.rangebase.valuechanged) 이벤트 처리기를 등록 취소한 후 슬라이더 컨트롤 값을 **ExposureControl**의 현재 값으로 설정합니다.
...
값이 설정되었을 때 이벤트가 트리거되지 않도록 [**ValueChanged**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.primitives.rangebase.valuechanged) 이벤트 처리기를 등록 취소한 후 슬라이더 컨트롤 값을 **ExposureCompensationControl**의 현재 값으로 설정합니다.
...
[  **Supported**](https://docs.microsoft.com/uwp/api/windows.media.devices.flashcontrol.supported) 속성을 확인하여 현재 캡처 디바이스가 **FocusControl**을 지원하는지 확인합니다. 그런 다음 [**FocusMode.Continuous**](https://docs.microsoft.com/uwp/api/Windows.Media.Devices.FocusMode) 값이 들어 있는지 보기 위해 [**SupportedFocusModes**](https://docs.microsoft.com/uwp/api/windows.media.devices.focuscontrol.supportedfocusmodes) 목록을 확인하여 연속 자동 초점이 지원되는지 보고, 지원되는 경우 연속 자동 초점 라디오 단추를 표시합니다.

[!code-cs[CAF](./code/BasicMediaCaptureWin10/cs/MainPage.ManualControls.xaml.cs#SnippetCAF)]

연속 자동 초점 라디오 단추의 [**Checked**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.primitives.togglebutton.checked) 이벤트 처리기에서 [**VideoDeviceController.FocusControl**](https://docs.microsoft.com/uwp/api/windows.media.devices.videodevicecontroller.focuscontrol) 속성을 사용하여 컨트롤의 인스턴스를 가져옵니다. 앱이 이전에 [**LockAsync**](https://docs.microsoft.com/uwp/api/windows.media.devices.focuscontrol.lockasync)를 호출하여 다른 초점 모드 중 하나를 사용하도록 설정한 경우 [**UnlockAsync**](https://docs.microsoft.com/uwp/api/windows.media.devices.focuscontrol.unlockasync)를 호출하여 컨트롤의 잠금을 해제합니다.
...
탭하여 초점 맞추기 라디오 단추에 대한 [**Checked**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.primitives.togglebutton.checked) 이벤트 처리기에서 [**VideoDeviceController.FocusControl**](https://docs.microsoft.com/uwp/api/windows.media.devices.videodevicecontroller.focuscontrol) 속성을 사용하여 컨트롤의 인스턴스를 가져옵니다. 앱이 이전에 연속 자동 초점을 사용하도록 [**UnlockAsync**](https://docs.microsoft.com/uwp/api/windows.media.devices.focuscontrol.unlockasync)를 호출한 경우 [**LockAsync**](https://docs.microsoft.com/uwp/api/windows.media.devices.focuscontrol.lockasync)를 호출하여 컨트롤을 잠근 다음 사용자가 화면을 탭하여 포커스를 변경할 때까지 기다립니다.
...
캡처 디바이스의 [**FocusControl**](https://docs.microsoft.com/uwp/api/Windows.Media.Devices.FocusControl)을 가져옵니다. 새 [**FocusSettings**](https://docs.microsoft.com/uwp/api/Windows.Media.Devices.FocusSettings) 개체를 만들고 [**Mode**](https://docs.microsoft.com/uwp/api/windows.media.devices.focuscontrol.mode) 및 [**AutoFocusRange**](https://docs.microsoft.com/uwp/api/windows.media.devices.focussettings.autofocusrange)를 원하는 값으로 설정합니다. 그전에 **FocusControl**에서 이러한 기능이 지원되는지 확인합니다. **FocusControl**에서 [**Configure**](https://docs.microsoft.com/uwp/api/windows.media.devices.focuscontrol.configure)을 호출하여 설정을 활성화하고 지정된 영역에서 초점 맞추기를 시작하라는 신호를 디바이스에 보냅니다.
...
마지막으로 **FocusControl**에서 [**FocusAsync**](https://docs.microsoft.com/uwp/api/windows.media.devices.focuscontrol.focusasync)를 호출하여 초점 지정을 시작합니다.
...
[  **Supported**](https://docs.microsoft.com/uwp/api/windows.media.devices.focuscontrol.supported) 속성을 확인하여 현재 캡처 디바이스가 **FocusControl**을 지원하는지 확인합니다. 컨트롤이 지원되는 경우 이 기능에 대한 UI를 표시하고 사용하도록 설정할 수 있습니다.
...
값이 설정되었을 때 이벤트가 트리거되지 않도록 [**ValueChanged**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.primitives.rangebase.valuechanged) 이벤트 처리기를 등록 취소한 후 슬라이더 컨트롤 값을 **FocusControl**의 현재 값으로 설정합니다.

[!code-cs[Focus](./code/BasicMediaCaptureWin10/cs/MainPage.ManualControls.xaml.cs#SnippetFocus)]

앱에서 이전에 [**UnlockAsync**](https://docs.microsoft.com/uwp/api/windows.media.devices.focuscontrol.unlockasync)에 대한 호출을 사용하여 초점을 잠금 해제한 경우 수동 포커스 라디오 단추의 **Checked** 이벤트 처리기에서 **FocusControl** 개체를 가져와서 [**LockAsync**](https://docs.microsoft.com/uwp/api/windows.media.devices.focuscontrol.lockasync)를 호출합니다.
...
값이 설정되었을 때 이벤트가 트리거되지 않도록 [**ValueChanged**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.primitives.rangebase.valuechanged) 이벤트 처리기를 등록 취소한 후 슬라이더 컨트롤 값을 **IsoSpeedControl**의 현재 값으로 설정합니다.
...
값이 설정되었을 때 이벤트가 트리거되지 않도록 [**ValueChanged**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.primitives.rangebase.valuechanged) 이벤트 처리기를 등록 취소한 후 슬라이더 컨트롤 값을 **WhiteBalanceControl**의 현재 값으로 설정합니다.
...
[  **Supported**](https://docs.microsoft.com/uwp/api/windows.media.devices.zoomcontrol.supported) 속성을 확인하여 현재 캡처 디바이스가 **ZoomControl**을 지원하는지 확인합니다. 컨트롤이 지원되는 경우 이 기능에 대한 UI를 표시하고 사용하도록 설정할 수 있습니다.
...
값이 설정되었을 때 이벤트가 트리거되지 않도록 [**ValueChanged**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.primitives.rangebase.valuechanged) 이벤트 처리기를 등록 취소한 후 슬라이더 컨트롤 값을 **ZoomControl**의 현재 값으로 설정합니다.

[!code-cs[ZoomControl](./code/BasicMediaCaptureWin10/cs/MainPage.ManualControls.xaml.cs#SnippetZoomControl)]

**ValueChanged** 이벤트 처리기에서 [**ZoomSettings**](https://docs.microsoft.com/uwp/api/Windows.Media.Devices.ZoomSettings) 클래스의 새 인스턴스를 만들어 [**Value**](https://docs.microsoft.com/uwp/api/windows.media.devices.zoomsettings.value) 속성을 확대/축소 슬라이더 컨트롤의 현재 값으로 설정합니다. **ZoomControl**의 [**SupportedModes**](https://docs.microsoft.com/uwp/api/windows.media.devices.zoomcontrol.supportedmodes) 속성이 [**ZoomTransitionMode.Smooth**](https://docs.microsoft.com/uwp/api/Windows.Media.Devices.ZoomTransitionMode)를 포함하는 경우 이는 디바이스가 확대/축소 수준에서 부드러운 전환을 지원한다는 것입니다. 이 모드는 더 나은 사용자 환경을 제공하기 때문에 일반적으로 **ZoomSettings** 개체의 [**Mode**](https://docs.microsoft.com/uwp/api/windows.media.devices.zoomsettings.mode) 속성에 이 값을 사용하고 싶을 수 있습니다.

마지막으로 **ZoomSettings** 개체를 **ZoomControl** 개체의 [**Configure**](https://docs.microsoft.com/uwp/api/windows.media.devices.zoomcontrol.configure) 메서드로 전달하여 현재 확대/축소 설정을 변경합니다.
