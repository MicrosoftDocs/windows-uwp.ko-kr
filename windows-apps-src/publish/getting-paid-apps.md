...
# 지급 받기
다음은 앱과 IAP(앱에서 바로 구매 제품) 및 Microsoft Advertising 수익에 대한 지급을 받는 데 필요한 중요한 정보입니다.
...
Windows 스토어 수수료는 IAP를 포함하여 Windows 스토어가 수금하는 모든 앱 매출에 적용됩니다.
...
> **참고** 이러한 기준 가격은 앱 내에서 제공하는 IAP에도 적용됩니다.
...
해당 결제 임계값이 충족되고 아래에 설명된 대로 지급을 보류하지 않은 경우 결제는 월 단위로 이루어집니다. 일반적으로 해당 월의 15일에 지정된 월에 대한 결제 대금을 보냅니다. 결제는 일반적으로 영업일 3~10일 내에 지급 계좌로 입금됩니다.
