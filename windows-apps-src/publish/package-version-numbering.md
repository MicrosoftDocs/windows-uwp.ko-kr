...
> [!NOTE]
> 이 항목에서는 "패키지"를 참조 하지만 별도로 언급 하지 않는 한는 동일한 규칙이 적용.msix/.appx와.msixbundle/.appxbundle 파일에 대 한 버전 번호.
...
게시 된 제출에서 UWP 패키지를 선택할 때 Microsoft Store는 항상 고객의 Windows 10 디바이스에 적용 되는 높은 버전의 패키지를 사용 합니다. 따라서 특정 장치 유형의 고객에게 제공되는 패키지에 대한 유연성 및 제어 기능이 향상됩니다. 특히, 이러한 패키지를 순서에 상관없이 제출할 수 있습니다. 이후의 각 제출에서 더 높은 버전의 패키지를 제공하지 않아도 됩니다.
...
이는 시간에 따라 앱을 진화시킬 수 있는 뛰어난 유연성을 제공합니다. 업로드 및 이전에 지원 하지 않는 Windows 10 장치에 대 한 지원을 추가 하 낮은 버전 번호를 사용 하는 새 패키지를 제출할 수, 하드웨어 또는 OS 기능을 활용 하기 위해 종속성이 엄격한 있는 상위 버전 패키지를 추가할 수 있습니다. 기본 기존 고객의 일부 또는 전부에 업데이트로 제공 되는 더 높은 버전의 패키지를 추가할 수 있습니다.
...
패키지의 복사본을 유지 하는 경우 롤백합니다 앱의 패키지 스토어 이전 Windows10 패키지로 릴리스에 문제가 발견 하면이 해야 하는 옵션을 해야 합니다. 이 방법은 문제를 해결 하는 데 시간이 걸릴 수 있는 동안 고객에 게 방편 임시입니다.
...
문제가 있는 패키지를 이미 받은 고객에 대 한 문제를 해결 하려면 새 Windows10 패키지 하면 최대한 빨리 잘못 된 패키지 보다 더 높은 버전 번호를 제출할 수 있습니다. 제출에 대한 인증 프로세스가 완료되면 모든 고객이 새 패키지(버전 번호가 더 높으므로)로 업데이트됩니다.


## <a name="version-numbering-for-windows81-and-earlier-and-windows-phone-81-packages"></a>버전 번호 Windows8.1에 대 한 () 및 Windows Phone 8.1 패키지
...
Windows8 및 Windows8.1를 대상으로 하는.appx 패키지에 대 한 아키텍처별으로 동일한 규칙이 적용: 새 제출에서 패키지의 버전 번호는 동일한 아키텍처에 대 한 스토어에 마지막으로 게시 하는 패키지의 이상 이어야 항상 합니다.

또한 Windows8.1 패키지의 버전 번호 항상 동일한 앱에 대 한 Windows8 패키지의 버전 번호 보다 클 수 있어야 합니다. 즉, 제출 하는 모든 Windows8 패키지의 버전 번호는 동일한 앱에 대 한 제출한 모든 Windows8.1 패키지의 버전 번호 보다 작아야 합니다.

> [!NOTE]
> 앱에는 또한 Windows10 패키지에, 있는 경우 Windows10 패키지의 버전 번호 Windows8, Windows8.1, 및/또는 Windows Phone 8.1 패키지 보다 높아야 합니다. 자세한 내용은 [Windows10 이전에 게시 된 앱에 대 한 추가 패키지](guidance-for-app-package-management.md#adding-packages-for-windows-10-to-a-previously-published-app)를 참조 하세요.
...
| x86, v1.0.0.0 <br> x64, v1.0.0.0            | x64, v1.0.0.1               | x86을 사용하는 고객의 경우 v1.0.0.0 <br> x64를 사용하는 고객의 경우 v1.0.0.1                 | x86 컴퓨터에서 앱을 실행하는 고객의 경우 없음 <br> x64 컴퓨터에서 앱을 실행하는 고객의 경우 v1.0.0.0이 v1.0.0.1로 업데이트됨 <br> **참고**경우 x64 버전의 앱이 실행 되는 x86 컴퓨터 앱 x64로 업데이트 되지 않습니다 버전 고객이 제거 하 고 다시 설치 하지 않는 한 합니다. |
