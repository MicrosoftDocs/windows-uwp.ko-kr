...
C + + /winrt는 Microsoft의 권장된 대체 합니다 [C + + CX](/cpp/cppcx/visual-c-language-reference-c-cx?branch=live) 언어 프로젝션 및 [Windows 런타임 c + + 템플릿 라이브러리 (WRL)](/cpp/windows/windows-runtime-cpp-template-library-wrl?branch=live). 전체 목록은 [항목 C + + WinRT](index.md#topics-about-cwinrt) 상호 운용 하 고에서 C + 이식 하는 방법에 대 한 정보를 포함 + CX 및 WRL 합니다.
...
그런 다음 만들기 및 빌드, 또는 열 C + 수 있게 됩니다 + WinRT Visual Studio에서 프로젝트를 배포 합니다. 또는 추가 하 여 기존 프로젝트를 변환할 수는 `<CppWinRTEnabled>true</CppWinRTEnabled>` 속성을 해당 `.vcxproj` 파일입니다.
