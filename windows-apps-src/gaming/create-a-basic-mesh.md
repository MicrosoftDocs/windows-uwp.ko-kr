---
기본 메시 만들기 및 표시
3-D UWP(유니버설 Windows 플랫폼) 게임에서는 대개 다각형을 사용하여 게임의 개체와 표면을 나타냅니다.
...
> **참고** 디바이스별 리소스를 설정할 때는 곱셈 결과가 현재 2-D 뷰포트 크기 매개 변수(대개 디스플레이의 픽셀 높이 및 너비에 해당)와 일치해야 하므로 일반적으로 투영 매트릭스를 선언합니다. 변경되면 그에 따라 x 및 y 좌표 값을 조정해야 합니다.
