...
GameClip 업로드 프로세스의 첫 번째 부분입니다. 비디오 캡처를 시 업로드 바로 시작 되도록 예약 되지 않은 경우에 비트, 업로드에 대 한 ID와 URI를 받을를 즉시 GameClips 서비스를 호출에 것입니다. 이 호출은 사용자 할당량 검사 및 클라이언트에 의해 업로드에 대 한 비디오 경우도 예약 해야 참조 하는 등 개인 정보, 콘텐츠 격리를 통해 다른 검사를 수행 합니다. 이 호출을 통해 긍정적인 응답을 업로드할 비디오 클립을 감수할 서비스를 나타냅니다. 업로드 하는 모든 클립 시스템에서 허용 (서비스 안내)를 통해 특정 제목을 연결 해야 합니다.
...
[InitialUploadResponse](../../json/json-initialuploadresponse.md) 또는 [ServiceErrorResponse](../../json/json-serviceerrorresponse.md) 개체 JSON 형식의 응답 될 수 있습니다.
