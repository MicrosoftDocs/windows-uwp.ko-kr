...
ms.locfileid: "57616818"
---
# <a name="get-usersxuidxuidgroupsmonikerbroadcastingcount-"></a>GET (/users/xuid({xuid})/groups/{moniker}/broadcasting/count )
지정 된 URI에 표시 되는 XUID 관련 그룹 모니커 브로드캐스팅 사용자의 수를 검색 합니다. 이러한 Uri에 대 한 도메인은 `userpresence.xboxlive.com`합니다.
...
지정 된 URI에 표시 되는 XUID 관련 그룹 모니커 브로드캐스팅 사용자의 수를 검색 합니다.
...
이 API 현재 브로드캐스트 매개 변수를 지정 된 수의 개수를 반환 합니다.
