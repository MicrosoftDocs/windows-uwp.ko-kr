...
[  **CreatePeriodicTimer**](https://docs.microsoft.com/uwp/api/windows.system.threading.threadpooltimer.createperiodictimer) 메서드를 사용하여 주기적 작업 항목을 만듭니다. 작업을 수행하는 람다를 제공하고 *period* 매개 변수를 사용하여 제출 간격을 지정합니다. 이 기간은 [**TimeSpan**](https://docs.microsoft.com/uwp/api/Windows.Foundation.TimeSpan) 구조를 사용하여 지정합니다. 기간이 경과할 때마다 작업 항목이 다시 제출되므로 작업을 완료할 만큼 기간이 긴지 확인합니다.

[**CreateTimer** ](https://docs.microsoft.com/uwp/api/windows.system.threading.threadpooltimer.createtimer) 반환 된 [ **ThreadPoolTimer** ](https://docs.microsoft.com/uwp/api/Windows.System.Threading.ThreadPoolTimer) 개체입니다. 타이머를 취소해야 하는 경우를 위해 이 개체를 저장합니다.
...
필요한 경우 [**TimerDestroyedHandler**](https://docs.microsoft.com/uwp/api/windows.system.threading.timerdestroyedhandler)를 사용하여 주기적 타이머의 취소를 처리할 수 있습니다. [  **CreatePeriodicTimer**](https://docs.microsoft.com/uwp/api/windows.system.threading.threadpooltimer.createperiodictimer) 오버로드를 사용하여 주기적 작업 항목의 취소를 처리하는 추가 람다를 제공합니다.
...
필요한 경우 [**Cancel**](https://docs.microsoft.com/uwp/api/windows.system.threading.threadpooltimer.cancel) 메서드를 호출하여 주기적 작업 항목이 반복되지 않도록 합니다. 주기적 타이머를 취소할 때 작업 항목이 실행 중이면 작업 항목을 완료할 수 있습니다. 주기적 작업 항목의 모든 인스턴스가 완료되면 [**TimerDestroyedHandler**](https://docs.microsoft.com/uwp/api/windows.system.threading.timerdestroyedhandler)(제공된 경우)가 호출됩니다.
