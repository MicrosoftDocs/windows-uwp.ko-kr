...
다음을 수행할 수 있습니다.
  ...
  2. 사용자의 소셜 그래프, 텍스트 파일 또는 Xbox 개발자 플랫폼 csv에서 팔로 워의 목록을 추가합니다
...
Xbox Live 계정 도구는 소매 계정을 관리 하는 데 사용할 확실히 수 있지만 일부 기능이 지원 됩니다. 예를 들어 소매 사용자의 평판을 변경할 수 없습니다.
...
수행 하려는 XUIDs 목록이 있는 경우 텍스트 파일에 복사 합니다. "수행" 탭을 찾아서 선택 "목록 가져오기" 파일을 선택 합니다. 목록 보기에는 XUIDs 채워야 합니다. "변경 내용 커밋" 클릭 사용자를 따라야 합니다.
...
## <a name="how-do-i-change-my-dev-accounts-repuation"></a>내 개발자 계정의 신뢰도 변경 하려면 어떻게 해야 합니까?
이동 "평판" 탭 기본값을 하 고 요청을 제출 하려면 "변경 내용 커밋" 단추를 눌러 선택 합니다. 요청이 성공 하면 값이 변경 평판 상태를 볼 수 있습니다.

## <a name="what-do-the-values-in-the-reputation-tab-mean"></a>값의 평판 탭에는 무엇을 의미 합니까?
평판 세 개의 하위 평판에서 계산 되는 전체: Fairplay (멀티 플레이 사항) 사용자가 생성 (비디오 클립 등), 콘텐츠 및 통신 (메시지 및 음성). 각 범주에 대 한 원시 값 까지입니다 0도에서 75, 높은 이므로 사용자의 평판 더 나은. OverallStatIsBad는 사용자에 게 "방지 Me" 신뢰도 알려 줍니다.

## <a name="whats-the-black-area-at-the-bottom"></a>맨 아래에 검은색 영역 란?
일을 유지 하기 위해 것 유용한 디버그 출력 UI에서 인쇄 생각 합니다. 영역 내용을 설명 도구 최대 되며 모든 오류를 인쇄에 실행 됩니다.

## <a name="wheres-my-gamerpic"></a>Gamerpic 내 어디 입니까?
여기서는 일부 개발자 계정에는 계정 생성 시 자동으로 생성 된 gamerpic 가져올 버그를 인식 합니다.

## <a name="why-are-things-happening-so-slowly"></a>왜 작업 발생 하므로 천천히?
(예: 팔로 워 추가) 일괄 처리 작업에 대 한 도구를 자동으로 거 대 한 요청 크기를 방지 하기 위해 일괄 처리를 수행 합니다.

## <a name="how-do-i-run-xbox-live-account-tool"></a>Xbox Live 계정 도구를 실행 하려면 어떻게 해야 합니까?
