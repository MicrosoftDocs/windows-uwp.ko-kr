...
이외에도, 피어는 피어 지원에서 발생시키는 예외와 관련하여 비교적 보수적이어야 합니다. 대부분의 클라이언트는 피어에서 발생한 예외를 처리하여 이를 사용자가 클라이언트 조작 시 결정할 수 있는 작동 가능한 선택으로 전환할 수 없습니다. 따라서 때로는 피어 구현 내에서 다시 발생시키지 않고 예외를 catch하는 No-op이 피어가 시도하는 일이 작동하지 않을 때마다 예외를 발생시키는 것보다 나은 전략입니다. 대부분의 UI 자동화 클라이언트는 관리 코드로 작성되지 않았다는 점도 고려하세요. 대부분은 COM으로 작성 되며, 피어에 대 한 액세스를 종료 하는 UI 자동화 클라이언트 메서드를 호출할 때마다 **HRESULT** 의 **\_OK** 를 확인 하는 것입니다.
