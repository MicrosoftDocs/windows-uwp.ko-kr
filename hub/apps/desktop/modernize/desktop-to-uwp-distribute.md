...
[MSIX 패키지에서 데스크톱 앱을 패키징하](/windows/msix/desktop/desktop-to-uwp-root)는 경우 패키지 응용 프로그램을 Microsoft Store에 게시 하거나 하나 이상의 장치에 테스트용으로 로드 수 있습니다.

> [!NOTE]
> 사용자를 패키지 된 응용 프로그램으로 전환 하는 방법에 대 한 계획이 있나요? 앱을 배포하기 전에 몇 가지 아이디어를 얻고 싶다면 이 가이드의 [패키지된 앱으로 사용자 전환](#transition-users) 섹션을 참조하세요.
...
>[!IMPORTANT]
> 응용 프로그램을 Microsoft Store에 게시할 계획인 경우 Windows 10 S를 실행 하는 장치에서 응용 프로그램이 올바르게 작동 하는지 확인 합니다. 저장소 요구 사항입니다. [Windows 10 S용 Windows 앱 테스트](/windows/msix/desktop/desktop-to-uwp-test-windows-s)를 참조하세요.

<a id="side-load" />

## <a name="distribute-your-application-without-placing-it-onto-the-microsoft-store"></a>응용 프로그램을 Microsoft Store에 배치 하지 않고 배포

저장소를 사용 하지 않고 응용 프로그램을 배포 하는 경우 하나 이상의 장치에 앱을 수동으로 배포할 수 있습니다.

배포 경험에 대한 강력한 제어를 원하거나 Microsoft Store 인증 프로세스에 참여하고 싶지 않은 경우에 적합한 방법입니다.

응용 프로그램을 저장소에 배치 하지 않고 다른 장치에 배포 하려면 인증서를 얻은 다음 해당 인증서를 사용 하 여 응용 프로그램에 서명 하 고 응용 프로그램을 해당 장치에 테스트용으로 로드 해야 합니다.
...
응용 프로그램을 Windows 10 S를 실행 하는 장치에 배포 하려는 경우 응용 프로그램을 해당 장치에 배포 하기 전에 스토어 제출 프로세스를 진행 해야 하므로 Microsoft Store에서 응용 프로그램에 서명 해야 합니다.
...
인증서를 사용 하 여 응용 프로그램에 서명 하려면 [SignTool를 사용 하 여 응용 프로그램 패키지 서명](/windows/msix/package/sign-app-package-using-signtool)을 참조 하세요.

응용 프로그램을 다른 장치 [에 테스트용으로 로드 Windows 10의 테스트용으로 로드 LOB apps](/windows/application-management/sideload-apps-in-windows-10)를 참조 하세요.
...
* 패키지 응용 프로그램을 일련의 파일 형식으로 연결 합니다.
* 패키지 된 응용 프로그램이 기본적으로 특정 형식의 파일을 열도록 설정 합니다.

전체 확장 목록과 사용 방법에 대한 지침은 [사용자를 앱으로 전환](desktop-to-uwp-extensions.md#transition-users-to-your-app)을 참조하세요.

또한 다음 작업을 수행 하는 패키지 된 응용 프로그램에 코드를 추가 하는 것이 좋습니다.

* 데스크톱 응용 프로그램에 연결 된 사용자 데이터를 패키지 된 앱의 적절 한 폴더 위치로 마이그레이션합니다.
...
사용자 데이터를 마이그레이션하는 코드를 추가 하려는 경우 응용 프로그램이 처음 시작 될 때만 해당 코드를 실행 하는 것이 가장 좋습니다. 사용자 데이터를 마이그레이션하기 전에 대화 상자를 표시하여 사용자에게 마이그레이션 과정과 이를 권장하는 이유, 기존 데이터에 미치는 영향 등을 설명합니다.
...
사용자 데스크톱 응용 프로그램은 먼저 사용자에 게 권한을 요청 하지 않고 제거 하는 것이 좋습니다. 해당 사용 권한을 요청하는 대화 상자를 표시합니다. 사용자가 데스크톱 버전의 앱을 제거하지 않을 수 있습니다. 이 경우 데스크톱 응용 프로그램의 사용을 차단할지 아니면 두 앱의 side-by-side 사용을 지원 하는지 여부를 결정 해야 합니다.
