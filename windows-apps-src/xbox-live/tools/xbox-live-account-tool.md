...
관리 하 고 올바른 샌드박스를 지정 하려는 사용자의 자격 증명을 해야 합니다. 개발자 계정에 액세스할 수 샌드박스, 그렇지 않으면 로그인 실패할 수 있는지 확인 합니다. 도구는 개발자 계정의 샌드박스를 사용 하 여 염두에 두고 설계 되었습니다.

## <a name="can-i-use-a-retail-account-or-does-it-have-to-be-a-sandboxed-account"></a>소매 계정을 사용할 수 또는 샌드박스가 적용 된 계정으로 있습니까?
Xbox Live 계정 도구 소매 계정을 관리 하는 데 사용할 만료할 수 있지만 일부 기능은 지원 합니다. 예를 들어 소매 사용자의 평판을 변경할 수 없습니다.
...
평판 세 개의 하위 평판에서 계산 되는 전체: Fairplay (멀티 플레이 규정) 사용자 생성 (비디오 클립 및 등), 콘텐츠 및 통신 (메시지 및 음성). 각 범주에 대 한 원시 값 까지입니다 0에서 75 높은 이므로 사용자의 평판 더 나은. 사용자에 게 "방지 Me" 신뢰도 OverallStatIsBad에서 알 수 있습니다.
...
여기서는 일부 개발자 계정 계정 생성 시 자동으로 생성 된 gamerpic를 받지 못한 버그를 인식 합니다.

## <a name="why-are-things-happening-so-slowly"></a>이유 사항이 발생 하므로 천천히?
일괄 처리 작업 (예: 팔로 워 추가)에 대 한 도구를 자동으로 거 대 한 요청 크기를 방지 하기 위해 일괄 처리를 수행 합니다.
...
모든 기능을 요청을 사용 하 여 Microsoft 담당자에 게 말하기 및 수행할 수 있는 것으로 표시 됩니다.
