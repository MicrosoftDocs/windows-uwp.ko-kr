...
지역화 된 언어 데이터가 포함 된 앱을 설치할 때 처음 여러 언어에 대 한 리소스를 포함 하는 경우에의 기본 언어를 앱에 사용할 수 있는지를 발견할 수 있습니다. 설치 프로세스는 현재 언어 및 문화 장치의 일치 하는 언어 리소스 설치만 하도록 최적화 되어 방식 때문에 발생 합니다. 즉, 디바이스가 en에 대해 구성 된 경우-en만 앱을 설치 하면 우리-우리 언어 리소스 설치 됩니다. 운영 체제의 기본 언어를 변경 하는 경우 여전히 표시 en-us 리소스 앱에 대해 설치 된 유일한 언어에 있기 때문입니다. 이 이번에 처음 설치한 후 앱에 대 한 지원을 추가 언어를 설치 하려면 방법은 있습니다. 

설치 후 모든 언어 리소스를 사용할 수 있는지 확인 하려는 경우 구성 파일을 설치 하는 동안 특정 리소스에 필요한는 지정 된 앱 패키지에 대 한 cerate를 수 있습니다. 이 구성 파일에서 리소스의 언어 리소스를 포함 하 여 설치를 요구할 수 있습니다. 보장 하는 리소스에 대 한 자세한 정보는 설치에 대 한이 문서를 참조 하십시오이: [리소스가 장치에 필요한 지 여부에 관계 없이 장치에 설치 되어 있는지 확인 합니다](https://docs.microsoft.com/en-us/previous-versions/dn482043(v=vs.140)) .
 
이 최적화 된 설치 기능이 패키징 하는 동안 앱에 대 한 appxbundle을 생성할 때에 자동으로 활성화 됩니다. 모든 리소스가 설치 되어 있는지 확인 하려면 필요에 따라 앱을 패키지 하기 appxbundle 생성을 비활성화할 수 있습니다. 그러나 권장 하지 않음 있으므로 앱의 설치 시간을 높일 수 있습니다. 대신 이전 단락에 따라 패키징 구성 파일을 만드는 하 고만 설치를 계속 최적화 불필요 한 리소스를 최적화할 수 있도록 필요한 리소스를 요청 해야 합니다. 
 
Appxbundle 생성을 해제 하 고 "앱 번들 생성" 특성 "안 함"으로 설정 하 여 패키지 된 모든 리소스를 포함 수 있습니다. 
