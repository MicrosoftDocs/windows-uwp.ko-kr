...
CPU (이러한 설정에 사용 가능한 경우 사용할 수 있습니다 BIOS)

-   하드웨어 지원 가상화 ([호환성 검사](https://www.microsoft.com/download/details.aspx?id=592)).
...
Windows 10 Mobile 용 Microsoft 에뮬레이터는 Visual Studio 2015 이상; 필요 이전 버전의 Visual Studio를 사용 하 여 이전 버전과 호환 되는 없습니다.
...
에뮬레이터에서 앱을 실행 하려면 단순히에서 에뮬레이트된 장치를 선택 합니다 **실행** Visual Studio에서 드롭 다운 메뉴.
...
> **중요**  tapper 도구를 처음 시작할 때 하면 Windows 방화벽을 확인 합니다. 3가지 확인란을 모두 선택하고 방화벽을 통한 도구를 허용해야 합니다. 그러지 않으면 도구가 자동으로 작동하지 않습니다.
...
> **참고**  휴대폰에서 사용 되므로 SD 카드 FAT32 파일 시스템으로 포맷, 32GB 최대 크기입니다.
...
**사용 하도록 설정 하 고 프레임 속도 카운터를 사용 하지 않도록 설정**
...
### <a name="error-message-failed-while-removing-virtual-ethernet-switch"></a>오류 메시지: "가상 이더넷 스위치를 제거 하는 동안 실패"

새 Windows 10 비행으로 업데이트 한 후 같은 특정 상황에서 에뮬레이터를 사용 하 여 연결 된 가상 네트워크 스위치를 사용자 인터페이스를 통해 삭제할 수 없는 상태로 가져올 수 있습니다.
...
**참고**  이 명령에는 에뮬레이터를 사용 하 여 연관 된 모든 네트워킹 장치 삭제 됩니다. 컴퓨터가 다시 시작되면 모든 하드웨어 네트워킹 디바이스가 자동으로 검색됩니다.
 
### <a name="unable-to-launch-the-emulators"></a>에뮬레이터를 시작할 수 없음

Microsoft 에뮬레이터는 모든 VM, diff 디스크 및 에뮬레이터 특정 네트워크 스위치를 삭제하는 도구인 XDECleanup.exe를 포함하며, 에뮬레이터(XDE) 이진 파일과 함께 제공됩니다. 잘못된 상태로 전환될 경우 이 도구를 사용하여 에뮬레이터 VM을 정리해야 합니다. 관리자 명령 프롬프트에서 도구를 실행 합니다.`C:\Program Files (x86)\Microsoft XDE\<version>\XdeCleanup.exe`

> **참고**  XDECleanup.exe 모든 에뮬레이터를 삭제 하며 특정 Hyper-v Vm에도 VM 검사점을 삭제 하거나 저장 된 상태입니다.
...
그런 다음 현재 릴리스에서 에뮬레이터용 VHD 파일을 수동으로 삭제해야 합니다. 기본 경로를 에뮬레이터를 설치한 경우 VHD 파일은 c:\\프로그램 파일 (x86)\\Windows 키트\\10\\에뮬레이션\\Mobile\\ <version> \\ flash.vhd 합니다.
...
1. 레지스트리 편집기를 시작합니다.
2. 존재 하지 않는 경우 다음 레지스트리 하위 키를 만듭니다. HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Microsoft\Xde\10.0
...
**참고:** 이 레지스트리 값으로 설정한 후 Visual Studio에서 시작 하는 구성에 대 한 Hyper-v 관리자에서 가상 머신을 삭제 하 고 그래픽 소프트웨어 렌더링을 사용 하 여 에뮬레이터를 다시 시작 합니다.
