...
리소스 이름을 분할 하는 경우 (포함 된 "." 문자), 다음 바꾸기 점선 슬래시 ("/")를 사용 하 여 리소스 이름에서 문자입니다. 속성 식별자는 예를 들어 점; 포함 따라서이 substition 코드에서이 중에서 로드 하기 위해 수행 해야 합니다.
...
리소스 파일 이름을 분리 된 경우 (포함 된 "." 문자)를 참조 하는 경우 이름에 점이 둡니다. 리소스 이름에 대 한 것 처럼 슬래시 ("/") 문자로 점을 대체 **하지 않습니다** .
