...
description: 이 백서 Windows10는 업무에 중요 한 핵심 작업을 담당 하는 기존 코드를 사용 하 여 쉽게 터치.NET 앱에서 지 원하는 기업 대상 기능에 설명 합니다.
ms.date: 02/08/2017
ms.topic: article
...
>**참고**설정에서 테스트용 로드를 켤 수 있지만&gt; 업데이트 및 보안-&gt; 개발자를 위한 합니다.
...
한 번 참조**winmd**만들어집니다 (프로젝트의 대상 폴더 아래 "참조" 폴더)는 손으로 (복사) 각 사용 하는 테스트용으로 로드 된 응용 프로그램 프로젝트를 수행 하 고 참조 합니다. 이 내용은 다음 섹션에서 자세히 설명하겠습니다. 위의 빌드 규칙에 포함 된 프로젝트 구조 되도록 구현 및 참조**winmd**혼동을 피하기 위해 빌드 계층 구조에서 명확 하 게 분리 된 디렉터리에 있습니다.
...
>**참고** 반드시**참조 winmd**를 사용 하 여테스트용으로 로드 되는 응용 프로그램의 합니다. **구현 winmd**를 통해 실수로 수행 하는 경우를 테스트용으로 로드 하는 앱 디렉터리로 참조를 될 가능성이 받게 됩니다 "IStringable을 찾을 수 없습니다"와 관련 된 오류가 발생 합니다. 이것은 확실 하는 잘못 된**winmd**참조 되었습니다. IPC 서버 앱 (다음 섹션에서 자세히 설명) 신중 하 게의 사후 빌드 규칙 분리 이러한 두**winmd**별도 디렉터리에 있습니다.
...
이전 두 섹션에서는 클래스 선언 및 참조**winmd**전송테스트용으로 로드 된 응용 프로그램 프로젝트에 있습니다. 데스크톱 구성 요소에서 남은 작업의 상당 부분이 구현과 관련됩니다. 데스크톱 구성 요소에서 중요한 점은 데스크톱 코드를 호출할 수 있는(일반적으로 기존 코드 자산을 재활용하기 위해) 기능이므로, 프로젝트를 특별한 방식으로 구성해야 합니다.
...
앱을 설치 하려면 복사 구현**winmd**연결된 된 테스트용으로 로드 된 응용 프로그램의 매니페스트에 지정 된 올바른 디렉터리를: <ActivatableClassAttribute>의 값 = "path"입니다. 관련된 지원 파일 및 프록시/스텁 dll도 복사합니다(이에 대한 자세한 내용은 아래에서 설명). 구현**winmd**복사 하지 않으면를 서버 디렉터리 위치 하면 테스트용으로 로드 된 모든 RuntimeClass 새 항목에 대 한 응용 프로그램의 호출을 "클래스가 등록 되지 않았습니다." 오류를 throw 합니다. 프록시/스텁을 설치하지 않거나 등록하지 않으면 모든 호출이 실패하고 값이 반환되지 않습니다. 이 두 번째 오류는 자주**없습니다**예외와 연결 합니다.
...
**6 단계:** "MyWinRTProxy" 프로젝트에 파일 추가 **(프로젝트 > 새 항목 추가 > 코드 > 모듈 정의 파일**) 콘텐츠를 업데이트 합니다.
...
-   반환*목록<T>* 있는*T*개체는 비동기 작업 또는 속성 반입에서 이면 크로스 프로세스 활성도 높아집니다 반입 됩니다. 예를 들어, 반환는*목록&lt;사용자&gt;* 개체입니다. 각 반복 단계가 크로스 프로세스 호출이 됩니다. 각*사용자*반환 되는 개체는 프록시 및 메서드를 호출할 때마다 나타내는 또는 해당 개별 개체에 속성 하면 크로스 프로세스 호출이 발생 합니다. 따라서 "순수"*목록&lt;사용자&gt;* 개체는*수*큰 많은 느린 호출 하면 합니다. 배열의 구조체를 대량으로 전송하면 더 높은 성능을 보입니다. 예:
...
모든 성능 고려 사항의 경우처럼, 측정과 테스트가 중요합니다. 원칙적으로, 걸리는 시간을 결정하기 위해 다양한 작업에 원격 분석을 삽입해야 합니다. 에 걸쳐 측정을 고려해 야 합니다: 예를 들어는 실제로 걸리는 모든*사용자*를 사용 하도록테스트용으로 로드 되는 응용 프로그램의 특정 쿼리가 개체?
