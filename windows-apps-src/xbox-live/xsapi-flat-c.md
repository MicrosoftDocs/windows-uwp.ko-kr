...
아직 C API 모든 XSAPI 기능을 다루지 않습니다 하지만 작업 중인 추가 기능. 모든 3 API 계층, C, c + + 및 WinRT는 계속 지원 하 고 추가 기능 시간이 지남에 따라 추가 해야 합니다.
...
C Api 및 c + + Api는 실제로 씬 래퍼는 공통 핵심 진입점 마찬가지로 기능을 변경 해야 하므로. 그러나 C Api만 이용 하는 사용자 지정 메모리 및 스레드 관리 기능 걸릴 수 있습니다.
