...
3. PC의 웹 브라우저에서 다음으로 이동합니다. `https://<YOUR_HOLOLENS_IP_ADDRESS>`
    - 브라우저에 "이 웹 사이트의 보안 인증서에 문제가 있습니다"라는 메시지가 표시됩니다. 이 오류는 디바이스 포털에 문제가 된 인증서가 테스트 인증서이기 때문에 발생합니다. 지금은 이 인증서 오류를 무시하고 계속 진행할 수 있습니다.
...
2. 요청 핀을 클릭 또는 탭하고 HoloLens 디스플레이를 확인하여 생성된 PIN을 가져옵니다.
...
- **기본 설정** 섹션에는 다음 설정이 포함되어 있습니다.
    ...
    - **절전 설정**: 디바이스가 전원에 연결된 경우 및 배터리에 연결된 경우 절전 모드로 전환되기 전에 대기할 시간을 설정합니다.
...
- **기록**: 기록할 스트림을 선택하고 기록에 대한 이름을 지정하고 기록을 클릭 또는 탭하여 기록을 시작합니다. HoloLens를 사용하여 작업을 수행한 다음 중지를 클릭하여 PC에 .xef 파일로 데이터를 저장합니다. 이 파일을 HoloLens 에뮬레이터 또는 디바이스에 로드할 수 있습니다.
