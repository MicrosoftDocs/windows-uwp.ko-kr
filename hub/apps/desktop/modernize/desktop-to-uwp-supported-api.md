...
[DualApiPartition](/uwp/api/Windows.Foundation.Metadata.DualApiPartitionAttribute)이 있는 API는 패키지 ID가 있는 데스크톱 앱과 패키지 ID가 없는 데스크톱 앱을 포함하여 모든 데스크톱 앱에서 지원됩니다. 데스크톱 애플리케이션에서 호출하려는 API가 지원되는지 확인하려면 [WinRT 네임스페이스에 대한 참조 설명서](/uwp/api/)에서 관심이 있는 클래스 또는 멤버에 대한 참조 문서를 찾아보세요. 그런 다음, 참조 문서의 **특성** 섹션에 [DualApiPartition](/uwp/api/Windows.Foundation.Metadata.DualApiPartitionAttribute) 특성이 포함되어 있는지 확인합니다.
