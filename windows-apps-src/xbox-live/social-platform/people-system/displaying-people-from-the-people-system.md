...
ms.locfileid: "5971264"
---
# <a name="display-people-from-the-people-system"></a>사용자가 시스템에서 사용자를 표시 합니다.

Xbox Live 전체 서비스 해당 서비스를 소유한 데이터만 돌아가고; 사용자에 게 XUID 참조만 반환 예를 들어 사용자가 서비스만 소유 하 고 사용자의 사용자 목록과 각 해당 XUIDs (예: 즐겨찾기 상태)에 대 한 몇 가지 기본적인 정보에 있는 XUIDs를 반환 합니다. 현재 상태 서비스 XUIDs의 온라인 상태 정보에 대 한 데이터를 소유합니다. 순위표 서비스 XUIDs의 목록에 대 한 순위 정보를 소유합니다. 프로필 서비스 이외의 모든 서비스에서 표시 이름 및 게이머 정보를 반환 하지 하 고 따라서 환경에서 사용자 목록을 렌더링 하는 데 필요한는 여러 서비스를 호출 합니다.

서비스에 대 한 일반 호출 패턴 Api는 하나의 왕복 먼저 XUIDs 목록을 가장 필터링 할 수 있는 서비스에서 얻을 수 있도록 또는 목록 정렬 다음 동시 왕복 호출 한 추가 메타 데이터를 가져오는 데 필요한 다른 서비스에 필요 각 XIUD에 대 한. 이미지의 경우 호출에 대 한 세 번째 왕복 이미지 Url에서 이미지를 얻으려면 필요할 수 있습니다.

사용자의 사용자 목록에 대 한 데이터를 얻는 데 필요한 왕복 수를 줄이기 위해 사람들이 *모니커* 관련 서비스를 도입 됩니다. 이 새로운 기능 호출자를 서비스 사용자 서비스에서 사용자의 사람 목록이 받아야 하며 다음 XUIDs 집합을 사용 하 여 반환 되는 범위를 지정 하는 기본 서비스 추상적으로 표현할 수 있습니다.

다음은 제목 사용자에 게 관련 된 서비스에서 데이터를 가져오는 방법을 설명 하는 몇 가지 예제 호출 흐름 시나리오입니다.
...
## <a name="goal"></a>목표
현재 사용자의 사용자가 목록에 있는 온라인 사용자 풍부한 목록을 렌더링
...
1. 병렬로:
 ...
 1. 사용자, 사용자의 즐겨찾기가 되어 있는지 알아보려면 XUIDs 목록에 전달 하 여 호출 합니다.
1. 프로필을 호출 합니다.
...
## <a name="call-flow"></a>호출 흐름
1. 특정 순위표를 가져오려면 XUIDs, 등급, 순위표 및 점수를 호출 합니다.
1. 병렬로:
...
## <a name="call-flow"></a>호출 흐름
1. 사용자의 사용자 목록으로 제한 하는 특정 순위표에 대 한 XUIDs, 순위를 가져오려면 사용자 모니커 전달 하 여, 순위표 및 점수를 호출 합니다.
1. 병렬로:
 1. 표시 이름을 가져올 및 각 URL 그림 XUIDs 목록에 전달 하 여 프로필을 호출 합니다.
 1. 사용자, 사용자의 즐겨찾기가 되어 있는지 알아보려면 XUIDs 목록에 전달 하 여 호출 합니다.
