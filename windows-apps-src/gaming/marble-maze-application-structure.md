...
-   초기화 단계에서 게임에 사용되는 런타임 및 라이브러리 구성 요소를 설정합니다. 게임 관련 리소스도 로드합니다.
...
| BasicTimer.h, BasicTimer.cpp               | 쉽게 총 시간과 경과 시간을 구할 수 있는 방법을 제공하는 **BasicTimer** 클래스를 정의합니다.                                                                                         |
...
| DirectXApp.h, DirectXApp.cpp               | 앱의 보기(창, 스레드 및 이벤트)를 캡슐화하는 **DirectXApp** 및 **DirectXAppSource** 클래스를 정의합니다.                                                     |
| DirectXBase.h, DirectXBase.cpp             | 많은 DirectX UWP 앱에 공통적으로 적용되는 인프라를 제공하는 **DirectXBase** 클래스를 정의합니다.                                                                            |
| DirectXSample.h                            | DirectX UWP 앱에서 사용할 수 있는 유틸리티 기능을 정의합니다.                                                                                                                      |
| LoadScreen.h, LoadScreen.cpp               | 앱을 초기화하는 동안 대기 화면을 표시하는 **LoadScreen** 클래스를 정의합니다.                                                                                         |
| MarbleMaze.h, MarbleMaze.cpp               | 게임 관련 리소스를 관리하고 대부분의 게임 논리를 정의하는 **MarbleMaze** 클래스를 정의합니다.                                                                          |
...
**AppMain** 클래스는 또한 지연된 게임용 리소스의 로딩을 시작합니다. 자세한 내용은 다음 섹션을 참조하세요. **DirectXPage** 생성자는 이벤트 처리기를 설정하고, **DeviceResources** 클래스를 만들고, **AppMain** 클래스를 만듭니다.
...
비동기 자산 로드는 **AppMain::Load** 메서드로 시작됩니다. 이 메서드는 [**task Class (Concurrency Runtime)**](https://msdn.microsoft.com/library/windows/apps/hh750113.aspx) 클래스를 사용하여 백그라운드에서 게임 자산을 로드합니다.
...
**MarbleMaze** 클래스는 비동기 로드가 완료되었음을 나타내는 *m\_deferredResourcesReady* 플래그를 정의합니다. **MarbleMaze::LoadDeferredResources** 메서드는 게임 리소스를 로드한 다음 이 플래그를 설정합니다. 앱의 업데이트(**MarbleMaze::Update**) 및 렌더링(**MarbleMaze::Render**) 단계에서 이 플래그를 검사합니다. 이 플래그가 설정된 경우 게임이 정상적으로 계속됩니다. 플래그가 설정되지 않은 경우 게임에 대기 화면이 표시됩니다.
...
게임 관련 코드에서 뷰 및 창 코드를 분리하기 위해 업데이트 및 렌더링 호출을 **MarbleMaze** 개체에 전달하는 **DirectXApp::Run** 메서드를 구현했습니다. **DirectPage::OnRendering** 메서드는 애니메이션 및 물리학 시뮬레이션에 사용되는 게임 타이머도 정의합니다.

다음 예제에서는 주 게임 루프가 포함된 **DirectPage::OnRendering** 메서드를 보여 줍니다. 게임 루프는 총 시간 및 프레임 시간 변수를 업데이트하고, 장면을 업데이트 및 렌더링합니다. 따라서 콘텐츠는 창이 보일 때에만 렌더링됩니다.
...
예를 들어 **MainMenu** 상태는 주 메뉴가 표시되고 게임이 활성화되지 않은 상태를 정의합니다. 반대로, **InGameActive** 상태는 게임이 활성화되고 메뉴가 표시되지 않는 상태를 정의합니다. **MarbleMaze** 클래스는 활성 게임 상태를 저장할 **m\_gameState** 멤버 변수를 정의합니다.

**MarbleMaze::Update** 및 **MarbleMaze::Render** 메서드는 switch 문을 사용하여 현재 상태에 대한 논리를 수행합니다. 다음 예제에서는 이 switch 문이 **MarbleMaze::Update** 메서드에서 어떻게 표시되는지를 보여 줍니다(구조를 설명하기 위해 세부 정보는 제거됨).
...
일시 중단 및 다시 시작을 지원하기 위해 Marble Maze는 **PersistentState** 클래스를 정의합니다. PersistentState.h 및 PersistentState.cpp를 참조하세요. 이 클래스는 [**Windows::Foundation::Collections::IPropertySet**](https://msdn.microsoft.com/library/windows/apps/br226054) 인터페이스를 사용하여 속성을 읽고 씁니다. **PersistentState** 클래스는 백업 저장소에서 기본 데이터 형식(**bool**, **int**, **float**, [**XMFLOAT3**](https://msdn.microsoft.com/library/windows/desktop/ee419475) 및 [**Platform::String**](https://msdn.microsoft.com/library/windows/apps/hh755812.aspx))을 읽고 쓰는 메서드를 제공합니다.
...
**MarbleMaze** 클래스는 **PersistentState** 개체를 저장합니다. **MarbleMaze** 생성자는 이 개체를 초기화하고 로컬 응용 프로그램 데이터 저장소를 백업 데이터 저장소로 제공합니다.
...
Marble Maze는 구슬이 검사점 또는 목표를 통과할 때(**MarbleMaze::Update** 메서드) 및 창이 포커스를 잃을 때(**MarbleMaze::OnFocusChange** 메서드) 상태를 저장합니다. 게임에 많은 상태 데이터가 있는 경우 몇 초 내에 일시 중단 알림에 응답해야 하므로 수시로 상태를 영구적 저장소에 유사한 방식으로 저장하는 것이 좋습니다. 이렇게 하면 앱이 일시 중단 알림을 수신할 때 변경된 상태 데이터만 저장하면 됩니다.

일시 중단 및 다시 시작 알림에 응답하기 위해 **DirectXPage** 클래스는 일시 중단 및 다시 시작 시 호출되는 **SaveInternalState** 및 **LoadInternalState** 메서드를 정의합니다. **MarbleMaze::OnSuspending** 메서드는 일시 중단 이벤트를 처리하고 **MarbleMaze::OnResuming** 메서드는 다시 시작 이벤트를 처리합니다.

**MarbleMaze::OnSuspending** 메서드는 게임 상태를 저장하고 오디오를 일시 중단합니다.
...
**MarbleMaze::SaveState** 메서드는 구슬의 현재 위치 및 속도, 최근 검사점, 최고 점수 테이블 등의 게임 상태 값을 저장합니다.
...
다시 시작을 지원하기 위해 시작 중에 호출되는 **MarbleMaze::Initialize** 메서드는 **MarbleMaze::LoadState** 메서드를 호출합니다. **MarbleMaze::LoadState** 메서드는 상태를 읽고 게임 개체에 적용합니다. 또한 이 메서드는 게임이 일시 중지된 경우 현재 게임 상태를 일시 중지됨으로 설정하고, 게임이 일시 중단된 경우 활성으로 설정합니다. 사용자가 예기치 않은 작업에 놀라지 않도록 게임을 일시 중지합니다. 또한 게임을 일시 중단할 때 게임이 플레이 상태가 아닌 경우 주 메뉴로 이동됩니다.
