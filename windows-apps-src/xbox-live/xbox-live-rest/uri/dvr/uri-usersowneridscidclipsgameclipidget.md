...
클립에 대 한 쿼리를 모든 데이터는 메타 데이터에서 반환 된 **GameClip** 개체에서 사용할 수 있습니다. 이 API 통해 단일 게임 클립을 가져오려면 **XUID**, **ServiceConfigId**, **GameClipId** 및 **SandboxId** 요청의 클레임에 필요 합니다. 게임 클립 적용 되도록 지정 또는 사용자를 특정 게임 클립을 가져올 권한이 없는 확인 검사 콘텐츠 격리 또는 개인 정보, API 404 (찾을 수 없음)의 HTTP 상태 코드를 반환 합니다.
...
> [!NOTE] 
> URI 새로 고침 40 30 초 후이 요청은 최대 걸릴 수 있습니다. URI 만료 되 면 경우 스트리밍 작업에 즉시 사용 하려고 시도 하면 IIS 부드러운 스트리밍 서버에서 500 HTTP 상태 코드를 발생 합니다. 이 검증 하는 방법을 노력 하 고이 해당 작업 진행 됨에 따라 그에 따라 업데이트 됩니다. 
