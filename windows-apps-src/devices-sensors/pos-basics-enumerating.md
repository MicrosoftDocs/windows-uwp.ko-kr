...
[선택 기가](https://docs.microsoft.com/uwp/api/windows.devices.pointofservice.posconnectiontypes) 값을 매개 변수로 사용 하는 **GetDeviceSelector** 메서드를 사용 하 여 제한할 수 있습니다 선택기를 열거 로컬, 네트워크, 또는 Bluetooth 연결 된 POS 디바이스 쿼리가 완료 하는 데 걸리는 시간이 줄어듭니다.  아래 샘플 로컬로 지 원하는 선택기를 정의 하려면이 메서드를 사용 하는 연결 POS 프린터 보여 줍니다.
