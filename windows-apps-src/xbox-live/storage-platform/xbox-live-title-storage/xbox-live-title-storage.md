...
|--------------------|--------------------|---------|--------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------|---------------------------------------------|
| 신뢰할 수 있는 플랫폼   | 사용자 당 256MB | 사용자 당 64 MB    | 사용자 데이터와 같은 저장 된 게임이 나 재생/일시 중지/다시 시작에 대 한 게임 상태. 플랫폼 제한 사항과 함께 있지만 더 안전 합니다. | 모든 플랫폼 읽을 수는 있지만 Xbox One, Xbox 360 또는 Windows Phone 쓸 수 있습니다.  | 공용 또는 소유자만 구성할 수 있습니다.       |
| 유니버설 플랫폼 | 사용자 당 64 MB | 사용자 당 64 MB    | 사용자 데이터와 같은 저장 된 게임이 나 재생/일시 중지/다시 시작에 대 한 게임 상태. | 모든 플랫폼 쓸 수 있지만 Xbox One, Xbox 360 또는 Windows Phone 이외의 플랫폼만 읽을 수 있습니다. | 공용 또는 소유자만 구성할 수 있습니다.       |
| 글로벌             | 256MB | 256MB            | 명단, 지도, 문제, 또는 이미지 리소스 등의 모든 사용자가 읽을 수 있는 데이터입니다. | Xbox 개발자 포털 또는 파트너 센터를 통해 쓰기만 모든 플랫폼 읽을 수 있습니다.                                | 모든 사용자가 읽을 수 있습니다.

### <a name="deprecated-storage-types"></a>사용 되지 않는 저장소 유형

다음 저장소 유형 되지 않습니다. 현재 사용 중인 타이틀에 대해서만 지원 됩니다. 새 제목에 사용할 수 있는 하지 않습니다.

| 저장소 유형       | 할당량  |   목적                                                                                                                                                      | 플랫폼                                                                                           | 사용자                                       |
|--------------------|--------------------|---------|--------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------|---------------------------------------------|
| JSON               | 사용자 당 64 MB     | 사용자 데이터와 같은 저장 된 게임이 나 재생/일시 중지/다시 시작에 대 한 게임 상태. 보안 강화 플랫폼 제한이 비슷하지만 데이터 형식을 제한 (JSON에만 해당). | 모든 플랫폼 읽거나 쓸 수 있습니다.                                                                     | 공용 또는 소유자만 구성할 수 있습니다.       |
...
| 세션 저장소    | 세션별 256MB | 모든 사용자에 대 한 데이터는 특정 멀티 플레이 게임 세션에 연결 합니다.                                                                                             | 세션에 참가할 수 있는 모든 플랫폼입니다.                                                             | 모든 사용자 세션에서 읽거나 쓸 수 있습니다. |
...
이미지, 소리 및 사용자 지정 데이터 이진 형식을 사용 합니다. HTTP를 통해 데이터를 전송 해야, 때문에 이진 데이터는 HTTP 문자로 인코딩해야 합니다. 예를 들어 데이터를 16 진수 문자열 변환 또는 base64 인코딩을 사용 해야 합니다. 타이틀 저장소 시스템 게임 인코드 및 디코드 데이터에서 읽고 타이틀 저장소에 쓸 때 같은 체계를 사용 해야 하므로 인코딩된 데이터를 처리 하지 않습니다.
...
**{유형}** 구성 blob 데이터를 나타내는 **구성** 될 수 있습니다. 구성 blob는 글로벌 타이틀 저장소에 저장 된 데이터 구조입니다. Blob 형식의 JSON 개체와 비슷합니다.

구성 blob 설정을 가능성 목록에서 반환 하는 가상 노드를 포함할 수 있습니다. 가상 노드는 제목 또는 로캘와 같은 특정 상황에 대 한 설정을 제공 하는 데 유용 합니다. 가상 노드 값 및 조건 값에서 선택 하기 위한 몇 가지 가능한 설정이 포함 되어 있습니다. 다음 예제에서는 **defaultCardDesign** 설정에에서 있을 수 값 중 하나로 가상 노드.
...
**\_SelectBy** 값 **\_selector** 값 작업을 수행 하는 선택의 선택에 사용 하 여 데이터를 나타냅니다. 가능한 값은 다음과 같습니다.
...
<tr>
<td >locale</td>
<td ><p><strong>_Selector</strong> Accept-language 헤더에서 로캘 문자열을 찾습니다.</p></td>
</tr>
<tr>
<td >사용자 지정</td>
<td ><p><strong>_Selector</strong> <strong>customSelector</strong> 쿼리 매개 변수에서 전달 된 사용자 지정 문자열을 찾습니다. <strong>CustomSelector</strong> 쉼표로 구분 된 하나 이상의 쿼리를 포함 합니다. 각 쿼리 <strong>selectBy</strong> 요소에서 이름이 며 <strong>_selector</strong> 요소의 값입니다.</p></td>
</tr>
</tbody>
</table>
...
읽기 나 쓰기 제목을 분당 만들 수는 고정된 제한이 하지만 일반적으로 사용할 수 없는 분당 하나 이상 평균 1 시간 세션에서. 예를 들어 제목을 세션 하지만 남아 있는 시간 동안 더 이상 시작 부분에서 60 읽기 또는 쓰기 만들 수 있습니다. 타이틀 해야 강화 호출에 대해 나중에 Xbox LIVE 서비스 요청을 제한 해야 하는 경우.
...
읽는 방법을 보여 줍니다 Xbox Live에서 구성 blob 저장소 제목입니다.
...
읽는 방법을 보여 줍니다 Xbox Live에서 JSON blob 저장소 제목입니다.
