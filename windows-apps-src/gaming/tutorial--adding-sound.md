...
게임에 오디오 재생을 위해 설정의 세 부분으로 이루어져 있습니다.
...
를 설정한 후 재생 소리 효과 트리거하는 방법에 알아봅니다 했습니다. 자세한 내용은 이동 [소리를 재생](#play-the-sound)합니다.
...
>[!Note]
>소리를 재생 하려면 실제 트리거 이동 및 이러한 게임 개체의 충돌에 의해 결정 됩니다. 따라서 실제로 이러한 소리를 재생에 대 한 호출 [Simple3DGame::UpdateDynamics](#simple3dgameupdatedynamics-method) 메서드에서 정의 됩니다. 자세한 내용은 이동 [소리를 재생](#play-the-sound)합니다.
...
게임 샘플에서 오디오 형식 파일을 읽는 코드는 [MediaReader.h](#mediareaderh)/cpp__에서 정의 됩니다.  인코드된.wav 오디오 파일을 읽고, [mediareader:: Loadmedia](#mediareaderloadmedia-method), 입력된 매개 변수로.wav 파일 이름을 전달 하 여 호출 합니다.
...
4. [:: Setcurrentmediatype](https://msdn.microsoft.com/library/windows/desktop/dd374667.aspx)호출 하 여 원본 뷰어 디코드된 출력 미디어 유형을 설정 합니다.
...
개체 간의 상호 작용 게임에 따라 크게 달라 하므로 게임 개체를 여기의 움직임에 설명 하지 않을 수 했습니다. 구현 이해 관심이 [Simple3DGame::UpdateDynamics](#simple3dgameupdatedynamics-method) 방법으로 이동 합니다.
...
* 원본 음성 개체 **m\_sourceVoice** 를 사용 하 여 사운드 데이터 버퍼 **m\_soundData** 재생을 시작 하려면
...
세 가지 유형의 XAudio2 음성 개체가 가지: 원본, 서브 믹스 및 마스터링 음성 합니다. 음성은 XAudio2 개체 조작 하 고 오디오 데이터를 재생 하는의 작업을 처리를 사용 합니다. 
