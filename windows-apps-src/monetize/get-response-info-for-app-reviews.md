...
* 아직 완료하지 않은 경우 Windows 스토어 분석 API에 대한 모든 [필수 조건](respond-to-reviews-using-windows-store-services.md#prerequisites)을 완료합니다.
...
* 리뷰에 응답할 수 있는지 확인하려는 리뷰의 ID를 가져옵니다. 리뷰 ID는 Windows 스토어 분석 API의 [앱 리뷰 가져오기](get-app-reviews.md) 메서드의 응답 데이터와 [리뷰 보고서](../publish/reviews-report.md)의 [오프라인 다운로드](../publish/download-analytic-reports.md)에서 사용할 수 있습니다.
...
| reviewId | 문자열 | 응답하려는 리뷰의 ID(GUID)입니다. 리뷰 ID는 Windows 스토어 분석 API의 [앱 리뷰 가져오기](get-app-reviews.md) 메서드의 응답 데이터와 [리뷰 보고서](../publish/reviews-report.md)의 [오프라인 다운로드](../publish/download-analytic-reports.md)에서 사용할 수 있습니다. <br/>이 매개 변수를 생략하면, 이 메서드에 대한 응답 본문에 지정된 앱의 리뷰에 응답할 수 있는 권한이 있는지 표시됩니다. |  아니요  |
...
| DefaultSupportEmail  | 문자열 |  앱의 스토어 목록에 지정된 앱의 [지원 이메일 주소](../publish/create-app-store-listings.md#support-contact-info)입니다. 지원 이메일 주소를 지정하지 않으면 이 필드는 비어 있습니다.    |
...
* [Windows 스토어 분석 API를 사용하여 리뷰에 응답 제출](submit-responses-to-app-reviews.md)
