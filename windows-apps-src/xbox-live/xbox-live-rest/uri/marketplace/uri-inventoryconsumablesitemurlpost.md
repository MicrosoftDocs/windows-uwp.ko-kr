...
## <a name="remarks"></a>설명
   ...
   * 호출자 소비 하 라는 수량 나머지 제공할 수 있는 항목을 초과 하면 호출이 거부 됩니다.
   * 호출자 소비 하 라는 수량 양의 정수 0 이어야 합니다. 소비 값이 0 또는 더 낮은 호출 거부 됩니다.
...
제거 수량 필드를 소모의 나머지 수량에서 제거 하고자 하는 소모 성 수량을 나타내는 호출자를 허용 합니다. 트랜잭션 ID 필드는 소모 성 콘텐츠 작업을 사용 하 여 동일한 사용량을 두 번 계산의 위험을 제한 하는 동안 다시 시도 하는 수단을 사용 하 여 호출자에 게를 제공 합니다.
...
POST, 인증을 통과 하 고 적절 한 권한 부여 컨텍스트 할당 한다고 가정 하에 대 한 응답이 동일한 transactionId 사용 하 여 영수증의는 acknolodgement POST 요청, 소모 성 항목의 URL 및 항목의 새 서비스에 전달 수량 값입니다.
