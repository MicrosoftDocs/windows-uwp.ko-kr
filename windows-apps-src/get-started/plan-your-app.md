...
좋은 IA는 사용자 시나리오를 가능하게 할 뿐만 아니라 시작할 주요 화면을 구상하는 데 도움이 됩니다. 예를 들어 [바꿀](http://go.microsoft.com/fwlink/p/?LinkID=268089) 앱 사용자의 라이브러리, 저장소, 뉴스 및 통계에 대 한 액세스를 제공 하는 허브로 바로 시작 합니다. 사용자가 가져오고 오디오북을 빠르게 즐길 수 있도록 경험 포커스가 합니다. 앱의 심층 수준에서는 보다 구체적인 작업에 중점을 둡니다.
...
-   **파일 선택기 및 파일 선택기 확장** 로드 하 고 로컬 파일 시스템, 연결 된 저장 장치, 홈 그룹 또는 기타 앱에서 해당 파일을 저장할 수 있도록 합니다. 다른 앱에서 사용자 앱의 콘텐츠를 로드할 수 있도록 파일 선택기 확장을 제공할 수도 있습니다.

자세한 내용은 [앱 계약 및 확장](https://msdn.microsoft.com/library/windows/apps/hh464906)을 참조하세요.
<!-- Win 8 page. Should have replacement. -->

**다른 보기, 폼 팩터 및 하드웨어 구성**Windows 충전 및 중점에서 앱에 사용자를 배치 합니다. 사용자가 해당 앱을 사용하기로 결정한 어떤 디바이스에서든, 어떤 입력 모드를 사용하든, 어떤 방향으로든, 어떤 하드웨어 구성에서든, 어떤 환경에서든 앱 UI가 두드러지게 하고 싶을 것입니다.
...
-   **알림**수 있도록 사용자에 게 알림 메시지를 통해 시간이 중요 하거나 개인적으로 관련 콘텐츠 및 앱이 닫힌 경우에 앱으로 다시 초대 합니다. [타일, 배지 및 알림 메시지](../design/shell/tiles-and-notifications/index.md)에 대해 자세히 알아보세요.
...
-   **로밍**중요 가장, 사용 중인 장치와 관계 없이 UX를 유지 하 고 사용자가 작업 한 위치를 선택할 수 있는 데이터를 로밍 하 여 장치에서 지속적인 환경을 만듭니다. 로밍을 통해 설정 및 상태를 유지 관리하여 부엌에 있는 가족용 PC, 회사 PC, 개인용 태블릿, 기타 폼 팩터 등 어디에서든 앱을 쉽게 사용할 수 있게 합니다. [응용 프로그램 데이터 관리](../design/app-settings/store-and-retrieve-app-data.md)에 대해 살펴보고, [로밍 중인 응용 프로그램 데이터에 대한 지침](https://msdn.microsoft.com/library/windows/apps/hh465094)을 참조하세요.
...
-   **근접 연결 제스처**가까이 있는으로 실제로 디바이스를 함께 탭 하 여 (멀티 플레이어 게임) 다른 사용자와 장치를 연결할 수 있게 합니다. [근접 연결 및 탭하기](https://msdn.microsoft.com/library/windows/apps/hh465229)에 대해 자세히 알아보세요.
...
**시작 화면**시작 화면, 가능한 한 빠르게 로드 되어야 하 고 화면에 남아만으로 앱 상태를 초기화 해야 합니다. 시작 화면에는 앱의 개성을 잘 나타내는 내용을 표시해야 합니다.
