...
**SoundLevelChanged** 이벤트 처리기에서 스트림에 대 한 새로운 오디오 레벨 결정에 **AudioStateMonitor** 보낸 처리기에 전달 된의 [**SoundLevel**](https://docs.microsoft.com/uwp/api/windows.media.audio.audiostatemonitor.soundlevel) 속성을 확인 합니다. 이 예제에서 앱은 사운드 레벨 음이 소거되면 오디오 캡처를 중지하고 오디오 레벨이 전체 볼륨으로 돌아가면 캡처를 다시 시작합니다.
