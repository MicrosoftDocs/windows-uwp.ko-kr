...
translationtype: HT
---
# <a name="desktop-to-uwp-bridge-sign"></a>데스크톱-UWP 브리지: 서명

이 문서에서는 UWP(유니버설 Windows 플랫폼)로 변환한 데스크톱 앱에 서명하는 방법에 대해 설명합니다. Windows 앱 패키지를 배포하려면 먼저 인증서로 서명해야 합니다.

## <a name="automatically-sign-using-the-desktop-app-converter-dac"></a>DAC(Desktop App Converter)를 사용하여 자동으로 서명

DAC 실행 시 ```-Sign``` 플래그를 사용하여 Windows 앱 패키지에 자동으로 서명합니다. 자세한 내용은 [Desktop App Converter Preview](desktop-to-uwp-run-desktop-app-converter.md)를 참조하세요.
...
```
마지막으로, SignTool.exe를 사용하여 인증서로 Windows 앱 패키지에 서명합니다.
