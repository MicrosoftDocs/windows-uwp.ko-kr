...
2.  AppServiceProvider 프로젝트에서 새 MyAppService 프로젝트에 프로젝트-프로젝트 참조를 추가합니다. 참조를 추가하지 않으면 컴파일 오류가 보이지는 않지만 앱 서비스가 런타임에 연결되지 않으므로 이 단계가 매우 중요합니다.
...
- `<Extensions>` 항목과 그 자녀 요소가 위의 [package.appxmanifest에 앱 서비스 확장 추가](#add-an-app-service-extension-to-package-appxmanifest)에서 지정된 앱 서비스 공급자 프로젝트에 속한 Package.appxmanifest 파일에 추가되었는지 확인합니다.
...
- `AppServiceConnection.PackageFamilyName`이 위의 [package.appxmanifest에 앱 서비스 확장 추가](#add-an-app-service-extension-to-package-appxmanifest)에서 지정한 앱 서비스 공급자 구성 요소의 패키지 패밀리 이름과 일치하는지 확인합니다.
