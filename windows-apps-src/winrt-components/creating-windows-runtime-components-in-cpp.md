...
일반적으로 코딩할 때 C + + /cli CX 구성 요소 및 사용 하 여 일반 c + + 라이브러리의 기본 제공 형식 제외 하 고 추상 ABI (이진 인터페이스) 경계에 전달 하는 코드에서 데이터를 다른.winmd 패키지에서 있습니다. 여기에서 Windows 런타임 형식 및 특수 구문을 사용 하 여 해당 C + + /cli CX 만들고 해당 형식을 조작 하기 위한 지원 합니다. 또한에서 C + + /CX 코드, JavaScript, Visual Basic, c + + 구성 요소에서 발생 하 고 처리할 수 있는 이벤트를 구현 하는 대리자와 이벤트 같은 형식 사용 하거나 C#. 자세한 내용은 C + + /CX 구문 참조 [Visual c + + 언어 참조 (C + + CX)](https://msdn.microsoft.com/library/windows/apps/xaml/hh699871.aspx)합니다.
...
## <a name="ccx-built-in-types-library-types-and-windows-runtime-types"></a>C + + /cli CX 기본 제공 형식, 라이브러리 형식 및 Windows 런타임 형식
...
Windows 런타임 구성 요소에는 여러 활성화 가능한 공용 클래스와 구성 요소에 내부적으로만 알려진 추가 클래스가 포함될 수 있습니다. 적용 된 [WebHostHidden](https://msdn.microsoft.com/library/windows/apps/windows.foundation.metadata.webhosthiddenattribute.aspx) 특성을 C + + /CX 형식 JavaScript에 표시 되지 합니다.
...
Abi 전반에서 사용자 정의 값 구조체를 전달 하려면 C +에 정의 된 값 구조체와 동일한 멤버를 가진 JavaScript 개체를 정의 + CX 합니다. 전달할 수 있습니다 개체 인수로 서 C + + CX 메서드는 개체는 암시적으로 변환할 C + + /CX 형식입니다.
...
C + + /cli CX public ref 클래스는 오버 로드 된 메서드를 포함할 수 있지만 JavaScript는 오버 로드 된 메서드를 구분 하는 기능이 제한적입니다. 예를 들어 다음 서명 간의 차이는 구분할 수 있습니다.
...
## <a name="net"></a>.NET
.NET 언어 인식 오버 로드는 C + + /CX ref 클래스가 모든.NET Framework 클래스와 마찬가지로 합니다.
...
.NET 언어는 액세스할 속성에는 네이티브 C + +.NET Framework 개체의 경우 처럼 CX 개체입니다.
...
## <a name="enums"></a>열거
Windows 런타임 열거형을 C + + /cli CX 공용 클래스 열거형;를 사용 하 여 선언 됩니다 표준 c + +의 범위가 지정 된 열거형을 비슷합니다.
...
C + 비동기 메서드를 구현 하 + /CX를 사용 하 여는 [만들\_비동기](https://msdn.microsoft.com/library/hh750102.aspx) ppltasks.h에 정의 된 함수입니다. 자세한 내용은 참조 하세요. [비동기 작업 만들기 C + + UWP 앱 용 CX](https://msdn.microsoft.com/library/vstudio/hh750082.aspx)합니다. 예를 들어 참조 [연습: C + 기본적인 Windows 런타임 구성 요소 만들기 + CX 및 JavaScript에서 호출 또는 C# ](walkthrough-creating-a-basic-windows-runtime-component-in-cpp-and-calling-it-from-javascript-or-csharp.md)합니다. .NET 언어 사용 C + + /cli CX 비동기 메서드는.NET Framework에 정의 된 비동기 메서드의 경우와 마찬가지로 합니다.
...
제거 하면 C + + /cli CX Windows 런타임 구성 요소 프로젝트 솔루션에서 수동으로 제거 해야 JavaScript 프로젝트에서 프로젝트 참조 합니다. 이렇게 하지 않으면 이후 디버그 또는 빌드 작업을 수행할 수 없습니다. 필요한 경우 DLL에 대한 어셈블리 참조를 추가할 수 있습니다.
