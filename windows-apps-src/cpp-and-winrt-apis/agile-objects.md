...
대부분 경우 표준 C++ 개체 같은 Windows 런타임 클래스 인스턴스는(대부분의 표준 C++ 개체와 마찬가지로) 모든 스레드에서 액세스할 수 있습니다. 이러한 Windows 런타임 클래스는 *Agile*입니다. Windows와 함께 제공되는 소수의 Windows 런타임 클래스는 Agile이 아닙니다. 하지만 이러한 클래스를 사용할 때는 스레딩 모델과 마샬링 동작을 고려해야 합니다. 여기에서 마샬링이란 아파트 경계에서 데이터를 전달하는 것을 말합니다. 모든 Windows 런타임 개체가 Agile이도록 설정하면 기본적으로 사용자 고유의 [C++/WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt) 형식도 Agile이기 때문에 매우 바람직합니다.
...
옵트아웃을 설정하지 않았기 때문에 이 구현은 Agile입니다. [**winrt::implements**](/uwp/cpp-ref-for-winrt/implements) 기본 구조체는 [**IAgileObject**](https://docs.microsoft.com/windows/desktop/api/objidl/nn-objidl-iagileobject)와 [**IMarshal**](/windows/desktop/api/objidl/nn-objidl-imarshal)을 구현합니다. **IMarshal** 구현은 **CoCreateFreeThreadedMarshaler**를 사용하여 **IAgileObject**에 대해 알지 못하는 레거시 코드에 올바른 작업을 수행합니다.
