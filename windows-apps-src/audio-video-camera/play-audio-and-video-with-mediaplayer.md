...
[  **SetMediaPlayer**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.mediaplayerelement.setmediaplayer)를 호출하면 요소가 바인딩되는 **MediaPlayer** 인스턴스를 설정할 수 있습니다.

[!code-cs[SetMediaPlayer](./code/MediaPlayer_RS1/cs/MainPage.xaml.cs#SnippetSetMediaPlayer)]

**MediaPlayerElement**에 재생 소스를 설정할 수도 있으며 요소는 [**MediaPlayer**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.mediaplayerelement.mediaplayer) 속성을 사용하여 액세스할 수 있는 **MediaPlayer** 인스턴스를 자동으로 만듭니다.
...
시스템이 재생 중인 미디어의 종류를 인식하도록 [**MediaPlayerAudioCategory**](https://docs.microsoft.com/uwp/api/Windows.Media.Playback.MediaPlayerAudioCategory) 열거형 값 중 하나에 **MediaPlayer**의 [**AudioCategory**](https://docs.microsoft.com/uwp/api/windows.media.playback.mediaplayer.audiocategory) 속성을 설정합니다. 다른 응용 프로그램이 백그라운드에서 음악을 재생하는 경우 게임 음악은 자동으로 음소거되도록 게임은 해당 음악 스트림을 **GameMedia**로 분류해야 합니다. 음악 또는 동영상 응용 프로그램은 **GameMedia** 스트림보다 우선하도록 **Media** 또는 **Movie**로 스트림을 분류해야 합니다.
...
디바이스 콤보 상자에 대한 [**SelectionChanged**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.primitives.selector.selectionchanged) 이벤트에서 **ComboBoxItem**의 [**Tag**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.frameworkelement.tag) 속성에 저장된 **MediaPlayer**의 [**AudioDevice**](https://docs.microsoft.com/uwp/api/windows.media.playback.mediaplayer.audiodevice) 속성이 선택한 디바이스로 설정됩니다.
...
마지막으로 **MediaPlaybackSession**의 [**NormalizedSourceRect**](https://docs.microsoft.com/uwp/api/windows.media.playback.mediaplaybacksession.normalizedsourcerect)가 새로 조정된 사각형으로 설정되고 렌더링해야 할 비디오 프레임 내의 영역을 지정합니다.
