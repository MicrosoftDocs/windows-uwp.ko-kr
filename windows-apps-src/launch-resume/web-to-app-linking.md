---
author: TylerMSFT
title: App URI 처리기를 사용 하 여 웹사이트에 대 한 앱 사용
description: 웹사이트 기능에 대 한 앱을 지원 하 여 앱의 사용자 커뮤니케이션을 구동 합니다.
...
ms.locfileid: "958316"
---
# <a name="enable-apps-for-websites-using-app-uri-handlers"></a>App URI 처리기를 사용 하 여 웹사이트에 대 한 앱 사용

앱 브라우저를 열지 않고 대신 시작 되 고 다른 사용자가 웹사이트에 대 한 링크를 열면 있도록 웹사이트와 응용 프로그램을 연결 하는 웹사이트에 대 한 앱입니다. 앱이 설치 되어있지 않은 브라우저에서 웹사이트 정상적으로 열립니다. 확인된 콘텐츠 소유자만 링크를 등록할 수 있기 때문에 사용자는 이 환경을 신뢰할 수 있습니다. 사용자가 등록 된 웹 응용 프로그램 링크의 모든 설정으로 이동 하 여 확인할 수 있게 됩니다 > 앱 > 웹사이트에 대 한 앱입니다.

웹와 app-하면 연결을 사용 하도록 설정 해야 합니다.
- 매니페스트 파일에서 앱이 처리할 URI를 식별합니다.
- 응용 프로그램 및 웹 사이트 간의 연결을 정의 하는 JSON 파일입니다. 응용 프로그램으로 동일한 호스트 루트 패키지 제품군 이름을 app 매니페스트 선언 합니다.
- 앱의 활성화를 처리합니다.

> [!Note]
> 지원 되는 링크를 Microsoft에 지에서를 클릭 한 Windows 10 작성자 업데이트 부터는 해당 응용 프로그램을 실행 됩니다. 다른 브라우저 (예: Internet Explorer, 등)를 클릭 하는 지원 되는 링크는 다음 검색 환경을에 유지 됩니다.
...
예를들어, `"excludePaths" : [ "/news/*", "/blog/*" ]` 위의 예제에서는 앱 아래을 **제외 하 고** 웹사이트의 주소 (예: msn.com)로 시작 하는 모든 경로 지원 합니다 `/news/` 및 `/blog/`합니다. 즉 **msn.com/weather.html**은 지원되지만 ****msn.com/news/topnews.html****은 지원되지 않습니다.
...
이 도구 아무것도 반환 하지 않는 경우 유효성 검사 업로드 하는 경우 해당 파일에서 작동 합니다. 오류 코드가 있으면 작동 하지 않습니다.

로컬 유효성 검사의 일부로 쪽 로드 된 앱에 대 한 일치 하는 경로 강제 실행 하려면 다음 레지스트리 키를 사용 하도록 설정할 수 있습니다.
...
[웹 응용 프로그램 예제 프로젝트](https://github.com/project-rome/AppUriHandlers/tree/master/NarwhalFacts)
[windows.protocol 등록](https://msdn.microsoft.com/library/windows/apps/br211458.aspx)
[URI 정품 인증 처리](https://msdn.microsoft.com/windows/uwp/launch-resume/handle-uri-activation)
[연결을 시작 하는 샘플](https://github.com/Microsoft/Windows-universal-samples/tree/master/Samples/AssociationLaunching) LaunchUriAsync() API를 사용 하는 방법을 보여줍니다.
