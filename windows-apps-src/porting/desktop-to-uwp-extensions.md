...
* [사용자가 자신의 PC에 장치를 연결할 때 응용 프로그램을 시작 합니다.](#autoplay)
...
> [!NOTE]
> 사용자가이 시작 작업을 등록 한 번 이상 응용 프로그램을 시작 됩니다.
...
|TaskId |작업의 고유 ID입니다. 이 식별자를 사용 하 여 응용 프로그램 프로그래밍 방식으로 사용 하 여 시작 작업을 사용 하지 않도록 설정 하거나 [Windows.ApplicationModel.StartupTask](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.StartupTask) 클래스의 Api를 호출할 수 있습니다. |
...
### <a name="enable-users-to-start-your-application-when-they-connect-a-device-to-their-pc"></a>사용자가 자신의 PC에 장치를 연결할 때 응용 프로그램을 시작 합니다.
...
사용자에 대 한 업데이트를 설치할 때 응용 프로그램 열려 있으면 응용 프로그램을 닫습니다.

업데이트가 완료 된 후 다시 시작 하는 응용 프로그램을 원하는 경우 다시 시작 하려는 모든 프로세스에서 [RegisterApplicationRestart](https://msdn.microsoft.com/library/windows/desktop/aa373347.aspx) 함수를 호출 합니다.
...
이 확장은 모든 장치에서 실행 되는 유니버설 Windows 플랫폼 사용자 인터페이스를 구현 하려는 경우 유용할 수 있습니다 싶으면 완전 신뢰 모드에서 계속 실행 되도록 Win32 응용 프로그램의 구성 요소가 있습니다.
