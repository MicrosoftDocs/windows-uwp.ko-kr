...
요약 사용자 메시지 메시지 제목만 포함 되어 있습니다. 사용자 생성 메시지,이 현재 메시지 텍스트의 처음 20 자입니다. 시스템 메시지 "라이브 시스템" 등의 대체 주체를 제공할 수 있습니다.

메시지; 수신 된 순서의 반대 방향으로 돌아갑니다. 즉, 최신 메시지는 먼저 반환 됩니다.
...
| continuationToken| 문자열|  | 이전 열거형; 호출에서 반환 된 문자열 열거형을 계속 하는 데 사용 합니다.|
...
사용자는 사용자 메시지 요약을 검색 하 고 있어야 합니다.
...
| isRead| 부울|  | 사용자가 메시지 세부 정보 읽었을 나타내는 플래그입니다.|
