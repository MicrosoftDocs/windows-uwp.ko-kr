...
Xbox 멀티 플레이어 서비스에 대 한 선택적 추가 제목 제공한 또는 사용자 통계에 저장 되어 있거나 사용자의 기본 설정에 따라 정보에 따라 함께 그룹화 플레이어의 메서드를 제공 하는 Xbox 매치 메이 킹 서버 기반 서비스 또는 서비스의 품질에 기반합니다.
...
새 매치 메이 킹 hopper를 만들 때 다음을 정의 해야 합니다.
...
| 확장 주기 규칙 해야 | 기본값은 3입니다. 기본값 일반 플레이어 채우기 변경할 필요는 없습니다. |
...
| 세션에서 자동 업데이트 | 이 필드를 사용 하면 세션의 구성원 목록을 변경 또는 이전에 제출한 티켓 구성원의 사용자 지정 속성은 자동으로 적용 합니다. |
...
멀티 플레이 서비스 구성 하는 방법에 대 한 자세한 내용은 다음 문서를 참조 하세요.
...
[멀티 플레이 세션 템플릿](session-templates.md) | 멀티 플레이 세션 템플릿의 간략 한 개요를 제공 하 고 템플릿 복사 하 고 로컬 멀티 플레이어 세션을 수정할 수 있는 몇 가지 예제를 제공 합니다.
