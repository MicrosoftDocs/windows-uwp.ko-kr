...
예를 들어 `myTypes` 접두사를 `myCompany.myTypes` 네임스페이스에 매핑하는 특성 구문은 `    xmlns:myTypes="using:myCompany.myTypes"`이며 대표적인 요소의 사용 예는 `<myTypes:CustomButton/>`입니다.
...
Windows 런타임은 또한 [{x:Null} 태그 확장](x-null-markup-extension.md)을 지원합니다. XAML에서 이 태그 확장을 사용하여 [**Nullable**](T:System.Nullable%601) 값을 **null**로 설정합니다. 예를 들어 **null**을 결정할 수 없는 확인 상태로 해석하는 [**CheckBox**](https://msdn.microsoft.com/library/windows/apps/br209316)에 대한 컨트롤 템플릿에서 이 태그 확장을 사용할 수 있습니다("결정할 수 없는" 시각적 상태 트리거).
