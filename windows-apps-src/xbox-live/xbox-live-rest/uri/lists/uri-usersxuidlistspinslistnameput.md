...
이 호출은 요청 본문에서 각 항목에 대해 지정 된 인덱스에 따라 목록에서 항목을 업데이트 합니다. 이 호출 목록에 항목을 삽입 됩니다 하 고 지정 된 인덱스에 항목이 없으면 다음 호출 400 잘못 된 요청 않았다는 상태가 반환 됩니다. 한 번 호출에서 여러 항목을 업데이트할 수 있습니다 하지만 현재 목록에 모두 있어야 합니다. 즉, 모든 업데이트 또는 none 업데이트 합니다.
 
이 호출을 **인덱스가**아닌 **itemId** 으로 지정 하는 업데이트가 항목을 수 있습니다. 이렇게 하려면 **IndexedItems** 구조의 서비스에 전송 되는 인덱스에 대 한 "-1"를 사용 합니다. 분명히이 경우 **itemId** 변경할 수 없으므로 업데이트 프로세스의 일부로 다른 메타 데이터 필드의 변경 사항을 에서만 작동 합니다. **공급자**/항목을 식별 하 **itemId** 대신 사용할 수 있으며**providerId** 콤보 합니다. 내부적으로 서비스는 이러한 항목 및 업데이트에 대 한 적절 한 인덱스가 확인에 대 한 목록을 검색 합니다. 항목을 찾을 수 없는 경우 400 잘못 된 요청 상태를 반환 됩니다 하 고 항목이 업데이트 됩니다.
 
이 호출은 필요는 **경우-일치: versionNumber** 헤더를 인덱스를 사용 하 여 항목을 식별 하는 경우 요청에 포함 합니다. 사용 하 여 항목을 식별 하는 Id를 항목 (및 목록 중복을 허용 하지 않음) 하는 경우 **는 헤더는** 선택 사항입니다. 있는 경우 **는 헤더** 항상 확인할 수 있습니다. 헤더에서 **versionNumber** 는 목록의 현재 버전 번호입니다. 것 및 되지 않는 포함 (필수), 또는 현재 목록 버전 번호를는 412 전제 조건 실패 HTTP 상태 코드를 반환 하는 일치 하지 및 응답 본문은 포함 않음 목록의 최신 메타 데이터는 현재 버전 번호를 포함 합니다. 서로에서 무시 되는 다른 클라이언트에서 업데이트를 방지 하는 데입니다.
