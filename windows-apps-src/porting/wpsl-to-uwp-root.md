...
|필터 앱 | [Windows Phone 8에 대 한 필터](https://docs.microsoft.com/previous-versions/windows/apps/jj206990(v=vs.105)) |
...
| [비즈니스 및 데이터 계층을 이식](wpsl-to-uwp-business-and-data.md) | UI의 뒤에는 비즈니스 및 데이터 계층이 있습니다. 이러한 계층의 코드는 운영 체제 및 .NET Framework API를 호출합니다(예제: 후순위 처리, 위치, 카메라, 파일 시스템, 네트워크 및 기타 데이터 액세스). 대부분은 [UWP 앱에서 사용할 수 있으므로](https://docs.microsoft.com/previous-versions/windows/br211369(v=win.10)), 이 코드의 대부분을 변경하지 않고 포팅할 수 있습니다. |
