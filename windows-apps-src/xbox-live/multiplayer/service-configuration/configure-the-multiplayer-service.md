...
ms.locfileid: "8748201"
---
# <a name="multiplayer-service-configuration"></a>멀티 플레이 서비스 구성
Xbox Live를 제공 하는 서비스를 활용 하 여 타이틀에 대 한 순서로 먼저 서비스 구성을 정의 해야 합니다. 이 서비스 구성 Xbox Live 클라우드 서비스에 존재 하 고 Xbox Live 서비스 제목/게임을 실행 하는 모든 디바이스와 상호 작용 하는 방법을 정의 합니다.
...
Xbox 멀티 플레이 서비스 게이머를를 만들고 세션을 다른 게이머와 같은 세션에서 세션 메시지를 교환 하 고 해당 재생 세션의 결과 게시할 수 있습니다. (결과 게시 세션 정리 및 세션에서 모든 플레이어에 게는 순위표 업데이트 합니다.)
...
Xbox 멀티 플레이어 서비스에 대 한 선택적 추가 제목에서 제공 하거나 사용자 통계에 저장 되어 있거나 사용자의 기본 설정에 따라 정보에 따라 함께 그룹화 플레이어의 메서드를 제공 하는 Xbox 매치 서버 기반 서비스 또는 서비스의 품질에 기반합니다.

Xbox One 매치 메이 킹 서버 기반 이므로 사용자 서비스 요청을 제공할 수 및 다음 받기를 나중에 일치 하는 경우. 즉: 사용자가 연결 프로세스는 발생 하는 동안 타이틀 대기할 강제로 적용 되지 않습니다-싱글 플레이어 부분 타이틀을 재생 하려면 무료 또는 다른 타이틀을 재생 하는 데도 되며 여전히 매치 메이 킹 제거할 수 있습니다. 이 일치를 찾을 수 전에 플레이어의 "중요 한 대용량"를 달성 하기 위해 하지 않아도 됩니다.
...
| 필드 | 설명 |
|---|---|
|이름| 매치 메이 킹 hopper 란 하 고 쉽게 기억 하 고 인식 이름을 입력 합니다. 이름은 텍스트 문자열을 140 자 여야 합니다. |
...
> **중요:** 타이틀 최종 인증을 통과 하면 해당 제목에 기존 연결 hoppers 변경 하거나 삭제 더 이상 수 없습니다.
...
[멀티 플레이 세션 템플릿](session-templates.md) | 멀티 플레이 세션 템플릿 대 한 간략 한 개요를 제공 하 고 템플릿 복사 하 고 로컬 멀티 플레이 세션을 수정할 수 있는 몇 가지 예제를 제공 합니다.
