...
이 섹션의 항목에서는 백그라운드 작업을 사용하여 백그라운드에서 고유한 경량 코드를 실행하는 방법을 보여 줍니다. 앱이 일시 중단되거나 실행되지 않을 때 백그라운드 작업을 사용하여 기능을 제공할 수 있습니다. VOIP, 메일 및 IM 같은 실시간 통신 앱에 백그라운드 작업을 사용할 수도 있습니다.
...
## 다중 프로세스 및 단일 프로세스 백그라운드 작업

백그라운드 작업을 구현하는 방법에는 In Process 및 Out of Process 두 가지가 있습니다. In Process 백그라운드 지원은 쓰기 백그라운드 작업을 단순화하기 위해 Windows 10 버전 1607에서 처음 소개되었습니다. 그러나 Out of Process 백그라운드 작업도 계속 작성할 수 있습니다. In Process 및 Out of Process 백그라운드 작업을 작성하는 경우에 대한 자세한 내용은 [백그라운드 작업 지침](guidelines-for-background-tasks.md)을 참조하세요.
...
단일 프로세스 백그라운드 작업을 빠르게 시작하려면 [단일 프로세스 백그라운드 작업 만들기 및 등록](create-and-register-a-singleprocess-background-task.md)을 참조하세요.

다중 프로세스 백그라운드 작업으로 빠르게 시작하려면 [별도 프로세스에서 실행되는 백그라운드 작업 만들기 및 등록](create-and-register-a-background-task.md)을 참조하세요.
...
별도 프로세스에서 실행되는 백그라운드 작업을 성공적으로 등록하려면 먼저 응용 프로그램 매니페스트에서 선언되어야 합니다. 호스트 앱과 동일한 프로세스에서 실행되는 백그라운드 작업은 응용 프로그램 매니페스트에서 선언할 필요가 없습니다. 자세한 내용은 [응용 프로그램 매니페스트에서 백그라운드 작업 선언](declare-background-tasks-in-the-application-manifest.md)을 참조하세요.
...
> [!IMPORTANT]
> **DeviceUseTrigger** 및 **DeviceServicingTrigger**를 단일 프로세스 백그라운드 작업과 함께 사용할 수 없습니다.
