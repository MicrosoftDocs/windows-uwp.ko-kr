...
에 **SoundLevelChanged** 이벤트 처리기를 확인은 [ **SoundLevel** ](https://docs.microsoft.com/uwp/api/windows.media.audio.audiostatemonitor.soundlevel) 의 속성을 **AudioStateMonitor** 에 전달 된 발신자를 스트림에 대 한 새 오디오 수준을 결정 하는 처리기입니다. 이 예제에서 앱은 사운드 레벨 음이 소거되면 오디오 캡처를 중지하고 오디오 레벨이 전체 볼륨으로 돌아가면 캡처를 다시 시작합니다.
