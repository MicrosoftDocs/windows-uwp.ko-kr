---
Windows 설정 앱 시작
앱에서 Windows 설정 앱을 시작하는 방법을 알아봅니다. 이 항목에서는 ms-settings URI 체계에 대해 설명합니다. 이 URI 스키마로 Windows 설정 앱을 실행하여 특정 설정 페이지를 표시할 수 있습니다.
...
개인 정보 설정으로 인해 앱에서 중요한 리소스에 액세스할 수 없는 경우 **설정** 앱의 개인 정보 설정에 대한 편리한 링크를 제공하는 것이 좋습니다. 이를 통해 사용자는 자신의 설정을 보다 쉽게 변경할 수 있습니다.

**설정** 앱을 바로 실행하려면 다음 예제에 표시된 대로 `ms-settings:` URI 체계를 사용합니다.
...
또는 앱이 [**LaunchUriAsync**](https://msdn.microsoft.com/library/windows/apps/hh701476) 메서드를 호출하여 코드에서 **설정** 앱을 실행할 수 있습니다.
...
이 예제에서는 `ms-settings:privacy-webcam` URI를 사용하여 카메라에 대한 개인 설정 페이지를 시작하는 방법을 보여 줍니다.
...
| Accounts           | 메일 및 계정                | 모두           | ms-settings:emailandaccounts              |
|                    | 작업 액세스                            | 모두           | ms-settings:workplace                     |
