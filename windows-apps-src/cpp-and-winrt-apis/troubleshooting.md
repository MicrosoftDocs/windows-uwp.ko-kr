...
| LLVM 및 Clang 도구 체인에서 C +과 함께 사용할 경우 오류가 발생 하면 + WinRT 합니다.|C + LLVM 및 Clang 도구 체인은 지원 되지 + WinRT 하지만 어떻게 사용 내부적으로 에뮬레이션 하려는 경우 다음에서 설명 하는 것 같은 실험을 시도할 수 있습니다 [LLVM/Clang C + 컴파일하는 데 사용할 수 있나요 + WinRT?](faq.md#can-i-use-llvmclang-to-compile-with-cwinrt)합니다.|
...
| C + + 컴파일러에서 "*C3861 오류: 'from_abi': 식별자를 찾을 수 없는*", 및 기타 오류 *base.h*에서 발생 합니다. Visual Studio 2017을 사용 하는 경우이 오류를 볼 수 있습니다 (15.8.0 버전 이상), Windows SDK 버전 10.0.17134.0 (Windows 10, 버전 1803)를 대상으로 합니다. | 하나는 이후 (자세한 준수)를 대상으로 버전의 Windows SDK 또는 설정 프로젝트 속성 **C/c + +** > **언어** > **적합성 모드: 아니요** (또한 경우 **허용 /-**  >  **언어** >  **추가 옵션****명령줄** 삭제). |
