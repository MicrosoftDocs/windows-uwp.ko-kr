...
- **리뷰 내용**: **등급 사용 하 여 내용을 검토 하**는 유일한 등급 검토 콘텐츠로 표시 됨을 의미 하는 기본 설정 됩니다. **모든** 표시 모든 평점, 리뷰 텍스트를 포함 하지 않는를 선택할 수 있습니다. 참고 **평점 분석** 차트 선택에 관계 없이 모든 리뷰를 항상 표시 됩니다.
...
또한 검색 상자를 사용 하 여 특정 단어 또는 구 앱 리뷰에 대 한 검색 합니다. 다른 언어로 작성 된 리뷰만 원래 고객이 작성 된 텍스트를 검토 하는 참고 검색 됩니다. 리뷰 번역 된 텍스트 검색 되지 않습니다.
