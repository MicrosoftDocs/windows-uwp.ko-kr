...
C++/ WinRT는 Microsoft의 권장 되는 대체 합니다 [ C++/CX](/cpp/cppcx/visual-c-language-reference-c-cx?branch=live) 언어 프로젝션 및 [Windows 런타임 C++ 템플릿 라이브러리 (WRL)](/cpp/windows/windows-runtime-cpp-template-library-wrl?branch=live). 전체 목록은 [C +에 대 한 항목 + WinRT](index.md#topics-about-cwinrt) 와 상호 운용 및에서 C + 이식에 대 한 정보를 포함 합니다. + CX 및 WRL 합니다.

> [!IMPORTANT]
> 가장 중요 한 부분 중 C++알아야 할 WinRT 섹션에서 설명 됩니다 [SDK에 대 한 지원 C++/WinRT](#sdk-support-for-cwinrt) 하 고 [Visual Studio 지원에 대 한 C++/WinRT, XAML, VSIX 확장 및 NuGet 패키지](intro-to-using-cpp-with-winrt.md#visual-studio-support-for-cwinrt-xaml-the-vsix-extension-and-the-nuget-package)합니다.
...
버전 10.0.17134.0(Windows 10 버전 1803)을 기준으로 Windows SDK는 자사 Windows API(Windows 네임스페이스의 Windows 런타임 API)를 사용하기 위한 헤더 파일 기반 표준 C++ 라이브러리를 포함합니다.

호환성을 위해 Windows SDK도 함께 `cppwinrt.exe` 도구입니다. 그러나 좋습니다를 대신 설치 및 사용의 가장 최신 버전 `cppwinrt.exe`에 포함 되어 있는 합니다 **Microsoft.Windows.CppWinRT** NuGet 패키지. 해당 패키지 및 `cppwinrt.exe`, 다음 섹션에 설명 되어 있습니다.

## <a name="visual-studio-support-for-cwinrt-xaml-the-vsix-extension-and-the-nuget-package"></a>Visual Studio 지원에 대 한 C++/WinRT, XAML, VSIX 확장 및 NuGet 패키지
Visual Studio 지원에 대 한 Windows SDK 대상 버전 (Windows 10, 버전 1803) 10.0.17134.0 외에도 해야 Visual Studio 2019 또는 Visual Studio 2017 (이상 버전 15.6; 15.7 이상 권장). 설치 해야 하지 않은 이미 설치한 경우에  **C++ 유니버설 Windows 플랫폼 도구** Visual Studio 설치 관리자 내에서 옵션입니다. 및 Windows **설정을** > **업데이트 \& 보안** > **개발자를 위한**을 선택 합니다 **개발자 모드** 옵션 대신 **테스트용으로 앱 로드** 옵션입니다.
...
- VSIX 확장을 사용 하면 C++Visual Studio에서 /WinRT 프로젝트 및 항목 템플릿을 사용 하 여 시작할 수 있도록 C++/WinRT 개발 합니다.
- 또한 하면 Visual Studio 기본 디버그 시각화 (natvis)의 C++/WinRT 형식 프로젝션 유사한 환경을 제공 C# 디버깅 합니다. Natvis는 디버그 빌드에 대해 자동으로 이루어집니다. WINRT_NATVIS 기호를 정의하여 빌드를 릴리스할 수도 있습니다.

에 대 한 Visual Studio 프로젝트 템플릿 C++/WinRT는 다음과 같습니다. 새로 만들 때 C++최신 버전의 설치 된 VSIX 확장을 사용 하 여 /WinRT 프로젝트, 새 C++WinRT 프로젝트를 자동으로 설치/합니다 [Microsoft.Windows.CppWinRT NuGet 패키지](https://www.nuget.org/packages/Microsoft.Windows.CppWinRT/)합니다. 합니다 **Microsoft.Windows.CppWinRT** NuGet 패키지는 제공 C++WinRT 빌드 지원 (MSBuild 속성 및 대상)의 경우 이식 가능한 프로젝트를 개발 컴퓨터와 빌드 에이전트 / (기반이 NuGet만 패키지 및 VSIX 확장 설치 됩니다).
...
> [!NOTE]
> 때문에 C++/WinRT 표준 C + + 17 기능을 사용 하 여, 프로젝트 속성을 설정 하는 NuGet 패키지 **C /C++** > **언어**  >   **C++언어 표준** > **ISO C + + 17 표준을 (/ /std: c + + 17)** Visual Studio에서. 또한 추가 된 [/bigobj](/cpp/build/reference/bigobj-increase-number-of-sections-in-dot-obj-file) 컴파일러 옵션입니다.
> 
> 설정할 수도 있습니다 **준수 모드: 예 (허용 /-)**, 표준 규격 코드를 추가로 제한 하는 합니다. 알고 있어야 할 또 다른 프로젝트 속성은 **C/C++** > **일반** > **경고를 오류로 처리**입니다. 이 속성은 필요에 따라 **예(/WX)** 또는 **아니오(/WX-)** 로 설정하세요. 간혹 `cppwinrt.exe` 도구에서 생성된 소스 파일이 구현체가 추가될 때까지 경고를 생성하는 경우가 있습니다.

체제 위에 설명 된 대로 최대 설정 수를 만들고 빌드, 열거나는 C++/WinRT Visual Studio에서 프로젝트를 배포 합니다.

또는 수동으로 설치 하 여 기존 프로젝트를 변환할 수 있습니다 합니다 **Microsoft.Windows.CppWinRT** NuGet 패키지. 후 설치 (또는 업데이트) 최신 버전의 VSIX 확장을 Visual Studio에서 기존 프로젝트를 열고, 클릭 **프로젝트** \> **NuGet 패키지 관리...** \> **찾아보기**를 입력 하거나 붙여 넣습니다 **Microsoft.Windows.CppWinRT** 검색 상자에서 검색 결과에서 항목을 선택 하 고 클릭 **설치** 해당 프로젝트에 대 한 패키지를 설치 합니다. 얻게 패키지를 추가한 후 C++WinRT MSBuild 호출을 비롯 한 프로젝트를 지원 합니다 `cppwinrt.exe` 도구입니다. 버전 2.0부터를 **Microsoft.Windows.CppWinRT** NuGet 패키지에 포함 된 `cppwinrt.exe` 도구입니다.

가리킬 수 있습니다는 `cppwinrt.exe` 도구는 Windows 런타임 메타 데이터 (`.winmd`) 파일 헤더 파일 기반 표준 생성을 C++ 라이브러리는 *프로젝트* 에서 소비에 대 한 메타 데이터에 설명 된 Api C++ / WinRT 코드입니다. Windows 런타임 메타데이터(`.winmd`) 파일은 Windows 런타임 API 표면을 정식으로 설명하는 방법을 제공합니다. 메타데이터를 `cppwinrt.exe` 가리켜 타사 Windows 런타임 구성 요소에서 구현된 또는 응용 프로그램에서 구현된 모든 런타임 클래스와 함께 사용하기 위한 라이브러리를 생성할 수 있습니다. 자세한 정보는 [C++/WinRT를 통한 API 사용](consume-apis.md)을 참조하세요.
...
Visual Studio의 XAML 디자인에 대 한 노출 지원 C++에 대응 하는 가까운 WinRT는 C#합니다. 한 가지 예외는 합니다 **이벤트** 탭의 **속성** 창입니다. 사용 하 여는 C# 프로젝트, 이벤트 처리기를 추가 하려면 해당 탭을 사용할 수 있습니다 사용 하 여는 C++/WinRT 프로젝트, 시설이 없습니다. 하지만 [C + 대리자를 사용 하 여 이벤트를 처리 + WinRT](handle-events.md) 이벤트 처리기 코드를 추가 하는 방법에 대 한 정보에 대 한 합니다.
...
측면에서 Windows SDK 및 Visual Studio 버전 및 Visual Studio 구성에서 정보를 지원 합니다 [Visual Studio 지원에 대 한 C++/WinRT, XAML, VSIX 확장 및 NuGet 패키지](#visual-studio-support-for-cwinrt-xaml-the-vsix-extension-and-the-nuget-package) 위의 섹션에 적용 됩니다. VSIX 확장의 버전입니다. 동작에 대 한 중요 한 차이점을 설명 하는 아래 정보 및 구성을 사용 하 여 만든 (또는 사용 하려면 업그레이드) 프로젝트 이전 버전입니다.
...
VSIX 확장의 최신 버전에서는 유효 하지 않은 빌드하도록 하려면 프로젝트에 대 한는 `<CppWinRTEnabled>true</CppWinRTEnabled>` 도 없는 경우 속성을 **Microsoft.Windows.CppWinRT** NuGet 패키지가 설치 합니다. 이 구성 사용 하 여 프로젝트 빌드 오류 메시지 생성 "을 C++WinRT VSIX 프로젝트 빌드 지원을 제공 하지 않습니다.  Microsoft.Windows.CppWinRT Nuget 패키지에 프로젝트 참조를 추가 하십시오. "

위에서 설명한 대로 C++/WinRT 프로젝트는 이제 NuGet 패키지를 설치 해야 합니다.
...
에 C++/WinRT 프로그래밍, 표준 사용할 수 있습니다 C++ 언어 기능 및 [표준 C++ 데이터 형식 및 C++/WinRT](std-cpp-data-types.md)&mdash;일부를 포함 하 여 C++ 표준 라이브러리 데이터 형식입니다. 그 밖에도 프로젝션에서 일부 사용자 지정 데이터 형식에 대해서도 알아둘 필요가 있으며, 실제로 사용자 지정 데이터 형식을 사용할 수도 있습니다. 예를 들어 [C++/WinRT 시작](get-started.md)의 빠른 시작 코드 예제에서 [**winrt::hstring**](/uwp/cpp-ref-for-winrt/hstring)을 사용합니다.
