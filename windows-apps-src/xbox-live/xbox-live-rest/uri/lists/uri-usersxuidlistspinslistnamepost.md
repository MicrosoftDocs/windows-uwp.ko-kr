...
ms.locfileid: "4359031"
---
# <a name="post-usersxuidxuidlistspinslistname"></a>POST (/users/xuid(xuid)/lists/PINS/{listname})
쿼리 문자열 매개 변수 **insertIndex**기반 인덱스 목록에 항목을 삽입 합니다. 이러한 Uri에 대 한 도메인은 `eplists.xboxlive.com`.
...
이 호출 된 인덱스는 쿼리 문자열 매개 변수 **insertIndex** (기본값은 0 또는 목록 맨)에 따라 목록에 항목을 삽입 됩니다. 요청 본문에 있는 모든 항목 목록에서 해당 지점에서 삽입 됩니다. **InsertIndex** 기존 목록에 항목 개수 보다 큰 경우 새 항목이 끝에 삽입 됩니다.
...
항목이 *하지* 시작 또는 목록의 끝에 삽입 되 면 **경우-일치: versionNumber** 헤더는 요청에 포함 해야 합니다. 헤더 시작 또는 종료를 삽입 하는 경우에 선택 사항입니다. 있는 경우 헤더 삽입 위치에 관계 없이 유효성이 검사 됩니다. 헤더 **VersionNumber** 목록의 현재 버전 번호입니다. 되지 포함 되어 있으며, 필요한 경우 HTTP 412 (전제 조건 실패)는 다음 현재 목록 버전 번호와 일치 하지 않는 또는 상태 코드가 반환 하 고 응답 본문의 현재 버전 번호를 포함 하는 목록 최신 메타 데이터를 포함 됩니다. 다른에서 무시 되는 다른 클라이언트에서 업데이트를 방지 하는 데입니다.
 
이 호출은 idempotent; 여러 삽입 동일한 데이터를 사용 하 여 반복 해 서 호출 될 수 있습니다. 그러나 목록이 현재 지원 하므로 중복, 반복 해 서 호출 HTTP 400 될 가능성이 반환 코드 됩니다.
