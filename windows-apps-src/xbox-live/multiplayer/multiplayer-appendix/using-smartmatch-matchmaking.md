...
연결을 시작 하기 전에 연결을 함께 입력 하려면 사용자 그룹을 나타낼 매치 메이 킹 "정찰" 일치 티켓 세션을 설정 합니다. 이 그룹의 모든 사용자 **(문자열, 부울, 부울) MultiplayerSession.Join 메서드**를 사용 하 여 세션에 참가 합니다.

티켓 세션을 만들고 플레이어 입력에, 제목 세션 **MatchmakingService.CreateMatchTicketAsync 메서드**를 사용 하 여 연결 서비스를 제출 합니다. 이 메서드는 티켓 세션을 나타내는 "검색"을 티켓 세션에서 /servers/matchmaking/properties/system/status 필드를 업데이트 하는 일치 티켓을 만듭니다. 자세한 내용은 참조 [하는 방법: 일치 티켓 만들기](multiplayer-how-tos.md)합니다.

일치 티켓 생성 메서드의 응답 **CreateMatchTicketResponse 클래스** 개체입니다. 응답 일치 티켓 ID가 포함 되어 사용할 수 있는 GUID를 사용 하 여 티켓을 삭제 하 여 연결을 취소할 수 있습니다. 응답에는 사용자 설정에 사용할 수 있는 hopper에 대 한 평균 대기 시간이 포함 됩니다.


## <a name="setting-matchmaking-attributes-on-the-session-and-players"></a>세션 및 플레이어에 매치 메이 킹 특성을 설정합니다.

연결에 대 한 세션을 제출할 때 제목 다른 세션을 사용 하 여 세션을 그룹화 할 매치 메이 킹 서비스가 사용 하는 특성을 설정할 수 있습니다. 티켓 수준 또는 구성원 단위 수준에서 특성을 지정할 수 있습니다.


### <a name="setting-matchmaking-attributes-at-the-match-ticket-level"></a>매치 메이 킹 특성 일치 티켓 수준 설정

제목은 **MatchmakingService.CreateMatchTicketAsync** 메서드의 *ticketAttributesJson* 매개 변수에서 일치 티켓 수준에서 특성을 제출합니다. 티켓 수준에서 지정 된 특성 구성원 단위 수준에서 지정 된 같은 특성을 재정의 합니다.
...
티켓 세션와 일치 하는 티켓 다른 그룹을 나타내는 다른 티켓 세션 표현된 티켓 세션 매치 메이 킹 서비스의 일치 항목을 설정 하 고 만들거나 일치 대상 세션을 식별 합니다. 서비스도 대상 세션에서 일치 하는 플레이어에 대 한 예약을 작성 하 고 일치 하는 것으로 다음 티켓 세션을 표시 합니다. MPSD 티켓 세션에 이러한 변경의 제목을 알립니다.

이제 제목 해야 다음, 충분 한 플레이어에 나타나는지 확인 하기 위해 대상 세션을 초기화 하는 단계를 수행 및 품질의 QoS (서비스) 검사를 성공적으로 연결할 수 있는를 수행 합니다. 초기화 및/또는 QoS 실패할 경우 제목 다른 그룹을 찾을 수 있도록 연결을 다시 제출에 대 한 티켓 세션을 표시 합니다. 프로세스에 대 한 자세한 내용은 [대상 세션 초기화 및 QoS를](smartmatch-matchmaking.md)참조 하세요.

일치 활동 동안 다음과 같은 변경 내용은 세션에 대 한 JSON 개체 적용 됩니다.

-   "발견"로 설정 하는 /servers/matchmaking/properties/system/status 필드
...
매치 메이 킹 서비스 티켓 세션의 스냅숏을 사용 하 여 세션에 대 한 일치 티켓 생성 될 때 한 번에 있습니다. 따라서 모든 플레이어에 가입 하거나 티켓 세션을 제목은 삭제 하 고 다시 일치 티켓 매치 메이 킹 서비스를 사용 해야 합니다.
...
| 중요                                                                                                                                                                                                                       |
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| *PreserveSession* 를 사용 하 여 두 개의 세션을 항상 설정 수 없음을 결합 될 수 있으므로, 서로 일치 시킬 수 있다는 것을 것입니다. 제목에서 사용 하는 연결 흐름 고려이 취해야 합니다. |

제목을 이미 진행 중인 게임에 참가 하려면 더 많은 플레이어를 찾으려고 일치 티켓 세션으로 기존 게임 세션을 다시 사용할 수 있습니다. 이렇게 하려면 제목 **CreateMatchTicketAsync** **PreserveSessionMode.Always**설정 *preserveSession* 매개 변수를 사용 하 여 호출 하 여 일치 티켓을 만들어야 합니다. 매치 메이 킹 서비스 티켓에 사용 되는 기존 세션 매치 메이 킹 프로세스 내내 유지 하 고 결과 대상 세션 수를 확인 합니다.
...
Xbox Live 계산 기반 게임 사용자 matchmade 갖도록 하는 개략적인 단계는 다음과 같습니다. 제 3 자에서 호스팅되는 게임에 대 한 유사한 흐름 적용 해야 합니다.
...
| 참고                                                                                                                                                                                                         |
|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 티켓 세션 개체 일치 하지 않는 상수 있으면 만들기 티켓 메서드를 완료할 수 없습니다. 필수인을 추가 하 여 방지할 수 있습니다 규칙 상수를 일치 하는 플레이어와 일치 하는 것을 방지 하려면 hopper입니다. |

사용 안 함에 **MatchTicketDetailsResponse.PreserveSession 속성** 을 설정 하는 경우 각 구성원에서 서버 측정 티켓의 내부 표현에 복사 매치 메이 킹 서비스 합니다. "특수" 티켓 특성으로 티켓의 내부 표현에 저장 된 티켓을 단일 서버 측정 컬렉션으로 티켓의 구성원 서버 측정을 병합 합니다.
...
5.  매치 메이 킹 서비스 계정으로 측정 컬렉션 서버에 수행할 다른 그룹을 나타내는 다른 사용자와 티켓 세션을 찾습니다. 기본으로 매우 동일한 데이터 센터에 있는 다른 그룹을 사용 하 여 그룹 일치 하려고 합니다.
...
8.  일부 또는 모든 클라이언트 **GameServerPlatformService.AllocateClusterAsync 메서드**를 호출 합니다. 첫 번째는 승인 다른 수신 하는 동안 할당을 트리거합니다. 메서드는 MPSD에서 대상 세션을 가져오고 대상 세션 뿐만 아니라 부하 및 기타 Xbox Live 계산 관련 기술 데이터 센터 기본 설정에 따라 데이터 센터를 선택 합니다.
9.  모든 클라이언트를 재생합니다.
