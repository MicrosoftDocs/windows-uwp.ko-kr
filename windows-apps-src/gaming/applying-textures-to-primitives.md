...
이 항목에서는 최대한 활용해 보세요를 가져오려면 c + +를 잘 알고 있어야 해야 합니다. 그래픽 프로그래밍 개념 기본 경험도 필요 합니다. 와 함께 이미 따랐는지 해야 하는 것이 이상적 [빠른 시작: DirectX 리소스 설정 및 이미지 표시](setting-up-directx-resources.md), [작성 셰이더 및 그리기 원형](creating-shaders-and-drawing-primitives.md)및 [원형에 사용 하 여 깊이 및 효과](using-depth-and-effects-on-primitives.md).
...
이 항목에서는 원시 텍스처 데이터를 로드 하 고 3D 원형에 해당 데이터를 적용 합니다.