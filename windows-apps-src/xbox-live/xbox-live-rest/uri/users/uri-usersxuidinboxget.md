...
| continuationToken| 문자열|  | 이전 열거형 호출;에서 반환 된 문자열 계속 열거 하는 데 사용 합니다.|
...
| --- | --- | --- | --- |
| 머리글| 헤더|  | 사용자가 메시지 헤더|
| messageSummary| string| 20| U T F-8입니다. 메시지의 대개 처음 20 자|
...
| hasText| 부울|  | 메시지 텍스트를 포함 되어 있는지 여부입니다.|

#### <a name="paging-info"></a>페이징 정보

| 속성| 형식| 최대 길이| 설명|
| --- | --- | --- | --- |
| continuationToken| 문자열| 100| 필요에 따라 서버에서 반환 합니다. 열거형을 계속 하려면 이후 호출을 허용 합니다.|
