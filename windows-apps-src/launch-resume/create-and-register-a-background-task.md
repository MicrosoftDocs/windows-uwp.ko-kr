...
> [!NOTE]
> 백그라운드 작업 클래스 자체&mdash;및 백그라운드 작업 프로젝트의 다른 모든 클래스가&mdash; **봉인** (또는 **마지막**)는 **공용** 클래스 여야 합니다.
...
4.  백그라운드 작업에서 비동기 코드를 실행할 경우 백그라운드 작업은 deferral을 사용해야 합니다. 지연을 사용 하지 않으면, 다음 백그라운드 작업 프로세스가 수 예기치 않게 종료 하는 비동기 작업 실행이 완료 되기 전에 **Run** 메서드가 반환 하는 경우.
...
1.  백그라운드 작업이 [**BackgroundTaskRegistration.AllTasks**](https://msdn.microsoft.com/library/windows/apps/br224787) 속성을 반복 하 여 이미 등록 되어 있는지 확인 합니다. 이 단계는 중요합니다. 앱이 기존 백그라운드 작업 등록을 확인하지 않는 경우 작업을 여러 번 등록하기 쉬우므로 성능에 문제가 발생하거나 작업이 완료되기 전에 작업의 가용 CPU 시간을 모두 사용할 수 있습니다.
