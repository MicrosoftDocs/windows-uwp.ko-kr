...
ms.locfileid: "5921814"
---
# <a name="post-itemid"></a>POST ({itemID})
소모 성 인벤토리 항목의 일부 또는 모두 사용 되었는지 나타냅니다 점감 수량을 요청 된 양만큼 소모 성입니다.
...
## <a name="remarks"></a>설명
   ...
   * 호출자를 사용 하 라는 메시지가 표시 수량 양의 정수 0 이어야 합니다. 소비 값이 0 또는 더 낮은 호출 거부 됩니다.
   * 호출자는 빈 트랜잭션 ID를 제공 하는 경우 요청이 거부 됩니다.
   * 사용 가능한 경우 어떤 제목 소비를 보고 하는 확인을 수행할 수 있도록 제목 클레임에 기록 됩니다.
...
제거 수량 필드 소모의 나머지 수량에서 제거 하고자 하는 소모 성 수량을 나타내기 위해 호출자 수 있습니다. 거래 ID 필드는 소모 성 콘텐츠 작업을 사용 하 여 동일한 사용을 두 번 계산의 위험을 제한 하는 동안 다시 시도 하는 수단을 사용 하 여 호출자에 게를 제공 합니다.
