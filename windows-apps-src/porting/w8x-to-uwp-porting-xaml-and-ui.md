...
참와 통합 되는 코드를 변경할 필요가 없습니다 하지만 앱이 Windows10 셸의 일부가 아닙니다 참 막대의 위치를 일부 UI를 추가 해야 합니다. Windows10에서 실행 되는 유니버설 8.1 앱에 고유한 대체 UI 앱의 제목 표시줄에는 시스템 렌더링 크롬에서 제공 합니다.

## <a name="controls-and-control-styles-and-templates"></a>컨트롤과 컨트롤 스타일 및 템플릿

8.1 모양 및 동작 컨트롤에 대해 Windows10에서 실행 되는 유니버설 8.1 앱은 유지 됩니다. 하지만 모양 및 동작 고려해 야 할 몇 가지 차이점이 있습니다 Windows10 앱에 해당 앱에 포팅하는 경우. 아키텍처 및 디자인 컨트롤의 [디자인 언어](#design-language-in-windows-10), 간소화 및 유용성 향상 주위 대부분은 변경 Windows10 앱의 경우 기본적으로 변경 되지 않습니다.

**참고**  PointerOver 시각적 상태는 사용자 지정 스타일/템플릿에서 Windows10 앱 및 Windows 런타임 8.x 앱 하지만 Windows Phone 스토어 앱에 없습니다. 이러한 이유로 (그리고 Windows10 앱에 대 한 지원 되는 시스템 리소스 키 때문), 앱 Windows10 포팅할 경우 Windows 런타임 8.x 앱에서 사용자 지정 스타일/템플릿에서 다시 사용 하는 것이 좋습니다.
확인 하려는 경우 사용자 지정 스타일/템플릿이 최신 시각적 상태 집합을 사용 하는 기본 스타일/템플릿에 수행한 성능 개선 으로부터 이점을 얻도록 다음 새 Windows10 기본 템플릿의 복사본을 편집 및 다시 적용 하면 사용자를 지정 합니다. 성능 개선의 한 가지 예는 이전에 **ContentPresenter** 또는 패널을 묶은 **Border**가 제거되었으며 이제 자식 요소가 테두리를 렌더링한다는 점입니다.
...
| [**CommandBar**](https://msdn.microsoft.com/library/windows/apps/hh701927) | Windows 런타임 8.x 앱에서 [**CommandBar**](https://msdn.microsoft.com/library/windows/apps/hh701927) 의 보조 명령이 항상 표시 됩니다. Windows Phone 스토어 앱 및 Windows10 앱의 명령 모음 열립니다 될 때까지 표시 되지 않습니다. |
| [**CommandBar**](https://msdn.microsoft.com/library/windows/apps/hh701927) | Windows Phone 스토어 앱의 경우 [**CommandBar.IsSticky**](https://msdn.microsoft.com/library/windows/apps/hh701944) 값은 명령 모음이 빠른 해제가 가능한지 여부에 영향을 주지 않습니다. **IsSticky** true 이면 **CommandBar** 로 설정 된 경우 응용 프로그램을 Windows10에 대 한 빠른 해제 제스처로 무시 합니다. |
...
사용할 수도 있습니다 다시 Windows 런타임 8.x 앱에서 XAML 태그 리터럴 차원 값이 태그 (크기 모양 또는 기타 요소, 입력 체계)에 사용 되는 위치입니다. 있지만 경우에 따라 큰 배율 인수가 유니버설 8.1 앱 보다 Windows10 앱에 대 한 장치에서 사용 됩니다 (예를 들어 150%는 이전에 140% 였던 하며 200%는 180% 였던 위치 사용 됩니다). 따라서 이러한 리터럴 값은 이제 Windows10에 비해 너무 큰 경우 크다고 보세요 0.8로 곱해 야 합니다. 자세한 내용은 [UWP 앱용 반응형 디자인 101](https://msdn.microsoft.com/library/windows/apps/dn958435)을 참조하세요.
...
다음은 [**GridView**](https://msdn.microsoft.com/library/windows/apps/br242705) 측면 있으며 있는 [**ListView**](https://msdn.microsoft.com/library/windows/apps/br242878) 변경 하거나 Windows10에서는 지원 되지 않습니다.
...
Windows 런타임 8.x 앱에서 앱이 비활성화 되 고 다시 활성화 때 (표시 되 고) 하는 경우 축소 보기가 사라집니다 하 고 확대 보기 대신 표시 됩니다. Windows Phone 스토어 앱 및 Windows10 앱에서는 표시 중인 경우 축소 보기 표시 유지 됩니다.
...
텍스트에 대 한 기본 언어는 빌드의 언어 또는 en Windows 런타임 8.x 앱 및 Windows Phone 스토어 앱에서 설정-주세요. Windows10 앱 기본 언어 (글꼴 대체) 상위 앱 언어로 설정 되어 있습니다. [**FrameworkElement.Language**](https://msdn.microsoft.com/library/windows/apps/hh702066)를 명시적으로 설정할 수 있지만, 해당 속성에 대해 값을 설정하지 않은 경우 더 나은 글꼴 대체 동작을 사용할 수 있습니다.
