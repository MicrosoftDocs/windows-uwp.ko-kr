...
1.  C#에서는 백그라운드 클래스의 Run 메서드에 중단점을 삽입(단일 프로세스 백그라운드 작업의 경우 App.OnBackgroundActivated()에 중단점 삽입)하거나 [**System.Diagnostics**](https://msdn.microsoft.com/library/windows/apps/xaml/hh441592.aspx)를 사용하여 디버깅 출력을 씁니다.

    C++에서는 백그라운드 클래스의 Run 함수에 중단점을 삽입(단일 프로세스 백그라운드 작업의 경우 App.OnBackgroundActivated()에 중단점 삽입)하거나 [**OutputDebugString**](https://msdn.microsoft.com/library/windows/desktop/aa363362)를 사용하여 디버깅 출력을 씁니다.
...
> [!NOTE]
> 이 섹션은 별도 프로세스에서 실행되는 특정 백그라운드 작업과 관련이 있으며 단일 프로세스 백그라운드 작업에는 적용되지 않습니다.
