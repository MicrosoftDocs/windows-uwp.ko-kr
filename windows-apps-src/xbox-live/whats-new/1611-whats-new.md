...
* [세션 찾아보기](../multiplayer/session-browse.md) 멀티 플레이어 게임을 찾을 수 사용자에 게는 새로운 방법을입니다.  세션 검색 플레이어를 지정 된 조건을 충족 하는 열린 멀티 플레이 게임 세션의 목록을 검색할 수 있습니다.
* [멀티 플레이어 관리자](../multiplayer/multiplayer-manager.md) 는 자동 채우기 기능 되었습니다.  사용 하도록 설정 하면 멀티 플레이어 관리자 게임 플레이 하는 동안 열린 슬롯 채우기 매치 메이 킹 통해 구성원 찾을 있습니다.
...
* 다양 한 [소셜 관리자](../social-platform/intro-to-social-manager.md) API 변경 내용:
    ...
    * `xbox_social_user`
        ...
        * 6 단계 중에도 사용자 지정 존재 기록 `social_manager_presence_title_record` 당 `social_manager_presence_record`
    * `social_event`
    ...
    * `xbox_social_user_group`
        * `users()` 이제 반환 API는 `std::vector<xbox_social_user*>`. 이러한 포인터에 대 한 다음 호출에서 무효화 됩니다. `social_manager::do_work()`
...
* C + + WinRT 지원 기능이 추가 되었습니다.   세부 정보를 찾을 수 [여기](../introduction-to-xbox-live-apis.md)
...
* [Xbox Live 소개](../get-started-with-partner/get-started-with-xbox-live-partner.md) 섹션 재작성 했습니다.  새 문서를 볼 수 새 Xbox Live가 지원 제목을 만드는 하거나는 게임에 Xbox Live 기능을 통합 하는 방법에 대 한 내용은 [다음과 같습니다](../get-started-with-partner/get-started-with-xbox-live-partner.md).
