...
Microsoft 빌드 2018 회의에서 발표 MSIX는 Win32, Windows Forms, WPF 및 UWP를 비롯 한 모든 Windows 응용 프로그램에 적용 되는 새 컨테이너 화 패키지 형식입니다. 이 새로운 형식에서 UWP 기능을 상속합니다.
...
우리의 Get revitalized 했습니다에서는 새 학습 트랙을 사용 하 여 콘텐츠를 시작 합니다. 이러한 새 항목 수행 하려는 몇 가지 일반적인 작업에 대 한 정보를 사용 하 여 새로운 Windows 10 개발자에 게 제공을 목표로 합니다. 바로 자습서 들지 및 핸드 연습을 제공 하지 않는 있지만 기존 설명서가 존재 하는 위치 및 사용 하는 방법 대신 지적 합니다. [코딩 시작](../get-started/create-uwp-apps.md) 개선 확인 페이지 또는 각 개별 학습 트랙을 살펴보세요.
...
이제 파트너 센터에서 [광고 성과 보고서](../publish/advertising-performance-report.md) 가시성 메트릭을 제공합니다. [광고 단위 가시성 최적화](../monetize/optimize-ad-unit-viewability.md) 문서 광고의 가시성을 최적화 하기 위한 권장 사항을 제공 하기 위해 추가 했습니다.
...
이 자습서의 고객 목록을 관리 하기 위한 기본 UWP 앱을 만들고 개념 및 엔터프라이즈 개발에 유용한 방법을 소개 합니다. UI 요소를 구현 하 여 로컬 SQLite 데이터베이스에 대해 작업을 추가 검색 하 고 더 나아가 하려는 경우 원격 REST 데이터베이스에 연결 하기 위한 느슨한 지침을 제공 합니다. [여기에 자습서를 확인](../enterprise/customer-database-tutorial.md)