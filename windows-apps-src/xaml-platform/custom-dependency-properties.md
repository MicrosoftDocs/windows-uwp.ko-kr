...
Windows 런타임에서의 종속성 속성의 예로: [**Control.Background**](https://msdn.microsoft.com/library/windows/apps/br209395)를 [ **FrameworkElement.Width**](/uwp/api/Windows.UI.Xaml.FrameworkElement.Width), 및 [ **TextBox.Text**](https://msdn.microsoft.com/library/windows/apps/br209702), 여러 간에 다른 방법을 사용 합니다.
...
> [!WARNING]
> 예외적인 상황을 모두 제외 하 고 래퍼 구현은 수행 해야 합니다 [ **GetValue** ](https://msdn.microsoft.com/library/windows/apps/br242359) 하 고 [ **SetValue** ](https://msdn.microsoft.com/library/windows/apps/br242361) 작업입니다. 그렇지 않으면 속성이 XAML을 통해 설정되는 경우와 코드를 통해 설정되는 경우에 동작이 달라집니다. 효율성을 위해 XAML 파서는 종속성 속성을 설정할 때 래퍼를 무시하고 **SetValue**를 통해 백업 저장소에 통신합니다.
...
> [!NOTE]
> 정의 하는 경우는 [ **CreateDefaultValueCallback** ](https://msdn.microsoft.com/library/windows/apps/hh701812) 구현에 유틸리티 메서드를 사용 해야 [ **PropertyMetadata.Create** ](https://msdn.microsoft.com/library/windows/apps/hh702099)호출 하는 대신는 [ **PropertyMetadata** ](https://msdn.microsoft.com/library/windows/apps/br208771) 정의 하는 생성자를 **PropertyMetadata** 인스턴스.
...
> [!NOTE]
> 기본값을 사용 하 여 등록 하지 마십시오 [ **UnsetValue**](https://msdn.microsoft.com/library/windows/apps/br242371)합니다. 등록하는 경우 속성 소비자를 혼란스럽게 하고 속성 시스템 내에 의도하지 않은 결과를 발생시킵니다.
...
Windows 런타임 API에서 컬렉션 형식 종속성 속성은 상대적으로 자주 사용되지 않습니다. 대부분의 경우 항목이 [**DependencyObject**](https://msdn.microsoft.com/library/windows/apps/br242356) 서브클래스인 컬렉션을 사용할 수 있으나 컬렉션 속성 자체는 기본 CLR 또는 C++ 속성으로 구현됩니다. 종속성 속성이 관련된 몇 가지 일반적인 시나리오에 컬렉션이 반드시 적합한 것은 아니기 때문입니다. 예를 들어 다음과 같은 가치를 제공해야 합니다.
