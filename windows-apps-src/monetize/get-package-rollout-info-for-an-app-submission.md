---
author: Xansky
description: Microsoft Store 제출 API에서 이 메서드를 사용하여 앱 제출에 대한 패키지 출시 정보를 받습니다.
...
| 이름        | 유형   | 설명                                                                 |
|---------------|--------|-----------------------------------------------------------------------------|
| applicationId | string | 필수. 가져올 패키지 출시 정보가 있는 제출을 포함하는 앱의 스토어 ID입니다. 스토어 ID에 대한 자세한 내용은 [앱 ID 세부 정보 보기](https://msdn.microsoft.com/windows/uwp/publish/view-app-identity-details)를 참조하세요.  |
| submissionId | string | 필수. 가져올 패키지 출시 정보가 있는 제출 ID입니다. 이 ID는 [앱 제출 만들기](create-an-app-submission.md) 요청에 대한 응답 데이터에서 사용할 수 있습니다. 개발자 센터 대시보드에서 만든 제출의 경우 이 ID는 대시보드에 있는 제출 페이지의 URL에도 사용할 수 있습니다.  |
...
다음 예제에서는 앱 제출에 대한 패키지 출시 정보를 가져오는 방법을 보여 줍니다.
...
다음 예제에서는 패키지 출시를 점진적으로 사용하도록 설정한 앱 제출에 대해 이 메서드를 성공적으로 호출하기 위한 JSON 응답 본문을 보여 줍니다. 응답 본문의 값에 대한 자세한 내용은 [패키지 출시 리소스](manage-app-submissions.md#package-rollout-object)를 참조하세요.
...
앱 제출에서 점진적인 패키지 출시를 사용할 수 없는 경우 다음 응답 본문이 반환됩니다.
...
|--------|------------------|
| 404  | 제출을 찾을 수 없습니다. |
| 409  | 제출이 지정된 앱에 속하지 않거나 앱이 [현재 Microsoft Store 제출 API에서 지원되지 않는](create-and-manage-submissions-using-windows-store-services.md#not_supported) 개발자 센터 대시보드 기능을 사용합니다. |   
