...
이 호출은는 쿼리 문자열 매개 변수 **insertIndex** (기본값은 0 또는 목록 맨)에 따라 인덱스 목록에 항목을 삽입 됩니다. 요청 본문에 있는 모든 항목 목록에서 해당 지점에 삽입 됩니다. **InsertIndex** 기존 목록에 항목 개수 보다 큰 경우 새 항목이 끝에 삽입 됩니다.
 
항목을 삽입 하는 기능 사양;에 표시 된 필수 필드 있어야 합니다. 그렇지 않은 경우 HTTP 400 반환 됩니다. 마찬가지로, 목록 (목록 종류에 따라 정의 됨)의 최대 크기를 초과 하는 삽입의 결과 다음 HTTP 400 반환 되 고 아무 삽입 됩니다.
 
항목이 *하지* 시작과 목록의 끝에 삽입 되 면 **경우-일치: versionNumber** 헤더는 요청에 포함 해야 합니다. 헤더 삽입 시작과 끝에 대 한 경우에 선택 사항입니다. 있는 경우 헤더 삽입 위치와 상관 없이 유효성이 검사 됩니다. 헤더 **VersionNumber** 목록의 현재 버전 번호입니다. 되지 포함 되어 있으며, 필요한 경우 HTTP 412 (전제 조건 실패) 다음 현재 목록 버전 번호와 일치 하지 않는 또는 상태 코드가 반환 하 고 응답 본문에는 현재 버전 번호를 포함 하는 목록의 최신 메타 데이터를 포함 됩니다. 다른에서 무시 되는 다른 클라이언트에서 업데이트를 방지 하기 위해서입니다.
 
이 호출은 idempotent; 여러 삽입 동일한 데이터를 사용 하 여 반복 해 서 호출 될 수 있습니다. 그러나 목록이 현재 지원 하므로 중복, 반복 해 서 호출 HTTP 400 될 가능성이 코드 반환 됩니다.
...
| 매개 변수| 유형| 설명| 
| --- | --- | --- | --- | --- | --- | 
| insertIndex| string| 선택 사항입니다. 항목을 삽입 위치를 정의 합니다. 값을 지원: 0, 양의 정수 및 "종료"입니다. 목록 항목의 개수 보다 큰 인덱스 값 목록 맨 아래에 새 항목을 추가 합니다 및 "공백" 목록에 삽입 되지 않습니다. 기본값: 0입니다.| 
