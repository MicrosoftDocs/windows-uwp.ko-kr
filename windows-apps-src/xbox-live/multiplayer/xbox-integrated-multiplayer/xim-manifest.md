...
XIM을 사용 하는 모든 Xbox One 단독 리소스 응용 프로그램에 특정 한 소켓 설명 확인 해야 하 고 AppXManifest의 해당 "네트워크 매니페스트" 확장에 템플릿 콘텐츠가 포함 된 (네트워크 매니페스트에 대 한 자세한 내용은 플랫폼 설명서를 참조). 앱도 다른 템플릿에 있을 수 있지만 다음 코드 조각은 있어야 합니다. 그렇지 XIM 네트워크에 이동 하지 못합니다.
...
XIM을 사용 하는 모든 유니버설 Windows 응용 프로그램에 특정 한 소켓 설명 확인 해야 하 고 템플릿 콘텐츠가 해당 "네트워크 매니페스트" 파일에 포함 된 (패키지 루트 디렉터리에 networkmanifest.xml 파일 참조 플랫폼 설명서 대 한 자세한 내용은 네트워크 매니페스트). 앱도 다른 템플릿에 있을 수 있지만 다음 코드 조각은 있어야 합니다. 그렇지 XIM 네트워크에 이동 하지 못합니다.
