...
**무료** 또는 사용 가능한 기준 가격 중 하나를 선택하여 앱의 기본 가격을 선택해야 합니다([표시 여부](choose-visibility-options.md#discoverability) 섹션의 **Store에서 사용할 수 있지만 검색되지 않는 앱으로 설정** 아래에서 **취득 중지** 옵션을 선택하지 않는 한). 또 가격 변경 예약으로 앱 가격이 변경되는 날짜와 시간을 지정할 수 있습니다. 뿐만 아니라 특정 시장에서 가격 변경을 사용자 지정할 수 있는 옵션도 있습니다. 
...
선택 **한시적** 고객이 특정 기간 (일)에 대 한 앱을 무료로 사용해 수 있도록 합니다. **1 일**, **7 일**를 **15 일**, 또는 **30 일 동안**합니다. [평가판의 기능을 제한하거나 없애는](../monetize/in-app-purchases-and-trials.md) 코드를 추가해 기능을 제한할 수 있습니다. 또는 평가 기간에 고객이 모든 기능에 액세스할 수 있도록 구성할 수 있습니다. 
> [!NOTE]
> 기간 한정 평가판은 Windows 10 빌드 10.0.10586에서 고객에 게 표시 되지 않습니다 앞 또는 Windows Phone 8.1 및 이전 버전의 고객에 게 합니다.
