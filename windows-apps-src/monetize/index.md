...
| [Microsoft Store Services SDK](microsoft-store-services-sdk.md)      | Microsoft Store Services SDK는 더 많은 수익을 창출하고 고객과의 소통을 도와주는 기능을 앱에 추가할 때 사용하는 라이브러리 및 도구를 제공합니다. 이러한 기능에는 앱에서 바로 광고, A/B 테스트 및 앱에서 피드백 허브 시작이 포함됩니다. |
...
| [앱에서 피드백 허브 시작](launch-feedback-hub-from-your-app.md)      |   UWP 앱에 코드를 추가하여 문제, 제안 및 좋아요를 제출할 수 있는 피드백 허브로 Windows 10 고객을 안내합니다. 그런 다음, Windows 개발자 센터 대시보드의 [피드백 보고서](../publish/feedback-report.md)에서 이 피드백을 관리합니다. 이 기능은 Microsoft Store Services SDK가 필요합니다.   |
...
| [개발자 센터에 대한 사용자 지정 이벤트 로깅](log-custom-events-for-dev-center.md)  | UWP 앱에서 사용자 지정 이벤트를 기록하고 Windows 개발자 센터 대시보드의 [사용 보고서](../publish/usage-report.md)에서 이벤트를 검토합니다. 이 기능은 Microsoft Store Services SDK가 필요합니다. |
| [앱에서 광고 표시](display-ads-in-your-app.md)      |   앱에 배너 광고 또는 동영상 중간 광고를 게재하여 수익을 늘리세요. 이 기능은 Microsoft Store Services SDK가 필요합니다.   |
| [Windows 스토어 서비스](using-windows-store-services.md)    |  RESTful API를 사용하여 스토어 제출을 자동화하고 앱 분석 데이터에 액세스하며 기타 스토어 관련 작업을 자동화하는 방법을 알아봅니다.    |
| [RDX(소매 데모 환경) 앱 만들기](retail-demo-experience.md)        |  소매점에서 PC 및 모바일 데모 장치에 설치할 수 있는 소매 데모 환경 앱을 만드는 방법을 알아봅니다.  |
