...
예를 들어 사용자가 목적지를 입력하고, 앱 이름 다음에 &lt;목적지&gt;에 대한 여행 표시라고 말하여 앱을 시작할 수 있는 여행 앱이 있다고 가정해 보겠습니다. **ListenFor** 요소 자체에서 `<ListenFor> Show trip to {destination}  </ListenFor>`라고 말할 수 있습니다. 여기서 "destination"은 **PhraseList**의 **Label** 특성 값입니다.
