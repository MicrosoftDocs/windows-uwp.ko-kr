...
이 가이드에서는 UWP(유니버설 Windows 플랫폼)과 Windows 10에 대해 알아봅니다.

-   디바이스 패밀리 무엇이며 대상 디바이스 패밀리를 결정하는 방법
-   확장 SDK란 무엇이며 API가 디바이스의 특정 클래스에 액세스하도록 지원하는 방법.
-   UI를 다른 화면 크기 또는 방향에 맞추기 위한 새로운 UI 컨트롤 및 패널.
-   앱에 사용할 수 있는 API 표면을 이해하고 제어하는 방법.

Windows 10에는 Windows 10을 실행하는 모든 디바이스에서 사용할 수 있는 공통 앱 플랫폼인 UWP(유니버설 Windows 플랫폼)가 도입되었습니다. UWP는 디바이스 간에 보장된 핵심 API를 제공합니다. 따라서 광범위한 디바이스에 설치할 수 있는 단일 앱 패키지를 만들 수 있습니다. 또한 단일 앱 패키지를 통해 Windows 스토어에서는 앱을 실행할 수 있는 모든 디바이스 유형에 연결되는 통합 배포 채널을 제공합니다. UWP를 대상으로 하는 앱은 모든 디바이스에 일반적인 WinRT API뿐만 아니라 앱이 실행되는 디바이스 클래스에 특정한 API(Win32 및 .NET API 포함)를 호출할 수 있습니다.
...
UWP 앱은 폼 팩터 및 입력 형식이 서로 다른 광범위한 디바이스에서 실행되므로 각 디바이스에 맞게 조정하고 각 디바이스의 고유한 기능을 활용할 수 있습니다. 보장된 핵심 API 계층에 추가로, 다른 디바이스에서 다른 환경을 제공하는 동안 앱이 하나의 장치 유형에 특정한 기능을 사용할 수 있도록 디바이스별 API에 액세스하는 코드를 작성할 수 있습니다. 적응형 UI 컨트롤 및 새 레이아웃 패널은 광범위한 디바이스 화면 해상도와 크기에서 UI를 조정하도록 도와줍니다.

## <a name="device-families"></a>디바이스 패밀리

Windows 10이 어떻게 여러 디바이스 클래스를 대상으로 삼을 수 있도록 지원하는지 알아보려면, 디바이스 패밀리라는 개념을 이해하면 유용합니다. 디바이스 패밀리는 여러 디바이스 클래스에서 기대할 수 있는 API, 시스템 특성 및 동작을 식별합니다. 또한 스토어에서 앱을 설치할 수 있는 디바이스 집합을 결정합니다. 디바이스 패밀리(유니버설 디바이스 패밀리는 예외)는 확장 SDK로 구현되며, 이에 대해서는 곧 설명하겠습니다. 디바이스 패밀리 계층 구조는 다음과 같습니다.

![디바이스 패밀리](images/device-family-tree.png)

장치 패밀리는 API 집합과 버전을 정의합니다. 장치 패밀리는 OS의 기초입니다. PC에서는 데스크톱 장치 패밀리를 기반으로 하는 데스크톱 OS를 실행합니다. 휴대폰은 모바일 장치 패밀리를 기반으로 하는 모바일 OS를 실행합니다.
...
유니버설 디바이스 패밀리의 한 가지 이점은 휴대폰, 태블릿, 데스크톱 컴퓨터, Surface Hub, Xbox 콘솔, HoloLens 등의 다양한 디바이스 중 하나 또는 모든 디바이스에서 앱을 실행할 수 있다는 점입니다. 또한 앱에서 적응형 코드를 사용하여 유니버설 디바이스 패밀리에 속하지 않는 디바이스의 기능을 동적으로 검색하고 사용할 수 있습니다.
...
앱의 대상 장치 범위를 극대화하고 최대한 많은 장치에서 실행하려면 앱의 대상을 유니버설 디바이스 패밀리로 지정합니다. 이렇게 하면 앱이 자동으로 유니버설을 기반으로 하는 모든 디바이스 패밀리(다이어그램에서 모든 유니버설의 자식)를 대상으로 지정합니다. 즉, 이러한 디바이스 패밀리를 기반으로 하는 모든 OS와 해당 운영 체제를 실행하는 모든 장치에서 앱이 실행됩니다. 이러한 모든 디바이스에서 사용할 수 있는 API는 대상으로 지정한 유니버설 디바이스 패밀리의 특정 버전에 정의된 API 집합뿐입니다(이 릴리스의 경우 앱이 대상 디바이스 패밀리 버전 외 API를 호출하는 방법을 알아보려면 이 항목 뒷부분의 [코드 작성](#writing-code)을 참조하세요.
...
앱이 대상으로 하는 디바이스 패밀리를 결정하면, 해당 디바이스 패밀리에 대한 API를 구현하는 확장 SDK에 대한 참조를 추가합니다.  유니버설 디바이스 패밀리를 대상으로 하는 경우, 확장 SDK를 참조할 필요가 없습니다. 하지만 유니버설이 아닌 디바이스 패밀리를 대상으로 하는 경우, Visual Studio에서 선택한 디바이스 패밀리와 일치하는 확장 SDK에 대한 참조를 추가합니다.  예를 들어, 모바일 디바이스 패밀리를 대상으로 하는 경우, Visual Studio의 참조 관리자에서 _UWP용 Windows 모바일 확장_에 대한 참조를 추가합니다.

디바이스 패밀리를 선택하는 것이 다른 유형의 디바이스에 대한 확장 SDK를 추가하는 것을 제한하지는 않습니다. 아래 [코드 작성](#writing-code)에 설명된 대로 선택된 디바이스 패밀리에 포함되지 않은 API의 존재를 테스트만 하면 됩니다.
...
-   [**PointerDevice**](https://msdn.microsoft.com/library/windows/apps/br225633)는 장치에서 사용 가능한 입력 형식을 결정할 수 있도록 장치 쿼리 기능을 지원하는 장치 API입니다.
...
## <a name="writing-code"></a>코드 작성
  ...
  [Visual Studio의 Windows10 프로젝트](https://msdn.microsoft.com/library/windows/apps/dn609832.aspx#target_win10)에 대한 프로그래밍 언어 옵션에는 Visual C++, C#, Visual Basic 및 JavaScript가 포함됩니다. Visual C++, C# 및 Visual Basic의 경우 고화질 네이티브 UI 환경을 위해 XAML을 사용할 수 있습니다. Visual C++의 경우 XAML 대신 또는 XAML과 함께 DirectX를 선택할 수 있습니다. JavaScript의 경우 프레젠테이션 계층이 HTML이므로 당연히 HTML이 플랫폼 간 웹 표준입니다. 대부분의 코드와 UI는 유니버설이며, 어디서든 같은 방식으로 실행됩니다. 하지만 특정 디바이스 패밀리에 맞게 조정된 코드 및 특정 폼 팩터에 맞게 조정된 UI의 경우 적응형 코드 및 적응형 UI를 사용할 수 있습니다. 다음과 같은 여러 가지 경우를 살펴보겠습니다.

**대상 디바이스 패밀리에서 구현하는 API 호출**

UWP 앱에서 API를 호출하려면 해당 API가 앱의 대상 디바이스 패밀리에서 구현되었는지 알아야 합니다. Visual Studio Intellisense는 선택한 확장 SDK에 대해 사용할 수 있는 API만 보여줍니다. 확장 SDK를 선택하지 않은 경우, 유니버설 디바이스 패밀리에 사용할 수 있는 API만 볼 수 있습니다.

API 문서를 보면 API가 어떤 디바이스 패밀리의 일부인지 볼 수 있습니다. 요구 사항 섹션을 보면 디바이스 패밀리 구현이 무엇이며 API가 표시되는 디바이스 패밀리 버전을 알 수 있습니다.

**대상 디바이스 패밀리에서 구현되지 않은 API 호출**

참조하려는 확장 SDK에서 API를 호출하려고 했지만, 이 API가 대상으로 하는 디바이스 패밀리의 일부가 아닌 경우가 있습니다. 예를 들어, 유니버설 디바이스 패밀리를 대상으로 하지만, 앱을 모바일 디바이스에서 실행하고자 할 때 데스크톱 API가 있는 경우가 있습니다. 이 경우 해당 API를 호출하기 위해 적응형 코드를 작성할 수 있습니다.
...
자세한 소개 자료를 보려면 [Windows10 - Windows10 디바이스용 Windows 앱 빌드 소개](https://msdn.microsoft.com/magazine/dn973012.aspx)를 참조하세요.
