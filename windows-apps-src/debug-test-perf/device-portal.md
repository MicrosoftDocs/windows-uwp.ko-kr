...
Windows Device Portal은 PC의 웹 브라우저에서 연결할 수 있는 장치에서 웹 서버. 디바이스에 웹 브라우저가 해당 장치에서 브라우저를 사용 하 여 있습니다 로컬로 연결할 수도 수 있습니다.
...
1. (설정 앱의 구성) 디바이스에서 개발자 모드와 디바이스 포털을 사용 합니다.
...
여기서 장치 패밀리의 공통 된 도구에 설명 되어 있습니다. 다른 옵션은 디바이스에 따라 사용할 수 있습니다. 자세한 내용은 장치 유형에 대 한 특정 페이지를 참조 하세요.

### <a name="apps-manager"></a>앱 관리자.

앱 관리자는 설치/제거를 제공 하 고 앱에 대 한 관리 기능 패키지 및 번들의 등급을 호스트 장치에 합니다.

![장치 포털 앱 관리자 페이지](images/device-portal/wdp-apps.png)

- **설치 된 앱**: 드롭다운 메뉴를 사용 하 여 제거 하거나 장치에 설치 된 앱을 시작 합니다. **추가**클릭 하 여 새 앱을 설치 합니다. 로컬에서 패키지 된 앱을 배포 하는 UX 설치를 시작, 네트워크 또는 웹 호스트와 네트워크 공유에서 느슨한 파일 등록 합니다.
- **실행 중인 앱**: 현재 실행 되 고 필요에 따라 지연을 닫습니다 앱에 대 한 정보를 가져옵니다.
...
2.  장치 포털의 앱 관리자 섹션에 **추가** 클릭 하 고 **로컬 저장소에서 앱 패키지 설치**를 선택 합니다.
3.  앱 패키지를 살펴보고 **찾아보기** 를 클릭 합니다.
3.  **찾아보기** 살펴보고 인증서 (_.cer_) 파일 (필요 없음 모든 장치에서).
4.  해당 상자 하려는 경우 설치 옵션 또는 프레임 워크 패키지 앱 설치와 함께 확인 합니다. 두 개 이상 있는 경우 개별적으로 각각 추가합니다.     
5.  **다음** 설치를 시작 하려면 다음 단계 및 **설치** 이동 하려면 클릭 합니다. 

#### <a name="uninstall-an-app"></a>앱 제거
1.  앱이 실행되고 있지 않은지 확인합니다. 
2.  **실행 중인 응용 프로그램으로** 이동 하 고 닫습니다. 앱이 실행 되는 동안 제거 하려고 하면 앱을 설치 하려고 할 때 문제가 발생 합니다. 
...
이 페이지를 보고 테스트용으로 로드 된 앱에서 저장 한 파일을 조작할 수 있습니다. [앱 파일 탐색기를 사용 하 여](https://blogs.windows.com/buildingapps/2016/06/08/using-the-app-file-explorer-to-see-your-app-data/) 블로그 게시물 파일 탐색기 및 그 사용 방법에 대 한 자세한 정보를 참조 하세요. 
...
### <a name="event-tracing-for-windows-etw-logging"></a>이벤트 추적 ETW (Windows 용) 로깅

ETW 로깅 페이지 디바이스에서 실시간 ETW 이벤트 추적에 대 한 Windows () 정보를 관리합니다.
...
- **활성화 된 공급자**:이 활성화 된 공급자를 나열 합니다. 추적을 중지하려면 드롭다운에서 공급자를 선택하고 **사용 안 함**을 클릭 또는 탭합니다. 모든 추적을 일시 중단하려면 **모두 중지**를 클릭 또는 탭합니다.
- **공급자 기록**: 현재 세션 중 활성화 된 ETW 공급자를 표시 합니다. 비활성화된 공급자를 활성화하려면 **사용**을 클릭 또는 탭합니다. 기록을 지우려면 **지우기**를 클릭 또는 탭합니다.
- **필터 / 이벤트**: **이벤트** 섹션 표 형식으로 선택된 된 공급자의 ETW 이벤트가 나열 되어 있습니다. 표는 실시간으로 업데이트 됩니다. **필터** 메뉴를 사용 하 여 이벤트는 표시할 사용자 지정 필터를 설정 합니다. 테이블에서 모든 ETW 이벤트를 삭제 하려면 **지우기** 단추를 클릭 합니다. 이렇게 해도 공급자는 비활성화되지 않습니다. 현재 수집 된 ETW 이벤트를 로컬 CSV 파일로 내보낼 **파일에 저장** 을 클릭 수 있습니다.
...
캡처됩니다. [Windows Performance Analyzer](https://msdn.microsoft.com/library/windows/desktop/hh448170.aspx)에서 분석을 위해 ETL 파일을 열 수 있습니다.

### <a name="device-manager"></a>장치 관리자

장치 관리자 페이지 장치에 연결 된 모든 주변 장치를 열거 합니다. 각각의 속성을 보려면 설정 아이콘을 클릭 수 있습니다.

![장치 포털 장치 관리자 페이지](images/device-portal/mob-device-portal-devices.png)

### <a name="networking"></a>네트워킹

네트워킹 페이지 디바이스에서 네트워크 연결을 관리합니다. USB 통해 디바이스 포털에 연결 하지 않는 한 이러한 설정을 변경 연결이 끊어질 수 장치 포털에서 합니다.
- **사용 가능한 네트워크**: WiFi 네트워크 장치 수를 보여 줍니다. 네트워크에서 클릭 또는 탭하면 여기에 연결되며 필요한 경우 암호를 제공합니다. Device Portal 아직 엔터프라이즈 인증을 지원 하지 않습니다. 또한 **프로필** 드롭다운을 사용 하 여 장치에 알려진 WiFi 프로필 중 하나에 연결 하려고 합니다.
