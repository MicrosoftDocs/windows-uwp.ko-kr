...
ms.locfileid: "57603648"
---
# <a name="behind-the-scenes-of-your-packaged-desktop-application"></a>패키지에 포함 된 데스크톱 응용 프로그램의 백그라운드

이 문서에서는 데스크톱 응용 프로그램에 대 한 Windows 앱 패키지를 만들 때 파일 및 레지스트리 항목에 발생 하는 새로운 심층 분석을 제공 합니다.

최신 패키지의 핵심 목표 다른 앱과 호환성을 유지 하면서 가능한 한 시스템 상태에서 응용 프로그램 상태를 분리 하는 것입니다. UWP(유니버설 Windows 플랫폼) 내부에 응용 프로그램을 배치한 다음 런타임에 파일 시스템과 레지스트리에 일부 변경한 내용을 검색하고 리디렉션하면 이 목표가 달성됩니다.

데스크톱 응용 프로그램에 대해 만든 패키지는 데스크톱 전용 완전 신뢰 응용 프로그램 및 가상화 된 또는 샌드박스있지 않습니다. 이렇게 하면 기존의 데스크톱 응용 프로그램이 수행한 방식과 동일하게 다른 앱을 조작할 수 있습니다.

## <a name="installation"></a>설치

앱 패키지는 *C:\Program Files\WindowsApps\package_name* 아래 *app_name.exe* 실행 파일을 사용하여 설치됩니다. 각 패키지 폴더에는 패키지 앱에 대한 특수 XML 네임스페이스를 포함하는 매니페스트(AppxManifest.xml)가 포함됩니다. 매니페스트 파일 내에는 완전 신뢰 앱을 참조하는 ```<EntryPoint>``` 요소가 있습니다. 해당 응용 프로그램을 시작할 때 앱 컨테이너 내에서 실행 되지 않습니다 하지만 대신 해당 사용자로 실행 되는 정상적으로 합니다.

배포 후 패키지 파일은 읽기 전용으로 표시되며 운영 체제에 의해 엄격하게 잠깁니다. Windows에서는 이러한 파일이 손상되는 경우 앱이 실행되지 않게 합니다.

## <a name="file-system"></a>파일 시스템

응용 프로그램 상태를 포함 하기 위해 응용 프로그램 데이터의 응용 프로그램 변경 내용이 캡처됩니다. 만들기, 삭제 및 업데이트를 포함하여 AppData 폴더(예: *C:\Users\user_name\AppData*)에 대한 사용자의 모든 쓰기가 사용자별/앱별 개인 위치에 기록 중 복사됩니다. 패키지 응용 프로그램이 실제로 개인 복사본을 수정 하는 경우 실제 응용 프로그램 데이터를 편집 하는 것 처럼 보이게를 만듭니다. 이 방식으로 쓰기를 리디렉션하면 시스템에서는 앱에서 수정한 모든 파일 내용을 추적할 수 있습니다. 이 통해 응용 프로그램을 제거할 때 해당 파일을 정리 하려면 시스템, 따라서 rot"system"을 줄이고 더 나은 응용 프로그램 제거를 제공 환경을 사용자입니다.

AppData 리디렉션, 하는 것 외에도 Windows의 잘 알려진 폴더 (System32, 프로그램 파일 (x86) 등) 앱 패키지에 해당 디렉터리를 사용 하 여 동적으로 병합 됩니다. 각 패키지에는 루트에 "VFS"라는 이름의 폴더가 포함되어 있습니다. VFS 디렉터리에 있는 파일이나 디렉터리의 읽기 권한은 런타임 시 각각 해당하는 네이티브 권한으로 병합됩니다. 예를 들어, 응용 프로그램 포함 *C:\Program Files\WindowsApps\package_name\VFS\SystemX86\vc10.dll* 에서 설치할 해당 앱 패키지 파일의 일부는 표시 *C:\Windows\System32\ vc10.dll*합니다.  이렇게 하면 패키지가 없는 위치에서도 파일이 있을 수 있다고 예상하는 데스크톱 응용 프로그램과 호환성을 유지합니다.
...
다음 표에서 패키지의 일부로 제공된 파일이 앱에 대한 시스템에 오버레이된 위치를 보여 줍니다. 응용 프로그램 내에서 리디렉션된 위치에는 실제로 나열 된 시스템 위치에 있는 것으로 이러한 파일을 느끼게 됩니다 *C:\Program Files\WindowsApps\package_name\VFS*합니다. FOLDERID 위치는 [**KNOWNFOLDERID**](https://msdn.microsoft.com/library/windows/desktop/dd378457.aspx) 상수입니다.

시스템 위치 | 위치 리디렉션 ([PackageRoot]에서 \VFS\) | 아키텍처에서 유효
 :--- | :--- | :---
...
*HKLM\Software*의 키만 패키지에 포함됩니다. *HKCU* 또는 레지스트리의 다른 부분에 있는 키는 포함되지 않습니다. 패키지에서 키 또는 값에 대한 기록은 허용되지 않습니다. 쓸 키 또는 값 하지 사용자에 게 권한이으로 패키지의 일부로 허용 됩니다.
...
모든 쓰기는 패키지를 업그레이드 하는 동안 유지 하 고 응용 프로그램을 완전히 제거 될 때만 삭제 됩니다.
...
모든 파일 및 폴더 아래에 사용자가 패키지를 제거 해도 *C:\Program Files\WindowsApps\package_name* 및 응용 프로그램 데이터 또는 중 캡처된 레지스트리에 리디렉션된 모든 쓰기를 제거 합니다 패키징 프로세스입니다.
