...
| 조건 | 심각도 | Message |
| --------- | -------- | ------- |
| 지원되는 값 중 하나 이외의 targetOsVersion이 지정됩니다. | 오류 | 잘못 된 구성: 잘못 된 targetOsVersion를 지정 합니다. |
| "6.2.1"의 targetOsVersion이 지정되었으며 `packaging` 요소가 있습니다. | 오류 | 잘못 된 구성: 이 targetOsVersion '패키징' 노드가 지원 되지 않습니다. |
...
| AutoResourcePackage 한정자에는 여러 한정자가 포함되어 있습니다. 예: language_scale | 오류 | 잘못 된 구성: 여러 한정자를 사용 하 여 AutoResourcePackage 지원 되지 않습니다. |
| ResourcePackage QualifierSet에는 여러 한정자가 포함되어 있습니다. 예: language-en-us_scale-100 | 오류 | 잘못 된 구성: 여러 한정자를 사용 하 여 QualifierSet 지원 되지 않습니다. |
| 중복된 리소스 팩 이름이 있습니다. | 오류 | 잘못 된 구성: 중복 된 리소스 팩 이름 <rpname>합니다. |
| 두 가지 리소스 패키지에 동일한 한정자 집합이 정의되었습니다. | 오류 | 잘못 된 구성: 여러 인스턴스의 QualifierSet "<qualifier tags>" 찾을 수 있습니다. |
| 'ResourcePackage' 노드에 나열된 QualifierSet에 대한 후보를 찾을 수 없습니다. | 경고 | 잘못 된 구성: 찾을 수 없는 후보 <Resource Package Name>합니다. |
| 'AutoResourcePackage' 노드 아래에 나열된 한정자에 대한 후보를 찾을 수 없습니다. | 경고 | 잘못 된 구성: 한정자를 찾을 수 없습니다 후보 <qualifier name>합니다. 리소스 패키지가 생성되지 않았습니다. |
| 어떤 모드도 찾을 수 없습니다. 즉, 'packaging' 노드가 비어 있습니다. | 경고 | 잘못 된 구성: 패키징 모드를 지정 하지 않았습니다. |
