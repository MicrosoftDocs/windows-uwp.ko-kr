...
클립에 대 한 쿼리를 모든 데이터는 모든 메타 데이터 쿼리에서 반환 된 **GameClip** 개체에서 사용할 수 있습니다. 이 API 통해 단일 게임 클립을 가져오려면 **XUID**, **ServiceConfigId**, **GameClipId** 및 **SandboxId** 요청의 클레임에 필요 합니다. 게임 클립 적용 되도록 지정 또는 콘텐츠 격리 또는 개인 정보, 사용자를 특정 게임 클립 가져올 권한이 없는 결정 확인, API 404 (찾을 수 없음)의 HTTP 상태 코드를 반환 합니다.
...
> [!NOTE] 
> URI 새로 고침 최대 40 30 초가이 요청 작업이 끝나면 걸릴 수 있습니다. URI 만료 된 경우 스트리밍 작업에 대 한 즉시 사용 하려고 IIS 부드러운 스트리밍 서버에서 500 HTTP 상태 코드를 발생 합니다. 이 단축 하는 방법을 노력 하 고이 해당 작업을 진행으로 적절 하 게 업데이트 됩니다. 
...
| TitleId| 64 비트의 부호 있는 정수| 예| 1234567890| <b>콘텐츠 격리</b> 검사에 사용 합니다.| 
