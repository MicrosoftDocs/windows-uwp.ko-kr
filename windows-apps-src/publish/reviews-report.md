---
author: jnHs
Description: "Windows 개발자 센터 대시보드의 리뷰 보고서를 통해 스토어에서 앱을 평가할 때 고객이 입력한 평점 및 의견을 확인할 수 있습니다."
title: "리뷰 보고서"
...
Windows 개발자 센터 대시보드의 **리뷰** 보고서를 통해 스토어에서 앱을 평가할 때 고객이 입력한 평점 및 의견을 확인할 수 있습니다. 대시보드에서 이 데이터를 보거나 [보고서를 다운로드](download-analytic-reports.md)하여 오프라인으로 볼 수 있습니다. 또는 [Windows 스토어 분석 REST API](../monetize/access-analytics-data-using-windows-store-services.md)의 [앱 리뷰 가져오기](../monetize/get-app-reviews.md) 메서드를 사용하여 프로그래밍 방식으로 이 데이터를 검색할 수 있습니다.

또한 [이 페이지에서 직접](respond-to-customer-reviews.md), [Windows 스토어 리뷰 API를 통해](../monetize/submit-responses-to-app-reviews.md) 프로그래밍 방식으로, 또는 [개발자 센터 앱](https://www.microsoft.com/store/apps/dev-center/9nblggh4r5ws)을 사용하여 고객 리뷰에 응답할 수 있습니다.
...
-   **리뷰 내용**: 기본 설정은 **모두**로, 리뷰 텍스트가 추가되지 않은 평점이 포함됩니다. **리뷰 내용이 포함된 평점**을 선택하여 작성된 리뷰 내용을 포함하는 평점만을 표시할 수 있습니다.
...
-   **범주 이름**: 기본 필터는 **모두**입니다. 특정한 [리뷰 통찰력 범주](#review-insights-categories)를 선택하여 해당 범주와 관련된 리뷰만 표시할 수 있습니다. 
...
> [!TIP]
> 스토어에서 고객에게 표시되는 평균 평점은 고객의 시장과 장치 유형을 고려한 것이므로 이 보고서의 표시 내용과 다를 수 있습니다. 스토어에서 평균 평점이 지정된 고객에게 표시되는 방식을 확인하려면 필터를 적용하여 특정 시장과 장치 유형을 선택해야 합니다.


## <a name="average-rating-over-time"></a>시간 경과에 따른 평균 평점

**시간 경과에 따른 평균 평점** 차트는 지난 12개월 동안 앱 평균 평점이 변경된 정도를 보여 줍니다.

**시간 경과에 따른 평균 평점** 차트는 **평점 분석** 차트에서와 같이 지난 12개월 동안 남겨진 모든 평점의 평균을 계산하기 보다는 고객이 해당 주에 앱에 어떻게 평점을 매겼는지를 보여 줍니다. 이 차트는 추세를 확인하거나, 업데이트나 다른 요인에 의해 평점이 영향을 받았는지를 확인하는 데 도움이 됩니다.


## <a name="reviews"></a>리뷰

고객이 남긴 리뷰가 이 페이지에 표시됩니다.  
...
페이지의 리뷰를 날짜별 및/또는 평점별 오름차순이나 내림차순으로 정렬할 수 있습니다. **정렬 기준** 링크를 클릭하여 날짜별 및/또는 평점별로 정렬하는 옵션을 표시합니다. 
...
## <a name="review-insight-categories"></a>리뷰 통찰력 범주

**리뷰 통찰력 범주**를 통해 리뷰와 관련이 있다고 판단한 범주에 따라 리뷰를 그룹화해서 볼 수 있습니다.
...
Windows 스토어 개발자 센터 대시보드, [Windows 스토어 리뷰 API](../monetize/submit-responses-to-app-reviews.md) 또는 [개발자 센터 앱](https://www.microsoft.com/store/apps/dev-center/9nblggh4r5ws)을 사용하여 다양한 고객 리뷰에 대한 응답을 보낼 수 있습니다. 자세한 내용은 [고객 리뷰에 응답](respond-to-customer-reviews.md)을 참조하세요.
