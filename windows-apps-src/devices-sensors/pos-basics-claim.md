...
> [!Note]
> 클레임 작업 장치에 대 한 단독 잠금을 설정 되지만 운영 상태로 배치 하지 않습니다.  자세한 내용은 [I/O 작업에 대 한 장치 사용](#Enable-device-for-I/O-operations) 을 참조 하세요.
...
클레임 작업은 단순히 장치에 독점 권한을 설정 하는 운영 상태로 배치 하지 않습니다.  이벤트를 수신 하거나 출력 작업을 수행 하려면 **EnableAsync**를 사용 하 여 장치를 활성화 해야 합니다.  반대로, **DisableAsync** 장치나 수행 출력에서 이벤트를 수신 대기를 중지를 호출할 수 있습니다.  디바이스의 상태를 확인 하려면 **IsEnabled** 를 사용할 수도 있습니다.
