...
고객 코드를 사용 하 고 Microsoft 저장소에서 응용 프로그램 또는 추가 기능을 설치 하기 위해 클릭할 수 있는 고유한 해당으로 교환할 수 URL을 포함 하는 각 프로 모션 코드입니다.  사용자가 홍보 코드를 사용하여 설치하기 전에 앱은 [앱 인증 프로세스](the-app-certification-process.md)의 최종 게시 단계를 통과해야 합니다.

단일 사용 코드를 생성 (및 배포할 수 각 고객에 게 하나), 또는 지정된 된 수의 고객에 의해 여러 번에 사용할 수 있는 코드를 생성 하도록 선택할 수 있습니다.
...
> [!NOTE]
> 앱 고객에 게 사용할 수 없는 경우에 프로 모션 코드를 사용할 수 있습니다 (즉, 선택한 경우 **사용할 수는 있지만 하지 저장소에서 검색할 수 있도록이 제품을 확인** 중지 취득 **와: 직접 연결 된 모든 고객 제품의 저장소를 볼 수 있습니다 목록, 있지만 다운로드할 수 자신이 소유 하는 이전에 제품 또는 프로 모션 코드 있고 Windows 10 장치를 사용 하는 경우** 전송의 [검색 가능성](choose-visibility-options.md#discoverability) 섹션에서 옵션). 이 옵션을 고객 프로 모션 코드를 사용 하 여 제품을 취득 하기 위해 Windows 10 (Xbox 포함) 이어야 합니다.


## <a name="order-promotional-codes"></a>홍보 코드 주문

주문은 응용 프로그램 또는 추가 기능에 대 한 프로 모션 코드:
...
3.  **새 홍보 코드 주문** 페이지에서 다음을 입력합니다.
    ...
    -   홍보 코드를 활성화할 시기를 지정합니다. 특정 시작 날짜와 시간을 선택하려면 **코드가 즉시 활성화됨** 확인란 선택을 취소합니다. 그렇지 않은 경우는 코드가 활성 상태가 됩니다 귀하가 (있지만 제품 코드를 사용 하 여 고객에 대 한 순서 대로 게시 프로세스를 완료 해야).
...
3.  선택한 편집기에서 .tsv 파일을 엽니다. 최상의 환경을 위해 Microsoft Excel 같은 표 형식 구조에서 데이터를 표시할 수 있는 응용 프로그램에서 .tsv 파일을 엽니다. 그러나 아무 텍스트 편집기에서나 파일을 열 수 있습니다.
    ...
    -   **교환 가능 URL**: 고객이 코드를 교환하고 앱 또는 추가 기능을 설치할 수 있는 URL입니다. URL의 형식은 다음과: http://go.microsoft.com/fwlink/?LinkId=532540&mstoken=&lt; promotional_code >
...
고객에 게 프로 모션 코드 (또는 해당으로 교환할 수 URL)을 배포한 후에 제품을 무료로 가져올 URL을 클릭할 수 있습니다. 교환 가능 URL을 클릭하면 <https://account.microsoft.com/billing/redeem>에서 인증된 **코드 교환** 페이지가 시작됩니다. 이 페이지에는 교환하려는 앱에 대한 설명이 포함되어 있습니다. 고객이 Microsoft 계정으로 로그인하지 않은 경우, 로그인 메시지가 표시됩니다. 고객은 <https://account.microsoft.com/billing/redeem>을 방문하여 직접 코드를 입력할 수 있습니다.
...
> [!NOTE]
> 일부 경우에는 고객 나타날 수 **를 설치**하는 대신 **구입** 단추 프로 모션 코드를 통해 앱을 회수할 성공적으로 된 경우에 합니다. 이 경우 **구입**을 클릭하여 무료로 앱을 설치할 수 있습니다.
