...
클라이언트 클립 또는 게시 상태에 도달한를 다운로드할 수 있는 형식의 **GameClipUri** 개체에 지정 된 대로 더 미리 보기를 다운로드할 수 있습니다. 파일을 요청 하는 것에 대 한 URI 사용자 또는 공개 클립의 목록을 검색 하는 경우 응답 본문에 포함 됩니다.
...
성공 하면 서버는 범위 요청 헤더에 따라 잘릴 수 비디오 클립을 반환 합니다. 잘린된 클립에 대해 응답 일부 콘텐츠 (206) 됩니다. 서버에서 반환 하는 전체 파일 확인 (200)에서 응답 합니다. 오류 발생 시 **GameClipsServiceErrorResponse** 개체는 적절 한 HTTP 상태 코드 (예: 416, 요청 된 범위 충분 하지 않음)와 함께 반환 될 수 있습니다.
