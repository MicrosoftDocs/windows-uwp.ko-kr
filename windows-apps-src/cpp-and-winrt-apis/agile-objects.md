...
ms.locfileid: "5401928"
---
# <a name="agile-objects-in-cwinrt"></a>C++/WinRT의 Agile 개체

대부분의 경우에는 Windows 런타임 클래스의 인스턴스 (과 마찬가지 방법 대부분의 표준 c + + 개체) 모든 스레드에서 액세스할 수 있습니다. 이러한 Windows 런타임 클래스는 *agile*입니다. 적은 수의 Windows와 함께 제공 되는 Windows 런타임 클래스는 agile 하지만 사용 하는 경우를 고려해 야 자신의 스레딩 모델 및 마샬링 동작 (마샬링를 전달 하는 데이터는 아파트 경계를 넘어). 모든 Windows 런타임 개체가 agile, 좋은 매우 하므로 자체 [C + + WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt) 유형은 기본적으로 agile입니다.
