...
**Windows.Graphics.Capture** 네임스페이스에 있는 API는 일반 기능이 응용 프로그램의 매니페스트에서 선언되어야 합니다. 다음과 같이 이것을 파일에 직접 추가해야 합니다.
    
1. **솔루션 탐색기**에서 **Package.appxmanifest**를 마우스 오른쪽 단추로 클릭합니다. 
