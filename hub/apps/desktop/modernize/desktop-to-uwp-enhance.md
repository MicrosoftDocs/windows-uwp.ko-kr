...
일부 UWP Api는 [Msix 패키지](https://docs.microsoft.com/windows/msix/desktop/desktop-to-uwp-root)에 패키지 된 데스크톱 앱 에서만 지원 됩니다. 자세한 내용은 [사용 가능한 UWP api](desktop-to-uwp-supported-api.md)를 참조 하세요.
...
3. **NuGet 패키지 관리자** 창에서 **시험판 포함** 이 선택 되어 있는지 확인 합니다. 그런 다음 **찾아보기** 탭을 선택 하 고을 `Microsoft.Windows.SDK.Contracts`검색 합니다.

4. 패키지가 검색 된 후에는 **NuGet 패키지 관리자** 창의 오른쪽 창에서 대상으로 지정할 Windows 10 버전에 따라 설치할 패키지의 버전을 선택 합니다. `Microsoft.Windows.SDK.Contracts`

    * **10.0.18362-미리 보기**: Windows 10 버전 1903에 대해이를 선택 합니다.
    * **10.0.17763-미리 보기**: Windows 10 버전 1809에 대해이를 선택 합니다.
    * **10.0.17134-미리 보기**: Windows 10 버전 1803에 대해이를 선택 합니다.
...
[ C++/Winrt](https://docs.microsoft.com/windows/uwp/cpp-and-winrt-apis/) 를 사용 하 여 Windows 런타임 api를 사용 합니다. C++/WinRT는 Windows 런타임(WinRT) API용 최신 표준 C++17 언어 프로젝션으로서 헤더 파일 기반 라이브러리로 구현되며, 오늘날 Windows API에 대해 최고 수준의 액세스를 제공하도록 설계되었습니다.
...
: white_check_mark: **먼저 추가 하려는 환경을 결정 합니다.**
...
: white_check_mark: **Api를 호출 하 여 환경을 추가 합니다.**
...
코드에 런타임 검사를 추가 하는 가장 쉬운 방법은 다음 Nuget 패키지를 설치 하는 것입니다. [데스크톱 브리지 도우미](https://www.nuget.org/packages/DesktopBridge.Helpers/) 에서 ``IsRunningAsUWP()`` 메서드를 사용 하 여 Windows 런타임 api를 호출 하는 모든 코드를 게이트 합니다. 자세한 내용은이 블로그 게시물을 참조 하세요. [데스크톱 브리지-응용 프로그램의 컨텍스트를 식별](https://blogs.msdn.microsoft.com/appconsult/2016/11/03/desktop-bridge-identify-the-applications-context/)합니다.
