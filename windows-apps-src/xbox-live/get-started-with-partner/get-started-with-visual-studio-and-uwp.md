...
모든 Xbox Live 타이틀 전에 로그인 및 Xbox Live 서비스를 호출 하는 수 있게 됩니다 [파트너 센터](https://partner.microsoft.com/dashboard) 에서 만든 제품이 있어야 합니다. 자세한 내용은 [UDC 제목 만들기](create-a-new-title.md) 를 참조 하세요.
...
샌드박스 [Xbox Live 서비스 구성](../xbox-live-service-configuration.md) 타이틀을 해제할 준비가 될 때까지 정품에서 격리 됨 상태로 유지 하는 방법을 제공 합니다. 누적 될 수 있는 약간의 데이터는 샌드박스 관련이 있습니다. 예를 들어 타이틀 라는 *헤드샷*는 통계를 정의 하 고 타이틀을 테스트 하는 동안 몇 개의 헤드샷 사용자 계정에 축적 가정해 봅니다. 이 값은 타이틀의 개발 샌드박스를 특정 되며는 헤드샷을 통해 수행 하지는 타이틀의 일반 정품 버전으로 전환 하는 경우.
...
2. **새 프로젝트** 대화 상자를 선택 하는 **Visual C#** > **Windows** > **유니버설** 노드 창 왼쪽된에서 오른쪽 창에서 **빈 앱 (유니버설 Windows)를** 클릭 합니다.
...
4. C + + 엔진을 사용 하 여 WinRT를 사용할 때 사용 해야 C + + CX hat (^)을 사용 합니다. C + +는 c + + 게임 엔진에 대 한 사용 하 여 권장된 API입니다.  
...
Windows 10 SDK 10.0.16299 대상 한 또는 이상 다음 됩니다 추가 작업을 수행 하지 않고 연결 된 저장소 네임 스페이스에 액세스할 수 있습니다. 보안 소켓에 액세스 하려면 **UWP 용 Windows 데스크톱 확장**에 대 한 참조를 추가 해야 합니다. 하 여이 수행할 수 있습니다.
...
2. **텍스트 파일** 유형을 선택 하 고 **xboxservices.config**로 이름을 지정 합니다.
