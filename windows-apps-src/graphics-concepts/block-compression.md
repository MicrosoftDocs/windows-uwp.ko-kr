...
## <a name="span-idbasicsspanspan-idbasicsspanspan-idbasicsspanhow-block-compression-works"></a><span id="Basics"></span><span id="basics"></span><span id="BASICS"></span>블록이 압축 작동 하는 방법
...
- [압축 된 데이터를 저장합니다.](#storing-compressed-data)

### <a name="span-idstoringuncompresseddataspanspan-idstoringuncompresseddataspanspan-idstoringuncompresseddataspanspan-idstoring-uncompressed-dataspanstoring-uncompressed-data"></a><span id="Storing_Uncompressed_Data"></span><span id="storing_uncompressed_data"></span><span id="STORING_UNCOMPRESSED_DATA"></span><span id="storing-uncompressed-data"></span>압축 되지 않은 데이터 저장
...
### <a name="span-idstoringcompresseddataspanspan-idstoringcompresseddataspanspan-idstoringcompresseddataspanspan-idstoring-compressed-dataspanstoring-compressed-data"></a><span id="Storing_Compressed_Data"></span><span id="storing_compressed_data"></span><span id="STORING_COMPRESSED_DATA"></span><span id="storing-compressed-data"></span>압축 된 데이터를 저장합니다.
...
## <a name="span-idusingblockcompressionspanspan-idusingblockcompressionspanspan-idusingblockcompressionspanusing-block-compression"></a><span id="Using_Block_Compression"></span><span id="using_block_compression"></span><span id="USING_BLOCK_COMPRESSION"></span>블록 압축을 사용 하 여
...
- [실제 크기 및 가상 크기](#virtual-size-versus-physical-size)

### <a name="span-idvirtualsizespanspan-idvirtualsizespanspan-idvirtualsizespanspan-idvirtual-size-versus-physical-sizespanvirtual-size-versus-physical-size"></a><span id="Virtual_Size"></span><span id="virtual_size"></span><span id="VIRTUAL_SIZE"></span><span id="virtual-size-versus-physical-size"></span>실제 크기 및 가상 크기
...
첫 번째 블록 압축 형식 (BC1)를 사용 하 여 (두 DXGI\_형식\_BC1\_TYPELESS, DXGI\_형식\_BC1\_UNORM, 또는 DXGI\_BC1\_UNORM \_SRGB)는 5를 사용 하 여 세 구성 요소 색 데이터를 저장 합니다. 6:5 색 (빨강 5 비트, 6 비트 녹색, 파란색 5 비트). 데이터에 1비트 알파도 들어 있는 경우에도 마찬가지입니다. 가능한 최대 데이터 형식을 사용하는 4×4 텍스처의 경우 BC1 형식은 필요한 메모리를 48바이트(16가지 색 × 3분/색 × 1바이트/분)에서 8바이트로 줄입니다.

알고리즘은 4×4 블록의 텍셀에 작용합니다. 16 개 색을 저장 하는 대신 알고리즘 저장 2 참조 색 (색\_0과 색\_1) 및 다음 다이어그램에 표시 된 것 처럼 16 2 비트 색 인덱스 (요소 a – p).

![bc1 압축의 레이아웃](images/d3d10-compression-bc1.png)

색 인덱스(a–p)는 색상표에서 원래 색을 조회하는 데 사용됩니다. 색상표에는 4가지 색이 있습니다. 먼저 두 색상-색\_0과 색\_1-최소 및 최대 색입니다. 다른 두 가지 색을 색\_2 및 색\_3, 중간 색 선형 보간을 사용 하 여 계산 됩니다.
...
이 알고리즘은 1비트 알파도 들어 있는 데이터에도 적용됩니다. 유일한 차이점은 해당 색\_3 0 (투명 한 색을 나타냄) 및 색으로 설정 됩니다\_2는 색의 선형 blend\_0과 색\_1입니다.
...
BC2 형식을 사용 하 여 (두 DXGI\_형식\_BC2\_TYPELESS, DXGI\_형식\_BC2\_UNORM, 또는 DXGI\_BC2\_UNORM\_SRGB)를 낮은 일관성을 사용 하 여 색 및 알파 데이터를 포함 하는 데이터를 저장 합니다. (사용 하 여 [BC3](#bc3) 알파 항상 일관 된 데이터에 대 한). BC2 형식은 RGB 데이터를 5:6:5 색(5비트 빨강, 6비트 녹색, 5비트 파랑)으로 저장하고 알파를 별도의 4비트 값으로 저장합니다. 가능한 최대 데이터 형식을 사용하는 4×4 텍스처의 경우 이 압축 기술은 필요한 메모리를 64바이트(16가지 색 × 4분/색 × 1바이트/분)에서 16바이트로 줄입니다.
...
BC3 형식을 사용 하 여 (두 DXGI\_형식\_BC3\_TYPELESS, DXGI\_형식\_BC3\_UNORM, 또는 DXGI\_BC3\_UNORM\_SRGB)를 항상 일관 된 색 데이터를 저장 합니다. (사용 하 여 [BC2](#bc2) 알파 덜 일관 된 데이터를 사용 하 여). BC3 형식은 5:6:5 색(5비트 빨강, 6비트 녹색, 5비트 파랑)을 사용하여 색 데이터를 저장하고 1바이트를 사용하여 알파 데이터를 저장합니다. 가능한 최대 데이터 형식을 사용하는 4×4 텍스처의 경우 이 압축 기술은 필요한 메모리를 64바이트(16가지 색 × 4분/색 × 1바이트/분)에서 16바이트로 줄입니다.
...
알고리즘은 4×4 블록의 텍셀에 작용합니다. 알고리즘의 2 참조 알파 저장 16 알파 값을 저장 하는 대신 (알파\_0 및 알파\_1) 및 다음 다이어그램에 표시 된 것 처럼 16 3 비트 색 인덱스 (알파-p).

![bc3 압축의 레이아웃](images/d3d10-compression-bc3.png)

BC3 형식은 알파 인덱스(a–p)를 사용하여 8개의 값이 들어 있는 조회 테이블에서 원래 색을 조회합니다. 처음 두 값-alpha\_0 및 알파\_1-최소 및 최대 값은 선형 보간을 사용 하 여 다른 6 개의 중간 값 계산 됩니다.

알고리즘은 2개의 참조 알파 값을 검사하여 보간된 알파 값의 수를 결정합니다. 경우 알파\_알파 0 보다\_BC3를 1 보간합니다 알파 값이 6; 4이 고, 그렇지 보간합니다. BC3이 4개의 알파 값만 보간하는 경우 2개의 추가 알파 값을 설정합니다(완전 투명을 나타내는 0과 완전 불투명을 나타내는 255). BC3은 지정된 텍셀에 대한 원래 알파와 가장 일치하는 보간된 알파 값에 해당하는 비트 코드를 저장하여 4×4 텍셀 영역의 알파 값을 압축합니다.
...
BC4 형식으로 각 색에 대해 8비트를 사용하여 1분 색 데이터를 저장합니다. 향상 된 정확도 결과로 (에 비해 [BC1](#bc1)), BC4 범위의 부동 소수점 데이터를 저장 하는 데 적합 \[0 ~ 1\] 는 DXGI를 사용 하 여\_형식\_BC4\_UNORM 형식 및 \[-1에서 + 1\] 는 DXGI를 사용 하 여\_형식\_BC4\_SNORM 형식입니다. 가능한 최대 데이터 형식을 사용하는 4×4 텍스처의 경우 이 압축 기술은 필요한 메모리를 16바이트(16가지 색 × 1분/색 × 1바이트/분)에서 8바이트로 줄입니다.

알고리즘은 4×4 블록의 텍셀에 작용합니다. 16 개 색을 저장 하는 대신 알고리즘 저장 2 참조 색 (빨간색\_0이 고 빨간색\_1)와 16 비트 3 색 인덱스 (빨간색를 통해 빨간색 p) 다음 다이어그램에 표시 된 것 처럼 합니다.

![bc4 압축의 레이아웃](images/d3d10-compression-bc4.png)

알고리즘에서 3비트 인덱스를 사용하여 8가지 색이 들어 있는 색상표에서 색을 조회합니다. 먼저 두 색상-빨간색\_0이 고 빨간색\_1-최소 및 최대 색입니다. 알고리즘에서 선형 보간을 사용하여 나머지 색을 계산합니다.

알고리즘에서 2개의 참조 값을 검사하여 보간된 색 값의 수를 결정합니다. 빨간색\_빨간색 0 보다\_BC4를 1 6 색 값을 보간합니다; 4이 고, 그렇지 보간합니다. BC4가 4개의 색 값만 보간하는 경우 2개의 추가 색 값을 설정합니다(완전 투명을 나타내는 0.0f와 완전 불투명을 나타내는 1.0f). BC4는 지정된 텍셀에 대한 원래 알파와 가장 일치하는 보간된 알파 값에 해당하는 비트 코드를 저장하여 4×4 텍셀 영역의 알파 값을 압축합니다.
...
DXGI\_형식\_BC4\_SNORM 범위의 데이터를 인코딩할 한다는 점을 제외 하 고 4 면 색 값을 보간 하는, SNORM는 완전히 동일 합니다. 다음 코드 샘플과 같이 1분 데이터의 보간이 수행됩니다.
...
BC5 형식으로 각 색에 대해 8비트를 사용하여 2분 색 데이터를 저장합니다. 향상 된 정확도 결과로 (에 비해 [BC1](#bc1)), BC5 범위의 부동 소수점 데이터를 저장 하는 데 적합 \[0 ~ 1\] 는 DXGI를 사용 하 여\_형식\_BC5\_UNORM 형식 및 \[-1에서 + 1\] 는 DXGI를 사용 하 여\_형식\_BC5\_SNORM 형식입니다. 가능한 최대 데이터 형식을 사용하는 4×4 텍스처의 경우 이 압축 기술은 필요한 메모리를 32바이트(16가지 색 × 2분/색 × 1바이트/분)에서 16바이트로 줄입니다.

- [BC5\_UNORM](#bc5-unorm)
- [BC5\_SNORM](#bc5-snorm)

알고리즘은 4×4 블록의 텍셀에 작용합니다. 알고리즘은 두 구성 요소에 대 한 16 색을 저장 하는 대신 각 구성 요소에 대 한 2 참조 색을 저장 (빨간색\_0, 빨간색\_1 녹색\_0 및 녹색\_1) 및 각 구성 요소에 대 한 16 3 비트 색 인덱스 (빨간색을 빨간색 p 통해 통해 녹색 p 녹색) 다음 다이어그램에 표시 된 것 처럼 합니다.

![bc5 압축의 레이아웃](images/d3d10-compression-bc5.png)

알고리즘에서 3비트 인덱스를 사용하여 8가지 색이 들어 있는 색상표에서 색을 조회합니다. 먼저 두 색상-빨간색\_0이 고 빨간색\_1 (또는 녹색\_0 및 녹색\_1)-최소 및 최대 색입니다. 알고리즘에서 선형 보간을 사용하여 나머지 색을 계산합니다.

알고리즘에서 2개의 참조 값을 검사하여 보간된 색 값의 수를 결정합니다. 빨간색\_빨간색 0 보다\_BC5를 1 6 색 값을 보간합니다; 4이 고, 그렇지 보간합니다. BC5가 4개의 색 값만 보간하는 경우 0.0f와 1.0f에 나머지 2개의 색 값을 설정합니다.
...
DXGI\_형식\_BC5\_SNORM는 정확히 동일 SNORM 범위의 데이터를 인코딩할 때 4 개의 데이터 값을 보간 하는 두 개의 추가 값은-1.0f 및 1.0f 있다는 점을 제외 하면 됩니다. 다음 코드 샘플과 같이 1분 데이터의 보간이 수행됩니다. 녹색 분에 대한 계산은 비슷합니다.
