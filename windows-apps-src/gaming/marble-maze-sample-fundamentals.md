...
description: 이 문서에서는 대리석 미로 프로젝트;의 기본 특성을 설명 등 Windows 런타임 환경에서 Visual c + +를 사용 하는 방법, 작성 및 구조적, 방법 및 작성 하는 방법입니다.
...
-   코드 분석 도구와 함께 [SAL 주석을](https://docs.microsoft.com/visualstudio/code-quality/using-sal-annotations-to-reduce-c-cpp-code-defects) 사용 하 여 응용 프로그램에서 오류를 검색 하는 데 도움이 됩니다.

## <a name="creating-the-visual-studio-project"></a>Visual Studio 프로젝트 만들기


다운로드 하 고 표본 추출, 하는 경우 Visual Studio에서 ( **c + +** 폴더)에 **MarbleMaze_VS2017.sln** 파일을 열 수 및 하면 앞의 코드 해야 합니다.
...
1. Visual Studio 2017 선택 **파일 > 새로 만들기 > 프로젝트...**

2. **새 프로젝트** 창의 왼쪽된 세로 막대에서 선택 **Installed > 서식 파일 > Visual c + +** 합니다.

3. 중간 목록에서 **DirectX 11 응용 프로그램 (범용 Windows)를**선택 합니다. 이 옵션을 표시 되지 않으면, 없을 수도 있습니다 설치 필수 구성 요소&mdash;추가 구성 요소를 설치 하는 방법에 대 한 정보를 [추가 하거나 작업 및 구성 요소를 제거 하 여 Visual Studio 2017 수정](https://docs.microsoft.com/visualstudio/install/modify-visual-studio) 을 참조 하십시오.

4. **이름**, 저장 되는 파일에 대 한 **위치** 및 **솔루션 이름**, 사용자가 프로젝트를 지정 하 고 **확인**을 클릭 합니다.
...
Microsoft 저장소에서 제공한 코드도 UWP app 모든 응용 프로그램 패키지의 형태로 제공 합니다. 앱 패키지에는 앱 정보가 포함된 패키지 매니페스트가 있습니다. 예를 들어 앱의 기능(즉, 보호된 시스템 리소스 또는 사용자 데이터에 대해 필요한 액세스)을 지정할 수 있습니다. 앱이 특정 기능을 사용하도록 결정하면 패키지 매니페스트를 사용하여 필요한 기능을 선언합니다. 매니페스트를 사용하여 지원되는 장치 회전, 타일 이미지 및 시작 화면과 같은 프로젝트 속성을 지정할 수도 있습니다. 프로젝트에서 **Package.appxmanifest**를 열어 매니페스트를 편집할 수 있습니다. 앱 패키지에 대한 자세한 내용은 [앱 패키징](https://msdn.microsoft.com/library/windows/apps/mt270969)을 참조하세요.
...
터치는 accelerometer, Xbox 하나 컨트롤러 또는 마우스 제어 대리석 미로를 사용할 수 있습니다.

-   컨트롤러의 방향 패드를 사용하여 활성 메뉴 항목을 변경합니다.
-   터치, A 또는 시작을 사용 하는 컨트롤러 또는 메뉴 항목을 선택 하는 마우스 단추입니다.
-   터치, 가속도계, 왼쪽 엄지스틱 또는 마우스를 사용하여 미로를 기울입니다.
-   터치, A 또는 시작을 사용 하 여 컨트롤러나는 높은 같은 메뉴를 닫으려면 마우스 단추 점수 테이블입니다.
-   컨트롤러 또는 바로 가기 키 P 키에서 시작 단추를 사용 하 여를 일시 중지 또는 게임을 다시 시작 합니다.
-   컨트롤러의 뒤로 단추 또는 키보드의 Home 키를 사용하여 게임을 다시 시작합니다.
-   높은 점수 테이블에 표시 된 경우, 컨트롤러 또는 바로 가기 키 Home 키에서 뒤로 단추를 사용 하 여 점수가 포함 된 모든의 선택을 취소 합니다.

##  <a name="code-conventions"></a>코드 규칙


Windows 런타임은 특수 응용 프로그램 환경에서만 실행되는 UWP 앱을 만드는 데 사용할 수 있는 프로그래밍 인터페이스입니다. 이러한 앱 권한이 부여 된 함수, 데이터 형식 및 장치를 사용 하 고 Microsoft 저장소에서 배포 됩니다. 최하위 수준에서 Windows 런타임은 ABI(응용 프로그램 이진 인터페이스)로 구성됩니다. ABI는 JavaScript, .NET 언어, Visual C++ 등 여러 프로그래밍 언어가 Windows 런타임 API에 액세스할 수 있게 하는 하위 수준 이진 계약입니다.
...
Marble Maze는 **^** 을 **Microsoft::WRL::ComPtr**과 함께 사용하여 힙 할당 개체를 관리하고 메모리 누수를 최소화합니다. 사용 하는 것이 좋습니다 ^ Windows 런타임 변수 수명을 **ComPtr** COM 변수 (예: 때 DirectX)을 사용 하 고 **std::shared\_ptr** 또는 기타 모든 수명 관리 **std::unique\_ptr** 수명 관리를 관리 하려면 c + + 개체 힙 할당 합니다.
...
-   **HRESULT**를 반환하는 DirectX 루틴을 호출하는 경우 **DX::ThrowIfFailed** 함수를 사용합니다. 이 함수는 [DirectXHelper.h](https://github.com/Microsoft/Windows-appsample-marble-maze/blob/master/C%2B%2B/Shared/DirectXHelper.h)에 정의 됩니다. 제공 된 **HRESULT** 오류 코드는 **ThrowIfFailed** 예외를 throw 합니다. 예를 들어 **E\_POINTER**인 경우 **ThrowIfFailed**에서 [Platform::NullReferenceException](https://msdn.microsoft.com/library/windows/apps/hh755823.aspx)이 발생합니다.
...
-   예기치 않은 오류에 대 한 **HRESULT** 를 사용 하지 못하도록 하는 것이 좋습니다, 이지만 코드의 흐름을 제어 하는 예외 처리 사용을 방지 하기 위해 더 중요 합니다. 따라서 필요한 경우 **HRESULT** 반환 값을 사용하여 코드 흐름을 제어하는 것이 좋습니다.
...
[BasicLoader.h](https://github.com/Microsoft/Windows-appsample-marble-maze/blob/e62d68a85499e208d591d2caefbd9df62af86809/C%2B%2B/Shared/BasicLoader.h)에 선언 되어있는 **BasicLoader::LoadMesh** 메서드를 사용 하는 것이 좋습니다. 이 메서드를 사용 하 여 `_In_` 해당 *filename* 이 입력된 매개 변수를 지정 하려면 (및 따라서 됩니다에서 읽을 수만) `_Out_` *vertexBuffer* 및 *indexBuffer* 출력 매개 변수를 지정 하려면 (및 따라서만에 기록 되지 않음) 및 `_Out_opt_` *vertexCount* 및 *indexCount* 는 옵션을 지정 하려면 출력 매개 변수 (및 저장할 수)입니다. *vertexCount* 및 *indexCount*는 선택적 출력 매개 변수이므로 **nullptr**이 될 수 있습니다. C/C++ 코드 분석 도구는 이 메서드 호출을 검사하여 전달되는 매개 변수가 이러한 조건을 충족하는지 확인합니다.
...
메뉴 모음에서 응용 프로그램에서 코드 분석을 수행 하려면 선택 **빌드 > 솔루션에서 코드 분석 실행**합니다. 코드 분석에 대한 자세한 내용은 [코드 분석을 사용하여 C/C++ 코드 품질 분석](https://docs.microsoft.com/visualstudio/code-quality/analyzing-c-cpp-code-quality-by-using-code-analysis)을 참조하세요.
