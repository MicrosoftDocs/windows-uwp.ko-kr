...
1. 웹 서버 (HTTP/1.1) 바이트 범위 요청에 대 한 지원이 해야 합니다.
    - HTTP/1.1 프로토콜을 지 원하는 서버 바이트 범위 요청에 대 한 지원이 있어야 합니다. 
2. 웹 서버, Windows 10 앱 패키지의 콘텐츠 유형을 파악 해야 합니다.
    - 일부로 새 콘텐츠 형식을 선언 하는 방법은 다음과 같습니다 [웹 구성 파일](web-install-IIS.md#step-7---configure-the-web-app-for-app-package-mime-types)
