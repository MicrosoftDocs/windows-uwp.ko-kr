...
-   MPSD를 호출할 때 HTTP/404 코드를 나타남 합니다.
...
-   요청 된 서비스 구성에 액세스할 수 없습니다.
    ...
    2.  올바른 샌드박스 내에 있는지 확인 합니다.
     3.  인증서 인증을 사용 하 고이 오류가 발생 하는 경우 개발자 계정 관리자에 게 문의 합니다.   요청 된 세션을 액세스할 수 없습니다. 호출 하는 사용자의 멀티 플레이 권한이 있어야 합니다. 및 개인 세션 세션 구성원이 읽을 수 있습니다.

    개인의 표시 여부를 세션에 액세스 하려고 하기 때문에 가능한 세션을 확인 하 고 않아도 됩니다. 가시성 Private로 설정 되 면 해당 세션의 구성원만 세션 문서를 읽을 수 있습니다.

| 참고                                                                                                                                                  |
|--------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 사용자가 새 세션을 등록 하려면 골드 계정이 있어야 합니다. 재생 중인 사용자에 대 한 골드 계정 권한이 없으면 새 세션을 등록 하는 요청 HTTP/403를 반환 합니다. |
...
### <a name="i-am-getting-an-http404-code-when-calling-mpsd"></a>MPSD를 호출할 때 HTTP/404 코드를 나타남 합니다.
...
5.  세션 비어의 결과로 삭제 되지 않았는지 확인 합니다. 세션 시간 제한 사용자 때문에 빈 될 수 있습니다. 이런 경우 자주 세션의 모든 구성원 시간 제한을 적용 되는, 예: "준비" 또는 "비활성" 상태 이므로 됩니다. 사용자 상태 세부 정보에 대 한 [세션 사용자 상태](mpsd-session-details.md) 를 참조 하세요.
...
-   제목 가입을 시작 하는 Xbox 셸을 UI 보기 만료 된 수도 있습니다. 열려 있는 동안 일부 UI 보기, 예를 들어 사용자의 프로필 카드 조인 핸들을 자동으로 새로 고쳐지지 않습니다. HTTP/404 코드를 받지 않으려면 제목 닫기 하 고 다시 가입 하기 전에 데이터를 새로 고치려면 보기 해야 합니다.
-   제목에 가입 하는 사용자 수 있는 전환 활동 제목 Xbox 셸 UI에서에서 조인 작업을 선택한 후 세션. 이러한 이유로 HTTP/404 코드에 대 한 거의 발생 하지 않아야 합니다.

이러한 경우 중 하나로, 제목 코드 나타내야 조인 사용자 가입 실패 했음을 나타내는 오류 메시지가 합니다.
...
다음 요청 세션 etag If 일치 헤더 일치 하지 않는 경우 HTTP/412를 반환 합니다.
...
자세한 정보를 가져오는 Fiddler 추적을 수집 하 고 확인 메시지 HttpResponse 본문의 일환으로 반환 하는 다음 키를 누릅니다. 식별 하 고 오류를 해결할 수 있는 충분 한 정보를 제공 해야이 또는 해상도 대 한 개발자 포럼을 검색할 수 있습니다.
...
400-\[ResponseBody\]:이 세션은 2 개 이상의 구성원 시작을 요구 하는 관리 되는 초기화에 대해 구성 합니다.

True로 "초기화" 필드 집합을 사용 하 여 부족 세션 멤버 예약 요청에 포함 되어 있으므로 세션을 만들 수 없습니다. 코드는 *initializeRequested* 매개 변수를 사용 하 여 **MultiplayerSession.Join** 메서드 또는 **MultiplayerSession.AddMemberReservation** 메서드에 대해 멤버에 대 한이 필드를 설정할 수 있습니다.
...
이 상태 코드는 사용자가 만든 경우 세션에 추가 되었는지 나타냅니다. 만든 시점과 세션 빈 시간 제한은 0 (기본값) 세션에서 사용자가 없는 경우 세션 만들어지지 않습니다.
...
제목은 MPSD 폴링 피해 야 합니다. 제목을 MPSD 세션에 변경 내용을 찾아야, 경우 세션 변경 이벤트에 대 한 구독 해야 합니다. 자세한 내용은 참조 [하는 방법: MPSD 세션 변경 알림을 등록할](multiplayer-how-tos.md)합니다.
...
플레이어가 되는 게임 세션을 알립니다 준비 하는 경우 제목이 실행 중인지 여부에 따라 다릅니다. 플레이어가 제목에는 제목 UI에서에서 게임 세션 알림을 받지 않습니다 경우 **MultiplayerSession.Leave** 메서드를 사용 하 여 게임 세션에서 플레이어를 제거 하려면 제목 최대 됩니다.
...
### <a name="what-is-the-key-difference-between-the-way-parties-are-properly-used-by-2015-multiplayer-and-the-way-they-were-used-in-2014-multiplayer"></a>2014 멀티 플레이어에서 사용 된 방식과 당사자 2015 멀티 플레이어에서 제대로 사용 되는 방법은 주요 차이점은 무엇입니까?
...
게임 세션을 시작할 때 매치 메이 킹 서비스에서 더 이상 보급 됩니다. 플레이어 슬롯 세션 내에서 사용할 수 있게 하는 경우 (호스트) 중재자 새로운 플레이어를 모집 하려는 중재자 해야 진행 중인 세션에 대 한 새 일치 티켓을 **MatchmakingService.CreateMatchTicketAsync** 메서드를 사용 하 여 만듭니다. 티켓 세션을 다시 공급자로 하 고 더 많은 플레이어를 찾습니다. [스마트 매치](smartmatch-matchmaking.md)를 참조 하세요.
...
항상 사용자 참여를 통해 세션에 추가 하는 제목, 제목 true로 **MultiplayerSessionMember.InitializeRequested** 속성을 설정 합니다. 이렇게 하면 MPSD 세션 멤버의 나머지 부분에 대 한 게임 초기화 단계에서 이동 하기 전에 대기할 됩니다. 그렇지 않으면 사용자가 가입 하는 매우 짧은 창 세션 변경 알림 놓칠 수 있습니다.
