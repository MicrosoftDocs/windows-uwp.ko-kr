...
2. 장치의 IP 주소를 조회 합니다. 장치의 IP 주소를 찾으려면 **설정 > 네트워크 및 인터넷 > Wi-fi > 하드웨어 속성**합니다. "안녕 코타나, 내 IP 주소는?"이라고 말하여 요청할 수도 있습니다.

3. PC에서 웹 브라우저에서로 이동 `https://<YOUR_HOLOLENS_IP_ADDRESS>`
...
> [!IMPORTANT]
> HoloLens 장치의 실제 네트워크 IP 주소를 사용 하 여 PC, 장치를 찾을 수 없으면 시도 대신 `http://127.0.0.1:10080`합니다.
...
2. 보안 아이콘의 오른쪽 위 목록에서 연결 하거나 이동할 클릭-장치 포털에서 "보안" 페이지에서이 장치의 인증서 다운로드: `https://<YOUR_HOLOLENS_IP_ADDRESS>/devicesecurity.htm`

3. 에 "신뢰할 수 있는 루트 인증 기관" 저장소 사용자 PC에-Windows 메뉴 형식에서에서 인증서를 설치 합니다. 컴퓨터 인증서를 관리 하 고 애플릿을 시작 합니다.
...
- **기본 설정** 섹션에는 다음 설정이 포함되어 있습니다.
    ...
    - **장치 이름**: HoloLens에 이름을 할당 합니다. 이 값을 변경 후에는 디바이스를 다시 부팅해야 적용됩니다. 저장을 클릭한 후 디바이스를 즉시 다시 부팅할 것인지 또는 나중에 다시 부팅할 것인지를 묻는 대화 상자가 나타납니다.
...
- **이동**: 오른쪽 단추 클릭 + 마우스
- **확대/축소**: 마우스 스크롤
- **추적 옵션**: Force visual 추적을 확인 하 여 연속 visual 추적을 켭니다. 일시 중지는 시각적 추적을 중지합니다.
- **옵션을 보려면**: 3D 뷰에서 옵션 설정: 추적-: Visual 추적 활성 상태 인지 여부를 나타냅니다.
...
- Surface 재구성 합니다. 클릭 하거나 장치에서 최신 공간 매핑 메시를 표시 하는 업데이트를 누릅니다. 전체 단계를 완료하는 데 시간이 최대 몇 초 걸릴 수 있습니다. 메시는 3D 뷰에서 자동으로 업데이트되지 않고 디바이스에서 수동으로 업데이트를 클릭해야만 최신 메시를 얻을 수 있습니다. 저장을 클릭하여 현재 공간 매핑 메시를 PC에서 obj 파일로 저장합니다.

### <a name="mixed-reality-capture"></a>혼합 현실 캡처 

혼합 현실 캡처 페이지를 사용하여 HoloLens에서 미디어 스트림을 저장합니다.
- 설정: 다음 설정을 확인 하 여 캡처되는 미디어 스트림 제어:-홀로그램: 비디오 스트림의 holographic 콘텐츠를 캡처합니다. 홀로그램은 스테레오가 아닌 모노로 렌더링됩니다.
...
- **대화방 캡처**: 사용자의 환경에 대 한 공간 매핑 메시를 포함 하는 시뮬레이션 된 공간 파일을 다운로드 하는 데 사용 합니다. 공간의 이름을 지정한 다음 캡처를 클릭하여 PC에 .xef 파일로 데이터를 저장합니다. 이 공간 파일을 HoloLens 에뮬레이터에 로드할 수 있습니다.
- **기록**: 기록, 녹음/녹화를 이름을 클릭 하거나 탭 기록을 시작 하려면 레코드 스트림을 확인 합니다. HoloLens를 사용하여 작업을 수행한 다음 중지를 클릭하여 PC에 .xef 파일로 데이터를 저장합니다. 이 파일을 HoloLens 에뮬레이터 또는 디바이스에 로드할 수 있습니다.
- **재생**: 클릭 하거나 탭 업로드 기록 PC에서 xef 파일을 선택 하는 HoloLens에 데이터를 보냅니다.
- **컨트롤 모드**: 드롭다운 목록에서 기본 또는 시뮬레이션 선택 클릭 하거나는 HoloLens에서 모드를 선택 하려면 설정 단추를 탭 합니다. "시뮬레이션"을 선택하면 HoloLens의 실제 센서를 사용할 수 없고 대신 업로드한 시뮬레이트된 데이터를 사용합니다. "시뮬레이션"으로 전환하면 HoloLens가 "기본"으로 다시 전환될 때까지 실제 사용자에게 응답하지 않습니다.
