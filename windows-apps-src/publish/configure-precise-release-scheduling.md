...
>
>이전에 게시 된 앱 모든 이전 OS 버전에서 지 원하는 경우 **취득 중지** 됩니다 계속 앱을 획득 하는 일을 할 수; 날짜 선택 하면 해당 고객에 게 적용 되지 것입니다 (에서 새 선택 영역을 사용 하 여 업데이트를 제출 하지 않는 한를 [가시성](choose-visibility-options.md#discoverability) 섹션을 선택 하는 경우 또는 **확인 앱에 사용할 수 없게** 에서 **앱 개요** 페이지).
...
두 가지 옵션을 볼 수 있습니다. **릴리스** 하 고 **취득 중지**합니다. 
...
이 옵션을 선택 하 여 동일한 영향이 있는지 알아야 할 것 **검색 가능 하지만 사용할 수 없는이 앱 만들기** 에 [가시성](choose-visibility-options.md#discoverability) 섹션과 선택 **취득 중지: 직접 링크를 사용 하 여 모든 고객을 나열 하는 제품의 저장소를 볼 수 있지만 이러한 이전에 제품을 소유 또는 프로 모션 코드를 Windows 10 장치를 사용 하는 경우만 다운로드할 수 있습니다.** 앱이 새 고객에게 제공되지 않게 하려면 앱 개요 페이지에서 **앱을 사용할 수 없도록 설정**을 클릭합니다. 자세한 내용은 [스토어에서 앱 제거](guidance-for-app-package-management.md#removing-an-app-from-the-store)를 참조하세요.
...
여러 지역/국가에 적용할 일정을 추가하려면 *지역/국가 그룹*을 생성합니다. 포함시킬 지역/국가를 선택한 후 해당 그룹의 이름을 입력합니다. (이 이름은 참조 목적 으로만 며 고객에 게 표시 되지 않습니다.) 예를 들어 북미 지역에 대 한 출시 그룹을 만들려는 경우 선택할 수 있습니다 **캐나다**를 **멕시코**, 및 **미국**를 하 고 이름을 **북아메리카**  나 다른 이름을 선택 하는 합니다. 지역/국가 그룹을 생성한 후 **저장**을 클릭합니다. 그러면 위에서 설명한 **릴리스** 및 **취득 중지** 옵션이 표시되지만, 해당 지역/국가 그룹에만 선택 사항이 적용되어 있을 것입니다.
