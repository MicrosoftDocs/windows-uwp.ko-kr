...
8 방향 디지털 조이스틱, 6 개의 **작업** 단추 (아래 이미지에 A1 a 6으로 표시) 및 (s 1과 S2로 표시), 두 가지 **특별 한** 단추를 사용 하 여 Xbox One 아케이드 스틱에 설치 되어 있습니다. 아날로그 컨트롤 또는 진동을 지원 하지 않는 디지털 전용 입력된 장치 일입니다. Xbox One 아케이드 스틱에 UI 탐색을 지 원하는 데 사용 되는 **보기** 및 **메뉴** 단추 장착 수도 있지만 게임 플레이 명령을 지원 하기 위해 아니며 조이스틱 버튼으로 즉시 액세스할 수 없습니다.

![아케이드 스틱 4 방향 조이스틱, 6 개의 단추 (a 1-a 6) 및 2 특별 한 단추 (s 1과 S2)](images/arcade-stick-1.png)
...
UI 탐색 컨트롤러로 아케이드 스틱은 탐색 명령의 [필수 집합](ui-navigation-controller.md#required-set) 조이스틱 및 **보기**, **메뉴**, **동작 1**및 **동작 2** 버튼 매핑
...
각 아케이드 스틱 버튼&mdash;4 방향 조이스틱, 6 개의 **작업** 단추 및 두 가지 **특별 한** 단추&mdash;누름 (아래) 또는 놓음 (위)에 있는지 여부를 나타내는 디지털 읽기를 제공 합니다. 버튼 판독값은 효율을 위해 개별 부울 값으로 표현 되지 않는 대신, 모두 압축 [ArcadeStickButtons][] 열거로 표현 되는 단일 비트 필드로 합니다.
