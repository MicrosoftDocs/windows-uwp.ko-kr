...
나열 된 저장소를 편집 하려면 제출 개요 페이지에서 언어 이름을 선택 합니다. 프로그램 스토어 목록 내보내기 오프 라인으로 작업 하 고 모든 목록 데이터를 한 번에 가져올 하도록 선택 하지 않으면 각 언어를 개별적으로 편집 해야 합니다. 작동 하는 방법에 대 한 자세한 참조 [가져오기 및 스토어 목록 내보내기](import-and-export-store-listings.md)합니다.

사용 가능한 필드에 대 한 설명은 다음과 같습니다.

## <a name="product-name"></a>제품 이름

이 드롭다운 목록 상자를 사용 하면 (둘 이상의 이름을 앱에 대 한 예약 된) 경우 스토어 목록에서 사용 되는 이름을 지정 합니다.

업로드 한 패키지를 나열 하는 저장소와 동일한 언어의 작업 중인 경우 해당 패키지에 사용할 이름을 선택 됩니다. 해야 할 경우 [앱 이름 바꾸기](manage-app-names.md#rename-an-app-that-has-already-been-published) 것은 이미 게시 된 후에 새 이름을 사용 하는 패키지를 업로드 한 후 새 제출을 만들면 여기에 다른 예약 된 이름을 선택할 수 있습니다.

언어에 대 한 패키지를 업로드 하지 않은 작업 중인 및 둘 이상의 이름을 예약한 경우 이름은 끌어오는 해당 언어에는 연결 된 패키지 없는 하므로 예약 된 앱 이름 중 하나를 선택 해야 합니다.

> [!NOTE]
> 합니다 **제품 이름** 있습니다 선택 스토어 목록에서 작업 하는 언어에서에 적용 됩니다. 고객 응용 프로그램을 설치할 때 표시 되는 이름 영향을 주지 않습니다. 설치 되는 패키지 매니페스트에서 해당 이름이 제공 됩니다. 혼동을 피하기 위해 각 언어의 패키지 및 저장소 목록에 동일한 이름을 사용 하는 것이 좋습니다.
...
앱을 처음으로 제출하려는 경우에는 이 필드를 비워둡니다. 기존 앱에 대 한 업데이트를 고객에 게 최신 릴리스의 변경 내용 수입니다. 이 필드는 1,500자로 제한됩니다. (전에는 이 필드가 **릴리스 정보**라고 불림).
...
> [!NOTE]
> 이러한 기능을 사용자 고유의 글머리 기호를 추가 하지 않도록 사용자 저장소 목록에 글머리 기호 표시 됩니다.
...
비디오 트레일러 및 프로 모션 이미지를 포함 하 여 제품에 대 한 추가 자산을 제출할 수 있습니다. 모두 선택 사항이지만, 가능한 자산을 많이 업로드 하는 것이 좋습니다. 이런 이미지는 고객의 제품에 대한 이해를 높이고, 더욱 소구력 높은 목록을 만드는 데 도움을 줍니다.

자세한 내용은 참조 하세요. [트레일러 및 추가 자산](app-screenshots-and-images.md#trailers-and-additional-assets)합니다.
