...
[**IBackgroundTask**](https://msdn.microsoft.com/library/windows/apps/br224794) 인터페이스를 구현하는 클래스를 작성하여 백그라운드로 코드를 실행할 수 있습니다. 이 코드는 특정 이벤트를 사용 하 여, 등 [**SystemTrigger**](https://msdn.microsoft.com/library/windows/apps/br224839) 또는 [**MaintenanceTrigger**](https://msdn.microsoft.com/library/windows/apps/hh700517)트리거될 때 실행 됩니다.

다음 단계에서는 [**IBackgroundTask**](https://msdn.microsoft.com/library/windows/apps/br224794) 인터페이스를 구현하는 새 클래스를 작성하는 방법을 보여 줍니다.

1.  백그라운드 작업에 대한 새 프로젝트를 만들고 솔루션에 추가합니다. 이 작업을 수행 하려면 **솔루션 탐색기** 에서 사용자 솔루션 노드를 마우스 오른쪽 단추로 클릭 하 고 **추가**선택 \> **새 프로젝트**입니다. **Windows 런타임 구성 요소** 프로젝트 형식에는 프로젝트 이름을 선택한 다음 확인을 클릭 합니다.
2.  UWP(유니버설 Windows 플랫폼) 앱 프로젝트에서 백그라운드 작업 프로젝트를 참조합니다. C# 또는 응용 프로그램 프로젝트에서 c + + 응용 프로그램에 대 한 **참조** 를 마우스 오른쪽 단추로 클릭 하 고 **새 참조 추가**선택 합니다. **솔루션** 아래에서 **프로젝트**를 선택하고 백그라운드 작업 프로젝트의 이름을 선택한 후 **확인**을 클릭합니다.
3.  백그라운드 작업 프로젝트에 [**IBackgroundTask**](/uwp/api/Windows.ApplicationModel.Background.IBackgroundTask) 인터페이스를 구현 하는 새 클래스를 추가 합니다. [**IBackgroundTask.Run**](/uwp/api/windows.applicationmodel.background.ibackgroundtask.run) 메서드는 지정된 된 이벤트 트리거되는; 때 호출 되는 필수 진입점 이 메서드는 모든 백그라운드 작업에 필요 합니다.

> [!NOTE]
> 자체 백그라운드 작업 클래스&mdash;및 백그라운드 작업 프로젝트의 다른 모든 클래스&mdash; **봉인** (또는 **최종**) 되는 **공용** 클래스 여야 합니다.

다음 예제 코드는 백그라운드 작업 클래스에 대 한 매우 기본적인 시작점을 표시합니다.
...
4.  백그라운드 작업에서 비동기 코드를 실행할 경우 백그라운드 작업은 deferral을 사용해야 합니다. 지연을 사용 하지 않으면 하는 경우 다음 백그라운드 작업 프로세스를 종료할 수 예기치 않게 **실행** 메서드를 사용 하는 모든 비동기 작업 실행이 완료 되기 전에 반환 하는 경우.

비동기 메서드를 호출 하기 전에 **실행** 메서드에서 지연을 요청 합니다. 비동기 메서드에서 액세스할 수 있도록 클래스 데이터 구성원에 게는 지연을 저장 합니다. 비동기 코드가 완료된 후에 deferral 완료를 선언합니다.

다음 예제 코드는 지연, 저장 하는 것을 가져오고 비동기 코드가 완료 되 면이 해제 합니다.
...
> [!NOTE]
> C#에서는 **async/await** 키워드를 사용하여 백그라운드 작업의 비동기 메서드를 호출할 수 있습니다. C + + / CX, 작업 체인을 사용 하 여 비슷한 결과 구현할 수 있습니다.
...
> [!NOTE]
> 백그라운드 작업을 등록을 위한 전용 포리스트로 함수를 만들 수도 있습니다&mdash; [등록 백그라운드 작업을](register-a-background-task.md)참조 하십시오. 이 경우 다음 세 단계를 사용 하는 대신 있습니다 수 단순히 트리거를 구성 하 고 작업 이름, 작업 진입점 및 (선택 사항) 조건이 함께 등록 함수를 제공 합니다.

## <a name="register-the-background-task-to-run"></a>실행할 백그라운드 작업 등록

1.  백그라운드 작업 [**BackgroundTaskRegistration.AllTasks**](https://msdn.microsoft.com/library/windows/apps/br224787) 속성을 통해 반복 하 여 이미 등록 되어 있는지 여부에 대해 알아봅니다. 이 단계는 중요합니다. 앱이 기존 백그라운드 작업 등록을 확인하지 않는 경우 작업을 여러 번 등록하기 쉬우므로 성능에 문제가 발생하거나 작업이 완료되기 전에 작업의 가용 CPU 시간을 모두 사용할 수 있습니다.

다음 예제에서는 **AllTasks** 속성에 반복 되 고 플래그 변수 작업이 이미 등록 하는 경우 true로 설정 하는.
...
등이 코드 새 백그라운드 작업을 만들고 **TimeZoneChanged** 트리거 발생할 때 실행 되도록 설정 합니다.
...
앱에서 백그라운드 작업의 결과를 가져올 수 있도록 [**BackgroundTaskCompletedEventHandler**](https://msdn.microsoft.com/library/windows/apps/br224781)를 사용하여 메서드를 등록해야 합니다. 응용 프로그램을 시작 하거나 다시 시작 된 백그라운드 작업이 마지막으로 응용 프로그램 없이 된 이후 완료 하는 경우 표시 된 메서드가 호출 됩니다. 앱이 포그라운드에서 현재 실행 중인 경우에는 백그라운드 작업이 완료되면 OnCompleted 메서드가 즉시 호출됩니다.
...
## <a name="declare-in-the-app-manifest-that-your-app-uses-background-tasks"></a>앱 백그라운드 작업을 사용 하는 응용 프로그램 매니페스트에서 선언

백그라운드 작업을 실행하려면 먼저 앱 매니페스트에서 각 백그라운드 작업을 선언해야 합니다. 앱을 매니페스트에 나열 되지 않은 트리거를 백그라운드 작업에 등록 하려고 하는 경우에 백그라운드 작업의 등록 "런타임 클래스를 등록 되지 않았습니다." 오류와 함께 실패 합니다.

1.  Package.appxmanifest라는 파일을 열어 패키지 매니페스트 디자이너를 엽니다.
2.  **선언** 탭을 엽니다.
3.  **사용 가능한 선언** 드롭다운에서 **백그라운드 작업**을 선택하고 **추가**를 클릭합니다.
4.  **시스템 이벤트** 확인란을 선택합니다.
5.  **진입점:** textbox의 경우 네임 스페이스와이 예제는 Tasks.ExampleBackgroundTask에 되는 배경 클래스의 이름을 입력 합니다.
6.  매니페스트 디자이너를 닫습니다.
