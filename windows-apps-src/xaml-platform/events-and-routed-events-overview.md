---
description: 사용 하는 경우 Windows 런타임 앱에서 이벤트의 프로그래밍 개념에 설명 했습니다 C#, Visual Basic 또는 Visual c + + 구성 요소 확장 (C + + /cli CX) 프로그래밍 언어와 XAML UI 정의 합니다.
...
실제 처리기는 모든 앱 코드 및 코드 숨김에 사용하는 프로그래밍 언어로 작성합니다. `Click="ShowUpdatesButton_Click"` 특성을 사용하여 XAML의 태그가 컴파일되고 구문 분석될 때 IDE의 빌드 작업에 포함된 XAML 태그 컴파일 단계와 앱이 로드될 때 이벤트 XAML 런타임 구문 분석 작업에서 `ShowUpdatesButton_Click`이라는 메서드를 앱 코드의 일부로 발견할 수 있다는 계약을 만들었습니다. `ShowUpdatesButton_Click` 모든 처리기 (대리자 기반)는 호환 되는 메서드 시그니처를 구현 하는 메서드가 있어야 합니다 [ **클릭** ](https://msdn.microsoft.com/library/windows/apps/br227737) 이벤트입니다. 예를 들어 이 코드에서는 `ShowUpdatesButton_Click` 처리기를 정의합니다.
...
**참고**  private 클래스 멤버로 헤더에 선언 된 partial 클래스에서 c + +에 대 한 이벤트 처리기를 정의 가져오기 하지 않습니다. C++ 프로젝트에 대한 빌드 작업에는 C++에 대한 XAML 형식 시스템 및 코드 숨김 모델을 지원하는 코드를 생성하는 작업이 포함됩니다.
...
C + + /CX에서는 또한 사용 하 여 합니다 **+=** 구문을 있지만 기본에서 차이가 있습니다 C# 양식:
...
드물긴 하지만 명시적으로 이벤트 처리기를 제거하려는 몇 가지 경우가 있습니다. 확인할 수 있습니다.
...
[**FrameworkElement.Unloaded** ](https://msdn.microsoft.com/library/windows/apps/br208748) 하거나 [ **Page.NavigatedFrom** ](https://msdn.microsoft.com/library/windows/apps/br227507) 상태 관리 및 개체 수명 이러한 적절 한 위치에 있는 가능한 이벤트 트리거 에 다른 이벤트에 대 한 처리기를 제거 하기 위해 사용할 수 있습니다.
...
- [**현재**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.uielement.dragleave)
- [**끌기**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.uielement.dragover)
...
- 요소의 **Background** 또는 **Fill** 속성 값이 **null**이 아닙니다. A **null** [ **브러시** ](/uwp/api/Windows.UI.Xaml.Media.Brush) 결과 투명도 값 및 적중 테스트 표시 안 함. 요소가 투명하나 적중 횟수를 테스트할 수 있게 하려면 **null** 대신 [**Transparent**](https://msdn.microsoft.com/library/windows/apps/hh748061) 브러시를 사용하세요.
...
일부 컨트롤에는 적중 횟수 테스트를 위한 특수한 규칙이 있습니다. 예를 들어 [**TextBlock**](https://msdn.microsoft.com/library/windows/apps/br209652)에는 **Background** 속성이 없으나 해당 차원의 전체 영역 내에서 여전히 적중 횟수 테스트가 가능합니다. [**이미지** ](https://msdn.microsoft.com/library/windows/apps/br242752) 하 고 [ **MediaElement** ](https://msdn.microsoft.com/library/windows/apps/br242926) 미디어에 알파 채널이 같은 투명 한 내용에 관계 없이 자신의 정의 된 사각형 차원을 통해 컨트롤 적중 테스트 표시 되는 원본 파일입니다. [**WebView** ](https://msdn.microsoft.com/library/windows/apps/br227702) 컨트롤에 특별 한 적중 횟수 테스트 동작 하므로 호스 티 드 HTML 및 화재 스크립트 이벤트에 의해 입력을 처리할 수 있습니다.
