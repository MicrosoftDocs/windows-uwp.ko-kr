...
태그는 언어 및 스크립트 하위 태그에 대해 일치하지만 지역 또는 일부 다른 하위 태그에서 다릅니다. 예를 들어, EN-US, en 또는 EN-US 일치 en-\*합니다.
...
- 리소스: "en-US", "pt-BR".
- 높은 점수의 리소스: "en-US".
- 설명: 비교 "PT-PT"로 시작 하지만 정확히 일치를 찾지 못했습니다. 사용자의 언어 목록에 "pt-BR"이 있기 때문에 부분 일치는 "pt-BR"과 비교하도록 연기되었습니다. 다음 언어 비교는 "en-US"이며 정확한 일치입니다. 따라서 우위 리소스가 "EN-US"입니다.
...
- 리소스: "zh-Hans-CN", "und".
- 높은 점수의 리소스: "und".
- 설명: 비교 "EN-US"로 시작 하지만 "en" (부분 또는 그 이상)에 따라 일치 하는 찾지 않습니다. "und" 태그가 지정된 리소스가 있으므로 일치 알고리즘은 이를 사용합니다.
...
- 설명: 비교 첫 번째 항목에 대 한 정확한 일치 하는 찾아서 하므로 "und" 레이블이 지정 된 리소스에 대 한 확인 하지 않습니다.
...
- 높은 점수의 리소스: "und-Cyrl".
- 설명: 비교 (부분 또는 그 이상) "ru" 일치 하는 항목을 찾지 못하면 하 고 따라서 언어 태그 "und"와 일치 합니다. 언어 태그 "ru"와 관련된 억제 스크립트 값 "Cyrl"은 리소스 "und-Cyrl"과 일치합니다.
...
세계 시장에 앱을 만들 때 몇 가지 문제를 표시 하는 중요 한 지역 변형 사용 하 여 이러한 언어: "얼마나 많은 다른 지역 변형 해야 지원 되나요"? "어떤 변형을 지원해야 할까요?" "내 앱에 대한 이러한 지역 변형 자산을 관리하는 가장 효과적인 방법은 무엇인가요?" 이 항목에서는 이러한 모든 질문 내용까지 다루지는 않습니다. 그러나 Windows의 언어 일치 매커니즘은 지역 변형을 처리하는 데 도움이 되는 기능을 제공합니다.
...
영어는 특별한 주의가 필요합니다. 앱에 두 영어 변형에 대한 지역화를 추가하는 경우 미국 영어 및 영국 영어, 또는 "국제" 영어가 될 가능성이 높습니다. 위에서 언급했듯이 미국 외의 특정 지역은 미국 철자법을 따르며 Windows 언어 일치는 이를 고려합니다. 이 경우에 해당 변형에 대해 지역 중립적 태그인 "en"를 사용하는 것은 권장되지 않으며 "en-GB" 및 "en-US"를 사용하는 것이 좋습니다. 그러나 (지정된 된 리소스는 별도 변형 필요 하지 않은, 경우 "en" 사용할 수 있습니다.) "EN-GB" 또는 "EN-US"를 "en"으로 대체 하면 Windows에서 제공 하는 직교 지역 선호도 사용 하 여 방해는입니다. 세 번째 영어 지역화가 추가되면 필요에 따라 추가 변형에 대한 특정 또는 매크로 지역 하위 태그를 사용하지만(예: "en-CA", "en-AU" 또는 "en-053") 계속해서 "en-GB" 및 "en-US"를 사용합니다.
