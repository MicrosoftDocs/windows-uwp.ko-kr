...
앱과 IAP(앱에서 바로 구매 제품) 카탈로그가 있는 경우 Windows 스토어 컬렉션 API 및 Windows 스토어 구매 API를 사용하여 서비스에서 이러한 제품에 대한 소유권 정보에 액세스할 수 있습니다.
서비스에서 제품 보기 및 권한 부여
---
...
1.  [Azure AD에서 웹 응용 프로그램을 구성합니다](#step-1:-configure-a-web-application-in-azure-ad). 이 응용 프로그램은 Azure AD 컨텍스트에서 플랫폼 간 서비스를 나타냅니다.
2.  [Windows 개발자 센터 대시보드에서 Azure AD 클라이언트 ID와 응용 프로그램을 연결합니다](#step-2:-associate-your-azure-ad-client-id-with-your-application-in-the-windows-dev-denter-dashboard).
3.  서비스에서 게시자 ID를 나타내는 [Azure AD 액세스 토큰을 생성](#step-3:-retrieve-access-tokens-from-azure-ad)합니다.
4.  Windows 앱의 클라이언트 쪽 코드에서 현재 사용자의 ID를 나타내는 [Windows 스토어 ID 키를 생성](#step-4:-generate-a-windows-store-id-key-from-client-side-code-in-your-app)하고 Windows 스토어 ID 키를 다시 사용자의 서비스로 전달합니다.
5.  필요한 Azure AD 액세스 토큰 및 Windows 스토어 ID 키가 있으면 [서비스에서 Windows 스토어 컬렉션 API를 호출합니다](#step-5:-call-the-windows-store-collection-api-or-purchase-api-from-your-service).
...
-   *resource* 매개 변수에 대해 다음 앱 ID URI(이전에 응용 프로그램 매니페스트의 `"identifierUris"` 섹션에 추가한 URI와 동일함) 중 하나를 지정합니다. 이 프로세스가 끝날 때 세 개의 액세스 토큰이 있어야 하며 각 토큰에는 이러한 앱 ID URI와 연결된 하나의 URI가 있어야 합니다.
    ...
    -   **https://onestore.microsoft.com**: 이후 단계에서 이 URI를 사용하여 만든 액세스 토큰을 사용Windows 스토어 컬렉션 API 또는 구매 API를 직접 호출합니다.
...
Windows 스토어 컬렉션 API 또는 구매 API를 호출하려면 Windows 스토어 ID 키를 가져와야 합니다. 이것은 액세스하려는 제품 소유권 정보의 소유자인 사용자의 ID를 나타내는 JWT(JSON 웹 토큰)입니다. 이 키의 클레임에 대한 자세한 내용은 [Windows 스토어 ID 키에서 클레임](#windowsstorekey)을 참조하세요.
...
-   앱의 [**GetCustomerCollectionsIdAsync**](https://msdn.microsoft.com/library/windows/apps/mt608674) 또는 [**GetCustomerPurchaseIdAsync**](https://msdn.microsoft.com/library/windows/apps/mt608675)에서 검색한 Windows 스토어 ID 키입니다. 이 키는 액세스하려는 제품 소유권 정보의 소유자인 사용자의 ID를 나타냅니다.
...
| aud                                                                    | 대상 그룹을 식별합니다. 다음 값 중 하나여야 합니다. **https://collections.mp.microsoft.com/v6.0/keys** 또는 **https://purchase.mp.microsoft.com/v6.0/keys**.                                                                                                                                                                                                                    |
...
| http://schemas.microsoft.com/marketplace/2015/08/claims/key/userId     | 서비스의 컨텍스트에서 현재 사용자를 식별하는 사용자 ID입니다. 이것은 키를 생성할 때 [**GetCustomerCollectionsIdAsync**](https://msdn.microsoft.com/library/windows/apps/mt608674) 또는 [**GetCustomerPurchaseIdAsync**](https://msdn.microsoft.com/library/windows/apps/mt608675) 메서드의 선택적 *publisherUserId* 매개 변수로 전달하는 동일한 값입니다. |
