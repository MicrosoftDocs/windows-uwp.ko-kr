...
- 각 PRI 파일에는 리소스 맵이라고 하는 이름이 지정된 리소스 컬렉션이 들어 있습니다. 패키지에서 PRI 파일을 로드할 때 리소스 맵 이름이 패키지 ID 이름과 일치하는지 확인합니다.
