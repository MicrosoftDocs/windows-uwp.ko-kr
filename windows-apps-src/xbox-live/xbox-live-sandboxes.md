...
ms.locfileid: "8336608"
---
# <a name="xbox-live-sandboxes-intro"></a>Xbox Live 샌드박스 소개

[Xbox Live 서비스 구성](xbox-live-service-configuration.md)정보를 구성 해야 온라인으로 타이틀에 대 한 일반적으로 [파트너 센터](https://partner.microsoft.com/dashboard)에서 설명 했습니다.  이 정보에 타이틀에서 플레이어의 잠금을 해제할 수 있는 도전 과제를 표시 하려는 순위표, 매치 메이 킹 구성 등이 포함 됩니다.

서비스 구성을 변경한 경우 이러한 변경 내용을 Xbox Live의 나머지 부분에 의해 적용 됩니다 타이틀 볼 수 전에 파트너 센터에서 게시 해야 합니다.

개발 샌드박스 섞어를 게시 합니다.  이러한 격리 된 환경에서 타이틀에 대 한 변경 내용에 작동 수 있습니다.  이러한에 설명 된 몇 가지 이점이 제공는 섹션 아래 합니다.
...
4. (일회성) 소매에서 무언가 테스트 하거나 즐겨 찾는 Xbox 게임을 플레이할 취하도록 하려면 해야 하는 경우에 샌드박스 소매 다시 전환 해야 합니다.
...
아래와 같이 적절 한 섹션으로 "Xbox Live"를 클릭 합니다.
...
* [PowerShell 갤러리](https://www.powershellgallery.com/packages/XboxlivePSModule)에서 사용 하는 PowerShell 창을 엽니다.
    ...
    3. 즉, 설정 XblSandbox XDKS.1 또는 Get XblSandbox cmdlet를 실행 합니다.
...
* zip 파일에서 사용 하도록 [https://aka.ms/xboxliveuwptools](https://aka.ms/xboxliveuwptools), PowerShell 창을 열고
    ...
    2. 즉, 설정 XblSandbox XDKS.1 또는 Get XblSandbox cmdlet를 실행 합니다.
...
일반 정품으로 다시 전환 하려면 간단 하 게 제공 하는 두 번째 인수인으로 합니다.
...
콘솔에서 샌드박스를 변경 하려면 Windows 개발자 포털을 사용할 수 있습니다.  이 작업을 수행 하려면 콘솔에서 "개발자 홈"로 이동 하 고 사용 하도록 설정 합니다.
...
개발 프로세스 동안 팀에서 개발자 간에 액세스 관리 하려면 샌드박스 기능을 사용할 수 있습니다.  예를 들어 다음 개발 팀과 테스터 간에 데이터를 분리 하는 것이 좋습니다.

샌드박스가 적용 된 데이터에는 다음이 포함 됩니다.
- 도전 과제, 순위표 및 통계 사용자에 대 한 합니다.  다른 샌드박스를 하나의 샌드박스 내에서 사용자에 대 한 누적 도전 과제 번역 하지 마십시오.
...
비 샌드박스가 적용 된 데이터는 주로 소셜 정보입니다.  예를 들어 사용자가 다른 사용자와 같이, 관계는 샌드박스 독립적 하도록 합니다.
...
*개발* 및 *테스트* 샌드박스를 사용 하 여 위의 예제를 수행 합니다.  아마도 서비스 구성 샌드박스, 간에 동일 이지만, 개발자가 멀티 플레이 기능 만드는 서로 매치 메이 킹 테스트 해야 합니다.  테스터는 멀티 플레이 테스트도 합니다.
