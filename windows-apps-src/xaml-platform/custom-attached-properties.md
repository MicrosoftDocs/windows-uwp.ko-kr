...
> [!WARNING]
> Windows 런타임 XAML 구현에 대 한 기존의 제한 사용자 지정 연결 된 속성 애니메이션 효과 적용할 수 없는 것입니다.
...
사용자 지정 연결된 속성 정의가 사용자 지정 종속성 속성과 가장 다른 부분은 접근자 또는 래퍼 정의 방식입니다. [사용자 지정 종속성 속성](custom-dependency-properties.md)에서 설명 하는 래퍼 기술을 사용 하는 대신 제공 해야 정적 **Get * * * PropertyName* 와 **설정 * * * PropertyName* 연결 된 속성 접근자 메서드. 비XAML 시나리오에서는 다른 호출자도 접근자를 사용하여 값을 설정할 수 있으나 접근자는 주로 XAML 파서에서 사용합니다.

> [!IMPORTANT]
> 접근자를 제대로 정의 하지 않으면 하 고 XAML 프로세서가 연결 된 속성에 액세스할 수 없는 것은 모든 사용자에 게 사용 하 여 XAML 파서 오류를 가져옵니다. 디자인 및 코딩 도구도 참조된 어셈블리에서 사용자 지정 종속성 속성이 발견되는 경우 "\*Property" 규칙에 따라 식별자를 명명하는 경우가 많습니다.
...
시그니처는 **설정 * * * PropertyName* 이 접근자 여야 합니다.
...
> [!NOTE]
> 속성 요소 구문을 통해 그 용도 연결된 된 속성을 정의 하는 것도 가능. 이 경우 값에 형식 변환은 필요하지 않지만 의도한 값을 XAML에서 생성할 수 있는지 확인해야 합니다. [**VisualStateManager.VisualStateGroups**](https://msdn.microsoft.com/library/windows/apps/hh738505)는 속성 요소 사용만 지원하는 기존 연결된 속성의 예입니다.
...
C +에서 연결 된 속성을 정의 + CX는 다소 복잡 합니다. 헤더 및 코드 파일 사이에서 팩터링하는 방법을 결정해야 합니다. 또한 [사용자 지정 종속성 속성](custom-dependency-properties.md)에 설명된 이유로 인해 **get** 접근자만 있는 속성으로 식별자를 노출해야 합니다. C + + /cli CX이 필드 속성의 관계를 정의 해야 명시적으로.NET **readonly** 키워드 및 암시적 사용 하는 대신 간단한 속성의 백업 합니다. 또한, 앱이 처음 시작되고 연결된 속성이 필요한 XAML 페이지가 로드되기 전에 도우미 함수 내에서 연결된 속성을 등록해야 합니다. 이 함수는 한 번만 실행됩니다. 일부 및 전체 종속성 속성이나 연결된 속성에 대해 속성 등록 도우미 함수를 호출하는 일반적인 위치는 app.xaml 파일의 코드 안에 있는 **App** / [**Application**](https://msdn.microsoft.com/library/windows/apps/br242325) 생성자입니다.
...
> [!NOTE]
> C + +를 사용 하 여 XAML UI를 작성 하는 경우 포함 해야 머리글을 언제 든 지 연결된 된 속성을 정의 하는 사용자 지정 형식 XAML 페이지에 해당 형식을 사용 하는. 각 XAML 페이지에는 관련된 .xaml.h 코드 숨김 헤더가 있습니다. 여기에 연결된 속성의 소유자 형식 정의에 대한 헤더를 포함해야 합니다(**\#include** 사용).
...
> [!NOTE]
> 패널 작동 하는 방법에 자세한 내용은 [XAML 사용자 지정 패널 개요](https://msdn.microsoft.com/library/windows/apps/mt228351)를 참조 하십시오.
