...
-   픽셀 및 래스터 좌표. 픽셀은 래스터 디스플레이의 단일 점이며 디스플레이의 위치를 나타내는 고유한 x, y 좌표 쌍이 있습니다. ("픽셀" 이라는 용어는 대개 서로 교환해 서 표시를 구성 하는 물리적 픽셀 사이의 디스플레이에 전송 되기 전에 픽셀의 색 및 알파 값을 포함 하는 데 필요한 주소 지정 가능 메모리 요소입니다.) 래스터 Api에서 종종이 1:1 집합과 물리적 픽셀 모눈을 표시 하는 픽셀 요소의 사각형 그리드로 처리 됩니다. 래스터 좌표계는 왼쪽 위에서 시작되며 그리드의 맨 왼쪽 위에 있는 (0, 0)의 픽셀을 사용합니다.
