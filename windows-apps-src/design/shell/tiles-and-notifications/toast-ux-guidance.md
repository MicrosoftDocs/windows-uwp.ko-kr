...
ctionable 알림은 사용자가 생산성 그래야 듣는 사람도 작업을 수행 하 고 앱 또는 웹 사이트를 사용 하 여 멋진 있고 효율적인 경험 하는 데 매우 강력한 도구입니다.  다음 기회 수많은 됩니다! 아이디어 브레인스토밍 도움말을 원한다 면 자유롭게 windows 알림 팀에 문의 하십시오.  본인 
...
이 두 가지 방식으로 함께 관련 콘텐츠를 유지 하므로 gorup 알림은 (즉, 다른 스포츠 리그 스포츠 앱에서는 좌우 또는 그룹 채팅 메시지를 정렬할 생각). 헤더는 보다 섬세 하지만 심사 하 고 알림을 더 빠르게 골라낼 수 있도록 모두 컬렉션은 알림 그룹화 하는 보다 명확 하 게 합니다. 
