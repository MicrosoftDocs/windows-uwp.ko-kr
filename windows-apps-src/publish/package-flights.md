...
패키지를 항공편으로 사용 하면 패키지만 다릅니다. 상점 목록 세부 정보는 모든 고객에 게 동일 합니다. 비행 그룹의 모든 사용자는 패키지에 포함 된 패키지를 제공 하지만 비행 그룹에 없는 고객은 계속 해 서 일반 (비 플라이트) 패키지를 수신 합니다.  이후에 모든 고객이 사용할 수 있는 패키지의 패키지를 만들려는 경우에는 플라이트 되지 않는 전송에서이 패키지를 쉽게 사용할 수 있습니다. 패키지 항공편은 [인증과 마찬가지로 인증 프로세스](the-app-certification-process.md)를 통과 해야 합니다.
...
패키지를 지정 하 고 필요한 옵션을 구성한 경우 **스토어에 제출** 을 클릭 합니다. 그러면 패키지 비행이 [앱 인증 프로세스](the-app-certification-process.md)를 진행 합니다. 패키지 비행에 포함 된 패키지는 모든 전송과 마찬가지로 [Microsoft Store 정책을](store-policies.md)준수 해야 합니다.
