...
ms.locfileid: "3931588"
---
# <a name="reviews-report"></a>리뷰 보고서


Windows 개발자 센터 대시보드에 **검토** 보고서에는 저장소에 응용 프로그램을 평가 하는 경우 고객이 입력 한 검토 (주석)을 볼 수 있습니다.

대시보드에서 이 데이터를 보거나 [보고서를 다운로드](download-analytic-reports.md)하여 오프라인으로 볼 수 있습니다. 또는 [Microsoft 저장소 분석 REST API](../monetize/access-analytics-data-using-windows-store-services.md) [app 리뷰를 get](../monetize/get-app-reviews.md) 메서드를 사용 하 여 프로그래밍 방식으로이 데이터를 검색할 수 있습니다.

고객 리뷰에서를 [이 페이지에서 직접](respond-to-customer-reviews.md), 프로그래밍 방식으로 [Microsoft 저장소 리뷰 API를 통해](../monetize/submit-responses-to-app-reviews.md)또는 [개발자 센터 응용 프로그램](https://www.microsoft.com/store/apps/dev-center/9nblggh4r5ws)을 사용 하 여 응답할 수 있습니다.
...
- **내용을 검토**: 기본 설정값은 **등급으로 내용을 검토**하는 유일한 등급 검토 내용이 표시 됨을 의미 합니다. **모든** 모든 등급 리뷰 텍스트를 포함 하지 않는를 선택할 수 있습니다. 참고 **등급 분류** 차트 모든 검토 내용과 관계 없이 항상 표시 됩니다.
...
-   **범주 이름**: 기본 필터는 **모두**입니다. [검토 통찰력 범주는](#review-insight-categories) 특정만 해당 범주와 관련 한 우리는 리뷰를 표시 하도록 선택할 수 있습니다. 
...
**등급 분류** 차트 다음 개요를 얻을 수 있도록이 보고서의 위쪽에 나타납니다. 
- 앱에 대한 평균 평점(별 평점).
- 지난 12개월 동안 앱의 총 평점.
- 각 별 평점에 대한 총 평점.
...
Note **내용을 검토** 페이지 필터를 **검토 내용을** 선택한 경우에이 차트 항상에 모두에서 검사 합니다.

이 차트는 또한 [등급 보고서](ratings-report.md)에서는 응용 프로그램의 등급에 대 한 자세한 내용은 함께 볼 수 있습니다.


< span id = "검토-통찰력-범주 / >

## <a name="insight-categories"></a>이해 범주

**통찰력 범주** 차트 그룹에서 검사를 확인 하는 범주에 따라 검토를 사용 하 여 연결할 수 있습니다.
...
-   리뷰 날짜
-   Microsoft 저장소에 그대로 검토자 이름이 나타납니다.
-   리뷰 작성자의 국가/지역
...
페이지의 리뷰를 날짜별 및/또는 평점별 오름차순이나 내림차순으로 정렬할 수 있습니다. **날짜** 및/또는 **평가**하 여 정렬 하는 옵션을 보려면 **정렬 하 여** 링크를 누릅니다.

또한 특정 단어나 구를 검토 응용 프로그램을 찾으려고 검색 상자를 사용할 수 있습니다. 참고 원래만 고객에 의해 작성 된 텍스트를 검토 검토는 다른 언어로 작성 된 경우에 검색 됩니다. 검토 번역 된 텍스트는 검색 되지 않습니다.
...
위에서 언급 했 듯이 사용자 리뷰를 검색, 고객이 남긴 원본 텍스트를 검색 (시점과 번역 텍스트), **번역 검토** 확인란을 선택 된 경우에.


## <a name="responding-to-customer-reviews"></a>고객 리뷰에 응답

다양 한 고객의 검토에 대 한 응답을 보낼 Microsoft 저장소 개발자 센터 대시보드, [Microsoft 저장소 API를 검토](../monetize/submit-responses-to-app-reviews.md), 또는 [개발자 센터 응용 프로그램](https://www.microsoft.com/store/apps/dev-center/9nblggh4r5ws) 을 사용할 수 있습니다. 자세한 내용은 [고객 리뷰에 응답](respond-to-customer-reviews.md)을 참조하세요.
