...
* Windows 커뮤니티 도구 키트에서 `Microsoft.Toolkit.Win32.UI.XamlHost.XamlApplication` 클래스의 인스턴스를 정의 하는 UWP 앱 프로젝트입니다.
  > [!NOTE]
  > 앱이 모든 XAML 고립 시나리오에서 제대로 작동 하도록 하려면 WPF (또는 Windows Forms) 프로젝트에 `XamlApplication` 개체에 대 한 액세스 권한이 있어야 합니다. 이 개체는 응용 프로그램의 현재 디렉터리에 있는 어셈블리의 UWP XAML 형식에 대 한 메타 데이터를 로드 하기 위한 루트 메타 데이터 공급자로 작동 합니다. 이 작업을 수행 하는 권장 방법은 WPF Windows Forms (유니버설 Windows) 프로젝트와 동일한 솔루션에 **빈 앱 (유니버설 Windows)** 프로젝트를 추가 하 고이 `App` `XamlApplication`프로젝트에서 파생 될 기본 클래스를 수정 하는 것입니다.
  >
  > 이 단계는 자사 uwp 컨트롤을 호스트 하는 것과 같은 간단한 xaml 고립 시나리오에는 필요 하지 않지만 사용자 지정 UWP `XamlApplication` 컨트롤 호스팅을 비롯 하 여 xaml 고립 시나리오의 전체 범위를 지원 하려면이 개체가 필요 합니다. 항상 UWP 프로젝트를 추가 하 고 XAML 아일랜드를 사용 하 `XamlApplication` 는 모든 솔루션에 개체를 정의 하는 것이 좋습니다. 솔루션에는 개체를 `XamlApplication` 정의 하는 프로젝트가 하나만 포함 될 수 있습니다. 앱의 모든 사용자 지정 UWP 컨트롤은 같은 `XamlApplication` 개체를 공유 합니다.
...
5. **찾아보기** 탭을 선택 하 고, 6.0.0 [패키지 (](https://www.nuget.org/packages/Microsoft.Toolkit.Wpf.UI.Controls) 버전 v-preview7 이상)를 검색 하 고, 패키지를 설치 합니다. 이 패키지는 WPF에 래핑된 UWP 컨트롤을 사용 하는 데 필요한 모든 기능을 제공 합니다 ( [InkCanvas](https://docs.microsoft.com/windows/communitytoolkit/controls/wpf-winforms/inkcanvas) 및 [Inktoolbar 모음](https://docs.microsoft.com/windows/communitytoolkit/controls/wpf-winforms/inktoolbar) 및 [windowsxamlhost](https://docs.microsoft.com/windows/communitytoolkit/controls/wpf-winforms/windowsxamlhost) 컨트롤 포함).
    > [!NOTE]
    > Windows Forms 앱은 [6.0.0 패키지 (](https://www.nuget.org/packages/Microsoft.Toolkit.Forms.UI.Controls) 버전 v-preview7 이상)를 사용 해야 합니다.
...
다음으로, WPF 프로젝트와 동일한 솔루션에 UWP 앱 프로젝트를 추가 합니다. Windows 커뮤니티 도구 키트에서 `App` 제공 하는 `Microsoft.Toolkit.Win32.UI.XamlHost.XamlApplication` 클래스에서 파생 되도록이 프로젝트의 기본 클래스를 수정 합니다. 단일 자사 UWP 컨트롤을 호스트 하는 것과 같은 간단한 XAML 고립 시나리오에는이 단계가 필요 하지 않지만, WPF 앱에 `XamlApplication` 는이 개체가 있어야 전체 범위의 xaml 아일랜드 시나리오를 지원할 수 있습니다. XAML 아일랜드를 사용 하는 모든 솔루션에 항상이 프로젝트를 추가 하는 것이 좋습니다.
...
3. **Mainwindow.xaml** 파일에서 기존 `<Grid>` 요소를 다음 xaml로 바꿉니다. 이 XAML은 `<Grid>` [InkCanvas](https://docs.microsoft.com/windows/communitytoolkit/controls/wpf-winforms/inkcanvas) 및 [Inktoolbar](https://docs.microsoft.com/windows/communitytoolkit/controls/wpf-winforms/inktoolbar) 컨트롤 (이전에 네임 스페이스로 정의한 Controls 키워드가 접두사로 추가 됨)을에 추가 합니다.
...
4. **Mainwindow.xaml** 파일을 저장 합니다.
    ...
    화면과 같이 디지털 펜을 지 원하는 장치를 사용 하 고이 랩을 물리적 컴퓨터에서 실행 하는 경우 이제 앱을 빌드 및 실행 하 고 펜을 사용 하 여 화면에 디지털 잉크를 그릴 수 있습니다. 그러나 펜 지원 장치가 없고 마우스로 서명 하려고 하면 아무 작업도 수행 되지 않습니다. **InkCanvas** 컨트롤은 기본적으로 디지털 펜에 대해서만 활성화 되기 때문에 발생 합니다. 그러나이 동작을 변경할 수 있습니다.
...
7. 생성자를 `MainWindow()` 찾습니다. `InitializeComponent()` 메서드 뒤에 다음 코드 줄을 추가 하 고 코드 파일을 저장 합니다.
...
10. 다음 `ChildChanged` 이벤트 처리기 메서드를 `MainWindow` 클래스에 추가 하 고 코드 파일을 저장 합니다. 호스트 컨트롤이 렌더링 되 면이 이벤트 처리기는를 실행 하 고 calendar 컨트롤의 `SelectedDatesChanged` 이벤트에 대 한 간단한 이벤트 처리기를 만듭니다.
