...
ms.locfileid: "6040727"
---
# <a name="use-connected-storage-to-delete-data"></a>연결 된 저장소를 사용 하 여 데이터를 삭제 합니다.

데이터 blob을 만들어 비동기적으로 삭제 됩니다는 `ConnectedStorageContainer` 에 `ConnectedStorageSpace` 사용자와 호출은 `SubmitUpdatesAsync` 삭제 blobsToDelete 매개 변수에 대 한 명명 된 blob을 나타내는 문자열의 목록을 제공 하는 컨테이너에 대해 메서드.
...
1.  검색 한 `ConnectedStorageSpace` 를 호출 하 여 사용자에 대 한 개체 `GetForUserAsync`.
