...
설정이 숫자: 최소값 - (숫자)이면 이 필드는 설정의 최소 numverical 값을 나타냅니다.
최대값 - (숫자) 이 필드는 설정의 최대 numverical 값을 나타냅니다.

이 설정이 선택: OptionsVariable - ("예" | "아니요")이면 이 필드는 설정 옵션이 변수인지, 유효한 옵션을 다시 부팅 없이 변경할 수 있는지 여부를 나타냅니다.
