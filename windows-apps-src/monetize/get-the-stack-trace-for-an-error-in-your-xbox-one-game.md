---
description: Xbox One에 오류에 대 한 스택 추적을 게임 가져오려면 Microsoft Store 분석 API에서에서이 메서드를 사용 합니다.
title: Xbox One 게임에서 오류에 대한 스택 추적 가져오기
ms.date: 11/06/2018
ms.topic: article
...
ms.locfileid: "57658688"
---
# <a name="get-the-stack-trace-for-an-error-in-your-xbox-one-game"></a>Xbox One 게임에서 오류에 대한 스택 추적 가져오기

Microsoft Store API 수 있도록 분석 Xbox One에 오류에 대 한 게임 스택 추적을 가져와서 Xbox 개발자 포털 (XDP)를 통해 수집 된 및 XDP 분석 파트너 센터 대시보드에서 사용할 수 있는에서이 메서드를 사용 합니다. 이 메서드는 지난 30일 동안 발생한 오류에 대한 스택 추적만 다운로드할 수 있습니다.

이 메서드를 사용 하려면 먼저를 먼저 사용 해야 합니다 [게임에 Xbox One에 오류에 대 한 세부 정보를 가져오기](get-details-for-an-error-in-your-xbox-one-game.md) 스택 추적을 검색 하려면 오류와 연결 된 CAB 파일의 ID를 검색 하는 방법입니다.
...
* 스택 추적을 검색하려는 오류와 연결된 CAB 파일의 ID를 가져옵니다. 이 ID를 가져오려면 합니다 [게임에 Xbox One에 오류에 대 한 세부 정보를 가져옵니다](get-details-for-an-error-in-your-xbox-one-game.md) 메서드를 사용 하 여 앱에서 특정 오류에 대 한 세부 정보를 검색 합니다 **cabId** 메서드의 응답 본문의 값.
...
| 매개 변수        | 형식   |  설명      |  필수  |
|---------------|--------|---------------|------|
| applicationId | 문자열 | 제품 ID를 검색 하는 스택 추적을 Xbox One 게임입니다. 게임의 제품 ID를 가져오려면 Xbox 개발자 포털(XDP)에서 사용자 게임으로 이동한 후 URL에서 제품 ID를 검색합니다. 또는 Windows 파트너 센터 분석 보고서에서 사용자 상태 데이터를 다운로드 하는 경우 제품 ID.tsv 파일에 포함 됩니다. |  예  |
| cabId | 문자열 | 스택 추적을 검색하려는 오류와 연결된 CAB 파일의 고유 ID입니다. 이 ID를 가져오려면 합니다 [게임에 Xbox One에 오류에 대 한 세부 정보를 가져옵니다](get-details-for-an-error-in-your-xbox-one-game.md) 메서드를 사용 하 여 앱에서 특정 오류에 대 한 세부 정보를 검색 합니다 **cabId** 메서드의 응답 본문의 값. |  예  |

 
### <a name="request-example"></a>요청 예제

다음 예제에서는이 메서드를 사용 하 여 게임을 Xbox One에 대 한 스택 추적을 가져오고 하는 방법에 설명 합니다. 대체는 *applicationId* 게임에 대 한 제품 ID 가진 값입니다.
