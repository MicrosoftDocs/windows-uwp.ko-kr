...
ms.prod: windows
ms.technology: uwp
keywords: xbox live, xbox, 게임, uwp, windows 10, 하나는 xbox, 도구, 플레이어 초기화, 라이브 추적 분석기, LTA, xbox live 계정 도구
...
> [!NOTE]
> 관리 되는 파트너가 다운로드에 포함 된 MatchSim 및 XboxLiveCompute 도구만 사용할 수 있습니다 또는에 등록 된 파트너는 [ID@Xbox](http://www.xbox.com/Developers/id) 프로그램. 사용 가능한 개발자 프로그램에 대 한 자세한 내용은 [개발자 프로그램 개요](https://docs.microsoft.com/windows/uwp/xbox-live/developer-program-overview)를 참조 하십시오. 

## <a name="global-storage"></a>글로벌 저장소
글로벌 타이틀 저장소 명단, 지도, 문제, 또는 이미지 리소스 등의 모든 사용자가 읽을 수 있는 데이터를 저장 하는 데 사용 됩니다. [타이틀 저장소](../storage-platform/xbox-live-title-storage/xbox-live-title-storage.md)의 형식입니다. 테스트 샌드박스의 글로벌 타이틀 저장소를 관리 하는 글로벌 저장소 도구 사용 됩니다. 데이터는 Windows 개발자 센터 또는 Xbox 개발자 포털 (XDP)을 통해 정품에 계속 게시 합니다. 명령줄을 통해 사용할 수 있는 도구는 개발 도구의 일부로 (https://aka.ms/xboxliveuwptools) zip 합니다. [개발자 도구 라이브러리](https://www.nuget.org/packages/Microsoft.Xbox.Services.DevTools)를 사용 하 여 사용자 지정 도구를 만들 수 있습니다.

## <a name="multiplayer-session-history-viewer"></a>멀티 플레이 세션 기록 뷰어
멀티 플레이어 세션 기록 뷰어 멀티 플레이 세션 문서 기록이 삭제 된 문서 등을 통해 모든 변경 내용 기록 타임 라인을 볼 수가 있습니다. 이 도구를 사용 하 여 시간이 지남에 따라 변경 된 MPSD 세션 문서 일어나는지 더 깊게 이해를 제공 됩니다. [개발 도구](https://aka.ms/xboxliveuwptools) zip에서 독립 실행형 도구로 나와 있습니다.
...
개발자 계정에 인증을 관리 하는 Xbox Live 개발자 계정 도구 사용 됩니다. 플레이어 초기화 및 글로벌 저장소와 같은 개발자 자격 증명을 요구 하는 다른 개발자 도구와 상호 작용 하는 데 필요. 명령줄을 통해 사용할 수 있는 도구는 [개발 도구](https://aka.ms/xboxliveuwptools) zip의 일환으로 합니다.
...
[소스로 구성](https://github.com/Microsoft/xbox-live-developer-tools/blob/master/CONFIGASSOURCE.md) 구성 서비스에 통합 하는 것에 대 한 공식적으로 지원 되는 도구와 Api를 제공 하 여 고급 사용자를 수용 하기 위해 Microsoft 개발 도구 모음입니다. 이러한 Xbox Live 서비스 타이틀 도전 과제, 웹 서비스와 신뢰 당사자 순위표에 이르는 서비스를 포함 하 여 개발자 센터에서 일반적으로 구성 됩니다. 대부분의 게임 개발자, 개발자 센터를 사용 하 여 면 충분 합니다. 하지만 고급 사용자를 위해는 자체 프로세스 및 도구에 일반적인 구성 작업을 통합 하는 데 필요한입니다.  소스로 구성은 명령줄 도구 및 기존 워크플로와 파이프라인에 사용자 지정 통합 지원 하기 위해 새로운 Api를 제공 하 여 이러한 시나리오를 지원 하기 위한 것입니다. 명령줄을 통해 사용할 수 있는 도구는 [개발 도구](https://aka.ms/xboxliveuwptools) zip의 일환으로 합니다.
