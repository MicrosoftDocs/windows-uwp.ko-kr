...
마찬가지로, Xbox One 개발 키트에서 연결 된 저장소 컨테이너에는 개발 PC에 XML 파일에서 데이터의 복사본을 사용 하면 해당 데이터를 클라우드로 업로드를 시작 하려면 콘솔 합니다. 그러나 가지 조건에 동기화 되지 것입니다: 개발 키트의 잠금을 가져올 수 없습니다 아니면 사용자가 하지 않기로 하 여 충돌을 해결 하는 경우 콘솔 동작 콘솔의 컨테이너와 클라우드에서 간의 데이터 충돌이 있는 경우 컨테이너를 유지 하 고 콘솔에의 한 버전을 선택 하면 제목 시작 될 때까지 사용자가 오프 라인 재생 하는 경우 작동 합니다.

이러한 명령에의 한 가지 예외 **** 초기화/는 **** 강제로/모든 SCIDs 및 사용자에 대 한 로컬 저장소에 저장 된 데이터를 지웁니다 되지만 클라우드에 저장 된 데이터를 변경 하지 않습니다. 사용자가 콘솔에 로밍 되었으며 제목을 재생 시 클라우드에서 데이터를 다운로드 하는 경우에 것 상태로 콘솔을 배치 하는 데 유용 합니다.
...
공장 설정으로 복원 로컬 콘솔에서 연결 된 저장소의 모든 로컬 데이터를 지웁니다. 클라우드로 유지 되는 데이터 수정 되지 않으며 다운로드할 수 다시 필요에 따라 합니다.

|옵션  |설명  |
|---------|---------|
|/force   |연결 된 저장소를 다시 설정 해야 한다는 것을 확인 합니다. 다음과 같은 메시지가 표시 될 **** 강제로/지정 하지 않고 리셋 명령을 실행 하면: 연결 된 저장소 공장 초기화는 파괴적 작업을 **** 강제로/플래그 아닌 경우이 명령을 초기화를 수행 하지 않는 존재 합니다. |
...
파일은 데이터가 포함 된 XML 파일. 예를 들어 [xbstorage 생성](#xbstorage_generate)을 참조 하세요. 파일의 XML 형식에 대 한 자세한 내용은이 항목의 뒷부분에 [가져오기 및 내보내기 파일 형식](#xbstorage_fileformat)참조 하세요.
...
>
> 동일한 기본 서비스 안내를 사용 하 여 응용 프로그램을 실행 하는 경우이 작업 경합 상태를 일으킬 수 및 연결 된 저장소 공간 콘텐츠의 확정 되지 않은 상태에 있을 수 있습니다.
>
...
더미 데이터를 생성 하 고 지정 된 파일에 저장 \ < 이름 > 합니다. 파일의 XML 형식에 대 한 자세한 내용은이 항목의 뒷부분에 [가져오기 및 내보내기 파일 형식](#xbstorage_fileformat)참조 하세요.    서비스 구성 id (서비스 안내) 00000000-0000-0000-0000-000000000000를 놓고 컴퓨터별 연결 된 저장소 공간에 대 한 계정 설정 됩니다. 해당 값을 변경 하려는 경우 직접 편집할 수 있습니다 파일 생성 됩니다.
...
|옵션  |설명  |
|---------|---------|
|/reserveremainingspace | 연결 된 저장소의 모든 나머지 공간을 예약합니다. 삭제 ConnectedStorage에서 사용할 수 있는 공간을 열립니다. |
...
| /X *: 주소* [*+ accesskey*]  | 호스트 이름 또는 대상된 콘솔의 주소 (콘솔에 **도구 IP** 로 표시 됨)를 지정 하지만 콘솔의 기본 변경 되지 않습니다. 이 옵션을 사용 하지 않는 경우 콘솔의 기본 사용 됩니다. *Accesskey* 은 선택 키를 알고 있는 사용자만 콘솔에 액세스를 제한 하는 데 사용할 수 있는 문자열입니다. **Xbconfig**명령을 사용 하 여 선택 키를 설정**accesskey = * 사용자 키*; 그런 다음, 선택 키를 적용 하려면 콘솔이 다시 시작 합니다. 선택 키를 사용 하 여 구성 된 콘솔에 액세스 하려면 콘솔의 IP 주소 또는 호스트 이름 뒤에 더하기 (+)와 선택 키를 포함 해야 합니다.
> [!NOTE]
...
파일은 데이터가 포함 된 XML 파일. 유형 `gamesaveutil help generate` 예를 생성 하는 방법을 볼 수 있습니다.
...
> [!NOTE]
> 앱 설치 해야 하 고 가져올 하기 위해 데이터를 이미 동기화 않은.
> 
...
/Context 매개 변수를 사용 하 고 파일 이름을 지정 된 경우 \ < infile > 올바르게 채워지는 ContextDescription 섹션에 다음 해당 파일을 사용 하 여 원본 데이터의 위치를 지정 합니다.
...
|옵션  |설명  |
|---------|---------|
|/markcontainerschanged     |힘 때 앱 바뀐 것 처럼 보이게 하는 모든 컨테이너에서 일시 중단 다시 시작 하 고 새 공급자를 가져옵니다. 모든 앱을 /stop을 사용 하 여 선택이 취소 될 때까지 영향을 줍니다.      |
|중지 /                      |모든 시뮬레이션을 중지합니다.         |
...
> [!NOTE]
> 이러한 XML 파일에는 데이터의 형식을 플랫폼에서 동일한를 가져오기 위해는 목적 으로만 내보내고 합니다. 중간 또는 유틸리티 형식, 보관 된 형식이 아니라으로 처리 해야 하므로 이러한 XML 파일에 대 한 데이터 형식을 나중에 변경 될 수 있습니다.
