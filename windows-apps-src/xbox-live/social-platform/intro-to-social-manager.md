...
* 간단한 API 호출 만들기
* 백그라운드에서 실시간으로 활동 서비스를 사용 하 여 최신 정보를 작성 합니다.
* 개발자는 서비스에 모든 추가 부담 없이 동기적으로 소셜 관리자 API를 호출할 수 있습니다.

소셜 관리자 다루는 여러 RTA 구독 및 사용자에 대 한 데이터를 새로 고침의 복잡성을 마스크 하 고 개발자가 관심 있는 시나리오를 만들려는 최신 그래프에 쉽게 액세스할 수 있습니다.
...
* 간소화 된 소셜 API
* 최신 소셜 그래프
* 자세한 정보 표시 되는 정보에 대 한 제어
* Xbox Live 서비스에 호출 수를 줄이기
...
**소셜 그래프**: 장치에 로컬 사용자에 대 한 *소셜 그래프* 만들어집니다. 모든 사용자가 친구에 대 한 정보를 최신 상태로 유지 하는 구조를 만듭니다.
...
* **필터 그룹**: 필터 그룹은 로컬 (호출) 사용자의 *소셜 그래프* 및 일관 되 게 새로운 사용자 지정된 필터 매개 변수에 따라 집합을 반환 합니다.
* **사용자 그룹**: 사용자 그룹을 사용자 목록 하 고 해당 사용자의 지속적으로 새로운 보기를 반환 합니다. 이러한 사용자는 사용자의 친구 들이 목록 외부 될 수 있습니다.

함수 날짜 까지의 *소셜 사용자 그룹* 을 유지 하기 위해 `social_manager::do_work()` 프레임 마다 호출 해야 합니다.
...
소셜 관리자는 xbox 소셜 사용자 그룹 최신 상태로 두고 사용자 그룹 또는 사용자에 게 관계를 필터링 할 수 있습니다.  예를 들어, 사용자의 온라인 상태인 친구의 모두 포함 하 고 현재 제목 재생 xbox 소셜 사용자 그룹을 만들 수 있습니다.  이 유지 최신 친구 시작 하거나 제목 재생이 중지 합니다.
...
`local_user_added`(C + +) | `LocalUserAdded`(C#)-사용자가 소셜 그래프의 로드가 완료 되었을 때 트리거합니다. 초기화 중에 오류가 발생 하는 데 알려줍니다.
...
이 시나리오에서는 멀티 플레이 세션의 사용자와 같은 사용자 목록의 소셜 정보를 할 수 있습니다.
...
소셜 관리자도 알려 이벤트의 형태로 내용입니다.  UI를 업데이트 하거나 다른 논리를 수행 하려면 해당 이벤트를 사용할 수 있습니다.
...
이 예제에서는 제공 추가 컨트롤의 일부를 보여 줍니다.  게임 루프 중 새 사용자 목록을 제공 하는 소셜 사용자 그룹 필터에 의존 하는 대신 소셜 그래프는 게임 루프 외부 초기화 됩니다.  제목에 의존 하는 다음는 `events` 에서 반환 되는 `socialManager->do_work()` 함수입니다.  `events` 목록은 `social_event`, 및 각 `social_event` 마지막 프레임 중에 발생 한 소셜 그래프에 대 한 변경 포함 되어 있습니다.  예를 들어 `profiles_changed`, `users_added`등.  자세한 정보를 찾을 수는 `social_event` API 설명서.
...
생성 된 소셜 사용자 그룹을 정리 합니다. 호출자는 이제 오래 된 데이터가 포함 된 대로 모든 만들어진된 소셜 사용자 그룹을 받아야 하는 모든 참조도 제거 해야 합니다.
