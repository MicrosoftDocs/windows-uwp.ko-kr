...
각 프로 모션 코드에 고객이 코드를 사용 하 고 Microsoft 저장소에서 응용 프로그램 또는 추가 기능 프로그램을 설치 하기 위해 클릭할 수 있는 해당 고유 구입할 수 URL입니다.  사용자가 홍보 코드를 사용하여 설치하기 전에 앱은 [앱 인증 프로세스](the-app-certification-process.md)의 최종 게시 단계를 통과해야 합니다.

단일 사용 코드를 생성 (하 고 각 고객에 게 배포)을 수 또는 지정된 된 수의 고객에 의해 여러 번 사용할 수 있는 코드를 생성 하도록 선택할 수 있습니다.
...
> [!NOTE]
> 응용 프로그램 고객에 게 사용할 수 없는 경우에 프로 모션 코드를 사용할 수 있습니다 ( **Stop 인수를 사용 하 여 **이 제품을 사용할 수 있지만 해당 저장소에서 검색 되지 않는 확인** 을 선택한 경우, 즉: 직접 링크를 사용 하 여 고객 제품의 저장소를 볼 수 있습니다 있지만 목록 수만 다운로드 하기 이전에 제품을 소유 또는 프로 모션 코드를 Windows 10 장치를 사용 하는 경우** 등록의 [검색](choose-visibility-options.md#discoverability) 섹션에서 옵션). 이 옵션을 사용 하 여 프로 모션 코드를 사용 하 여 제품을 확보 하기 위해 Xbox 등 Windows 10 고객 이어야 합니다.


## <a name="order-promotional-codes"></a>홍보 코드 주문

주문 추가 기능 또는 응용 프로그램 프로 모션 코드:
...
3.  **새 홍보 코드 주문** 페이지에서 다음을 입력합니다.
    ...
    -   홍보 코드를 활성화할 시기를 지정합니다. 특정 시작 날짜와 시간을 선택하려면 **코드가 즉시 활성화됨** 확인란 선택을 취소합니다. 그렇지 않은 경우 코드는 활성화 됩니다 즉시 (제품 코드를 사용 하는 고객을 위해 게시 프로세스를 완료 해야) 하지만.
...
3.  선택한 편집기에서 .tsv 파일을 엽니다. 최상의 환경을 위해 Microsoft Excel 같은 표 형식 구조에서 데이터를 표시할 수 있는 응용 프로그램에서 .tsv 파일을 엽니다. 그러나 아무 텍스트 편집기에서나 파일을 열 수 있습니다.
    ...
    -   **교환 가능 URL**: 고객이 코드를 교환하고 앱 또는 추가 기능을 설치할 수 있는 URL입니다. URL 형식은 다음과 같습니다: http://go.microsoft.com/fwlink/?LinkId=532540&mstoken=&lt; promotional_code >
...
고객에 게 프로 모션 코드 (또는 구입할 수 URL)을 배포한 후 무상으로 받을 제품에 대 한 URL을 클릭할 수 있습니다. 교환 가능 URL을 클릭하면 <https://account.microsoft.com/billing/redeem>에서 인증된 **코드 교환** 페이지가 시작됩니다. 이 페이지에는 교환하려는 앱에 대한 설명이 포함되어 있습니다. 고객이 Microsoft 계정으로 로그인하지 않은 경우, 로그인 메시지가 표시됩니다. 고객은 <https://account.microsoft.com/billing/redeem>을 방문하여 직접 코드를 입력할 수 있습니다.
...
> [!NOTE]
> 경우에 따라 고객 수 볼 **구입** 단추를 **설치**하는 대신 응용 프로그램을 프로 모션 코드를 통해 회수할 수 성공적으로. 이 경우 **구입**을 클릭하여 무료로 앱을 설치할 수 있습니다.
