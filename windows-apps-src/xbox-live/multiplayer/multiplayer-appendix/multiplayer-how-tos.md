...
3.  둘 이상의 사용자에 대 한 변경 내용에 가입 하면 불필요 한 작업을 방지 하기 위해 **MultiplayerSessionChanged** 이벤트 처리기에 코드를 추가 합니다. **RealTimeActivityMultiplayerSessionChangeEventArgs.Branch 속성** 및 **RealTimeActivityMultiplayerSessionChangeEventArgs.ChangeNumber 속성**을 사용 합니다. 이러한 속성을 사용 하 여 표시 마지막 변경 내용 추적 하 고 이전 변경 무시 수 있습니다.
...
1.  새 **XboxLiveContext 클래스** 개체를 만듭니다. 타이틀 한 번에이 개체를 만듭니다. 하 고 저장 한 전체 소스 코드에서 필요에 따라 다시 사용 합니다. 세션 구독을 사용 하는 경우에 특히 정확히 동일한 컨텍스트를 사용 하는 데 필요한 것입니다.
...
4.  로컬 변경 작업을 마쳤으면 연락처를 작성 MPSD에 설명 된 대로 **하는 방법: 업데이트 멀티 플레이어 세션**.
...
Xbox One 프로토콜 활성화 하는 동안 **CoreApplicationView.Activated 이벤트** 를 발생 합니다. 멀티 플레이 API의 컨텍스트에서이 이벤트는 사용자가 초대를 수락 하거나 다른 사용자를 조인 발생 합니다. 이러한 작업 제목 조인 사용자 대상 사용자를 사용 하 여 게임 플레이에 연결 하 여에 반응 해야 하는 활성화를 트리거합니다.

| 참고                                                                                       |
|---------------------------------------------------------------------------------------------------------|
| 타이틀 언제 든 지 새로운 활성화 인수가 되 고 길이 대해 코딩할 수 없습니다. |

제목 제목 활성화를 처리 하려면 다음 주요 단계를 수행 해야 합니다.
...
3.  대기실 세션을 사용 하는 경우 **MultiplayerService.SendInvitesAsync 메서드**를 호출 하 여 초대를 보냅니다. 이 메서드는 게임에서 UI 명단 **SystemUI.ShowPeoplePickerAsync 메서드** 또는 **PartyChat.GetPartyChatViewAsync 메서드**를 사용 하 여 건물이 필요할 수 있습니다.
...
일반적인 시나리오는 플레이어를 수집 하는 게임 플레이 세션이 나 매치 메이 킹 세션으로 해당 플레이어가 이동 로비 세션을 만드는 것입니다. 게임 플레이 세션을 공개적으로 있는 없는 경우 게임 클라이언트 충족 하지 않는 한 게임 플레이 세션에 참가할 수 있지만 합니다 `joinRestriction` 설정은이 시나리오에 대 한 대부분의 경우 너무 제한적입니다.

솔루션은 로비 세션 및 게임 세션 연결할 전송 핸들을 사용 합니다.  제목 다음을 수행 하 여이 수행할 수 있습니다.

1. 게임 세션을 만들 때 사용 합니다 `multiplayer_service::set_transfer_handle(gameSessionRef, lobbySessionRef)` 로비 세션 및 게임 세션에 연결 된 전송 핸들을 만드는 API입니다.
...
5. 멤버 로비 세션 인 게임 세션에 액세스할 수 없게 됩니다.
...
3.  **ProtocolActivatedEventArgs** 개체를 검사 합니다. URI에 표시 된 경우 **ProtocolActivatedEventArgs.Uri 속성** 일치 하거나 inviteHandleAccept (허용 된 초대에 해당) 또는 activityHandleJoin (셸 UI 통해 가입 해당)은 URI의 쿼리 문자열을 구문 분석 다음 필드를 추출 하는 키/값 쌍을 사용 하 여 일반 URI 쿼리 문자열 형식의:
    -   수락 된 초대에 대 한:
        1.  핸들
        2.  invitedXuid
        3.  senderXuid
...
사용자의 현재 활동 제목에 대 한 Xbox 대시보드 사용자 환경에 표시 됩니다. 세션을 통해 또는 제목 활성화를 통해 사용자에 대 한 작업을 설정할 수 있습니다. 후자의 경우 사용자가 매치를 통해 또는 게임을 시작 하 여 세션을 입력 합니다.
...
2.  모든 변경 될 때 이러한 메서드를 사용 하 여 MPSD 로컬 변경 내용을 쓸:
    ...
    다른 제목 수정할 수도 있는 공유 부분을 작성 하는 경우 **SynchronizedUpdate** 를 쓰기 모드를 설정 합니다. 자세한 내용은 [세션 업데이트 동기화를](multiplayer-session-directory.md) 참조 하세요.
...
3.  이전 로컬 세션 개체를 무시 하 고 이후 작업은 최신 알려진된 세션 상태를 기반 새로 검색된 된 세션 개체를 사용 합니다.
...
2.  업데이트, MPSD에서 게임 세션에 설명 된 대로 **하는 방법: 업데이트 멀티 플레이어 세션**.
3.  대기실 세션에 대 한 **두고** 메서드를 호출 하 고 해당 세션을 업데이트 합니다.
...
매치 메이 킹 하는 동안 티켓 세션에서 열린 슬롯 채우기 제목 다음과 유사한 단계를 따라야 합니다.
...
2.  매치 메이 킹 서비스를 새 세션 세션이 나 다른 기존 세션의 멤버를 일치 하는 경우에을 사용 안 함 *preserveSession* 매개 변수를 설정 합니다. 제목 게임 플레이 계속 하려면 티켓 세션으로 기존 게임 세션을 다시 사용할 수 있도록 항상 *preserveSession* 매개 변수를 설정 합니다. 제출 된 세션 유지 되며 해당 세션에 추가 되는 일치 하는 모든 플레이어 매치 메이 킹 서비스 확인 한 다음 수 있습니다.

3.  매치 메이 킹 시간의 사용자 설정에 **CreateMatchTicketResponse 클래스** 개체에 반환 된 **CreateMatchTicketResponse.EstimatedWaitTime 속성** 을 사용 합니다.
4.  매치 메이 킹 티켓을 삭제 하 여 필요한 경우 세션에 대 한 취소 하려면 응답 개체에 반환 되는 **CreateMatchTicketResponse.MatchTicketId 속성** 을 사용 합니다. **MatchmakingService.DeleteMatchTicketAsync 메서드**를 사용 하는 티켓 삭제 합니다.
