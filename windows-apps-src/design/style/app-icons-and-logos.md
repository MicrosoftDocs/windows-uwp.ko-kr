...
(이 문서는 앱 자체를 나타내는; 일반 아이콘 지침 [아이콘](icons.md) 문서를 참조 하는 아이콘에 특별히 임)

## <a name="icon-types-locations-and-scale-factors"></a>아이콘 유형, 위치 및 배율 인수

기본적으로 Visual Studio는 자산 하위 아이콘 자산을 저장합니다. 다양 한 유형의 나타나는, 아이콘 및 라고 기능 목록을 다음과 같습니다. 
...
기본적으로 사용 하 여 패키지에서 이미지의 일부가 스토어에서 다른 [제출 프로세스 중에 제공 하는 이미지](https://docs.microsoft.com/en-us/windows/uwp/publish/app-screenshots-and-images)) (함께이 페이지 맨 위에 있는 표에 설명 된 대로 합니다. 그러나 스토어 Windows 10 (Xbox 포함) 고객에 게 목록을 표시할 때 앱 패키지의 로고 이미지를 사용 하는 것을 방지 하며 대신 업로드 한 이미지만 사용 스토어 수가 있습니다. 이렇게 하면 앱의 모양 보다 잘 제어 Store 전반에서 다양 한 디스플레이에서. (단 제품 이전 OS 버전을 지 원하는 경우 해당 고객 수 여전히 이미지에서 패키지에이 옵션을 사용 하는 경우에.) **스토어 로고** 섹션 제출 프로세스의 **스토어 목록** 단계에서이 수행할 수 있습니다.

![앱 제출 프로세스 동안 스토어 로고를 지정합니다.](images/app-icons/storelogodisplay.png)

이 확인란을 선택 하면 **이미지를 표시 하는 저장소** 라는 섹션이 새로 나타납니다. 여기에서 스토어 앱 패키지의 로고 이미지를 대신 사용 하는 3 가지 이미지 크기를 업로드할 수 있습니다: 300 x 300, 150 x 150 71 x 71 픽셀입니다. 300 x 300 크기에만 필요 하지만 3 크기를 제공 하는 것이 좋습니다.
...
Visual Studio **매니페스트 디자이너**를 호출 하면 앱 아이콘을 관리 하기 위한 유용한 도구를 제공 합니다. 

> Visual Studio 2017 없는 경우는 사용할 수 있습니다 (Visual Studio 2017 Community Edition), 무료 버전을 포함 하 여 여러 버전 및 다른 버전 무료 평가판을 제공 합니다. 여기서 다운로드할 수 있습니다.[https://developer.microsoft.com/windows/downloads](https://developer.microsoft.com/windows/downloads)
...
**모든 시각적 자산**, **시각적 자산** 탭에 첫 번째 메뉴 항목 정확 하 게 어떤 그 이름에서 알 수 없습니다: 단추 누르기를 사용 하 여 앱에 필요한 모든 시각적 자산을 생성 합니다.
...
Visual Studio에서는 이미지 파일을 생성 하 고 프로젝트에 추가 합니다. 자산을 변경 하려는 경우 간단 하 게 하는 프로세스를 반복 합니다. 
...
Visual Studio가 기본적으로 배지 로고를 생성 하지 않는 것을 확인 합니다. 배지 로고는 고유 하며 아마도 다른 앱 아이콘이 일치 하지 않아야 하기 때문입니다. 자세한 내용은 [배지 알림은 UWP 앱 문서](/windows/uwp/design/shell/tiles-and-notifications/badges)를 참조 하세요. 


## <a name="more-about-app-icon-assets"></a>앱 아이콘 자산에 대 한 자세한
Visual Studio에서 프로젝트에 필요한 모든 앱 아이콘 자산을 생성 하지만 다른 응용 프로그램 자산 다는 방법을 이해 하기 쉽게 사용자 지정 하려는 경우. 
...
기본적으로 Visual Studio 자산 생성기 권장된 안쪽 여백 보이는 이미지에 적용 됩니다. 이미지 안쪽 여백에 이미 포함 하거나 타일의 끝까지 블리드 이미지, **안쪽 여백 권장 적용** 확인란 선택을 취소 하 여이 기능을 끌 수 있습니다. 
