...
이 호출은는 쿼리 문자열 매개 변수 **insertIndex** (기본값은 0 또는 목록 부분)에 따라 인덱스 목록에 항목을 삽입 됩니다. 요청 본문에 있는 모든 항목 목록에서 해당 지점에 삽입 됩니다. **InsertIndex** 기존 목록의 항목 수가 보다 큰 경우 새 항목이 끝에 삽입 됩니다.
 
항목을 삽입 하는 기능 사양;에 표시 된 필수 필드 있어야 합니다. 그렇지 않은 경우 HTTP 400 반환 됩니다. 마찬가지로, 삽입의 결과 목록 (목록 종류에 따라 정의 됨)의 최대 크기를 초과 하는 경우 다음 HTTP 400 반환 되 고 아무것도 삽입 됩니다.
 
항목이 *없는* 시작 또는 목록의 끝에 삽입 되 면 **경우-일치: versionNumber** 헤더는 요청에 포함 되어야 하는 데 필요한 합니다. 헤더 시작 또는 종료를 삽입 하는 경우에 선택 사항입니다. 있는 경우 헤더 삽입 위치에 관계 없이 유효성이 검사 됩니다. 헤더 **VersionNumber** 목록의 현재 버전 번호입니다. 포함 된 및 필요한 되지 않는 경우 현재 목록 버전 번호를 다음는 HTTP 412 (전제 조건 실패)와 일치 하지 않는 또는 상태 코드가 반환 하 고 응답 본문에는 현재 버전 번호를 포함 하는 목록의 최신 메타 데이터를 포함 됩니다. 다른에서 무시 하는 다른 클라이언트에서 업데이트를 방지 하기 위해서입니다.
 
이 호출은 idempotent; 아닌지 참고 동일한 데이터를 사용 하 여 반복 해 서 호출 여러 삽입 될 수 있습니다. 그러나 목록이 현재 지원 하므로 중복, 반복 해 서 호출 HTTP 400 될 가능성이 코드를 반환 됩니다.
