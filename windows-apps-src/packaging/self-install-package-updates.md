...
Windows10 버전 1607 이상을 대상으로 하는 앱은 [StoreContext](https://docs.microsoft.com/uwp/api/windows.services.store.storecontext) 클래스의 다음 메서드를 사용하여 패키지 업데이트를 다운로드하고 설치할 수 있습니다.
...
이러한 메서드는 [StorePackageUpdate](https://docs.microsoft.com/uwp/api/windows.services.store.storepackageupdate) 개체를 사용하여 사용 가능한 업데이트 패키지를 표시합니다. 업데이트 패키지에 대한 정보를 가져오려면 다음의 **StorePackageUpdate** 속성을 사용하세요.
