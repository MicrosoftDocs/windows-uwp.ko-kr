...
잉크를 [TextBox](https://docs.microsoft.com/en-us/uwp/api/windows.ui.xaml.controls.textbox), [RichEditBox](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.richeditbox)와 같은 UWP 텍스트 컨트롤에서 지 원하는 텍스트 입력에 대 한 기본 제공 필기 보기를 사용자 지정 하 고 이러한 [AutoSuggestBox](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.autosuggestbox)등에서 파생 된 컨트롤.

## <a name="overview"></a>개요

XAML 텍스트 입력된 상자 [Windows Ink](../input/pen-and-stylus-interactions.md)를 사용한 펜 입력에 대 한 지원이 포함 되어 있습니다. 사용자가 Windows 펜을 사용 하 여 텍스트 입력된 상자를 때 텍스트 상자 필기 표면에는 별도 입력된 창을 열지로 변환 합니다.
...
> [!NOTE]
> 필기 보기는 기본적으로 활성화 되지만 컨트롤 별로에 사용 하지 않도록 설정 하 고 텍스트 입력된 패널로 돌아갈 수 있습니다.
...
필기 보기의 내부 텍스트 컨트롤 위에 있는 고 사용자의 필기 기본 설정에 맞게 크기가 (참조 **설정-gt_ 장치-gt_ 펜 & Windows Ink-gt_ 필기-gt_ 텍스트 필드에 직접 쓸 때 글꼴 크기 **). 텍스트 컨트롤 및 응용 프로그램 내에서 해당 위치를 기준으로 보기도 자동으로 맞춰집니다.

시스템을 중요 한 UI를 폐색 보기를 발생 시킬 수 있으므로 더 큰 제어를 수용 하기 위해 응용 프로그램 UI를 재배치 하지 않습니다.

여기에서는 필기 보기에 맞게 사용 되는 기본 텍스트 컨트롤에는 앵커 지정 하려면 [HandwritingView](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview) 는 [TextBox](https://docs.microsoft.com/en-us/uwp/api/windows.ui.xaml.controls.textbox) [PlacementAlignment](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview.placementalignment) 속성을 사용 하는 방법을 보여줍니다.
...
텍스트 제안을 팝업 인식 후보 최상위 후보 올바르지 않은 경우 사용자가 선택할 수 있는 최상위 잉크 목록을 제공 하기 위해 기본적으로 활성화 됩니다.

응용 프로그램에 이미 강력한에서 제공 하는 경우 사용자 지정 인식 기능을 사용할 수 있습니다 [AreCandidatesEnabled](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview.arecandidatesenabled) 속성에 기본 제공 제안을 사용 하지 않도록 설정 하려면 다음 예제에 표시 된 대로.
...
사용자가 잉크 인식에 따라 텍스트 렌더링 사용 하기 위해 필기 기반 글꼴의 미리 정의 된 컬렉션에서 선택할 수 있습니다 (참조 **설정-gt_ 장치-gt_ 펜 & Windows Ink-gt_ 필기-gt_ 필기를 사용 하는 경우 글꼴**).
...
1. 먼저 처리 Loaded 이벤트 응용 프로그램의 시각적 트리 통과 시작 하는 FindInnerTextBox 함수 라고 합니다.
...
2. 그런 다음 FindVisualChildByName 호출 하 여 FindInnerTextBox 함수에서 시각적 트리 (AutoSuggestBox부터 시작)를 통해 반복 시작 합니다.
...
3. 마지막으로,이 함수 TextBox 검색 될 때까지 시각적 트리를 반복 합니다.
...
StackPanel TextBox 보다 큰 이제 그대로 [HandwritingView](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview) 모든 복합 cotnrol 안 보일 하지 않을 수 있습니다.
...
또한 보기에는 중요 한 UI 안 보일 하지 않도록 해야 할 때 유용할 수 있는 [HandwritingView](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview)의 크기를 설정할 수 있습니다.
...
이렇게 하려면 채워집니다 해야 하는 UI 요소 너비의 [HandwritingView](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview) MaxWidth 속성을 바인딩합니다.
...
사용자 지정 템플릿을 [사용자 지정 전송 컨트롤 만들기](https://docs.microsoft.com/windows/uwp/design/controls-and-patterns/custom-transport-controls) 방법 또는 [사용자 지정 편집 컨트롤 샘플](https://github.com/Microsoft/Windows-universal-samples/tree/master/Samples/CustomEditControl)체크아웃 만드는 전체 예제를 확인 합니다.
