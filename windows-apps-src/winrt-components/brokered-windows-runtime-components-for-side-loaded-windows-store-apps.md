...
description: 이 문서에서는 쉽게 터치.NET 앱 업무에 중요 한 핵심 작업을 담당 하는 기존 코드를 사용 하도록 허용 하는 windows 10에서 지원 하는 기업 대상 기능을 설명 합니다.
ms.date: 02/08/2017
ms.topic: article
...
ms.locfileid: "9048420"
---
# <a name="brokered-windows-runtime-components-for-a-side-loaded-uwp-app"></a>테스트용으로 로드하는 UWP 앱의 조정된 Windows 런타임 구성 요소

이 문서에서는 쉽게 터치.NET 앱 업무에 중요 한 핵심 작업을 담당 하는 기존 코드를 사용 하도록 허용 하는 windows 10에서 지원 하는 기업 대상 기능에 설명 합니다.
...
Windows*응용 프로그램 테스트용 로드에 대 한 Windows 런타임 구성 빌드하기 위한*새로운 기능을 포함 합니다. Microsoft는 하나의 프로세스에서 기존 데스크톱 소프트웨어 자산(데스크톱 구성 요소)을 실행하는 한편, UWP 앱에서 이 코드를 조작하는 기능을 설명하는 데 IPC(프로세스 간 통신)라는 용어를 사용합니다. 데이터베이스 응용 프로그램과 Windows에서 NT 서비스를 활용하는 응용 프로그램이 유사한 다중 프로세스 아키텍처를 공유하므로, 이것은 기업 개발자에게 친숙한 모델입니다.
...
>**참고**이 기능은.NET의 사용에 대 한 단독으로 합니다. 클라이언트 앱과 데스크톱 구성 요소는 둘 다 .NET을 사용해서 작성해야 합니다.
...
>**참고**열거형은 데스크톱 구성 요소와이 시간에 테스트용으로 로드 된 응용 프로그램 간의 Windows 런타임 구성 요소 계약에서 지원 되지 않습니다.
...
>**참고**설정-에서 테스트용 로드를 켤 수 있지만&gt; & 보안-업데이트&gt; 개발자를 위한 합니다.
...
한 번 참조**winmd**만들어집니다 (프로젝트의 대상 폴더 아래 "참조" 폴더)는 손으로 프로젝트가 각 응용 프로그램 테스트용으로 로드 하는 지연 (복사) 및 참조 합니다. 이 내용은 다음 섹션에서 자세히 설명하겠습니다. 위의 빌드 규칙에 포함 된 프로젝트 구조 되도록 구현 및 참조**winmd**혼동을 피하기 위해 빌드 계층 구조에서 명확 하 게 분리 된 디렉터리에 있습니다.
...
이 응용 프로그램 구성에 적용되지 않는 outOfProcessServer 범주에 항목이 여러 개 있기 때문에, 범주는 inProcessServer입니다. 하지만 유의 합니다 <Path> 구성 요소는 항상 clrhost.dll 포함 (이것은**하지**적용 하 고 다른 값을 지정 하면 정의 되지 않은 방식으로 실패).
...
"계약 정의" 섹션의 설명대로, 데스크톱 구성 요소의 참조 winmd를 프로젝트에서 참조해야 합니다. Visual Studio 프로젝트 시스템은 일반적으로 이름이 같은 2개 수준의 디렉터리 구조를 만듭니다. 샘플에서는 이 구조가 EnterpriseIPCApplication\\EnterpriseIPCApplication입니다. 참조 **winmd**이 두 번째 수준 디렉터리 및 대화 상자를 사용 하 여 프로젝트 참조를 수동으로 복사한 (클릭를**찾습니다.** 단추)를 찾아이 **winmd**를 참조 합니다. 그러면 데스크톱 구성 요소의 최상위 수준 네임스페이스(예제: Fabrikam)가 프로젝트의 참조 부분에서 최상위 수준 노드로 나타납니다.

>**참고** 반드시**참조 winmd**를 사용 하 여테스트용으로 로드 되는 응용 프로그램의 합니다. **구현 winmd**를 통해 실수로 수행 하는 경우를 테스트용으로 로드 하는 앱 디렉터리로 참조, 될 가능성이 받게 됩니다 "IStringable을 찾을 수 없습니다"와 관련 된 오류가 발생 합니다. 이것은 확실 하는 잘못 된**winmd**참조 되었습니다. IPC 서버 앱 (다음 섹션에서 자세히 설명) 신중 하 게의 사후 빌드 규칙 분리 이러한 두**winmd**별도 디렉터리에 있습니다.
...
앞의 두 섹션에서는 클래스 선언 및 참조**winmd**를 전송 설명테스트용으로 로드 하는 응용 프로그램 프로젝트에 있습니다. 데스크톱 구성 요소에서 남은 작업의 상당 부분이 구현과 관련됩니다. 데스크톱 구성 요소에서 중요한 점은 데스크톱 코드를 호출할 수 있는(일반적으로 기존 코드 자산을 재활용하기 위해) 기능이므로, 프로젝트를 특별한 방식으로 구성해야 합니다.
...
앱을 설치 하려면 복사 구현**winmd**연결된 된 테스트용으로 로드 된 응용 프로그램의 매니페스트에 지정 된 올바른 디렉터리를: <ActivatableClassAttribute>의 값 = "path". 관련된 지원 파일 및 프록시/스텁 dll도 복사합니다(이에 대한 자세한 내용은 아래에서 설명). 구현**winmd**복사 하지 않으면를 서버 디렉터리 위치 하면 테스트용으로 로드 된 모든 응용 프로그램의 호출을 RuntimeClass 새 항목에 "클래스가 등록 되지 않았습니다." 오류를 throw 합니다. 프록시/스텁을 설치하지 않거나 등록하지 않으면 모든 호출이 실패하고 값이 반환되지 않습니다. 이 두 번째 오류는 자주**없습니다**예외와 연결 합니다.
...
아래 단계에서는 서버 구성 요소**MyWinRTComponent**라고 가정 합니다.

**3단계:** 프로젝트에서 모든 CPP/H 파일을 삭제합니다.

**4 단계:** 이전 섹션인 "계약 정의"**winmdidl.exe**,**midl.exe**,**mdmerge.exe**및 등을 실행 하는 Post-build 명령이 포함 되어 있습니다. 이 Post-Build 명령의 midl 단계 출력 중 하나는 다음 네 가지 중요한 출력을 생성합니다.
...
**6 단계:** "MyWinRTProxy" 프로젝트에 파일 추가 **(gt_ 새 항목 추가 gt_ 코드 gt_ 모듈 정의 파일을 프로젝트**) 되도록 내용을 업데이트 하 고 있습니다.
...
-   반환*목록<T>* 여기서*T*개체는 비동기 작업 또는 속성 반입에서 이면 크로스 프로세스 활성도 높아집니다 반입 됩니다. 예를 들어, 돌아가면는*목록&lt;사람&gt;* 개체입니다. 각 반복 단계가 크로스 프로세스 호출이 됩니다. 각*사용자*반환 되는 개체는 프록시 및 메서드를 호출할 때마다 나타내는 또는 해당 개별 개체에 속성 하면 크로스 프로세스 호출이 발생 합니다. 따라서 "순수"*목록&lt;사용자&gt;* 개체를*수*큰 많은 저속 호출 하면 합니다. 배열의 구조체를 대량으로 전송하면 더 높은 성능을 보입니다. 예:
...
모든 성능 고려 사항의 경우처럼, 측정과 테스트가 중요합니다. 원칙적으로, 걸리는 시간을 결정하기 위해 다양한 작업에 원격 분석을 삽입해야 합니다. 범위 전체에서 측정을 고려해 야: 예를 들어는 실제로 걸리는 모든*사용자*를 사용 하도록개체 테스트용으로 로드 되는 응용 프로그램의 특정 쿼리가?
...
-   [NorthwindRT 조정된 WinRT 구성 요소 샘플](https://go.microsoft.com/fwlink/p/?LinkID=397349)

-   [안정적이고 신뢰할 수 있는 Microsoft Store 앱 제공](https://go.microsoft.com/fwlink/p/?LinkID=393644)
