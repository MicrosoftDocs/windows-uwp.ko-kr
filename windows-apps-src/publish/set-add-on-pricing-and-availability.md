...
(선택 하지 않은 경우 **가져오기 중지** 옵션 **표시 유형** 섹션에서) 프로그램 추가 기능에 대 한 기본 가격을 선택 해야 합니다. **자유**는 기본적으로 선택, 있으므로 추가 기능에 대 한 비용이 청구 하려는 경우 해야 (.99 USD부터 시작)는 사용할 수 있는 가격 계층 중 하나를 선택 합니다.

또한 가격 변경 예약을 통해 추가 기능의 가격이 변경되는 날짜와 시간을 표시할 수 있습니다. 뿐만 아니라 특정 시장에서 가격 변경을 사용자 지정할 수 있는 옵션도 있습니다. 

> [!TIP]
> 구독 추가 기능에 대 한 이후 전송에 더 높은 기본 가격을 선택 하 여 또는 예약 가격 증가 하는 가격 변경 하 여 추가 기능을 게시 한 후 가격을 발생 시킬 수 없습니다. 이러한 방법 중 하나를 사용 하 여 더 낮은 가격을 선택할 수 있지만 가격 내려갑니다 한 후에 해당 새 가격 보다 높은 발생 시킬 수 없습니다. 이 때문에 특히 중요 구독 추가 기능에 대 한 적절 한 가격 계층을 선택 하는 확실 하 게 하기는 합니다. 
