...
ms.locfileid: "4538324"
---
# <a name="managing-local-connected-storage"></a>연결 된 로컬 저장소 관리
연결 된 저장소는 게임 데이터를 클라우드에 저장할 데는 또한 연결 된 저장소 서비스는 로컬 저장소 구성 요소. PC 또는 콘솔에는 인지 클라우드로 동기화 된 데이터를 포함 하는 연결 된 저장소 데이터의 로컬 캐시. XDK 또는 UWP 제목 만들려는 인지 로컬 저장소 연결 된 데이터를 관리할 수 있도록 하는 도구입니다.
...
- *Xbstorage* 는 Xbox One 콘솔에서 로컬에 캐시 된 연결 된 저장소를 관리 하기 위한 XDK 명령 프롬프트에서 실행 되는 명령줄 도구입니다. 파일 경로에서 Xbox One XDK에서 *xbstorage* 도구를 찾을 수 있습니다: **/Program 파일 (x86)/Microsoft Durango XDK/bin/xbstorage.exe**
- *Gamesaveutil* PC에 연결 된 저장소를 캐시 UWP를 로컬로 관리 하기 위한 명령줄 도구입니다. *Gamesaveutil* 도구는 [Windows 10 SDK](https://developer.microsoft.com/en-US/windows/downloads/windows-10-sdk) 가 크리에이터 스 업데이트 이상으로 패키지 (빌드 10.0.16299.15 이상). 적절 한 버전의 Windows 10 SDK를 설치한 후 *gamesaveutil* 폴더에서 찾을 수 있습니다: **ProgramFiles(x86)/Windows 키트/10/bin / [SDK Version]/x64/gamesaveutil.exe**.
- *Xbox 장치 Portal(XDP)* Xbox One 콘솔에서 로컬에 캐시 된 저장소 UWP 연결 된 데이터를 관리할 수 있도록 하는 온라인 포털입니다. 이 문서에서는 XDP 사용을 다루지 않습니다. XDP를 사용 하 여 자세한 [Xbox 용 디바이스 포털](https://docs.microsoft.com/en-us/windows/uwp/debug-test-perf/device-portal-xbox)을 읽습니다.

## <a name="xbstorage"></a>Xbstorage

*Xbstorage* 하드 드라이브의 연결 된 저장소 데이터를 로컬에 캐시 된 데이터 지우기 뿐 아니라 가져오기 및 XML 파일을 사용 하 여 사용자 또는 컴퓨터에서 연결 된 저장소 공간에 대 한 데이터를 내보낼 수 있습니다.
...
마찬가지로, Xbox One 개발 키트에 연결 된 저장소 컨테이너에는 개발 PC에서 XML 파일에서 데이터의 복사본을 사용 하면 해당 데이터를 업로드 하려면 콘솔 합니다. 그러나 조건이 없는 동기화 되지 것입니다: 개발 키트, 잠금 얻을 수 없습니다 하거나 콘솔의 컨테이너와 클라우드에서 간의 데이터 충돌이 있는 경우 사용자가 하지 않기로 하 여 충돌을 해결 하는 경우 콘솔 동작 컨테이너를 유지 하 고 콘솔에의 한 버전을 선택 하면 제목 시작 될 때까지 사용자가 오프 라인 재생 하는 경우 작동 합니다.

이러한 명령에의 한 가지 예외 **** 초기화/는 **** 강제로/모든 SCIDs 및 사용자에 대 한 로컬 저장소의 저장 된 데이터를 지웁니다 되지만 클라우드에 저장 된 데이터를 변경 하지 않습니다. 사용자가 콘솔에 로밍 및 제목을 재생 시 클라우드에서 데이터를 다운로드 하는 경우에 것 상태로 콘솔을 배치 하는 데 유용 합니다.
...
공장 설정으로 복원 로컬 콘솔에서 연결 된 저장소의 모든 로컬 데이터를 지웁니다. 클라우드로 유지 되는 데이터 수정 되지 않으며 다운로드 다시 필요에 따라 합니다.

|옵션  |설명  |
|---------|---------|
|/force   |연결 된 저장소를 다시 확인 합니다. **** 강제로/지정 하지 않고 리셋 명령을 실행 하면 다음 메시지가 표시 될 수: 연결 된 저장소 공장 초기화는 파괴적 작업을 **** 강제로/플래그 아닌 경우이 명령은 초기화를 수행 하지 않는 존재 합니다. |
...
> [!NOTE]
> 지정 된 연결 된 저장소 공간에 가져오기 전에 시스템은 실행 중인 앱 연결 된 저장소 공간을 확보 때 실행 되는 동일한 논리를 사용 하 여 클라우드와 동기화 하려고 합니다.
>
> 동일한 기본 서비스 안내를 사용 하 여 응용 프로그램을 실행 하는 경우이 작업으로 인해 경합 상태 및 연결 된 저장소 공간의 내용을 확정 되지 않은 상태에서 수 있습니다.
>
> **** 대체/지정 하지 않으면 입력된 파일에 지정 된 컨테이너는 입력된 파일에 지정 된 blob을 작성 하기 전에 지워집니다. 입력된 파일에 지정 되지 않은 연결 된 저장소 공간에서 컨테이너는 그대로 유지 됩니다.

|옵션  |설명  |
|---------|---------|
|*파일 이름*     |가져오려는 데이터가 포함 된 XML 파일을 지정 합니다.         |
|/scid:*서비스 안내*    |서비스 구성 Id (서비스 안내)을 지정합니다.         |
|/machine        |-컴퓨터 연결 된 저장소 공간을 지정합니다.  이 옵션은 **/msa** 옵션을 사용 하 여 동시에 사용할 수 없습니다.         |
|/msa:*계정*  |사용자 당 연결 된 저장소를 사용 하 여 계정을 지정 합니다. 사용자가 콘솔에 사용할 공간 서명 해야 합니다.  이 옵션은 **/machine** 옵션을 사용 하 여 동시에 사용할 수 없습니다.         |
...
더미 데이터를 생성 하 고 지정 된 파일에 저장 \ < 이름 > 합니다. 파일의 XML 형식에 대 한 자세한 내용은이 항목의 뒷부분에 [가져오기 및 내보내기 파일 형식](#xbstorage_fileformat)을 참조 하세요.    서비스 구성 id (서비스 안내) 00000000-0000-0000-0000-000000000000을를 설정과 시스템 연결 된 저장소 공간에 대 한 계정 설정 됩니다. 이러한 값을 변경 하려는 경우 직접 편집할 수 있습니다 파일 생성 됩니다.
...
| / forceoutoflocalstorage | 왼쪽 공간이 필요 하는 연결 된 저장소 서비스를 시뮬레이트합니다. 연결 된 저장 장치에서 삭제 메모리 부족 보고 연결 된 저장소 서비스는 변경 되지 않습니다. |
...
| /X *: 주소* [*+ accesskey*]  | 호스트 이름 또는 대상된 콘솔의 주소 (콘솔에 **도구 IP** 로 표시 됨)를 지정 하지만 기본 콘솔 변경 되지 않습니다. 이 옵션을 사용 하지 않으면 기본 콘솔 사용 됩니다. *Accesskey* 은 선택 키를 알고 있는 사용자만 콘솔에 액세스를 제한 하는 데 사용할 수 있는 문자열입니다. **Xbconfig**명령을 사용 하 여 선택 키를 설정 **accesskey = * * * your 키*; 선택 키를 적용 하려면 콘솔을 다시 시작 합니다. 선택 키를 사용 하 여 구성 하는 콘솔에 액세스 하려면 콘솔의 IP 주소 또는 호스트 이름을 후 더하기 기호 (+)와 선택 키를 포함 해야 합니다.
> [!NOTE]
> Xbconnect 하 여 콘솔의 기본 설정 된 경우 선택 키 제공 하는 경우 선택 키 콘솔의 기본 주소의 일부로 저장 됩니다.
|

## <a name="gamesaveutil"></a>Gamesaveutil

*Gamesaveutil* 같은 모든 앱 *xbstorage* 해당 기능에 대 한 로컬에 캐시 된 연결 된 저장소를 관리할 수 있도록 제공 합니다. Xbstorage 도구와 마찬가지로 *gamesaveutil* 동작에 몇 가지 차이점이 기능 관리 같은 6 개까지 데이터를 제공 합니다. 가져오기, 내보내기, 삭제 하 고 초기화 명령에 로그인 하는 Xbox Live 사용자 필요 합니다. 확인 하 고 현재 사용자를 변경할 Windows 10의 Xbox 앱을 사용할 수 있습니다.
...
입력된 파일에 올바르게 채워지는 ContextDescription 섹션 경우 대상 응용 프로그램을 지정 하려면 사용할이 PFN 및 서비스 안내 합니다.
...
> 
> /Replace를 지정 하지 않으면 입력된 파일에 지정 된 하지 않는 한 기존 컨테이너 터치 하지 않습니다.
...
로 **가져오기**, **내보내기**및 *gamesaveutil* **생성** 에 대 한이 xml를 포맷 하는 데 필요한 변경 되는 \ < 계정 > 구성원 노드는 \ < ContextDescription > 노드는 \ < PackageFamilyName > 노드.
이렇게 하면 변경 됩니다는 \ < ContextDescription > 노드 여기에서:
...
> [!NOTE]
> XML 파일에 데이터 형식이 플랫폼에서 동일한, 가져오기에는 목적 으로만 내보냅니다. 중간 또는 유틸리티 형식, 보관 된 형식이 아니라으로 처리 해야 하므로이 XML 파일에 대 한 데이터 형식을 나중에 변경 될 수 있습니다.
...
Blob 및 컨테이너 게임 또는 파일 생성 되는 응용 프로그램에 의해 제공 해당 이름을 사용 해야 합니다.
각 blob 콘텐츠의 **CryptBinaryToStringW** **CRYPT_STRING_BASE64** 해당 blob raw 바이트 배열에 대 한 데이터를 제공 하는 플래그를 사용 하 여 호출 하 여 생성 된 **CDATA** 태그에 문자열 이어야 합니다. 반대로, 데이터 blob **CryptStringToBinary** 호출 이전의 암호화 된 문자열을 제공 하 여 다시 변환할 수 있습니다. 이 두 함수를 사용 하는 예제는 Visual Studio에 대 한 MSDN 포럼에서 [잘못 된 바이트를 반환 하는 CryptBinaryToString](http://social.msdn.microsoft.com/Forums/vstudio/en-US/9acac904-c226-4ae0-9b7f-261993b9fda2/cryptbinarytostring-returns-invalid-bytes?forum=vcgeneral) 에 표시 됩니다.
