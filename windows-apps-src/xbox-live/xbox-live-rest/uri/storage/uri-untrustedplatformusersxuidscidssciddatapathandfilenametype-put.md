...
파일을 업로드합니다. 데이터 및 메타 데이터를 전송 되는 단일 메시지 또는 일련의 작은 블록에서 데이터 및 메타 데이터는 전송 다중 블록 업로드를 전체 업로드에서 데이터를 업로드할 수 있습니다. 4 (메가바이트 단위) 보다 작은 파일만 단일 메시지로 보낼 수 있습니다. 다중 블록 업로드 형식 json 데이터에 대 한 지원 되지 않습니다. 이러한 Uri에 대 한 도메인은 `titlestorage.xboxlive.com`합니다.
...
| clientFileTime| DateTime| 마지막으로 날짜/시간 모든 클라이언트에 있는 파일의 파일을 업로드 합니다.| 
| displayName| 문자열| 사용자에 게 표시 되는 파일의 이름입니다.| 
| continuationToken| 문자열| 이전 업로드 요청의 응답에서 연속 토큰입니다. 첫 번째 블록의 경우이 지정할 수 없습니다. | 
| finalBlock| 부울| 파일의 마지막 블록에 대 한 true로 설정 합니다. 다른 모든 블록에 대해서는 false로 설정 합니다.| 
...
요청 본문에는 업로드 중인 파일의 내용을 포함 합니다. 단일 메시지 업로드에 대 한 본문 파일의 전체 콘텐츠입니다. 다중 블록 업로드를 위해 본문에는 쿼리 문자열 매개 변수에 지정 된 파일의 부분입니다. 
...
호출 다중 블록 요청을 성공적으로 수행 하는 경우 서비스는 다음 블록으로 전달할 continution 토큰을 반환 합니다.
