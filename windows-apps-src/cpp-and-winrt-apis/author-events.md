...
파일을 저장합니다. 프로젝트는 순간 완료 될 때까지 빌드 없습니다 수 있지만 **BankAccount** 런타임 클래스를 구현 하는 소스 코드 파일을 생성 하기 때문에 작업을 수행 하는 것은 이제 구축 합니다. 해 보시기 이제 빌드 (이 단계에서 표시 되는 수 빌드 오류가 발생 한와 관련이 `Class.h` 및 `Class.g.h` 찾을 수 없음). 빌드 과정의 `midl.exe` 구성 요소의 Windows 런타임 메타 데이터 파일을 만드는 도구를 실행 하는 (즉 `\BankAccountWRC\Debug\BankAccountWRC\BankAccountWRC.winmd`). 그런 다음 `cppwinrt.exe` 도구가 실행되어(`-component` 옵션과 함께) 구성 요소를 작성하도록 지원하는 소스 코드 파일을 생성합니다. 이러한 파일에는 IDL에서 선언 하는 **BankAccount** 런타임 클래스를 구현 시작할 수 있는 스텁이 포함 됩니다. 이 스텁이 `\BankAccountWRC\BankAccountWRC\Generated Files\sources\BankAccount.h`와 `BankAccount.cpp`입니다.
...
참조를 추가 하 고 이동 `\BankAccountWRC\Debug\BankAccountWRC\BankAccountWRC.winmd` (또는 두 프로젝트가 동일한 솔루션의 경우 프로젝트-프로젝트 참조를 추가). **추가**와 **확인**을 차례대로 클릭합니다. 이제 BankAccountCoreApp을 빌드합니다. 표시 오류가 발생 한 이벤트에는 페이로드 파일인 `readme.txt` 하지 존재, Windows 런타임 구성 요소 프로젝트에서 해당 파일을 제외, 후 BankAccountCoreApp 리 빌드합니다.
...
모든 매개 변수 또는 이벤트 인수를 전달 해야 하지 고유의 간단한 Windows 런타임 대리자 형식을 정의할 수 있습니다. 아래 예제에서는 간단한 **BankAccount** 런타임 클래스의 버전을 보여 줍니다. **SignalDelegate** 이라는 대리자 형식을 선언 하 고를 사용 하는 매개 변수를 사용 하 여 이벤트 대신 신호 유형 이벤트가 발생 합니다.
...
추가 하는 방법을 이벤트에 원하는 만큼의 구독 대리자를 확인 합니다. 그러나는 이벤트와 관련 된 오버 헤드가 발생 합니다. 필요한 모든 것은 단일 구독 대리자를 사용 하 여 간단한 콜백 있으면 [**winrt:: delegate를 사용할 수 있습니다&lt;... T&gt; **](/uwp/cpp-ref-for-winrt/delegate) 자체적으로 합니다.
...
이벤트 및 대리자 하지 함수 매개 변수로 전달 하는 것이 좋습니다. **추가** 기능 [**winrt::event**](/uwp/cpp-ref-for-winrt/event) 중 경우 대리자를 전달 해야 하기 때문에 한 가지 예외를입니다. 이 지침에 대 한 이유 대리자 여러 다른 Windows 런타임 언어 (측면에서 여부 하나의 클라이언트 등록 또는 여러 지원) 익숙합니다 걸리므로 때문입니다. 이벤트는 여러 구독자 모델을 사용 하 여 훨씬 더 예측할 수이 고 일관 된 옵션을 구성합니다.
