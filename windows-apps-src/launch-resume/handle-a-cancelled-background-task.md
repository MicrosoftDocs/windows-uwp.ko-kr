...
취소 요청이 수신되면 백그라운드 작업을 수행하는 메서드는 **\_cancelRequested**가 **true**로 설정되는 것을 인식하여 작업을 중지하고 종료해야 합니다. In-process 백그라운드 작업의 경우 **OnBackgroundActivated** 메서드에서 반환을 의미 합니다. Out of process 백그라운드 작업의 경우 **실행** 메서드에서 반환을 의미 합니다.
...
설명을 위해 샘플 코드는 [백그라운드 작업 샘플](http://go.microsoft.com/fwlink/p/?LinkId=618666)에서 **Run** 메서드 (및 타이머 콜백)의 일부만 보여줍니다.
