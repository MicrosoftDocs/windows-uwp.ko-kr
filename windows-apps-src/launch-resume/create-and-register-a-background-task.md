...
> [!NOTE]
> 백그라운드 작업 클래스 자체&mdash;백그라운드 태스크 프로젝트의 다른 모든 클래스 및&mdash;해야 **공용** 클래스를 **봉인** (또는 **최종**).

다음 샘플 코드는 백그라운드 태스크 클래스에 대 한 기본적인 시작점을 보여 줍니다.
...
4.  백그라운드 작업에서 비동기 코드를 실행할 경우 백그라운드 작업은 deferral을 사용해야 합니다. 지연은 사용 하지 않는 경우 백그라운드 작업 프로세스는 경우 예기치 않게 종료 수는 **실행** 비동기 작업이 완료 될 때까지 실행 되기 전에 메서드를 반환 합니다.

요청에서 지연 된 **실행** 메서드를 비동기 메서드를 호출 하기 전에 합니다. 비동기 메서드에서 액세스할 수 있도록 클래스 데이터 멤버에는 지연을 저장 합니다. 비동기 코드가 완료된 후에 deferral 완료를 선언합니다.

다음 샘플 코드 지연을 가져옵니다 저장 하며 비동기 코드가 완료 되 면이 해제 합니다.
...
> [!NOTE]
> 전용 백그라운드 작업을 등록 하는 함수를 만들 수도 있습니다&mdash;참조 [백그라운드 작업을 등록할](register-a-background-task.md)합니다. 이런 경우 다음 세 단계를 사용 하는 대신 간단히 트리거를 생성 하 작업 이름, 작업 항목 지점 및 (선택 사항) 조건을 함께 등록 함수를 제공 합니다.
...
## <a name="declare-in-the-app-manifest-that-your-app-uses-background-tasks"></a>앱이 백그라운드 작업에는 응용 프로그램 매니페스트에서 선언

백그라운드 작업을 실행하려면 먼저 앱 매니페스트에서 각 백그라운드 작업을 선언해야 합니다. 앱을 매니페스트에 나열 되지 않은 트리거를 사용 하 여 백그라운드 작업을 등록 하려고 하는 경우 백그라운드 작업의 등록 "런타임 클래스가 등록 되지 않았습니다" 오류와 함께 실패 합니다.

1.  Package.appxmanifest라는 파일을 열어 패키지 매니페스트 디자이너를 엽니다.
2.  **선언** 탭을 엽니다.
3.  **사용 가능한 선언** 드롭다운에서 **백그라운드 작업**을 선택하고 **추가**를 클릭합니다.
4.  **시스템 이벤트** 확인란을 선택합니다.
5.  에 **진입점:** 텍스트 상자이 예제는 Tasks.ExampleBackgroundTask 되는 배경 클래스의 이름과 네임 스페이스를 입력 합니다.
6.  매니페스트 디자이너를 닫습니다.
