---
title: "시작 자습서: MonoGame 2D로 작성된 UWP 게임"
description: "C# 및 MonoGame으로 작성된 간단한 Windows 스토어용 UWP 게임"
author: mukin
ms.author: mukin
ms.date: 03/06/2017
ms.topic: article
...
translationtype: HT
---
# <a name="get-started-tutorial-a-uwp-game-in-monogame-2d"></a>시작 자습서: MonoGame 2D로 작성된 UWP 게임

## <a name="a-simple-2d-uwp-game-for-the-windows-store-written-in-c-and-monogame"></a>C# 및 MonoGame으로 작성된 간단한 Windows 스토어용 2D UWP 게임
...
+    Windows 10 및 Microsoft Visual Studio 2015. [Visual Studio를 사용하여 설정하는 방법을 알아보려면 여기를 클릭하세요](https://docs.microsoft.com/en-us/windows/uwp/get-started/get-set-up).
...
Visual Studio 2015에서 프로젝트를 열고 **F5** 키를 눌러 샘플을 실행합니다. 처음 실행하는 경우 Visual Studio가 현재 설치된 파일에서 누락된 NuGet 패키지를 가져와야 하므로 다소 시간이 걸릴 수 있습니다.
...
플레이어 아바타로는 믿음직한 티라노사우루스를 타고 있는 Microsoft 고유의 닌자 고양이를 사용하겠습니다. [이미지를 다운로드하려면 여기를 클릭하세요](https://github.com/muhsinking/VeggieJump/blob/master/MonoGame2D/Content/ninja-cat-dino.png).
...
7. 키보드 입력 처리 다음으로 키보드를 통해 사용자 입력을 처리하는 새 메서드가 필요합니다. 다음 메서드를 **Game1.cs**에 추가합니다.
...
### <a name="9-draw-spriteclass-objects"></a>9. SpriteClass 그리기 개체
...
## <a name="publish-to-the-windows-store"></a>Windows 스토어에 게시
이 게임은 UWP 앱입니다. 따라서 이 프로젝트를 Windows 스토어에 게시할 수 있습니다. 몇 가지 단계를 처리해야 합니다.
...
자세한 내용은 [Windows 스토어 앱 게시](https://developer.microsoft.com/en-us/store/publish-apps)를 참조하세요.
