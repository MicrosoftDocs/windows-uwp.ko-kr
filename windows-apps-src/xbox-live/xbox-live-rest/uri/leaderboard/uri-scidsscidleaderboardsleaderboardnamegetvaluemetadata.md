...
? 포함 valuemetadata = 쿼리 매개 변수는 순위표 값과 연결 된 모든 메타 데이터를 포함 하도록 응답을 허용 합니다. 지정 된 클라이언트를 포함 하는 값 메타 데이터 모델 및 경주 트랙의 가장 좋은 시간을 달성 하는 데 사용 하는 자동차의 색 등의 값을 사용 하 여 연결 된 데이터입니다.
 
메타 데이터 값 자체 순위표에 없는 겠죠의 기반이 되는 사용자 상태에서 정의 됩니다.
...
| 매개 변수| 형식| 설명| 
| --- | --- | --- | --- | --- | --- | 
| include=valuemetadata| 문자열| 응답 순위표 값과 연결 된 모든 값 메타 데이터를 포함 함을 나타냅니다.| 
...
| X-Xbl-Contract-Version| 문자열입니다. 사용 하는 API의 버전을 나타냅니다. 이 값은 응답에서 메타 데이터 값을 포함 하기 위해 "3"에 설정 되어야 합니다.| 
...
| totalItems| 64 비트 부호 없는 정수| 필수. 순위 표에서 항목의 총 수입니다. 예를 들어 값: 1234567890| 
...
| columnDefinition| JSON 개체| 필수. 순위 표에서 열을 설명 합니다.| 
| columnDefinition.displayName| 문자열| 필수. 순위 표에서 열의 설명이 포함 된 이름입니다.| 
| columnDefinition.statName| 문자열| 필수. 이름 순위표를 기반으로 하는 사용자 상태입니다.| 
| columnDefinition.type| 문자열| 필수. 데이터 형식 열에서 사용자 상태입니다.| 
 
userList | userList| 섹션| 필수. 항상 반환 됩니다. 요청한 순위표에 대 한 실제 사용자 점수를 포함 합니다.| 
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | 
| 게이머 태그| 문자열| 필수. 순위표 항목에서 사용자의 게이머 태그입니다.| 
| xuid| 64 비트 부호 없는 정수| 필수. 순위표 항목에서 사용자의 Xbox 사용자 ID입니다.| 
| 백분위 수| 문자열| 필수. 사용자의 백분위 수 순위 표에서 합니다.| 
| 순위| 문자열| 필수. 사용자의 순위 표에서 순위가 숫자입니다.| 
| value| 문자열| 필수. 사용자의 값을 순위표 기반이 되는 상태입니다.| 
| valueMetadata| 문자열| 필수. 형식에서 문자열 쌍을 구분 하는 문자열로 쉼표로 "\"이름을\" : \"값\",..."

이 값은 빈 문자열 메타 데이터가 없는 경우. | 
...
값 메타 데이터를 반환 하기 위해도 다음 헤더를 지정 합니다.
