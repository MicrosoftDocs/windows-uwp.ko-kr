...
description: "Windows 스토어 분석 API에서 이 메서드를 사용하여 지정된 날짜 범위 및 다른 선택적 필터에 대한 Windows 10 드라이버의 집계 오류 보고 데이터를 가져옵니다. 이 메서드는 IHV용으로만 제공됩니다."
...
Windows 스토어 분석 API에서 이 메서드를 사용하여 지정된 날짜 범위 및 다른 선택적 필터에 대한 Windows 10 드라이버 오류의 집계 보고 데이터를 가져옵니다. [Windows 10 드라이버 오류에 대한 세부 정보 가져오기](get-details-for-a-windows-10-driver-error.md) 메서드를 사용하여 추가 오류 정보를 검색할 수 있습니다.
