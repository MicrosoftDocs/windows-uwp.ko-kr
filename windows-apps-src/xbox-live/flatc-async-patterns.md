...
일반적으로 게임에 거의 스레드를 통해 실행 하는 **비동기 작업** 을 제어 하 고 스레드 **완료 콜백에서**사용 하는 경우 결과 반환 합니다.  일부 게임은 힙의 섹션에는 스레드 동기화 하는 단일 스레드 접촉만 되도록 설계 되었습니다. **완료 콜백에서** 스레드에서 호출 되지는 **비동기 작업** 의 결과 사용 하 여 공유 상태를 업데이트 하 고 게임 컨트롤 스레드 동기화를 해야 합니다.
...
* **AsyncQueue** **비동기 작업** 을 실행 스레드를 결정 해야 하 고 스레드 AsyncBlock의 **모든 완료 콜백이**호출 수 있습니다.
...
* *콜백* -비동기 작업이 완료 된 후 호출 되는 선택적 콜백 함수입니다.  콜백을 지정 하지 않으면, **AsyncBlock** **GetAsyncStatus** 완료 한 다음 결과 얻습니다를 기다릴 수 있습니다.
* *상황에 맞* 는-를 사용 하면 데이터는 콜백 함수를 전달할 수 있습니다.
...
**비동기 작업이** 완료 되었음을 알 수 다른 방식:

* AsyncBlock의 **모든 완료 콜백이** 호출 되어
* 완료 될 때까지 기다리는 true로 **GetAsyncStatus** 를 호출 합니다.
* **AsyncBlock** 에 waitEvent를 설정 하 고 신호 이벤트 대기

**하지만 GetAsyncStatus** 및 waitEvent AsyncBlock의 **완료 콜백에서** AsyncBlock의 **완료 콜백에서** 선택 사항를 실행 한 후 **비동기 작업이** 완료 간주 됩니다.

**비동기 작업이** 완료 되 면 결과 얻을 수 있습니다.

### <a name="getting-the-result-of-the-asynchronous-task"></a>**비동기 작업** 의 결과 가져오는 데 필요한

결과 얻기 위해 대부분의 **비동기 API** 함수는 Function\의 해당 \[Name] 결과 함수는 비동기 호출의 결과 받을 수 있습니다. 이 예제 코드 **XblProfileGetUserProfileAsync** 해당 **XblProfileGetUserProfileResult** 함수를 포함 합니다. 이 함수는 함수의 결과 반환 하 고 그에 따라 역할을 사용할 수 있습니다.  자세한 내용은 각 **비동기 API** 함수의 documention 검색 결과에서 참조 하세요.
...
* *수동* -수동 큐는 자동으로 발송 하지 않습니다.  개발자가 원하는 모든 스레드에서 디스패치합니다 수입니다. 특정 스레드에 비동기 호출에 있는 작업 또는 콜백 할당을 사용할 수 있습니다.  이 아래에 자세히 설명 합니다.
...
* *고정 스레드* -QueueUserAPC를 사용 하 여 비동기 큐를 만든 스레드에서 디스패치. 사용자 모드 APC 대기 중인 경우 스레드 하지 APC 함수를 호출 하 여 경고 상태가 아니면로 리디렉션됩니다. 경고 대기 작업을 수행 하도록 **SleepEx**, **SignalObjectAndWait**, **WaitForSingleObjectEx**, **WaitForMultipleObjectsEx**또는 **MsgWaitForMultipleObjectsEx** 를 사용 하 여 상태가 경고 스레드
