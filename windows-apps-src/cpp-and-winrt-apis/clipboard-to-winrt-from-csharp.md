...
프로젝트에 새 **XAML** > **빈 페이지(C++/WinRT)** 항목을 추가합니다(**빈 페이지** 가 아니라 **빈 페이지(C++/WinRT)** 항목 템플릿이어야 함). 이 EventHandler의 이름을 `CopyText`로 지정합니다. 새 XAML 페이지는 우리가 원하는 **SDKTemplate** 네임스페이스 내에서 정의됩니다.
