...
Arm 기반 Windows 10 Microsoft Store에서 모든 x86, ARM32, 및 ARM64 [UWP 앱](../get-started/universal-application-platform-guide.md) 을 실행합니다. 기본적으로 어떠한 에뮬레이션 없이 실행 되는 ARM32 및 ARM64 앱 동안 x86 에뮬레이션 앱 실행 합니다. UWP 개발자인 경우 이 디바이스에 대한 최적의 사용자 환경을 제공하므로 앱에 대한 ARM 패키지를 제출할 수 있는지 확인하십시오. 자세한 내용은 [앱 패키지 아키텍처](../packaging/device-architecture.md)를 참조하세요.
...
UWP 앱 외에도 arm 기반 Windows 10 실행할 수도 x86 Win32 모든 PC 마찬가지로 좋은 성능 및 원활한 사용자 경험을 수정 하지 않고 앱 (예: Adobe Reader). 이러한 x86 Win32 앱 ARM에 대해 다시 컴파일할 필요가 없으며를 인식 하지은 ARM 프로세서에서 실행 됩니다. 64비트 x64 Win32 앱은 지원되지 않지만 대부분의 앱은 모두 x86 버전의 앱을 가지고 있으므로 사용자의 관점에서는 ARM 기반 Windows PC에서 실행할 32비트 x86 설치 관리자만 선택하면 됩니다.
...
|[Arm ARM 앱 문제 해결](apps-on-arm-troubleshooting-arm32.md)|일반적인 문제를 해결 하는 방법과 ARM에서 실행 되는 경우 ARM32 및 ARM64 앱. |
