...
- **리뷰 내용**: 기본 설정은 **사용 하 여 평가**검토 콘텐츠로 유일한 등급 표시 되지 즉입니다. **모든** 모든 평가 표시를 모든 리뷰 텍스트를 포함 하지 않는 비롯 선택할 수 있습니다. 참고 **평점 분석** 차트 선택에 관계 없이 모든 리뷰를 항상 표시 됩니다.
...
-   리뷰 날짜
-   Microsoft Store에 그대로 리뷰 작성자의 이름을 표시 됩니다.
-   리뷰 작성자의 국가/지역
...
위에서 언급 했 듯이 리뷰를 검색, 고객이 남긴 원래 텍스트만 검색 됩니다 (시점과 텍스트는 번역), **리뷰 번역** 상자가 선택 되어 있는 경우에 합니다.
