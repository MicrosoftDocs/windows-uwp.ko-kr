...
앱 제한 된 접근 권한 값을 선언 하는 경우 Microsoft Store에 앱을 게시 하려면 승인 하려면 [앱 제출 프로세스](../publish/app-submissions.md) 중 정보를 제공 해야 합니다. 앱에서 선언 하는 각 제한 된 접근 권한 값을 사용 하는 방법을 설명 하 제출의 [제출 옵션](../publish/manage-submission-options.md#restricted-capabilities) 페이지에서이 정보를 제공 합니다.

> [!IMPORTANT]
> 제한 된 접근 권한 값은 매우 구체적인 시나리오를 위한 합니다. 이 접근 권한 값은 사용이 엄격히 제한되며 추가 Microsoft Store 등록 정책 및 검토가 적용됩니다. 참고 모든 승인이 필요 없이 제한 된 접근 권한 값을 선언 하는 앱을 테스트용으로 로드를 수 있습니다. 승인은 이러한 앱을 Microsoft Store에 제출하는 경우에만 필요합니다.
...
제한 된 접근 권한 값을 선언 하려면 [앱 패키지 매니페스트](https://msdn.microsoft.com/library/windows/apps/BR211474) 소스 파일을 수정 (`Package.appxmanifest`). **Xmlns: rescap** XML 네임 스페이스 선언을 추가 하 고 제한 된 기능을 선언 하면 **rescap** 접두사를 사용 합니다. 예를 들어 여기에 **appCaptureSettings** 접근 권한 값을 선언하는 방법이 나와 있습니다.
...
>
> 이 방법은 승인을 요청 하 고 사용할 수도 있습니다 (이러한 정보를 제공 제출 하는 동안), 대신 여부를 사용 하는 개발 샌드박스를 시작 하기 전에 제한 된 접근 권한 값을 사용 하 여 승인 된 확인 하려는 경우에 제출 합니다.
...
| **완전 신뢰 권한 수준이** | **RunFullTrust** 제한 된 접근 권한 값을 완전 신뢰 권한 수준에서 사용자의 컴퓨터에서 실행할 수 있습니다. 이 접근 권한이 값은 [FullTrustProcessLauncher](https://docs.microsoft.com/uwp/api/windows.applicationmodel.fulltrustprocesslauncher) API 사용 해야 합니다.<br /><br />이 접근 권한이 값은 msix 또는 appx 패키지로 제공 되는 모든 데스크톱 응용 프로그램에 대 한 필수도 (와 마찬가지로 [데스크톱 브리지](https://developer.microsoft.com/windows/bridges/desktop)), Desktop App Converter (DAC)를 사용 하 여 이러한 앱을 패키징할 때 자동으로 매니페스트의 표시 됩니다 또는 Visual Studio 합니다. |
| **권한 상승** | **AllowElevation** 제한 된 접근 권한 값은 기존 데스크톱에 필요한 기능을 시작 하거나 앱의 수명 동안 자동 승격을 유지 하기 위해 Microsoft 파트너와 엔터프라이즈에서 생성 된 앱 수 있습니다.<br/><br/>Microsoft Store에 제출된 앱에서 이 접근 권한 값을 선언하지 않는 것이 좋습니다. 대부분 개발자의 경우 이 접근 권한 값 사용은 승인되지 않습니다. 비즈니스용 Microsoft 스토어를 통해 자신의 개인 저장소에 엔터프라이즈 배포 기간 업무 앱에 대 한만 승인 됩니다.  |
...
| **Windows 팀 응용 프로그램 보기** | **TeamEditionView** 제한 된 접근 권한 값을 Windows 10 버전 1703 이상을 실행 하는 Surface Hub 장치에서 응용 프로그램 보기 호스팅에 대 한 Api에 액세스할 수 있습니다.<br/><br/>Microsoft Store에 제출된 앱에서 이 접근 권한 값을 선언하지 않는 것이 좋습니다. 대부분 개발자의 경우 이 접근 권한 값 사용은 승인되지 않습니다. |

## <a name="custom-capabilities"></a>사용자 지정 기능

위의 [제한 된 접근 권한 값](#restricted-capabilities) 섹션에서는 사용자 지정 기능을 사용 하 여 승인을 요청 하는 데 사용할 수 있는 동일한 접근 권한 값 승인 프로세스를 설명 합니다. [포함 된 SIM](/uwp/api/windows.networking.networkoperators.esim) Api 사용자 지정 기능을 필요로 하는 Api의 예입니다. 개발자 모드에서 응용 프로그램을 로컬로 실행 하려는 경우 사용자 지정 기능이 필요 하지 않습니다. 하지만 외부 개발자 모드에서 실행 하거나 Microsoft Store에 앱을 게시 하도록 해야 합니다.
