...
ms.locfileid: "3664753"
---
# <a name="navigation-view-preview-version"></a>탐색 보기 (미리 보기 버전)

> **미리 보기 버전**:이 문서에서는 개발 중인 NavigationView 컨트롤의 새 버전에 설명 합니다. 사용 하 여 이제 해야 [최신 Windows 참가자 빌드 및 SDK](https://insider.windows.com/for-developers/) 나 [Windows UI 라이브러리](https://docs.microsoft.com/uwp/toolkits/winui/)합니다.
...
다른 탐색 컨트롤을 [탐색 디자인 기초](../basics/navigation-basics.md)를 참조 하세요.
...
:::row:::
    ...
    ::: 열 범위 = "2"::: **XAML 컨트롤 갤러리**<br>
        XAML 컨트롤 갤러리 앱이 설치 된 경우 클릭 <a href="xamlcontrolsgallery:/item/NavigationView">여기</a> 에 앱을 열고 중인 NavigationView를 참조 하세요.
...
- 동일 하 게 중요 한 최상위 수준 탐색 범주의 중간에서 높은 쪽 (5-10) 있습니다.
...
- 앱 콘텐츠에 대 한 더 많은 공간을 알리기 위해 합니다.
...
중간 화면 및 큰 화면 왼쪽에 작은 화면 LeftMinimal LeftCompact 사이 맞춰 조정 합니다. 자세한 내용은 [적응형 동작](#adaptive-behavior) 섹션을 참조 하세요.
...
창을 통해에 위쪽 또는 왼쪽 배치 수는 `PanePosition` 속성입니다.
...
뒤로 단추는 창의 왼쪽 위 모서리에 나타나지만 NavigationView 콘텐츠 백 스택을 자동으로 추가 하지 않습니다. 뒤로 탐색을 사용 하려면 참조는 [뒤로 탐색](#backwards-navigation) 섹션입니다.

NavigationView 창 포함 될 수도 있습니다.

1. 특정 페이지를 탐색 하는 것에 대 한 [NavigationViewItem](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationviewitem)형태로 탐색 항목
2. 구분 기호 [NavigationViewItemSeparator](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationviewitemseparator)숫자 형태로 탐색 항목 그룹화를 위한 합니다. 공간으로 구분 기호를 렌더링 하는 0으로 [Opacity](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationviewitemseparator.opacity) 속성을 설정 합니다.
3. 항목 그룹에 레이블을 지정 [NavigationViewItemHeader](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationviewitemheader)형식의 헤더.
4. 선택 사항 [AutoSuggestBox](auto-suggest-box.md) 를 앱 수준 검색을 허용 합니다.
...
:::row:::
    :::column:::
    <b>왼쪽된 탐색</b><br>
    ![창 바닥글의 왼쪽된 탐색](images/navview-freeform-footer-left.png)<br>
    :::column-end:::
    :::column:::
     <b>상단 탐색</b><br>
...
헤더는 창 [PaneHeader](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.PaneHeader) 속성에 추가 하는 경우의 자유 형식 콘텐츠
...
[PaneCustomContent](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.PaneCustomContent) 속성에 추가 하는 경우 창에서 자유 형식 콘텐츠
...
하드웨어 및 소프트웨어 요구 사항이 충족 되 면 NavigationView는 자동으로 해당 창에 [아크릴 재질](../style/acrylic.md) 및 [강조](../style/reveal.md) 왼쪽된 창에만 사용 됩니다.
...
NavigationView의 기본 적응형 동작 작은 창 너비에 큰 창 너비에는 확장 된 왼쪽된 창, 중간 창 너비에 아이콘 전용 왼쪽된 탐색 창 및 햄버거 메뉴 단추를 표시 됩니다. 적응형 동작에 대 한 창 크기에 대 한 자세한 내용은 [화면 크기 및 중단점을](../layout/screen-sizes-and-breakpoints-for-responsive-design.md)참조 하세요.
...
두 번째 일반적인 적응 패턴 큰 창 너비와 중간 크기 및 작은 창 너비 모두에 햄버거 메뉴에서 확장 된 왼쪽된 창을 사용 하는 것입니다.
...
- 작은 창 너비의 콘텐츠를 앱에 대 한 더 많은 공간을 알리기 위해 합니다.
- 아이콘이 있는 탐색 범주를 명확 하 게 나타낼 수 없습니다.

### <a name="compact"></a>컴팩트

세 번째 일반적인 적응 패턴 큰 창 너비와 중간 크기 및 작은 창 너비 모두에 아이콘 전용 왼쪽된 탐색 창에서 확장 된 왼쪽된 창을 사용 하는 것입니다. 이 좋은 예에는 메일 앱입니다.
...
- 것 항상 화면의 모든 탐색 옵션을 표시 하는 것이 중요 합니다.
- 아이콘이 있는 탐색 범주를 명확 하 게 나타낼 수 있습니다.

### <a name="no-adaptive-behavior"></a>적응형 동작 없음

경우에 따라 수 하지 알리기 위해 적응 동작을 가져오지 전혀 합니다. 항상 되도록 확장, 항상 컴팩트 또는 항상 최소 창을 설정할 수 있습니다.
...
큰 창 크기와 중소기업의 왼쪽된 탐색에서 상단 탐색을 사용 하는 것이 좋습니다 창 크기는 경우:

- 이 설정에서 하나의 범주는 화면에 맞지 않는, 바람직한을 제공 하려면 왼쪽된 탐색을 축소 되도록 함께 표시할 중요 한 최상위 수준 탐색 범주의 동일 하 게 설정을 해야 합니다.
- 작은 창 크기에 최대한 많은 콘텐츠 공간 유지 하고자 합니다.
...
탭 모델 선택 및 포커스 연결 됩니다. 에 전환이 포커스 선택을 이동은 일반적으로 작업 합니다. 에 아래 예에서는 오른쪽 화살표 지정 이동 선택 표시기 디스플레이에서 돋보기를 합니다. 사용으로 [SelectionFollowsFocus](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.selectionfollowsfocus) 속성을 설정 하 여이 얻을 수 있습니다.
...
- 사용으로 [SelectionFollowsFocus](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.PanePostion) 속성 설정
- 탐색 스택에 추가 하지 않는 프레임 탐색을 사용 합니다.
- 게임 패드 왼쪽/오른쪽 범퍼 앱의 최상위 수준 탐색 범주를 탐색 하는 경우를 나타내는 데 사용 되는 [ShoulderNavigationEnabled](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.PanePostion) 속성에서 기본값을 유지 합니다. 기본값은 "WhenSelectionFollowsFocus". 다른 가능한 값은 "항상" 및 "없습니다".
...
원활한 모양을 + 느낌, ScrollViewer를 사용 하 여 앱 페이지가 이며 탐색 창 위쪽의 위치를 지정 하는 경우 권장 상단 탐색 창 아래 콘텐츠 스크롤 필요 합니다.
...
앱에 콘텐츠를 매우 긴 스크롤 하는 경우 하려는 경우 상단 탐색 창에 연결 하 고 원활 하 게 표면 형성 하는 고정 헤더를 통합 하는 것이 좋습니다. 

![navview 스크롤 고정 헤더](images/nav-scroll-stickyheader.png)

NavigationView의 [ContentOverlay](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.ContentOverlay) 속성을 설정 하 여이 얻을 수 있습니다. 

경우에 따라 사용자가 아래로 스크롤 하는 경우 NavigationView에서 [IsPaneVisible](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.ContentOverlay) 속성을 false로 설정 하 여 탐색 창을 숨기려면는 것이 좋습니다.
