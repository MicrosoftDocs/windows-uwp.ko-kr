...
>**참고**이 기능은 용 으로만.NET 사용 합니다. 클라이언트 앱과 데스크톱 구성 요소는 둘 다 .NET을 사용해서 작성해야 합니다.
...
>**참고**열거형 데스크톱 구성 요소와 지금 테스트용으로 로드 된 응용 프로그램 간의 Windows 런타임 구성 요소 계약에서는 지원 되지 않습니다.
...
참조**winmd**한 번만들어집니다 (프로젝트의 대상 폴더 아래 "참조" 폴더)는 것이 손 (복사) 각 사용 하는 테스트용으로 로드 하는 응용 프로그램 프로젝트를 수행 하 고 참조 합니다. 이 내용은 다음 섹션에서 자세히 설명하겠습니다. 위의 빌드 규칙에 포함 된 프로젝트 구조 되도록 구현 및 참조**winmd**혼동을 피하기 위해 빌드 계층 구조에서 명확 하 게 분리 된 디렉터리에 있습니다.
...
이전 두 섹션에서는 클래스 선언 및 참조**winmd**전송테스트용으로 로드 하는 응용 프로그램 프로젝트에 있습니다. 데스크톱 구성 요소에서 남은 작업의 상당 부분이 구현과 관련됩니다. 데스크톱 구성 요소에서 중요한 점은 데스크톱 코드를 호출할 수 있는(일반적으로 기존 코드 자산을 재활용하기 위해) 기능이므로, 프로젝트를 특별한 방식으로 구성해야 합니다.
...
**6 단계:** "MyWinRTProxy" 프로젝트에 def 파일을 추가 **(프로젝트 > 새 항목 추가 > 코드 > 모듈 정의 파일**) 되도록 내용을 업데이트 하 고 있습니다.
...
-   반환*목록<T>* 여기서*T*개체는 비동기 작업 또는 속성 반입에서 이면 크로스 프로세스 활성도 높아집니다 반입 됩니다. 예를 들어, 돌아가면는*목록&lt;사람&gt;* 개체입니다. 각 반복 단계가 크로스 프로세스 호출이 됩니다. 각*사용자*반환 되는 개체는 프록시 및 메서드를 호출할 때마다 나타내는 또는 해당 개별 개체에 속성 하면 크로스 프로세스 호출이 발생 합니다. 따라서 "순수"*목록&lt;사람&gt;* 개체 위치*개수*큰 많은 느린 호출 하면 합니다. 배열의 구조체를 대량으로 전송하면 더 높은 성능을 보입니다. 예:
...
그런 다음 반환*PersonStruct\ [\]* 대신*목록&lt;PersonObject&gt;*.
그러면 한 번의 크로스 프로세스 "홉"에서 모든 데이터를 전달합니다.

모든 성능 고려 사항의 경우처럼, 측정과 테스트가 중요합니다. 원칙적으로, 걸리는 시간을 결정하기 위해 다양한 작업에 원격 분석을 삽입해야 합니다. 범위 전체에서 측정 하려면 반드시: 예를 들어는 실제로 걸리는 모든*사용자*를 사용 하도록테스트용으로 로드 되는 응용 프로그램의 특정 쿼리가 개체?
