---
title: XDK 게임용 Visual Studio를 사용 하 여 시작
description: Visual Studio를 사용 하 여 Xbox 개발자 키트 (XDK)를 개발 하는 방법은 링크 게임을 제공 합니다.
ms.assetid: 3430cf97-0136-4c69-bcec-9cc3edf5fc95
ms.date: 04/04/2017
ms.topic: article
keywords: xbox live, xbox, 게임, uwp, windows 10, 하나는 xbox xdk
ms.localizationpriority: medium
ms.openlocfilehash: c197115bf7c71c8e794c0009d5f3435ea77f592a
ms.sourcegitcommit: d7613c791107f74b6a3dc12a372d9de916c0454b
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 12/06/2018
ms.locfileid: "8735050"
---
# <a name="get-started-with-using-visual-studio-for-xdk-based-games"></a>XDK 기반 게임용 Visual Studio를 사용 하 여 시작

이 섹션에서는 기반 XDK 프로젝트를 사용 하 여 Xbox live 시작

## <a name="in-this-section"></a>이 섹션의 내용

[Xdk 다운로드 위치](where-to-get-xdk.md)

[개발 콘솔 구성](configure-your-development-console.md)

[새로운 또는 기존 XDK 프로젝트에 Xbox Live 추가](add-xbox-live-to-an-xdk-project.md)

[Xbox 개발 키트 (XDK)를 사용 하 여 제공 되는 Xbox Live Api를 사용 합니다.](using-xbox-live-apis-built-into-the-xdk.md)

[Xbox Live API NuGet 패키지를 사용 하 여 XDK 타이틀 개발](use-xbox-live-nuget-with-xdk.md)

[XDK는 함께 제공 되는 Xbox Live 소스 컴파일](compile-the-xdk-xbox-live-api-source.md)
