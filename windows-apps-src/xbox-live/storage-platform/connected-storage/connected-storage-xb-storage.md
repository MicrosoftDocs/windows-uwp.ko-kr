...
ms.locfileid: "5162461"
---
# <a name="managing-local-connected-storage"></a>연결 된 로컬 저장소 관리
연결 된 저장 게임 데이터가 클라우드에 저장을 사용 하는 동안은 연결 된 저장소 서비스는 로컬 저장소 구성 요소. PC 또는 콘솔에는 인지 클라우드로 동기화 된 데이터를 포함 하는 연결 된 저장소 데이터의 로컬 캐시. XDK 또는 UWP 제목은 만들려는 인지 로컬 저장소 연결 된 데이터를 관리할 수 있도록 하는 도구입니다.
...
- *Xbstorage* 는 Xbox One 콘솔에서 로컬에 캐시 된 연결 된 저장소를 관리 하기 위한 XDK 명령 프롬프트에서 실행, 명령줄 도구입니다. 파일 경로에서 Xbox One XDK에서 *xbstorage* 도구를 찾을 수 있습니다: **/Program 파일 (x86)/Microsoft Durango XDK/bin/xbstorage.exe**
- *Gamesaveutil* PC에 연결 된 저장소를 캐시 UWP를 로컬로 관리에 대 한 명령줄 도구입니다. *Gamesaveutil* 도구는 [Windows 10 SDK](https://developer.microsoft.com/en-US/windows/downloads/windows-10-sdk) Fall Creators Update 이상으로 패키지 (빌드 10.0.16299.15 이상). 적절 한 버전의 Windows 10 SDK를 설치한 후 *gamesaveutil* 폴더에서 찾을 수 있습니다: **ProgramFiles(x86)/Windows 키트/10/bin / [SDK Version]/x64/gamesaveutil.exe**.
...
*Xbstorage* 로컬에 캐시 된 데이터 하드 드라이브에서 연결 된 저장소 데이터 지우기 뿐 아니라 가져오기 및 XML 파일을 사용 하 여 사용자 또는 컴퓨터에서 연결 된 저장소 공간에 대 한 데이터의 내보내기 수 있습니다.

시스템은 로컬 파일에 연결 된 저장소 공간에서 데이터 읽기 작업 동기화를 하면 하므로 앱 자체에서 해당 작업이 수행 된 것 이었습니다 처럼 작동 *xbstorage* 도구를 사용 하 여 로컬 데이터에 대 한 작업을 수행 하는 경우 복사 하기 전에 클라우드 합니다.

마찬가지로, Xbox One 개발 키트에서 연결 된 저장소 컨테이너에는 개발 PC에 있는 XML 파일에서 데이터의 복사본을 사용 하면 클라우드 해당 데이터를 업로드 하려면 콘솔 합니다. 그러나 조건이 없는 동기화 되지 것입니다: 개발 키트, 잠금 얻을 수 없습니다 아니면 사용자가 하지 않기로 하 여 충돌을 해결 하는 경우에 따라 콘솔 동작 콘솔의 컨테이너와 클라우드에서 간의 데이터 충돌이 발생 하는 경우 선택 하 고 컨테이너를 유지 하 고 콘솔의 한 버전 사용자가 플레이 오프 라인 제목 시작 될 때까지 마치 작동 합니다.

이러한 명령에의 한 가지 예외 **** 초기화/는 **** 강제로/모든 SCIDs 및 사용자에 대 한 저장 된 데이터의 로컬 저장소를 지웁니다 되지만 클라우드에 저장 된 데이터를 변경 하지 않습니다. 콘솔 사용자 콘솔에 로밍 되었으며 제목을 재생 시 클라우드에서 데이터를 다운로드 하는 경우에 것 상태에 배치 하는 데 유용 합니다.

### <a name="xbstorage-commands"></a>Xbstorage 명령

Xbstorage에 다음 6 개의 명령을 개발자가 Xbox One 개발 키트의 로컬 데이터 관리 XDK 명령 프롬프트를 사용 하 여 사용할 수 있습니다.

<a id="xbstorage_reset"></a>

|명령  |설명  |
|---------|---------|
|다시 설정    |연결 된 저장소에서 초기화 팩터리를 수행 합니다.         |
...
|옵션  |설명  |
|---------|---------|
|/force   |연결 된 저장소를 다시 설정 해야 한다는 것을 확인 합니다. **** 강제로/지정 하지 않고 초기화 명령을 실행 하면 표시할 다음 메시지가: 연결 된 저장소 공장 초기화는 파괴적 작업을 **** 강제로/플래그 아닌 경우이 명령을 초기화를 수행 하지 않는 존재 합니다. |
...
연결 된 저장소 공간으로 *파일 이름* 지정 된 데이터를 가져옵니다.
...
- 입력된 파일이 올바르게 채워지는 **ContextDescription** 섹션에 있는 경우 다음이 사용할 연결 된 저장소 공간을 지정 합니다.
- 저장소 공간 지정할 수도 있습니다 부분적으로 또는 완전히 입력된 파일에 지정 된 저장소 공간의 각 요소 보다 우선 적용 하는 명령줄 매개 변수를 통해 합니다.
...
> [!NOTE]
> 지정 된 연결 된 저장소 공간에를 가져오기 전에 시스템은 실행 중인 앱을 연결 된 저장소 공간을 확보 때 실행 되는 동일한 논리를 사용 하 여 클라우드와 동기화 하려고 합니다.
>
> 동일한 기본 서비스 안내를 사용 하 여 응용 프로그램을 실행 하는 경우이 작업에는 경합 상태를 일으킬 수 있으며 연결 된 저장소 공간의 내용을 확정 되지 않은 상태에서 일 수 있습니다.
>
> **** 바꾸기/를 지정 하지 않으면 입력된 파일에 지정 된 컨테이너는 입력된 파일에 지정 된 blob을 작성 하기 전에 지워집니다. 입력된 파일에 지정 되지 않은 연결 된 저장소 공간에서 컨테이너는 그대로 유지 됩니다.
...
|/msa:*계정*  |사용자별 연결 된 저장소에 사용할 계정을 지정 합니다. 사용자가 콘솔에 사용할 공간 서명 해야 합니다.  이 옵션은 **/machine** 옵션을 사용 하 여 동시에 사용할 수 없습니다.         |
...
- **/Context** 매개 변수를 사용 하 고 파일 이름을 지정 하 여 \ < infile > 섹션이 **ContextDescription** 올바르게 채워지는 다음 연결 된 저장소 공간을 지정 하는 파일이 사용 됩니다.
- 저장소 공간 지정할 수도 있습니다 부분적으로 또는 완전히 **/context** 파일에 지정 된 저장소 공간의 각 요소 보다 우선 적용 하는 명령줄 매개 변수를 통해 합니다.
...
> [!NOTE]
> 지정된 된 연결 된 저장소 공간을 내보내기 전에 시스템은 실행 중인 앱을 연결 된 저장소 공간을 확보 때 실행 되는 동일한 논리를 사용 하 여 클라우드와 동기화 하려고 합니다.
>
> 동일한 기본 서비스 안내를 사용 하 여 응용 프로그램을 실행 하는 경우이 작업에는 경합 상태를 일으킬 수 있으며 연결 된 저장소 공간의 내용을 확정 되지 않은 상태에서 일 수 있습니다.

|옵션  |설명  |
|---------|---------|
|*출력*             |XML 파일의 데이터를 내보냅니다.         |
|/context:*입력 파일* |공간 정보를 읽을 수 있는 입력된 파일을 지정 합니다.         |
|/scid:*서비스 안내*          |서비스 구성 id (서비스 안내)을 지정합니다.         |
|/machine              |컴퓨터별 연결 된 저장소 공간을 지정합니다.  이 옵션은 **/msa** 옵션을 사용 하 여 동시에 사용할 수 없습니다.         |
|/msa:*계정*        |사용자별 연결 된 저장소에 사용할 계정을 지정 합니다. 사용자가 콘솔에 사용할 공간 서명 해야 합니다.  이 옵션은 **/machine** 옵션을 사용 하 여 동시에 사용할 수 없습니다.         |
|/verbose              |내보내기 작업의 상태를 표시 합니다.         |
...
> [!NOTE]
> 지정된 된 연결 된 저장소 공간을 삭제 하기 전에 시스템은 실행 중인 앱을 연결 된 저장소 공간을 확보 때 실행 되는 동일한 논리를 사용 하 여 클라우드와 동기화 하려고 합니다.
>> 동일한 기본 서비스 안내를 사용 하 여 응용 프로그램을 실행 하는 경우이 작업에는 경합 상태를 일으킬 수 있으며 연결 된 저장소 공간의 내용을 확정 되지 않은 상태에서 일 수 있습니다.
...
더미 데이터를 생성 하 고 지정 된 파일에 저장 \ < 이름 > 합니다. 파일의 XML 형식에 대 한 자세한 내용은이 항목의 뒷부분에 [가져오기 및 내보내기 파일 형식](#xbstorage_fileformat)을 참조 하세요.    서비스 구성 id (서비스 안내) 00000000-0000-0000-0000-000000000000로 설정 됩니다 및 계정 컴퓨터별 연결 된 저장소 공간에 대 한 설정 됩니다. 해당 값을 변경 하려는 경우 직접 편집할 수 있습니다 파일 생성 됩니다.
...
> [!NOTE]
> 바이트 데이터는 간단한 오름차순; 예를 들어 5 비트 blob 다음 바이트 없어서: 00 01 02 03 04 합니다. >> 사용자별 연결 된 저장소 공간을 지정 하려는 경우 XML 파일의 **계정** 노드에 다음과 같이 변경 합니다.
>  ```
...
|/containers:*n* | 숫자를 *n*을 생성 하는 컨테이너를 지정 합니다. 기본 횟수는 2입니다.  |
|blob /:*n*      | 숫자를 *n*을 생성 하는 blob을 지정 합니다. 기본 횟수는 3입니다.  |
|/blobsize:*n*   | 숫자를 *n*, blob 당 바이트를 지정합니다. 기본 크기는 1024 바이트입니다.  |
...
| / forceoutoflocalstorage | 왼쪽 공간이 필요 하는 연결 된 저장소 서비스를 시뮬레이트합니다. 연결 된 저장소에서 삭제 메모리 부족 보고에서 연결 된 저장소 서비스는 변경 되지 않습니다. |
...
|옵션  |설명  |
|---------|---------|
| /?                           |  Xbstorage.exe에 대 한 디스플레이 도움말 |
| /X *: 주소* [*+ accesskey*]  | 호스트 이름 또는 주소 (콘솔에 **도구 IP** 로 표시 됨)의 대상된 콘솔을 지정 하지만 기본 콘솔 변경 되지 않습니다. 이 옵션을 사용 하지 않는 경우 콘솔의 기본 사용 됩니다. *Accesskey* 선택 키를 알고 있는 사용자만 콘솔에 대 한 액세스를 제한 하는 데 사용할 수 있는 문자열입니다. **Xbconfig**명령을 사용 하 여 선택 키를 설정 **accesskey = * * * your 키*; 그런 다음 선택 키를 적용 하려면 콘솔이 다시 시작 합니다. 선택 키를 사용 하 여 구성 된 콘솔에 액세스 하려면 콘솔의 IP 주소 또는 호스트 이름 뒤에 더하기 (+) 및 선택 키를 포함 해야 합니다.
> [!NOTE]
> Xbconnect 하 여 콘솔의 기본 설정 된 경우 선택 키 제공 하는 경우 선택 키의 기본 콘솔 주소의 일부로 저장 됩니다.
|

## <a name="gamesaveutil"></a>Gamesaveutil

*Gamesaveutil* 같은 모든 앱 *xbstorage* 해당 기능에 대 한 로컬에 캐시 된 연결 된 저장소를 관리할 수 있도록 제공 합니다. Xbstorage 도구와 마찬가지로 *gamesaveutil* 동작에 몇 가지 차이점이 함수 관리 동일한 6 개의 데이터를 제공 합니다. 가져오기, 내보내기, 삭제 및 초기화 명령을 로그인 하는 Xbox Live 사용자 필요 합니다. 보고 현재 사용자를 변경할 수 Windows 10의 Xbox 앱을 사용할 수 있습니다.
...
|다시 설정    |지정된 된 앱에 대해서만 로컬 데이터를 삭제합니다.         |
...
앱을 지정 하는 방법은 두 가지가 PFN 및 데이터가 저장 되는 위치 서비스 안내 합니다.

입력된 파일에 올바르게 채워지면 ContextDescription 섹션이 있는 경우이 하는 데 대상 앱 지정 PFN 및 서비스 안내 합니다.

PFN 및 서비스 안내 또는 부분적으로 지정할 수 입력된 파일에서 지정 된 PFN 및 서비스 안내의 각 요소 보다 우선 하는 명령줄 매개 변수를 통해 합니다.
...
> 
> /Replace를 지정 하지 않으면 확장 입력된 파일에 지정 되어 하지 않는 한 기존 컨테이너 터치 하지 않습니다.
...
파일은 데이터가 포함 된 XML 파일. Gamesaveutil 도움말 입력 예를 생성 하는 방법을 보려면 생성 합니다.

내보내기 데이터의 위치를 지정 하는 방법은 두 가지가 있습니다.

/Context 매개 변수를 사용 하 고 파일 이름을 지정 하 여 \ < infile > 올바르게 채워지는 ContextDescription 섹션에 다음 원본 데이터의 위치를 지정 하려면 해당 파일이 사용 됩니다.

/Context 파일에서 지정 된 각 요소 보다 우선 적용 하는 명령줄 매개 변수를 통해 위치를 지정할 수도 있습니다.

|옵션  |설명  |
|---------|---------|
|/context: \ < infile >     |지정 된 파일을 사용 하 여 앱을 지정 하려면 PFN 및 서비스 안내 합니다.         |
...
지정 된 PFN 및 서비스 안내에 대 한 모든 데이터가 삭제 됩니다.
...
더미 데이터를 생성 하 고 지정 된 파일에 저장 \ < 이름 > 합니다.
...
> [!NOTE]
> 바이트 데이터 이란 순차적으로 간단한 오름차순, 5 바이트 blob 한 바이트가 00 01 02 03 04 즉 합니다.
...
**가져오기**, **내보내기**및 *gamesaveutil* 에서 **생성** 에이 xml 서식을 지정 하는 데 필요한 변경 교체 하는 것은 \의 < 계정 > 구성원 노드는 \ < ContextDescription > 노드는 \ < PackageFamilyName > 노드.
...
> [!NOTE]
> XML 파일에 데이터의 형식을 플랫폼에서 동일 이므로 가져오기 및 내보내기 목적 으로만 합니다. 중간 또는 유틸리티 형식, 보관 된 형식의 하지으로 처리 해야 하므로 이러한 XML 파일에 대 한 데이터 형식을 앞으로 변경 될 수 있습니다.

**ContextDescription** 노드 선택 사항입니다. 컴퓨터에 대해 연결 된 저장소 공간을 하는 경우 사용할 수 있습니다 `<Account machine="true"/>` 대신 `<Account msa="user@domain.com"/>`. 그렇지 않으면 컨텍스트 가져오기에 대 한 명령줄에서 지정할 수 있습니다.
Blob 및 컨테이너 게임 또는 파일 생성 되는 응용 프로그램에 게 부여 된 해당 이름을 사용 해야 합니다.
각 blob 콘텐츠의 **CryptBinaryToStringW** **CRYPT_STRING_BASE64** 해당 blob 원시 바이트 배열에 대 한 데이터를 제공 하는 플래그를 사용 하 여 호출 하 여 생성 된 **CDATA** 태그에 래핑된 문자열 이어야 합니다. 반대로, blob 데이터 **CryptStringToBinary** 호출 이전의 암호화 된 문자열을 제공 하 여 다시 변환할 수 있습니다. 이러한 두 함수를 사용 하는 예제는 Visual Studio에 대 한 MSDN 포럼에서 [잘못 된 바이트를 반환 하는 CryptBinaryToString](http://social.msdn.microsoft.com/Forums/vstudio/en-US/9acac904-c226-4ae0-9b7f-261993b9fda2/cryptbinarytostring-returns-invalid-bytes?forum=vcgeneral) 에 표시 됩니다.
