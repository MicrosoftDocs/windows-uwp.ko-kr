...
이 API를 사용 하면 사용자의 자체 클립 물론 다른 사용자의 클립 서비스에 저장 된 목록에 다양 한 방법입니다. 여러 진입점 수준에서 데이터를 반환 하 고 쿼리 매개 변수를 통해 필터링에 대 한 허용 합니다. XUID 클레임에서 일치 하는 URI에 지정 된 소유자 콘텐츠 격리 확인 한 후 사용자의 클립 반환 됩니다. URI의 소유자 클레임 XUID와 일치 하지 않는 경우 다음 지정 된 사용자의 클립에 따라 반환 됩니다 개인 정보 보호 검사 및 요청 XUID에 대 한 콘텐츠 격리 검사 합니다.
...
동일한 API 호출 내에서 여러 사용자 목록 가져오기 위한 API를 배치 하지 있습니다. 그러면 각 사용자에 대 한 쿼리를 SLS 설계자에서 (현재) 권장된 패턴은.
...
| maxItems| 32 비트 부호 있는 정수| 선택 사항입니다. 최대 (skipItems 항목의 범위를 반환 하는 continuationToken와 결합할 수 있습니다) 컬렉션에서 반환할 항목 수입니다. 서비스는 maxItems 존재 이며 (경우에 결과의 마지막 페이지 아직 반환 되지 않은) maxItems 보다 적은 반환할 수 있습니다 기본값을 제공할 수 있습니다.|
| 주문| 유니코드 문자| 선택 사항입니다. 지정 (D) escending의 목록을 반환 하는 경우 (처음 값 가장 높은) scending (A) 또는 (먼저 값 최저) 순서. 기본값: D.|
...
| 한정자| string| 선택 사항입니다. 클립을 가져오는 데 사용할 순서 한정자를 지정 합니다. <ul><li>만든-클립 시스템에 반환 날짜의 순서로 지정</li><li>-[최고 등급]-등급 클립에서 등급 값을 반환 지정</li><li>[본 가장]-보기-지정 클립 보기의 수를 반환 합니다.</li></ul><br/> 최대 크기: 12 합니다. 기본값: "만든".| 
...
다음 URI는 기본 채널 서비스 안내를 지정 하는 추가 경로 매개 변수를 사용 하면서이 문서의 동일 합니다. 해당 서비스 안내 해당 사용자의 클립만 반환 됩니다. 요청 사용자에 게 요청 된 서비스 안내에 액세스할 수 있어야, 그렇지 않으면 HTTP 오류 403 반환 됩니다.
