...
이번 항목에서는 [C++/CX](/cpp/cppcx/visual-c-language-reference-c-cx?branch=live) 개체와 C++/WinRT 개체를 서로 변환하는 데 사용할 수 있는 두 가지 도우미 함수에 대해서 설명합니다. 두 언어 프로젝션을 사용 하는 코드 사이의 상호 운용성 활용할 수 또는 점차 마이그레이션하 코드에서 C + 함수를 사용할 수 + CX C + + WinRT (참조 [이동 C + +에서 C + + CX](move-to-winrt-from-cx.md)).
