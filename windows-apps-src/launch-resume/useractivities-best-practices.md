...
* **관련된 사용자 작업 그룹에 대 한 단일 활동을 기록 합니다.** 이 특히 음악 재생 목록 또는 TV 프로그램에 대 한 관련 된: 단일 활동을 사용자의 진행률을 반영 하기 위해 정기적으로 업데이트할 수 있습니다. 이 경우 여러 일 또는 주에 걸쳐 참여 기간을 나타내는 여러 기록 항목을 사용 하 여 단일 사용자 활동을 해야 합니다. 동일한 문서 기반 활동에는 사용자가 앱 내에서 점진적 진행률에 적용 됩니다.
...
* **사용자가 다시 시작 필요 하지 않은 작업에 대 한 활동을 만들지 않습니다.** 응용 프로그램을 간단 하 고 일회성 작업 상태를 유지 하지 않는 완료 하는 데 사용 하는 경우에 아마도 사용자 활동을 만들 하지 필요가 합니다.
...
* **게임** -각 게임 저장 또는 세계에 대해 하나의 활동을 만듭니다. 게임 수준만 단일 시퀀스를 지 원하는 경우 게시할 수 있습니다 다시 동일한 활동 시간이 지남에 따라 있지만 최신 진행 중 또는 도전 과제를 표시 하려면 콘텐츠 데이터를 업데이트 하고자 할 수 있습니다.
...
* **미디어 재생 앱** -콘텐츠 (예: 재생 목록, 프로그램 또는 독립 실행형 콘텐츠)의 논리적 그룹화 당 하나의 활동을 만듭니다. 앱 개발자를 위한 기본 질문 독립 실행형 콘텐츠 또는 컬렉션의 일부로 (TV 에피소드, 노래) 콘텐츠의 각 부분을 계산 하는 여부입니다. 일반적으로 사용자가 컬렉션 또는 순차적 콘텐츠를 재생 하는 경우 전체 컬렉션 활동입니다. 단일 콘텐츠의 재생을 옵트인 (opt)을 하는 경우 해당 하나의 콘텐츠의 활동입니다. 다음 자세한 지침을 참조 하세요.
  * **음악: 장르/앨범/아티스트** -사용자는 앨범, 아티스트, 또는 장르를 선택 하 고 **재생**부딪치는, 해당 컬렉션 인지 작업입니다. 각 노래에 대 한 별도 활동을 기록 하지 않습니다. 단일 앨범 같은 짧은 컬렉션 또는 컬렉션 임의의 순서로 다시 재생 중인 경우 하지 않아도 사용자의 현재 위치를 반영 하도록 해당 활동을 업데이트 합니다. 앨범 또는 재생 목록 등의 장기 순차적 재생을 위해 앨범 내 위치에 기록 적합할 수 있습니다.
  ...
  * **동영상** -동영상 콘텐츠의 단일 이며 자체 기록 레코드가 있어야 합니다. 경우 사용자가 동영상 일부 방법을 통해 감시를 자신의 위치를 기록 하는 것이 좋습니다. 나중에 다시 시작 하려고 할 때 활동에 마지막으로, 영화를 다시 시작 하거나 다시 시작 하거나 처음부터 시작 하려는 경우 사용자도 요청할 수 없습니다.
...
* 필수 및 선택적 속성 집합으로 이루어진 시각적 데이터는 (예: 제목, 설명 또는 적응형 카드 요소로), 사용자가 활동을 시각적으로 식별할 수 있도록 합니다. 사용자 활동에 대 한 적응형 카드 시각적 개체 만들기에 대 한 지침은 아래를 참조 하세요.
...
* 각 활동은 항상 표시 해야 하는 활동 이름을 제공 해야 합니다.
  * 이 이름은 큰 굵게 옵션을 사용 하 여 카드의 왼쪽 위 모서리에 표시할지 나타내야 합니다. 이름은 쉽게 인식할 수 있는 유일한 이것이 반드시 활동은 Cortana 시나리오에 표시 되는 경우 일부 사용자가 표시 됩니다. 타임 라인에 동일한 이름을 보여 주는 쉽게 사용자가 많은 작업을 찾아볼 수 있습니다.
...
다음 예제에서는 콘텐츠 메타 데이터 [Schema.org](https://schema.org/)표준을 따르도록 JSON 시나리오를 나타냅니다.: "John 재생 화난 새 Steve와."
