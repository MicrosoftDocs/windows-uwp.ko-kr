...
- 확장된 모드에서 기본 및 보조 명령이 표시 됩니다. (컨트롤 보조 항목에 대해서만 있으면에 나타나는 MenuFlyout 컨트롤에 유사한 방식으로.)

## <a name="is-this-the-right-control"></a>올바른 컨트롤인가요?

명령 모음 단추 및 앱 캔버스에 있는 요소의 컨텍스트 메뉴 항목 등의 사용자에 게 표시 하도록 CommandBarFlyout 컨트롤을 사용 합니다.

TextCommandBarFlyout TextBox, TextBlock, RichEditBox, RichTextBlock, 및 PasswordBox 컨트롤의 명령을 텍스트를 표시합니다. 현재 선택한 텍스트에 명령은 적절 하 게 자동으로 구성 됩니다. CommandBarFlyout를 사용 하 여 텍스트 컨트롤의 기본 텍스트 명령은 바꿉니다.

상황에 맞는 표시 하려면 목록 항목에 명령을 [명령 컬렉션 및 리스트에 대 한 상황별](collection-commanding.md)에서 지침을 따릅니다.
...
사전 호출에서 명령을 연결 된 항목와 상호 작용할 명령이 자동으로 표시 됩니다. 예를 들어 텍스트 서식 명령이 수 팝업 사용자가 텍스트 상자에 텍스트를 선택 합니다. 이 경우 명령 모음 플라이 아웃은 포커스를 고려 하지 않습니다. 대신, 사용자가 상호 작용 항목 가까운 관련 명령을 표시 됩니다. 명령을 사용 하 여 사용자 상호 작용 하지 않는 경우 해제 됩니다.
...
이 예제에서는 명령 모음 플라이 아웃을 만들고 사전 및 사후 방식으로 사용 하는 방법을 보여 줍니다. 이미지를 누를 때 축소 모드에서 플라이 아웃이 표시 됩니다. 상황에 맞는 메뉴로 표시 되 면 플라이 아웃이 확장 모드로 표시 됩니다. 두 경우 모두 사용자가 확장 하거나를 연 후 플라이 아웃을 축소할 수 있습니다.
...
사전에 상황에 맞는 명령을 표시 한 경우 기본 명령이 (명령 모음 플라이 아웃 축소할 해야)는 기본적으로 표시 되어야 합니다. 기본 명령이 컬렉션에 추가 명령은 보조 명령이 컬렉션에 상황에 맞는 메뉴에서 일반적으로 이동 하는 가장 중요 한 명령을 배치 합니다.

사전 명령을 표시 하려면 일반적으로 명령 모음 플라이 아웃을 표시 하려면 [클릭](/uwp/api/windows.ui.xaml.controls.primitives.buttonbase.click) 하거나 [Tapped](/uwp/api/windows.ui.xaml.uielement.tapped) 이벤트를 처리 합니다. 플라이 아웃의 [ShowMode](/uwp/api/windows.ui.xaml.controls.primitives.flyoutbase.showmode) 설정 **임시** 추가 또는 **TransientWithDismissOnPointerMoveAway** 포커스를 수행 하지 않고 해당 축소 모드에서는 플라이 아웃을 엽니다.

Windows 10 Insider Preview부터 텍스트 컨트롤에는 **SelectionFlyout** 속성이 있습니다. 이 속성에 플라이 아웃을 할당 하는 경우 텍스트를 선택 하면 자동으로 표시 됩니다.
...
명령을 상황에 맞는 메뉴를 표시 하려면 일반적으로 UI 요소의 [ContextFlyout](/uwp/api/windows.ui.xaml.uielement.contextflyout) 속성에는 플라이 아웃을 할당 합니다. 이렇게이 하면 요소에 의해 처리 되 플라이 아웃을 열고 하 고 더 많은 아무 작업도 수행할 필요가 없습니다.
...
앱 바 단추 컨트롤은 아이콘 및 텍스트 레이블에 따라 구분됩니다. 이러한 컨트롤은 명령 모음에서 사용 하기 위해 최적화 하 고는 컨트롤이 명령 모음 또는 오버플로 메뉴에 표시 되는 여부에 따라 모양이 변경 됩니다.

- 기본 명령으로 사용 되는 앱 바 단추 아이콘;만 있는 명령 모음에 표시 됩니다. 텍스트 레이블이 표시 되지 않습니다. 다음과 같이 명령에 대 한 설명을 표시 하는 도구 설명이 사용 하는 것이 좋습니다.
    ```xaml
    <AppBarButton Icon="Copy" ToolTipService.ToolTip="Copy"/>
    ```
...
여기서는 AppBarElementContainer 명령 모음 플라이 아웃에 추가 요소를 추가 하는 데 사용 됩니다. 분할 단추는 색 중에서 선택할 수 있도록 기본 명령에 추가 됩니다. StackPanel 확대/축소 컨트롤에 대 한 더 복잡 한 레이아웃을 허용 하기 위해 보조 명령에 추가 됩니다.
...
[TextCommandBarFlyout](/uwp/api/microsoft.ui.xaml.controls.textcommandbarflyout) 텍스트 편집에 대 한 명령을 포함 하는 특수 명령 모음 플라이 아웃입니다. 각 텍스트 컨트롤과 텍스트를 선택한 경우 또는 상황에 맞는 메뉴 (마우스 오른쪽 단추)으로 TextCommandBarFlyout 자동으로 표시 됩니다. 텍스트 명령 모음 플라이 아웃은 관련 명령을 표시 하도록 선택한 텍스트에 적응 합니다.
...
이 표에서 TextCommandBarFlyout 및 표시 되 면 포함 된 명령을 보여 줍니다.

| 명령 | 표시 된... |
| ------- | -------- |
| 굵게 | 때 텍스트 컨트롤은 읽기 전용 (RichEditBox만) 없습니다. |
| 기울임꼴 | 때 텍스트 컨트롤은 읽기 전용 (RichEditBox만) 없습니다. |
| 밑줄 | 때 텍스트 컨트롤은 읽기 전용 (RichEditBox만) 없습니다. |
...
TextCommandBarFlyout 인쇄할 수 및 각 텍스트 컨트롤에 의해 자동으로 관리 됩니다. 그러나 사용자 지정 명령으로 기본 TextCommandBarFlyout 바꿀 수 있습니다.

- 기본 텍스트 선택에 표시 되는 TextCommandBarFlyout 바꾸려면 사용자 지정 CommandBarFlyout (또는 다른 플라이 아웃 형식)을 만들 수 있으며 **SelectionFlyout** 속성에 할당 키를 누릅니다. SelectionFlyout를 **null로**설정 하면 명령이 없는 선택에 표시 됩니다.
- 상황에 맞는 메뉴로 표시 되는 TextCommandBarFlyout 기본 바꾸려면 **ContextFlyout** 속성 텍스트 컨트롤에 사용자 지정 CommandBarFlyout (또는 다른 플라이 아웃 형식)을 할당 합니다. ContextFlyout를 **null로**설정 하면 이전 버전의 텍스트 컨트롤에 표시 된 메뉴 플라이 아웃은 TextCommandBarFlyout 대신 표시 됩니다.
