...
--- | --- | --- | ---
이끌이 이름 | 종료 시간-이 토너먼트가 있는 날짜/시간에 "취소" 또는 "Completed" 상태에 도달 합니다. 토너먼트 업데이트 될 때 자동으로 설정 됩니다. | 이전 일치 (토너먼트 게임 결과 상태, 등급, 일치 시작 시간, 종료 시간은는 Bye, 설명, 팀 Id 반대) | 등록 상태 (알 수 없음, 보류, 제거, 거부, 등록, 완료 됨)
...
상품에 | 상대 팀 Id | 최종 순위 팀 | 등록 시작/종료 시간
최소/최대 팀 크기 | | 연속 URI-아레나 ui 팀 구성원을 다시 사용 | 시작/종료 시간을 확인
게임 모드 | | 현재 일치 메타 데이터 (설명, 시작 시간, Bye, 반대 팀 Id는) | 등록 팀의 수입니다.
