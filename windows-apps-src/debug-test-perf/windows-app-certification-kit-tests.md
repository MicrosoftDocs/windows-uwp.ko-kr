...
description: Windows 앱 인증 키트에는 앱을 Microsoft Store에 게시할 준비가 되었는지 확인하는 데 도움이 되는 다양한 테스트가 들어 있습니다.
ms.date: 02/08/2017
ms.topic: article
...
[Windows 앱 인증 키트](windows-app-certification-kit.md)에는 앱을 Microsoft Store에 게시할 준비가 되었는지 확인하는 데 도움이 되는 다양한 테스트가 들어 있습니다. 아래에는 오류 발생 시 조건, 세부 정보 및 권장 작업과 함께 테스트가 나열되어 있습니다.
...
앱은 HKEY\-LOCAL\-MACHINE\\Software\\Microsoft\\Windows NT\\CurrentVersion\\Windows\\AppInit\-DLL 레지스트리 키에 로드할 DLL을 나열하면 안 됩니다.
...
운영 체제 버전 정보의 용도는 Microsoft Store로 제한됩니다. 이 정보는 종종 앱에서 앱 OS 버전과 관련된 기능을 사용자에게 제공할 수 있도록 OS 버전을 확인하는 데 잘못 사용되었습니다.
...
앱 패키지(.msix, .appx 또는 앱 번들)에 애플리케이션이 하나 포함되어 있는지 확인합니다. 키트에서 독립 실행형 테스트가 되도록 이 검사가 변경되었습니다.
...
Windows Phone 8.1 앱의 경우 이 테스트는 번들에 포함된 총 .appx 패키지 수가 512개 미만(&lt; 512)이고, 번들의 기본 패키지가 하나이고, 번들에 포함된 기본 패키지의 아키텍처가 ARM으로 표시되는지 아니면 중립으로 표시되는지 여부를 확인합니다.
...
[Binscope 이진 분석기](https://www.microsoft.com/download/details.aspx?id=44995) 테스트는 앱의 이진 파일을 검사하여 앱을 공격이나 공격 벡터로 사용되는 것에 덜 취약하게 만드는 코딩 및 빌드 규칙을 확인합니다.
...
### <a name="span-idbinscope-2spansafeseh-exception-handling-protection"></a><span id="binscope-2">/SafeSEH 예외 처리 보호</span>
...
### <a name="span-idbinscope-3spandata-execution-prevention"></a><span id="binscope-3">데이터 실행 방지</span>
...
관리형 실행 파일이 테스트에 실패할 경우 Microsoft Visual Studio 같은 최신 컴파일러와 링커를 사용하여 UWP 앱을 빌드했는지 확인하세요.
...
Microsoft Store용으로 인증을 받으려면 앱에서 UWP 앱용 API(Windows 런타임 또는 지원되는 Win32 API)를 사용해야 합니다. 이 테스트는 관리되는 이진 파일이 승인된 프로필 외부의 기능에 종속하는 경우도 식별합니다.

### <a name="test-details"></a>테스트 정보

-   이진 파일의 가져오기 주소 테이블을 검사하여 앱 패키지 내의 각 이진 파일이 UWP 앱 개발에 지원되지 않는 Win32 API에 종속되지 않는지 확인합니다.
...
<p>MakePRI.exe는 <strong>AutoMerge</strong>라는 옵션을 지원합니다. <strong>AutoMerge</strong>의 기본값은 <strong>off</strong>입니다. 이 옵션을 사용하면 <strong>AutoMerge</strong>에서 런타임에 앱의 언어 팩 리소스를 단일 resources.pri에 병합합니다. Microsoft Store를 통해 배포하려는 앱에는 이 옵션을 권장하지 않습니다. Microsoft Store를 통해 배포되는 앱의 resources.pri는 앱 패키지의 루트에 있어야 하며 앱에서 지원하는 모든 언어 참조를 포함해야 합니다.</p>
</td></tr>
<tr><td>
...
UWP 앱의 모든 기능이 완벽하게 작동해야 합니다. 템플릿 또는 SDK 샘플의 기본 이미지를 사용하는 앱은 부적절한 사용자 환경을 제공하며 스토어 카탈로그에서 쉽게 식별할 수 없습니다.
...
Microsoft Store를 사용하려면 Direct3D를 사용하는 모든 애플리케이션이 기능 수준 9\-1 그래픽 카드에서 제대로 렌더링되거나 정상적으로 실패해야 합니다.
...
이 테스트는 앱이 기능 수준 9\-1에서 정확하게 렌더링되는지 확인합니다.
...
> **참고**  이 테스트는 Windows 8.1 이상용으로 개발된 UWP 앱에만 적용됩니다.
...
애플리케이션이 MAX\-PATH보다 긴 하위 디렉터리를 설치 과정에서 만들지 않는지 확인합니다.
