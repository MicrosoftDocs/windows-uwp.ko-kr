---
author: QuinnRadich
Description: "이러한 지침은 앱에 대한 효과적인 도움말 콘텐츠를 디자인하는 방법을 설명합니다."
title: "앱 도움말에 대한 지침"
label: Guidelines for app help
template: detail.hbs
ms.sourcegitcommit: 0aa2db498ab7ec25839da259dd0026b0a7cd2b13
ms.openlocfilehash: f2522afa91abe26303a85cbfbabd5ec5b3dba59c

---

# 앱 도움말에 대한 지침

\[ Windows 10의 UWP(유니버설 Windows 플랫폼) 앱에 대해 업데이트되었습니다. Windows 8.x 문서는 [보관](http://go.microsoft.com/fwlink/p/?linkid=619132)을 참조하세요. \]

응용 프로그램은 복잡할 수 있으며, 사용자에게 효과적인 도움말을 제공하면 환경을 훨씬 개선할 수 있습니다. 모든 응용 프로그램이 사용자에게 도움말을 제공할 필요는 없으며, 응용 프로그램과 사용 방법에 따라 제공되는 도움말의 유형이 크게 달라질 수 있습니다.

도움말을 제공하려는 경우 여기에서 설명하는 지침을 고려합니다. 유용하지 않은 도움말은 도움말이 없는 것보다 나쁠 수 있다는 점도 명심해야 합니다.

## <span id="intuitive_design"></span><span id="INTUITIVE_DESIGN"></span>직관적인 디자인

도움말 콘텐츠는 유용할 수 있지만 앱에서 사용자에게 좋은 환경을 제공하기 위해 도움말에 의존할 수는 없습니다. 앱의 중요한 기능을 즉시 찾아서 사용할 수 없다면 앱을 사용하려고 하지 않을 것입니다. 도움말 콘텐츠는 이러한 첫인상을 변화시킵니다.

직관적이고 사용자에게 친숙한 디자인은 유용한 도움말을 작성하는 첫 번째 단계입니다. 사용자가 보다 고급 기능을 사용할 만큼 오랜 시간의 참여를 유지할 뿐만 아니라 계속 사용할 때 토대로 사용할 수 있는 앱의 핵심 기능에 대한 지식도 제공합니다.

## <span id="general_instructions"></span><span id="GENERAL_INSTRUCTIONS"></span>일반 지침

사용자는 문제가 없을 경우 도움말 콘텐츠를 찾지 않으므로 도움말은 해당 문제에 대한 빠르고 효과적인 응답을 제공해야 합니다. 도움말이 즉시 유용하지 않거나 너무 복잡한 경우 사용자가 무시할 가능성이 커집니다.

종류에 관계없이 모든 도움말은 다음과 같은 기본 원칙을 따라야 합니다.

-   **이해하기 쉬움:** 사용자에게 혼동을 주는 도움말은 도움말이 없는 것보다 더 나쁩니다.

-   **명확함:** 도움말을 찾는 사용자는 명확한 답변이 바로 제공되기를 원합니다.

-   **관련됨:** 사용자는 특정 문제를 검색할 필요가 없기를 원합니다. 가장 관련된 도움말이 먼저 제공되는 것이나(이를 "상황에 맞는 도움말"이라고 함), 쉽게 탐색되는 인터페이스가 선호됩니다.

-   **직접성:** 사용자는 도움말을 찾을 때 도움말을 보려고 합니다. 앱에 버그 보고, 피드백 제공, 서비스 조건 확인 또는 유사한 기능에 대한 페이지가 포함된 경우 이러한 페이지는 주 도움말 페이지에서 중요한 항목으로 포함되지 않고 링크나 각주로 포함되어야 합니다.

-   **일관성:** 유형에 관계없이 도움말은 여전히 앱의 일부이며 UI의 다른 부분으로 처리되어야 합니다. 앱의 나머지 부분 전체에서 사용되는 유용성, 접근성 및 스타일의 동일한 디자인 원칙이 제공하는 도움말에도 있어야 합니다.

## <span id="types_of_help"></span><span id="TYPES_OF_HELP"></span>도움말 유형

각각 다른 장점이 있고 다른 용도에 적합한 세 가지 기본 범주의 도움말 콘텐츠가 있습니다. 요구에 따라 앱에서 이러한 범주를 임의로 조합해서 사용할 수 있습니다.

### <span id="instructional_ui"></span><span id="INSTRUCTIONAL_UI"></span>UI 사용 안내

이상적으로는 사용자가 지침 없이 앱의 모든 핵심 기능을 사용할 수 있어야 하지만, 가끔 앱에서 특정 제스처를 사용해야 하거나 즉각적으로 명확하지 않은 앱의 보조 기능이 있을 수 있습니다. 이러한 경우 사용 안내 UI를 사용하여 특정 작업을 수행하는 방법에 대한 지침을 사용자에게 제공해야 합니다.

[사용 안내 UI에 대한 지침 참조](instructional-ui.md)

### <span id="in_app_help"></span><span id="IN_APP_HELP"></span>앱 내 도움말

도움말을 표시하는 표준 방법은 사용자 요청 시 응용 프로그램 내에서 표시하는 것입니다. 도움말 페이지, 유용한 설명 등 여러 가지 방법으로 구현할 수 있습니다. 이 방법은 복잡하지 않게 사용자 질문에 직접 답변하는 범용 도움말에 적합합니다.

[앱 내 도움말에 대한 지침 참조](in-app-help.md)

### <span id="external_help"></span><span id="EXTERNAL_HELP"></span>외부 도움말

너무 커서 응용 프로그램에 맞지 않는 자세한 자습서, 고급 함수 또는 도움말 항목 라이브러리의 경우 외부 웹 페이지에 대한 링크가 적합합니다. 이러한 링크는 응용 프로그램 환경에서 사용자를 벗어나게 하므로 가능한 한 사용하지 않도록 해야 합니다.

[외부 도움말에 대한 지침 참조](external-help.md)

\[이 문서에는 UWP(유니버설 Windows 플랫폼) 앱 및 Windows 10과 관련된 정보가 있습니다. Windows 8.1 참고 자료는 [Windows 8.1 지침 PDF](https://go.microsoft.com/fwlink/p/?linkid=258743)를 다운로드하세요.\]



<!--HONumber=Jun16_HO5-->


