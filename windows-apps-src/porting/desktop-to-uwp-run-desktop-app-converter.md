...
Desktop App Converter(DAC)를 사용해 유니버설 Windows 플랫폼(UWP)에 데스크톱 앱을 가져올 수 있습니다. Win32 앱 및 사용자가 .NET 4.6.1을 사용해 생성한 앱이 여기에 포함됩니다.
...
<div style="float: left; ">
    ![확인](images/desktop-to-uwp/check.png)
</div>
...
앱을 테스트할 수 있도록 패키지를 자동으로 로그인합니다.
...
1. 운영 체제의 버전 번호를 확인합니다.
   ...
   이를 위해 컴퓨터에서 **시스템 정보 앱**을 열고 운영 체제의 버전 번호를 찾으면 됩니다.

    ![시스템 정보 앱의 운영 체제 버전](images/desktop-to-uwp/os-version.png)

2. 해당 [Desktop App Converter 기본 이미지](https://aka.ms/converterimages)를 다운로드합니다.

   파일 이름에 표시된 버전 번호가 Windows 빌드의 버전 번호와 일치하는지 확인합니다.

   ![일치 하는 기본 이미지 버전](images/desktop-to-uwp/base-image-version.png)

   컴퓨터의 아무 곳에나 다운로드한 파일을 두어도 나중에 찾을 수 있습니다.

3. Desktop App Converter를 시작할 때 나타난 콘솔 창에서 ```Set-ExecutionPolicy bypass``` 명령을 실행합니다.
4.  ```DesktopAppConverter.exe -Setup -BaseImage .\BaseImage-1XXXX.wim -Verbose``` 명령을 실행하여 변환기를 설정합니다.
5.  컴퓨터를 다시 시작하라는 메시지가 표시되면 작업을 수행합니다.
...
* [앱 패키징, 앱 서명, 스토어 제출 준비](#optional-parameters)
...
> [!NOTE]
> 설치 관리자가 독립 폴더에 위치하고 해당 설치 관리자와 관련된 파일만 같은 폴더에 있는지 확인합니다. 변환기는 격리된 Windows 환경에 해당 폴더의 내용을 모두 복사합니다.
...
#### 앱 패키징, 앱 서명, 패키지에서 유효성 검사 실행

이 예는 로컬 테스트를 위해 앱에 로그인한 다음, 데스크톱 브리지 및 Windows 스토어 요구 사항에 대한 앱의 유효성을 검사할 수 있는 방법을 보여준다는 점을 제외하고 첫 번째 경우와 비슷합니다.
...
||||
|-------------|-----------|-------------|
|<span id="setup-params" /> <strong>설치 매개 변수</strong>  ||
...
|<span id="conversion-params" /> <strong>변환 매개 변수</strong>|||
...
|<span id="identity-params" /><strong>패키지 ID 매개 변수</strong>||
|-PackageName &lt;String&gt; |필수 |유니버설 Windows 앱 패키지의 이름 |
|-Publisher &lt;String&gt; |필수 |유니버설 Windows 앱 패키지의 게시자 |
|-Version &lt;Version&gt; |필수 |유니버설 Windows 앱 패키지의 버전 번호 |
|<span id="manifest-params" /><strong>패키지 매니페스트 매개 변수</strong>||
...
|-AppId &lt;String&gt; |선택 사항 |응용 프로그램 ID를 Windows 앱 패키지 매니페스트로 설정하기 위한 값을 지정합니다. 값을 지정하지 않으면 *PackageName*에 대해 제공한 값으로 설정됩니다.|
...
|<span id="cleanup-params" /><strong>정리 매개 변수</strong>|||
...
|<span id="architecture-params" /><strong>패키지 아키텍처 매개 변수</strong>|||
...
|<span id="other-params" /><strong>기타 매개 변수</strong>|||
...
| - [&lt;SwitchParameter&gt;] 확인 |선택 사항 |스위치가 있을 경우, DAC를 통해 앱 패키지가 데스크톱 브리지 및 Windows 스토어의 요구 사항에 맞는지 확인합니다. 결과는 "VerifyReport.xml"이라는 유효성 검사 보고서로 출력되며, 브라우저에서 가장 잘 보입니다. 이 스위치는 `-MakeAppx` 스위치와 함께 있어야 합니다. |
...
3. **인증서 가져오기 마법사**에서 **로컬 컴퓨터**에 인증서를 설치하고 이를 **신뢰할 수 있는 사용자** 인증서 스토어에 배치합니다.

   ![신뢰할 수 있는 사용자 스토어](images/desktop-to-uwp/trusted-people-store.png)
...
변경을 한 후에는 변환기를 다시 실행할 필요가 없습니다. MakeAppx 도구와 DAC가 앱에 대해 생성한 appxmanifest.xml 파일을 사용해 앱을 다시 패키징할 수 있습니다. [Windows 앱 패키지 생성](desktop-to-uwp-manual-conversion.md#make-appx)을 참조하세요.
...
## <a name="known-issues-and-disclosures"></a>알려진 문제 및 공개 정보

### <a name="known-issues-and-workarounds"></a>알려진 문제 및 해결 방법

몇 가지 알려진 문제와 이를 해결할 수 있는 몇 가지 방법들이 여기 나와 있습니다.
