...
Windows 10, 버전 1803 (10.0; 빌드 17134) 이후, UWP 앱 다중 인스턴스를 지원 하도록에서 선택할 수 있습니다. 다중 인스턴스 UWP 앱의 인스턴스를 실행 중이며 후속 정품 인증을 요청하는 경우 플랫폼은 기존 인스턴스를 활성화하지 않습니다. 대신 다른 프로세스에서 실행 중인 인스턴스를 새로 만듭니다.
...
다중 인스턴스 UWP 앱 만들기에 대 한이 동영상을 시청 하 중인을 확인 합니다.
...
[**AppInstance.RecommendedInstance**](/uwp/api/windows.applicationmodel.appinstance.recommendedinstance) 속성이 있을 경우이 정품 인증 요청에 대 한 셸 제공 기본 인스턴스를 나타냅니다 (또는 `null` 경우 없는). 셸 기본 설정에서 제공 하는 경우 다음 리디렉션할 수 있습니다 수 활성화를 해당 인스턴스로 또는 선택 하는 경우 무시할 수 있습니다.
