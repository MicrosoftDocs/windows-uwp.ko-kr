...
고객은 소매점에서 경우 Pc 및 디바이스의 데모를 사용해 수를 기대 합니다. 종종 했느냐가 [소매 데모 환경 (RDX)](https://docs.microsoft.com/windows-hardware/customize/desktop/retail-demo-experience)를 통해 앱을 사용 하 여 수행해 시간.
...
고객의 관점에서 한 앱에만 있습니다. 두 가지 모드를 구별 하는 고객을 위해 권장 앱 정품 모드로 상태일가 표시 되는지 단어 "소매" 눈에 띄게 제목 표시줄 또는 적절 한 위치에.

앱에 대 한 Microsoft Store 요구 사항 외에도 RDX 인식 앱도와 호환 되어야 RDX 설치, 삭제 및 업데이트 프로세스는 소매점에서 고객이 일관 되 게 긍정적인 경험 있는지 확인 합니다.

## <a name="design-principles"></a>디자인 원칙

* **최고를 표시**합니다. 뛰어난 앱 소매 데모 환경을 사용 하 여. 이 처음으로 고객에 게 것일 하므로 최상의 조각을 표시 앱!
...
인식 RDX 앱은 소매 고객에 게 앱의 선보여 것 이므로, 기술 요구 사항을 충족 하 고 모든 소매 데모 환경 앱에 대 한 Microsoft Store에 있는 개인 정보 보호 규정을 준수 해야 있습니다.
...
* **익명 통신 없음**입니다. 소매 데모 장치를 사용 하는 고객은 익명의 사용자 이기 때문에 장치에서 메시지 콘텐츠를 공유할 수 해야 합니다.
...
정리 2 분 구매자 장치와 상호 작용을 중지 한 후 시작 됩니다. 소매 데모를 재생 하 고 Windows 연락처, 사진 및 다른 앱에서 샘플 데이터 초기화를 시작 합니다. 장치에 따라이 완벽 하 게 모든 normal로 다시 초기화 하는 데 1 ~ 5 분 사이 걸릴 수 있습니다. 이렇게 하면 모든 고객은 소매점에서 장치를 이용 하 고 동일한 환경의 경우 장치와 상호 작용 수 있습니다.
...
사용자 세션 간에 데이터를 저장, 기본 정리 프로세스에서이 폴더의 데이터를 자동으로 삭제 되지 __ApplicationData.Current.TemporaryFolder__ 에 정보를 저장할 수 있습니다. 참고 정보 *LocalState* 를 사용 하 여 저장 된 정리 프로세스 중 삭제 됩니다.
