...
ms.locfileid: "6276900"
---
# <a name="z-depth-and-shadow"></a>Z-깊이 및 그림자

![true 깊이](images/elevation-shadow/depth.svg)

3D 위치, 조명 등의 실제 개념을 사용 하 여 흐름 깊이 시스템 및 그림자 선사할 디지털 UI를 더 계층화 된 물리적 환경에서 감지 될 수 있는 합니다. Z-깊이 또는 상대 깊이 및 그림자는 UWP 앱에 깊이 통합 다음 두 가지가 있습니다.

## <a name="what-is-z-depth"></a>Z-깊이 란 무엇 인가요?

Z-깊이, z 축 따라 두 표면 사이의 거리 이며 뷰어에 얼마나 가까이 개체는 방법을 보여 줍니다.
...
![컨텍스트 메뉴에서 z 깊이](images/elevation-shadow/whyelevation.svg)

이외에 의미 있는 시각적 계층 구조 z 깊이 해줍니다 제공 만들 수 일관 된 환경을 2D에서 원활 하 게 모든 장치와 폼 팩터에 걸쳐 앱을 확장 하는 3D 환경에 합니다. 

![2d에서 3d의 z-depth](images/elevation-shadow/elevation-2d3d.svg)

### <a name="how-is-z-depth-perceived"></a>Z 깊이 최적이?

실제 세계에서 깊이 떨어지는 하는 방법에 따라, 근접 디지털 UI에 표시를 사용할 수 있는 여러 가지 기술을 다음은입니다.

- **배율** 멀리 개체는 동일한 크기의 가까운 개체 보다 작게 표시 합니다. 이 방법은 하므로 일반적으로 권장 되지 2D 공간에서 효과적으로 설명 하기 어렵습니다. 그러나 2d에서 사용자에 게 더 가깝게 이동 하는 개체의 유효 시뮬레이션을 만들려면 배율 및 [그림자](#what-is-shadow) 를 사용할 수 있습니다.

    ![배율로 근접 연결](images/elevation-shadow/elevation-scale.svg)

- **분위기** 개체는 멀리 내부 및 외부로 "연기가 찬" 오버레이 또는 다른 대기 효과 사용 하 여 초점 나타날 수 있습니다.

    ![분위기를 사용 하 여 근접 연결](images/elevation-shadow/elevation-atmosphere.svg)

- **동작** 상대 속도 근접 연결을 보여 주는 데 사용할 수 있습니다: 가까운 개체 배경 멀리 있는 개체 보다 빠르게 이동 합니다. 이 효과 구현 하는 방법을 알아보려면 [시차](../motion/parallax.md)를 참조 하세요.

    ![동작의 근접 연결](images/elevation-shadow/elevation-motion.svg)

### <a name="recommendations-for-z-depth"></a>Z 깊이 위한 권장 사항

명확한 시각적 포커스를 제공 하기 위해 높은 평면의 수를 줄입니다. 대부분의 시나리오에 대 한 두 개의 평면 만으로도: 포그라운드 항목 (높은 근접)이 고 다른 배경 항목 (낮은 근접)에 대 한 합니다. 여러 상승 된 항목을 겹치지 않는 경우 평면의 수를 줄이는 동일 평면 (즉, 전경) 그룹화 합니다.
...
그림자는 권한 상승 떨어지는 하는 방법입니다. 관리자 권한 개체 위에 빛이 있는 경우 아래 표면에 그림자가 않습니다. 클수록 개체, 더 큰 및 부드러운 그림자가 됩니다. 관리자 권한 개체, 그림자가 필요 없습니다 하지만 그림자 권한 상승을 표시 작업을 수행 합니다.

UWP 앱에서 그림자 체계적으로 하지 미학적 있어야 합니다. 그림자 강제로에서 초점과 생산성을 하는 경우 다음 그림자의 사용을 제한 합니다.

ThemeShadow 또는 DropShadow Api를 사용 하 여 그림자를 사용할 수 있습니다.

## <a name="themeshadow"></a>ThemeShadow

종류를 그리려는 모든 XAML 요소에 적용할 수 ThemeShadow에 따라 적절 하 게 x, y, z 좌표를 숨깁니다. ThemeShadow 다른 환경 사양에 대해서도 자동으로 조정합니다.
...
ThemeShadow [팝업](/uwp/api/windows.ui.xaml.controls.primitives.popup)모든 XAML 요소에 적용 될 때 그림자를 자동으로 캐스팅 합니다. 그림자 및 그 아래 다른 열려 팝업 뒤 앱 백그라운드 콘텐츠에 캐스트할 합니다.

ThemeShadow 팝업을 사용 하려면 사용 합니다 `Shadow` 속성을 XAML 요소는 ThemeShadow 적용 합니다. 그런 다음 권한 상승 요소가 다른 요소 뒤에서 예의 z 구성 요소를 사용 하 여는 `Translation` 속성입니다.
대부분의 팝업 UI에 대 한 앱 백그라운드 콘텐츠를 기준으로 권장 되는 기본 권한 상승은 32 유효 픽셀입니다.

이 예에서는 보여줍니다 앱 백그라운드 콘텐츠 및 그 뒤 다른 팝업 그림자 팝업에서 사각형.
...
2. 동일한 권한 상승에 여러 수신기 요소는 경우 대신 단일 부모 요소를 대상으로 하 여이 결합 하 여 하십시오.
...
- True 3d에서 HMDs, ThemeShadow를 사용 합니다. DropShadow 측에서, 부모가 시각적 개체에서 지정 된 오프셋 그립니다 이후 공간에서 부동 처럼 보일 합니다. 반면, ThemeShadow 수신기로 정의 된 시각적 개체 위에 렌더링 됩니다.
