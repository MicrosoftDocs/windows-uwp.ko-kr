...
이 예제에서는 명령 모음 플라이 아웃을 만들고 사전 및 사후 방식으로 사용 하는 방법을 보여 줍니다. 이미지는 터치 할 때 축소 모드에서 플라이 아웃이 표시 됩니다. 상황에 맞는 메뉴로 표시 될 때 해당 확장 모드로 플라이 아웃이 표시 됩니다. 두 경우 모두 사용자가 확장 하거나 플라이 아웃을 연 후 축소할 수 있습니다.
