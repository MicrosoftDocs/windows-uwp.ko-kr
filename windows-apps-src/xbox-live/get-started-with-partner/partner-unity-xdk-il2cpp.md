...
Unity 5.6f3 릴리스되면서 엔진에는 새 기능을 직접 게임 프로젝트에 포함 하 여 스크립트에서 직접 Windows 런타임 (WinRT) 구성 요소를 사용 하 여 개발자 포함 되어 합니다. 까지 5.6 개발자는 플러그 인, 또는 dll 게임 스크립트에서 Xbox Live SDK 등 모든 플랫폼 기능을 지원 하기 위해 필요 합니다. 이 새 프로젝션 계층 플러그인 요건을 제거 하 고 IL2CPP 스크립팅 백 엔드를 선택 하는 게임에만 지원 새롭고 간소화 된 워크플로 소개 합니다.
...
Unity 5.6 이상을 설치 하 고 Xbox One 편집기 확장 설치 해야 합니다.
...
**Xbox One 적절 한 시스템 라이브러리 7) 모두에 추가 됩니다 자동으로 프로젝트를 및 플랫폼 바이너리를 포함 하는 데 필요한 단계를 추가 하지 합니다.**
