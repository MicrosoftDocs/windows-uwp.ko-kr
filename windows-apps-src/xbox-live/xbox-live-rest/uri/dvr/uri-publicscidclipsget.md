...
쿼리는 서비스 구성 id (서비스 안내) 별로 최적화 됩니다. 필터 또는 아래에 나열 된 기본값이 아닌 정렬 순서 지정 더 상황에 따라 시간이 오래 걸릴 수를 반환 합니다. 이 동영상의 더 큰 집합에 대 한 더 분명 하 게 합니다. 쿼리 오름차순으로 정렬 지정할 수 없습니다.
 
한정자는 특정 컬렉션 ofpublic 클립을 다운로드 해야 합니다. 요청 사용자에 게 요청 된 서비스 안내에 액세스할 수 있어야, 그렇지 않으면 HTTP 403 반환 됩니다.
