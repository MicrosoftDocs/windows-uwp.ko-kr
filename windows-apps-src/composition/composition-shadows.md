...
ms.locfileid: "4740844"
---
# <a name="shadows-in-windows-ui"></a>Windows UI의 그림자

[DropShadow](/uwp/api/Windows.UI.Composition.DropShadow) 클래스에는 [SpriteVisual](/uwp/api/windows.ui.composition.spritevisual) 또는 [LayerVisual](/uwp/api/windows.ui.composition.layervisual) (하위 트리 시각적 개체)에 적용할 수 있는 구성 가능한 그림자를 만드는 방법을 제공 합니다. 시각적 계층의 개체에 대 한 일반적인 되 면 CompositionAnimations를 사용 하 여 모든 속성은 DropShadow의 애니메이션 수 있습니다.
...
- **기본 사용** -DropShadow 셰이프 기본적으로 CompositionDropShadowSourcePolicy에서 '기본' 모드에 의해 정의 됩니다. SpriteVisual, 기본 않는 사각형 마스크 제공 됩니다. LayerVisual에 대 한 기본값은을 사용 하 여 시각적 개체의 브러시의 알파 마스크를 상속 합니다.
- **마스크 설정** – 그림자에 대 한 불투명 마스크를 정의 하는 [마스크](/uwp/api/windows.ui.composition.dropshadow.mask) 속성을 설정할 수 있습니다.
...
시각적 개체의 내용과 일치 하 여 그림자를 원하는 경우에 그림자 마스크 속성에 대 한 시각적 개체의 브러시를 사용 하거나 콘텐츠에서 마스크를 자동으로 상속 하도록 그림자를 설정 합니다. LayerVisual를 사용 하 여, 그림자 기본적으로 마스크를 상속 합니다.
...
경우에 따라 시각적 요소의 콘텐츠 맞지 되도록 그림자를 구체화 하는 것이 좋습니다. 이 효과 얻기 위해 알파를 사용 하 여 브러시를 사용 하 여 마스크 속성을 명시적으로 설정 해야 합니다.

에 아래 예에서는 시각적 콘텐츠 및 그림자 마스크에 대해 하나씩 두 표면을-로드:
...
시각적 계층의 표준 그대로 컴퍼지션 애니메이션을 사용 하 여 DropShadow 속성 애니메이션할 수 있습니다. 아래 뿌리기 위 샘플에서 그림자 흐림 반경을 애니메이션 효과를 주는 코드를 수정 했습니다.
...
1. 컴퍼지션 샘플 갤러리의 [SamplesCommon](https://github.com/Microsoft/WindowsUIDevLabs/tree/master/SamplesCommon/SamplesCommon) 사용자 지정 CompositionShadow 컨트롤을 사용 합니다. 다음 사용에 대 한 예제를 참조 하세요.

## <a name="performance"></a>성능

시각적 계층에 효과를 만들려면 효율적이 고 사용 가능한 갖추어져 많은 최적화, 그림자 생성 어떤 옵션을 설정에 따라 상대적으로 비용이 많이 드는 작업 될 수 있습니다. 다음은 다양 한 유형의 그림자에 대 한 높은 수준 '비용'입니다. 참고는 특정 그림자 비용이 많이 드는 수도 있지만 경우에 따라서는 여전히 특정 시나리오에서 자주 사용 하 여 적절할 수 있습니다.
