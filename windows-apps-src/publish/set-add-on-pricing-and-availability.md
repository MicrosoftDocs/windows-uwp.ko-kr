...
-   **만 상위 제품 내에서 구매 가능**: 모든 고객이 앱 내에서 추가 기능 구매를 통해이 옵션을 선택 하지만 추가 기능이 앱의 스토어 목록에 표시 하거나 스토어에서 검색 되지 것입니다. 내부 테스트의 초기 기간과 같이 제품을 광범위하게 사용할 수 없는 경우에만 이 옵션을 사용합니다.
-   **취득 중지: 직접 링크가 있는 고객은 제품의 Store 목록을 확인할 수 있지만, 이전에 제품을 소유했거나 홍보 코드를 가지고 있거나 Windows 10 장치를 사용 중인 경우에는 다운로드만 가능합니다. 이 추가 기능은 상위 제품의 목록에 표시되지 않습니다.**: 이 옵션을 선택하면 앱 목록에 추가 기능이 표시되지 않고 신규 고객이 추가 기능을 구입할 수 없습니다. 그러나 **이 옵션은 Windows8.1 또는 이전 버전의 고객에 대 한 지원 되지 않습니다**. 이전에 게시 된 앱을 구매할 Windows8.1에서, 추가 기능 해당 고객을 구매할 수 있습니다. Windows8.1 또는 이전 버전의 고객에 게 추가 기능을 제공을 중지 하려면 추가 기능을 제공 하는 코드를 제거 하려면 앱을 업데이트 하는 앱에 대 한 새 제출을 게시 해야 합니다. 앱에는 Windows8.1 대상으로 하지 않는 경우에이 방법은 권장 또는 이전 버전 고객에 대 한 더 나은 환경을 제공 하지 수 없도록 선택한 추가 기능 하는 경우입니다.
