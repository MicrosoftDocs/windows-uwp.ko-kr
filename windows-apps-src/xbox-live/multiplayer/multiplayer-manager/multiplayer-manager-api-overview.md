...
ms.locfileid: "7555122"
---
# <a name="multiplayer-manager-api-overview"></a>멀티 플레이어 관리자 API 개요

이 페이지는 멀티 플레이어 관리자 API 및 게임에서 사용 되는 방식을는 광범위 한 개요를 설명 합니다. 가장 중요 한 클래스와 API에서 메서드를 호출합니다. 자세한 API 정보에 대 한 참조 설명서를 참조 하세요. 응용 프로그램에서 이러한 Api를 사용 하는 방법의 예제를 멀티 플레이어 게임 샘플을 참조 하세요.
...
있습니다 목록은 다음 이해 해야 하는 주요 클래스를 설명 합니다.
...
이 클래스는 멀티 플레이어 관리자와 상호 작용 하기 위한 기본 클래스입니다. 이므로 singleton 클래스를 생성 하 고 한 번 클래스를 초기화 해야 합니다.
...
| JoinGame() | join_game() | 제 3 자 매치 메이 킹 서비스를 통해 일반적으로 발견 된 고유한 세션 이름이 지정 된 기존 게임 세션을 결합 합니다. Xbox 사용자 Id는 게임의 일부로 목록에 전달할 수 있습니다.|
| FindMatch() | find_match() | 매치 메이 킹 Xbox Live를 사용 하 여를 찾아 게임에 참여 합니다. |
| LeaveGame() | leave_game() | 게임 에서만 유지 하 고 대기실으로 멤버 및 모든 로컬 멤버를 반환 합니다. |
| **특성** | | |
...
호출 하는 경우 `do_work()`, 마지막으로 호출 되므로 변경 내용을 세션을 나타내는 이벤트는 목록을 반환 하는 멀티 플레이어 관리자 `do_work()`. 이러한 이벤트 구성원 세션에 연결 같은 변경 사항을 포함, 구성원 세션 떠난 멤버 속성이 변경, 호스트 클라이언트 변경 되었습니다 등.
...
이 장치와 함께 재생 하고자 하는 친구 초대에 로컬인 사용자 관리에 사용 되는 영구 세션 합니다. 대기실 세션 멀티 플레이 작업을 수행할 멀티 플레이어 관리자에 대 한 순서로 멤버를 하나 이상 포함 해야 합니다. 호출 하 여 새 로비 세션을 처음 만들 수는 `add_local_user()` 메서드.
...
| RemoveLocalUser() | remove_local_user() | 대기실 및 게임 세션에서 지정된 된 구성원을 제거합니다. |
...
| SetProperties() / SetSynchronizedProperties() | set_properties() / set_synchronized_properties() | 대기실 세션에 대 한 사용자 지정 속성을 설정합니다. 속성은 JSON 문자열에 저장 됩니다. 속성 장치 간 공유를 동시에 여러 장치에서 업데이트 될 수 있습니다 동기화 된 버전의 메서드를 사용 합니다. |
...
| 멤버 | members() | 대기실 세션에 있는 구성원의 컬렉션입니다. |
...
| --- | --- | --- |
| **메서드** | | |
| SetProperties() / SetSynchronizedProperties() | set_properties() / set_synchronized_properties() | 게임 세션에 대 한 사용자 지정 속성을 설정합니다. 속성은 JSON 문자열에 저장 됩니다. 속성 장치 간 공유를 동시에 여러 장치에서 업데이트 될 수 있습니다 동기화 된 버전의 메서드를 사용 합니다. |
