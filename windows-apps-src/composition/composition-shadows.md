...
- **기본값을 사용 하 여** -DropShadow 셰이프 기본적으로 CompositionDropShadowSourcePolicy에서 '기본' 모드에 의해 정의 됩니다. SpriteVisual에 대 한 기본값은 마스크는 제공 하지 않는 한 사각형입니다. LayerVisual, 기본 시각적 개체의 브러시의 알파를 사용 하 여 마스크 상속를입니다.
...
시각적 개체의 내용과 일치 하 여 그림자를 원하는 경우 시각적 개체의 브러시를 사용 하 여 그림자 마스크 속성을 하거나 저장소 콘텐츠에서 마스크를 자동으로 상속 하도록 설정 합니다. LayerVisual를 사용 하 여, 그림자 기본적으로 마스크를 상속 합니다.
...
경우에 따라 시각적 개체의 콘텐츠를 일치 하지 않이 되도록 그림자를 구체화 하는 것이 좋습니다. 이 효과 얻기 위해 알파를 사용 하 여 브러시를 사용 하 여 마스크 속성을 명시적으로 설정 해야 합니다.
...
![마스크 원 그림자를 사용 하 여 연결 된 웹 이미지](images/ms-brand-web-masked-dropshadow.png)
...
시각적 계층에 여러 최적화 효율적이 고 사용할 수 있는 효과에 그림자 생성 설정 옵션에 따라 상대적으로 비용이 많이 드는 작업 될 수 있습니다. 다음은 다양 한 유형의 그림자에 대 한 높은 수준 '비용'입니다. 참고는 특정 그림자 비용이 많이 드는 수도 있지만 경우에 따라서는 여전히 특정 시나리오에서 자주 사용 하 여 적절할 수 있습니다.
