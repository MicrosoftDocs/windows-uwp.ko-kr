...
잉크를 [TextBox](https://docs.microsoft.com/en-us/uwp/api/windows.ui.xaml.controls.textbox), [RichEditBox](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.richeditbox), 같은 UWP 텍스트 컨트롤에서 지원 되는 텍스트 입력에 대 한 기본 제공 필기 보기를 사용자 지정 하 고 이러한 [AutoSuggestBox](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.autosuggestbox)등에서 파생 된 컨트롤.

## <a name="overview"></a>개요

XAML 텍스트 입력된 상자 [Windows Ink](../input/pen-and-stylus-interactions.md)를 사용한 펜 입력에 대 한 지원이 포함 되어 있습니다. 사용자가 Windows 펜을 사용 하는 텍스트 입력된 상자에 탭 하면, 텍스트 상자 필기 표면에는 별도 입력된 창을 열지로 변환 합니다.

사용자가 쓰기 어디서 나 텍스트 상자에 후보 창에 인식 결과가 표시 텍스트 인식 됩니다. 사용자는 결과를 터치하여 선택하거나 쓰기를 계속하여 제안된 후보 창을 수락할 수 있습니다. 리터럴(글자별) 인식 결과가 후보 창에 포함되어 있기 때문에 인식이 사전의 단어로 제한되지 않습니다. 사용자가 쓰기를 하면 자연스럽게 필기하는 느낌을 유지할 수 있도록 수락된 텍스트 입력이 스크립트 글꼴로 변환됩니다.
...
기본 제공 필기 보기는 기본적으로 활성화 됩니다.

응용 프로그램에서 해당 잉크-텍스트 기능을 이미 제공 또는 텍스트 입력된 환경을 서식 또는 특수 문자 (예: 탭) 필기를 통해 사용할 수 없는 일부 종류에 의존 하는 경우 필기 보기를 사용 하지 않도록 설정 하려고 합니다.
...
필기 보기의 내부 텍스트 컨트롤 위에 이며 사용자의 필기 기본 설정에 맞게 크기 조정 (장치-> **설정 참조 펜-> 및 Windows Ink 필기-> 텍스트 필드에 직접 쓸 때 글꼴 크기를-> **). 보기는 자동으로 텍스트 컨트롤 및 앱 내 위치를 기준으로 정렬 됩니다.
...
텍스트 제안을 팝업 최상위 잉크 목록을 최상위 후보 올바르지 않은 경우 사용자가 선택할 수 있는 인식 후보를 제공 하기 위해 기본적으로 사용 됩니다.

응용 프로그램에 이미 강력한을 제공 하는 경우 사용자 지정 인식 기능을 사용할 수 있습니다 [AreCandidatesEnabled](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview.arecandidatesenabled) 속성에 기본 제공 제안을 사용 하지 않도록 설정 하려면 다음 예제에 표시 된 대로.
...
사용자가 잉크 인식에 따라 텍스트 렌더링 될 때 사용 하 여 필기 기반 글꼴의 미리 정의 된 컬렉션에서 선택할 수 있습니다 (참조 **설정-장치 > 펜-> 및 Windows Ink 필기-> 필기를 사용 하는 경우 글꼴->**).
...
이 예제에서는 것 [TextBox](https://docs.microsoft.com/en-us/uwp/api/windows.ui.xaml.controls.textbox) 의 [TextChanged](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.textbox.textchanged) 이벤트를 수신 하 고 텍스트 변경의 HandwritingView (또는 기본 글꼴, 그렇지 않은 경우)에서 발생 하는 경우 사용자의 선택 된 글꼴을 적용 합니다.
...
2. 그런 다음 FindVisualChildByName 호출 하 여 FindInnerTextBox 함수에서 시각적 트리 (AutoSuggestBox 달러부터 시작)를 통해 반복 시작 하겠습니다.
...
여기서 받아쓰기 (StackPanel를 TextBox와 받아쓰기 단추를 배치 하 여 구현)를 지 원하는 TextBox를 만듭니다.
...
이 문제를 해결 하려면이 해야 맞출 수 있는 UI 요소에 [HandwritingView](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview) 의 PlacementTarget 속성을 설정 합니다.
...
이렇게 하려면 채워집니다 해야 하는 UI 요소의 너비 [HandwritingView](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview) 의 MaxWidth 속성을 바인딩합니다.
