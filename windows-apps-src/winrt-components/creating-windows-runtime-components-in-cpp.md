...
description: 이 항목에서는 C +를 사용 하는 방법을 보여줍니다. + / CX는 C#, Visual Basic, c + +, 또는 Javascript를 사용 하 여 작성 범용 Windows 응용 프로그램에서 호출할 수 있는 구성 요소를 Windows 런타임 구성 요소를 만듭니다.
...
ms.locfileid: "2888192"
---
# <a name="creating-windows-runtime-components-in-ccx"></a>C++/CX로 Windows 런타임 구성 요소 만들기
> [!NOTE]
> 이 항목은 C++/CX 응용 프로그램 유지에 도움을 주기 위해 작성되었습니다. 하지만 새로운 응용 프로그램에 대해 [C++/WinRT](../cpp-and-winrt-apis/intro-to-using-cpp-with-winrt.md)를 사용하는 것이 좋습니다. C++/WinRT는 Windows 런타임(WinRT) API용 최신 표준 C++17 언어 프로젝션으로서 헤더 파일 기반 라이브러리로 구현되며, 오늘날 Windows API에 대해 최고 수준의 액세스를 제공하도록 설계되었습니다. C +를 사용 하 여 Windows 런타임 구성 요소를 만드는 방법을 + / WinRT, 참조 [작성 이벤트 C + + / WinRT](../cpp-and-winrt-apis/author-events.md)합니다.

이 항목에서는 C +를 사용 하는 방법을 보여줍니다. + / CX는 C#, Visual Basic, c + +, 또는 Javascript를 사용 하 여 작성 범용 Windows 응용 프로그램에서 호출할 수 있는 구성 요소를 Windows 런타임 구성 요소를 만듭니다.

Windows 런타임 구성 요소를 구축 하기 위한 몇가지 이유가 있습니다.
...
일반적으로 코딩할 때 C + + / CX 구성 요소를 사용 하는 일반 c + + 라이브러리 및 기본 제공 형식 제외 하 고에서 추상 이진 인터페이스 (ABI) 경계를 사용 하는 코드에서 데이터를를 전달 하는 다른.winmd 패키지에는 위치입니다. Windows 런타임 형식 및 특수 구문을 사용 하 여, 해당 C + + / CX를 만들고 해당 형식을 조작 (영문)을 지원 합니다. 또한에서 프로그램 C + + / CX 코드, JavaScript, Visual Basic, c + + 또는 C#에서 사용자의 구성 요소에서 발생 하 고 처리 하는 수 있는 이벤트를 구현 하는 대리인 및 이벤트와 같은 형식을 사용 합니다. C +에 대 한 자세한 내용은 + / CX 구문을 참조 [Visual c + + 언어 참조 (C + + / CX)](https://msdn.microsoft.com/library/windows/apps/xaml/hh699871.aspx)합니다.
...
Windows 런타임 형식만 ABI 경계를 통해 전달할 수 있습니다. 구성 요소에 공용 메서드의 매개 변수나 반환 형식으로 std::wstring과 같은 형식이 있으면 컴파일러가 오류를 발생시킵니다. Visual c + + 구성 요소 확장 (C + + / CX)의 기본 제공 형식 int 및 double, 및 해당 typedef 해당 하는 int32 float64와 같은 일반적인 스칼라 하 고 포함 합니다. 자세한 내용은 [형식 시스템(C++/CX)](https://msdn.microsoft.com/library/windows/apps/hh755822.aspx)을 참조하세요.
...
## <a name="ccx-built-in-types-library-types-and-windows-runtime-types"></a>C + + / CX 기본 제공 형식, 라이브러리 유형 및 Windows 런타임 형식
...
Windows 런타임 구성 요소에는 여러 활성화 가능한 공용 클래스와 구성 요소에 내부적으로만 알려진 추가 클래스가 포함될 수 있습니다. 특성을 적용할 [WebHostHidden](https://msdn.microsoft.com/library/windows/apps/windows.foundation.metadata.webhosthiddenattribute.aspx) C + + / CX 형식을 JavaScript를 표시 하려면이 아닙니다.
...
사용자 정의 값 구조체는 ABI 별를 통과 하려면 C +에 정의 된 값 구조체와 동일한 멤버는 JavaScript 개체를 정의 + / CX 합니다. 수을 전달 하면 해당 개체를 인수로 C + + / CX 메서드는 개체는 암시적으로 변환 되도록 C + + / CX 유형입니다.
...
.NET 언어에서 방금는 C +에 정의 된 형식의 변수가 만들기 + / CX 구성 요소입니다.
...
C + + / CX 공용 ref 클래스 오버 로드 된 메서드를 포함할 수 있지만 JavaScript 오버 로드 된 메서드를 구분할 수 있는 기능을 제한 합니다. 예를 들어 다음 서명 간의 차이는 구분할 수 있습니다.
...
## <a name="net"></a>.NET
.NET 언어 인식 오버 로드 C + + / CX ref 클래스는.NET Framework 클래스에서와 마찬가지로 합니다.
...
C +에서 DateTime 값을 전달 하면 + / JavaScript CX, JavaScript 날짜 개체로 허용 하 고 long 형식의 날짜 문자열로 기본적으로 표시 됩니다.
...
때.NET 언어 전달 System.DateTime C + + / CX 구성 요소 메서드를 수락 하는 Windows::Foundation::DateTime로 합니다. 구성 요소가 Windows::Foundation::DateTime을 .NET Framework 메서드로 전달하면 Framework 메서드는 이 값을 DateTimeOffset으로 수락합니다.
...
.NET 언어 액세스 속성에서 네이티브 C + + /.NET Framework 개체에서와 마찬가지로 방금 CX 개체입니다.
...
## <a name="enums"></a>열거
Windows 런타임 열거형에서 C + + / CX 공용 클래스 열거;를 사용 하 여 선언 됩니다 표준 c + +에서 범위가 지정 된 열거와 같이 합니다.
...
간에 C + 전달 되는 열거형 값은 + / CX 및 정수로 JavaScript 합니다. C +과 같은 명명 된 값을 포함 하는 JavaScript 개체를 선택적으로 선언할 수 + /로 이어지는 CX 열거 하 고 사용 합니다.
...
C + 비동기 메서드를 구현 하려면 + / CX, ppltasks.h에 정의 된 [create\_async](https://msdn.microsoft.com/library/hh750102.aspx) 함수를 사용 합니다. 자세한 내용은 참조 하십시오. [만들기 (영문) 비동기 작업의 C + + / UWP 앱에 대 한 CX](https://msdn.microsoft.com/library/vstudio/hh750082.aspx)합니다. 예제를 보려면 [연습: C +에 기본 Windows 런타임 구성 요소 만들기 (영문) + / CX JavaScript 또는 C#에서 호출 하 고](walkthrough-creating-a-basic-windows-runtime-component-in-cpp-and-calling-it-from-javascript-or-csharp.md)합니다. .NET 언어 사용 (영문) C + + / CX 비동기 메서드는.NET Framework에 정의 된 모든 비동기 메서드를 것 처럼 합니다.
...
JavaScript 코드가 구성 요소의 public 속성 또는 메서드를 인식하지 못하는 경우 JavaScript에서 카멜식 대/소문자 표기를 사용하고 있는지 확인합니다. 예, LogCalc C+ + / JavaScript에서 logCalc로 CX 메서드를 참조 해야 합니다.

제거 하면 C + + / CX Windows 런타임 구성 요소 프로젝트에서 솔루션을 수동으로 JavaScript 프로젝트에서 프로젝트 참조를 제거 해야 합니다. 이렇게 하지 않으면 이후 디버그 또는 빌드 작업을 수행할 수 없습니다. 필요한 경우 DLL에 대한 어셈블리 참조를 추가할 수 있습니다.
