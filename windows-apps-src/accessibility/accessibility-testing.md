...
> [!NOTE]
> Techniques for WCAG 2.0 G18에 나열된 도구 중 일부는 Windows 스토어 앱에서 대화형으로 사용할 수 없습니다. 도구에서 수동으로 전경색 및 배경색 값을 입력하고, 앱 UI의 화면 캡처를 만든 다음 화면 캡처 이미지에서 대비 비율 도구를 실행하거나, 앱에서 해당 이미지를 로드할 때가 아니라 이미지 편집 프로그램에서 원본 비트맵 파일을 열 때 도구를 실행해야 할 수도 있습니다.
