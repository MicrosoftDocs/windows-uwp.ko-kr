...
In-process 작업에 out of process (OOP) 백그라운드 작업을 이식 하는 가장 간단한 방법은 제공 하는 것에 [IBackgroundTask.Run](https://docs.microsoft.com/uwp/api/windows.applicationmodel.background.ibackgroundtask.run?f=255&MSPPError=-2147217396) 메서드는 응용 프로그램 내에서 코드 및 시작에서 [OnBackgroundActivated ](/uwp/api/windows.ui.xaml.application.onbackgroundactivated). 여기에 설명 된 기술을; in process 백그라운드 태스크로 OOP 백그라운드 태스크에서 shim을 만드는 방법에 대 되지 에 대 한 재작성 (또는 이식) in process 버전으로는 OOP 버전입니다.
...
* 프로세스에서 백그라운드 작업 트리거를 지원 하지 않습니다.  [DeviceUseTrigger](https://docs.microsoft.com/uwp/api/windows.applicationmodel.background.deviceusetrigger?f=255&MSPPError=-2147217396)하십시오 [DeviceServicingTrigger](https://docs.microsoft.com/uwp/api/windows.applicationmodel.background.deviceservicingtrigger) 고 **IoTStartupTask**
