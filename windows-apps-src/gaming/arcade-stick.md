...
Xbox One 아케이드 스틱은 8 방향 디지털 조이스틱, 6 **실행** 단추 (아래 이미지에서 A1 a 6으로 표시) 및 두 가지 **특별 한** 단추 (s 1과 S2로 표시 됨); 설치 아날로그 컨트롤 또는 진동을 지원 하지 않는 디지털 전용 입력된 장치 일입니다. Xbox One 아케이드 스틱에 UI 탐색을 지 원하는 데 사용 되는 **보기** 및 **메뉴** 단추 장착 수도 있지만 게임 플레이 명령을 지원 하기 위해 아니며 조이스틱 버튼으로 즉시 액세스할 수 없습니다.

![아케이드 스틱 4 방향 조이스틱, 6 작업 단추 (a 1-a 6) 및 2 특별 한 단추 (s 1과 S2)](images/arcade-stick-1.png)
...
UI 탐색 컨트롤러로 아케이드 스틱은 탐색 명령의 [필요한](ui-navigation-controller.md#required-set) 조이스틱 및 **보기**, **메뉴**, **동작 1**및 **동작 2** 버튼 매핑
...
검색 및 추적 아케이드 게임 패드를 제외 하 고와 [게임 패드](https://docs.microsoft.com/uwp/api/Windows.Gaming.Input.Gamepad) 클래스 대신 [ArcadeStick][] 클래스를 사용 하 여 동일한 방식으로 작동을 스틱 합니다. 자세한 내용은 [게임 패드 및 진동](gamepad-and-vibration.md)을 참조하세요.
