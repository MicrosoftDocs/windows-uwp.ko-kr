...
새 [ApplicationTrigger](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Background.ApplicationTrigger)를 만듭니다. 아래 코드 조각 에서처럼 필드에서이 저장할 수 있습니다. 이 편의 위해 우리는 트리거를 알리기 위해 때 나중 새 인스턴스를 만들 필요가 없습니다. 하지만 트리거를 알리기 위해 모든 **ApplicationTrigger** 인스턴스를 사용할 수 있습니다.
...
작업을 실행 하는 경우 제어 하는 백그라운드 작업 조건을 만들 수 있습니다. 조건이 충족 될 때까지 실행에서 백그라운드 작업을 조건이 있습니다. 자세한 내용은 [백그라운드 작업 실행 조건 설정](set-conditions-for-running-a-background-task.md)참조 하세요.

이 예제에서는 한 번 트리거되는 조건 **InternetAvailable** 로 설정 되어만 작업이 실행 인터넷 액세스를 사용할 수 있습니다. 가능한 조건 목록은 [**SystemConditionType**](https://msdn.microsoft.com/library/windows/apps/br224835)을 참조하세요.
...
포그라운드 프로세스의 수명을 연장 하는 응용 프로그램 트리거를 사용 하 여를 고려 하는 경우 [확장 실행](run-minimized-with-extended-execution.md) 을 대신 사용 하는 것이 좋습니다. Application 트리거는 작업을 수행 하는 별도로 호스트 프로세스를 만들기 위한 설계 되었습니다. 다음 코드 조각은 out of process 백그라운드 트리거를 등록합니다.
...
또한 한 번에 하나만 AppTrigger 실행할 수 있습니다. 다른 이미 실행 되는 동안에 AppTrigger 실행 하려고 하면 [CurrentlyRunning](https://docs.microsoft.com/uwp/api/windows.applicationmodel.background.applicationtriggerresult)반환 합니다.
