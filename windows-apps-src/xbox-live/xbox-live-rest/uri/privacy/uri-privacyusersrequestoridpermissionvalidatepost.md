...
요청 본문에는 사용자 목록 및 설정의 목록 및 결과 각 사용자/설정 쌍에 대 한 차단 허용 되는 결과입니다.

네트워크 간 멀티 플레이어 시나리오 (여기서 개인 정보 보호 통신 검사 수행 해야 Xbox 사용자 ID (XUID)이 있는 사용자와 하지 않는 네트워크 외부 사용자 간의)에서 사용자가 입력할 [PermissionCheckBatchRequest (JSON)를](../../json/json-permissioncheckbatchrequest.md) 참조 하세요.
