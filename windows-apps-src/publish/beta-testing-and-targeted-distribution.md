...
**개인 대상 그룹** 옵션 수만 사용할 수 공용 대상 그룹에 앱 이미 게시 되지 않은 경우. 모든 운영 체제 버전을 대상으로 하는 앱이이 옵션을 사용할 수 있지만 대화 테스터 (Xbox 하나)를 포함 1607 이상 버전, Windows 10을 실행 해야 하 고 사용자가 제공 하는 전자 메일 주소와 연결 된 Microsoft 계정을 사용 하 여 서명 되어야 합니다.
...
이 옵션은 때 다른 사용자 저장소에 앱을 검색 하지 않도록만 특정 그룹으로 테스터, 응용 프로그램 배포를 제한 하는 다른 방법은 제공 (또는 프로 모션 코드 없이 얻기). 그러나 개인 대상 옵션과 달리, 직접 링크가 있는 경우에는 누구라도 앱 목록을 볼 수 있습니다. 제출에 있어 기밀성이 중요한 경우에는 대신에 개인 대상에 게시하는 것이 좋습니다.
