...
**개인 대상** 옵션 수만 사용할 수 공개 대상 앱 아직 게시 하지 않은 경우. 모든 운영 체제 버전을 대상으로 하는 응용 프로그램을 사용 하 여이 옵션을 사용할 수 있지만 프로그램 테스터 Windows 10, 1607 (포함 Xbox 1) 이상 버전을 실행 해야 제공 하는 전자 메일 주소와 연결 된 Microsoft 계정을 사용 하 여 서명 되어야 합니다.
...
이 옵션은 때 다른 사람이 저장소에 응용 프로그램을 검색 하지 않도록만 특정 그룹에 테스터는 응용 프로그램의 배포를 제한할 수 있습니다 (또는 프로 모션 코드 없이 획득). 그러나 개인 대상 옵션과 달리, 직접 링크가 있는 경우에는 누구라도 앱 목록을 볼 수 있습니다. 제출에 있어 기밀성이 중요한 경우에는 대신에 개인 대상에 게시하는 것이 좋습니다.
