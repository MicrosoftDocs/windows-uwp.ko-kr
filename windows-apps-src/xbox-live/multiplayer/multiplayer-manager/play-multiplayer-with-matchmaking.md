...
ms.locfileid: "4339874"
---
# <a name="find-a-multiplayer-game-by-using-smartmatch"></a>멀티 플레이 게임 스마트를 사용 하 여 찾기

경우에 따라 게이머는 없을 수 있습니다 충분 한 친구 온라인 게임을 플레이 하려는 하거나 온라인 사람이 플레이 하 려 합니다. Xbox Live 스마트 서비스를 사용 하 여 Xbox Live 다른 플레이어를 찾으려면
...
여기에 프로세스의 순서도 볼 수: [순서도-스마트 매치를 사용 하 여 멀티 플레이어 게임을 플레이](mpm-flowcharts/mpm-play-with-smartmatch-matchmaking.md)합니다.
...
로비 세션 개체 (구성 서비스 구성에서)는 유효한 세션 템플릿 이름 지정 한다고 가정 하는 멀티 플레이어 관리자 초기화 시 자동으로 만들어집니다. Note는 만들어지지 않습니다 로비 세션 인스턴스를 서비스 합니다.
...
멀티 플레이어 관리자 발생 한 `user_added` 이벤트는 사용자가 로비 세션에 추가 될 때마다 합니다. 것이 좋습니다 해당 사용자 성공적으로 추가 된 이벤트의 오류 코드를 검사할 수 있습니다. 오류가 발생 한 경우 오류가 발생 하는 이유를 자세히 설명 오류 메시지가 제공 됩니다.
...
초대, 후, 허용 된 경우 호스트는 게임을 시작할 준비가 데 사용할 수 스마트 충분 한 플레이어를 사용 하는 기존 게임 슬롯의 모든 구성원에 대 한 로비 세션에서 호출 하 여 두 찾기 `find_match()`, 또는 새 게임 세션 만들기  호출 하 여 동일한 게임 유형의 일치를 찾고 다른 사람과 모든 멤버 열기 지점을를 채우기 및 로비 세션에서이 포함 된 `join_game_from_lobby()` 아래에 `set_auto_fill_members_during_matchmaking()`.
