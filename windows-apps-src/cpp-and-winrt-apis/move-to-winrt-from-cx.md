...
이 항목에서는의 코드를 이식 하는 방법을 보여 줍니다.는 [ C++/CX](/cpp/cppcx/visual-c-language-reference-c-cx) 에 해당 하는 프로젝트 [ C++/WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt).

## <a name="porting-strategies"></a>포팅 전략

점차적으로 이식 하려는 경우에 C++/CX 코드를 C++/WinRT을 수 있습니다. C++/CX 및 C++/WinRT 코드 XAML 컴파일러 지원 및 Windows 런타임 구성 요소를 제외 하 고 동일한 프로젝트에서 공존할 수 있습니다. 대상으로 해야 이러한 두 가지 예외에 대 한 C++/CX 또는 C++동일한 프로젝트 내의 /WinRT 합니다.

> [!IMPORTANT]
> 프로젝트는 XAML 응용 프로그램을 빌드 경우 권장 되는 하나의 워크플로 중 하나를 사용 하 여 Visual Studio에서 새 프로젝트를 만들어야 하는 것은 C++/WinRT 프로젝트 템플릿 (참조 [Visual Studio 지원에 대 한 C++/WinRT](intro-to-using-cpp-with-winrt.md#visual-studio-support-for-cwinrt-xaml-the-vsix-extension-and-the-nuget-package)). 그런 다음 C +에서 소스 코드와 태그를 통해 복사를 시작 + CX 프로젝트입니다. 새 XAML 페이지를 추가할 수 있습니다 **프로젝트** \> **새 항목 추가...** \> **Visual C++**   >  **빈 페이지 (C++/WinRT)** .
>
> 요소 코드에서 XAML C + Windows 런타임 구성 요소를 사용할 수는 또는 + CX 것 처럼 프로젝트입니다. 만큼 이동 하거나 C++구성 요소를 수 있습니다 하 고 변경한 다음 XAML 프로젝트 /CX 코드 C++/WinRT 합니다. 다른 XAML 프로젝트를 유지 하거나 C++/CX, 새 C++/WinRT 구성 요소를 이식 시작 하 고 C++/CX 코드 구성 요소 및 XAML 프로젝트에서. 지정할 수도 있습니다는 C++와 함께 /CX 구성 요소 프로젝트를 C++WinRT 구성 요소 프로젝트를 동일한 솔루션 내에서 둘 다 응용 프로그램 프로젝트에서 참조 하 고 다른 하나에서 점진적으로 포트입니다. 참조 [간의 상호 운용성 C++/WinRT 및 C++/CX](interop-winrt-cx.md) 두 언어는 예측을 사용 하 여 동일한 프로젝트에 대 한 자세한 내용은 합니다.
...
위에서 언급 한 예외를 염두에 첫 번째 단계 이식는 C++/CX 프로젝트를 C++WinRT 수동으로 추가 하는 C++를 /WinRT 지원 (참조 [Visual Studio 지원에 대 한 C++/WinRT](intro-to-using-cpp-with-winrt.md#visual-studio-support-for-cwinrt-xaml-the-vsix-extension-and-the-nuget-package)). 이 작업을 수행 하려면 설치 합니다 [Microsoft.Windows.CppWinRT NuGet 패키지](https://www.nuget.org/packages/Microsoft.Windows.CppWinRT/) 프로젝트로 합니다. Visual Studio에서 프로젝트를 열고 클릭 **프로젝트** \> **NuGet 패키지 관리...** \> **찾아보기**를 입력 하거나 붙여 넣습니다 **Microsoft.Windows.CppWinRT** 검색 상자에서 검색 결과에서 항목을 선택 하 고 클릭 **설치** 해당 프로젝트에 대 한 패키지를 설치 합니다. 해당 변경의 효과 중 하나는 프로젝트에서 C++/CX에 대한 지원이 꺼진다는 것입니다. 종속성의 모든 빌드 메시지 도움이 찾기 (및 포트)을 해제 하는 지원을 유지 하는 것이 좋습니다 C++CX를 만들거나 다시 켤 수 지원 / (프로젝트 속성에서 **C /C++**  \>  **일반적인** \> **Windows 런타임 확장 사용** \> **예 (/ZW)** )를 점진적으로 포트입니다.
...
작성 하는 경우 C++전달 /CX 소스 코드 C++/CX hat 함수 매개 변수 형식 (\^) 참조 합니다.
...
작성 하는 경우 C++/CX 소스 코드를 hat를 사용 하 여 (\^) 화살표 및 Windows 런타임 개체 참조 변수 (-&gt;) hat 변수 역참조 연산자.
...
해당 하는 C +를 이식 하는 경우 + WinRT 코드 따라서 hats를 제거 하 고 화살표 연산자를 변경 하 여 가져올 수 있습니다 (-&gt;) 점 연산자 (.). C++/ WinRT 형식을 예상 값 및 포인터가 아닌 됩니다.
...
에 대 한 기본 생성자는 C++/CX hat 포인터를 null로 초기화 합니다. 다음은 C++/CX 코드 예제에서는 정확한 형식에 초기화 되지 않은 것의 변수/필드를 만들겠습니다. 즉,를 처음 참조 하지 않습니다는 **TextBlock**; 나중에 대 한 참조를 할당 하려고 합니다.
...
해당 C++/WinRT를 참조 하세요 [지연 된 초기화](consume-apis.md#delayed-initialization)합니다.
...
작업할는 C++는 hat 라고 것에 대 한 핸들을 통해 /CX 개체 (\^) 참조 합니다. `ref new` 키워드를 통해 새로운 개체를 만들면 [**RoActivateInstance**](https://docs.microsoft.com/windows/desktop/api/roapi/nf-roapi-roactivateinstance)를 호출하여 런타임 클래스의 새로운 인스턴스를 활성화합니다.
...
것이 일반적으로 참조 기본 파생 형식의 개체를 참조 하는 것이 알고 있는 있습니다. C++사용, CX `dynamic_cast` 를 *캐스트* 는 참조 기본에는 참조에서 파생 합니다. 합니다 `dynamic_cast` 실제로 숨겨진 호출 [ **QueryInterface**](https://docs.microsoft.com/windows/desktop/api/unknwn/nf-unknwn-iunknown-queryinterface(q_))합니다. 다음은 일반적인 예제&mdash;종속성 속성 변경 이벤트를 처리 하 고에서 캐스팅 하려는 **DependencyObject** 종속성 속성을 소유 하는 실제 형식입니다.
...
에 해당 하는 C++WinRT 코드는 대체/합니다 `dynamic_cast` 를 호출 하 여는 [ **IUnknown::try_as** ](/uwp/cpp-ref-for-winrt/windows-foundation-iunknown#iunknowntry_as-function) 함수를 캡슐화 하는 **QueryInterface**. 호출 하는 옵션도 있습니다 [ **IUnknown::as**](/uwp/cpp-ref-for-winrt/windows-foundation-iunknown#iunknownas-function)를 대신 하는 경우 예외를 throw 필요한 인터페이스 (요청 하는 형식의 기본 인터페이스)에 대 한 쿼리는 반환 되지 않습니다. 다음은 C++/WinRT 코드 예제입니다.
...
합니다 **platform:: agile\^**  입력 C++/CX 모든 스레드에서 액세스할 수 있는 Windows 런타임 클래스를 나타냅니다. C++/WinRT 표현은 [ **winrt::agile_ref**](/uwp/cpp-ref-for-winrt/agile-ref)합니다.
...
합니다 **platform:: exception\^**  에 생성 되는 형식 C++Windows 런타임 API가 아닌 S 돌아오면 /CX\_확인 HRESULT입니다. C++/WinRT에 대한 등가는 [**winrt::hresult_error**](/uwp/cpp-ref-for-winrt/error-handling/hresult-error)입니다.

포트를 C++/WinRT를 사용 하는 모든 코드 변경 **platform:: exception\^**  사용 하도록 **winrt::hresult_error**.
...
각 클래스(**hresult_error** 기본 클래스를 통해)는 오류의 HRESULT를 반환하는 [**to_abi**](/uwp/cpp-ref-for-winrt/error-handling/hresult-error#hresult_errorto_abi-function) 함수와 해당 HRESULT의 문자열 표현을 반환하는 [**메시지**](/uwp/cpp-ref-for-winrt/error-handling/hresult-error#hresult_errormessage-function) 함수를 제공합니다.
...
사용 하 여 C++에 액세스할 수 있습니다, CX/합니다 [ **Platform::String::Data** ](https://docs.microsoft.com/en-us/cpp/cppcx/platform-string-class#data) C 스타일 문자열을 검색할 속성 **const wchar_t\***  (예를 들어, 배열 에 전달 **std::wcout**).
...
사용 하거나 문자열을 반환 하는 Api를 구현 하는 데 있어 일반적으로 변경한 C++/CX 코드를 사용 하는 **platform:: string\^**  사용 하도록 **winrt::hstring** 대신 합니다.
...
C++/CX 제공 합니다 [object:: tostring](/cpp/cppcx/platform-object-class?view=vs-2017#tostring) 메서드.
...
C++/ WinRT 직접이 기능을 제공 하지 않지만 대안을 받을 수 있습니다.
...
* [이벤트를 작성 C++/WinRT](author-events.md)
* [동시성 및 비동기 작업을 C++/WinRT](concurrency.md)
