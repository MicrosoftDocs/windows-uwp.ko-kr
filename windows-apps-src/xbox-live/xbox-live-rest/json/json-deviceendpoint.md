...
| endpointUri| 문자열| 필수. (Windows 또는 Windows Phone) 클라이언트 플랫폼에 푸시 알림 서비스 (WNS 또는 MPNS)에서 가져온의 URL입니다.| 
| locale| 문자열| 필수. 원하는 언어가이 끝점에 전송 되는 알림입니다. 쉼표로 구분 된 값 우선 순위에 목록이 될 수 있습니다. 예: "DE-DE, EN-US, en"입니다.| 
...
| systemId| GUID| 필수. "앱"인스턴스에 대 한 고유 식별자 (디바이스/사용자 조합). 모범 사례 구현 시 설치/첫 실행, 임의의 GUID를 생성 하는 응용 프로그램에 대 한 이며 앱의 후속 실행에서 해당 값을 사용 하 여 계속 합니다.| 
| titleId| 32 비트 부호 없는 정수| 필수. 서비스에 대 한 호출을 실행 하는 게임의 타이틀 ID입니다.| 
