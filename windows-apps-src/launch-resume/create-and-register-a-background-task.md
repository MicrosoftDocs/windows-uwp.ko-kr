...
> [!NOTE]
> 백그라운드 작업 클래스 자체&mdash;및 백그라운드 작업 프로젝트의 다른 모든 클래스가&mdash; **봉인** (또는 **마지막**)는 **공용** 클래스 해야 합니다.
...
> [!NOTE]
> C#에서는 **async/await** 키워드를 사용하여 백그라운드 작업의 비동기 메서드를 호출할 수 있습니다. C + + /CX 작업 체인을 사용 하 여 비슷한 결과 얻을 수 있습니다.
...
> [!NOTE]
> 백그라운드 작업을 등록 하는 데만 사용 되는 함수를 만들 수도 있습니다&mdash; [백그라운드 작업 등록](register-a-background-task.md)을 참조 하세요. 이 경우 다음 세 단계를 사용 하는 대신 간단히 트리거를 생성을 작업 이름, 작업 진입점 및 조건 (선택 사항)과 함께 등록 함수를 제공 합니다.
...
다음 예제에서는 **AllTasks** 속성 반복 하 고 작업이 이미 등록 하는 경우 true 플래그 변수를 설정 합니다.
