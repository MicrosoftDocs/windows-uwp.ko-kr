...
ms.locfileid: "4611889"
---
# <a name="post-usersxuidxuidresetreputation"></a>POST (/users/xuid({xuid})/resetreputation)
있습니다 적용 하는 계정 하이재킹 (예를 들어) 후 지정 된 사용자의 평판 점수 일부 임의의 값으로 설정 됩니다. 이러한 Uri에 대 한 도메인은 `reputation.xboxlive.com`.
...
이 메서드 정품을 제외한 모든 샌드박스에 파트너가 및 테스트 목적으로 일반 정품을 제외 하 고 모든 샌드박스에서 사용자가 호출할 수 있습니다. Note이 요청 사용자의 "기본" 평판 점수를 설정 하 고 자신의 양수 피드백 weightings은 모두 0으로 설정 하세요. 이러한 기본 점수와 그 특사로 보너스, 자신의 워 보너스가 호출 후 사용자의 실제 평판이 됩니다.
...
파트너 로부터:에 대 한 정품 샌드박스를 적용 팀;의 **PartnerClaim** 모든 다른 샌드박스에, **PartnerClaim**합니다.
 
사용자의: 정품, **XuidClaim** 및 **TitleClaim**를 제외 하 고 모든 샌드박스에 합니다.
