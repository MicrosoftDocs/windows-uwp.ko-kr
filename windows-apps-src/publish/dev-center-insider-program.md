...
|**비정상 세부 정보 한눈에 보기**: 구입, 사용 및 상태 데이터의 중요한 변경 사항을 빠르게 살펴볼 수 있습니다. | 일주일 동안 이전 주에 비해 비정상(큰 증가 또는 감소)이 감지되면 한 주가 끝날 때 **구입**, **사용**, 및 **상태** 보고서에 원형 마커가 표시됩니다. 변경된 내용에 대한 자세한 내용을 보려면 원 위로 마우스를 가져갑니다. | 2018년 1월 |
|**알림 기본 설정 사용자 지정**: 개발자 센터 계정에 대해 수신하는 알림을 선택하고 이메일을 통해, 대시보드에서, 또는 둘 다에서 받을 것인지를 지정합니다. | 대시보드의 위쪽에 있는 아이콘을 사용하여 **메시지 센터**로 이동한 다음 **설정**을 선택합니다. 알림 기본 설정 섹션에서 선택합니다. 메일 알림을 보낼 따 사용할 주소를 지정할 수 있습니다.  | 2017년 11월 |
...
|**근 실시간 상태 데이터**: 이제 **72H** 페이지 필터 옵션을 사용하는 경우 **상태** 보고서에는 거의 실시간으로 충돌, 중단, 메모리 오류 및 JavaScript 예외에 대한 데이터가 표시됩니다.  | 앱의 상태 보고서에서 페이지 상단의 **72H** 옵션을 선택합니다. **72H** 및 **30D** 필터를 비교할 경우 72시간 동안 데이터에 불일치가 발생할 수 있습니다. 이는 알려진 문제로, 해결하기 위해 노력하고 있습니다.  | 2017년 9월|
