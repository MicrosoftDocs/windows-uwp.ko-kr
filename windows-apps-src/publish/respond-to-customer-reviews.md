...
> [!IMPORTANT]
> 리뷰 응답을 받지 않겠다고 선택한 고객이 작성한 리뷰에는 직접 응답할 수 없습니다. 또한 미국에 거주하지 않는 고객이 Windows Phone 8 장치를 사용하여 작성한 리뷰에도 응답을 남길 수 없습니다.

앱 리뷰를 보고 응답을 제공하려면 Windows 개발자 센터 대시보드에서 해당 앱을 찾습니다. 왼쪽 탐색 메뉴에서 **분석**을 확장하고 **리뷰**를 클릭하여 [리뷰 보고서](reviews-report.md)를 표시합니다. **리뷰에 응답**을 선택하여 응답을 제공합니다.

> [!TIP]
> 대시보드를 사용하여 리뷰에 응답하는 것 외에도 [프로그래밍 방식으로](../monetize/submit-responses-to-app-reviews.md) 또는 [개발자 센터 앱](https://www.microsoft.com/store/apps/dev-center/9nblggh4r5ws)을 사용하여 리뷰에 응답할 수 있습니다. 
...
기본적으로 사용자의 응답은 사용자가 응답한 고객만 볼 수 있습니다. 다른 고객이 응답을 보도록 하려면 앱의 스토어 목록에서 고객 리뷰 바로 아래에 응답을 표시할 수 있도록 **이 응답을 공개로 설정** 확인란을 선택하세요. 공개되는 응답은 Windows 10 장치에서 스토어를 보는 모든 고객에게 보입니다.

> [!IMPORTANT] 
> 고객이 원래 리뷰를 수정하지 않는 한, 나중에 공개 응답을 변경할 수 없게 되므로 공개로 지정하기 전에 응답을 신중히 검토하세요. 고객이 원래 리뷰를 수정하면 응답은 제거됩니다. 그러면 원할 경우 **응답 업데이트**를 선택하여 수정된 리뷰에 새 응답을 제출할 수 있습니다.
...
> [!NOTE]
> 고객은 개발자의 부적절한 리뷰 응답을 Microsoft에 보고할 수 있습니다. 리뷰 응답을 받지 않도록 선택할 수도 있습니다. 
>
