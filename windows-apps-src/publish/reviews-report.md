...
- **리뷰 내용**: **등급 사용 하 여 내용을 검토 하**는 유일한 등급 검토 콘텐츠로 표시 됨을 의미 하는 기본 설정은입니다. **모든** 표시 모든 평점, 리뷰 텍스트를 포함 하지 않는를 선택할 수 있습니다. 참고 **평점 분석** 차트 선택에 관계 없이 모든 리뷰를 항상 표시 됩니다.
...
**콘텐츠를 검토** 페이지 필터에서 **사용 하 여 평가** 선택 하는 경우에이 차트 항상에 모든 리뷰를 note 합니다.
...
-   리뷰 작성자의 국가/지역
-   리뷰가 남겨졌을 때 고객 장치에 있는 앱의 패키지 버전. (이 정보는 온라인 제출 하거나 Windows8.1 이하의 고객이 제출한 리뷰에 대 한 사용할 수 있습니다.)
-   고객이 리뷰를 남길 때 사용했던 장치의 OS 버전
-   고객이 리뷰를 남길 때 사용했던 장치의 이름. (이 정보는 온라인 제출 하거나 Windows8.1 이하의 고객이 제출한 리뷰에 대 한 사용할 수 있습니다.)
...
검색 상자를 사용 하 여 특정 단어 또는 구 앱의 리뷰에서 검색할 수 있습니다. 다른 언어로 작성 된 리뷰만 원래 고객이 작성 된 텍스트를 검토 하는 참고 검색 됩니다. 리뷰 번역 된 텍스트 검색 되지 않습니다.
