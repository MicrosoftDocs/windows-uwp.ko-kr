...
ms.locfileid: "4567176"
---
# <a name="xbox-live-account-tool"></a>Xbox Live 계정 도구

## <a name="what-is-xbox-live-account-tool"></a>Xbox Live 계정 도구란 무엇 인가요?
Xbox Live 계정 도구는 제목 개발자가 게임 시나리오를 테스트 하는 것에 대 한 기존 개발자 계정을 설정할 수 있도록 설계 되었습니다. 예를 들어 Xbox Live 계정 도구를 사용 하 여 개발자 계정 게이머를 변경 하거나 신속 하 게 1000 팔로 워 계정의 친구 목록에 추가 수 있습니다.
...
다음을 수행할 수 있습니다.
  ...
  4. 개발자 사용자의 평판 변경 (및 원시 평판 시작 값이 즉시 표시)
...
관리 하 고 올바른 샌드박스를 지정 하려는 사용자의 자격 증명을 해야 합니다. 개발자 계정에 대 한 액세스를 샌드박스, 그렇지 않으면 로그인 실패할 수 있는지 확인 합니다. 도구는 개발자 계정의 샌드박스를 사용 하 여 염두에 두고 설계 되었습니다.

## <a name="can-i-use-a-retail-account-or-does-it-have-to-be-a-sandboxed-account"></a>소매 계정을 사용할 수 있나요 아니면 샌드박스가 적용 된 계정 수 있습니까?
소매 계정 관리를 만료할 Xbox Live 계정 도구를 사용할 수 있지만 일부 기능은 지원 됩니다. 예를 들어 소매 사용자의 평판을 변경할 수 없습니다.

## <a name="how-do-i-change-a-dev-users-gamertag"></a>개발자 사용자의 게이머를 변경 하려면 어떻게 해야 합니까?
...
이용 하려는 XUIDs 목록이 있는 경우 텍스트 파일에 복사 합니다. "수행" 탭을 찾아서 선택 "목록 가져오기" 파일을 선택 합니다. 목록 보기에는 XUIDs 채워야 합니다. "변경 내용 커밋" 클릭 사용자를 따라야 합니다.

## <a name="how-do-i-block-someone"></a>사용자가 차단지 않습니다는 방법
"사람" 탭으로 이동 차단 하려는 사용자를 선택 합니다. "차단" 단추를 누르고 일괄 처리에서 차단 될 수 있습니다. 오류가 발생 하면 단순히 나중에 다시 시도 합니다.

## <a name="how-do-i-change-my-dev-accounts-repuation"></a>내 개발자 계정 신뢰도 변경 하려면 어떻게 해야 하나요?
이동 "신뢰도" 탭 기본값을 하 고 요청을 제출 하려면 "변경 내용 커밋" 단추를 눌러 선택 합니다. 요청이 성공 하면 값이 변경 평판 상태를 볼 수 있습니다.

## <a name="what-do-the-values-in-the-reputation-tab-mean"></a>값의 평판 탭에는 무엇을 의미 합니까?
평판 세 개의 하위 평판에서 계산 되는 전체: Fairplay (멀티 플레이어 사항) 사용자가 생성 (비디오 클립 등), 콘텐츠 및 통신 (메시지 및 음성). 각 범주에 대 한 원시 값 까지입니다 0도에서 75, 사용자의 신뢰도 높은 것을 의미 하는 더 나은. OverallStatIsBad는 사용자에 게 "방지 Me" 신뢰도 알려 줍니다.

## <a name="whats-the-black-area-at-the-bottom"></a>맨 아래에 검은색 영역 란?
일을 유지 하기 위해 것 유용한 디버그 출력 UI에 인쇄 생각 합니다. 영역 어떤 도구는 최대 알려 되며 모든 오류를 인쇄에 실행 됩니다.

## <a name="wheres-my-gamerpic"></a>Gamerpic 내 어디 입니까?
여기서는 일부 개발자 계정에는 계정 생성 시 자동으로 생성 된 gamerpic 가져올 버그를 인식 합니다.
