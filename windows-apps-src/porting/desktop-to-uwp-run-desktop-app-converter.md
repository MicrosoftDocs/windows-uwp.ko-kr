...
``InstallerArguments`` 매개 변수는 선택적 매개 변수입니다. 그러나 Desktop App Converter를 무인된 모드에서 실행 설치 관리자가 필요 하므로 응용 프로그램에서 자동으로 실행할 수 자동 플래그가 필요한 경우이 방법을 사용 해야 할 수 있습니다. ``/S`` 플래그는 매우 일반적인 자동 플래그이지만, 설치 파일을 만드는 데 사용한 설치 관리자 기술에 따라 사용하는 플래그가 달라질 수 있습니다.
...
이 예제에서 사용 하 여는 ``Installer`` 매개 변수 응용 프로그램 파일의 루트 폴더를 가리키도록 합니다.
...
또 다른 방법은 인증서를 사용 하 여 응용 프로그램에 서명 하는 것입니다. 사용 하는 경우는 ```sign``` 매개 변수, Desktop App Converter를 위한 인증서를 생성 되며 다음이 사용 하 여 응용 프로그램에 서명 합니다. 해당 파일의 이름은 **auto-generated.cer**이며, 패키징된 앱의 루트 폴더에서 찾을 수 있습니다.
...
될 가능성이 하면 패키지 된 응용 프로그램 버그를 해결, 시각적 자산을 추가 또는 라이브 타일 같은 최신 기능을 사용 하 여 응용 프로그램 향상 변경할 것 수 있습니다.
...
다음 두 섹션에 대해 몇 가지 선택적 수정 하기 위한 패키지 된 응용 프로그램을 고려할 수 있습니다.
