...
기본적으로 PaneDisplayMode 자동으로 설정 됩니다. 자동 모드에서 탐색 보기 창이 좁은 LeftCompact를 LeftMinimal 사이 맞춰 조정 하 고 창을 더 넓은 높아질수록 다음 왼쪽 합니다. 자세한 내용은 [적응형 동작](#adaptive-behavior) 섹션을 참조 하세요.
...
이러한 이미지 창 머리글과 콘텐츠 영역의 _위쪽_ 또는 _왼쪽_ 탐색 하도록 구성 된 경우 컨트롤의 레이아웃을 보여 줍니다.
...
탐색 보기에 뒤로 단추는 창의 왼쪽 위 모서리에 배치 됩니다. 그러나는 자동으로 뒤로 탐색을 처리 하 고 백 스택에 콘텐츠를 추가 합니다. 뒤로 탐색 사용을 참조 합니다 [뒤로 탐색](#backwards-navigation) 섹션.
...
이미지 또는 로고와 같은 텍스트가 아닌 콘텐츠를 추가 하려면 [PaneHeader](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.PaneHeader) 속성에 추가 하 여 창의 헤더에서 모든 요소를 배치할 수 있습니다.

PaneTitle와 PaneHeader를 설정 하는 경우 콘텐츠 메뉴 단추에 가장 가까운 PaneTitle 사용 하 여 메뉴 단추 옆 가로로 누적 됩니다.
...
- A 아이콘 전용 왼쪽 탐색 창 (LeftCompact) 중간 창 너비에 (641px ~ 1007px).
...
- 작은 창 너비에 콘텐츠를 앱에 대 한 더 많은 공간을 원합니다.
- 아이콘이 있는 탐색 범주를 명확 하 게 나타낼 수 없습니다.

![왼쪽된 탐색 최소한의 적응 동작](images/adaptive-behavior-minimal.png)<br/>
_탐색 보기 "최소" 적응형 동작_

이 동작을 구성 하려면 CompactModeThresholdWidth 축소 하 고 창 너비를 설정 합니다. 여기에서는 640를 1007 기본값에서 변경 됩니다. 값이 충돌 하지 않는 되도록 ExpandedModeThresholdWidth 설정 해야 합니다.
...
자동으로 적응 동작을 비활성화 하려면 PaneDisplayMode 자동 이외의 값으로 설정 합니다. 여기서 설정 창 너비에 관계 없이 LeftMinimal, 메뉴 단추에만 표시 됩니다.
...
_디스플레이 모드_ 섹션의 앞에서 설명한 대로 창은 항상 확장, 항상 컴팩트 또는 항상 최소한의 위쪽에 항상 되도록 설정할 수 있습니다. 또한 앱 코드에서 디스플레이 모드를 직접 관리할 수도 있습니다. 이 예제는 다음 섹션에 표시 됩니다.

### <a name="top-to-left-navigation"></a>왼쪽된 탐색 위쪽

상단 탐색을 사용 하 여 앱에서 탐색 항목이 오버플로 메뉴로 창 너비 감소로 축소 합니다. 앱 창이 좁은 경우 LeftMinimal 탐색 항목 축소 오버플로 메뉴로 모든 수 있도록 하는 대신 위에서 PaneDisplayMode 전환 하는 더 나은 사용자 환경을 제공할 수 것입니다.

큰 창 크기와 작은에서 왼쪽된 탐색에서 상단 탐색을 사용 하는 것이 좋습니다 창 크기는 시기를 지정 합니다.

- 이 설정 앱에서 하나의 범주는 화면에 맞지 않는, 바람직한 수 있도록 왼쪽된 탐색으로 축소 되도록 함께 표시할 중요 최상위 수준 탐색 범주 동일 하 게 집합이 있습니다.
...
> [!TIP]
> AdaptiveTrigger.MinWindowWidth를 사용 하면 창이 지정된 된 최소 너비 보다 더 넓은 시각적 상태가 트리거됩니다. 즉, 기본 XAML 좁은 창을 정의 하 고는 VisualState 창을 더 넓은 가져올 때 적용 되는 수정 작업을 정의 합니다. 기본 PaneDisplayMode 탐색 보기가 자동으로 되어 있으므로 창 너비가 CompactModeThresholdWidth, 보다 작거나 같은 경우 LeftMinimal 탐색 사용 합니다. 창을 더 넓은 가져오면는 VisualState 기본값을 재정의 하 고 위쪽 탐색 사용 됩니다.
...
**ItemInvoked** 이 이미 선택 하는 경우에 사용자가 탐색 항목을 탭 할 때마다 발생 합니다. (항목 마우스, 키보드 또는 다른 입력을 사용 하 여 이와 동등한 작업으로 호출할 수 수도 있습니다. 자세한 내용은 참조 [입력 및 조작](../input/index.md)합니다.) ItemInvoked 처리기를 탐색 하는 경우 기본적으로 페이지 다시 로드할 수, 및 중복 된 항목 탐색 스택에 추가 됩니다. 항목이 호출 되 면을 탐색 하는 경우에 페이지를 다시 로드를 허용 하지 않습니다 하거나 페이지 다시 로드할 때 중복 된 항목 탐색 백 스택에에서 만든 하지는 확인 해야 합니다. (코드 예제 참조).
...
NavigationView에 기본 제공 뒤로 단추가 있습니다. 하지만 앞으로 탐색을 사용 하는 것이 하지 뒤로 탐색 자동으로 수행 합니다. 사용자가 뒤로 단추를 탭 하면 [BackRequested](/uwp/api/windows.ui.xaml.controls.navigationview.BackRequested) 이벤트는 발생 합니다. 뒤로 탐색을 수행 하려면이 이벤트를 처리 합니다. 자세한 내용과 코드 예제를 참조 하세요. [탐색 기록 및 뒤로 탐색](../basics/navigation-history-and-backwards-navigation.md).

최소 또는 컴팩트 모드 탐색 보기 창 플라이 아웃으로 열릴 합니다. 이 경우 뒤로 단추를 클릭 하면 창이 고 닫고 대신 **PaneClosing** 이벤트를 발생 시킵니다.

숨기 거 나 이러한 속성을 설정 하 여 뒤로 단추를 사용 하지 않도록 설정할 수 있습니다.

- [IsBackButtonVisible](/uwp/api/windows.ui.xaml.controls.navigationview.IsBackButtonVisible): 사용 하 여 표시 하 고 뒤로 단추를 숨깁니다. 이 속성 [navigationviewbackbuttonvisible 열거](/uwp/api/windows.ui.xaml.controls.navigationviewbackbuttonvisible) 열거형의 값을 사용 하 고 기본적으로 **자동** 으로 설정 됩니다. 단추를 축소 하면 레이아웃에 공백이 예약 된 합니다.
...
> [!IMPORTANT]
> 앱의 페이지에 대 한 정보는 [ValueTuple](https://docs.microsoft.com/dotnet/api/system.valuetuple)에 저장 됩니다. 이 구조체는 앱 프로젝트에 대 한 최소 버전 17763 이상 SDK 여야 필요 합니다. NavigationView의 WinUI 버전을 사용 하 여 이전 버전의 Windows 10을 대상으로 하는 경우 대신 [System.ValueTuple NuGet 패키지](https://www.nuget.org/packages/System.ValueTuple/) 를 사용할 수 있습니다.
