...
- 광원을 사용 하 여 개체 쌍으로 함께 이동 하도록 하는 기능 독립의 나머지 부분에서는 장면 Fluent [표시](/design/style/reveal.md) 강조와 같은 시나리오를 사용 하도록 설정 합니다.
...
조명 시각적 대상으로 지정할 때 ( [대상](/uwp/api/windows.ui.composition.compositionlight.targets) 목록에 추가), 시각적 및 모든 하위 항목의 인식 되며이 광원에 응답 합니다. 점 광원 방향의 애니메이션에 반응 트리와 아래의 모든 시각적 개체의 루트 점 조명은 소스 설정으로 단순할 수 있습니다.
...
속성 | 설명
--- | ---
**색상** | 광원의 [색](/uwp/api/windows.ui.color) 입니다. 조명 색상 값 [D3D](https://docs.microsoft.com/windows/uwp/graphics-concepts/light-properties) 확산, 주변광, 및 방출 되 고 색상을 정의 하는 Specular에 의해 정의 됩니다. 조명; 조명에 RGBA 값 사용 알파 색 구성 요소 사용 되지 않습니다.
...
- 조명 개체를 식별: 관련 시각적 개체에 빛을 대상으로 지정 합니다.
- [선택 사항] 개별 개체를 정의 조명에 반응: 조명 반사가 SpriteVisual 표시 하기 위한 사용자 지정 하는 EffectBrush 사용 하 여 SceneLightingEffect 사용 합니다. 반사 기본값은 광원의 CoordinateSpace의 자식의 조명을 지원합니다.  시각적 개체는 SceneLightingEffect와 그리고 해당 시각적에 대 한 기본 조명을 덮어씁니다.
