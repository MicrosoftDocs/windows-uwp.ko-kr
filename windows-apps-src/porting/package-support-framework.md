...
파일 시스템 액세스 오류가 의심 되는 경우 System32/SysWOW64 또는 패키지 파일 경로 아래에 있는 실패 한 이벤트를 검색 합니다. 너무 필터 여기서 도움이 수도 있습니다. 이 목록의 맨 아래에서 시작 하 고 위쪽으로 이동 합니다. 이 목록의 맨 아래에 표시 되는 오류 발생 한 가장 최근에 합니다. "액세스 거부" 같은 문자열을 포함 하는 오류 및 "경로/이름 찾을 수 없음", 대부분의 주의 기울이는 하 고 의심 스러운 표시 되지 않는 작업을 무시 합니다. [PSFSample](https://github.com/Microsoft/MSIX-PackageSupportFramework/blob/master/samples/PSFSample/) 에 두 가지 문제가 있습니다. 다음 그림에 표시 되는 목록에 이러한 문제를 확인할 수 있습니다.

![ProcMon Config.txt](images/desktop-to-uwp/procmon_config_txt.png)

이 이미지에 표시 되는 첫 번째 문제를 "C:\Windows\SysWOW64" 경로에 있는 "Config.txt" 파일에서 읽는 응용 프로그램이 실패 했습니다. 응용 프로그램은 해당 경로 직접 참조 하려는 가능성이 아닙니다. 상대 경로 사용 하 여 해당 파일에서 읽은 하려고 하 고 기본적으로 "System32/SysWOW64"은 응용 프로그램의 작업 디렉터리 될 가능성이 높습니다. 응용 프로그램 패키지의 위치 설정할 수는 현재 작업 디렉터리는 예상 원인 중 하나입니다. Appx 내에서 검색 하는 실행 파일과 동일한 디렉터리에 파일이 있는지 볼 수 있습니다.
...
MSIX 컨테이너에서 실행 되는 응용 프로그램에서 액세스할 수 없는 디렉터리의 데이터를 읽거나 쓰기 시도 리디렉션할 수 있으며 [파일 리디렉션 수정 하기 위해](https://github.com/Microsoft/MSIX-PackageSupportFramework/tree/develop/FileRedirectionShim) 사용할 수 있습니다.

예를 들어, 응용 프로그램 실행 파일 응용 프로그램와 동일한 디렉터리에 있는 로그 파일에 쓰는, [파일 리디렉션 수정 하기 위해](https://github.com/Microsoft/MSIX-PackageSupportFramework/tree/develop/FileRedirectionShim) 를 사용 하 여 로컬 앱 데이터 저장소 등의 다른 위치에 해당 로그 파일을 만들 수 있습니다.

### <a name="runtime-fixes-from-the-community"></a>커뮤니티에서 런타임 수정

커뮤니티 기여 하려면 [GitHub](https://github.com/Microsoft/MSIX-PackageSupportFramework/tree/develop) 페이지를 검토 해야 합니다. 다른 개발자가 유사한 문제를 해결 하 고 런타임 수정 공유한 가능 합니다.
...
.Msix (또는.appx) 파일을 이미 있는 경우 패키지에 대 한 준비 영역으로 지원할 수 있는 레이아웃 폴더에 콘텐츠 압축을 풉니다 수 있습니다.  이를 수행할 수는 **x64 VS 2017 용 네이티브 도구 명령 프롬프트**, 또는 실행 파일 검색 경로에 SDK bin 경로 사용 하 여 수동으로 합니다.
...
### <a name="get-the-package-support-framework-files"></a>지원 프레임 워크 패키지 파일 가져오기
...
파일 이름을 만들기 ``config.json``, 해당 파일을 패키지의 루트 폴더에 저장 합니다. 방금 교체 하는 실행 파일을 가리키도록 config.json 파일의 선언 된 응용 프로그램 ID를 수정 합니다. 프로세스 모니터를 사용 하 여 얻은 정보를 사용 하 여도 작업 디렉터리를 설정 외에 아니라 수 파일 리디렉션 수정 하기 위해 사용 하 여 패키지 상대 "PSFSampleApp" 디렉터리 아래에 있는.log 파일을 읽기/쓰기를 리디렉션하 합니다.
...
`applications`, `processes`, 및 `fixups` 키가 배열입니다. 즉, 응용 프로그램, 프로세스 및 DLL 수정 하기 위해 여러 개 지정 config.json 파일을 사용할 수 있습니다.
...
기본적으로 수정 추적 하기 위해 "예상"으로 간주할 수도 있는 오류를 필터링 합니다.  예를 들어, 응용 프로그램 무조건적으로 확인 하는 경우 이미 있는 결과 무시 하 고 않고 파일을 삭제 하려고 할 수 있습니다. 이 파일 시스템 기능에서 모든 오류를 수신 하도록 선택 하는 위의 예제에서는 있으므로 맬웨어 결과 일부 예기치 않은 오류 필터링 얻을 수 있습니다. 그 전에 Config.txt 파일에서 읽기를 시도 실패 하는 메시지 "파일 찾을 수 없음"에서 알고 있으므로이 야기 합니다. 이 자주 관찰 되 고 하지 일반적으로 예상 된 것으로 간주 되는 실패 합니다. 실제로 예기치 않은 오류를 필터링 하 고 다음 대체 모든 오류는 문제가 있는 여전히 식별할 수 없는 경우 시작 가능성이 가장 됩니다.

기본적으로 수정 추적 하기 위해 출력 연결 된 디버거를 전송 합니다. 이 예제에서는 디버거를 첨부할 생각 하 고 출력을 보려면 SysInternals에서 [DebugView](https://docs.microsoft.com/en-us/sysinternals/downloads/debugview) 프로그램을 대신 사용 됩니다. 앱을 실행 한 후 알 수 있습니다 동일한 오류 이전과 하는 것 가리킵니다 우리는 동일한 런타임 수정 합니다.
...
Visual Studio를 사용 하 여 런타임 수정 디버깅, 런타임 수정 확장 하거나 처음부터 새로 만들 수 있습니다. 성공 하려면 다음이 작업을 수행 해야 합니다.
...
| PSFLauncher | C + + 빈 프로젝트입니다. 이 프로젝트는 패키지 지원 프레임 워크의 런타임 배포 가능한 파일을 수집 하는 위치입니다. 실행 파일을 출력 합니다. 해당 실행 파일은 먼저 솔루션을 시작할 때 실행 됩니다. |
...
**솔루션 탐색기**패키징 프로젝트를 마우스 오른쪽 단추로 클릭 하 고 **편집**선택 프로젝트 파일의 맨 아래에 추가 하는이:
...
해당 프로젝트를 마우스 오른쪽 단추로 클릭 하 고 상황에 맞는 메뉴에 **Nuget 패키지 관리** 옵션을 선택 합니다. **패키지 원본** 옵션의 **모든** 또는 **nuget.org**설정 되어 있는지 확인 합니다.
...
디버그 하거나 기존 런타임 수정 확장 하려는 경우이 가이드의 [런타임 수정 찾기](#find) 섹션에 설명 된 지침을 사용 하 여 얻은 런타임 수정 파일을 추가 합니다.
...
**Open **일반** 설정 페이지에는 프로젝트에 대 한 속성 페이지의 대상 이름** 속성을 설정 ``PSFLauncher32`` 또는 ``PSFLauncher64`` 응용 프로그램의 아키텍처에 따라 합니다.
...
라는 파일 추가 ``config.json`` 패키징 프로젝트에 다음, 복사 및 파일에 다음과 같은 json 텍스트를 붙여 넣습니다. **콘텐츠**를 **패키지 동작** 속성을 설정 합니다.
...
| 수정 | 구성 | (선택 사항) 수정 하기 위해 DLL의 동작을 제어 합니다. 이 값의 정확한 형식으로 원하는 각 수정이 "blob" 해석할 수 수정 하 여 수정 하기 위해 별로 달라 집니다. |
...
Visual Studio에서 f5 키를 눌러 디버거를 시작 합니다.  먼저 시작 되는 대상 데스크톱 응용 프로그램을 시작 하는 PSF 시작 관리자 응용 프로그램입니다.  대상 데스크톱 응용 프로그램을 디버깅 하려면 수동으로 **디버그**선택 하 여 데스크톱 응용 프로그램 프로세스에 연결 해야 합니다->**프로세스에 연결**하 고 다음 응용 프로그램 프로세스를 선택 합니다. 기본 런타임 수정 DLL 사용 하 여.NET 응용 프로그램의 디버깅을 허용 하려면 코드와 네이티브 코드 형식 (혼합된 모드 디버깅)를 선택 합니다.  

했으므로이 설정한 후 데스크톱 응용 프로그램 코드와 런타임 수정 프로젝트에서 줄의 코드로 옆에 중단점을 설정할 수 있습니다. 응용 프로그램에 소스 코드 없으면 런타임 수정 프로젝트에서 줄의 코드로 옆에 있는 중단점을 설정할 수 있습니다.

>[!NOTE]
> Visual Studio는 가장 간단한 개발 하는 동안 몇 가지 제한은 디버깅,이 가이드의 뒷부분에서 적용할 수 있는 디버깅 기술 다른 설명 하겠습니다.
...
먼저, 응용 프로그램 MSIX 컨테이너에서 실행 되는 경우 호출이 실패 하는 함수를 식별 합니다. 그런 다음 런타임 관리자를 대신 호출 하 고 싶은 대체 함수를 만들 수 있습니다. 이렇게 하면 최신 런타임 환경 규칙을 준수 하는 동작을 사용 하 여 함수를 구현 하는 대체 수 있습니다.

Visual Studio에서 만든이 가이드 앞부분의 런타임 수정 프로젝트를 엽니다.

선언 합니다 ``FIXUP_DEFINE_EXPORTS`` 매크로 대 한 include 문을 추가 합니다 `fixup_framework.h` 각각의 위쪽에 합니다. 런타임 수정 기능을 추가 하려는 cpp 합니다.
...
함수의 동일한 서명이 있는 함수를 만듭니다 사용자가 수정 하려는 동작 합니다. 다음은 대체 하는 예제 함수는 `MessageBoxW` 함수.
...
호출 `DECLARE_FIXUP` 지도 `MessageBoxW` 함수를 새 대체 함수입니다. 응용 프로그램 호출 하려고 할 때의 `MessageBoxW` 함수를 호출 대체 함수 대신 합니다.

#### <a name="protect-against-recursive-calls-to-functions-in-runtime-fixes"></a>런타임 수정 함수를 호출 재귀적으로 으로부터 보호

필요에 따라 적용할 수 있습니다는 `reentrancy_guard` 런타임 수정 함수를 호출 재귀적으로 으로부터 보호 하는 함수를 입력 합니다.

대체 기능에 대 한 생성 수 예를 들어는 `CreateFile` 함수입니다. 구현을 호출할 수는 `CopyFile` 작동 하지만 구현은 `CopyFile` 함수를 호출할 수는 `CreateFile` 함수입니다. 이에 대 한 호출의 무한 반복 주기 발생할 수 있습니다는 `CreateFile` 함수입니다.
...
런타임 수정 구성 데이터를 추가 하려는 경우 추가할 것을 고려 하는 ``config.json``. 사용할 수 있는 방식으로 `FixupQueryCurrentDllConfig` 를 쉽게 해당 데이터를 구문 분석 합니다. 이 예제에서는 해당 구성 파일에서 문자열 및 부울 값을 구문 분석합니다.
...
첫째,는.msix에서 설치 하는 것이 아니라 패키지 레이아웃 폴더 경로에서 느슨한 파일 배포 하 여 응용 프로그램을 실행 F5 디버깅 /.appx 패키지입니다.  일반적으로 레이아웃 폴더가 설치 패키지 폴더와 같은 보안 제한을가지고 있지 않습니다. 결과적으로, 런타임 수정 적용 하기 전에 패키지 경로 액세스 거부 오류를 재현 불가능할 수도 있습니다 것입니다.

이 문제를 해결 하기 위해.msix 사용 하 여.appx 패키지 배포 보다 F5 느슨한 파일 배포 / 합니다.  .msix 만들려면.appx 패키지 파일을 / 위에서 설명한 대로 Windows SDK에서 [MakeMSIX](https://docs.microsoft.com/en-us/windows/desktop/appxpkg/make-appx-package--makeappx-exe-) 유틸리티를 사용 합니다. 또는에서 Visual Studio 내에서 응용 프로그램 프로젝트 노드를 마우스 오른쪽 단추로 클릭 하 고 **저장소**선택->**앱 패키지 만들기**.

또 다른 문제가 Visual Studio 디버거를 통해 시작 된 모든 하위 프로세스를 연결 하는 것에 대 한 기본 지원을 없는입니다.   이렇게 하면 논리를 실행 한 후 Visual Studio에서 수동으로 연결 되어 있어야 합니다 대상 응용 프로그램의 시작 경로 디버그 하기가 어렵습니다.

이 문제를 해결 하기 위해 하위 프로세스를 지 원하는 디버거를 사용 하 여 연결 합니다.  참고 아닌지 일반적으로 대상 응용 프로그램에 적시에 (JIT) 디버거를 첨부할 수 있습니다.  대부분의 JIT 기술 조합 ImageFileExecutionOptions 레지스트리 키를 통해 대상 앱 대신 디버거를 시작 하는 때문입니다.  이 방법을 detouring 메커니즘 PSFLauncher.exe FixupRuntime.dll 대상 앱에 삽입 하는 데 사용 합니다.  [Windows 용 디버깅 도구](https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/index)포함 되어 있으며 [Windows SDK](https://developer.microsoft.com/en-US/windows/downloads/windows-10-sdk)가져온 WinDbg 지원 하위 프로세스를 연결 합니다.  또한 지원 직접 [시작 하 고 UWP 앱을 디버깅](https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/debugging-a-uwp-app-using-windbg#span-idlaunchinganddebuggingauwpappspanspan-idlaunchinganddebuggingauwpappspanspan-idlaunchinganddebuggingauwpappspanlaunching-and-debugging-a-uwp-app)합니다.
...
>[!NOTE]
> [PLMDebug](https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/plmdebug) 디버거를 시작 시 응용 프로그램 연결할도 사용할 수 있으며 [Windows 용 디버깅 도구에](https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/index)에서 포함 되어 있습니다.  그러나는 이제 WinDbg에서 제공 하는 직접 지원 보다 사용 하기 더 복잡 합니다.
