...
-   *Microsoft Store 앱* 기본 설정입니다. 사용하는 앱이 개발용 앱이 아니거나 회사에서 지급한 특수한 내부 앱을 사용하는 경우 이 설정을 계속 활성 상태로 유지하세요.
...
설정 앱 사이드 로드는 일반적으로 회사 관리 장치에 Microsoft Store 거치지 않고 사용자 지정 앱을 설치 해야 하는 학교용 또는 Microsoft 이외의 원본에서 앱을 실행 해야 하는 다른 사람의 사용 됩니다. 이 경우 이전에 설정 페이지 이미지에 표시된 대로 조직에서 *UWP 앱* 설정을 사용하지 않는 정책을 적용하는 경우가 일반적입니다. 또한 조직에서는 필요한 인증서 및 테스트용 로드 앱의 설치 위치를 제공합니다. 자세한 내용은 TechNet 문서 [Windows 10에서 앱을 테스트용으로 로드](https://technet.microsoft.com/library/mt269549.aspx) 및 [Microsoft Intune에서 앱 배포 시작](https://technet.microsoft.com/library/dn646955.aspx)을 참조하세요.

디바이스 패밀리 관련 정보

-   데스크톱 디바이스 패밀리에서: 앱 패키지 (.appx)를 설치할 수 있습니다 하 고 있는 Windows PowerShell 스크립트를 실행 하 여 앱을 실행 하는 데 필요한 모든 인증서 패키지 ("Add-AppDevPackage.ps1")를 사용 하 여 만들어집니다. 자세한 내용은 [UWP 앱 패키징](../packaging/packaging-uwp-apps.md)을 참조하세요.

-   모바일 디바이스 패밀리에서: 필요한 인증서가 이미 설치 하는 경우에 SD 카드 또는 전자 메일을 통해 사용자에 게 전송 하는 모든.appx 설치 파일을 눌러 수 있습니다.


**앱 테스트용 로드**를 사용하면 신뢰할 수 있는 인증서가 없는 디바이스에 앱을 설치할 수 없으므로 개발자 모드보다 안전합니다.

> [!NOTE]
> 앱을 테스트용으로 로드하는 경우에도 여전히 신뢰할 수 있는 소스의 앱만을 설치해야 합니다. Microsoft Store에서 인증되지 않은 테스트용 로드 앱을 설치할 경우 이러한 앱을 테스트용으로 로드하는 데 필요한 모든 권한을 확보했으며 이러한 앱을 설치하고 실행하여 발생할 수 있는 피해에 대해 전적으로 책임을 진다는 점에 동의하는 것입니다. 이 [개인 정보 취급 방침](https://go.microsoft.com/fwlink/?LinkId=521839)의 &gt; Microsoft Store 섹션을 참조하세요.
...
- Microsoft 장치 포털. **디바이스 포털 사용** 옵션이 켜진 경우에만 Device Portal을 사용할 수 있고 방화벽 규칙이 구성됩니다.
...
> [!NOTE]
> 개발자 모드는 제대로 설치 하지 않습니다, 경우 좋습니다 피드백 요청을 제출할 수 있습니다. 에 **피드백 허브** 앱을 선택 **새 피드백을 추가**를 선택 합니다 **개발자 플랫폼** 범주 및 **개발자 모드** 하위 범주입니다. 사용자 의견 보내기 Microsoft 발생 한 문제를 해결 하는 데 도움이 됩니다.
...
**Gpedit를 사용 하 여 장치를 사용 하도록 설정 하려면**
...
3.  테스트용 로드를 사용하도록 설정하려면 다음을 사용하도록 정책을 편집합니다.
    ...
    -   **신뢰할 수 있는 모든 앱 설치 허용**
    -   **UWP 앱 및 설치 하는 통합된 개발 환경 (IDE)에서 개발을 가능**

4.  컴퓨터를 다시 부팅합니다.

**Regedit를 사용 하 여 장치를 사용 하도록 설정 하려면**
...
2.  테스트용 로드를 사용하도록 설정하려면 다음 명령을 실행합니다.
    ...
    -   **PS c:\\WINDOWS\\system32&gt; reg 추가 "HKEY\_로컬\_MACHINE\\소프트웨어\\Microsoft\\Windows\\CurrentVersion\\ AppModelUnlock"/t REG\_DWORD /f /v"AllowDevelopmentWithoutDevLicense"/d"1"**
