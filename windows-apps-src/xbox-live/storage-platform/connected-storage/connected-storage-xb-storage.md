...
마찬가지로, Xbox One 개발 키트에서 연결 된 저장소 컨테이너에는 개발 PC에 XML 파일에서 데이터의 복사본을 사용 하면 클라우드 해당 데이터를 업로드 하려면 콘솔 합니다. 그러나 조건이 없는 동기화 되지 것입니다: 개발 키트, 잠금 얻을 수 없습니다 아니면 사용자가 하지 않기로 하 여 충돌을 해결 하는 경우 콘솔 동작 콘솔의 컨테이너와 클라우드에서 간의 데이터 충돌이 있는 경우 선택 하 고 컨테이너를 유지 하 고 콘솔의 한 버전 제목 시작 될 때까지 사용자가 오프 라인 재생 하는 경우 작동 합니다.

이러한 명령에의 한 가지 예외 **** 초기화/는 **** 강제로/모든 SCIDs 및 사용자에 대 한 저장 된 데이터의 로컬 저장소를 지웁니다 있지만 클라우드에 저장 된 데이터를 변경 하지 않습니다. 사용자가 콘솔에 로밍 및 제목을 재생 시 클라우드에서 데이터를 다운로드 하는 경우에 것 상태로 콘솔을 배치 하는 데 유용 합니다.

### <a name="xbstorage-commands"></a>Xbstorage 명령

Xbstorage에 6 개의 명령을 개발자가 Xbox One 개발 키트의 로컬 데이터를 관리 하 XDK 명령 프롬프트를 사용 하 여 사용할 수 있습니다.
...
>
> 동일한 기본 서비스 안내를 사용 하 여 응용 프로그램을 실행 중인 경우이 작업으로 인해는 경합 상태 및 연결 된 저장소 공간의 내용을 확정 되지 않은 상태에서 수 있습니다.
>
...
- **/Context** 매개 변수를 사용 하 고 파일 이름을 지정 하 여 \<infile>에 올바르게 채워지는 **ContextDescription** 섹션 경우 해당 파일 연결 된 저장소 공간을 지정 하는 데 사용 됩니다.
...
더미 데이터를 생성 하 고 \<filename> 하 여 지정 된 파일을 저장 합니다. 파일의 XML 형식에 대 한 자세한 내용은이 항목의 뒷부분에 [가져오기 및 내보내기 파일 형식](#xbstorage_fileformat)을 참조 하세요.    서비스 구성 id (서비스 안내) 00000000-0000-0000-0000-000000000000를 놓고 컴퓨터별 연결 된 저장소 공간에 대 한 계정 설정 됩니다. 해당 값을 변경 하려는 경우 직접 편집할 수 있습니다 파일 생성 됩니다.
...
> [!NOTE]
> 바이트 데이터는 간단한 오름차순; 예를 들어 5 비트 blob에 다음 바이트: 00 01 02 03 04 합니다. >>는 사용자별 연결 된 저장소 공간을 지정 하려면 변경 XML 파일의 **계정** 노드에 다음과 같습니다.
>  ```
...
| /X *: 주소* [*+ accesskey*]  | 호스트 이름 또는 주소 (콘솔에 **도구 IP** 로 표시 됨)의 대상된 콘솔을 지정 하지만 콘솔의 기본 변경 되지 않습니다. 이 옵션을 사용 하지 않는 경우 콘솔의 기본 사용 됩니다. *Accesskey* 선택 키를 알고 있는 사용자만 콘솔에 액세스를 제한 하는 데 사용할 수 있는 문자열입니다. **Xbconfig**명령을 사용 하 여 선택 키를 설정**accesskey = * * * 사용자 키*; 그런 다음, 선택 키를 적용 하려면 콘솔이 다시 시작 합니다. 선택 키를 사용 하 여 구성 된 콘솔에 액세스 하려면 콘솔의 IP 주소 또는 호스트 이름 뒤에 더하기 (+)와 선택 키를 포함 해야 합니다.
> [!NOTE]
...
*Gamesaveutil* 같은 모든 앱 *xbstorage* 해당 기능에 대 한 로컬에 캐시 된 연결 된 저장소를 관리할 수 있도록 제공 합니다. Xbstorage 도구와 마찬가지로 *gamesaveutil* 동작에 몇 가지 차이점이 기능 관리 동일한 6 개까지 데이터를 제공 합니다. 가져오기, 내보내기, 삭제 및 재설정 명령은 로그인 하는 Xbox Live 사용자 필요 합니다. 확인 하 고 현재 사용자를 변경할 Windows 10의 Xbox 앱을 사용할 수 있습니다.
...
\_LT_filename>에 지정 된 데이터를 가져옵니다.

파일은 데이터가 포함 된 XML 파일. 유형 `gamesaveutil help generate` 예를 생성 하는 방법을 확인 합니다.

앱을 지정 하는 방법은 두 가지가 PFN 및 데이터가 저장 되는 위치 서비스 안내 합니다.

입력된 파일에 올바르게 채워지는 ContextDescription 섹션 경우 이렇게 하는 데 대상 앱 지정 PFN 및 서비스 안내 합니다.

PFN 및 서비스 안내 또는 부분적으로 지정할 수 입력된 파일에서 지정 된 PFN와 서비스 안내의 각 요소 보다 우선 하는 명령줄 매개 변수를 통해 합니다.

|옵션  |설명  |
|---------|---------|
|/pfn:\<PFN>       |에 대 한 가져오기를 수행 하는 앱에 대 한 패키지 패밀리 Name(PFN)를 지정 합니다. 앱을 설치 해야 합니다.         |
|/scid:\<SCID>     |서비스 구성 Id (서비스 안내)을 지정합니다. Xbox Live 구성과에서입니다.         |
...
\_LT_outfile> 하 여 지정 된 파일 데이터를 내보냅니다.

파일은 데이터가 포함 된 XML 파일. Gamesaveutil 도움말 입력 예를 생성 하는 방법을 보려면 생성 합니다.

내보내려면 데이터의 위치를 지정 하는 방법은 두 가지가 있습니다.

/Context 매개 변수를 사용 하 여 지정 된 파일 이름 및 \<infile>에 올바르게 채워지는 ContextDescription 섹션 경우 해당 파일 원본 데이터의 위치를 지정 사용 됩니다.

/Context 파일에서 지정 된 각 요소 보다 우선 명령줄 매개 변수를 통해 위치를 지정할 수도 있습니다.
...
더미 데이터를 생성 하 고 \<filename> 하 여 지정 된 파일을 저장 합니다.
서비스 구성 식별자 (서비스 안내) 00000000-0000-0000-0000-000000000000로 설정 됩니다. 원하는 경우 해당 값을 변경 하기 위해 생성 한 후 파일을 수동으로 편집 합니다.

|옵션  |설명  |
|---------|---------|
|/containers:\<n>     |생성에 얼마나 많은 컨테이너를 지정 합니다. 기본값은 2입니다.         |
|/blobs:\<n>          |컨테이너를 생성 당 얼마나 많은 blob을 지정 합니다. 기본값은 3입니다.        |
...
**가져오기**, **내보내기**및 *gamesaveutil* **생성** 에 대 한이 xml 서식을 지정 하는 데 필요한 변경 \<PackageFamilyName> 노드와 \<ContextDescription> 노드의 \<Account> 구성원 노드를 대체 하는 것입니다.
이 \<ContextDescription> 노드도 변경 됩니다.
...
> [!NOTE]
> 이러한 XML 파일에는 데이터의 형식을 플랫폼에서 동일, 가져오기 위한 것이 목적 으로만 내보내기 및 합니다. 중간 또는 유틸리티 형식, 보관 된 형식이 아니라으로 처리 해야 하므로 이러한 XML 파일에 대 한 데이터 형식을 앞으로 변경 될 수 있습니다.

**ContextDescription** 노드 선택 사항입니다. 컴퓨터에 대해 연결 된 저장소 공간을 하는 경우 사용할 수 있습니다 `<Account machine="true"/>` 대신 `<Account msa="user@domain.com"/>`. 그렇지 않으면 컨텍스트 가져오기에 대 한 명령줄에 지정할 수 있습니다.
Blob 및 컨테이너 게임 또는 파일이 생성 되 고 응용 프로그램에 게 부여 된 해당 이름을 사용 해야 합니다.
각 blob의 내용을 **CryptBinaryToStringW** **CRYPT_STRING_BASE64** 해당 blob 원시 바이트 배열에 대 한 데이터를 제공 하는 플래그를 사용 하 여 호출 하 여 생성 된 **CDATA** 태그에 문자열 이어야 합니다. 반대로, 데이터 blob **CryptStringToBinary** 를 호출 하 고 이전의 암호화 된 문자열을 제공 하 여 다시 변환할 수 있습니다. 이 두 함수를 사용 하는 예제는 Visual Studio에 대 한 MSDN 포럼에서 [잘못 된 바이트를 반환 하는 CryptBinaryToString](https://social.msdn.microsoft.com/Forums/vstudio/en-US/9acac904-c226-4ae0-9b7f-261993b9fda2/cryptbinarytostring-returns-invalid-bytes?forum=vcgeneral) 에 표시 됩니다.
