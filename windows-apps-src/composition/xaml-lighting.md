...
[**XamlLight**](/uwp/api/windows.ui.xaml.media.xamllight) 개체는 [**CompositionLights**](/uwp/api/Windows.UI.Composition.CompositionLight)에 적용하여 동적으로 XAML UIElements를 조명하는 데 사용됩니다. XamlLight는 UIElements 또는 XAML 브러시에 uielements 트리에 적용 타기 팅을 위한 메서드를 제공 하 고 현재 있는지 여부에 따라 리소스 사용 CompositionLight의 수명을 관리 합니다.
...
그런 다음 XAML UIElement 또는 브러시 하 게 빛을이 빛을 적용할 수 있습니다. 이 예제에서는 다른 잠재적 사용법을 보여 줍니다.

> [!Important]
> 에 대 한 [C + + WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt)를 두 개를 제거 `local:OrangeSpotLight.IsTarget="True"` 아래의 태그에서. 연결 된 속성은 코드 숨김에서 이미 설정 됩니다.
...
![Xaml 조명에 의해 켜 집니다 요소의 예](images/orange-spot-light.png)
