...
- **리뷰 내용**: 기본 설정은 **사용 하 여 평가**검토 콘텐츠로 등급 표시 되지 즉입니다. **모든** 모든 평점을 표시 하려면 모든 리뷰 텍스트를 포함 하지 않는 선택할 수 있습니다. 참고 **평점 분석** 차트 선택에 관계 없이 모든 리뷰를 항상 표시 됩니다.
...
**통찰력 범주** 차트 그룹으로 결정 하는 범주에 따라 리뷰를 검토 연결할 수 있습니다.
...
또한 검색 상자를 사용 하 여 특정 단어 또는 구 앱 리뷰에 대 한 검색 합니다. 리뷰는 다른 언어로 작성 된 경우에 참고 원래만 검토 고객이 작성 된 텍스트는 검색 됩니다. 리뷰 번역 된 텍스트 검색 되지 않습니다.
...
위에서 언급 했 듯이 때 리뷰를 검색, 고객이 남긴 원래 텍스트만 검색 됩니다 (및 텍스트는 번역) **리뷰 번역** 상자를 선택 해야 하는 경우에 합니다.
