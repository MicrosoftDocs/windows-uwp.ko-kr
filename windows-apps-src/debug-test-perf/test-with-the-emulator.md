...
CPU (BIOS에서 사용할 수 있는 경우이 설정을 사용할 수 있음)
...
Windows 10 Mobile 용 Microsoft 에뮬레이터를 사용 하려면 Visual Studio 2015 이상이 필요 합니다. 이전 버전의 Visual Studio와 호환 되지 않습니다.
...
컴퓨터 하드웨어 키(에뮬레이터 빌드 10.0.14295 및 이전 버전) | 컴퓨터 하드웨어 키(에뮬레이터 빌드 10.0.14332 이상) | Windows Phone 하드웨어 버튼 | 메모
--------------------- | ------------------------- | ----------------------------- | -----
...
> **중요**  tapper 도구를 처음 시작할 때 Windows 방화벽 프롬프트가 표시 됩니다. 3가지 확인란을 모두 선택하고 방화벽을 통한 도구를 허용해야 합니다. 그러지 않으면 도구가 자동으로 작동하지 않습니다.
...
> 휴대폰에서 사용 하는 SD 카드는 FAT32 파일 시스템으로 포맷 되므로 32GB는 최대 크기입니다 **.  **
...
새 Windows 10 비행으로 업데이트 한 후를 비롯 한 특정 상황에서는 에뮬레이터와 연결 된 가상 네트워크 스위치가 사용자 인터페이스를 통해 삭제할 수 없는 상태로 전환 될 수 있습니다.
...
**참고**  이 명령은 에뮬레이터와 연결 된 장치 뿐 아니라 모든 네트워킹 장치를 삭제 합니다. 컴퓨터가 다시 시작되면 모든 하드웨어 네트워킹 디바이스가 자동으로 검색됩니다.
 
...
> **참고**  XDECleanup는 모든 에뮬레이터 관련 hyper-v vm을 삭제 하 고 모든 VM 검사점 또는 저장 된 상태를 삭제 합니다.

### <a name="uninstall-windows-10-for-mobile-image"></a>모바일용 Windows 10 이미지 제거

에뮬레이터를 설치하면 모바일용 Windows 10 VHD 이미지가 설치되고 제어판의 **프로그램 및 기능** 목록에 해당 항목이 표시됩니다. 이미지를 제거 하려면 설치 된 프로그램 목록에서 **모바일 이미지\<버전\>Windows 10** 을 찾아 마우스 오른쪽 단추로 클릭 한 다음 **제거**를 선택 합니다.

그런 다음 현재 릴리스에서 에뮬레이터용 VHD 파일을 수동으로 삭제해야 합니다. 에뮬레이터를 기본 경로에 설치한 경우 VHD 파일은 C:\\Program Files (x86)\\Windows 키트\\10\\에뮬레이션\\Mobile\\\<\>\\입니다.
