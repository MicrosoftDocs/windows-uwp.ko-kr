...
**In process 및 out of process 백그라운드 작업:** Windows 10 버전 1607 도입 [프로세스에서 백그라운드 작업](create-and-register-an-inproc-background-task.md) 포그라운드 응용 프로그램으로 동일한 프로세스에서 배경 코드를 실행할 수 있습니다. 백그라운드 작업에 대해 In-process를 사용할지 또는 Out-of-process를 사용할지를 결정할 때 고려할 요소는 다음과 같습니다.
...
|사용 가능한 트리거 | 프로세스에서 백그라운드 작업 트리거를 지원 하지 않습니다. [DeviceUseTrigger](https://docs.microsoft.com/uwp/api/windows.applicationmodel.background.deviceusetrigger?f=255&MSPPError=-2147217396)하십시오 [DeviceServicingTrigger](https://docs.microsoft.com/uwp/api/windows.applicationmodel.background.deviceservicingtrigger) 하 고 **IoTStartupTask**합니다. |
|VoIP | In-process 프로세스 백그라운드 작업은 응용 프로그램 내에서 VoIP 백그라운드 작업 활성화를 지원하지 않습니다. |  

**트리거 인스턴스 수를 제한 합니다.** 앱에 등록할 수 일부 트리거의 인스턴스 개수에 제한이 있습니다. 앱은 앱 인스턴스당 [ApplicationTrigger](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Background.ApplicationTrigger), [MediaProcessingTrigger](https://docs.microsoft.com/uwp/api/windows.applicationmodel.background.mediaprocessingtrigger) 및 [DeviceUseTrigger](https://docs.microsoft.com/uwp/api/windows.applicationmodel.background.deviceusetrigger?f=255&MSPPError=-2147217396)를 한 번씩만 등록할 수 있습니다. 앱이 이 제한을 초과하는 경우 등록이 예외를 throw합니다.

**CPU 할당량:** 백그라운드 작업 트리거 형식에 따라 가져오기는 벽 시계 사용 기간으로 제한 됩니다. 대부분의 트리거는 벽시계로 측정하는 30초로 제한되지만 일부는 집중적인 작업을 완료하기 위해 최대 10분을 실행할 수 있습니다. 배터리 사용 시간을 절약하고 포그라운드 앱에 대한 효율적인 사용자 환경을 제공하도록 백그라운드 작업은 간결해야 합니다. 백그라운드 작업에 적용되는 리소스 제약 조건은 [백그라운드 작업을 사용하여 앱 지원](support-your-app-with-background-tasks.md)을 참조하세요.

**백그라운드 작업을 관리 합니다.** 앱 등록 된 백그라운드 작업 목록을 가져오려면 하 고 진행률 및 완료 처리기를 등록 하 고 해당 이벤트를 적절 하 게 처리 해야 합니다. 백그라운드 작업 클래스는 진행률, 취소 및 완료를 보고해야 합니다. 자세한 내용은 [취소된 백그라운드 작업 처리](handle-a-cancelled-background-task.md) 및 [백그라운드 작업 진행 및 완료 모니터링](monitor-background-task-progress-and-completion.md)을 참조하세요.

**사용 하 여 [BackgroundTaskDeferral](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Background.BackgroundTaskDeferral):** 백그라운드 작업 클래스에서 비동기 코드를 실행 하는 경우에 지연 사용 해야 합니다. 그렇지 않으면 백그라운드 작업을 종료할 수 있습니다 중간 경우 합니다 [실행](https://docs.microsoft.com/uwp/api/windows.applicationmodel.background.ibackgroundtask.run) 메서드가 반환 되는 (또는 [OnBackgroundActivated](https://docs.microsoft.com/uwp/api/windows.ui.xaml.application.onbackgroundactivated) 메서드 프로세스에서 백그라운드 작업의 경우). 자세한 내용은 [Out-of-process 백그라운드 작업 만들기 및 등록](create-and-register-a-background-task.md)을 참조하세요.

또는 지연을 요청하고 **async/await**를 사용하여 비동기 메서드 호출을 완료합니다. **await** 메서드가 호출된 후 지연을 닫습니다.

**앱 매니페스트를 업데이트 합니다.**  Out of process를 실행 하는 백그라운드 작업을 함께 사용 하는 트리거의 형식과 함께 응용 프로그램 매니페스트에서 각 백그라운드 작업을 선언 합니다. 그러지 않으면 앱에서 런타임에 백그라운드 작업을 등록할 수 없습니다.
...
**앱 업데이트를 준비 합니다.** 앱을 업데이트할 경우 만들기 및 등록 된 **ServicingComplete** 백그라운드 작업 (참조 [SystemTriggerType](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Background.SystemTriggerType)) 등록 및 앱의 이전 버전에 대 한 백그라운드 작업을 해제 하는 새 버전에 대 한 백그라운드 작업입니다. 이때 포그라운드에서 실행 컨텍스트 외부에서 필요한 앱 업데이트도 수행하는 것이 좋습니다.

**백그라운드 작업을 실행 하는 요청:**

> **중요**  Windows 10 부터는 앱은 더 이상 필요 하지 백그라운드 작업 실행을 위한 필수 조건으로 잠금 화면에 배치 해야 합니다.
