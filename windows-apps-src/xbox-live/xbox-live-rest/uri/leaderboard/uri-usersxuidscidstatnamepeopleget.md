...
소셜 순위표 순위 상태는 현재 사용자의 모든 알려진된 연락처 중 하나에 해당 사용자에 의해 즐겨 찾는 사용자 지정 된 연락처에만 (점수) 값을 반환 합니다.
...
순위표 Api 모두 읽기 전용 이며 따라서 (HTTPS)를 통해 GET 동사만 지원 합니다. 순위 되 고 정렬 된 "페이지" 데이터 플랫폼을 통해 작성 된 개별 사용자 통계에서 파생 되는 인덱싱된 플레이어 통계을 반영 합니다. 전체 순위표 인덱스 상당히 크며 수 및 호출자 읽으세요에서 하나 묻지 않습니다, 그리고 따라서이 URI는 호출자가 해당 순위표 으로부터 보려는 보기의 종류에 대 한 특정 되도록 허용 하는 몇 가지 쿼리 문자열 인수를 지원 합니다.
...
| 서비스 안내| string| 액세스 되는 리소스를 포함 하는 서비스 구성의 식별자.|
| statname| string| 액세스 되는 사용자 통계 리소스의 고유 식별자입니다.|
| 모두|즐겨찾기| 열거형| 현재 사용자의 알려진된 모든 연락처에 해당 사용자에 의해 즐겨 찾는 사용자 지정 된 연락처에만 (점수) 값의 상태를 순위 여부입니다.|
...
| 매개 변수| 유형| 설명|
| --- | --- | --- | --- | --- | --- |
| maxItems| 32 비트 부호 없는 정수| 결과 페이지에 반환할 순위표 레코드의 최대 수입니다. 지정 하지 않으면 기본 번호 (10)을 반환 됩니다. MaxItems에 대 한 최대 값이 여전히 정의 하지만 큰 데이터 집합을 방지 하려면,이 값이 대상으로 아마도 해야 하므로 가장 큰 설정 하는 UI 호출 당 처리할 수 튜너. |
| skipToRank| 32 비트 부호 없는 정수| 지정 된 순위표 순위부터 결과 페이지를 반환 합니다. 결과의 나머지 부분 순위 순서 대로 정렬 됩니다. 이 쿼리 문자열 "다음 페이지" 결과 얻으려면 후속 쿼리를 다시 입력 될 수 있는 연속 토큰을 반환할 수 있습니다. |
...
| continuationToken| 문자열| 이전 호출 <b>continuationToken</b>반환 되 면 다음 호출자에 게 전달할 수 다시 "있는 그대로" 토큰 결과의 다음 페이지를 가져오는 쿼리 문자열. |
| sort| string| 여부 낮은 높은 값 순서 ("오름차순")에서 한 플레이어 목록이 순위를 지정 하거나 높은-낮은 값 순서 ("내림차순"). 이 선택적 매개 변수입니다. 기본값은 내림차순 합니다.|
...
| 수락| 문자열입니다. 허용 되는 콘텐츠 형식 값. 예제 값: "application/json".|
...
| 헤더| 설명|
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| If-None-Match| 문자열입니다. 엔터티 태그-캐싱을 지원 하 고 클라이언트 경우 사용 합니다. 예제 값: "686897696a7c876b7e".|
...
호출자는 최적의 표시를 위해 다시 방해가 되는 데이터를 이해 하는 기능을 극대화 하기 각 사용자에 대 한 통계 각 값이 있는 것 해야 표시 되며 accept language에 지정 된 로캘과 일치 하도록 포맷 형식에서 문자열로 반환 됩니다. 요청에서 헤더입니다. 해당 순위표에 대 한 statname에 대 한 지역화 된 "표시 이름"도 반환 됩니다.
...
| continuationToken| 문자열| 필수. 원하는 경우 해당 URI에 대 한 결과의 다음 페이지를 가져오려면 "continuationToken" 쿼리 매개 변수를 다시 피드에 값을 지정 합니다. 없는 pagingInfo 반환 하는 경우에 데이터를 가져올 수 없습니다 "다음 페이지"입니다.|
| totalItems| 64 비트 부호 없는 정수| 필수. 순위표 항목의 총 수입니다.|
...
| totalCount| string| 필수. 순위표 항목의 총 수입니다.|
| 열| array| 필수.|
| displayName| string| 필수. 순위표 열에 해당합니다.|
| statName| string| 필수. 순위표 열에 해당합니다.|
| 유형| 문자열| 필수. 순위표 열에 해당합니다.|
| <b>userList</b>| 섹션| 필수. 항상 반환 됩니다. 요청 순위표에 대 한 실제 사용자 점수가 포함 되어 있습니다.|
| 게이머 태그| string| 필수. 순위표 항목에서 사용자에 해당합니다.|
| xuid| 32 비트의 부호 있는 정수| 필수. 순위표 항목에서 사용자에 해당합니다.|
...
| 406| 허용할 수 없음| 리소스 버전 지원 되지 않습니다. MVC 계층에 의해 거부 되어야 합니다.|
