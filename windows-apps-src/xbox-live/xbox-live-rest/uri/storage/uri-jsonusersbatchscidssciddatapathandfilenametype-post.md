...
ms.locfileid: "4354786"
---
# <a name="post-jsonusersbatchscidssciddatapathandfilenamejson"></a>POST (/json/users/batch/scids/{scid}/data/{pathAndFileName},json)
여러 사용자가 동일한 파일 이름으로에서 여러 파일을 다운로드합니다. 파일 다운로드 요청 URI에 의해 결정 됩니다. 요청 본문 파일을 다운로드 하려면 사용자의 XUIDs 목록이 포함 되어 있습니다. 응답 본문에는 다중 파트 MIME 메시지 자체 헤더의 집합을 사용 하 여 특정 사용자에 대 한 파일을 나타내는 각 부분에 됩니다. 것의 성공 및 실패에 대 한 응답의 부분에 대해 가능 합니다. 이러한 Uri에 대 한 도메인은 `titlestorage.xboxlive.com`.
...
호출에 성공 하면 서비스는 요청 된 파일의 내용을 다중 파트 응답에 반환 합니다.
