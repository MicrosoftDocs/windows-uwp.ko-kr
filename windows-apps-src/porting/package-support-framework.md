...
[프로세스 모니터](https://docs.microsoft.com/en-us/sysinternals/downloads/procmon) 는 앱의 파일 및 레지스트리 작업의 결과 관찰 하기 위한 강력한 유틸리티입니다.  응용 프로그램 호환성 문제를 이해 하는 데 도움이 수 있습니다.  프로세스 모니터를 연 후 응용 프로그램 실행 파일에서 이벤트만 포함 하는 필터 (필터 gt_ 필터...)를 추가 합니다.
...
이 문제에 응용 프로그램 패키지 경로에.log 파일을 쓸 하지 못했습니다. 이 것이 좋습니다 파일 리디렉션 수정 도움이 될 수 있습니다.
...
.Msix (또는.appx) 파일이 이미 있는 경우 패키지에 대 한 준비 영역으로 지원할 레이아웃 폴더에 해당 콘텐츠 압축을 풉니다 수 있습니다. Sdk 설치 경로에 따라 makemsix 도구를 사용 하 여 명령 프롬프트에서 이렇게 하려면,이 찾습니다 makeappx.exe 도구를 Windows 10 pc는: x86: C:\Program Files (x86) \Windows Kits\10\bin\x86\makeappx.exe x64: C:\Program Files ( x86) \Windows Kits\10\bin\x64\makeappx.exe
...
파일 이름을 만들기 ``config.json``, 해당 파일을 패키지의 루트 폴더에 저장 합니다. 방금 교체 하는 실행 파일을 가리키도록 config.json 파일의 선언 된 응용 프로그램 ID를 수정 합니다. 프로세스 모니터를 사용 하 여 얻은 정보를 사용 하 여 있습니다 또한 작업 디렉터리를 설정 뿐 아니라 수 파일 리디렉션 수정을 사용 하 여 패키지 상대 "PSFSampleApp" 디렉터리 아래에 있는.log 파일을 읽기/쓰기를 리디렉션하 합니다.
...
| 프로세스 | 실행 | 대부분의 경우 이름이 됩니다는 `executable` 위에 제거 경로 파일 확장명으로 구성 합니다. |
| 수정 | dll | 로드 하는.msix/.appx 수정 하기 위해 패키지 상대 경로입니다. |
| 수정 | 구성 | (선택 사항) 수정 메일 그룹 동작 하는 방법을 제어 합니다. 이 값의 정확한 형식은 각 수정이 "blob" 해석할 수 원하는 대로 수정 하 여 수정 하기 위해 별로 달라 집니다. |

`applications`, `processes`, 및 `fixups` 키가 배열입니다. 즉,는 둘 이상의 응용 프로그램, 프로세스 및 수정 하기 위해 DLL 지정 하려면 config.json 파일을 사용할 수 있습니다.
...
패키지 된 응용 프로그램 호환성 문제를 진단 하는 대체 기술을 수정 추적 하기 위해 사용 하는 것입니다. 이 DLL의 PSF에 포함 되어 이며 앱의 동작을 프로세스 모니터와 유사한의 진단 세부 보기를 제공 합니다.  응용 프로그램 호환성 문제를 표시 하도록 특별히 설계 되었습니다.  추적 수정을 사용 하 여를 추가 하 DLL 패키지에에 config.json에 다음 코드를 추가 하 고 다음 응용 프로그램을 설치 합니다.
...
기본적으로 수정 추적 하기 위해 "예상"으로 간주할 수도 있는 오류를 필터링 합니다.  예를 들어, 응용 프로그램 무조건적으로 확인 하는 경우 이미 있는 결과 무시 하 고 않고 파일을 삭제 하려고 할 수 있습니다. 이것은 파일 시스템 기능에서 모든 오류를 수신 하도록 선택 하는 위의 예제에서는 되므로 일부 예기치 않은 오류 필터링 얻을 수 있는 맬웨어 결과가 있습니다. 그 전에 Config.txt 파일에서 읽기를 시도 실패 하는 메시지 "파일 찾을 수 없음"에서 끝나지 때문에이 야기 합니다. 이 자주 관찰 되 고 하지 일반적으로 예상 된 것으로 간주 되는 실패 합니다. 실제로 가능성이 가장 예기치 않은 오류를 필터링 하 고 다음 폴백 모든 오류를 여전히 식별할 수 없는 문제가 있는 경우 시작 하는 것 있습니다.

기본적으로 출력을 수정 추적 하기 위해 연결 된 디버거에 전송 합니다. 이 예제에서는 디버거를 첨부할 생각 하 고 출력을 보려면 SysInternals에서 [DebugView](https://docs.microsoft.com/en-us/sysinternals/downloads/debugview) 프로그램을 대신 사용 됩니다. 앱을 실행 한 후 알 수 있습니다 동일한 오류 이전과 하는 것 가리킵니다 우리는 동일한 런타임 수정 합니다.
...
| Runtimefix | 이것이 역할을 런타임 수정 하는 하나 이상의 대체 함수를 포함 하는 c + + Dynamic-Linked 라이브러리 프로젝트입니다. |
...
수 있는 모든 응용 프로그램 프로젝트를 추가 하려면 수도 있습니다.
...
**솔루션 탐색기**패키징 프로젝트를 마우스 오른쪽 단추로 클릭 **편집**을 선택한 다음이 프로젝트 파일의 맨 아래에 추가 합니다.
...
완전히 새로운 수정 만들려는 경우 추가 하지 마십시오 아무 것도이 프로젝트에 아직. 이 가이드의 뒷부분에서이 프로젝트에 올바른 파일을 추가 도와 드립니다. 지금은 솔루션 설정을 계속 됩니다.
...
**응용 프로그램** 노드에서 PSF 시작 관리자 응용 프로그램을 마우스 오른쪽 단추로 클릭 한 다음 **진입점으로 설정**를 선택 합니다.
...
| 응용 프로그램 | workingDirectory | (선택 사항) 시작 하는 응용 프로그램의 작업 디렉터리를 사용 하 여 패키지 상대 경로입니다. 이 값을 설정 하지 않으면, 운영 체제에서 사용 합니다 `System32` 응용 프로그램의 작업 디렉터리로 디렉터리입니다. |
| 프로세스 | 실행 | 대부분의 경우 이름이 됩니다는 `executable` 위에 제거 경로 파일 확장명으로 구성 합니다. |
| 수정 | dll | 정의 로드 하는 DLL 패키지 상대 경로입니다. |
| 수정 | 구성 | (선택 사항) 수정 하기 위해 DLL의 동작을 제어 합니다. 이 값의 정확한 형식은 각 수정이 "blob" 해석할 수 원하는 대로 수정 하 여 수정 하기 위해 별로 달라 집니다. |
...
Visual Studio에서 f5 키를 눌러 디버거를 시작 합니다.  먼저 시작 하는 차례로 대상 데스크톱 응용 프로그램을 시작 하는 PSF 시작 관리자 응용 프로그램입니다.  대상 데스크톱 응용 프로그램을 디버깅 하려면 수동으로 **디버그**선택 하 여 데스크톱 응용 프로그램 프로세스에 연결 해야 합니다->한 다음 응용 프로그램 프로세스를 선택 하 고**프로세스에 연결**합니다. 기본 런타임 수정을 DLL.NET 응용 프로그램의 디버깅을 허용 하려면 관리 코드와 네이티브 코드 유형 (혼합된 모드 디버깅)를 선택 합니다.  
...
없는 아직 런타임을 해결 하려면, 대체 함수를 작성 하 고 모든 구성 데이터를 포함 하 여 새로운 런타임 수정을 만들 수 있습니다 문제에 대 한 해결 하는 것이 좋습니다. 각 부분에 대해 살펴보겠습니다.

### <a name="replacement-functions"></a>대체 기능

먼저, 호출이 실패 응용 프로그램이 MSIX 컨테이너에서 실행 되는 함수를 식별 합니다. 그런 다음, 런타임 관리자를 대신 호출 하 고 싶은 대체 함수를 만들 수 있습니다. 이렇게 하면 최신 런타임 환경 규칙에 맞는 동작 함수의 구현을 바꿉니다 수 있습니다.
...
함수의 동일한 서명을 포함 된 함수를 만들기 사용자가 수정 하려는 동작 합니다. 다음은 대체 하는 예제 함수는 `MessageBoxW` 함수.
...
선택적으로 적용할 수 있습니다는 `reentrancy_guard` 런타임 수정 함수를 재귀적 호출에 대 한 보호 하는 함수를 입력 합니다.
...
에 ``WinDbg`` 프롬프트, 디버깅 자식을 활성화 하 고 적절 한 중단점을 설정 합니다.
