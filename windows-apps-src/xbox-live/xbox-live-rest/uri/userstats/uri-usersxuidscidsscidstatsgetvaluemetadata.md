...
? 포함 = valuemetadata 쿼리 매개 변수는 모델 및 경합 트랙에 한 번 달성 하는 데 사용 되는 자동차의 색 등의 사용자 통계 값와 관련 된 모든 메타 데이터를 포함 하도록 응답 합니다.

응답에서 값 메타 데이터를 포함 하도록 요청 호출 3 X Xbl-계약 버전 헤더 값도 설정 해야 합니다.
...
콘텐츠 격리 및 액세스 제어 시나리오에 대 한 구현 된 권한 부여 논리가 있습니다.
   ...
   * 순위표와 사용자 통계 호출자가 요청을 사용 하 여 유효한 XSTS 토큰 전송 된 모든 플랫폼에서 클라이언트에서 읽을 수 있습니다. 쓰기는 데이터 플랫폼에서 지원 되는 클라이언트를 제한 됩니다.
...
| X Xbl-계약 버전| string| 사용 하는 API의 버전을 나타냅니다. 이 값에 응답 값 메타 데이터를 포함 하려면 "3"으로 설정 해야 합니다.|
