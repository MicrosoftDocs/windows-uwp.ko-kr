...
예를 들어 있다고 가정해 보겠습니다 라고 하는 게임 개체 `UserProfile1` 를 기반으로 합니다 `UserProfile` prefab 합니다. 플레이어 1이 게임 개체에 연결 하 고 사용 하 여 로그인 하려는 경우는 `A` 자신의 Xbox 컨트롤러에서 단추를 간단 하 게 작성 `joystick 1 button 0` 에 `Input Controller Button` 관리자에서 필드.
...
- **컨트롤러 단추 입력:** 추가 하 고 있는 Xbox Live 사용자가 로그인 합니다.
...
Unity에서 Xbox 컨트롤러 단추 매핑을 위해이 [Unity 컨트롤러 Wiki 페이지](https://wiki.unity3d.com/index.php?title=Xbox360Controller)를 확인 합니다.
