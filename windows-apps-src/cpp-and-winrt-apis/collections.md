...
ms.locfileid: "5406102"
---
# <a name="collections-with-cwinrt"></a>C++/WinRT로 작성된 컬렉션

내부적으로 Windows 런타임 컬렉션에는 많은 복잡 한 이동 부분이 있습니다. 하지만 함수와 기본 클래스에는 Windows 런타임 함수에 컬렉션 개체를 전달 하거나 고유한 컬렉션 속성 및 컬렉션 형식과 구현 해야 경우 [C + + WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt) 하도록 지 원하는 합니다. 이러한 기능 손, 아웃 복잡성을 가져와 시간과 노력에 많은 오버 헤드가 저장 합니다.

[**IVector**](/uwp/api/windows.foundation.collections.ivector_t_) 는 요소의 모든 임의 액세스 수집에 의해 구현 된 Windows 런타임 인터페이스입니다. **IVector** 를 구현 하는 것을 [**IIterable**](/uwp/api/windows.foundation.collections.iiterable_t_), [**IVectorView**](/uwp/api/windows.foundation.collections.ivectorview_t_)및 [**IIterator**](/uwp/api/windows.foundation.collections.iiterator_t_)구현 해야 합니다. *필요한* 사용자 지정 컬렉션을 입력 하는 경우에 많은 작업입니다. 하지만 데이터 **std:: vector** (또는 **std:: map**또는 **std::unordered_map**)에 있고 Windows 런타임 API로 전달 하는 모든 작업을 수행 하려는 경우 다음 하려는 가능 하면 해당 수준의 작업을 수행 하지 않는. 때문에 ** 가능한을 지정 하 고 C + + WinRT를 사용 하면 효율적이 고 적은 노력으로 컬렉션을 만들 수 있습니다.
...
범용 컬렉션을 구현 하는 형식의 새 개체를 검색 하려면 [**winrt::single_threaded_vector**](/uwp/cpp-ref-for-winrt/single-threaded-vector) 함수 템플릿을 호출할 수 있습니다. 개체 [**IVector**](/uwp/api/windows.foundation.collections.ivector_t_)으로 돌아가고 반환 된 개체의 함수 및 속성 호출를 통해 인터페이스입니다.
...
**Winrt::single_threaded_vector**데이터를 포함 하는 임시 개체를 전달할 수와 마찬가지로 `coll1`위의 합니다. (가정 하면 없습니다 수 액세스 다시) **std:: vector** 를 이동할 수 또는 함수로 합니다. 두 경우 모두 *rvalue* 는 함수에 전달 합니다. 이 통해 데이터를 복사 하 고 효율적일 컴파일러. *Rvalue*에 대해 자세히 알고 싶다면 [값 범주 및 참조를](cpp-value-categories.md)참조 하세요.
...
Windows 런타임 컬렉션에서 데이터를 만들고 그에 대 한 보기를 보다 준비 복사 아무것도 하지 않고 모든 API로 전달 수 있습니다.
...
위 예제에서 *수 있습니다.* 만듭니다 컬렉션 XAML 항목 컨트롤;에 바인딩할 수 하지만 컬렉션에 관찰 되지 않습니다.
...
[**IObservableVector**](/uwp/api/windows.foundation.collections.iobservablevector_t_)으로 개체가 반환 됩니다 및 인터페이스를 통해 사용자 (또는 연결 된 컨트롤) 호출 반환 된 개체의 함수 및 속성입니다.
...
**Std:: map** 유형 또는 **std::unordered_map** *rvalue* 함수에 전달 하 여 데이터를 사용 하 여 이러한 컬렉션을 초기화할 필요에 따라 있습니다.
...
모든 동시성을 제공 하지 않는 나타냅니다는 "단일 스레드" 이러한 함수 이름에&mdash;즉, 달라도 스레드로부터 안전 합니다. 이러한 함수에서 반환 된 개체 모두 agile 되므로 스레드 언급 하는 것은 아파트를 상관이 없습니다 (참조 [Agile 개체 C + + WinRT](agile-objects.md)). 개체는 단일 스레드는 방금 것입니다. 및 완전히 데이터는 한 가지 방법은 또는 다른 응용 프로그램 이진 인터페이스 (ABI) 간에 전달 하려는 경우에 적합 합니다.

## <a name="base-classes-for-collections"></a>컬렉션에 대 한 기본 클래스

전체 유연성을 위해 사용자 지정 컬렉션을 구현 하려는 경우 수행 하는 노력을 방지 하려는 됩니다. 예를 들어,이 사용자 지정 벡터 보기 이러한 모습 *없이 지원 C + + /winrt의 기본 클래스*.
...
**Get_container** 반환한 컨테이너 **시작** 및 **종료** 인터페이스 해당 **winrt::vector_view_base** 제공 해야 것으로 예상 합니다. 위 예제에서와 같이 **std:: vector** 를 제공 합니다. 하지만 고유한 사용자 지정 컨테이너를 포함 하 여 동일한 계약 fulfils 모든 컨테이너를 반환할 수 있습니다.
...
이 기본 클래스는 C + + /winrt 사용자 지정 컬렉션을 구현 하는 데는 제공.
