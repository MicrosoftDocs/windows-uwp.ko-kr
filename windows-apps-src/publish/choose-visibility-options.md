...
- 테스터가 작성한 모든 리뷰는 [리뷰 보고서](reviews-report.md)에 표시가 됩니다. 하지만 이들 리뷰는 **공중 대상**으로 제출이 이동한 후에도 앱의 Store 목록에 게시되지 않습니다.  
- **개인 대상**에서 **공중 대상**으로 앱을 이동시킬 때 Store 목록에 표시되는 **릴리스 날짜**는 공중 대상에 처음 앱이 게시된 날짜입니다(다른 [릴리스 날짜 표시](set-app-pricing-and-availability.md#display-release-date)를 지정하지 않은 한).
