---
author: libbymc
title: "시작 자습서 - REST API 백 엔드를 사용한 단일 페이지 웹앱"
description: "인기 있는 웹 기술을 사용하여 Windows 스토어용 호스트된 웹앱 제작"
...
---
# <a name="get-started-tutorial---single-page-web-app-with-rest-api-backend"></a>시작 자습서 - REST API 백 엔드를 사용한 단일 페이지 웹앱

**인기 있는 전체 스택 웹 기술을 사용하여 Windows 스토어용 호스트된 웹앱 제작**

![단일 페이지 웹앱인 간단한 기억력 게임](images/fullstack.png)

두 부분으로 구성된 이 자습서에서는 브라우저와 Windows 스토어용 호스트된 웹앱 모두로 작동하는 간단한 기억력 게임을 제작하는 과정을 통해 최신 전체 스택 웹 개발을 간략히 살펴봅니다. 1부에서는 게임의 백 엔드를 위한 간단한 REST API 서비스를 작성합니다. 게임 논리를 API 서비스로 클라우드에서 호스팅함으로써, 게임 상태를 유지하여 사용자가 다른 디바이스에서도 동일한 게임 인스턴스를 플레이할 수 있도록 합니다. 2부에서는 프런트 엔드 UI를 반응형 레이아웃을 가진 단일 페이지 웹앱으로 제작합니다.
...
Azure 부분을 생략하거나 연기하려면, Azure 호스팅과 Windows 스토어를 위한 앱 패키징을 다루는 1부와 2부의 마지막 섹션을 건너뛰면 됩니다. 제작할 API 서비스와 웹앱은 컴퓨터에서 로컬(각각 `http://localhost:8000`과 `http://localhost:3000`)로 실행됩니다.
...
VS Code의 모든 변경 내용을 저장하고, 서버를 다시 시작(VS Code에서 F5 또는 셸에서 `npm start`를 입력하고 [http://localhost:8000](http://localhost:8000)으로 이동)하여 게임 API를 테스트합니다.
...
Azure 문서는 다음 내용을 설명합니다.
 ...
 - [Azure Portal에 새 *API 앱* 등록](https://docs.microsoft.com/en-us/azure/app-service-api/app-service-api-nodejs-api-app#a-idcreateapiappa-create-a-new-api-app)
 - [API 앱을 위한 Git 배포 설정](https://docs.microsoft.com/en-us/azure/app-service-api/app-service-api-nodejs-api-app#set-up-your-new-api-app-for-git-deployment)
 - [Azure에 API 앱 코드 배포](https://docs.microsoft.com/en-us/azure/app-service-api/app-service-api-nodejs-api-app#deploy-your-api-code-to-azure)

앱을 등록할 때 *http://memorygameapi.azurewebsites.net* URL의 변경을 요청하는 다름 사람과의 이름 충돌을 피하기 위해 *앱 이름*을 차별화하십시오.
...
## <a name="part-ii-build-a-single-page-web-appl"></a>2부: 단일 페이지 웹앱 제작
...
5. [http://localhost:3000/](http://localhost:3000/)으로 이동하여 애플리케이션을 봅니다.

    ![http://localhost:3000/ 스크린샷](./images/express.png)
...
이제 모바일, 태블릿, 데스크톱과 같은 서로 다른 디바이스 폼 팩터에서 테스팅하기 위해 앱을 Azure(또는 다른 클라우드 호스팅 서비스)에 배포할 수 있습니다. 다른 브라우저에서도 테스트하는 것을 잊지 마세요!. 앱이 프로덕션 준비가 되면 손쉽게 *유니버설 Windows 플랫폼*(UWP)을 위한 *호스트된 웹앱*(HWA)으로 패키징하여 Windows 스토어에서 배포할 수 있습니다.

Windows 스토어에 게시하는 기본 단계는 다음과 같습니다.
...
더 발전시키기 위한 유용한 몇 가지 리소스는 다음과 같습니다.
 ...
 - [응용 프로그램 개발 프로젝트를 Azure 웹 사이트에 배포](https://docs.microsoft.com/en-us/azure/documentdb/documentdb-nodejs-application#a-nametoc395783182aPart-7-deploy-your-application-development-project-to-azure-websites)
