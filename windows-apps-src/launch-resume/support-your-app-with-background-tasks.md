...
Out of process 백그라운드 작업은 없기 때문에 백그라운드 프로세스 앱 프로세스에 문제가 있는 경우에 훨씬 복원성이 큽니다. 그러나 가격으로 앱과 백그라운드 작업 간의 프로세스 간 통신 관리가 더 복잡 합니다.
...
하지만 엔터프라이즈 앱 및 Microsoft Store에 게시 되지 것입니다 앱에 대 한 백그라운드 작업 또는 확장된 실행 세션을 백그라운드에서 무기한 실행 하는 기능을 사용 하는 방법을 알아보려면 [무기한으로 백그라운드에서 실행](run-in-the-background-indefinetly.md) 을 참조 합니다.
