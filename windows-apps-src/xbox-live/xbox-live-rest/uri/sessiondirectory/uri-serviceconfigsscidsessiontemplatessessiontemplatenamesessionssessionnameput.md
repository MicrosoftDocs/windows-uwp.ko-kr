...
PUT 메서드에서 세션 업데이트 모드에 대 한 실시간 형식은 다음과 같습니다.
...
세션 속성을 업데이트할 PUT 메서드에서 와이어 형식은 다음과 같습니다. 것 세션 URI에 PUT 연산에 해당 하는 것은 본문 속성으로 아래 개체에만 필요 합니다. 차이점은이 작업 오류 코드 404 반환 되는 세션 존재 하지 않는 경우 찾을 수 없습니다. 이 작업은 헤더를 지원합니다.
