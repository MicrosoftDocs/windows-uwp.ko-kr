...
PaneDisplayMode 속성을 사용 하 여 여러 탐색 스타일을 구성 하거나 NavigationView에 대 한 디스플레이 모드, 수 있습니다.
...
- 해야 5 또는 동일 하 게 중요 하다 고 더 적은 최상위 수준 탐색 범주 및 모든 추가 최상위 수준 탐색 범주 드롭다운 오버플로 메뉴에 있는 덜 중요 한 것으로 간주 됩니다.
- 화면에서 모든 탐색 옵션을 표시 해야 합니다.
- 원하는 앱 콘텐츠에 대 한 더 많은 공간입니다.
- 아이콘을 앱의 탐색 범주 설명 명확 하 게 수 없습니다.

:::row:::
    :::column:::
    ### <a name="left"></a>왼쪽
    창이 확장 되며 콘텐츠 왼쪽에 배치 합니다.</br>
...
- 수 있는 5-10 동일 하 게 중요 한 최상위 수준 탐색 범주입니다.
...
- [NavigationViewItem](/uwp/api/windows.ui.xaml.controls.navigationviewitem) 개체입니다. 특정 페이지를 탐색에 대 한 항목을 탐색 합니다.
- [NavigationViewItemSeparator](/uwp/api/windows.ui.xaml.controls.navigationviewitemseparator) 개체입니다. 탐색 항목 그룹화를 위한 구분 합니다. 공간으로 구분 기호를 렌더링 하는 0으로 [Opacity](/uwp/api/windows.ui.xaml.controls.navigationviewitemseparator.opacity) 속성을 설정 합니다.
- [NavigationViewItemHeader](/uwp/api/windows.ui.xaml.controls.navigationviewitemheader) 개체입니다. 항목 그룹에 레이블을 지정 헤더입니다.
- 앱 수준 검색을 허용할 선택적 [AutoSuggestBox](auto-suggest-box.md) 컨트롤입니다. 컨트롤 [NavigationView.AutoSuggestBox](/uwp/api/windows.ui.xaml.controls.navigationview.autosuggestbox) 속성을 할당 합니다.
...
탐색 보기에 뒤로 단추가 창의 왼쪽 위 모서리에 배치 합니다. 그러나는 자동으로 처리 뒤로 탐색을 백 스택에 콘텐츠를 추가 합니다. 참조를 사용 하려면 뒤로 탐색 합니다 [뒤로 탐색](#backwards-navigation) 섹션입니다.

창 위쪽 및 왼쪽 위치에 대 한 자세한 창 구조는 다음과 같습니다.
...
![왼쪽 창 분석 탐색 보기](images/navview-pane-anatomy-vertical.png)
...
:::row:::
    :::column:::
    ![폴더 상단 탐색 창](images/navview-freeform-footer-top.png)<br>
     _상단 창 폴더_<br>
    :::column-end:::
    :::column:::
    ![왼쪽된 탐색 창 폴더](images/navview-freeform-footer-left.png)<br>
    _왼쪽된 창 폴더_<br>
    :::column-end:::
:::row-end:::
...
[PaneTitle](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.PaneTitle) 속성을 설정 하 여 창 헤더 영역에 텍스트 콘텐츠를 배치할 수 있습니다. 문자열을 사용 하 고 메뉴 단추 옆 텍스트를 보여 줍니다.

이미지 또는 로고와 같은 텍스트가 아닌 콘텐츠를 추가 하려면 [PaneHeader](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.PaneHeader) 속성에 추가 하 여 창의 헤더의 모든 요소를 배치할 수 있습니다.

PaneTitle 및 PaneHeader 설정 된 경우 콘텐츠 메뉴 단추에 가장 가까운 PaneTitle 된 메뉴 단추 옆에 있는 가로로 누적 됩니다.

:::row:::
    :::column:::
    ![헤더 상단 탐색 창](images/navview-freeform-header-top.png)<br>
     _상단 창 헤더_<br>
    :::column-end:::
    :::column:::
...
두 번째 일반적인 적응 패턴 확장된 왼쪽된 창 큰 창 너비와 중간 크기 및 작은 창 너비 모두에 메뉴 단추에 사용 하는 것입니다.

이 경우를 사용 하는 것이 좋습니다.

- 원하는 앱 콘텐츠를 더 작은 창 너비에 대 한 더 많은 공간.
...
세 번째 일반적인 적응 패턴 큰 창 너비와는 LeftCompact, 아이콘 전용 모두 중간 크기 및 작은 창 너비에 탐색 창에서 확장 된 왼쪽된 창 사용 하는 것입니다.
...
상단 탐색을 사용 하 여 앱에서 탐색 항목이 오버플로 메뉴에 창 너비 감소로 축소 합니다. 앱 창이 좁은 전환할 수 있도록 모든 오버플로 메뉴에 항목을 축소 하는 대신 LeftMinimal 탐색 위에서 PaneDisplayMode 더 나은 사용자 환경을 제공할 수 것입니다.

큰 창 크기와 작은에서 왼쪽된 탐색에서 상단 탐색을 사용 하는 것이 좋습니다 창 크기는 경우:

- 이 집합에서 하나의 범주 화면에 맞지 않는, 바람직한 수 있도록 왼쪽된 탐색을 축소 되도록 동일 하 게 중요 한 최상위 수준 탐색 범주를 함께 표시 될 집합이 있습니다.
...
> [!TIP]
> AdaptiveTrigger.MinWindowWidth를 사용 하면 창이 지정된 된 최소 너비 보다 더 넓은 시각적 상태가 트리거됩니다. 즉, 기본 XAML 좁은 창을 정의 하 고는 VisualState 정의 창을 더 넓은 가져올 때 적용 되는 수정 합니다. 기본 PaneDisplayMode 탐색 보기가 자동으로 되어 있으므로 창 너비가 CompactModeThresholdWidth, 보다 작거나 같은 경우 LeftMinimal 탐색 사용 합니다. 창을 더 넓은 가져오면는 VisualState 기본값을 재정의 하 고 위쪽 탐색 사용 됩니다.

## <a name="navigation"></a>탐색

탐색 보기는 자동으로 탐색 작업을 수행 하지 않습니다. 사용자가 탐색 항목에 탭 탐색 보기 해당 항목을 선택된 되며 [ItemInvoked](/uwp/api/windows.ui.xaml.controls.navigationview.ItemInvoked) 이벤트를 발생 시킵니다. 탭에 새 항목이 선택 되 고 결과가 [SelectionChanged](/uwp/api/windows.ui.xaml.controls.navigationview.SelectionChanged) 이벤트가 발생 합니다.
...
NavigationView에 기본 제공 뒤로 단추가 있습니다. 하지만 앞으로 탐색을 사용 하는 것이 하지 뒤로 탐색 자동으로 수행 합니다. 사용자가 뒤로 단추를 탭 하면 [BackRequested](/uwp/api/windows.ui.xaml.controls.navigationview.BackRequested) 이벤트가 발생 합니다. 뒤로 탐색을 수행 하려면이 이벤트를 처리 합니다. 자세한 내용과 코드 예제를 참조 하세요. [탐색 기록 및 뒤로 탐색](../basics/navigation-history-and-backwards-navigation.md)합니다.
...
- [IsBackButtonVisible](/uwp/api/windows.ui.xaml.controls.navigationview.IsBackButtonVisible): 표시 및 숨기기 뒤로 단추를 사용 합니다. 이 속성 [navigationviewbackbuttonvisible 열거](/uwp/api/windows.ui.xaml.controls.navigationviewbackbuttonvisible) 열거형의 값을 사용 하 고 기본적으로 **자동** 으로 설정 됩니다. 단추를 축소 하는 공간이 없고 레이아웃에 것에 대 한 예약 되어 있습니다.
...
이 예제에서는 작은 창 크기에 왼쪽된 탐색 창 큰 창 크기에는 위쪽 탐색 창 및 NavigationView를 사용 하는 방법을 보여 줍니다. VisualStateManager에서 _상단_ 탐색 설정을 제거 하 여 왼쪽 전용 탐색에 적용 될 수 있습니다.
...
> [!IMPORTANT]
> 앱의 페이지에 대 한 정보는 [ValueTuple](https://docs.microsoft.com/dotnet/api/system.valuetuple)에 저장 됩니다. 이 구조체는 응용 프로그램 프로젝트에 대 한 최소 버전 17763 이상 SDK 여야 필요 합니다. NavigationView의 WinUI 버전을 사용 하 여 Windows 10의 이전 버전을 대상으로 하는 경우 대신 [System.ValueTuple NuGet 패키지](https://www.nuget.org/packages/System.ValueTuple/) 를 사용할 수 있습니다.

> [!IMPORTANT]
> 이 코드는 NavigationView의 [Windows UI 라이브러리](https://docs.microsoft.com/uwp/toolkits/winui/) 버전을 사용 하는 방법을 보여 줍니다. NavigationView의 플랫폼 버전을 대신 사용 하는 경우 앱 프로젝트에 대 한 최소 버전이 SDK 17763 이상 이어야 합니다. 플랫폼 버전을 사용 하려면 모든 참조를 제거 `muxc:`.
...
다음은 한 [C + + WinRT](/windows/uwp/cpp-and-winrt-apis/index) C# 코드 예제에서는 위의 **NavView_ItemInvoked** 처리기의 버전입니다. 기술은 C + + WinRT 처리기 ( [**NavigationViewItem**](/uwp/api/windows.ui.xaml.controls.navigationviewitem)태그)에 저장 된 먼저 페이지의 전체 형식 이름을 사용자와 관련를 탐색할 수도 있습니다. 해당 값을 언박싱 처리기에서 [**Windows::UI::Xaml::Interop::TypeName**](/uwp/api/windows.ui.xaml.interop.typename) 개체로 설정 및 대상 페이지를 탐색할 수 있는 사용 합니다. 라는 매핑 변수의 필요가 없습니다 `_pages` C# 예제에서는;에서 볼 수 있는 및 잘못 된 형식의 태그 내에서 값이 있는지 확인 하는 단위 테스트를 만들 수 있게 됩니다. 참고 [Boxing 및 unboxing 스칼라 값을 IInspectable로 C + + WinRT](/windows/uwp/cpp-and-winrt-apis/boxing)합니다.
...
기본적으로 NavigationView 창의 디스플레이 모드에 따라 다른 배경을 사용합니다.

- 창이 회색 단색 나란히 (왼쪽된 모드)에서 콘텐츠를 사용 하 여 왼쪽의 확장입니다.
- 창이 열릴 때 인 앱 아크릴 (위쪽, 최소 또는 컴팩트 모드)에서 콘텐츠 위에 오버레이로 사용 합니다.

창 배경을 수정 하려면 각 모드의 배경 렌더링 하는 데 XAML 테마 리소스를 재정의할 수 있습니다. (이 기술을 사용 단일 PaneBackground 속성이 아닌 다른 배경 다양 한 디스플레이 모드에 대 한 지원 하기 위해.)

이 표에서 각 디스플레이 모드에는 테마 리소스를 사용 합니다.
...
> [!IMPORTANT]
> 이 코드의 AcrylicBrush [Windows UI 라이브러리](https://docs.microsoft.com/uwp/toolkits/winui/) 버전을 사용 하는 방법을 보여 줍니다. 플랫폼 버전 AcrylicBrush 대신 사용 하는 경우 앱 프로젝트에 대 한 최소 버전이 SDK 16299 이상 이어야 합니다. 플랫폼 버전을 사용 하려면 모든 참조를 제거 `muxm:`.
