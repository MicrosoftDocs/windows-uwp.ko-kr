...
> [!NOTE]
> 앱은 포그라운드에서 이며 사용자가 기본 타일 bepinned (예를 들어 사용자가 타일을 고정 하는 방법에 대 한 설명에 대해 예 클릭) 한 후 요청 의도적으로이 APIafterthe만 호출 해야 하는 동안 UI 스레드에서이 API를 호출 해야 합니다.
