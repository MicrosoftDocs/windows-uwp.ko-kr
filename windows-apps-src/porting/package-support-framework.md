...
.Msix (또는.appx) 파일이 이미 있는 경우 패키지에 대 한 준비 영역으로 지원할 레이아웃 폴더에 해당 콘텐츠 압축을 풉니다 수 있습니다. Sdk 설치 경로에 따라 MakeAppx 도구를 사용 하 여 명령 프롬프트에서 이렇게 하려면,이 찾습니다 makeappx.exe 도구를 Windows 10 pc는: x86: C:\Program Files (x86) \Windows Kits\10\bin\x86\makeappx.exe x64: C:\Program Files ( x86) \Windows Kits\10\bin\x64\makeappx.exe
...
이 문제를 해결 하기 위해.msix 사용 하 여.appx 패키지 배포 보다 F5 느슨한 파일 배포 / 합니다.  .msix 만들려면 /.appx 패키지 파일을 위에서 설명한 대로 Windows SDK에서 [MakeAppx](https://docs.microsoft.com/en-us/windows/desktop/appxpkg/make-appx-package--makeappx-exe-) 유틸리티를 사용 합니다. 또는에서 Visual Studio 내에서 응용 프로그램 프로젝트 노드를 마우스 오른쪽 단추로 클릭 하 고 **저장소**선택->**앱 패키지 만들기**.
