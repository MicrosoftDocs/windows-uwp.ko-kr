...
1.  Visual Studio 2015 업데이트 3, Visual Studio 2017 또는 Visual Studio 2019를 설치 합니다.
    ...
    Visual Studio 2017 또는 Visual Studio 2019를 설치 하는 경우 확인을 선택 해야 합니다 **유니버설 Windows 플랫폼 개발** 워크 로드. 경우는 C++ 개발자의는 **요약** 오른쪽 창에서 **유니버설 Windows 플랫폼 개발**를 선택 했는지를  **C++ 유니버설 Windows 플랫폼 도구** 확인란을 선택 합니다. 기본 설치의 일부가 아닙니다.
...
2.  **에 대 한 C++ 및 HTML/Javascript 프로젝트**: 비슷한 경로 따라 C# 로 이동 하지만 프로젝트 속성에는 프로젝트를 **디버깅** 탭을 선택 **원격 컴퓨터** 드롭 다운 목록을 열려면 디버거에서 IP 주소 또는 호스트 이름 입력 콘솔에는 **컴퓨터 이름** 필드 및 선택 **유니버설 (암호화 되지 않은 프로토콜)** 에 **인증 유형** 필드.
