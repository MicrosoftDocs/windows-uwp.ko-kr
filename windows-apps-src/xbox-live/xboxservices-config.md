...
ms.locfileid: "5158432"
---
# <a name="xboxservicesconfig-file-description"></a>XboxServices.config 파일 설명

개발할 때 UWP 게임에 사용 하도록 설정 된 Xbox Live, 프로젝트에 XboxServices.config 파일을 포함 해야 합니다.  이 파일에는 Xbox Live SDK 게임 개발자 센터 앱 및 Xbox Live 서비스 구성와 연결할 수 있습니다. 이 파일 서비스 구성 ID, 타이틀 ID 등과 같은 세부 정보는 JSON 개체를 포함 합니다.
...
>[!NOTE]
> Xbox Live 연결 마법사에서 생성 된 파일, 아래 설명한 것 이상으로 추가 필드가 포함 하지만 서비스에서 사용 되지 않습니다.

다음 필드는 구성 파일의 JSON 개체에 정의 됩니다.
...
**범위** 필드를 지정 하지 않은 경우 다음 범위는 설정 **XboxLiveCreatorsTitle** 필드의 값에 종속 되는 기본값을 다음 표에 설명 된 대로:
...
현재 **범위** 필드를 지정 하는 이유는 이며 경우 하는 Xbox Live 크리에이터 스 프로그램 게임, 게임 친구 목록이 나 소셜 순위표 (친구 범위는 순위표)에 액세스할 필요가 없습니다. 이 경우 XboxServices.config 파일에 다음 줄을 추가할 수 있습니다.
