...
ms.locfileid: "5979350"
---
# <a name="get-started-with-cwinrt"></a>C++/WinRT 시작

사용 하 여 속도를 [C + + WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt),이 항목은 단순한 코드 예제를 기반으로 새 안내 **Windows 콘솔 응용 프로그램 (C + + WinRT)** 프로젝트. 이 항목에서는 또한 표시 하는 방법을 [추가 C + + /winrt 지원을 Windows 데스크톱 응용 프로그램 프로젝트를](#modify-a-windows-desktop-application-project-to-add-cwinrt-support).
...
이 섹션 표시 추가 하는 방법을 C + + /winrt 지원을 포함 될 수 있는 Windows 데스크톱 응용 프로그램 프로젝트를 합니다. 기존 Windows 데스크톱 응용 프로그램 프로젝트를 설정 하지 않은 경우 다음 따르면 이러한 단계와 함께 만드는 첫 번째 하 여. 예를 들어 Visual Studio를 열고 **Visual c + +** 만들기 \> **Windows 데스크톱** \> **Windows 데스크톱 응용 프로그램** 프로젝트.
...
사용자는 영향을 받지 않도록 확인 [새 프로젝트 컴파일되지 않습니다 이유?](/windows/uwp/cpp-and-winrt-apis/faq).

때문에 C + + /winrt가 C + + 17 표준의 기능을 사용, **C/c + +** 프로젝트 속성을 설정 > **언어** > **c + + 언어 표준** 에 *ISO C + + 17 표준 (/ /std: + + 17)*.

### <a name="the-precompiled-header"></a>미리 컴파일된 헤더

이름 바꾸기에 `stdafx.h` 및 `stdafx.cpp` 를 `pch.h` 및 `pch.cpp`각각 합니다. **C/c + +** 프로젝트 속성을 설정 > **미리 컴파일된 헤더** >  *pch.h***미리 컴파일된 헤더 파일** .
...
C + + [WindowsApp.lib](/uwp/win32-and-com/win32-apis) 상위 라이브러리에 연결 해야 하는 /winrt 언어 프로젝션은 특정 Windows 런타임 무료 (비 구성원) 함수 및 진입점에 따라 의존 합니다. 이 섹션에서는 링커 만족 하는 방법을 설명 합니다.

첫 번째 옵션은 Visual Studio에 추가할 프로젝트 모든 C + + /winrt MSBuild 속성 및 대상. 편집 프로그램 `.vcxproj` 파일을 찾고 `<PropertyGroup Label="Globals">` 해당 속성 그룹 내에서 속성을 설정 및 `<CppWinRTEnabled>true</CppWinRTEnabled>`합니다.
