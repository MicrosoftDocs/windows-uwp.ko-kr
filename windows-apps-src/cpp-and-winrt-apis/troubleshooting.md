...
| C + + 링커 생성 "*오류 LNK2019: Unresolved 외부 symbol*"|참조 [이유는 링커 링커에서 "LNK2019: Unresolved 외부 기호" 오류?](faq.md#why-is-the-linker-giving-me-a-lnk2019-unresolved-external-symbol-error)|
| LLVM 및 Clang 도구 체인에서 C +와 함께 사용할 때 오류가 발생 하면 + WinRT 합니다.|C + LLVM 및 Clang 도구 체인은 지원 되지 + 어떻게 사용 내부적으로 에뮬레이션 하려고 하지만 WinRT 하면에서 설명 하는 것 같은 실험을 시도할 수 있습니다 [C + 컴파일하는 데 LLVM/Clang를 사용할 수 있는 + WinRT?](faq.md#can-i-use-llvmclang-to-compile-with-cwinrt).|
| C + + 컴파일러가 프로젝션 된 형식에 대 한 "*적절 한 기본 생성자가 사용할 수 없습니다*"을 생성합니다. | 런타임 클래스 개체를 초기화를 지연 또는 사용 하 고 동일한 프로젝트에서 런타임 클래스를 구현 하려면 하려는 경우 호출 해야 합니다 `nullptr_t` 생성자입니다. 자세한 정보는 [C++/WinRT를 통한 API 사용](consume-apis.md)을 참조하세요. |
| C + + 컴파일러가 생성 "*C3861 오류: 'from_abi': 식별자를 찾을 수 없음*", 및 기타 오류 *base.h*에서 발생 합니다. Visual Studio 2017을 사용 하는 경우이 오류를 볼 수 있습니다 (15.8.0 버전 이상), Windows SDK 버전 10.0.17134.0(windows (Windows 10, 버전 1803)를 대상으로 합니다. | 하나는 이후 (자세한 준수)를 대상으로 버전의 Windows SDK 또는 설정 프로젝트 속성 **C/c + +** > **언어** > **적합성 모드: 아니요** (또한 경우 **허용 /-**  >  **언어** >  **관련 된 추가 옵션****명령줄** 다음 삭제). |
