...
나타날 때까지 기다려야 [멀티 플레이어 관리자를 초기화](play-multiplayer-with-friends.md)한 하 고 [로컬 사용자를 추가 하 여 로비 세션을 생성](play-multiplayer-with-friends.md)했습니다는 `user_added` 이벤트 전에 초대 보내기 시작할 수 있습니다.
...
호출 `invite_friends()` 친구 초대를 위한 표준 Xbox UI를 표시 합니다. 이 친구 선택 플레이어가 또는 최근 플레이어 게임에 초대할 수 있도록 UI를 표시 합니다. 플레이어 수를 확인 되 면 멀티 플레이어 관리자 선택한 플레이어의 초대를 보냅니다.
...
타이틀 보기 온라인 친구 초대를 사용자 지정 TCUI를 구현할 수 있습니다. 게임에서 사용할 수는 `invite_users()` Xbox Live 사용자 Id가 정의한 사용자 집합에 대 한 보내는 방법을 초대 합니다. Xbox UI 주식 대신 고유한 게임에서 UI를 사용 하려는 경우 유용 합니다.
