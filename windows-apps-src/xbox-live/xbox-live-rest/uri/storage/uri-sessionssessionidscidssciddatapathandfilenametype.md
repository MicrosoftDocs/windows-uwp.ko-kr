...
&nbsp;&nbsp;파일을 업로드합니다. 데이터는 메타 데이터와 데이터 전송 되는 일련의 작은 블록에 메타 데이터와 데이터를 전송 하는 여러 블록 업로드 또는 단일 메시지에서 전체 업로드를 업로드할 수 있습니다. 4 개의 메가바이트 보다 작은 파일만 단일 메시지로 보낼 수 있습니다. 다중 블록 업로드 형식 json의 데이터에 대 한 지원 되지 않습니다. 
