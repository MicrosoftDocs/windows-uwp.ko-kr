...
author: KevinAsgari
description: IL2CPP 스크립팅 백 엔드를 사용 하 여 UWP 용 Unity에 Xbox Live 지원 추가 ID@Xbox 관리 파트너 및
...
ms.locfileid: "4339817"
---
# <a name="add-xbox-live-support-to-unity-for-uwp-with-il2cpp-scripting-backend-for-idxbox-and-managed-partners"></a>IL2CPP 스크립팅 백 엔드를 사용 하 여 UWP 용 Unity에 Xbox Live 지원 추가 ID@Xbox 관리 파트너 및

## <a name="overview"></a>개요

IL2CPP Unity에 대 한 Windows 런타임 지원

Unity 5.6f3 릴리스되면서 엔진에 스크립트에서 직접 Windows 런타임 (WinRT) 구성 요소를 사용 하 여 직접 게임 프로젝트에 포함 하 여 개발자 수 있도록 하는 새로운 기능 포함 되어 있습니다. 까지 플러그 인, 또는 dll 기능을 지 원하는 모든 플랫폼 (Xbox Live SDK 포함)에서 UWP 게임 스크립트에서 5.6 개발자가 필요 합니다. 이 새 프로젝션 계층 플러그 인 요구를 제거 하 고 IL2CPP 스크립팅 백 엔드를 선택 하는 게임에만 지원 새롭고 간소화 된 워크플로 소개 합니다.
...
**6) 최신 버전의 Xbox Live WinRT Unity 자산 패키지를 가져올** 에서를 확인할 수 있습니다https://github.com/Microsoft/xbox-live-api/releases
...
**8) 스크립트 (VSTU 3.1 + 설치)와 Visual Studio에서 열기**
...
d) 주식.pfx 파일 Unity 제공한 디스크에서 삭제 하 고, 참조 하는.csproj에서 줄을 제거 하거나 올바른 id 없습니다 또는 Visual Studio에서 프로젝트를 클릭 하 고 "저장"을 선택 하는 오른쪽 \ | "연결 앱 스토어를 사용 하 여" 적절 한.pfx 파일 배치 됩니다.  그런 다음 이동 해야 Unity에 다시 클릭 "빌드 설정"에서 **유니버설 Windows 플랫폼** 플레이어 하 고 Visual Studio의 "연결 앱으로 스토어에 있음" 작업에서 가져온.pfx 파일을 바꾸어야 PFX 단추를 클릭 합니다.
