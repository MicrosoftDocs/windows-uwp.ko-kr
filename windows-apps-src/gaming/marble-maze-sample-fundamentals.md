...
description: 이 문서에서는 Marble Maze 프로젝트;의 기본 특성 설명 예를 들어 Windows 런타임 환경에서 Visual c + +를 사용 하는 방법, 생성 및 구성 하는 방법 및 빌드되면 어떻게 합니다.
...
다운로드 하 고 추출 샘플을 하는 경우 Visual Studio에서 ( **c + +** 폴더)에 **MarbleMaze_VS2017.sln** 파일을 열 수 및 코드 해야 합니다.
...
1. Visual Studio 2017에서 **파일 gt_ 새 gt_ 프로젝트를** 선택 합니다.

2. **새 프로젝트** 창 왼쪽된 사이드바에서에서 **설치 된 gt_ 템플릿 gt_ Visual c + +를**선택 합니다.

3. 중간 목록에서 **DirectX 11 앱 (유니버설 Windows)을**선택 합니다. 이 옵션을 보이지 않으면 없을 경우 설치 필수 구성 요소&mdash; [워크 로드 및 구성 요소 추가 또는 제거 하 여 Visual Studio 2017 수정](https://docs.microsoft.com/visualstudio/install/modify-visual-studio) 추가 구성 요소를 설치 하는 방법에 대 한 정보를 참조 하세요.
...
> **주의**  **/ZW** 옵션은 **/clr**등의 옵션과 호환 되지 않습니다. **/clr**의 경우 동일한 Visual C++ 프로젝트에서 .NET Framework 및 Windows 런타임을 둘 다 대상으로 지정할 수 없다는 의미입니다.
...
메뉴 모음에서 앱에서 코드 분석을 수행 하려면 **빌드 gt_ 솔루션에서 코드 분석 실행**을 선택 합니다. 코드 분석에 대한 자세한 내용은 [코드 분석을 사용하여 C/C++ 코드 품질 분석](https://docs.microsoft.com/visualstudio/code-quality/analyzing-c-cpp-code-quality-by-using-code-analysis)을 참조하세요.
