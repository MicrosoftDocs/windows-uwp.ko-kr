...
Xbox One 아케이드 스틱은 8 방향 디지털 조이스틱, 6 개의 **작업** 단추 (아래 이미지에 A1 a 6으로 표시) 및 두 가지 **특별 한** 단추 (s 1과 S2로 표시 됨); 장착 되어 아날로그 컨트롤 또는 진동을 지원 하지 않는 디지털 전용 입력된 장치 이므로 합니다. Xbox One 아케이드 스틱에 UI 탐색을 지 원하는 데 사용 되는 **보기** 및 **메뉴** 단추 장착 수도 있지만 게임 플레이 명령을 지원 하기 위해 아니며 조이스틱 버튼으로 즉시 액세스할 수 없습니다.
...
UI 탐색 컨트롤러로 아케이드 스틱은 탐색 명령의 [필수 집합](ui-navigation-controller.md#required-set) 를 조이스틱 및 **보기**, **메뉴**, **동작 1**및 **동작 2** 버튼에 매핑합니다.
...
다음 예제에서는 **동작** 1 버튼이 눌 렸 지 결정 합니다.
