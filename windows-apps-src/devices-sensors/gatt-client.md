...
이 문서에서는 유니버설 Windows 플랫폼 (UWP) 앱의 경우 일반적인 GATT 클라이언트 작업에 대 한 샘플 코드와 함께 Bluetooth 일반 특성 (GATT) 클라이언트 Api의 사용을 보여 줍니다.
- 주변 장치에 대 한 쿼리
- 장치에 연결
- 지원 되는 서비스 및 디바이스의 특성을 열거
- 읽기 및 쓰기 특성
- 알림 때 특성 값이 변경에 대 한 구독
...
Bluetooth LE GATT Api 원시 전송에 대 한 액세스 아닌 개체 및 기능을 노출 합니다. GATT Api 개발자는 다음 작업을 수행 하는 기능을 사용 하 여 Bluetooth LE 장치는 사용 하도록 설정할 수도 있습니다.

-   특성 검색 수행
-   읽기 및 쓰기 특성 값
-   특성의 ValueChanged 이벤트에 대 한 콜백 등록

유용한 구현을 만들려면 개발자 이전 알고 있어야 GATT 서비스 및 응용 프로그램을 사용 하려는 특성을 처리 하는 데 API에서 제공 되는 이진 데이터는 변환 되도록 특정 특성 값 사용자에 게 제공 되기 전에 유용한 데이터입니다. Bluetooth GATT API는 Bluetooth LE 장치와 통신하는 데 필요한 기본 요소만 표시합니다. 데이터를 해석하려면 Bluetooth SIG 표준 프로필이나 장치 공급업체에서 구현한 사용자 지정 프로필을 통해 응용 프로그램 프로필을 정의해야 합니다. 프로필은 교환되는 데이터가 어떻게 표현되고 그 데이터를 어떻게 해석해야 하는지를 규정하는 응용 프로그램과 장치 간의 바인딩 계약을 만듭니다.
...
```
반면,는 BluetoothLEDevice에 대 한 모든 참조가 삭제 장치 개체 (및 시스템에 없는 다른 앱에 장치에 대 한 참조 하는 경우)을 트리거합니다 자동 작은 제한 시간 후 분리 합니다. 

```csharp
bluetoothLeDevice.Dispose();
```
앱을 장치에 다시 액세스 해야 하는 경우 간단 하 게 다시 장치 개체를 만들고 (다음 섹션에서 설명)는 특성에 액세스을 트리거합니다 OS가 필요한 경우 다시 연결 합니다. 디바이스가 근처에 DeviceUnreachable 오류를 지 원하는 반환 될 그렇지 않으면 장치에 대 한 액세스를 받게 됩니다.  

## <a name="enumerating-supported-services-and-characteristics"></a>지원 되는 서비스 및 특성에 열거
이제는 BluetoothLEDevice 개체가 장치를 노출 하는 데이터를 검색 하려면 다음 단계가입니다. 이렇게 하려면 첫 번째 단계는 서비스에 대 한 쿼리 합니다. 
...
```
다음 단계를 관심 서비스를 식별 하는 일단 특성에 대 한 쿼리 합니다. 
...
```  
OS는 GattCharacteristic ReadOnly 목록에서 작업 수행할 수 있는 개체를 반환 합니다.

## <a name="perform-readwrite-operations-on-a-characteristic"></a>특성에 대 한 읽기/쓰기 작업 수행

특성은 GATT의 기본 단위 기반 통신 합니다. 장치에서 데이터의 각각 다른 부분을 나타내는 값을 포함 합니다. 예를 들어 배터리 수준 특성 장치의 배터리 수준을 나타내는 값을 가집니다.
...
표시 또는 알림 특성을 지원 하는지 (되도록 특성 속성을 확인). 

> **별도로**: 나타내는 각 값 변경 이벤트는 클라이언트 장치에서 승인을 결합 하기 때문에 보다 안정적으로 간주 됩니다. 알림 아니라 있기 때문에 대부분의 GATT 거래 것 보다 전기를 매우 안정적 많이 됩니다. 어떤 경우에 모든 처리 컨트롤러 계층에서 되므로 앱 참여 하지 않습니다. 단순히 "notifications"로 통칭 하 여 지칭를 하지만 지금까지 알아보았습니다. 

두 가지 방법으로 알림 가져오기 전에 다루려면.
- 클라이언트 특성 구성 설명자 (CCCD) 작성
- Characteristic.ValueChanged 이벤트 처리

쓰기는 CCCD이 클라이언트가 해당 특정 특성 값이 변경 될 때마다 알아야 서버 장치를 알려 줍니다. 이렇게 하려면 다음을 수행합니다. 
