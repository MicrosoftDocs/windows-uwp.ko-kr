...
제거할 항목 목록에서 인덱스를 통해 표시 됩니다 및 쿼리 문자열 매개 변수에서 식별 된 **인덱스**합니다. 인덱스 목록을 쉼표로 구분 해야 하 고 단일 호출에서 100 개의 인덱스를 전달할 수 있습니다. 그러나 인덱스 없음 (빈 쿼리 문자열 매개 변수)를 전달 하는 경우 다음 전체 목록의 콘텐츠를 삭제 (빈 목록을 유지 됩니다 및 버전 번호는 계속 증가). 항목이 제거 되 면 목록 "압축"는 빈 인덱스 순서에서 유지 됩니다. 따라서이 호출은 idempotent가 아닙니다.
 
이 호출은 필요는 **경우-일치: versionNumber** 요청에 포함 될 헤더 위치 **versionNumber** 파일의 현재 버전입니다. 포함 하지 않거나 현재 목록 버전 번호를는 HTTP 412 (전제 조건 실패)와 일치 하지 않습니다 상태 코드가 반환 되 고 응답의 본문에는 현재 버전 번호를 포함 하는 목록의 최신 메타 데이터 포함 됩니다. 이 작업은 서로에서 무시 되는 다른 클라이언트에서 업데이트를 방지 하기 위해 수행 됩니다.
...
| 매개 변수| 형식| 설명| 
| --- | --- | --- | --- | --- | --- | 
| 인덱스| 문자열| 선택 사항. 항목을 제거 하는 위치를 지정 합니다. 지원 되는 값: 0으로 양의 정수로 및 "end"입니다. 기본값: 0.| 
 
예: **인덱스 = 1, 8** 인덱스 1 및 8에서 항목을 제거 합니다. 인덱스는 고유 해야 합니다. 인덱스가 없는 제공 하는 경우 전체 목록이 지워집니다.
