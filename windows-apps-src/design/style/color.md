...
앱에서 자체 색을 정의 하는 것 외에도 **ColorSchemeResources** 태그를 사용 하 여 앱 전체에서 원하는 지역에는 systematized 색을 또한 범위 수 있습니다. 이 API 뿐만 아니라 색을 지정 하 고 테마 많은 컨트롤 그룹을 한 번에 몇 가지 속성과 뿐만 아니라 제공 하면 다른 많은 시스템 혜택을 설정 하 여 사용자 지정 색을 수동으로 정의 게 일반적으로 허용 합니다.

- **ColorSchemeResources** 를 사용 하 여 설정한 모든 색상 고대비 영향을 주지 않습니다.
...
- **ColorSchemeResources** 에 설정 된 색도 해당 시스템 색을 사용 하는 모든 유사한 컨트롤이 나타날 때까지 아래로 계단식 배열 됩니다.
  * 이렇게 하면 사용자는 일관 된 색 스토리 앱에서 브랜드의 모양을 유지 하면서
- 템플릿을 다시 만들 필요 없이 모든 시각적 상태, 애니메이션 및 불투명도 변형 효과

### <a name="how-to-use-colorschemeresources"></a>ColorSchemeResources를 사용 하는 방법

ColorSchemeResources는 리소스 되는 시스템 범위 위치를 알려 주는 API입니다. ColorSchemeResources는 [X:key](https://docs.microsoft.com/windows/uwp/xaml-platform/x-key-attribute)세 가지 옵션 중 하나를 수행 해야 합니다.
- Default
...
X:key 설정 하면 시스템 또는 앱 테마 색 적절 하 게 변경 두 테마에서 다른 사용자 지정 모양을 선택 해야 합니다.

### <a name="how-to-apply-scoped-colors"></a>범위가 지정 된 색을 적용 하는 방법

범위는 **ColorSchemeResources** API xaml에서을 통해 리소스 시스템 색 또는 [테마 리소스](https://docs.microsoft.com/windows/uwp/design/controls-and-patterns/xaml-theme-resources) 라이브러리가 이며 페이지 또는 컨테이너의 범위 내에서 재정의 하는 브러시를 수행할 수 있습니다.

예를 들어, 두 시스템 색- **SystemBaseLowColor** 및 grid에서 내부 **SystemBaseMediumLowColor** 정의 하 고 다음 페이지에서 두 개의 단추를 배치 하는 경우: 해당 그리드 내 1과 하나의 외부:
...
적용 된 새로운 색과 **Button_A** 얻었습니다 미치며 **Button_B** 는 우리의 시스템 기본 단추 처럼 보이는:

![범위가 지정 된 시스템 색 단추](images/color/scopedcolors_cyan_button.png)

그러나는 모든 시스템 색 다른 컨트롤으로도 모두 하므로 **SystemBaseLowColor** 와 **SystemBaseMediumLowColor** 설정 이상의 단추 적용 됩니다. 이 경우 제어 **토글 단추**, **라디오 단추** 및 **슬라이더** 는 또한 영향을 받을 이러한 시스템 색 변경 처럼 해당 컨트롤에 저장 되어야 exampl 그리드 범위 위에 합니다.
범위는 시스템 색 변경을 *하는 단일 컨트롤* 하고자 하는 경우 해당 컨트롤의 리소스 내에서 **ColorSchemeResources** 를 정의 하 여이 수행할 수 있습니다.
...
시스템 색 중첩 가능한 이기도 하 고 **ColorSchemeResources** 앱 레이아웃의 태그 내에 중첩 된 요소가 리소스에 배치 하 여 수행 됩니다.
...
임의 개수의 리소스가 색상 정의 하는 중첩 된 요소에 대해 작동 합니다.
...
첫째, ResourceDictionary 만듭니다. 다음는 ThemeDictionaries 내 **ColorSchemeResources** 배치 하 고 원하는 시스템 색을 재정의 합니다.
