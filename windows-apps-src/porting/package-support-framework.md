...
지원 프레임 워크 패키지를 만들려면는 Microsoft Research (MSR) 하 여 개발 하는 오픈 소스 프레임 워크 및 API 리디렉션 및 연결을 사용할 때 도움이 되는 [자유롭게](https://www.microsoft.com/en-us/research/project/detours) 기술을 활용 하겠습니다.
...
작동 방식은 다음과 같습니다. 응용 프로그램에 적용 하려는 fix(s) 지정 하는 구성 파일을 만들어야 합니다. 그런 다음 패키지 shim 시작 관리자 실행 파일을 수정 합니다.
...
응용 프로그램이 MSIX 컨테이너 내부에서 실행 되기 위해 필요할 때 런타임 관리자는 해결 방법은 적용 합니다.
...
[프로세스 모니터](https://docs.microsoft.com/en-us/sysinternals/downloads/procmon) 는 앱의 파일 및 레지스트리 작업 및 그 결과 관찰 하기 위한 강력한 유틸리티입니다.  응용 프로그램 호환성 문제를 이해 하는 데 도움이 수 있습니다.  프로세스 모니터를 연 다음 필터를 추가 합니다 (필터 >... 필터) 응용 프로그램 실행 파일에서 이벤트에만 포함 하도록 합니다.
...
이 문제를 해당 패키지 경로를.log 파일을 작성 하는 응용 프로그램 실패 했습니다. 이 것이 좋습니다 파일 리디렉션 shim 도움이 될 수 있습니다.
...
예를 들어, 응용 프로그램을 응용 프로그램 실행 파일과 동일한 디렉터리에 있는 로그 파일에 쓰는 경우 [파일 리디렉션 Shim](https://github.com/Microsoft/MSIX-PackageSupportFramework/tree/develop/FileRedirectionShim) 를 사용 하 여 로컬 앱 데이터 저장소 등의 다른 위치에 해당 로그 파일을 만들 수 있습니다.

### <a name="runtime-fixes-from-the-community"></a>커뮤니티에서 런타임 수정

커뮤니티 기여 하려면 [GitHub](https://github.com/Microsoft/MSIX-PackageSupportFramework/tree/develop) 페이지를 검토 해야 합니다. 다른 개발자가 유사한 문제를 해결 한 런타임 수정 공유한 수 있으며 됩니다.
...
.Appx 파일을 이미 있는 경우 패키지에 대 한 준비 영역으로 지원할 수 있는 레이아웃 폴더에 해당 콘텐츠 압축을 푸는 수 있습니다.  이를 수행할 수는 **x64 VS 2017 용 네이티브 도구 명령 프롬프트**, 또는 실행 파일 검색 경로에 SDK bin 경로 사용 하 여 수동으로 합니다.
...
Visual Studio에서 사용자 솔루션 또는 프로젝트 노드를 마우스 오른쪽 단추로 클릭 하 고 Nuget 패키지 관리 명령 중 하나를 선택 합니다.  **Microsoft.PackageSupportFramework** 또는 **PSF** Nuget.org에서 패키지를 찾을 수를 검색 합니다. 그런 다음 설치 합니다.
...
텍스트 편집기에서 패키지 매니페스트를 열고 다음을 설정 합니다 `Executable` 의 특성은 `Application` shim 시작 관리자 실행 파일의 이름 지정할 요소를 합니다.  대상 응용 프로그램의 아키텍처를 알고 있는 경우 적절 한 버전 ShimLauncher32.exe 또는 ShimLauncher64.exe를 선택 합니다.  그렇지 않으면 ShimLauncher32.exe 모든 경우에 적합 합니다.  예를 들면 다음과 같습니다.
...
파일 이름을 만들기 ``config.json``, 해당 파일을 패키지의 루트 폴더에 저장 합니다. 방금 교체 하는 실행 파일을 가리키도록 config.json 파일의 선언 된 응용 프로그램 ID를 수정 합니다. 프로세스 모니터를 사용 하 여에서 얻은 정보를 사용 하 여 있습니다 수도 작업 디렉터리를 설정 뿐 아니라 수.log 패키지 상대 "PSFSampleApp" 디렉터리 아래에 있는 파일을 읽기/쓰기를 리디렉션하 파일 리디렉션 shim 사용 합니다.
...
| 응용 프로그램 | 실행 | 시작 하려는 실행 파일의 패키지 상대 경로입니다. 대부분의 경우, 수정 하기 전에이 값 패키지 매니페스트 파일에서 가져올 수 있습니다. 값은 `Executable` 의 특성은 `Application` 요소. |
...
| shim | 구성 | (선택 사항) Shim 메일 그룹의 동작을 제어 합니다. 이 값의 정확한 형식으로 원하는 각 shim이 "blob" 해석할 수 shim 하 여 shim 별로 달라 집니다. |

`applications`, `processes`, 및 `shims` 키가 배열입니다. 즉,는 둘 이상의 응용 프로그램, 프로세스 및 shim DLL 지정 하려면 config.json 파일을 사용할 수 있습니다.
...
응용 프로그램을 실행 하 고 런타임 수정 적용 된 동작을 관찰 합니다.  진단 및 필요에 따라 패키징 단계를 반복 합니다.

### <a name="use-the-trace-shim"></a>추적 Shim 사용

패키지 된 응용 프로그램 호환성 문제를 진단 하는 대체 기술을 추적 Shim를 사용 하는 것입니다. 이 DLL의 PSF에 포함 된 하 고 앱의 동작을 유사한 프로세스 모니터의 진단 상세 보기를 제공 합니다.  응용 프로그램 호환성 문제를 표시 하도록 특별히 설계 되었습니다.  추적 Shim 사용를 추가 하 DLL 패키지에 다음 조각에 config.json를 추가 하 고 다음 패키지 응용 프로그램을 설치 합니다.
...
기본적으로 추적 Shim "예상"으로 간주할 수도 있는 오류를 필터링 합니다.  예를 들어, 응용 프로그램 무조건적으로 확인 하는 경우 이미 있는 결과 무시 하 고 않고 파일을 삭제 하려고 할 수 있습니다. 이것은 파일 시스템 기능에서 모든 오류를 수신 하도록 옵트인 하는 위의 예제에서는 되므로 일부 예기치 않은 오류 필터링 가져올 수 있는 맬웨어 결과 있습니다. 그 전에 Config.txt 파일에서 읽는 시도 실패 하는 메시지 "파일이 없습니다"에서 알고 있으므로이 수행 합니다. 이 자주 관찰 되 고 하지 일반적으로 예상 된 것으로 간주 되는 실패 합니다. 실제로 예기치 않은 오류를 필터링 하 고 다음 폴백 모든 오류를 여전히 식별할 수 없는 문제가 있으면 시작 가능성이 가장 됩니다.

기본적으로 연결 된 디버거에 추적 Shim에서 출력 전송 가져옵니다. 이 예제에서는 디버거를 첨부할 생각 하 고 출력을 보려면 SysInternals에서 [DebugView](https://docs.microsoft.com/en-us/sysinternals/downloads/debugview) 프로그램을 대신 사용 됩니다. 앱을 실행 한 후 알 수 있습니다 동일한 오류 이전과 마찬가지로 하는 것 가리킵니다 우리는 동일한 런타임 수정 합니다.
...
Visual Studio 디버그 런타임 수정, 런타임 수정, 연장 하거나 처음부터 새로 만들기를 사용할 수 있습니다. 성공 하려면 다음이 작업을 수행 해야 합니다.

> [!div class="checklist"]
> * 패키징 프로젝트 추가
> * 런타임 수정에 대 한 프로젝트 추가
> * 실행 Shim 시작 관리자를 시작 하는 프로젝트 추가
> * 패키징 프로젝트를 구성
...
| ShimLauncher | C + + 빈 프로젝트입니다. 이 프로젝트는 패키지 지원 프레임 워크의 런타임 배포 파일을 수집 하는 위치입니다. 실행 파일을 출력 합니다. 해당 실행 파일은 먼저 솔루션을 시작할 때 실행 됩니다. |
...
**솔루션 탐색기**패키징 프로젝트를 마우스 오른쪽 단추로 클릭 하 고 **편집**선택 다음이 프로젝트 파일의 맨 아래에 추가:
...
새로운 수정 만들려는 경우 추가 하지 마십시오 아무 것도이 프로젝트에 아직. 이 가이드의 뒷부분에서이 프로젝트에 올바른 파일을 추가 도와 드립니다. 지금은 솔루션 설정을 계속 진행 됩니다.

### <a name="add-a-project-that-starts-the-shim-launcher-executable"></a>실행 Shim 시작 관리자를 시작 하는 프로젝트 추가
...
**열기 **일반** 설정 페이지에는 프로젝트에 대 한 속성 페이지의 대상 이름** 속성을 설정 ``ShimLauncher32`` 또는 ``ShimLauncher64`` 응용 프로그램의 아키텍처에 따라 합니다.
...
Shim 시작 관리자 프로젝트 및 데스크톱 응용 프로그램 프로젝트를 선택 하 고 **확인** 단추를 선택 합니다.

![데스크톱 프로젝트](images/desktop-to-uwp/package-project-references.png)

>[!NOTE]
> 응용 프로그램에 소스 코드 없으면 shim 시작 관리자 프로젝트를 선택 하기만 합니다. 구성 파일을 만들 때 실행 파일을 참조 하는 방법을 살펴보겠습니다.
...
Visual Studio에서 f5 키를 눌러 디버거를 시작 합니다.  먼저 시작 하는 차례로 대상 데스크톱 응용 프로그램을 시작 하는 shim 시작 관리자 응용 프로그램입니다.  대상 데스크톱 응용 프로그램을 디버깅 하려면 수동으로 **디버그**선택 하 여 데스크톱 응용 프로그램 프로세스에 연결 해야 합니다->**프로세스에 연결**하 고 다음 응용 프로그램 프로세스를 선택 합니다. .NET 네이티브 런타임 수정 DLL 사용 하 여 응용 프로그램의 디버깅을 허용 하려면 관리 코드와 네이티브 코드 형식 (혼합된 모드 디버깅)를 선택 합니다.  

했으므로이 설정한 후 데스크톱 응용 프로그램 코드 및 런타임 수정 프로젝트에서 줄의 코드로 옆에 중단점을 설정할 수 있습니다. 응용 프로그램에 소스 코드가 없는, 런타임 수정 프로젝트 줄의 코드로 옆에 중단점을 설정할 수 있습니다.

>[!NOTE]
> Visual Studio에서는 가장 간단한 개발 환경을 디버깅, 일부의 제한이 하는 동안이 가이드의 뒷부분에서 적용할 수 있는 디버깅 기술 다른 설명 하겠습니다.

## <a name="create-a-runtime-fix"></a>런타임 수정 만들기

런타임을 해결 하려면, 대체 함수를 작성 하 고 모든 구성 데이터를 포함 하 여 새로운 런타임 수정을 만들 수 있습니다 문제에 대 한 해결 아직 없는 경우는 타당 합니다. 각 부분에 대해 살펴보겠습니다.

### <a name="replacement-functions"></a>대체 기능

먼저, 응용 프로그램 MSIX 컨테이너에서 실행 되 면 호출이 실패 하는 함수를 식별 합니다. 그런 다음 런타임 관리자를 대신 호출 하 고 싶은 대체 함수를 만들 수 있습니다. 이렇게 하면 기회를 동작의 최신 런타임 환경 규칙을 준수 하는 함수를 구현 바꿉니다.

Visual Studio에서 만든이 가이드 앞부분의 런타임 수정 프로젝트를 엽니다.

선언 합니다 ``SHIM_DEFINE_EXPORTS`` 매크로 대 한 include 문을 추가 합니다 `shim_framework.h` 각각의 맨 합니다. 런타임 수정 기능을 추가 하려는 cpp 합니다.
...
```
>[!IMPORTANT]
>있는지 확인 합니다 `SHIM_DEFINE_EXPORTS` 매크로 포함 문을 앞에 나타납니다.

함수의 동일한 서명을 포함 된 함수를 만들 사용자가 수정 하려는 동작 합니다. 다음은 대체 하는 함수 예제는 `MessageBoxW` 함수 합니다.
...
호출 `DECLARE_SHIM` 지도 `MessageBoxW` 함수에 새 대체 함수를 합니다. 응용 프로그램 호출 하려고 할 때의 `MessageBoxW` 함수를 호출 대체 함수 대신 합니다.

#### <a name="protect-against-recursive-calls-to-functions-in-runtime-fixes"></a>런타임 수정 함수를 호출 재귀적으로 으로부터 보호

선택적으로 적용할 수 있습니다는 `reentrancy_guard` 런타임 수정 함수를 호출 재귀적으로 으로부터 보호 하는 함수를 입력 합니다.

에 대 한 대체 함수 발생할 수 있습니다 예는 `CreateFile` 함수입니다. 구현을 호출할 수도 있습니다 합니다 `CopyFile` 함수를 하지만 구현의 `CopyFile` 함수를 호출할 수는 `CreateFile` 함수 합니다. 이에 대 한 호출에서 무한 반복 주기 시킬 수는 `CreateFile` 함수 합니다.
...
런타임 수정 구성 데이터를 추가 하려는 경우 추가할 것을 고려 하는 ``config.json``. 사용할 수 이런 식으로는 `ShimQueryCurrentDllConfig` 를 쉽게 해당 데이터를 구문 분석 합니다. 이 예제에서는 해당 구성 파일에서 문자열 및 부울 값을 구문 분석합니다.
...
먼저, F5 디버깅.appx 패키지를 설치 하는 것이 아니라 패키지 레이아웃 폴더 경로에서 느슨한 파일 배포 하 여 응용 프로그램을 실행 합니다.  일반적으로 레이아웃 폴더가 설치 패키지 폴더와 동일한 보안 제한을가지고 있지 않습니다. 결과적으로, 런타임 수정 적용 하기 전에 패키지 경로 액세스 거부 오류를 재현 하는 불가능할 수도 있습니다 것입니다.

이 문제를 해결 하기 위해 F5 느슨한 파일 배포 보다는.appx 패키지 배포를 사용 합니다.  .Appx 패키지 파일을 만들려면 위에서 설명한 대로 Windows SDK에서 [MakeAppx](https://docs.microsoft.com/en-us/windows/desktop/appxpkg/make-appx-package--makeappx-exe-) 유틸리티를 사용 합니다. 또는에서 Visual Studio 내에서 응용 프로그램 프로젝트 노드를 마우스 오른쪽 단추로 클릭 하 고 **저장소**선택->**앱 패키지 만들기**.

Visual Studio를 사용 하 여 다른 문제 디버거를 통해 시작 된 모든 자식 프로세스에 연결에 대 한 기본 지원을 없는입니다.   이렇게 하면 시작 된 후 Visual Studio에서 수동으로 연결 되어 있어야 합니다 대상 응용 프로그램의 시작 경로 논리를 디버그 하기가 어렵습니다.

이 문제를 해결 하기 위해 하위 프로세스를 지 원하는 디버거를 사용 하 여 연결 합니다.  참고 아닌지 일반적으로 대상 응용 프로그램에 적시에 (JIT) 디버거를 첨부할 수 있습니다.  대부분의 JIT 기술 관련 ImageFileExecutionOptions 레지스트리 키를 통해 대상 앱 대신 디버거 실행 때문입니다.  이 방법을 detouring 메커니즘 ShimLauncher.exe ShimRuntime.dll 대상 앱에 삽입 하는 데 사용 합니다.  WinDbg, [Windows 용 디버깅 도구](https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/index)포함 되어 있으며 [Windows SDK](https://developer.microsoft.com/en-US/windows/downloads/windows-10-sdk)에서 가져온 지원 하위 프로세스를 연결 합니다.  또한 이제 지원 직접 [시작 하 고 UWP 앱을 디버깅](https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/debugging-a-uwp-app-using-windbg#span-idlaunchinganddebuggingauwpappspanspan-idlaunchinganddebuggingauwpappspanspan-idlaunchinganddebuggingauwpappspanlaunching-and-debugging-a-uwp-app)합니다.
...
>[!NOTE]
> [PLMDebug](https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/plmdebug) , 시작 시 앱에 디버거를 추가할도 사용할 수 있으며 [Windows 용 디버깅 도구에](https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/index)에서 포함 되어 있습니다.  그러나는 이제 WinDbg에서 제공 하는 직접 지원 보다 사용 하기 복잡 합니다.
