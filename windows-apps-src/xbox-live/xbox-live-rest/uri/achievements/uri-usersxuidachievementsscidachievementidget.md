...
| 제목| 아니요| 호출 제목입니다.| 인증에 따라 달라 집니다. 2013, 월 1 일 기준 인증 누락 및 공용으로 표시 되지 않은 모든 SCIDs 액세스 거부 따라서 됩니다 때 클레임을 제공 하지 않습니다.| 
| 샌드박스| 아니요| 결과 검색할 샌드박스입니다.| 인증에 따라 달라 집니다. 2013, 월 1 일 기준 인증 제공 하지 않습니다 기본 클레임이 누락 된 경우.| 
...
| 헤더| 유형| 설명| 
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | 
| X RequestedServiceVersion| string| 이 요청 전달 되어야 하는 Xbox LIVE 서비스의 이름/번호를 빌드하십시오. 요청만으로 라우팅되는 서비스의 인증 토큰을 클레임 헤더의 유효성을 확인 한 후에 있습니다. 기본값: 1.| 
| x xbl-계약 버전| string| V1 기본값입니다.| 
| Accept Language| string| 원하는 로캘 및 대체 (예: FR-FR, fr, EN-GB, en-전세계, EN-US) 목록입니다. 지역화 된 문자열을 일치를 찾을 때까지 도전 과제 서비스 목록을 통해 작동 합니다. None 발견 되 면 사용자의 IP 주소에서 제공 되는 사용자 토큰에 정의 된 위치와 일치 하려고 합니다. 여전히 일치 하는 지역화 된 문자열 없음 발견 되 면 제목 개발자/게시자가 제공 하는 기본 문자열 사용 합니다. | 
