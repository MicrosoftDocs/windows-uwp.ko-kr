...
일반적으로 게임에 거의 제어 스레드를 통해 **비동기 작업** 을 실행 하 고 스레드 **완료 콜백에서**사용 하는 경우 결과 반환 합니다.  일부 게임은 힙의 섹션에는 스레드 동기화 하는 단일 스레드 접촉만 되도록 설계 되었습니다. **완료 콜백에서** 스레드에서 호출 되지 **비동기 작업** 의 결과 사용 하 여 공유 상태를 업데이트 하 고 게임 컨트롤 스레드 동기화를 해야 합니다.

**비동기 API** **XblSocialGetSocialRelationshipsAsync()**, **XblProfileGetUserProfileAsync()** 및 **등의 호출을 만들 때 개발자가 직접 스레드 제어를 제공 하는 새로운 비동기 C API를 노출 하는 XSAPI C API XblAchievementsGetAchievementsForTitleIdAsync()**.
...
* **AsyncQueue** **비동기 작업** 을 실행 스레드를 결정할 수 AsyncBlock의 **완료 콜백에서**스레드가 있습니다.
...
* *큐* -핸들 되는 async_queue_handle_t는 **AsyncQueue**를 지정 합니다. 설정 되지 않은 경우 기본 큐 사용 됩니다.

각 비동기 API 호출에 대 한 힙에서 새 AsyncBlock 만들어야 합니다.  AsyncBlock AsyncBlock의 모든 완료 콜백이 호출 되 고 삭제 한 다음 될 때까지 live 해야 합니다.

> [!IMPORTANT]
> **비동기 작업이** 완료 될 때까지 **AsyncBlock** 는 메모리에 남아 있어야 합니다. 동적으로 할당 하는 경우에 AsyncBlock **완료 콜백에서**내에서 삭제할 수 있습니다.

### <a name="waiting-for-asynchronous-task"></a>**비동기 작업** 을 기다리는

**비동기 작업** 을 완료 되는 알 수 다양 한 다양 한 방법:

* AsyncBlock의 **모든 완료 콜백이** 호출 되어
* 완료 될 때까지 기다릴 true로 **GetAsyncStatus** 를 호출 합니다.
* **AsyncBlock** 에서 waitEvent를 설정 하 고 신호 이벤트 대기

**하지만 GetAsyncStatus** 와 waitEvent AsyncBlock의 **완료 콜백에서** AsyncBlock의 **완료 콜백에서** 선택 사항를 실행 한 후 **비동기 작업이** 완료 간주 됩니다.

**비동기 작업이** 완료 되 면 결과 얻을 수 있습니다.

### <a name="getting-the-result-of-the-asynchronous-task"></a>**비동기 작업** 의 결과 가져오는 데 필요한

결과 얻기 위해 대부분의 **비동기 API** 함수 Function\의 해당 \[Name 있는] 결과 함수는 비동기 호출의 결과 받을 수 있습니다. 이 예제 코드 **XblProfileGetUserProfileAsync** 해당 **XblProfileGetUserProfileResult** 함수를 포함 합니다. 이 함수는 함수의 결과 반환 하 고 그에 따라 역할을 사용할 수 있습니다.  검색 결과에 자세한 내용은 각 **비동기 API** 함수의 documention를 참조 하세요.

## <a name="the-asyncqueue"></a>**AsyncQueue**

**AsyncQueue** **비동기 작업** 을 실행 스레드를 결정할 수 AsyncBlock의 **완료 콜백에서**스레드가 있습니다.

*디스패치 모드*를 설정 하 여 이러한 작업을 수행 하는 스레드를 제어할 수 있습니다. 사용할 수 있는 세 가지 디스패치 모드 있습니다.

* *수동* -수동 큐 자동으로 발송 하지 않습니다.  개발자가 원하는 모든 스레드에서 디스패치합니다 수입니다. 특정 스레드에 비동기 호출의 작업 또는 콜백 측면을 할당 하려면이 사용할 수 있습니다.  이 아래에 자세히 설명 합니다.

* *스레드 풀* -디스패치 스레드 풀을 사용 합니다.  스레드 풀 threadpool 스레드를 사용할 수 있게 하는 대로 차례로 큐에서 실행에 대 한 호출을 수행 하는 동시에 호출을 호출 합니다.  사용 하 여 easist 이지만 최소한의 스레드 사용 되는 컨트롤을 제공 합니다.

* *고정 스레드* -QueueUserAPC를 사용 하 여 비동기 큐를 만든 스레드에서 디스패치. 사용자 모드 APC, 대기 중인 스레드 경고 상태의 경우 APC 함수를 호출할 보내지지 않은 합니다. 경고 대기 작업을 수행 하도록 **SleepEx**, **SignalObjectAndWait**, **WaitForSingleObjectEx**, **WaitForMultipleObjectsEx**또는 **MsgWaitForMultipleObjectsEx** 를 사용 하 여 상태가 경고 스레드
...
**workDispatchMode** **completionDispatchMode** 비동기 작업이 완료를 처리 하는 스레드에 대 한 디스패치 모드를 결정 하는 동안 비동기 작업을 처리 하는 스레드에 대 한 디스패치 모드를 결정 합니다.

큐의 ID를 지정 하 여 동일한 큐 종류는 **AsyncQueue** 만드는 **CreateSharedAsyncQueue** 호출할 수 있습니다.
...
> [!NOTE]
> 이 ID 및 디스패치 모드를 사용 하 여 큐가 이미 않으면 참조 됩니다.  그렇지 않은 경우 새 큐 생성 됩니다.

에 **AsyncQueue** 를 간단 하 게 만든 후 작업 및 완료 함수에서 스레딩 제어 하려면 **AsyncBlock** 를 추가 합니다.
**AsyncQueue**사용이 완료 되 면 일반적으로 게임을 종료할 때 닫을 수 있습니다 **CloseAsyncQueue**를 사용 하 여:
...
작업 큐와 완료 큐 수동으로 발송 설정 되는 **AsyncQueue** 생성 된 가정해 되도록 합니다.
...
* *토큰* -콜백을 제거할 **RemoveAsynceCallbackSubmitted** 이후 호출에 사용할 토큰입니다.

예를 들어 다음은 **AddAsyncQueueCallbackSubmitted**에 대 한 호출이입니다.

`AddAsyncQueueCallbackSubmitted(queue, nullptr, HandleAsyncQueueCallback, &m_callbackToken);`

해당 **AsyncQueueCallbackSubmitted** 콜백은 다음과 같이 구현 될 수 있습니다.
...
다음 백그라운드 스레드에서이 세마포에 **DispatchAsyncQueue**호출에 대 한 수신할 수 있습니다.
...
이 가장 좋습니다 Win32 세마포의 개체를 사용 하 여이 시행 합니다.  구현 하는 경우 대신 Win32 이벤트 개체를 사용 하 여 확인 해야 하는 다음 잊지 코드를 사용 하 여 모든 이벤트와 같은:
...
[소셜 C 샘플 AsyncIntegration.cpp](https://github.com/Microsoft/xbox-live-api/blob/master/InProgressSamples/Social/Xbox/C/AsyncIntegration.cpp) 에 예로 비동기 통합에 대 한 모범 사례를 볼 수 있습니다.
