...
> [!NOTE]
> 프로그래밍 방식으로 검토에 응답 하도록이 메서드를 사용 하는 것 외에도 응답할 수 있습니다 또는 리뷰 [파트너 센터를 사용 하 여](../publish/respond-to-customer-reviews.md)입니다.
...
| 값        | 형식   | 설명           |  필수  |
|---------------|--------|-----------------------------|-----|
| ApplicationId | 문자열 |  응답할 리뷰가 포함된 앱의 스토어 ID입니다. Store ID 사용할 수는 [앱 id 페이지](../publish/view-app-identity-details.md) 파트너 센터입니다. 스토어 ID의 예로는 9WZDNCRFJ3Q8이 있습니다.   |  예  |
...
| IsPublic | 부울 |  지정 하는 경우 **true**, 응답이 앱의 저장소를 나열 하는 고객의 검토를 바로 아래에 표시 되 고 모든 고객에 게 표시 됩니다. 지정 하는 경우 **false** 및 사용자 전자 메일 응답 수신을 옵트아웃 옵트인 하지 않은, 응답 전자 메일을 통해 고객에 게 전송 되 고 앱의 스토어 목록에서 다른 고객에 게 표시 되지 것입니다. 지정 하는 경우 **false** 사용자 전자 메일 응답 수신에 참여할 수, 오류가 반환 됩니다.   |  예  |
...
| 성공 | 문자열 | 값 **true**는 응답이 성공적으로 전송되었음을 나타냅니다. 값 **false** 응답이 실패했음을 나타냅니다.    |
