...
**PrebuiltPackage** 요소를 사용 하면 포함 되거나 빌드될 앱 번들 파일에서 참조 될 패키징 레이아웃에 정의 되지 않은 패키지를 추가 합니다. 이 경우 다른 DLC 선택적 패키지가 여기에 포함 되어 기본 앱 번들 파일을 참조 하 고 관련된 집합의 일부로 가질 수 있도록 합니다.
