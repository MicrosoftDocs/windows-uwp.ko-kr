...
반환 된 데이터의 **listCount** 필드 서비스-으로 유지 총 목록에 얼마나 많은 항목이, 여기서 목록의 끝, 이며이 잠재적으로 얼마나 많은 특정 항목이 retu 된 다른 숫자를 확인 하기 위해 사용할 수 있습니다. 요청으로 rned 합니다.
...
| maxItems| 32 비트 부호 있는 정수| 선택 사항입니다. 최대 반환할 항목 수입니다. 기본 25 개 항목 이면 무제한 요청에 지정 됩니다. 서비스에이 값은 최대 두지 않습니다. 값이 목록의 항목 수가 보다 큰, 모든 항목 오류가 없는 반환 됩니다.| 
...
| filterDeviceType| string| 선택 사항입니다. 반환할 장치 종류의 쉼표로 구분 된 목록 지정 (목록에 없는 종류를 반환 합니다). 만 특정 장치 유형 중에서 삽입 된 항목을 반환 반환 집합을 필터링 합니다. 장치 종류의 쉼표로 구분 된 목록 (여러 디바이스 유형에 한 번 호출에서 쿼리할 수 있습니다)이이 필터에 사용 됩니다. 가능한 값: XboxOne, MCapensis, WindowsPhone, WindowsPhone7, 웹, PC, MoLive 합니다. 기본값: null (모든 콘텐츠 형식).| 
