...
요청이 유효한 Xbox LIVE 권한 부여 헤더를 포함 해야 합니다. 호출자에이 리소스에 액세스를 허용 되지 않습니다 서비스는 403 사용할 수 없음 응답을 반환 합니다. 헤더 잘못 되었거나 누락 된 경우 서비스는 401 무단된 응답을 반환 합니다. 
...
Blob 유형에 따라 달라 집니다. 이진 blob 쿼리 매개 변수를 지원 하지 않습니다.
 
| 매개 변수| 유형| 설명| 
| --- | --- | --- | --- | --- | --- | 
|  목록에서 | string| 유형이 json 경우에 사용할 수 있습니다. 응답이 매개이 변수에서 결정 된 대로 특정 속성/값은 JSON 포함만 해야 지정 합니다. "점" (.)를 사용 하 여 하위 속성 및 대괄호 지정 ('[' 및 ']') 배열 인덱스를 지정 합니다. 예를 들어 "array1 [4].prop2" "array1" 배열의 인덱스 4의 "prop2" 속성을 지정 합니다.| 
| customSelector| string| 형식 파일을 구성 합니다. 포함 하는 사용자 지정 가상 노드를 나타냅니다. 타이틀 저장소 구성 형식 파일에 대 한 자세한 정보를 참조 하세요.| 
...
| If-None-Match| 모든 기존 항목 작업을 완료 해야 맞는 ETag를 지정 합니다.| 
...
| 헤더| 설명| 
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | 
| ETag| ETag는 URL에서 찾은 리소스의 특정 버전에는 웹 서버에서 할당 불투명 식별자입니다. 해당 URL에 리소스 콘텐츠가 계속 해 서 변경 다양 하며 ETag 할당 됩니다.| 
