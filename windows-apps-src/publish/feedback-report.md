...
[Microsoft Store Services SDK](http://aka.ms/store-em-sdk)의 피드백 API를 사용하여 고객이 [직접 앱에서 피드백 허브를 시작](../monetize/launch-feedback-hub-from-your-app.md)하도록 할 수 있습니다. 피드백 허브를 지원하는 Windows 10 장치에 앱을 다운로드한 모든 고객은 피드백 허브 앱을 사용하여 해당 피드백을 남길 수 있습니다. 따라서 하지 구체적으로 요청 하 신의 피드백에 대 한 응용 프로그램 내에서 하는 경우에이 보고서에 고객 의견에 나타날 수 있습니다.

피드백 보고서는 피드백을 남겼을 때 각 고객이 장치에 설치한 특정 패키지를 보여준다는 점에서 [패키지 플라이팅](package-flights.md)을 사용할 때 피드백이 도움이 될 수 있습니다.

> [!TIP]
> 지난 30 일간의 reviews (영문), 등급 및 앱의 모든 사용자에 게 피드백에 빠르게 확인, 왼쪽된 탐색 메뉴에서 **사로잡는 구성** 을 확장 하 고 선택 **검토 및 피드백.** 
...
각 메모 옆에 피드백이 남겨진 날짜와 피드백 유형이 표시됩니다. 고객의 시장에 피드백을 제출 하는 고객을 사용 하면 Windows 내부 인의 멤버가 피드백, 해당 장치와 **Windows 내부 인** 유형의 마지막 때 사용 하 여 된 장치에 설치 된 특정 패키지에 대해서도 설명 프로그램입니다.
...
기본적으로 기본 설정 언어로 하지 않고 만든 적용 되는 피드백 하면 변환 됩니다. 필요하면 페이지 필터 근처 오른쪽 위에서 **번역 피드백** 확인란을 선택 취소하여 피드백 번역을 사용하지 않을 수 있습니다.
