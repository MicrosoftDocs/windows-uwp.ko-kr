...
연결된 된 애니메이션에서 요소가 새 보기의 대상 원본 보기의 위치에서 화면을 가로질러 비행 UI 콘텐츠가 바뀌는 동안 두 보기 간에 "계속"를 표시 합니다. 이 보기 간의 공통 콘텐츠가 강조 하 고 전환의 일부로 아름 답 고 역동적인 효과 만듭니다.
...
> [!IMPORTANT]
> 이 기능을 사용 하려면 앱의 대상 버전 RS5 하다 (Windows SDK 버전 10.0.NNNNN.0 (Windows 10, 버전 YYMM) 이상. 구성 속성은 이전 Sdk에서 사용할 수 없습니다. RS5 보다 낮은 최소 버전을 대상으로 지정할 수 있습니다 (사용 하 여 Windows SDK 버전 (Windows 10, 버전 YYMM) 10.0.NNNNN.0 적응 코드나 조건부 XAML입니다. 자세한 내용은 [버전 적응 앱](/debug-test-perf/version-adaptive-apps)을 참조 하세요.

연결 된 애니메이션 추가 기능 RS5부터 흐름 디자인 애니메이션을 제공 하 여 구성을 맞춤식 앞으로 및 뒤로 탐색 페이지.

구성 속성은 ConnectedAnimation에서 설정 하 여 애니메이션 구성을 지정 합니다. (살펴보겠습니다이 예제는 다음 섹션에서.)
...
사용자가 앱 (A)에서 앞으로 연결 된 요소를 물리적으로 "페이지" 나타납니다. 그렇게 요소 z 공간에서 앞으로 이동할 표시 되며 보류 라인 무게의 효과로 약간 삭제 됩니다. 무게의 효과 해결 하려면 요소 속도 획득 하 고 최종 위치에 가속화 합니다. 결과 "scale 및 dip" 애니메이션입니다. |

| [DirectConnectedAnimationConfiguration]() |
| - |
| 사용자를 앱 (a B)에서 뒤로 탐색 하면서, 애니메이션은 더 합니다. 연결 된 요소를 사용 하 여 직접적 입방 형 3 차원 감속/가속 함수 B에서 선형으로 변환 합니다. 뒤로 시각적 어포던스 반환 사용자를 이전 상태로 화면은 가능한 한 빠르게 탐색 흐름의 컨텍스트를 유지 하면서 합니다. |
...
| - | - | - |
| 무게 | 예 | 예* <br/> **이 감속/가속 함수를 사용 하는 기본 변환 a에서 B에 되었으나 "무게 dip" 자체 감속/가속 함수입니다.*  |
...
뒤로 탐색 (Page_A Page_B)에 대 한 동일한 단계를 수행 하지만 원본 및 대상 페이지 취소 됩니다.
...
*조정 된 애니메이션* 은 특별 한 종류의 입구 애니메이션 요소가 화면을 넘어갈 때 연결 된 애니메이션 요소와 함께에서 애니메이션 효과 주는 연결 된 애니메이션 대상 함께 표시 합니다. 조정된 애니메이션은 전환에 더 많은 시각 효과를 추가하여 원본 보기와 대상 보기 사이에 공유되는 상황에 대해 사용자의 관심을 더 많이 끌 수 있습니다. 다음 그림에서 항목의 UI는 조정된 애니메이션을 사용하여 애니메이션 효과를 적용합니다.

조정 된 애니메이션에서는 중력 구성 하는 경우 중력 연결 된 애니메이션 요소와 조정 된 요소에 적용 됩니다. 조정 된 요소는 "급강하" 연결 된 요소와 함께 해야만 요소 진정으로 조정 된 유지 합니다.

**TryStart**의 2-매개 변수 오버로드를 사용하여 연결된 애니메이션에 조정된 요소를 추가합니다. 이 예제에서는 "입장 DescriptionRoot"가 "CoverImage" 라는 연결 된 애니메이션 요소와 동시에 입력 하 라는 그리드 레이아웃이의 조정 된 애니메이션을 보여 줍니다.
...
- 앞으로 탐색에 대 한 [GravityConnectedAnimationConfiguration]() 를 사용 합니다.
