...
ms.locfileid: "4529795"
---
# <a name="text-input-with-the-handwriting-view"></a>필기 보기를 사용 하 여 텍스트 입력

![펜으로 터치할 때 텍스트 상자가 확장](images/pen-input-expand-cropped.gif)
...
XAML 텍스트 입력된 상자 [Windows Ink](../input/pen-and-stylus-interactions.md)를 사용한 펜 입력에 대 한 지원이 포함 되어 있습니다. 사용자가 Windows 펜을 사용 하 여 텍스트 입력된 상자, 텍스트 상자에 별도 입력된 창을 여는 것이 아니라 필기 표면 변환 합니다.
...
필기 보기 기본 텍스트 컨트롤 위에 있는 및 사용자의 필기 기본 설정에 맞게 크기 조정 (장치-> **설정 참조 펜-> 및 Windows Ink 필기-> 텍스트 필드에 직접 쓸 때 글꼴 크기를-> **). 보기는 자동으로 텍스트 컨트롤 및 앱 내 위치를 기준으로 정렬 됩니다.
...
텍스트 제안을 팝업 최상위 잉크 목록이 최상위 후보 올바르지 않은 경우 사용자가 선택할 수 있는 인식 후보를 제공 하기 위해 기본적으로 사용 됩니다.

강력한 이미 응용 프로그램에서 제공 하는 경우 사용자 지정 인식 기능을 사용할 수 있습니다 [AreCandidatesEnabled](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview.arecandidatesenabled) 속성에 기본 제공 제안을 사용 하지 않도록 설정 하려면 다음 예와 같이.
...
이 예제에서는 [TextBox](https://docs.microsoft.com/en-us/uwp/api/windows.ui.xaml.controls.textbox) 의 [TextChanged](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.textbox.textchanged) 이벤트에 대 한 수신 하 고 텍스트 변경의 HandwritingView (또는 그렇지 않은 경우 기본 글꼴)에서 생성 된 경우 사용자의 선택 된 글꼴을 적용 합니다.
...
2. 그런 다음 반복 FindVisualChildByName 호출 하 여 FindInnerTextBox 함수에서 시각적 트리 (AutoSuggestBox부터 시작) 시작 합니다.
...
이 문제를 해결 하는 수 맞추면 UI 요소에 [HandwritingView](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview) 의 PlacementTarget 속성을 설정 합니다.
...
이렇게 하려면 안 보일 해야 하는 UI 요소의 너비를 [HandwritingView](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview) MaxWidth 속성을 바인딩합니다.
...
정보는 팝업 등의 텍스트 입력에 대 한 응답에 표시 되는 사용자 지정 UI가 필기 보기 채워집니다 하지 되므로 해당 UI의 위치를 변경 해야 합니다.
...
모든 XAML 프레임 워크 컨트롤에서와 마찬가지로 특정 요구 사항에 대 한 시각적 구조와 [HandwritingView](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview) 의 시각적 동작을 지정할 수 있습니다.
