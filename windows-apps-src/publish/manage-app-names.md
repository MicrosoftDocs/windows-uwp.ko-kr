...
ms.prod: windows
ms.technology: uwp
keywords: windows 10, uwp, 앱 이름 변경 응용 프로그램 이름, 업데이트 앱 이름, 게임, 제품 이름
...
ms.locfileid: "3849057"
---
# <a name="manage-app-names"></a>앱 이름 관리

모든 앱에 예약한 이름을 볼 **앱 이름 관리** 수 있으며 (다른 언어에 대 한 하거나 앱의 이름을 변경 하려면), 추가 이름을 예약 하 고 필요 없는 이름을 삭제 합니다. [Windows 개발자 센터 대시보드에서](https://partner.microsoft.com/dashboard) 앱에 대 한 왼쪽된 탐색 메뉴에서 **앱 관리** 섹션을 확장 하 여이 페이지를 찾을 수 있습니다.
...
참고 앱에 하나 이상의 예약 된 이름이 있어야 합니다. 완전히 대시보드에서 앱을 제거 (및 앱 용으로 예약한 모든 이름도 해제) 하려면 **앱 개요** 페이지에서 **이 앱 삭제** 를 클릭 합니다. 앱 제출이 진행 중인 경우에는 먼저 해당 제출을 삭제해야 합니다. 참고는 스토어에 앱을 이미 게시 하면 삭제할 수 없습니다 대시보드에서 (하지만 **개요** 페이지에서 **제품 표시/숨기기** 기능을 사용 하 여 숨길 수 있습니다). 
...
- 먼저, 수동으로 또는 Visual Studio를 사용 하 여 새 이름을 사용 하 여 Package.StoreAssociation.xml 파일을 업데이트 (**프로젝트 > 스토어 > 스토어에 연결...에 앱 연결**). 자세한 내용은 [Visual Studio를 사용 하 여 UWP 앱 패키지](../packaging/packaging-uwp-apps.md)를 참조 하세요.
...
> [!TIP]
> 각 앱에 대 한 예약 된 이름을 사용 하 여 대시보드에 표시 됩니다. 앱 이름을 위의 단계를 따랐다면 대시보드에서 새 이름을 사용 하 여 표시를 원하는 경우 ( **앱 이름 관리** 페이지에서 **삭제** 를 클릭) 하 여 원래 이름을 삭제 해야 합니다. 
