...
앱 설치 관리자는 웹 서버에서 직접 앱을 설치할 수 있습니다. 사용자가 호스팅된 웹 앱 패키지를 클릭하면 앱 설치 관리자가 자동으로 호출됩니다. 사용자는 앱 설치 관리자의 앱 정보 보기로 이동한 다음 클릭 한 번으로 앱을 직접 실행하게 됩니다. 
...
> [!NOTE]
> 새로운 기능을 지원하려면 1.0.12271.0 이상의 앱 설치 관리자 버전이 필요합니다.
...
다음은 앱 설치 관리자의 Windows 10 Fall Creator 업데이트 버전과 이전 버전을 빠르게 비교한 것입니다.
