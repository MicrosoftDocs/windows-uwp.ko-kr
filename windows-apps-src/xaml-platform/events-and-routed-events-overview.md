...
프로그래밍 언어로, C#Visual Basic 또는 C++/cx를 사용 하는 경우 UI는 태그 (XAML)로 정의 됩니다. XAML 태그 구문에서 태그 요소와 런타임 코드 엔터티 간에 이벤트를 연결 하는 몇 가지 원칙이 ASP.NET 또는 HTML5와 같은 다른 웹 기술과 비슷합니다.

**참고**  XAML 정의 UI에 대 한 런타임 논리를 제공 하는 코드를 *코드 숨김이* 나 코드 숨김이 라고도 합니다. Microsoft Visual Studio 솔루션 뷰에서는이 관계가 그래픽으로 표시 되 고 코드 숨김이 참조 하는 XAML 페이지와 종속 파일 및 중첩 파일이 됩니다.
...
모든 앱의 코드 및 코드 숨김으로 사용 중인 프로그래밍 언어로 실제 처리기를 작성 합니다. 특성 `Click="ShowUpdatesButton_Click"`를 사용 하 여 XAML이 태그 컴파일 및 구문 분석 되 면, IDE의 빌드 작업에서 XAML 태그 컴파일 단계와 앱이 로드 될 때 최종 XAML 구문 분석 모두 응용 프로그램의 일부로 `ShowUpdatesButton_Click` 라는 메서드를 찾을 수 있는 계약을 만들었습니다. 취소. `ShowUpdatesButton_Click`은 [**클릭**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.primitives.buttonbase.click) 이벤트 처리기에 대 한 호환 메서드 시그니처 (대리자 기반)를 구현 하는 메서드 여야 합니다. 예를 들어이 코드는 `ShowUpdatesButton_Click` 처리기를 정의 합니다.
...
**팁**  Visual STUDIO는 XAML을 편집 하는 동안 이벤트 처리기의 이름을 지정 하 고 처리기 메서드를 정의 하는 편리한 방법을 제공 합니다. XAML 텍스트 편집기에서 이벤트의 특성 이름을 제공 하는 경우 Microsoft IntelliSense 목록에 표시 될 때까지 잠시 기다립니다. 목록에서 **새 이벤트 처리기&gt;&lt;** 을 클릭 하면 요소의 **x:Name** (또는 유형 이름), 이벤트 이름 및 숫자 접미사를 기반으로 메서드 이름을 제안 Microsoft Visual Studio 합니다. 그런 다음 선택한 이벤트 처리기 이름을 마우스 오른쪽 단추로 클릭 하 고 **이벤트 처리기로 이동을**클릭 합니다. 이렇게 하면 XAML 페이지에 대 한 코드 숨겨진 파일의 코드 편집기 보기에 표시 된 대로 새로 삽입 된 이벤트 처리기 정의로 직접 이동 합니다. 이벤트 처리기에는 *보낸 사람* 매개 변수 및 이벤트에서 사용 하는 이벤트 데이터 클래스를 포함 하 여 올바른 서명이 이미 있습니다. 또한 올바른 서명이 있는 처리기 메서드가 코드 숨김으로 이미 존재 하는 경우 해당 메서드의 이름이 **&lt;새 이벤트 처리기&gt;** 옵션과 함께 자동 완성 드롭다운에 표시 됩니다. IntelliSense 목록 항목을 클릭 하는 대신 Tab 키를 바로 가기로 누를 수도 있습니다.
...
**참고**  이벤트 처리기는 C++ 부분 클래스에서 정의 되지 않으며, 헤더에서 전용 클래스 멤버로 선언 됩니다. C++ 프로젝트에 대 한 빌드 작업은 XAML 형식 시스템 및에 대 한 C++코드를 사용 하는 모델을 지 원하는 코드를 생성 합니다.
...
**비동기** 패턴을 사용 하 여 사용자 상호 작용 이벤트를 처리 하는 예제는 [파일 액세스 및 선택](https://docs.microsoft.com/previous-versions/windows/apps/jj655411(v=win.10)) ([사용 하 여 C# 첫 번째 Windows 런타임 앱 만들기 또는 Visual Basic](https://docs.microsoft.com/previous-versions/windows/apps/hh974581(v=win.10)) 시리즈의 일부)을 참조 하세요. [C에서 비동기 Api 호출)도 참조 하세요.
...
에서 C#구문은`+=`연산자를 사용 하는 것입니다. 연산자의 우변에 있는 이벤트 처리기 메서드 이름을 참조 하 여 처리기를 등록 합니다.

코드를 사용 하 여 런타임 UI에 표시 되는 개체에 이벤트 처리기를 추가 하는 경우 일반적인 방법은 개체 수명 이벤트 또는 콜백에 대 한 응답에 이러한 처리기를 추가 하는 것입니다 (예: [**로드**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.frameworkelement.loaded) 된 또는 [**OnApplyTemplate**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.frameworkelement.onapplytemplate)). 관련 개체는 런타임에 사용자가 시작한 이벤트에 대해 준비 됩니다. 이 예제에서는 페이지 구조의 XAML 개요를 표시 한 다음 개체에 이벤트 C# 처리기를 추가 하기 위한 언어 구문을 제공 합니다.
...
자세한 구문이 **있습니다  .** 2005에서 대리자 C# 유추 라는 기능을 추가 했습니다 .이 기능을 사용 하면 컴파일러가 새 대리자 인스턴스를 유추 하 고 보다 단순한 이전 구문을 사용할 수 있습니다. 자세한 구문은 이전 예제와 기능적으로 동일 하지만 새 대리자 인스턴스를 등록 하기 전에 명시적으로 만들어 대리자 유추를 활용 하지 않습니다. 이 명시적 구문은 일반적이 아니지만 일부 코드 예제에서 볼 수 있습니다.
...
Visual Basic 구문에는 두 가지 가능성이 있습니다. 하나는 C# 구문을 병렬 처리 하 고 인스턴스에 직접 처리기를 연결 하는 것입니다. 이렇게 하려면 **AddHandler** 키워드와 처리기 메서드 이름을 역참조 하는 **AddressOf** 연산자도 필요 합니다.
...
/Cx에서 C++이벤트 처리기를 제거 해야 하는 경우`+=`이벤트 처리기 등록의 반환 값에서 받은 등록 토큰이 필요 합니다. 이는 C++/cx 구문에서 `-=` 등록 취소의 오른쪽에 사용 하는 값이 메서드 이름이 아닌 토큰 이기 때문입니다. /Cx C++의 경우/cx 생성 코드에서 C++토큰을 저장 하지 않기 때문에 XAML 특성으로 추가 된 처리기를 제거할 수 없습니다.

## <a name="routed-events"></a>라우트된 이벤트

Windows 런타임 C#, Microsoft Visual Basic 또는 C++/CX는 대부분의 UI 요소에 있는 이벤트 집합에 대해 라우트된 이벤트의 개념을 지원 합니다. 이러한 이벤트는 입력 및 사용자 상호 작용 시나리오에 사용 되며 [**UIElement**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.UIElement) 기본 클래스에서 구현 됩니다. 라우트된 이벤트의 입력 이벤트 목록은 다음과 같습니다.
...
DHTML (Dynamic HTML) 또는 HTML5와 같은 웹 기술을 알고 있는 경우 이미 *버블링* 이벤트 개념에 대해 잘 알고 있을 수 있습니다.
...
- C# 및 Visual Basic CLR 이벤트를 정의 합니다. 사용자 지정 접근자를 사용 하지 않는 경우 (**추가**/**제거**) 표준 .net 이벤트 패턴을 사용할 수 있습니다. 추가 팁:
    - 이벤트 처리기의 경우 Windows 런타임 일반 이벤트 대리자 [**eventhandler<T>** ](https://docs.microsoft.com/uwp/api/windows.foundation.eventhandler)에 대 한 기본 제공 변환이 있으므로 [**system.object<TEventArgs>** ](https://docs.microsoft.com/dotnet/api/system.eventhandler-1) 를 사용 하는 것이 좋습니다.
...
- /Cx C++의 경우 [이벤트 (C++/cx)](https://docs.microsoft.com/cpp/cppcx/events-c-cx)를 참조 하세요.
