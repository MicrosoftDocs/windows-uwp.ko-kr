...
- **리뷰 내용**: **등급으로 내용을 검토 하**는 유일한 등급 검토 콘텐츠로 표시 됨을 의미 하는 기본 설정은입니다. **모든** 모든 평가 표시를 모든 리뷰 텍스트를 포함 하지 않는 비롯 선택할 수 있습니다. 참고 **평점 분석** 차트 선택에 관계 없이 모든 리뷰를 항상 표시 됩니다.
...
Note **내용을 검토** 페이지 필터에서 **사용 하 여 평가** 선택한 경우에이 차트 항상에 모든 리뷰를 합니다.
...
위에서 언급 했 듯이 때 리뷰를 검색, 고객이 남긴 원래 텍스트 검색 됩니다 (및 텍스트는 번역) **리뷰 번역** 상자가 선택 되어 있는 경우에 있습니다.


## <a name="responding-to-customer-reviews"></a>고객 리뷰에 응답

여러 고객 리뷰에 응답을 보낼 Microsoft 스토어 개발자 센터 대시보드에서, [Microsoft Store 리뷰 API](../monetize/submit-responses-to-app-reviews.md), 또는 [개발자 센터 앱](https://www.microsoft.com/store/apps/dev-center/9nblggh4r5ws) 을 사용할 수 있습니다. 자세한 내용은 [고객 리뷰에 응답](respond-to-customer-reviews.md)을 참조하세요.
