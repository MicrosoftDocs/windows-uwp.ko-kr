...
Windows Device Portal은 PC의 웹 브라우저에서 연결할 수 있는 장치에서 웹 서버. 디바이스에 웹 브라우저가 있는 해당 장치에서 브라우저를 사용 하 여 있습니다 로컬로 연결할 수도 수 있습니다.
...
1. (설정 앱의 구성) 디바이스에서 개발자 모드 및 디바이스 포털을 사용 합니다.
...
앱 관리자는 설치/제거 및 관리 기능을 앱 패키지 및 번들의 호스트 디바이스에서.
...
- **실행 중인 앱**: 현재 실행 되 고 필요에 따라 닫으라는 하는 앱에 대 한 정보를 가져옵니다.
...
이 페이지를 사용 하면 확인 하 고 모든 테스트용으로 로드 된 앱에서 저장 된 파일을 조작할 수 있습니다. [앱 파일 탐색기를 사용 하 여](https://blogs.windows.com/buildingapps/2016/06/08/using-the-app-file-explorer-to-see-your-app-data/) 블로그 파일 탐색기 및 그 사용 방법에 대 한 자세한 내용은 게시물을 참조 하세요. 
...
추적을 중지하려면 **중지**를 클릭합니다. 이 페이지에 추적 파일 될 때까지 계속 (합니다. ETL) 다운로드를 완료 했습니다.
...
장치 관리자 페이지에 디바이스가 연결 된 모든 주변 장치를 열거 합니다. 각각의 속성을 보려면 설정 아이콘을 클릭할 수 있습니다.
