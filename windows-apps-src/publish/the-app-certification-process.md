...
ms.locfileid: "2841167"
---
# <a name="the-app-certification-process"></a>앱 인증 프로세스

앱 제출 생성을 완료하고 **Store에 제출**을 클릭하면 제출이 인증 단계로 들어갑니다. 이 프로세스는 대개 몇 시간 내에 완료되지만 경우에 따라 영업일 기준으로 3일까지 걸릴 수 있습니다. 제출을 인증 (영문)를 통과 한 후 새 전송, 또는 패키지에 대 한 업데이트 된 전송 변경 내용으로 응용 프로그램의 목록을 확인 하는 고객을 위한 24 시간까지 걸릴 수 있습니다. 업데이트 내용을 세부 정보를 나열 하는 저장소만 변경 되 면 1 시간 보다 작은에 게시 하는 프로세스 완료 됩니다.  제출을 게시 된 및 대시보드의 앱의 상태에 **저장소**될 때 알림을 받을 수 있습니다.
...
앱 인증 (영문)를 전달 하는 경우 **게시** 프로세스를 진행할 준비가 됩니다.

- 제출을 (기본 옵션) 최대한 빨리 게시 해야 표시 했을 때, 한 경우에 게시 하는 프로세스는 즉시 시작 됩니다.
- 이것은 처음으로 응용 프로그램을 게시 한 하 고 [일정](configure-precise-release-scheduling.md#release) 섹션에서 **출시 날짜** 를 지정, 앱 **출시 날짜** 선택에 따라 사용할 수 있습니다.
- [게시 유지 옵션을](manage-submission-options.md#publishing-hold-options) 사용 하는 것을 해제 될 특정 날짜까지 지정 하 했을 때, 경우 **변경 릴리스 날짜**를 선택 하지 않은 경우 게시 프로세스를 시작 하려면 다음 해당 날짜까지 기다릴 게 했습니다.
- 제출을 수동으로 게시 하려면 지정 하는 [게시 유지 옵션을](manage-submission-options.md#publishing-hold-options) 사용 했을 때, 선택 **지금 게시** (또는 **변경을 릴리스 날짜** 를 선택 하는 특정 날짜를 선택) 때까지 게시 프로세스를 시작 되지 않습니다 했습니다.
...
새 응용 프로그램 및 응용 프로그램의 패키지에 대 한 변경 내용을 포함 하는 업데이트에 대 한 24 시간 내에서 게시 하는 프로세스를 완료 됩니다. 만 세부 정보를 나열 하는 저장소와 같이 옵션을 변경 하 여 앱의 패키지를 변경 하지 않고 업데이트에 대 한 게시 하는 프로세스에는 1 시간 보다 작은 수행 됩니다.

앱 게시 단계에서 이지만, 앱의 전송에 대 한 상태 열에서 **자세한 정보 표시** 링크 통해 새 패키지 및 세부 정보를 나열 하는 저장소에 지원 되는 운영 체제 버전의 각 고객에 게 사용할 수 있는 시기를 알 수 있습니다. 아직 완료되지 않은 단계들은 **보류 중**으로 표시됩니다. 앱 게시 단계는 프로세스가 완료 될 때까지 유지 됩니다, 그리고 새 패키지는 누구나 및/또는 세부 정보 나열 하는 앱의 잠재 고객에 게는 모든 사용할 수 있습니다.
