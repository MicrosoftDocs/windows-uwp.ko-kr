---
Description: 다음 문서에서는 타일 콘텐츠 내의 모든 속성과 요소를 설명합니다.
title: 타일 콘텐츠 스키마
...
| 속성 | 형식 | 필수 | 설명 |
|---|---|---|---|
| **Visual** | [ToastVisual](#tilevisual) | true | 타일 메시지의 시각적 개체 부분을 설명합니다. |
...
| 속성 | 형식 | 필수 | 설명 |
|---|---|---|---|
| **자식** | IList<ITileBindingContentAdaptiveChild> | false | 인라인 시각 요소입니다. [AdaptiveText](#adaptivetext), [AdaptiveImage](#adaptiveimage), [AdaptiveGroup](#adaptivegroup) 개체를 추가할 수 있습니다. 하위 항목은 수직 StackPanel 방식으로 표시됩니다. |
...
| 속성 | 형식 | 필수 |설명 |
|---|---|---|---|
| **아이콘** | [TileBasicImage](#tilebasicimage) | true | 최소한 데스크톱 및 모바일, 작은 크기와 중간 크기 타일을 모두 지원하려면 투명 및 흰색만 허용하는 PNG 형식의 정사각형 가로세로 비율 이미지를 200x200 해상도로 제공합니다. 자세한 내용은 다음을 참조하세요. [특별 한 타일 템플릿](../tiles-and-notifications/special-tile-templates-catalog.md)합니다. |
...
1511 새로운 기능: 보통, 지원 폭이 크고 (데스크톱 및 모바일). 이전에는 모바일 및 중간 크기, 와이드 타일 전용이었습니다.

| 속성 | 형식 | 필수 |설명 |
|---|---|---|---|
| **Images** | IList<[TileBasicImage](#tilebasicimage)> | true | 원으로 롤링할 이미지입니다. |
...
| 속성 | 형식 | 필수 |설명 |
|---|---|---|---|
| **Images** | IList<[TileBasicImage](#tilebasicimage)> | true | 최대 12개 이미지가 제공될 수 있으며(모바일은 최대 9개까지만 표시) 슬라이드 쇼에 사용됩니다. 12개가 넘는 이미지를 추가하면 예외가 throw됩니다. |
