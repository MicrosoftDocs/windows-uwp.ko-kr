...
**개인 대상** 옵션은 모든 OS 버전을 대상으로 하는 앱에서 사용이 가능하지만, 테스터가 Windows 10, 버전 1607 이상(Xbox One 포함)을 실행해야 하고 사용자가 제공하는 이메일 주소에 연결된 Microsoft 계정을 통해 로그인을 해야 합니다.
...
이 옵션은 광범위하게 사용할 수 있는 제출을 먼저 게시하지 않고도 특정 테스터 그룹으로만 앱 배포를 제한할 수 있는 또 다른 방법을 제공합니다. 그러나 개인 대상 옵션과 달리, 직접 링크가 있는 경우에는 누구라도 앱 목록을 볼 수 있습니다. 제출에 있어 기밀성이 중요한 경우에는 대신에 개인 대상에 게시하는 것이 좋습니다.
