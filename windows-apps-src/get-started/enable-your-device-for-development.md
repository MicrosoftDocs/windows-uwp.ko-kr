...
> [!NOTE]
> 디바이스가 조직의 소유인 경우 조직에서 일부 옵션을 사용하지 못하게 설정해 놓았을 수도 있습니다.
...
-   *Windows 스토어 앱*이 기본 설정입니다. 사용하는 앱이 개발용 앱이 아니거나 회사에서 지급한 특수한 내부 앱을 사용하는 경우 이 설정을 계속 활성 상태로 유지하세요.
...
-   데스크톱 디바이스 패밀리에서:
    ...
    개발자 모드를 사용하여 Visual Studio에서 앱을 개발하고 디버그합니다. 앞에서 설명한 대로 개발자 모드를 사용하도록 설정하지 않으면 Visual Studio에서 메시지가 표시됩니다.

    가을 크리에이터 업데이트 이전 PC에서는 이를 통해 Linux용 Windows 하위 시스템을 사용할 수 있습니다. 자세한 내용은 [Windows의 Ubuntu 기반 Bash 정보](https://msdn.microsoft.com/commandline/wsl/about)를 참조하세요.  가을 크리에이터 업데이트부터는 더 이상 WSL에 개발자 모드가 필요 없습니다.  

-   모바일 장치 패밀리에서:

    개발자 모드를 사용하여 Visual Studio에서 앱을 배포하고 장치에서 디버그합니다.

    파일을 탭하여 메일로 전송되거나 SD 카드에 있는 .appx를 설치할 수 있습니다. 출처가 확인되지 않은 앱은 설치하지 마세요.
...
이 이미지는 Windows 10의 모바일 장치 패밀리에 제공되는 개발자 기능을 보여 줍니다.

![모바일 장치에 제공되는 개발자 모드 옵션](images/devmode-mob-options.png) 
...
디바이스 검색을 사용하는 경우 mDNS를 통해 디바이스가 네트워크의 다른 디바이스에 표시되도록 허용합니다.  또한 이 기능을 통해 이 디바이스에 페어링할 SSH PIN을 얻을 수 있습니다.  
...
-   Windows 10 버전 1511부터 [장치 포털](../debug-test-perf/device-portal-desktop.md)을 사용하여 브라우저에서 Windows 10 버전 1511 이상을 실행하는 모바일 장치로 배포할 수 있습니다. Device Portal의 **[앱](../debug-test-perf/device-portal.md#apps)** 페이지를 사용하여 앱 패키지(.appx)를 업로드하고 디바이스에 설치할 수 있습니다.
