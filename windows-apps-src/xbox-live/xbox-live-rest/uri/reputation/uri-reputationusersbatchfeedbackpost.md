...
| evidenceId| string| 제출한 피드백의 증거로 사용할 수 있는 리소스의 ID입니다. 예를 들어 비디오 파일의 ID입니다.| 
...
> [!NOTE] 
> 일괄 처리 피드백을 제출 하려면 요청을 사용 하 여 파트너 청구 인증서 보내야 합니다. 
...
| X RequestedServiceVersion| 101| 이 요청은 전송 Xbox LIVE 서비스의 이름/번호를 빌드하십시오. 요청만으로 라우팅되는 인증 토큰의 클레임 헤더의 유효성을 확인 한 후 서비스는 합니다.| 
