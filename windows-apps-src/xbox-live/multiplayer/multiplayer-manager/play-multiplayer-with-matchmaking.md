...
여기에 프로세스의 순서도 피: [순서도-스마트 매치를 사용 하 여 멀티 플레이어 게임을 플레이](mpm-flowcharts/mpm-play-with-smartmatch-matchmaking.md)합니다.
...
초대를 후 경우를 수락한 하 고 게임을 시작 하려면 호스트를 사용할 수 있습니다 스마트 충분 한 플레이어를 사용 하는 기존 게임 슬롯의 모든 구성원에 대 한 로비 세션에서 호출 하 여 중 하나를 `find_match()`, 또는 새 게임 세션 만들기  호출 하 여 동일한 게임 유형의 일치 하는 항목을 찾고 다른 사람과 모든 멤버 열기 지점 위쪽으로 채우기 및 로비 세션에서이 포함 된 `join_game_from_lobby()` 뒤에 `set_auto_fill_members_during_matchmaking()`.
