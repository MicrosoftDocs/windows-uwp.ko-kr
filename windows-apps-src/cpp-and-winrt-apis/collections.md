...
ms.locfileid: "7437462"
---
# <a name="collections-with-cwinrt"></a>C++/WinRT로 작성된 컬렉션

내부적으로 Windows 런타임 컬렉션에는 많은 복잡 한 이동 부분이 있습니다. 하지만 함수 및 기본 클래스에는 Windows 런타임 함수에 컬렉션 개체를 전달 하거나 고유한 컬렉션 속성 및 컬렉션 형식과 구현 해야 경우 [C + + WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt) 하도록 지 원하는 합니다. 이러한 기능 손, 아웃 복잡성을 가져와 시간과 노력에 많은 오버 헤드를 저장 합니다.

[**IVector**](/uwp/api/windows.foundation.collections.ivector_t_) 는 요소의 모든 임의 액세스 수집에 의해 구현 된 Windows 런타임 인터페이스입니다. **IVector** 를 구현 하는 것을 [**IIterable**](/uwp/api/windows.foundation.collections.iiterable_t_), [**IVectorView**](/uwp/api/windows.foundation.collections.ivectorview_t_)및 [**IIterator**](/uwp/api/windows.foundation.collections.iiterator_t_)구현 해야 합니다. *필요한* 사용자 지정 컬렉션을 입력 하는 경우에 작업을 많이입니다. 하지만 데이터 **std:: vector** (또는 **std:: map**또는 **std::unordered_map**)에 있고 Windows 런타임 API로 전달 하는 모든 작업을 수행 하려는 경우 다음 하려는 가능 하면 해당 수준의 작업을 수행 하지 않는. 때문에 ** 가능한 방지 및 C + + WinRT 효율적이 고 적은 노력으로 컬렉션을 만들 수 있습니다.

참고 [XAML 항목 컨트롤, 바인딩 C + + /winrt 컬렉션](binding-collection.md)합니다.

> [!NOTE]
> Windows SDK 버전 10.0.17763.0 (Windows 10, 버전 1809)를 설치 하지 않은 하거나 나중에 다음 필요가 함수 및이 항목에서 설명 하는 기본 클래스에 대 한 액세스 합니다. 대신, 대신 사용할 수 있는 관찰 가능한 벡터 템플릿의 목록에 대 한 [이전 버전의 Windows SDK 있는지](/uwp/cpp-ref-for-winrt/single-threaded-observable-vector#if-you-have-an-older-version-of-the-windows-sdk) 를 확인 합니다.
...
위의 코드 예제에서 알 수 있듯이 컬렉션을 만든 후 요소를 추가, 반복을 API에서 수신 하는 모든 Windows 런타임 컬렉션 개체와 마찬가지로 일반적으로 개체를 처리 합니다. 컬렉션 변경 불가능 보기를 필요한 경우 표시 된 대로 [**IVector::GetView**](/uwp/api/windows.foundation.collections.ivector-1.getview)호출할 수 있습니다. 위에 표시 된 패턴&mdash;만들기 및 사용 하는 컬렉션의&mdash;API에서 데이터를 가져오거나 데이터를 전달 하려는 간단한 시나리오에 적합 합니다. **IVector**또는 **IVectorView**를 전달할 수, [**IIterable**](/uwp/api/windows.foundation.collections.iiterable_t_) 는 예상 되는 곳입니다.

위의 코드 예제에서는 **init_apartment** 호출 초기화 COM; 기본적으로 다중 스레드 아파트에서 됩니다.
...
**Winrt::single_threaded_vector**사용자 데이터가 포함 된 임시 개체를 전달할 수와 마찬가지로 `coll1`위의 합니다. (가정 하면 없습니다 수 액세스 다시) **std:: vector** 를 이동할 수 또는 함수로 합니다. 두 경우 모두 *rvalue* 는 함수에 전달 합니다. 컴파일러가 효율적일 데이터를 복사 하 고 있습니다. *Rvalue*에 대해 자세히 알고 싶다면 [값 범주 및 참조를](cpp-value-categories.md)참조 하세요.
...
위 예제에서 만들 *수 있는* 컬렉션 XAML 항목 컨트롤;에 바인딩할 수 하지만 컬렉션은 관찰 가능 합니다.
...
개체 [**IObservableVector**](/uwp/api/windows.foundation.collections.iobservablevector_t_)으로 돌아가고 인터페이스를 통해 사용자 (또는 연결 된 컨트롤) 호출 반환 된 개체의 함수 및 속성입니다.
...
모든 동시성을 제공 하지 않는 나타냅니다는 "단일 스레드" 이러한 함수 이름에&mdash;즉, 달라도 스레드로부터 안전 합니다. 이러한 함수에서 반환 된 개체 모두 agile 때문에 스레드를 언급 하는 것은 아파트를 상관이 없습니다 (참조 [Agile 개체 C + + WinRT](agile-objects.md)). 개체는 단일 스레드는 방금 것입니다. 및 완전히 데이터는 한 가지 방법은 또는 다른 응용 프로그램 이진 인터페이스 (ABI) 간에 전달 하려는 경우에 적합 합니다.

## <a name="base-classes-for-collections"></a>컬렉션에 대 한 기본 클래스

전체 유연성을 위해 사용자 지정 컬렉션을 직접 구현 하려는 경우 노력을 수행 하지 않는 하려는 합니다. 예를 들어,이 사용자 지정 벡터 보기 이러한 모습 *C +의 도움 없이 + /winrt의 기본 클래스*합니다.
...
대신, 훨씬 쉽게 [**winrt::vector_view_base**](/uwp/cpp-ref-for-winrt/vector-view-base) 구조체 템플릿에서 사용자 지정 벡터 보기를 파생 하 고 데이터를 보유 하 고 컨테이너를 노출 하는 **get_container** 함수를 구현할 수는 있습니다.
