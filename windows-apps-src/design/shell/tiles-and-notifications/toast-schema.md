...
| 속성 | 형식 | 필수 |설명 |
|---|---|---|---|
| **텍스트 모드** | 문자열 또는 [BindableString](#bindablestring) | false | 표시할 텍스트입니다. 데이터 바인딩 지원은 크리에이터 업데이트에 추기되었지만 최상위 텍스트 요소에 대해서만 작동합니다. |
...
| **Value** | 이중 또는 [AdaptiveProgressBarValue](#adaptiveprogressbarvalue) 또는 [BindableProgressBarValue](#bindableprogressbarvalue) | false | 진행률 표시줄의 값을 가져오거나 설정합니다. 데이터 바인딩을 지원합니다. 기본값은 0입니다. |
...
| 속성 | 형식 | 필수 | 설명 |
|---|---|---|---|
| **Value** | double | false | 완료 비율을 나타내는 (0.0-1.0) 값을 가져오거나 설정합니다. |
| **IsIndeterminate** | bool | false | 진행률 표시줄의 미확정 여부를 나타내는 값을 가져오거나 설정합니다. 이 값이 true이면 **Value**가 무시됩니다. |
...
| 속성 | 형식 | 필수 | 설명 |
|---|---|---|---|
| **콘텐츠** | string | true | 필수. 단추에 표시할 텍스트입니다. |
| **인수** | string | true | 필수. 사용자가 이 단추를 클릭할 경우 나중에 앱에서 받을 앱에서 정의한 인수 문자열입니다. |
...
| 속성 | 형식 | 필수 | 설명 |
|---|---|---|---|
| **콘텐츠** | string | true | 필수. 표시할 텍스트입니다. |
| **인수** | string | true | 필수. 사용자가 메뉴 항목을 클릭할 때 활성화 후 앱에서 나중에 검색할 수 있는 앱에서 정의한 인수 문자열입니다. |
...
* [빠른 시작: 로컬 알림 및 핸들 활성화 보내기](https://blogs.msdn.microsoft.com/tiles_and_toasts/2015/07/08/quickstart-sending-a-local-toast-notification-and-handling-activations-from-it-windows-10/)
