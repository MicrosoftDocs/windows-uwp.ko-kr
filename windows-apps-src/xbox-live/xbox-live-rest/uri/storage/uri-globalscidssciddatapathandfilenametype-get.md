...
| 유형| 문자열| 데이터의 형식입니다. 가능한 값은: 이진 파일, 구성 또는 json 합니다.| 
...
| 매개 변수| 유형| 설명| 
| --- | --- | --- | --- | --- | --- | 
|  목록에서 | string| 형식이 json 때에 사용할 수 있습니다. 응답이 매개이 변수에 의해 결정 된 대로 특정 속성/값은 JSON 포함만 해야 지정 합니다. "점" (.)를 사용 하 여 하위 속성 및 대괄호 지정 ('[' 및 ']') 배열 인덱스를 지정할 수 있습니다. 예를 들어 "array1 [4].prop2" "array1" 배열의 인덱스 4의 "prop2" 속성을 지정 합니다.| 
| customSelector| string| 형식 파일을 구성 합니다. 어떤 사용자 지정 가상 포함할 노드를 나타냅니다. 타이틀 저장소 구성 형식 파일에 대 한 자세한 정보를 참조 하세요.| 
...
| 헤더| 설명| 
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | 
| ETag| ETag는 URL에서 찾은 리소스의 특정 버전 웹 서버에서 할당 불투명 식별자입니다. 해당 URL에 리소스 콘텐츠가 계속 해 서 변경 새롭고 다른 ETag 할당 됩니다.| 
| 콘텐츠 범위| 이 부분 다운로드 용인 경우이 헤더 다운로드 된 바이트의 범위를 지정 합니다.| 
