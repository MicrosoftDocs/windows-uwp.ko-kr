...
Windows 10에서 앱 패키지, 앱 번들 또는 [파트너 센터](https://partner.microsoft.com/dashboard)에 완전 한 앱 패키지 업로드 파일을 제출할 수 있습니다. 이러한 옵션 중 앱 패키지 업로드 파일을 제출 하면 최상의 환경을 제공 됩니다.
...
- **앱 패키지 업로드 파일 (.appxupload 또는.msixupload)**  
    다양한 프로세서 아키텍처를 지원하기 위해 여러 앱 패키지 또는 앱 번들을 포함할 수 있는 단일 파일. 앱 패키지 업로드 파일을 또한 앱이 Microsoft Store에 게시 된 후 기호 파일을을 [분석 앱 성능](https://docs.microsoft.com/windows/uwp/publish/analytics) 포함 되어 있습니다. 게시에 대 한 파트너 센터에 제출 하려는 의도로 Visual Studio를 사용 하 여 앱을 패키징하는 경우이 파일을 자동으로 생성 됩니다.
...
위의 단계를 완료한 후에는 앱을 배포할 수 있습니다. 내부 사용자 전용 이므로 판매 하지 않으려는 업무-(lob 기간 업무) 응용 프로그램이 있는 경우 모든 windows 10 장치에 설치 하기 위해이 앱 테스트용으로 로드할 수 있습니다.
...
Microsoft Store를 통해 앱을 배포 하는 경우 Visual Studio 스토어를 사용 하 여 패키지를 연결할 수 있습니다. 이 작업을 수행 하려면 솔루션 탐색기에서 프로젝트 이름을 마우스 오른쪽 단추로 클릭 하 고 **스토어**선택->**스토어에 앱 연결**합니다. 다음 섹션에서 설명 하는 **앱 패키지 만들기** 마법사에서도 수행할 수 있습니다. 앱을 연결하면 매니페스트 디자이너의 패키징 탭에 있는 일부 필드가 자동으로 업데이트됩니다.

## <a name="create-an-app-package-upload-file"></a>앱 패키지 업로드 파일을 만들기

Microsoft Store를 통해 앱을 배포 하려면 앱 패키지 (.appx 또는.msix), 앱 번들 (.appxbundle 또는.msixbundle) 또는 앱 패키지 업로드 파일 (.appxupload 또는.msixupload) 및 [파트너 센터에 패키지 된 앱 제출](https://docs.microsoft.com/windows/uwp/publish/app-submissions)만들어야 합니다. 앱 패키지 또는 앱 번들만 파트너 센터에 제출할 수 있지만, 앱 패키지 업로드 파일을 제출 하는 것이 좋습니다. Visual Studio에서 **앱 패키지 만들기** 마법사를 사용 하 여 앱 패키지 업로드 파일을 만들거나 기존 앱 패키지 또는 앱 번들에서 수동으로 만들 수 있습니다.

>[!NOTE]
> 앱 패키지 (.appx 또는.msix) 또는 앱 번들 (.appxbundle 또는.msixbundle)을 수동으로 구현 하려는 경우 [MakeAppx.exe 도구로 앱 패키지 만들기](https://docs.microsoft.com/windows/uwp/packaging/create-app-package-with-makeappx-tool)를 참조 하세요.
...
3.  첫 번째 대화 상자에서 **새 앱 이름을 사용 하 여 Microsoft Store에 업로드할 패키지를 만들 수 고** 를 선택 하 고 **** 을 클릭 합니다.  
    ![표시되는 패키지 만들기 대화 창](images/packaging-screen3.jpg)

    스토어에서 앱을 사용 하 여 이미 프로젝트를 연결 하는 경우 연결 된 스토어 앱에 대 한 패키지를 만드는 옵션이 있습니다. **테스트용 로드에 대 한 패키지 만들기**를 선택 하면 Visual Studio 앱 패키지 업로드 (.msixupload 또는.appxupload) 파일 파트너 센터 제출에 대 한 생성 되지 않습니다. 내부 장치에서만 실행하거나 테스트 목적으로 앱을 사이드로드하려는 경우에는 이 옵션을 선택하면 됩니다. 사이드로드에 대한 자세한 내용은 [디바이스를 개발에 사용하도록 설정](https://docs.microsoft.com/windows/uwp/get-started/enable-your-device-for-development)을 참조하세요.
4.  다음 페이지에서 파트너 센터로 개발자 계정으로 로그인 합니다. 아직 개발자 계정이 없으면 마법사를 사용하여 만들 수 있습니다.
    ![앱 이름 선택이 표시된 앱 패키지 만들기 창](images/packaging-screen4.jpg)
5.  현재 계정에 등록 된 앱 목록에서 패키지에 대 한 앱 이름을 선택 하거나 하면 예약 하지 않은 파트너 센터의 경우 새 이름을 예약 합니다.  
6.  **패키지 선택 및 구성** 대화 상자에서 세 개의 아키텍처 구성(x86, x64, ARM)을 모두 선택했는지 확인하여 앱이 가장 넓은 범위의 장치에 배포될 수 있는지 확인합니다. **앱 번들 생성** 목록 상자에서 **항상**을 선택합니다. 각 유형의 프로세서 아키텍처에 대해 구성 된 컬렉션을 포함 하기 때문에 단일 앱 패키지 파일을 통해 앱 번들 (.appxbundle 또는.msixbundle)는 것이 좋습니다. 앱 번들 생성을 선택 하면 앱 번들 최종 앱 패키지 업로드 (.appxupload 또는.msixupload) 파일에 디버깅 및 크래시 분석 정보와 함께 포함 됩니다. 어떤 아키텍처를 선택할지 잘 모르겠거나 다양한 디바이스에서 사용되는 아키텍처에 대한 자세한 내용을 보려면 [앱 패키지 아키텍처](https://docs.microsoft.com/windows/uwp/packaging/device-architecture)를 참조하세요.  
    ![패키지 구성이 표시된 앱 패키지 만들기 창](images/packaging-screen5.jpg)
7.  앱이 게시 된 후 파트너 센터에서 [분석 앱 성능](https://docs.microsoft.com/windows/uwp/publish/analytics) 에 전체 PDB 기호 파일을 포함 합니다. 버전 번호 또는 패키지 출력 위치 등 추가 세부 정보를 구성합니다.
9.  **만들기**를 클릭하여 앱 패키지를 만듭니다. 3 단계에서 **Microsoft Store에 업로드할 패키지를 만들 하 고** 옵션 중 하나를 선택 하 고 파트너 센터 제출에 대 한 패키지를 만드는 경우 마법사는 패키지 업로드 (.appxupload 또는.msixupload) 파일을 만듭니다. 3 단계에서 **테스트용 로드에 대 한 패키지 만들기** 를 선택한 경우 마법사는 단일 앱 패키지 또는 앱 번들 6 단계에서의 선택에 따라 만들어집니다.
10. 앱이 성공적으로 패키지 되 면이 대화 상자가 나타납니다 하 고 지정된 된 출력 위치에서 앱 패키지 업로드 파일을 검색할 수 있습니다. 이 시점에서 [로컬 컴퓨터 또는 원격 컴퓨터에서 앱 패키지 유효성을 검사할](#validate-your-app-package)수 있습니다.
...
1. 다음 폴더에 저장 합니다.
    - 하나 이상의 앱 패키지 (.appx 또는.msix) 또는 앱 번들 (.msixbundle 또는.appxbundle).
    - .Appxsym 파일입니다. 파트너 센터에서 [크래시 분석](../publish/health-report.md) 사용 되는 앱의 공용 기호를 포함 하는 압축된.pdb 파일입니다. 이 파일을 생략할 수 있지만 없는 충돌 분석 이나 디버깅 정보 앱에 대 한 사용할 경우.
2. 폴더를 압축 합니다.
3. 압축된 폴더 확장명을.zip에서.msixupload 또는.appxupload로 변경 합니다.

### <a name="validate-your-app-package"></a>앱 패키지 유효성 검사

로컬 또는 원격 컴퓨터에서 인증을 위해 파트너 센터에 제출 하기 전에 앱의 유효성을 검사 합니다. 앱 패키지의 릴리스 빌드만 유효성 검사할 수 있으며 디버그 빌드는 검사할 수 없습니다. 파트너 센터에 앱 제출에 대 한 자세한 내용은 [앱 제출](https://docs.microsoft.com/windows/uwp/publish/app-submissions)을 참조 하세요.

**로컬로 앱 패키지의 유효성을 검사 하려면**

1. 최종 **패키지 만들기 완료** 페이지에서 **앱 패키지 만들기** 마법사의 **로컬 컴퓨터** 옵션을 선택한 상태로 두고 하 고 **Windows 앱 인증 키트 시작을**클릭 합니다. Windows 앱 인증 키트로 앱을 테스트하는 방법에 대한 자세한 내용은 [Windows 앱 인증 키트](https://msdn.microsoft.com/library/windows/apps/Mt186449)를 참조하세요.
    ...
    테스트에 사용할 원격 windows 10 장치에 있는 경우 해당 장치에서 수동으로 Windows 앱 인증 키트를 설치 해야 합니다. 다음 섹션에서 다음 단계를 안내합니다. 작업이 완료되면 **원격 컴퓨터**를 선택하고 **Windows 앱 인증 키트 시작**을 클릭하여 원격 디바이스에 연결하고 유효성 검사를 실행할 수 있습니다.

2. WACK가 완료 된 후 앱이 인증을 통과, 파트너 센터에 앱을 제출할 준비가 되었습니다. 올바른 파일을 업로드하는지 확인합니다. 파일의 기본 위치는 솔루션의 루트 폴더에서 찾을 수 있습니다 `\[AppName]\AppPackages` 있으며.msixupload 또는.appxupload 파일 확장명으로 끝납니다. 형식 이름은 `[AppName]_[AppVersion]_x86_x64_arm_bundle.appxupload` 또는 `[AppName]_[AppVersion]_x86_x64_arm_bundle.msixupload` 모든 선택한 패키지 아키텍처를 사용 하 여 앱 번들을 선택한 경우.

**원격 windows 10 장치에서 앱 패키지의 유효성을 검사 하려면**

1.  [개발을 위해 디바이스 사용](https://msdn.microsoft.com/library/windows/apps/Dn706236) 지침에 따라 개발을 위해 windows 10 장치를 사용 합니다.
    >[!IMPORTANT]
...
UWP 앱 패키지를 사용 하 여 데스크톱 응용 프로그램에서와 같이 앱 장치에 설치 하지 않습니다. 일반적으로 Microsoft Store에서 UWP 앱을 다운로드하고, 장치에 앱을 설치합니다. Microsoft Store에 게시하지 않고도 앱을 설치할 수 있습니다(사이드로드). 이렇게 하면 설치 하 고 앱 패키지를 사용 하 여 테스트 앱 파일을 생성 합니다. Microsoft Store에서 판매하지 않을 앱(예: LOB(기간 업무) 앱)이 있는 경우에는 회사 내의 다른 사용자가 사용할 수 있도록 이 앱을 사이드로드할 수 있습니다.

대상 장치에 앱 테스트용으로 로드 하려면, 먼저 [개발을 위해 장치를 사용 하도록 설정](../get-started/enable-your-device-for-development.md)해야 합니다.

Windows 10 Mobile 장치에서 앱 사이드 로드 하려면 [WinAppDeployCmd.exe](install-universal-windows-apps-with-the-winappdeploycmd-tool.md) 도구를 사용 합니다. 데스크톱, 랩톱, 태블릿, 아래 지침을 따릅니다.

### <a name="sideload-your-app-package-on-windows-10-anniversary-update-or-later"></a>테스트용 로드 앱 패키지로 Windows 10 1 주년 업데이트 이상
...
### <a name="sideload-your-app-package-on-previous-versions-of-windows"></a>이전 버전의 Windows에서 앱 패키지를 테스트용으로 로드
...
3.  마우스 오른쪽 단추로 **AppDevPackage.ps1** 파일을 클릭합니다. **Run with PowerShell**을 선택한 다음 프롬프트를 따릅니다.  
    ...
    >[!TIP]
    > 태블릿에서 바로 가기 메뉴를 열려면 마우스 오른쪽 단추로 클릭, 완전 한 원이 나타날 때까지 계속 누르고 있다가 손가락을 저장할 화면을 터치 합니다. 손가락을 뗀 후에 바로 가기 메뉴가 나타납니다.
