...
다른 미디어 항목 종류의 여러 Id를 하나의 전달할 수 있습니다 (만큼 ProviderContentID-아래 참조 형식의 달라도)를 호출 하지만 모두 동일한 미디어 그룹에 속해야 합니다. 그러나 호출자 미디어 그룹을 알지 못할 수 없는 클라이언트 시나리오의 몇 가지 있습니다. API는 다음과 같은 경우에는 미디어 그룹에 대 한 "알 수 없음" sepcial 값을 허용 하 여이 지원 합니다.
 
   * idType AppType 또는 게임 항목을 얻을 수 있는 XboxHexTitle =
   * idType MovieType 또는 TVType 항목을 얻을 수 있는 ProviderContentId =
  
아래 차트는 어떤 ID의 형식을 제공할 수 미디어 그룹을 사용 하 여 전체 매핑을 요약 되어 있습니다.
...
이 조회 공급자 데 특정 id입니다. 예를 들어 Netflix Id 또는 Hulu id입니다.
 
IdType ProviderContentId 이면 단일 값만 허용 됩니다. ProviderContentIds ID 포함할 수 있는 유일한 형식의 있기 때문에 '.' 문자입니다. 이후는 '.' 문자 사이의 Id를 사용 하 여 구분 기호 이기도, 모호 Id 간 delimieter 란 간에 및 ID 자체 포함 됩니다. API의 나머지 부분 대량 조회 기능을 제외 하 고 ProviderContentIds, 동일한 방식으로 작동 합니다.
