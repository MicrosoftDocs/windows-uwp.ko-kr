---
Description: 앱의 제출 만들기를 완료 하 고를 스토어에 제출 클릭 제출 인증 단계를 입력 합니다.
title: 앱 인증 프로세스
...
앱 제출 생성을 완료하고 **Store에 제출**을 클릭하면 제출이 인증 단계로 들어갑니다. 이 프로세스는 대개 몇 시간 내에 완료되지만 경우에 따라 영업일 기준으로 3일까지 걸릴 수 있습니다. 통과 인증 제출을 후 고객이 새로운 서브 미션, 또는 패키지에 변경 내용으로 업데이트를 제출에 대 한 앱의 목록을 참조 하는 데 최대 24 시간이 걸릴 수 있습니다. 업데이트 세부 정보를 나열 하는 저장소만 변경 하는 경우 게시 프로세스는 한 시간 이내에 완료 됩니다.  제출물을 게시 하 고 대시보드에서 응용 프로그램의 상태가 표시 됩니다 하는 경우 알림을 받게 됩니다 **스토어에서**합니다.
...
-   **보안 테스트:** 이 첫 번째 테스트는 앱의 패키지 바이러스 및 맬웨어를 검사합니다. 앱이 이 테스트에 실패하면 최신 바이러스 백신 소프트웨어를 실행하여 개발 시스템을 확인한 다음 클린 시스템에서 앱 패키지를 다시 빌드해야 합니다.
...
이동할 준비가 되었다고 인증을 앱에 전달 된 **게시** 프로세스입니다.

- 제출을 (기본 옵션) 최대한 빨리 게시할 수 나타난, 하는 경우 게시 프로세스는 바로 시작 됩니다.
- 처음으로 앱을 게시 하 고 지정한를 **릴리스 날짜** 에 [일정](configure-precise-release-scheduling.md#release) 섹션에서 앱에 따라 사용할 수 있게 됩니다 프로그램 **릴리스 날짜**선택 합니다.
- 적 [게시 옵션을 저장할](manage-submission-options.md#publishing-hold-options) 는 해당 특정 날짜까지 해제 되지 해야를 지정 하려면 알게 게시 프로세스를 시작 하려면 해당 날짜까지 선택 하지 않으면 **변경 릴리스 날짜**합니다.
- 적 [게시 옵션을 저장할](manage-submission-options.md#publishing-hold-options) 제출을 수동으로 게시 하려면를 지정 하려면 시작 되지 않습니다 선택할 때까지 게시 프로세스 **지금 게시** (선택 또는 **변경 릴리스 날짜** 특정 날짜를 선택 하 고).
...
새 앱 및 앱의 패키지 변경 내용을 포함 하는 업데이트에 대 한 게시 프로세스는 24 시간 이내 완료 됩니다. 만 세부 정보를 나열 하는 스토어와 같은 옵션을 변경 하 여 앱의 패키지를 변경 하지 않고는 업데이트에 대 한 게시 프로세스에 1 시간 미만 걸립니다.

앱 게시 단계에서 중일 합니다 **세부 정보 표시** 상태 열에 앱 제출 알려줍니다에 새 패키지 및 세부 정보를 나열 하는 저장소에서 지원 되는 OS의 각 고객에 게 사용할 수 있는 경우에 대 한 링크 버전입니다. 아직 완료되지 않은 단계들은 **보류 중**으로 표시됩니다. 앱 게시 단계는 작업이 완료 될 때까지 유지 됩니다, 그리고 즉 새 패키지 및/또는 세부 정보를 나열 합니다. 앱의 잠재 고객의 모든 사용할 수 있습니다.
