...
이 API를 사용 하면 다양 한 방법으로 사용자의 고유 서비스에 저장 된 다른 사용자의 클립 클립 목록입니다. 여러 진입점 수준에서 데이터를 반환 하 고 대 한 쿼리 매개 변수를 통해 필터링 합니다. 클레임에서 XUID URI에 지정 된 소유자와 일치 하는 경우 콘텐츠 격리 확인 한 후 사용자의 클립 반환 됩니다. URI의 소유자 클레임 XUID와 일치 하지 않는 경우 다음 지정 된 사용자의 클립에 따라 반환 됩니다 개인 정보 보호 검사 및 요청 XUID에 대 한 콘텐츠 격리 검사 합니다.
...
동일한 API 호출 내에서 여러 사용자의 목록을 가져오기 위해는 API를 배치 하지 않습니다. SLS 설계자에서 (현재) 권장된 패턴은에 각 사용자에 대 한 쿼리를 합니다.
...
| 매개 변수| 유형| 설명|
| --- | --- | --- | --- | --- | --- |
| skipItems| 32 비트 부호 있는 정수| 선택 사항입니다. 컬렉션 (예: 건너뛰기 N 항목)의 N + 1에서 시작 하는 항목을 반환 합니다.|
...
| maxItems| 32 비트 부호 있는 정수| 선택 사항입니다. 최대 (skipItems continuationToken 항목의 범위를 반환할 수와 결합할 수 있습니다) 컬렉션에서 반환할 항목 수입니다. 서비스는 maxItems 없는 하 고 (경우에 결과의 마지막 페이지 아직 반환 되지 않은) maxItems 보다 적은 반환할 수 기본값을 제공할 수 있습니다.|
...
| 한정자| string| 선택 사항입니다. 클립을 가져오는 데 필요한 사용할 순서 한정자를 지정 합니다. <ul><li>만든-시스템에 클립이 날짜의 순서로 반환 되도록 지정</li><li>클립의 등급 값으로 반환 지정-[최고 등급]-등급</li><li>보기-[본 가장]-지정 클립 보기의 수를 반환 합니다.</li></ul><br/> 최대 크기: 12입니다. 기본값: "만든"입니다.| 
...
다음 URI는 기본 채널은 서비스 안내를 지정 하는 추가 경로 매개 변수를 사용 하면서이 문서의 동일 합니다. 해당 서비스 안내에 대 한 해당 사용자의 클립만 반환 됩니다. 요청 사용자에 게 요청 된 서비스 안내에 액세스할 수 있어야, 그렇지 않으면 HTTP 오류 403 반환 됩니다.
