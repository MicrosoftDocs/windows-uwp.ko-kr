...
점수 체계는 [BCP-47](https://tools.ietf.org/html/bcp47) 하위 태그 레지스트리 및 기타 데이터 소스에 포함된 데이터를 사용합니다. 이로 인해 점수가 일치하는 다른 품질과 그라데이션되고 여러 후보를 사용할 수 있는 경우 가장 일치하는 점수를 가진 후보를 선택합니다.
...
태그는 언어 및 스크립트 하위 태그에 대해 일치합니다. 두 태그에는 지역 하위 태그가 있으며 이 중 하나는 다른 지역을 포함하는 매크로 지역을 나타냅니다. 매크로 지역 하위 태그는 항상 숫자이고 유엔통계처 M.49 국가 및 지역 코드에서 파생됩니다. 포함하는 관계에 대한 세부 정보는 [매크로 지리(대륙)적 지역, 지리적 하위 지역, 선택된 경제 및 기타 그룹의 구성](https://unstats.un.org/unsd/methods/m49/m49regin.htm)을 참조하세요.
...
언어 목록에 동일한 언어 및 스크립트 하위 태그를 가지는 두 개 이상의 지역 변형이 포함되는 경우 첫 번째 언어 태그에 대한 비교는 정확한, 변형 및 지역 일치에 대해서만 점수가 매겨집니다. 부분 일치 점수는 마지막 지역 변형으로 연기됩니다. 그러면 사용자가 자신의 언어 목록에 대한 일치 동작을 제어할 수 있습니다. 일치 동작은 첫 번째의 언어 및 스크립트와 일치하는 세 번째 항목이 있는 경우 목록의 보조 항목에 대한 정확한 일치가 목록의 첫 번째 항목에 대한 부분 일치보다 선호되도록 할 수 있습니다. 예제는 다음과 같습니다.
...
더 나은 일치가 없을 경우 모든 언어와 일치하는 리소스를 지정하기 위해 언어 태그 "und"를 사용할 수 있습니다. BCP-47 언어 범위 " *" 또는 "* -&lt;script&gt;"와 유사한 것으로 간주할 수 있습니다. 예제는 다음과 같습니다.
