...
> [!IMPORTANT]
> 대리자를 등록, 위의 코드 예제는 원시 *이* 포인터 (현재 개체를 가리킴)로 전달 합니다. 강한 또는 약한 참조 현재 개체를 설정 하는 방법을 알아보려면 [이벤트 처리 대리자를 사용 하 여 *이* 포인터를 안전 하 게 액세스](weak-references.md#safely-accessing-the-this-pointer-with-an-event-handling-delegate)섹션에서 **멤버 함수를 대리인으로 사용 하는 경우** 하위 섹션을 참조 하세요.
...
> [!NOTE]
> 비동기 작업을 실행 하는 것에 대 한 둘 이상의 *완료 처리기* 의 구현 올바르지 않습니다. 완료 된 이벤트에 대 한 단일 대리자 하거나 할 수 있습니다 `co_await` 것입니다. 둘 다 있는 경우 두 번째 실패 합니다.
