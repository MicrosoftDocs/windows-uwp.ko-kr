...
title: XIM 프로젝트 구성
author: KevinAsgari
description: UWP 응용 프로그램 매니페스트에서 사용 하 여 Xbox 통합 멀티 플레이어 (XIM)에 맞게 구성 하는 방법에 설명 합니다.
...
기본적으로 XIM을 사용 하 여 응용 프로그램에 연결 하 고 네트워크 리소스 모두 인터넷 및 로컬 네트워크에서에서 연결을 수락 해야 합니다. 음성 채팅을 지 원하는 장치 마이크에 액세스를 해야 합니다. 결과적으로, 앱 "internetClientServer" 및 "privateNetworkClientServer" 기능 "마이크" 장치 접근 권한 값은 AppXManifest에서 선언 해야 합니다. 각 부분에 대 한 자세한 내용은 플랫폼 설명서를 참조 하세요. 다음 조각은 패키지/기능 노드 아래에 있어야 하는 노드에서만 그렇지 채팅 및 연결을 차단 됩니다.
...
만 유니버설 Windows 응용 프로그램에 필요 합니다. Xbox Live 제목 ID 확장을 선언 하 여 자동으로 등록 하는 대로 Xbox One 단독 리소스 ("XDK") 응용 프로그램 프로토콜 활성화 확장을 명시적으로 선언 하지 않습니다.

## <a name="xbox-one-exclusive-resource-application-network-manifest-templates"></a>Xbox One 단독 리소스 응용 프로그램 네트워크 매니페스트 템플릿

XIM을 사용 하는 모든 Xbox One 단독 리소스 응용 프로그램에 특정 한 소켓 설명 확인 해야 하 고 템플릿 콘텐츠가 AppXManifest 확장명이 "네트워크 매니페스트"에 포함 된 (네트워크 매니페스트에 대 한 자세한 내용은 플랫폼 설명서를 참조). 앱도 다른 템플릿에 있을 수 있지만 다음 코드 조각은 있어야 합니다. 그렇지 실패 XIM 네트워크에 이동:
...
XIM을 사용 하는 모든 유니버설 Windows 응용 프로그램에 특정 한 소켓 설명 확인 해야 하 고 템플릿 콘텐츠는 "네트워크 매니페스트" 파일에 포함 된 (패키지 루트 디렉터리에 networkmanifest.xml 파일 참조 세부 정보에 대 한 플랫폼 설명서 네트워크 매니페스트). 앱도 다른 템플릿에 있을 수 있지만 다음 코드 조각은 있어야 합니다. 그렇지 실패 XIM 네트워크에 이동:
