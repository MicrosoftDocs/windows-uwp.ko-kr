...
하나의 Xbox 게임 패드를 보유 한 한 방향 패드 (또는 D-패드); **A**, **B**, **X**, **Y**, **보기**및 **메뉴** 단추입니다. 다음은 왼쪽과 오른쪽 스틱, 범퍼, 및 트리거; 4 개의 진동 모터 총 하 고 있습니다. 두 섬스틱(thumbstick)은 X 및 Y 축에서 이중 아날로그 판독값을 제공하며, 안쪽으로 눌리면 버튼 역할도 합니다. 각 트리거 얼마나 것 끌어옵니다 다시 나타내는 아날로그 읽기를 제공 합니다.
...
> [!NOTE]
> `Windows.Gaming.Input.Gamepad` 또한 표준 Xbox 하나 게임 패드도는 동일한 컨트롤의 레이아웃을 포함 하는 Xbox 360 게임 패드를 지원 합니다.
...
> [!NOTE]
> Xbox 360 게임 패드 _임펄스 트리거_를 보유 한 하지 않습니다.

자세한 내용은 [진동 및 임펄스 트리거 개요](#vibration-and-impulse-triggers-overview)를 참조하세요.

### <a name="thumbstick-deadzones"></a>섬스틱(thumbstick) 데드존

중앙 위치에 서 있는 섬스틱(thumbstick)은 매번 X 축과 Y 축에 동일한 중립 판독값을 이상적으로 생성합니다. 하지만 섬스틱(thumbstick)의 기계력과 민감도로 인해 중앙 위치의 실제 판독값은 이상적인 중립값의 근사치일 뿐이며 후속 판독값마다 달라질 수 있습니다. 이러한 이유로 항상 작은 _deadzone_를 사용 해야&mdash;이상적인 센터 위치 가까이 무시 되는 값 범위에&mdash;제조 차이점, 기계 절약 또는 기타 게임 패드 문제에 대 한 보상을 합니다.
...
UI 탐색 컨트롤러로 게임 패드 왼쪽된 스틱을, 패드, **보기**, **메뉴**, **A**및 **B** 단추를 탐색 명령의 [필요한 집합](ui-navigation-controller.md#required-set) 에 매핑합니다.
...
[Gamepad][] 클래스는 정적 속성 [Gamepads][]를 제공하는데, 이는 현재 연결된 게임 패드의 읽기 전용 목록입니다. 있을 경우에 연결 된 게임 패드 중 일부에 관심이, 것이 좋습니다를 통해 액세스 하는 대신 직접 컬렉션을 유지 관리 하는 `Gamepads` 속성입니다.

다음 예제에서는 연결된 모든 게임 패드를 새 컬렉션에 복사합니다. 메모는 백그라운드에서 다른 스레드 됩니다 ( [GamepadAdded][] 및 [GamepadRemoved][] 이벤트)에서이 컬렉션 액세스 하기 때문에 배치 해야 읽거나 컬렉션을 업데이트 하는 모든 코드 주위 잠금을 합니다.
...
[GamepadAdded][] 및 [GamepadRemoved][] 이벤트는 한 게임 패드를 추가 하거나 제거 하는 경우에 발생 합니다. 이러한 이벤트의 처리기를 등록하면 현재 연결된 게임 패드를 추적할 수 있습니다.
...
다음 예제에서는 제거 된 게임 패드 추적을 중지 합니다. 제거 되기; 때 추적 하는 게임 패드에 수행 하는 작업을 처리 해야도 이 코드만 입력이 하나의 게임 패드를 추적 하 고 단순히을 설정 하는 등 `nullptr` 제거 됩니다. 모든 프레임에 게임 패드, 활성 상태 이면 및 어떤 게임 패드 컨트롤러가 연결 되어 있고 연결이 끊어진 경우 입력이를 수집 하려는 없다 업데이트를 확인 해야 합니다.
...
자세한 내용은 [게임에 대 한 입력 사례](input-practices-for-games.md) 를 참조 하십시오.
...
각 섬스틱(thumbstick)은 X 축과 Y 축에 -1.0과 +1.0 사이의 아날로그 판독값을 제공합니다. X 축에서 -1.0 값은 맨 왼쪽 섬스틱(thumbstick) 위치에 해당하고, +1.0 값은 맨 오른쪽 위치에 해당합니다. Y 축에서 -1.0 값은 맨 아래쪽 섬스틱(thumbstick) 위치에 해당하고, +1.0 값은 맨 위쪽 위치에 해당합니다. 두 축에서의 값은 약 후속 개가; 간의 0.0 가운데 위치에는 메모리는 이지만, 다양 하 게 정확한 값에 대 한 기본 하는 경우에 이 변형을 완화 하기 위한 전략이이 섹션의 뒷부분에서 설명 됩니다.
...
각 게임 패드 단추의&mdash;패드, 왼쪽 및 오른쪽 범퍼, 왼쪽 및 오른쪽 스틱 press, **A**, **B**, **X**, **Y**, **보기**및 **메뉴**의 네 방향&mdash;제공 하는 읽기 디지털 눌려진 () 또는 릴리스 (를)가 있는지 여부를 나타냅니다. 효율성을 위해 단추 개가 개별 부울 값이 있습니다;로 표시 되지 않습니다. 대신, 자신이 모두에 압축 [GamepadButtons][] 열거형으로 표시 되는 단일 비트 필드입니다.
...
하려는 경우도 있습니다 수는 버튼에서 전환 하는 시기를 결정에 출시 된 하거나 단추의 집합은 특정 방식으로 정렬 하는 경우 또는 여러 단추는 누름 또는 해제 여부를 누름 놓을&mdash;일부를 누르면 일부 되지 않습니다. 이러한 각 상태를 검색하는 방법에 대한 자세한 내용은 [버튼 전환 검색](input-practices-for-games.md#detecting-button-transitions) 및 [복잡한 버튼 정렬 검색](input-practices-for-games.md#detecting-complex-button-arrangements)을 참조하세요.
...
Xbox One 게임 패드에는 총 4개의 독립적 진동 모터가 탑재되어 있습니다. 게임 패드 본문;에 있는 대형 모터는 2 왼쪽된 motor 오른쪽 motor 줄어들면서 보다 매끄럽고, 더 미세 진동 제공 하는 동안 대략적인, 높은 진폭 진동을 제공 합니다. 다른 두 모터는 소형 모터입니다. 하나는 각 트리거 안에 있는 모터로, 사용자의 트리거 손가락에 즉시 선명하게 파열되는 진동을 제공합니다. Xbox One 게임 패드의 이러한 고유한 기능 때문에 Xbox One 게임 패드 트리거를 _임펄스 트리거_라고 합니다. 이러한 모터를 함께 오케스트레이션하면 광범위한 촉감을 재현할 수 있습니다.
...
하지만의 구성원은 `Gamepad.Vibration` 속성을 직접 수정할 수, 별도 초기화 하는 것이 좋습니다. `GamepadVibration` 한 다음에 복사 하는 값으로 인스턴스는 `Gamepad.Vibration` 속성이 실제 모터 강도 한번에 모든 변경 합니다.
