...
| voiceReasonId| string| 추가 피드백 이유를 설명 하기 위해 보낸 사람은 Kinect에서 사용자가 제공한 음성 파일의 ID (Base 64)를 제출 합니다. |
| evidenceId| string| 예를 들어 비디오 파일 전송 중 피드백의 증거로 사용할 수 있는 리소스의 ID를 게임 플레이 하는 동안 기록 합니다. |
...
| 유형| 보낸| 메모|
| --- | --- | --- | --- | --- | --- |
| CommsAbusiveVoice| 사용자| 사용자에 게 보고서 부적절 한 음성 통신의 제목을 내에서 그리고 Xbox 대시보드에서 피드백 보내기. |
...
| CommsTextMessage| 사용자| 받은 편지함 서비스 발신자의 신뢰도 업데이트 하 고 적용 팀에 피드백을 보고 평판 서비스를 호출 합니다. **참고:** 받은 편지함 UI는 사용자가 메시지에 플래그 지정 하도록 허용 하는 단추가 있어야 합니다. |
  ...
  | FairPlayBlock | 개인 정보 | 개인 정보는 사용자가 다른 플레이어를 차단 하는 경우이 피드백 신뢰도 서비스에 보냅니다.  |
  ...
  | FairPlayIdler | 사용자, 파트너 | 타이틀 경우 사용자는 유휴 의도적으로 반올림, 후 라운드 일반적으로 게임에서 결정 하는 사용자 개입 없이이 피드백을 보낼 수 있습니다.  |
  | FairPlayKicked | 사용자, 파트너 | 사용자 (시작) 게임에서 투표 된 경우를 감지 하는 제목을 사용자 개입 없이이 피드백을 보낼 수 있습니다.  |
  | FairPlayKillsTeammates | 사용자, 파트너 | 플레이어가 killls 때 자동으로 확인할 수 있는 타이틀 하면 팀 동료 사용자 개입 없이이 피드백을 보낼 수 있습니다.  |
  | FairPlayQuitter | 사용자, 파트너 | 사용자가 게임을 빠르게 종료를 결정 하는 타이틀 사용자 개입 없이이 피드백을 보낼 수 있습니다.  |
  ...
  | PositiveSkilledPlayer | 사용자, 파트너 | 사용자와 파트너 타이틀 MPSD 세션의 끝에 MVP에 응답할 수 있도록 할 수는 나타내는이 피드백을 보냅니다.  |
  ...
  | UserContentInappropriateUGC | 사용자, 파트너 | 사용자와 파트너 타이틀 Forza에서 그림판 작업 예를 들어 게임 내에서 부적절 한 공유 UGC 플래그를 사용자를 허용 해야 나타내는이 피드백을 보냅니다.  |
