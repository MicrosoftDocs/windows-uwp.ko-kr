...
> [!IMPORTANT]
> Windows 10 (UWP) 패키지에 대 한 버전 번호의 마지막 (네 번째) 섹션 스토어 사용을 위해 예약 되며 (하지만 스토어가이 섹션의 값을 변경할 수 있습니다) 패키지를 빌드할 때 0 이어야 합니다. 다른 섹션은 0과 65535 (0이 될 수 없는 첫 번째 섹션) 제외 사이의 정수로 설정 해야 합니다.
...
이는 시간에 따라 앱을 진화시킬 수 있는 뛰어난 유연성을 제공합니다. 업로드 및 낮은 버전 번호를 사용 하 여 이전에 지원 하지 않는 Windows 10 장치에 대 한 지원을 추가 하는 새 패키지를 제출, 하드웨어 또는 OS 기능을 활용 하기 위해 종속성이 엄격한 있는 더 높은 버전의 패키지를 추가할 수 있습니다. 기본 기존 고객의 일부 또는 전부에 업데이트로 제공 되는 더 높은 버전의 패키지를 추가할 수 있습니다.
...
패키지의 복사본을 유지 하는 경우 앱의 패키지 롤백할 스토어에서 이전 Windows10 패키지로 릴리스를 사용 하 여 문제를 검색 해야 하는 경우를 해야 합니다. 이 방법은 문제를 해결 하는 데 시간이 걸릴 수 있는 동안 고객에 게 방편 임시입니다.
...
Windows8 및 Windows8.1를 대상으로 하는.appx 패키지에 대 한 아키텍처별으로 동일한 규칙이 적용: 새 제출에서 패키지의 버전 번호 된 같은 아키텍처에 대 한 스토어에 마지막으로 게시 된 패키지 보다 항상 해야 합니다.
...
Windows8 및 Windows8.1을 대상으로 하는 패키지에 대 한 다양 한 버전 번호 업데이트 시나리오에서 발생 하는 상황의 몇 가지 예는 다음과 같습니다.
