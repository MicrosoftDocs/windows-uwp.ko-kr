...
**앱 계약**앱, 앱 간, 기능 간 사용자 흐름을 사용 하는 앱 계약에 참여할 수 있습니다.

-   **공유**에 사용자가 다른 앱을 통해 다른 사람들과 앱의 콘텐츠를 공유 하 고 다른 사람 및 앱 으로부터 공유 가능한 콘텐츠를 너무 받을 수 있도록 합니다.
-   **원격 재생**사용자가 오디오, 비디오 또는 홈 네트워크의 다른 장치에 앱에서 이미지를 즐길 수 있도록 합니다.
-   **파일 선택기 및 파일 선택기 확장** 사용자가 로드 하 고 로컬 파일 시스템, 연결 된 저장 장치, 홈 그룹 또는 기타 앱에서 파일을 저장할 수 있도록 합니다. 다른 앱에서 사용자 앱의 콘텐츠를 로드할 수 있도록 파일 선택기 확장을 제공할 수도 있습니다.

자세한 내용은 [앱 계약 및 확장](https://msdn.microsoft.com/library/windows/apps/hh464906)을 참조하세요.
<!-- Win 8 page. Should have replacement. -->

**다른 보기, 폼 팩터 및 하드웨어 구성**Windows 충전 및 중점에서 앱에 사용자를 추가 합니다. 사용자가 해당 앱을 사용하기로 결정한 어떤 디바이스에서든, 어떤 입력 모드를 사용하든, 어떤 방향으로든, 어떤 하드웨어 구성에서든, 어떤 환경에서든 앱 UI가 두드러지게 하고 싶을 것입니다.
...
-   **로밍**작업 한 위치를 선택할 수 있도록 하 고 관심 가장, 사용 중인 장치와 관계 없이 UX를 유지 하는 데이터를 로밍 하 여 디바이스에서 지속적인 환경을 만듭니다. 로밍을 통해 설정 및 상태를 유지 관리하여 부엌에 있는 가족용 PC, 회사 PC, 개인용 태블릿, 기타 폼 팩터 등 어디에서든 앱을 쉽게 사용할 수 있게 합니다. [응용 프로그램 데이터 관리](../design/app-settings/store-and-retrieve-app-data.md)에 대해 살펴보고, [로밍 중인 응용 프로그램 데이터에 대한 지침](https://msdn.microsoft.com/library/windows/apps/hh465094)을 참조하세요.
...
-   **근접 연결 제스처**는 물리적으로 가까이에서 실제로 디바이스를 함께 탭 하 여 (멀티 플레이어 게임) 다른 사용자와 장치를 연결할 수 있게 합니다. [근접 연결 및 탭하기](https://msdn.microsoft.com/library/windows/apps/hh465229)에 대해 자세히 알아보세요.
-   **카메라 및 외장형 저장 장치**사용자가 자신의에서 연결 또는 기본 제공 카메라 채팅 및 회의 기록, 세계를 문서화 하는 프로필 pics 라인 vlogs 연결 또는 어떠한 활동 앱은 특성입니다. [이동식 저장소의 콘텐츠 액세스](https://msdn.microsoft.com/library/windows/apps/hh465189)에 대해 자세히 알아보세요.
-   **가 속도계 및 기타 센서**   장치에 달려 다양 한 센서를 사용 하 여 합니다. 앱은 주변 광원에 따라 디스플레이를 흐리게 하거나 밝게 하고, 사용자가 디스플레이를 돌릴 경우 UI를 재배치하며, 물리적 이동에 반응할 수 있습니다. [센서](../devices-sensors/sensors.md)에 대해 자세히 알아보세요.
...
**시작 화면**시작 화면, 가능한 한 빠르게 로드 되어야 하 고 화면에만 동안 남아 앱 상태를 초기화 해야 합니다. 시작 화면에는 앱의 개성을 잘 나타내는 내용을 표시해야 합니다.

**처음 시작**하기 전에 사용자가 서비스에 대 한 등록 하 고 해당 계정에 로그인 하거나 자체 콘텐츠를 추가, 어떤 보게 될까요? 사용자에게 정보를 요청하기 전에 앱의 가치를 입증해 보세요. 사람들에게 사용 확정을 요청하기 전에 살펴보고 앱의 기능을 이해할 수 있도록 샘플 콘텐츠를 보여 주는 것이 좋습니다.
