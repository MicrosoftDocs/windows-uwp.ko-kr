...
이미 만들어진 앱 서비스를 사용하든, 고유한 앱 서비스를 작성하든 관계없이 서비스가 원격 시스템과 호환되도록 하려면 몇 가지 편집 작업이 필요합니다. Visual Studio에서 앱 서비스 공급자 프로젝트(샘플의 "AppServicesProvider")로 이동한 다음 해당 _Package.appxmanifest_ 파일을 선택합니다. 마우스 오른쪽 단추를 클릭하고 **코드 보기**를 선택하여 파일의 전체 내용을 표시합니다. 프로젝트를 앱 서비스로 정의하고 부모 프로젝트의 이름을 지정하는 **Extension** 요소를 찾습니다.
...
아직 없는 경우 다음과 같이 **SupportsRemoteSystems** 속성을 추가합니다.
