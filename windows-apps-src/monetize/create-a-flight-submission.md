...
description: 파트너 센터 계정에 등록 된 앱에 대 한 새 패키지 비행 제출을 만들려면 Microsoft Store 제출 API 사용 하 여이 메서드를 사용 합니다.
...
* 앱에 대 한 패키지 항공편을 만듭니다. 파트너 센터에서이 수행할 수 있는 또는 사용 하 여이 수행할 수는 [패키지 항공편 만들기](create-a-flight.md) 메서드.
...
| 메서드 | 요청 URI                                                      |
|--------|------------------------------------------------------------------|
| 올리기    | `https://manage.devcenter.microsoft.com/v1.0/my/applications/{applicationId}/flights/{flightId}/submissions` |
...
| 이름        | 형식   | 설명                                                                 |
|---------------|--------|-----------------------------------------------------------------------------|
| applicationId | string | 필수 사항입니다. 패키지 플라이트 제출을 만들려는 앱의 스토어 ID입니다. 스토어 ID에 대한 자세한 내용은 [앱 ID 세부 정보 보기](https://msdn.microsoft.com/windows/uwp/publish/view-app-identity-details)를 참조하세요.  |
| flightId | string | 필수. 제출을 추가하려는 패키지 플라이트의 ID입니다. 이 ID는 개발자 센터 대시보드에서 사용할 수 있으며 [패키지 플라이트 만들기](create-a-flight.md) 및 [앱의 패키지 플라이트 가져오기](get-flights-for-an-app.md) 요청에 대한 응답 데이터에 포함되어 있습니다.  |
...
| 409  | 파트너 센터 기능을 사용 하는 앱 또는 앱의 현재 상태로 인해 비행 제출 패키지를 만들 수 없습니다 [현재 Microsoft Store 전송 API에 의해 지원 되지 않습니다](create-and-manage-submissions-using-windows-store-services.md#not_supported)합니다. |   
