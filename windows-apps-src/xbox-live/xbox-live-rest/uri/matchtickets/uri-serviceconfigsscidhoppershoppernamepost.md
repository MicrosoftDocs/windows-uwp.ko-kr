...
| 장치 유형| 예| 때 사용자의 deviceType 없거나 또는 비-콘솔에 일치 하는 제목으로 설정 하 고 콘솔 전용 제목 않아야 합니다. | 403|
...
| preserveSession| 열거형| 일치 하는 세션 세션 다시 사용 하는 경우를 나타내는 값입니다. 가능한 값은 "항상" 및 "없습니다". |
...
일치 티켓을 만들 수 있습니다 세션 플레이어 관련 속성과 함께 일치 시킬 플레이어 있어야 합니다. 전에 **ticketSessionRef** 개체에서 참조 하 여 세션을 만들어야 합니다. 각 플레이어 만들거나 세션에 관련 된 일치 특성을 추가 MPSD에 대해 세션에 참가 해야 합니다. 일치 특성은 각 플레이어에 matchAttrs 라는 사용자 지정 속성 필드에 배치 됩니다.
...
| waitTime| 32 비트의 부호 있는 정수| 평균 대기 hopper (정수 초 수)에 대 한 시간입니다.|
