...
description: 이 문서에서는 Marble Maze 프로젝트;의 기본 특성 설명 예를 들어 Windows 런타임 환경에서 Visual c + +를 사용 하는 방법, 생성 및 구성 하는 방법 및 구축 방법.
...
-   코드 분석 도구와 함께 [SAL 주석](https://docs.microsoft.com/visualstudio/code-quality/using-sal-annotations-to-reduce-c-cpp-code-defects) 사용 하 여 앱에서 오류를 검색 하는 데.
...
2. 선택은 왼쪽된 사이드바에서 **새 프로젝트** 창에서 **설치 됨 > 템플릿 > Visual c + +**.

3. 가운데 목록에서 **DirectX 11 앱 (유니버설 Windows)을**선택 합니다. 이 옵션을 보이지 않으면 없을 경우 설치 필수 구성 요소&mdash; [워크 로드 및 구성 요소 추가 또는 제거 하 여 Visual Studio 2017 수정](https://docs.microsoft.com/visualstudio/install/modify-visual-studio) 추가 구성 요소를 설치 하는 방법에 대 한 정보를 참조 하세요.
...
터치가 속도계, Xbox One 컨트롤러 또는 마우스 컨트롤 Marble Maze를 사용할 수 있습니다.
...
Windows 런타임은 특수 응용 프로그램 환경에서만 실행되는 UWP 앱을 만드는 데 사용할 수 있는 프로그래밍 인터페이스입니다. 이러한 앱 인증 된 기능, 데이터 형식 및 장치를 사용 하 고 Microsoft Store에서 배포 됩니다. 최하위 수준에서 Windows 런타임은 ABI(응용 프로그램 이진 인터페이스)로 구성됩니다. ABI는 JavaScript, .NET 언어, Visual C++ 등 여러 프로그래밍 언어가 Windows 런타임 API에 액세스할 수 있게 하는 하위 수준 이진 계약입니다.
...
메뉴 모음에 앱에서 코드 분석을 수행 하려면 선택 **빌드 > 솔루션에서 코드 분석 실행**. 코드 분석에 대한 자세한 내용은 [코드 분석을 사용하여 C/C++ 코드 품질 분석](https://docs.microsoft.com/visualstudio/code-quality/analyzing-c-cpp-code-quality-by-using-code-analysis)을 참조하세요.
