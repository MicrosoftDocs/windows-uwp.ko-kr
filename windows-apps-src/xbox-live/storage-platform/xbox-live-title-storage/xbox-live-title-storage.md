...
다음 저장소 유형 되지 않습니다. 이러한 현재 사용 중인 타이틀에 대해서만 지원 됩니다. 새 제목에 사용할 수 있는 하지 않습니다.
...
| 세션 저장소    | 세션별 256MB | 누구 든 지에 대 한 데이터는 특정 멀티 플레이 게임 세션에 가입 합니다.                                                                                             | 세션에 참가할 수 있는 모든 플랫폼입니다.                                                             | 세션의 모든 사용자가 읽거나 쓸 수 있습니다. |
...
이미지, 소리 및 사용자 지정 데이터 이진 형식을 사용 합니다. HTTP를 통해 데이터를 전송 해야, 때문에 HTTP 받아들이는 문자로 이진 데이터 인코딩해야 합니다. 예를 들어 데이터를 16 진수 문자열 변환 하거나 base64 인코딩 사용할 수 있습니다. 타이틀 저장소 시스템 게임 인코드 및 디코드 데이터에서 읽고 타이틀 저장소에 쓸 때 같은 체계를 사용 해야 하므로 인코딩된 데이터를 처리 하지 않습니다.
...
구성 blob 설정을 가능성의 목록에서 반환 하는 가상 노드를 포함할 수 있습니다. 가상 노드는 제목 또는 로캘와 같은 특정 상황에 대 한 설정을 제공 하는 데 유용 합니다. 가상 노드 값 및 조건 값에서 선택 하기 위한 몇 가지 가능한 설정이 포함 됩니다. 다음 예제에서는 **defaultCardDesign** 설정에에서 있을 수 값 중 하나로 가상 노드.
...
게임을이 파일을 읽고, 시스템 **\_sourceNodes** 배열에서 값 중 하나를 선택 합니다. 이 경우 항목이 게임의 제목 ID에 따라 선택 합니다. 사용자가 게임 **12456799** 재생을 참조 하세요.
...
**\_SelectBy** 값을 나타냅니다. 작업을 수행 하는 선택 및 **\_selector** 값의 유형을 선택에 사용 하 여 데이터를 나타냅니다. 가능한 값은 다음과 같습니다.
...
URI의 **{경로}** 부분 시간차 요청의 타입 이며 245 자 이하로 이어야 합니다.

<a name="ID4ETEAC"></a>

## <a name="throttle-limit"></a>스로틀 제한

얼마나 많은 읽기 또는 쓰기 제목을 분당 만들 수에 고정된 제한이 하지만 일반적으로 사용할 수 없는 분당 하나 이상 평균 1 시간 세션에서. 예를 들어 제목을 세션의 하지만 남아 있는 시간 동안 더 이상 시작 시 60 읽기 또는 쓰기 만들 수 있습니다. 타이틀 해야 강화 호출에 대해 나중에 Xbox LIVE 서비스 요청을 제한 하는 데 필요한 경우.
...
타이틀 저장소를 사용 하 여 시작 하려면 먼저 저장 하려는 데이터의 종류 확인 합니다. 몇 가지 예로 저장 된 게임, 게임 상태, 매일 문제, 게임 지도 및 아트 리소스입니다.

그런 다음 제목 및 플랫폼은 필요한 데이터에 액세스할 수를 결정 합니다. 타이틀 저장소 지원 클라우드 데이터 액세스 단일 플랫폼에서 단일 제목 및 여러 플랫폼에서 여러 타이틀에서.
