...
사용자가 메시지 요약 메시지 제목을 포함 되어 있습니다. 사용자 생성 된 메시지는 현재 메시지 텍스트의 처음 20 문자입니다. 시스템 메시지 "라이브 시스템" 등의 대체 주체를 제공할 수 있습니다.

메시지; 수신 된 순서의 반대 방향으로 돌아갑니다. 즉, 최신 메시지 먼저 반환 됩니다.
...
| continuationToken| 문자열|  | 이전 열거형 전화;에서 반환 된 문자열 열거형을 계속 하는 데 사용 합니다.|
...
사용자가 메시지 요약을 검색 하 고 고유한 사용자가 있어야 합니다.
...
성공적으로 호출 하는 경우 서비스 결과 데이터를 JSON 형식으로 반환 합니다.
...
| isRead| 부울|  | 사용자가 메시지 세부 정보를 읽을 이미 있는지를 나타내는 플래그입니다.|
| 전송| DateTime|  | UTC 날짜 및 메시지를 보낸 시간입니다. (서비스에서 제공).|
| 만료| DateTime|  | UTC 날짜 및 시간 메시지에 만료 됩니다. (모든 메시지 수명이 최대, 나중에 따라 결정 됩니다.)|
...
| 속성| 형식| 최대 길이| 설명|
| --- | --- | --- | --- |
| continuationToken| 문자열| 100| 필요에 따라 서버에 의해 반환 됩니다. 열거형을 계속 하려면 이후 호출할 수 있습니다.|
