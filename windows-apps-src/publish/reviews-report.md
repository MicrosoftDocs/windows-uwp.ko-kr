...
- **리뷰 내용**: **등급 사용 하 여 내용을 검토 하**는 유일한 등급 검토 콘텐츠로 표시 됨을 의미 하는 기본 설정은입니다. **모든** 모든 평가 표시를 리뷰 텍스트를 포함 하지 않는 비롯 선택할 수 있습니다. 참고 **평점 분석** 차트 선택에 관계 없이 모든 리뷰를 항상 표시 됩니다.
...
**리뷰 내용** 페이지 필터를 **사용 하 여 평가** 선택한 경우에이 차트 항상에 모든 리뷰를 note 합니다.
...
검색 상자를 사용 하 여 특정 단어 또는 구 앱의 리뷰에서 검색할 수 있습니다. 리뷰는 다른 언어로 작성 된 원래만 고객이 작성 된 텍스트를 검토 하는 참고 검색 됩니다. 리뷰 번역 된 텍스트 검색 되지 않습니다.
...
위에서 언급 했 듯이 때 리뷰를 검색, 고객이 남긴 원래 텍스트만 검색 됩니다 (및 텍스트는 번역) **리뷰 번역** 상자가 선택 되어 있는 경우에 합니다.
