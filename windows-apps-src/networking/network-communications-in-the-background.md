---
author: DelfCo
description: "앱은 포그라운드에 있지 않을 때 통신을 유지하기 위해 백그라운드 작업과 두 가지 기본 메커니즘을 사용합니다."
title: "백그라운드 네트워크 통신"
...
-   [**DatagramSocket**](https://msdn.microsoft.com/library/windows/apps/dn804256)의 [**TransferOwnership**](https://msdn.microsoft.com/library/windows/apps/br241319) 메서드 중 하나
-   [**StreamSocket**](https://msdn.microsoft.com/library/windows/apps/dn781433)의 [**TransferOwnership**](https://msdn.microsoft.com/library/windows/apps/br226882) 메서드 중 하나
-   [**StreamSocketListener**](https://msdn.microsoft.com/library/windows/apps/dn804407)의 [**TransferOwnership**](https://msdn.microsoft.com/library/windows/apps/br226906) 메서드 중 하나
...
원시 비동기 패턴을 사용하면 Windows에서 [**ControlChannelTrigger**](https://msdn.microsoft.com/library/windows/apps/br224811)에 대한 백그라운드 작업의 [**IBackgroundTask.Run**](https://msdn.microsoft.com/library/windows/apps/hh701032) 메서드를 수신 완료 콜백의 반환값과 동기화할 수 있습니다. **Run** 메서드는 모든 완료 콜백이 반환된 후에 호출됩니다. 따라서 앱은 **Run** 메서드가 호출되기 전에 데이터/오류를 받습니다.

앱은 다른 읽기를 게시해야만 완료 콜백에서 컨트롤을 반환받습니다. 또한 [**MessageWebSocket**](https://msdn.microsoft.com/library/windows/apps/br208119) 또는[**StreamWebSocket**](https://msdn.microsoft.com/library/windows/apps/br226842) 전송에서 [**DataReader**](https://msdn.microsoft.com/library/windows/apps/br226923)를 직접 사용하면 안 됩니다. 그럴 경우 위에서 설명한 동기화가 중단되기 때문입니다. 전송에서 [**DataReader.LoadAsync**](https://msdn.microsoft.com/library/windows/apps/br208135) 메서드를 직접 사용하는 것은 지원되지 않습니다. 대신 [**StreamWebSocket.InputStream**](https://msdn.microsoft.com/library/windows/apps/br241656) 속성의 [**IInputStream.ReadAsync**](https://msdn.microsoft.com/library/windows/apps/br241719) 메서드에서 반환된 [**IBuffer**](https://msdn.microsoft.com/library/windows/apps/br226936)를 나중에 [**DataReader.FromBuffer**](https://msdn.microsoft.com/library/windows/apps/br208133) 메서드에 전달하여 더 처리할 수 있습니다.
...
다음 샘플에서는 [**StreamWebSocket**](https://msdn.microsoft.com/library/windows/apps/hh701032)의 푸시 알림 및 WebSocket keep-alive에 대한 [**ControlChannelTrigger**](https://msdn.microsoft.com/library/windows/apps/br226923)를 등록합니다.
...
**참고** SSL을 사용하는 [HttpClient](http://go.microsoft.com/fwlink/p/?linkid=241637)는 네트워크 트리거 기능 및 [**ControlChannelTrigger**](https://msdn.microsoft.com/library/windows/apps/hh701032) 사용은 현재 지원되지 않습니다.
...
-   앱은 특정 URI로 요청을 보내기 전에 [System.Net.Http](http://go.microsoft.com/fwlink/p/?linkid=241637) 네임스페이스에서 [HttpClient](http://go.microsoft.com/fwlink/p/?linkid=241638) 또는 [HttpClientHandler](http://go.microsoft.com/fwlink/p/?linkid=227894) 개체에 여러 속성 및 헤더를 설정해야 합니다.
-   앱은 [**ControlChannelTrigger**](http://go.microsoft.com/fwlink/p/?linkid=241637)와 함께 사용할 [HttpClient](https://msdn.microsoft.com/library/windows/apps/hh701032) 전송을 만들기 전에 초기 요청을 만들어 전송을 테스트하고 적절하게 설정해야 합니다. 앱에서 전송이 적절히 설정되었다고 판단하면 [HttpClient](http://go.microsoft.com/fwlink/p/?linkid=241637) 개체는 **ControlChannelTrigger**와 함께 사용할 전송 개체로 구성될 수 있습니다. 이 프로세스는 일부 시나리오에서 전송을 통해 설정된 연결이 끊어지는 것을 방지하기 위해 설계되었습니다. SSL 인증서를 사용할 경우, 또는 선택할 수 있는 인증서가 여러 개인 경우 PIN 입력에 사용할 대화 상자가 앱에 필요할 수 있습니다. 또한 프록시 인증 및 서버 인증이 필요할 수 있습니다. 프록시 또는 서버 인증이 만료되면 연결이 닫힙니다. 앱에서 이러한 인증 만료 문제를 처리할 수 있는 방법은 타이머를 설정하는 것입니다. HTTP 리디렉션이 필요한 경우에는 두 번째 연결을 안정적으로 설정할 수 있는지 여부가 확실치 않습니다. 초기 테스트 요청은 **ControlChannelTrigger** 개체와 함께 전송으로서 [HttpClient](http://go.microsoft.com/fwlink/p/?linkid=241637) 개체를 사용하기 전에 앱이 가장 최근에 리디렉션된 URL을 사용할 수 있음을 보장합니다.

다른 네트워크 전송과 달리 [HttpClient](http://go.microsoft.com/fwlink/p/?linkid=241637) 개체는 [**ControlChannelTrigger**](https://msdn.microsoft.com/library/windows/apps/hh701175) 개체의 [**UsingTransport**](https://msdn.microsoft.com/library/windows/apps/hh701032) 메서드로 직접 전달할 수 없습니다. 대신 [HttpClient](http://go.microsoft.com/fwlink/p/?linkid=259153) 개체 및 **ControlChannelTrigger**와 함께 사용하기 위해 [HttpRequestMessage](http://go.microsoft.com/fwlink/p/?linkid=241637) 개체를 특별히 생성해야 합니다. [HttpRequestMessage](http://go.microsoft.com/fwlink/p/?linkid=259153) 개체는 [RtcRequestFactory.Create](http://go.microsoft.com/fwlink/p/?linkid=259154) 메서드를 사용하여 만듭니다. 그런 다음 만든 [HttpRequestMessage](http://go.microsoft.com/fwlink/p/?linkid=259153) 개체를 **UsingTransport** 메서드에 전달합니다.

다음 샘플에서는 [HttpClient](http://go.microsoft.com/fwlink/p/?linkid=259153) 개체 및 [**ControlChannelTrigger**](http://go.microsoft.com/fwlink/p/?linkid=241637)와 함께 사용할 [HttpRequestMessage](https://msdn.microsoft.com/library/windows/apps/hh701032) 개체를 생성하는 방법을 보여 줍니다.
...
[HttpClient](http://go.microsoft.com/fwlink/p/?linkid=241637)를 사용하면 [**ControlChannelTrigger**](https://msdn.microsoft.com/library/windows/apps/br224811)에 대한 백그라운드 작업의 [**IBackgroundTask.Run**](https://msdn.microsoft.com/library/windows/apps/hh701032) 메서드를 수신 완료 콜백의 반환값과 동기화할 수 있습니다. 이 때문에 앱은 **Run** 메서드에서 차단 HttpResponseMessage 방법을 사용하고 전체 응답이 수신될 때까지 기다릴 수만 있습니다.
...
다음 샘플에서는 [**ControlChannelTrigger**](http://go.microsoft.com/fwlink/p/?linkid=241637)와 함께 사용할 경우 [HttpClient](https://msdn.microsoft.com/library/windows/apps/hh701032)에서 보내기 요청을 처리하는 방법을 보여 줍니다.
...
다음 샘플에서는 [**ControlChannelTrigger**](http://go.microsoft.com/fwlink/p/?linkid=241637)를 사용할 경우 [HttpClient](https://msdn.microsoft.com/library/windows/apps/hh701032)에서 받은 응답을 읽는 방법을 보여 줍니다.
