...
| 참고                                                                                                                                                                                                                                                                                                                                    |
|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 세션에 대 한 변경 내용에 대 한 구독에 연결 된 세션에서 활성 플레이어가 필요 합니다. ConnectionRequiredForActiveMembers 필드 설정 해야 세션에 대 한 /constants/system/capabilities 개체의 true입니다. 이 필드는 세션 템플릿에서 일반적으로 설정 됩니다. [MPSD 세션 템플릿](multiplayer-session-directory.md)를 참조 하세요. |
...
3.  둘 이상의 사용자에 대 한 변경 내용에 가입 하면 불필요 한 작업을 방지 하기 위해 **MultiplayerSessionChanged** 이벤트 처리기에 코드를 추가 합니다. **RealTimeActivityMultiplayerSessionChangeEventArgs.Branch 속성** 및 **RealTimeActivityMultiplayerSessionChangeEventArgs.ChangeNumber 속성**을 사용 합니다. 이러한 속성을 사용 하 여 마지막 표시 한 시간 변경 추적 하 고 이전 변경 무시 수 있습니다.
...
7.  이제 쓰려고 세션 MPSD에 설명 된 대로 **하는 방법: 업데이트 멀티 플레이어 세션**.

다음 흐름 차트가이 절차에 설명 된 이벤트를 구독 하 여 Multplayer를 시작 하는 방법을 보여 줍니다.
...
여러 사용자가 동일한 세션에 대 한 알림을 구독 경우 변경 될 때마다 해당 세션에 각 사용자에 대 한 어깨 탭을 트리거합니다. 다음 중 하나를 제외한 모든 중복 됩니다. 제목을; 알림을 이미 되는 모든 변경 내용을 무시 해야 하지만 여전히 제목을 알림에 세션에서 모든 사용자를 구독 하는 것이 좋습니다, 분기 및 ChangeNumber 속성을 사용 하 여 수행할 수 있습니다.
...
| 참고                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **RealTimeActivityMultiplayerSessionChangeEventArgs.ChangeNumber 속성** 값은 세션을 받지 **RealTimeActivityMultiplayerSessionChangeEventArgs.Branch 속성**의해 추적 해야 합니다. **RealTimeActivityMultiplayerSessionChangeEventArgs.Branch 속성** 은 세션의 수명 내에서 값을 변경 (및 다시 **RealTimeActivityMultiplayerSessionChangeEventArgs.ChangeNumber 속성** ) . |
...
| 참고                                                                                                                                                                                                                           |
|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 기본적으로 MPSD 세션에 연결 하는 첫 번째 멤버 때 만들어집니다. 제목 논리에서 없거나 가입 시 존재 하지 제목을 기대 하는 경우 세션 업데이트 중에 적절 한 쓰기 모드 값 쓰기 메서드에 전달할 수 것. |



제목 새 세션을 만들려면 다음을 수행 해야 합니다.

1.  새 **XboxLiveContext 클래스** 개체를 만듭니다. 타이틀 한 번에이 개체를 만듭니다., 저장 및 전체 소스 코드에서 필요에 따라 다시 사용 합니다. 세션 구독을 사용 하는 경우에 특히 정확히 동일한 컨텍스트를 사용 하는 데 필요한 것입니다.
...
4.  로컬 변경 작업을 마치면 연락처를 작성 MPSD에 설명 된 대로 **하는 방법: 업데이트 멀티 플레이어 세션**.
5.  MPSD에서 많은 필드 새 **MultiplayerSession** 개체를 받습니다.
6.  앞으로, 새 세션 개체를 사용 하 고 새 세션을 만들려면 숨겨진된 요청을 포함 하는 이전 복사본을 삭제 합니다.
...
| 참고                                                                                                                                       |
|---------------------------------------------------------------------------------------------------------------------------------------------------------|
| 멤버 세션에 참가 있고 해당 보안 장치 주소를 포함 될 때까지 (잠재적인 호스트) 멤버에 대 한 장치 토큰은 사용할 수 없습니다. |

1.  **MultiplayerSession.Members 속성**을 사용 하 여 호스트 후보는 MPSD에서 장치 토큰을 검색 합니다.

| 참고                                                                                                                                                                                                                     |
|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
    | 스마트 매치 메이 킹 하 여 세션이 만들어진 클라이언트에서 MPSD **MultiplayerSession.HostCandidates 속성**을 통해 사용할 수 있는 호스트 후보를 사용할 수 있습니다. |
...
4.  호스트 디바이스 토큰을 설정 하려고 할 때 수신 되는 HTTP/412 상태 코드 세션 데이터를 쿼리 하 고 로컬 콘솔에 대 한 호스트 장치 토큰 있는지 확인 합니다. 로컬 콘솔에 대 한 없으면 중재자도 지정 된 다른 콘솔.
...
5.  에 설명 된 대로, MPSD 세션을 업데이트할 **하는 방법: 업데이트 멀티 플레이어 세션**.

| 참고                                                                                                                                                                                                                                           |
|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 더 나은 알고리즘이 없는 경우 클라이언트에는 각 호스트 후보를 아직 완료가 아무도 호스트로 설정 하려고 하는 최대 알고리즘을 구현할 수 있습니다. 자세한 내용은 [중재자 세션을](mpsd-session-details.md)참조 하세요. |

## <a name="manage-title-activation"></a>제목 정품 인증 관리

Xbox One 프로토콜 활성화 하는 동안 **CoreApplicationView.Activated 이벤트** 를 발생 합니다. 멀티 플레이 API의 컨텍스트에서이 이벤트는 사용자가 초대를 수락 하거나 다른 사용자를 조인 발생 합니다. 이러한 작업 트리거 조인 사용자 대상 사용자와 게임 플레이에 연결 하 여 제목에 반응 해야 하는 활성화 합니다.
...
2.  제목 정품 인증에서 세션을 시작 하 고 세션 변경 알림을 등록 합니다. 참조 **하는 방법: MPSD 세션 변경 알림을 등록**.
3.  사용자가 활성 세션에 가입 합니다. 참조 **하는 방법: 제목 정품 인증에서 MPSD 세션에 참가할**.
4.  프로필 UI 통해 노출, 활동 세션으로 로비 세션을 설정 합니다. 참조 **하는 방법: 사용자의 현재 동작을 설정**.
...
7.  현재 세션으로 세션을 설정 합니다. 참조 **하는 방법: 사용자의 현재 동작을 설정**.

다음 흐름 차트에는 사용자가 게임을 하는 동안 다른 플레이어에 참여할 수 있도록 하기 위해 단계를 보여 줍니다.
...
1.  게임 초대 플레이어가 참여할 확인 합니다. 참조 **하는 방법: 사용자가 참여할 확인**.
2.  초대 UI를 사용 하 여 또는 초대 로비 세션을 통해 보낼 수 있는지 확인 합니다.
3.  로비 세션을 사용 하는 경우 **MultiplayerService.SendInvitesAsync 메서드**를 호출 하 여 초대를 보냅니다. 이 메서드는 게임에서 UI 명단 **SystemUI.ShowPeoplePickerAsync 메서드** 또는 **PartyChat.GetPartyChatViewAsync 메서드**를 사용 하 여 건물이 필요할 수 있습니다.
...
일반적인 시나리오를 플레이어를 수집 하 고 다음 세션 또는 매치 메이 킹 세션에는 게임 플레이에 해당 플레이어가 이동 로비 세션을 만드는 것입니다. 게임 플레이 세션을 공개적으로 참여할 없는 경우 게임 클라이언트 충족 하지 않는 한 게임 플레이 세션에 참가할 수 있지만 합니다 `joinRestriction` 설정은이 시나리오에 대 한 대부분의 경우 너무 제한적입니다.
...
3. 제목을 게임 세션 로비 세션에서 멤버를 이동 하려는 경우 각 클라이언트 사용 전송 핸들 로비 세션에서 사용 하 여 게임 세션에 참가 하는 `multiplayer_service::write_session_by_handle(multiplayerSession, multiplayerSessionWriteMode, handleId)` API입니다.
4. MPSD 로비 세션 전송 핸들을 사용 하 여 게임 세션에 참가 하는 구성원은 대기실 세션에도 확인을 조회 합니다.
5. 멤버 로비 세션에 있는 경우 게임 세션에 액세스할 수 없게 됩니다.
...
1.  **CoreApplicationView.Activated 이벤트**에 대 한 이벤트 처리기를 구현 합니다. 이 이벤트의 제목에 대 한 정품 인증에 알립니다.
2.  처리기 발생 하는 경우 **IActivatedEventArgs.Kind 속성**을 확인 합니다. 프로토콜에 설정 된 경우 **ProtocolActivatedEventArgs 클래스**에 대 한 이벤트 인수를 캐스팅 합니다.
3.  **ProtocolActivatedEventArgs** 개체를 검사 합니다. URI에 표시 된 경우 **ProtocolActivatedEventArgs.Uri 속성** 일치 하거나 inviteHandleAccept (허용 된 초대에 해당) 또는 activityHandleJoin (셸 UI 통해 조인 해당)은 URI의 쿼리 문자열을 구문 분석 다음 필드를 추출 하는 키/값 쌍을 사용 하 여 일반 URI 쿼리 문자열 형식의:
    -   허용 된 초대에 대 한:
        1.  핸들
        2.  invitedXuid
        3.  senderXuid
...
8.  **MultiplayerService.WriteSessionByHandleAsync 메서드**3 단계에 설명 된 대로 획득 핸들을 사용 하 여 호출 합니다. 이제 사용자 세션의 구성원을 게임에 연결할 세션에서 데이터를 사용할 수 있습니다.

## <a name="set-the-users-current-activity"></a>사용자의 현재 동작을 설정 합니다.

사용자의 현재 활동 제목에 대 한 Xbox 대시보드 사용자 환경에 표시 됩니다. 세션을 통해 또는 제목 활성화를 통해 사용자에 대 한 작업을 설정할 수 있습니다. 후자의 경우 사용자가 매치 메이 킹 또는 게임을 시작 하 여 세션을 입력 합니다.
...
제목 활성화를 통해 사용자의 현재 동작을 설정 하려면 다음을 참조 **하는 방법: 제목 정품 인증에서 MPSD 세션에 참가할**.

## <a name="update-an-mpsd-session"></a>MPSD 세션을 업데이트 합니다.

| 참고                                                                                                                                                 |
|-------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 타이틀 멀티 플레이 API를 사용 하 여 기존 세션을 업데이트 하면 세션을 작성 하는 호출 될 때까지 로컬 복사본으로 작동 해야 합니다. |
...
2.  모든 변경 될 때 이러한 메서드를 사용 하 여 MPSD 로컬 변경 내용을 쓸:
    ...
    쓰기 메서드 서버에 조인을 작성 하 고 다른 세션 멤버와가 콘솔의 보안 장치 주소 (SDAs)를 검색 하는 최신 세션을 가져옵니다. 이러한 콘솔 간에 네트워크 연결을 설정 하는 방법에 대 한 자세한 내용은 **Winsock Xbox One에서 소개**를 참조 하세요.

3.  로컬 이전 세션 개체를 무시 하 고 이후 작업은 최신 알려진된 세션 상태에 따라 새로 검색된 된 세션 개체를 사용 합니다.
...
2.  에 설명 된 대로 MPSD에서 게임 세션을 업데이트할 **하는 방법: 업데이트 멀티 플레이어 세션**.
...
## <a name="fill-open-session-slots-during-matchmaking"></a>매치 메이 킹 동안 열려 세션 슬롯 채우기

매치 메이 킹 티켓 세션에서 열린 슬롯 채우기 제목 다음과 유사한 단계를 따라야 합니다.

1.  매치 메이 킹 만들어진 티켓 세션에 대 한 최신 상태를 세션에 액세스 합니다.
2.  로비 세션에서 사용할 수 있는 플레이어에 대 한 게임 플레이 추가 합니다.
3.  티켓 세션 전체 인지 확인 합니다.
4.  세션을 전체 경우 게임 플레이 계속 합니다.
5.  세션 아직 전체 없는 경우에 설명 된 대로 일치 티켓을 만듭니다 **하는 방법: 일치 티켓 만들기**. 항상 설정 *preserveSession* 매개 변수를 사용 하 여 티켓 만들기를 사용 해야 합니다.
6.  매치 메이 킹 계속 진행 합니다. [스마트 매치를 사용 하 여](using-smartmatch-matchmaking.md)참조 하세요.

다음 순서도 매치 메이 킹 동안 열려 세션 슬롯 채우기 하는 방법을 보여 줍니다.
...
1.  **MatchmakingService.CreateMatchTicketAsync 메서드**를 호출 티켓 세션에 대 한 참조를 전달 합니다. 메서드는 티켓 세션 MPSD에서 읽고 매치 메이 킹 사용자 세션에서 시작 합니다. 내부적으로 메서드 **POST (/serviceconfigs/ {서비스 안내} /hoppers/ {hoppername})를**호출 합니다.
2.  매치 메이 킹 서비스를 새 세션 세션이 나 다른 기존 세션의 멤버를 일치 하는 경우 Never를 *preserveSession* 매개 변수를 설정 합니다. 제목을 게임 플레이 계속 하려면 티켓 세션으로 기존 게임 세션을 다시 사용할 수 있도록 항상 *preserveSession* 매개 변수를 설정 합니다. 제출 된 세션은 유지 하 고 일치 하는 모든 플레이어 해당 세션에 추가 되는 매치 메이 킹 서비스를 확인 한 다음 수 있습니다.

3.  매치 메이 킹 시간 사용자 설정에 **CreateMatchTicketResponse 클래스** 개체에 반환 된 **CreateMatchTicketResponse.EstimatedWaitTime 속성** 을 사용 합니다.
4.  매치 메이 킹 티켓을 삭제 하 여 필요한 경우 세션에 대 한 취소 하려면 응답 개체에 반환 된 **CreateMatchTicketResponse.MatchTicketId 속성** 을 사용 합니다. 티켓 삭제 **MatchmakingService.DeleteMatchTicketAsync 메서드**를 사용합니다.
...
3.  없으면 일치 하는 세션 및 대상 세션 참조에 대 한 일반적인 대기 시간, 매치 메이 킹 프로세스의 상태를 확인 하려면 **MatchmakingServer** 개체를 확인 합니다.
