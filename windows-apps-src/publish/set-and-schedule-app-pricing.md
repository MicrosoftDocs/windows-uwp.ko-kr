...
> [!NOTE]
> 이 항목은 앱을 가리키지만, 추가 기능 제출을 위한 가격 선택은 동일한 프로세스를 사용합니다. 에 대 한 유의 [구독 추가 기능](../monetize/enable-subscription-add-ons-for-your-app.md)를 선택 하는 기본 가격 적이 향상 될 수 없습니다 (여부를 변경 하 여 기본 가격 또는 가격 변경을 예약 하 여), 있지만 줄일 수 있습니다.
...
> [!NOTE]
> 선택 하면를 **취득을 중지** 옵션 **저장소에 사용할 수 있지만 검색할 수 없는이 제품을 확인** 에 [가시성](choose-visibility-options.md#discoverability) 섹션)를 설정할 수 없습니다 (아무도 앱을 무료로 다운로드 하는 프로 모션 코드를 사용 하지 않는 경우 앱을 획득할 수) 때문에 제출에 대 한 가격입니다.
...
> [!IMPORTANT]
> 가격 변경은 Windows 10 장치(Xbox 포함)고객에게만 표시됩니다. 이전에 게시 된 앱에서 이전 OS 버전을 지 원하는 경우 해당 고객에 게 가격 변경 내용이 적용 되지 않습니다. WIndows 8 고객의 경우, 추가 가격 변경을 예약한 경우에도 앱은 항상 **기본 가격**으로 제공됩니다(지역/국가에 특정적인 가격 없음). 고객의 Windows 8.1 및 Windows Phone 8.1 및 이전 버전의 경우 앱에서 고객의 시장에 대 한 첫 번째 가격대 제공 항상 됩니다.

가격 변경 옵션을 확인하려면 **가격 변경 예약**을 클릭합니다. 사용할 기준 가격을 선택한 다음(또는 단일 시장 기본 가격 재정의를 위해 자유 형식의 가격을 입력한 다음) 날짜, 시간 및 표준 시간대를 선택합니다.

클릭할 수 **가격 변경을 예약** 다시 원하는 수 만큼의 후속 변경 되도록 합니다.
...
> [!IMPORTANT]
> 고객의 앱을 항상 나타납니다 이전에 게시 된 앱에서 Windows 8을 지 원하는 경우 해당 **기본 가격**해당 시장에 대 한 다양 한 가격을 선택 하는 경우에 합니다.
...
여러 지역/국가에 대해 기준 가격을 재지정하려면 *지역/국가 그룹*을 만듭니다. 포함할 지역/국가를 선택하고 원하는 경우 해당 그룹의 이름을 입력합니다. (이 이름은 참조 목적 으로만 며 고객에 게 표시 되지 않습니다.) 완료 되 면, 클릭 **만들기**합니다. 그러면 위에서 설명한 **기본 가격** 및 **가격 변경 예약** 옵션이 표시되지만, 특정 시장 그룹에만 선택 사항이 적용됩니다. 자유 형식 가격은 지역/국가 그룹과 함께 사용할 수 없습니다. 사용 가능한 기준 가격을 선택해야 합니다.
