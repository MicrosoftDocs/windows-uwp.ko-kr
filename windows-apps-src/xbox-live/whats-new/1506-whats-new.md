...
Xbox Live SDK는 이제 Windows 10 PC/모바일 장치에서 주식 UI를 표시 하기 위한 제목 호출할 UI (TCUI)를 지 원하는 Api 래퍼를 포함 됩니다.
...
C + + SDK의 모든 이벤트 처리기에서 단일 변경 되었습니다 `void set_*_handler()` 메서드를 한 쌍의 `function_context add_*_handler()` 및 `void remove_*_handler(function_context context)` 메서드.
...
이러한 새 Api의 사용 예를 들어 새 *MultiplayerManager* 샘플을 참조 하세요. {*SDK 소스 루트*} \Samples\MultiplayerManager에서 샘플을 찾을 수 있습니다.
