...
이 Microsoft 스토어를 지 원하는 언어입니다. 앱에는 이러한 언어 중 하나 이상이 지원 해야 합니다. 

여기에 포함 되지 않은 언어 코드 스토어에서 지원 되지 않습니다. 이외의; 아래에 나열 된 언어 코드를 대상으로 하는 패키지를 포함 하지 않는 것이 좋습니다. 이러한 패키지는 고객에 게 배포 되지 않습니다 및 인증에 지연 또는 오류가 발생할 수 있습니다.
