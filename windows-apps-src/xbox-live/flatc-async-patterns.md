...
* **AsyncBlock** 모든 **비동기 작업** 및 **완료 콜백에서**에 관련 된 정보를 제공 합니다.
...
* *콜백* -비동기 작업이 완료 된 후 호출 되는 선택적 콜백 함수입니다.  콜백을 지정 하지 않으면 **AsyncBlock** **GetAsyncStatus** 완료 하 고 결과 얻으려면 다음을 기다릴 수 있습니다.
* *상황에 맞* 는-를 사용 하면 데이터 콜백 함수를 전달할 수 있습니다.
* *큐* 에 대 한 핸들 인 async_queue_handle_t **AsyncQueue**를 지정 합니다. 설정 되지 않은 경우 기본 큐 사용 됩니다.

각 비동기 API 호출에 대 한 힙에서 새로운 AsyncBlock 만들어야 합니다.  AsyncBlock AsyncBlock의 모든 완료 콜백이 호출 되 고 삭제 한 다음 될 때까지 라이브 해야 합니다.

> [!IMPORTANT]
> **비동기 작업이** 완료 될 때까지 **AsyncBlock** 메모리에 남아 있어야 합니다. 동적으로 할당 AsyncBlock의 **완료 콜백에서**내에서 삭제할 수 있습니다.

### <a name="waiting-for-asynchronous-task"></a>**비동기 작업** 에 대 한 대기

**비동기 작업이** 완료 되었음을 알 수 여러 가지가 있습니다.

* AsyncBlock의 **모든 완료 콜백이** 호출 되어
* 완료 될 때까지 기다릴 true로 **GetAsyncStatus** 를 호출 합니다.
* **AsyncBlock** 에 waitEvent를 설정 하 고 신호 이벤트 대기

**하지만 GetAsyncStatus** 와 waitEvent **비동기 작업이** 완료 후 간주 됩니다는 AsyncBlock의 **완료 콜백에서** 실행 AsyncBlock의 **완료 콜백에서** 선택 사항입니다.

**비동기 작업이** 완료 되 면 결과 얻을 수 있습니다.

### <a name="getting-the-result-of-the-asynchronous-task"></a>**비동기 작업** 의 결과 가져오기

결과 얻기 위해 대부분의 **비동기 API** 함수는 Function\의 해당 \[Name] 결과 함수는 비동기 호출의 결과 받을 수 있습니다. 이 예제 코드 **XblProfileGetUserProfileAsync** 해당 **XblProfileGetUserProfileResult** 함수를 포함 합니다. 이 함수를 사용 하 여 함수의 결과 반환 하 고 그에 따라 역할을 수 있습니다.  자세한 내용은 각 **비동기 API** 함수의 documention 검색 결과에서 참조 하세요.
...
* *수동* -수동 큐는 자동으로 발송 하지 않습니다.  개발자가 원하는 모든 스레드에서 발송 됩니다. 특정 스레드에 비동기 호출에 있는 작업 또는 콜백 할당을 사용할 수 있습니다.  이 아래에 자세히 설명 합니다.

* *스레드 풀* -디스패치 스레드 풀을 사용 합니다.  스레드 풀 threadpool 스레드를 사용할 수 있는 되 면 큐에서 다시 실행에 대 한 호출을 수행 하는 동시에 호출을 호출 합니다.  사용 하 여 easist 이지만 최소한의 스레드 사용 되는 컨트롤을 제공 합니다.

* *고정 스레드* -QueueUserAPC를 사용 하 여 비동기 큐를 만든 스레드 디스패치 됩니다. 사용자 모드 APC 대기 중인 경우 스레드 하지 APC 함수를 호출 하 여 경고 상태가 아니면로 리디렉션됩니다. 경고 대기 작업을 수행 하도록 **SleepEx**, **SignalObjectAndWait**, **WaitForSingleObjectEx**, **WaitForMultipleObjectsEx**또는 **MsgWaitForMultipleObjectsEx** 를 사용 하 여 상태가 경고 스레드
...
큐에 대 한 ID를 지정 하 여 큐 유형이 동일한 **AsyncQueue** 만들려면 **CreateSharedAsyncQueue** 호출할 수도 있습니다.
...
> [!NOTE]
> 이미 있는 경우이 ID와 디스패치 모드를 사용 하 여 큐, 참조 됩니다.  그렇지 않은 경우 새 큐 생성 됩니다.

만든 후에 **AsyncQueue** 에 추가 하면 **AsyncBlock** 작업과 완료 함수에는 스레딩 제어할 수 있습니다.
...
### <a name="manually-dispatching-an-asyncqueue"></a>수동으로 **AsyncQueue** 디스패치

**AsyncQueue** 작업 또는 완료 큐에 대 한 수동 큐 디스패치 모드를 사용 하는 경우 수동으로 발송 해야 합니다.
작업 큐와 완료 큐 수동으로 발송 설정 되어 있는 **AsyncQueue** 생성 된 가정해 되도록 합니다.
...
* *timeoutInMs* -uint32_t 밀리초에서 시간 제한 합니다.
...
* *상황에 맞* 는-전송 콜백을 전달 되는 데이터에 대 한 포인터입니다.
...
이 가장 좋습니다 Win32 세마포의 개체를 사용 하 여 시행 합니다.  구현 하는 경우 대신 Win32 이벤트 개체를 사용 하 여 확인 해야 하는 다음 잊지 코드를 사용 하 여 모든 이벤트와 같은:
...
[소셜 C 샘플 AsyncIntegration.cpp](https://github.com/Microsoft/xbox-live-api/blob/master/InProgressSamples/Social/Xbox/C/AsyncIntegration.cpp) 에서 비동기 통합에 대 한 유용한의 예를 볼 수 있습니다.
