...
UWP 용 생성 되는 특수 한 프로젝트 이며 자산에 배치한 winmd 파일에 대 한 참조를 포함 합니다.
...
5.  Unity UWP 응용 프로그램에서 Unity 게임을 래핑하는 UWP Visual Studio 프로젝트를 생성에 대 한 "빌드"를 클릭 합니다. 위치에 대 한 메시지가 나타나면 많은 새 파일이 생성 됩니다 때문에 혼동을 피하기 위해 새 폴더를 만듭니다. "빌드" 폴더를 호출 하 고 해당 폴더를 선택 하면 좋습니다.
...
이렇게 하려면 파일로 이동 \ | 빌드 설정 하 고 **유니버설 Windows 플랫폼** 플레이어의 "빌드 설정" 클릭 위에서 가져온 PFX 파일을 바꾸어야 PFX 단추를 클릭 합니다. Unity에서 프로젝트를 다시 빌드할 때마다 PFX 파일을 삭제할 수도 있습니다.
...
d) 주식.pfx 파일 Unity 제공한 디스크에서 삭제 하 고, 참조 하는.csproj 줄을 제거 하거나 올바른 id 없습니다 또는 Visual Studio에서 프로젝트를 클릭 하 고 "저장"을 선택 하는 오른쪽 \ | "연결 앱 스토어를 사용 하 여" 적절 한.pfx 파일 배치 됩니다.  Unity를 다시 클릭 "빌드 설정" **유니버설 Windows 플랫폼** 플레이어에서 하 고 Visual Studio의 "의 저장소와 앱 연결" 작업에서 가져온.pfx 파일로 대체 하려면 PFX 단추 클릭 이동 하는 다음 해야 합니다.
