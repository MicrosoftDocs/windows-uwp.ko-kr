...
Windows 스토어 제출 API에서는 다음 메서드를 사용하여 앱의 추가 기능(앱에서 바로 구매 제품 또는 IAP라고도 함)을 관리합니다. Windows 스토어 제출 API에 대한 자세한 내용은 [Windows 스토어 서비스를 사용하여 제출 만들기 및 관리](create-and-manage-submissions-using-windows-store-services.md)를 참조하세요.
