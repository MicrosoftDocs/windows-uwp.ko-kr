...
3. 프로젝트가 열리면 Visual Studio에서 **프로젝트 메뉴**를 클릭하고 **스토어**를 가리킨 다음 **스토어에 앱 연결**을 클릭합니다. 테스트에 사용 하려는 파트너 센터 계정에서 앱을 사용 하 여 앱 프로젝트에 연결 마법사의 지침을 완료 합니다.
    > [!NOTE]
...
> [!NOTE]
> 추가 기능에 대 한 파트너 센터에서 입력 한 제품 ID에 대 한 추가 기능 [스토어 ID](#store-ids)보다 차이가 있습니다. 스토어 ID는 파트너 센터에서 생성 됩니다.
