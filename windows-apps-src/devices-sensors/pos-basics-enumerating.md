...
<br/>
장치 선택기 UI를 표시 하 고 사용자를 연결 된 장치를 선택 합니다. 이 메서드는 장치를 연결 하 고 제거 때 목록 업데이트를 처리 하 고 더 간단 하 고 다른 방법 보다 안전 하 게.
...
**방법 4:** [열거 및 감시](#Method-3:-Enumerate-and-watch)<br />[DeviceWatcher](https://docs.microsoft.com/uwp/api/Windows.Devices.Enumeration.DeviceWatcher) 를 현재 존재 하는 장치를 열거 하 고 장치가 추가 되거나 시스템에서 제거 되 면 알림을 받을 수 있는 더 강력 하 고 유연한 열거형 모델입니다.  스냅샷이 표시되기까지 기다리는 것보다 UI에 표시하기 위해 백그라운드에서 디바이스의 현재 목록을 유지하려는 경우 유용합니다.
...
[선택 기가](https://docs.microsoft.com/uwp/api/windows.devices.pointofservice.posconnectiontypes) 값을 매개 변수로 사용 하는 **GetDeviceSelector** 메서드를 사용 하 여 제한할 수 있습니다 선택기를 열거 로컬, 네트워크, 또는 Bluetooth 연결 된 POS 디바이스 쿼리가 완료 하는 데 걸리는 시간이 줄어듭니다.  아래 샘플 로컬로 지 원하는 선택기를 정의 하려면이 메서드를 사용 하는 연결 POS 프린터를 보여 줍니다.
...
서비스 지점 장치 하는 가장 간단한 방법은 서비스 지점 디바이스 클래스 내에서 사용 가능한 첫 번째 장치 가져올 **GetDefaultAsync** 를 사용 하는 것입니다. 
...
* [선택 기가 열거형](https://docs.microsoft.com/uwp/api/windows.devices.pointofservice.posconnectiontypes)
