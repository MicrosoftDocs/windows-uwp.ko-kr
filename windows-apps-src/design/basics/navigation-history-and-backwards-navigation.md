...
위의 단일 페이지 탐색을 처리 뒤로 합니다. 뒤로 탐색에서 특정 페이지를 제외 하려는 또는 페이지를 표시 하기 전에 페이지 수준 코드를 실행 하려는 경우 각 페이지에서 탐색을 처리할 수 있습니다.
...
앱 [AppViewBackButtonVisibility](https://docs.microsoft.com/uwp/api/windows.ui.core.appviewbackbuttonvisibility)를 사용 하 여 계속 되 면 UI는 시스템 렌더링 시스템 뒤로 단추:
...
- 앱 **탭**을 경우 새 시스템 뒤로 내 뒤로 단추가 렌더링 됩니다 모음입니다.
