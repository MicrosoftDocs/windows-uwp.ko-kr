...
| 메서드 | 요청 URI                                                      |
|--------|------------------------------------------------------------------|
| 가져오기   | ```https://admetadata.azure-api.net/v1/app/{app_id}``` |
...
| 이름        | 형식   | 설명                                                                 |
|---------------|--------|-----------------------|
| app_id | string | 필수. 메타데이터를 검색할 앱의 ID입니다. 다음 값 중 하나일 수 있습니다.<br/><br/><ul><li>앱의 스토어 ID입니다. 예를 들어 스토어 ID는 9NBLGGH29DM8입니다.</li><li>Windows 8.x 또는 Windows Phone 8.x용으로 작성한 앱의 제품 ID(*앱 ID*라고도 함)입니다. 제품 ID는 GUID입니다.</li></ul> |
...
| description           | string  | 앱의 스토어 목록에 대한 설명입니다.  |
| phoneStoreGuid           | string  | 앱의 제품 ID(Windows Phone 8.x)입니다. 이 ID는 GUID입니다.  |
| windowsStoreGuid           | string  | 앱의 제품 ID(Windows 8.x)입니다. 이 ID는 GUID입니다. |
| storeCategory           | string  | 스토어에서 앱 범주입니다. 지원되는 값은 스토어에서 앱의 [범주 및 하위 범주 테이블](../publish/category-and-subcategory-table.md)을 참조하세요.  |
| iabCategory           | string  | IAB(Interactive Advertising Bureau)에서 정의한 앱의 콘텐츠 범주입니다. 예를 들면 **뉴스** 또는 **스포츠**가 됩니다. 콘텐츠 범주 목록은 IAB 웹 사이트에서[IAB Tech Lab Content Taxonom](https://www.iab.com/guidelines/iab-quality-assurance-guidelines-qag-taxonomy/)(IAB 기술 랩 콘텐츠 분류) 페이지를 참조하세요.   |
| iabCategoryId           | string  | 앱에 대한 콘텐츠 범주의 ID입니다. 예를 들어 **IAB12**는 뉴스 범주에 대한 ID, **IAB17**은 스포츠 범주에 대한 ID입니다. 콘텐츠 범주 ID 목록은 [OpenRTB API Specification](https://www.iab.com/wp-content/uploads/2015/05/OpenRTB_API_Specification_Version_2_3_1.pdf)(OpenRTB API 사양)의 섹션 5.1을 참조하세요. |
| coppa           | Boolean  | 앱이 13세 미만 아동을 대상으로 하여 COPPA(아동에 관한 온라인 개인 정보 보호법)에 따른 의무 조항이 있으면 True, 그렇지 않으면 False입니다.  |
| downloadUrl           | string  | 스토어에서 앱의 목록에 대한 링크입니다. 이 링크는 ```https://www.microsoft.com/store/apps/<Store ID>``` 형식입니다.  |
| isLive           | Boolean  | 앱이 현재 스토어에 제공되면 True이고, 그렇지 않으면 false입니다.  |
| iconUrls           | 배열  |  앱과 연결된 아이콘 URL의 상대 경로를 포함하는 하나 이상의 문자열 배열입니다. 아이콘을 검색하려면 URL 앞에 *http* 또는 *https*를 추가합니다.  |
| type           | string  | 다음 문자열 중 하나입니다. **앱** 나 **게임**합니다.  |
| 디바이스           |  배열  | 하나 이상의 앱을 지 원하는 장치 유형을 지정 하는 다음 문자열의 배열: **PC**, **Phone**, **Xbox**를 **IoT**, **Server**, 및 **Holographic**합니다.  |
| platformVersions           | 배열  |  하나 이상의 앱을 지 원하는 플랫폼을 지정 하는 다음 문자열의 배열: **Windows.Universal**하십시오 **Windows.Windows8x**, 및 **Windows.WindowsPhone8x**합니다.  |
