...
--- | --- | --- | ---
이끌이 이름 | 종료 시간 등이 토너먼트 된 날짜/시간에 "취소" 또는 "Completed" 상태에 도달 합니다. 토너먼트 업데이트 될 때 자동으로 설정 됩니다. | 이전 일치 (토너먼트 게임 결과 상태, 등급, 일치 시작 시간, 종료 시간은는 Bye, 설명, 팀 Id 반대) | 등록 상태 (보류, 알 수 없는 제거, 거부, 등록, 완료 됨)
토너먼트 이름 (최대 128 문자) | 이 Bye   | 팀 상태 (알 수 없음, 등록, 인원이, 대기, 체크 인, 재생, 완료 됨) | 등록 이유 (알 수 없는, 닫힘, 구성원을 이미 등록, 전체 토너먼트, 제거 팀, 완료 토너먼트)
토너먼트 설명 (최대 1000 문자) | 토너먼트 게임 결과 상태 (없음 콘테스트/취소 win, 손실, 그리기, 순위, 표시 되지 않음) | 팀은 완료 된 상태의 이유 (거부, 제거, 제거, 완료 됨) | 등록 된 팀의 최소 및 최대 수
등록 시작/종료 시간 | 중재 상태: 완료 되 면 없음, 취소, 결과, 일부 결과 없음 | 등록 날짜-날짜 및 시간 팀 등록 되었습니다. |
체크 인 시작/종료 시간 | 설명 '레이블을' – 일치 ("최종, 열 1") | 팀이 서 | 등록은 열기
재생 시작/종료 시간 | 시작 시간 | 팀 이름 | 열기에서 확인 됩니다.
상품에 | 상대 팀 Id | 팀 최종 순위 | 등록 시작/종료 시간
최소/최대 팀 크기 | | 연속 작업 URI-아레나 ui 팀 구성원을 다시 사용 | 체크 인 시작/종료 시간
