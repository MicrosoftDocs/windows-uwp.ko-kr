...
-   **등급 (알림)을 요청 합니다.** 고객에게 앱 평가를 요청하는 알림 메시지. 알림을 선택하면 앱에 대한 Microsoft Store 평점 페이지가 표시됩니다.
...
-   **알림 (알림).** 선택한 다른 앱을 홍보하기 위한 알림 메시지. 고객이 알림을 선택하면 다른 앱의 Microsoft Store 목록이 표시됩니다.
    > [!NOTE]
...
-   **판매 (알림)를 홍보 합니다.** 앱 특가 판매를 발표하기 위해 사용할 수 있는 알림 메시지. 고객이 알림을 선택하면 앱의 Microsoft Store 목록이 표시됩니다.
