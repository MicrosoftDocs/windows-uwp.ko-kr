---
터치 패드에 최적화된 동시에 입력 디바이스 간에 기능적으로 일관된, 직관적이고 차별화된 사용자 조작 환경을 갖춘 UWP(유니버설 Windows 플랫폼) 앱을 만들 수 있습니다.
터치 패드 조작
...
터치 패드는 간접 멀티 터치 입력을 마우스와 같은 포인팅 디바이스의 정밀도 입력과 결합합니다. 이러한 결합을 통해 터치 패드는 터치 최적화된 UI와 생산성 앱 및 데스크톱 환경의 작은 대상에 모두 적합합니다. Windows 스토어 앱 디자인을 터치식 입력에 최적화하고 기본적으로 터치 패드 지원을 받으세요.
...
-   가리키기 피드백에 커서를 사용하지 마세요. 요소에서 제공하는 피드백만으로 충분합니다(아래 [커서](#Cursors) 참조).
...
각 표준 커서에는 해당 기본 이미지가 연결되어 있습니다. 사용자나 앱은 언제든지 표준 커서와 연결된 기본 이미지를 바꿀 수 있습니다. Windows 스토어 앱은 [**PointerCursor**](https://msdn.microsoft.com/library/windows/apps/br208273) 함수를 통해 커서 이미지를 지정합니다.
