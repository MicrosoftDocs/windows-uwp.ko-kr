...
description: 이 섹션에는 Windows 10 버전을 Windows 8.1 이전 버전에서 변경 내용을 지원 하기 위해 PlayReady 웹 앱을 수정 하는 방법을 설명 합니다.
...
이 섹션에는 Windows 10 버전을 Windows 8.1 이전 버전에서 변경 내용을 지원 하기 위해 PlayReady 웹 앱을 수정 하는 방법을 설명 합니다.
...
이 섹션에서는 Windows 10에 대 한 PlayReady 콘텐츠 보호를 사용 하도록 설정 하는 PlayReady 확장 EME (Encrypted Media) 대 한 변경 내용 목록을 제공 합니다.

다음 목록에는 새로운 기능 및 PlayReady 암호화 된 미디어 확장에 대 한 Windows 10에 대 한 변경 내용을 설명 합니다.
...
-   한 메시지로 여러 라이선스를 취득할 수 있습니다.
    ...
    Windows 8.1 같이 여러 키 식별자 (KeyIDs)를 사용 하 여 PlayReady를 사용 하거나 사용 하 여 [콘텐츠 모델 데이터 암호 해독 (CDMData)](https://go.microsoft.com/fwlink/p/?LinkID=626819) 여러 KeyIDs를 사용 하 여 합니다.

    > [!NOTE]
    > Windows 10에서 여러 키 식별자에서 지원 되는 &lt;KeyID&gt; CDMData에서.
...
> [!NOTE]
> 보안 중지 데이터 `<SessionID>B64 encoded session ID</SessionID>` 위의 샘플에서 별표를 수 있습니다 (\*), 기록 된 모든 보안 중지 세션에 대 한 와일드 카드는 합니다. 즉,는 **SessionID** 태그는 특정 세션 또는 와일드 카드를 수 있습니다 (\*) 모든 보안 중지 세션을 선택 합니다.

## <a name="programming-considerations-for-encrypted-media-extension"></a>암호화된 미디어 확장에 대한 프로그래밍 고려 사항

이 섹션에서는 Windows 10 용 PlayReady 사용 웹 앱을 만들 때 고려해 야 하는 프로그래밍 고려 사항을 나열 합니다.

앱에서 만든 **MSMediaKeys** 및 **MSMediaKeySession** 개체는 앱을 닫을 때까지 활성화되어 있어야 합니다. 이러한 개체를 활성 상태로 유지하는 한 가지 방법은 해당 개체를 전역 변수로 할당하는 것입니다. 변수가 함수 내부의 로컬 변수로 선언된 경우에는 변수가 범위에서 벗어나게 되며 가비지 수집에 따라 달라질 수 있습니다. 다음 샘플에서 변수를 할당 하는 예를 들어 *g\_msMediaKeys* 하 고 *g\_mediaKeySession* 전역 변수로 다음에 할당 되는 **MSMediaKeys** 하 고 **MSMediaKeySession** 함수에는 개체입니다.
