...
요약 사용자 메시지 메시지 제목을 포함 되어 있습니다. 사용자 생성 메시지,이 현재 메시지 텍스트의 처음 20 자입니다. 시스템 메시지 "라이브 시스템" 등의 대체 주체를 제공할 수 있습니다.

메시지; 보낸 된 순서의 반대 방향으로 돌아갑니다. 즉, 최신 메시지는 먼저 반환 됩니다.
...
| skipItems| int| 100| 다양 한 메시지를 건너뜁니다. continuationToken 존재 하는 경우 무시 됩니다.|
...
| 전송| DateTime|  | UTC 날짜 및 시간 보낸 메시지입니다. (서비스에서 제공).|
...
| hasText| 부울|  | 메시지 텍스트를 포함 되어 있는지 여부.|
