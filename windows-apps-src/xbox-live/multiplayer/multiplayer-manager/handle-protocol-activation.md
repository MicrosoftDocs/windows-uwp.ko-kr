...
마지막으로, 멀티 플레이어 관리자 게임 이미 진행 중에서에 있는 경우 피어에 대 한 공간이 가입 게임 세션에 사용자를 자동 됩니다. 제목 통해 알려주지 합니다 `join_game_completed` 이벤트는 적절 한 오류 코드와 메시지를 제공 합니다.
