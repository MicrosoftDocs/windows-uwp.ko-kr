...
더 대상된 일련의 항목에는 엔터테인먼트 검색 서비스 (EDS) 쿼리를 구체화 하는 다음 매개 변수를 사용할 수 있습니다. 모든 API에 필요한 것은 이러한 매개 변수 중 아니지 쿼리 구체화를 허용 하는 모든 API에서 허용 하 게 합니다.
 
매개 변수 이름은로 전달할 수의 값으로 "queryRefiners" 매개 변수입니다. 이 다음 쿼리 구체화를 사용 하 여 요청 된 반복 하는 경우 반환 되는 항목 수가 적용 쿼리 구체화의 각 값으로 나눌 반환 합니다.
...
| 매개 변수| 데이터 형식| 설명| 
| --- | --- | --- | 
| <b>10 년</b>| string| 모든 항목 해야 릴리스된 지난 10 년간 합니다.| 
| <b>장르</b>| 문자열의 배열| 목록 항목을 모두 있어야 하는 장르입니다.| 
| <b>labelOwner</b>| string| 트랙, 앨범 또는 아티스트 관련 된 음악 레이블.| 
| <b>네트워크</b>| 문자열의 배열| 항목에서 만든 네트워크입니다.| 
| <b>studio</b>| 문자열의 배열| 항목에서 만든 studio를 지정 합니다.| 
| <b>xboxAppCategories</b>| 문자열의 배열| 목록 모든 Xbox 앱에 있어야 하는 범주입니다.| 
