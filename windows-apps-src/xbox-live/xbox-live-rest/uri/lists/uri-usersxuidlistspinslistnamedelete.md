...
제거할 항목 목록에서 해당 인덱스가 표시 됩니다 하 고 쿼리 문자열 매개 변수 **인덱스**에 식별 됩니다. 인덱스 목록을 쉼표로 구분 된 및 100 개의 인덱스 단일 호출에 전달 될 수 있습니다. 그러나 인덱스가 (빈 쿼리 문자열 매개 변수)을 전달 하는 경우 전체 목록 내용을 삭제 됩니다 (빈 목록을 유지 됩니다 및 버전 번호를 계속 증가 됩니다). 항목이 제거 되 면 목록은 "압축"를 빈 인덱스 순서를 정하는에 남아 있습니다. 따라서이 호출이 idempotent 아닙니다.
 
이 호출 해야는 **경우-일치: versionNumber** 헤더를 **versionNumber** 는 파일의 현재 버전 번호는 요청에 포함 되어야 합니다. 포함 하지 않거나는 HTTP 412 (전제 조건 실패) 다음 현재 목록 버전 번호와 일치 하지 않는 경우에 상태 코드 반환 되 고 응답 본문의 현재 버전 번호를 포함 하는 목록 최신 메타 데이터를 포함 됩니다. 다른에서 무시 되는 다른 클라이언트에서 업데이트를 수행 됩니다.
...
예: **인덱스 = 1, 8** 인덱스 1과 8에는 항목을 제거 합니다. 인덱스는 고유 해야 합니다. 인덱스가 제공 되는 경우 전체 목록을 지워집니다.
