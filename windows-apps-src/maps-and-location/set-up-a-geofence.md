---
지오펜스 설정
앱에서 지오펜스를 설정하고 포그라운드 및 백그라운드에서 알림을 처리하는 방법을 알아봅니다.
...
이 단계의 코드는 지오펜싱 백그라운드 작업을 등록합니다. 지오펜스가 생성되면 위치 사용 권한을 확인한다는 점을 기억하세요. 자세한 내용은 [지오펜스 설정](#setup)을 참조하세요.
...
-   **이 디바이스의 위치...** 가 **켜짐** 상태임(Windows 10 Mobile에는 해당되지 않음)
