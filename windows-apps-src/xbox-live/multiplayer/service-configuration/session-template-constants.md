...
패키지를 할당할 수를 계산 하는 클라우드의 속성을 지정 합니다. 해야 하는 `cloudCompute` 접근 권한 값을 설정 합니다.
...
마지막 소유자 세션을 벗어날 때 세션을 처리 하는 방법을 지정 합니다. 해야 하는 `hasOwners` 접근 권한 값을 설정 합니다.
