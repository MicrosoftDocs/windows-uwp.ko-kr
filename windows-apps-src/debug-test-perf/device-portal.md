...
- [Xbox 용 장치 포털](device-portal-xbox.md)
...
성능 추적 페이지 보기 수를 [Windows 성능 기록기 (WPR)](https://msdn.microsoft.com/library/hh448205.aspx) 호스트 장치에서 추적 합니다.
...
캡처됩니다. 분석에 대해 ETL 파일을 열 수를 [Windows Performance Analyzer](https://msdn.microsoft.com/library/windows/desktop/hh448170.aspx)합니다.
...
D | 문자열 | 디바이스의 유형입니다. "Windows.*" 형식으로 제공됩니다(예: Windows.Xbox 또는 Windows.Desktop).
A | 문자열 | 디바이스 아키텍처입니다. ARM, x86 또는 AMD64입니다.  
...
> - "자동-" username을 사용 합니다. 사용자 이름 앞에 "auto-"를 추가하는 클라이언트는 CSRF 보호를 우회하게 됩니다. 이 사용자 이름은 브라우저를 통해 디바이스 포털에 로그인하는 데 사용할 수 없습니다. 서비스가 CSRF 공격에 노출되기 때문입니다. 예제: 장치 포털의 사용자 이름은 "admin" 경우 ```curl -u auto-admin:password <args>``` CSRF 보호 하지 않으려면 사용 해야 합니다. 
