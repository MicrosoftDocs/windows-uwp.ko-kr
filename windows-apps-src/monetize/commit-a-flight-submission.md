...
# Windows 스토어 제출 API를 사용하여 패키지 플라이트 제출 커밋




Windows 스토어 제출 API에서 이 메서드를 사용하여 Windows 개발자 센터에 새롭거나 업데이트된 패키지 플라이트 제출을 커밋합니다. 커밋 작업은 제출 데이터가 업로드되었음(모든 관련된 패키지 포함)을 개발자 센터에 알려줍니다. 응답에서 개발자 센터는 수집 및 게시를 위해 제출 데이터에 대한 변경 사항을 커밋합니다. 커밋 작업이 성공하면 개발자 센터 대시보드에 제출의 변경 사항이 표시됩니다.

커밋 작업이 Windows 스토어 제출 API를 사용하여 패키지 플라이트 제출을 만드는 프로세스에 적용되는 방법은 [패키지 플라이트 제출 관리](manage-flight-submissions.md)를 참조하세요.
...
| flightId | 문자열 | 필수. 커밋할 제출이 포함된 패키지 플라이트의 ID입니다. 이 ID는 개발자 센터 대시보드에서 사용할 수 있으며 [패키지 플라이트 만들기](create-a-flight.md) 및 [앱의 패키지 플라이트 가져오기](get-flights-for-an-app.md) 요청에 대한 응답 데이터에 포함되어 있습니다.  |
| submissionId | 문자열 | 필수. 커밋할 제출의 ID입니다. 이 ID는 개발자 센터 대시보드에서 사용할 수 있으며 [패키지 플라이트 제출 만들기](create-a-flight-submission.md) 요청에 대한 응답 데이터에 포함되어 있습니다.  |
