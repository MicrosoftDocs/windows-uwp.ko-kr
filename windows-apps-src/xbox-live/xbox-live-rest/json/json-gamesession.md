...
| isClosed| 부울 값| True 이면 없는 더 많은 플레이어 수에 추가 되 고 false 그렇지 않으면 고 세션이 닫힙니다. 이 값이 true 이면 세션에 참가할 요청이 거부 됩니다. | 
...
| 명단| 플레이어 개체의 배열| 세션에서 플레이어의 배열입니다. 명단 현재 및 플레이어 세션의 이전에 않지만 단어가 포함 되어 있습니다. 플레이어 명단의 순서 변경 되지 않습니다. 새 플레이어는 배열의 끝에 추가 됩니다. | 
