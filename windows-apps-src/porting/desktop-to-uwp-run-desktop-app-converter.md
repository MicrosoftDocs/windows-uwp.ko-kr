...
``InstallerArguments`` 매개 변수는 선택적 매개 변수입니다. 그러나 Desktop App Converter를 무인된 모드에서 실행 되도록 설치 관리자가 필요 하므로 응용 프로그램에서 자동으로 실행 하도록 자동 플래그가 필요한 경우 사용 해야 할 수 있습니다. ``/S`` 플래그는 매우 일반적인 자동 플래그이지만, 설치 파일을 만드는 데 사용한 설치 관리자 기술에 따라 사용하는 플래그가 달라질 수 있습니다.
...
이 예는 로컬 테스트를 위해 응용 프로그램 서명 하 고 다음 응용 프로그램 패키지 된 앱 및 Microsoft Store 요구 사항에 대해 유효성을 검사할 수 방법을 보여준다 첫 번째 비슷합니다.
...
|-MakeMSIX [&lt;SwitchParameter&gt;]  |선택 사항 |스위치가 있을 경우이 스크립트를 MSIX 패키지로 출력 패키지를 통해 합니다. |
...
또 다른 방법은 인증서를 사용 하 여 응용 프로그램에 서명 하는 것입니다. 사용 하는 경우는 ```sign``` 매개 변수, Desktop App Converter를 위한 인증서를 생성 및 다음 받는지 응용 프로그램에 서명 합니다. 해당 파일의 이름은 **auto-generated.cer**이며, 패키징된 앱의 루트 폴더에서 찾을 수 있습니다.
