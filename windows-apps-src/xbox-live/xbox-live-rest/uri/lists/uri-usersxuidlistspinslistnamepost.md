...
ms.locfileid: "57589978"
---
# <a name="post-usersxuidxuidlistspinslistname"></a>POST (/users/xuid(xuid)/lists/PINS/{listname})
쿼리 문자열 매개 변수 기반으로 인덱스에 있는 목록에 항목 삽입 **insertIndex**합니다. 이러한 Uri에 대 한 도메인은 `eplists.xboxlive.com`합니다.
...
이 호출은 쿼리 문자열 매개 변수 기반으로 하는 인덱스에 있는 목록에 항목 삽입 **insertIndex** (기본값은 0 또는 목록의 시작 부분). 요청 본문의 모든 항목 목록에서 해당 지점에서 삽입 됩니다. 경우는 **insertIndex** 수보다 크면 기존 목록에 항목을 새 항목 끝에 삽입 됩니다.
 
삽입할 항목을 필수 필드 표시 기능 사양;에 있어야 합니다. 그렇지 않은 경우 HTTP 400이 반환 됩니다. 마찬가지로, 목록 (각 목록 형식 정의)의 최대 크기를 초과 하는 삽입의 결과 HTTP 400이 반환 됩니다 고 아무 것도 삽입 됩니다.
 
항목이 *되지* 시작 또는 목록 끝에 삽입 되는 **경우-일치: versionNumber** 헤더는 요청에 포함 합니다. 헤더 시작 또는 끝을 삽입 하는 경우 선택 사항입니다. 있는 경우 삽입 위치에 관계 없이 헤더 유효성 검사가 수행 됩니다. 헤더에 **VersionNumber** 목록의 현재 버전입니다. 포함 및 필요한 되지 않는 경우 현재 목록 버전 번호를는 HTTP 412 (전제 조건 실패)와 일치 하지 않습니다 또는 상태 코드가 반환 되 고 응답의 본문에는 현재 버전 번호를 포함 하는 목록의 최신 메타 데이터 포함 됩니다. 이 서로에서 무시 되는 다른 클라이언트에서 업데이트에 대해 보호 됩니다.
 
이 호출은 idempotent;는 동일한 데이터를 사용 하 여 반복 해 서 호출 여러 삽입 될 수 있습니다. 그러나 현재 목록이 없습니다. 중복 요소를 지원 하므로 반복 해 서 호출 HTTP 400 발생 코드 반환 됩니다.
...
| 매개 변수| 형식| 설명| 
| --- | --- | --- | --- | --- | --- | 
| insertIndex| 문자열| 선택 사항. 항목을 삽입할 위치를 정의 합니다. 지원 되는 값: 0으로 양의 정수로 및 "end"입니다. 목록 항목 수보다 큰 인덱스 값 목록의 맨 아래에 새 항목 추가 하 고 "공백" 목록에 삽입 되지 않습니다. 기본값: 0.| 
...
**ContentType**, **ItemId** 또는 **ProviderId**, **공급자** 하 고 **로캘** 은 필수입니다.
