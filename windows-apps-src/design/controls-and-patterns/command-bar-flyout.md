...
> 관련된 정보 [플라이 아웃](../controls-and-patterns/dialogs-and-flyouts/flyouts.md) [메뉴 및 상황에 맞는 메뉴를](menus.md)및 [명령 모음](app-bars.md)을 참조 하세요.

[CommandBar](app-bars.md)같은 CommandBarFlyout 속성이 **PrimaryCommands** 와 **SecondaryCommands** 는 명령을 추가 하는 데 사용할 수 있습니다. 명령 컬렉션 또는 둘 다에 배치할 수 있습니다. 기본 및 보조 명령이 표시 되어 시기와 방법을 디스플레이 모드에 따라 달라 집니다.

명령 모음 플라이 아웃에 두 가지 표시 모드: *확장*및 *축소* 합니다.

- 축소 된 모드에서 기본 명령만 표시 됩니다. 기본 및 보조 명령 모음 플라이 아웃에 있으면 명령에 "자세히" 단추를 줄임표로 표시 되는 \ [• • •] 표시 됩니다. 이 확장 모드로 전환 하 여 보조 명령에 액세스할 수가 있습니다.
- 확장 된 모드에서 기본 및 보조 명령이 표시 됩니다. (컨트롤 보조 항목에 대해서만 있으면에 나타나는 MenuFlyout 컨트롤에 유사한 방식으로.)
...
명령 모음 단추 및 앱 캔버스 요소의 컨텍스트 메뉴 항목 등의 사용자에 게 표시 하려면 CommandBarFlyout 컨트롤을 사용 합니다.

TextCommandBarFlyout TextBox, TextBlock, RichEditBox, RichTextBlock, 및 PasswordBox 컨트롤의 명령을 텍스트를 표시합니다. 현재 텍스트 선택 영역 명령은 적절 하 게 자동으로 구성 됩니다. CommandBarFlyout를 사용 하 여 텍스트 컨트롤에 기본 텍스트 명령을 바꿉니다.

상황에 맞는 표시 목록 항목에 명령을 [명령 컬렉션 및 리스트에 대 한 상황별](collection-commanding.md)의 지침을 따릅니다.

### <a name="commandbarflyout-vs-menuflyout"></a>CommandBarFlyout vs MenuFlyout

명령을 상황에 맞는 메뉴를 표시 하려면 CommandBarFlyout MenuFlyout을 사용할 수 있습니다. MenuFlyout 보다 더 많은 기능을 제공 하기 때문에 CommandBarFlyout 권장 합니다. 동작을 가져오거나는 MenuFlyout의 모양 및 기본 및 보조 명령을 사용 하 여 전체 명령 모음 플라이 아웃을 사용 하 여 CommandBarFlyout만 보조 명령으로 사용할 수 있습니다.
...
<td>
    <p><strong style="font-weight: semi-bold">XAML 컨트롤 갤러리</strong> 앱이 설치 된 경우 여기를 클릭 하 <a href="xamlcontrolsgallery:/item/CommandBarFlyout">앱을 열고 중인 CommandBarFlyout를 참조 하세요</a>.</p>
    <ul>
...
일반적으로 두 가지 방법으로 플라이 아웃 이나 UI 캔버스에 요소와 연결 된 메뉴 호출: _사전 호출_ 및 _사후 호출_합니다.

사전 호출에서 명령을와 관련 된 항목을 사용 하 여 사용자가 명령이 자동으로 표시 됩니다. 예를 들어 텍스트 서식 명령이 수 팝업 사용자가 텍스트 상자에 텍스트를 선택 합니다. 이 경우 명령 모음 플라이 아웃은 포커스를 고려 하지 않습니다. 대신 사용자가 상호 작용 항목 가까운 관련 명령을 제공 합니다. 사용자는 명령을 상호 작용 하지, 해제 됩니다.

사후 호출 명령은 표시 되어 명시적인 사용자 작업에 대 한 응답에서 명령을 요청 예를 들어, 마우스 오른쪽 단추를 클릭 합니다. 이 [상황에 맞는 메뉴](menus.md)의 일반적인 개념에 해당합니다.
...
이 예제에서는 명령 모음 플라이 아웃을 만들고 사전 및 사후 방식으로 사용 하는 방법을 보여 줍니다. 이미지는 터치 할 때 축소 된 모드로 플라이 아웃이 표시 됩니다. 상황에 맞는 메뉴로 표시 하는 경우 플라이 아웃이 확장 모드로 표시 됩니다. 두 경우 모두 사용자가 확장 하거나를 연 후 플라이 아웃을 축소할 수 있습니다.
...
사전에 상황에 맞는 명령을 표시 한 경우 기본 명령만 (명령 모음 플라이 아웃은 축소)는 기본적으로 표시 되어야 합니다. 기본 명령 수집 하 고 보조 명령이 컬렉션에 상황에 맞는 메뉴에서 일반적으로 이동 하는 추가 명령에서 가장 중요 한 명령을 배치 합니다.

사전 명령을 표시 하려면 일반적으로 명령 모음 플라이 아웃을 표시 하려면 [클릭](/uwp/api/windows.ui.xaml.controls.primitives.buttonbase.click) 하거나 [Tapped](/uwp/api/windows.ui.xaml.uielement.tapped) 이벤트를 처리 합니다. 플라이 아웃의 [ShowMode](/uwp/api/windows.ui.xaml.controls.primitives.flyoutbase.showmode) **임시** 또는 **TransientWithDismissOnPointerMoveAway** 포커스 회수 없이 축소 된 모드로 플라이 아웃을 열로 설정 합니다.
...
상황에 맞는 명령을 사후 상황에 맞는 메뉴를 표시 하는 경우 (명령 모음 플라이 아웃은 확장)는 기본적으로 보조 명령이 표시 됩니다. 이 경우 기본 및 보조 명령 또는 보조 명령이 명령 모음 플라이 아웃 있을 수 있습니다.

명령을 상황에 맞는 메뉴를 표시 하려면 일반적으로 UI 요소의 [ContextFlyout](/uwp/api/windows.ui.xaml.uielement.contextflyout) 속성에는 플라이 아웃을 할당 합니다. 이렇게이 하면 요소에 의해 처리 되며 플라이 아웃을 열과 더 많은 아무 작업도 수행할 필요가 없습니다.
...
앱 바 단추 컨트롤은 아이콘 및 텍스트 레이블에 따라 구분됩니다. 이러한 컨트롤은 명령 모음에서 사용 하기 위해 최적화 하 고는 컨트롤이 명령 모음 또는 오버플로 메뉴에 표시 되는 여부에 따라 해당 모양이 변경 합니다.
...
여기서는 AppBarElementContainer 명령 모음 플라이 아웃에 추가 요소를 추가 하는 데 사용 됩니다. 분할 단추는 색을 선택할 수 있도록 하기 위해 기본 명령에 추가 됩니다. StackPanel 확대/축소 컨트롤에 대 한 더 복잡 한 레이아웃을 허용 하기 위해 보조 명령에 추가 됩니다.

> [!NOTE]
> 이 예제에 표시 되는 명령 중 하나에 구현 하지 않는 것만 명령 모음 플라이 아웃 UI를 보여줍니다. 명령을 구현에 대 한 자세한 내용은 [단추](buttons.md) 및 [명령 디자인 기본 사항](../basics/commanding-basics.md)를 참조 하세요.

:::row:::
    :::column:::
        분할 된 열기 단추를 사용 하 여 축소 된 명령 모음 플라이 아웃<br/>
...
[상황에 맞는 메뉴](menus.md)는 MenuFlyout 대신으로 CommandBarFlyout만 보조 명령으로 사용할 수 있습니다.
...
[TextCommandBarFlyout](/uwp/api/microsoft.ui.xaml.controls.textcommandbarflyout) 텍스트 편집에 대 한 명령을 포함 하는 특수 명령 모음 플라이 아웃입니다. 각 텍스트 컨트롤 (마우스 오른쪽 단추 클릭) 상황에 맞는 메뉴로 또는 텍스트를 선택한 경우는 TextCommandBarFlyout를 자동으로 표시 됩니다. 텍스트 명령 모음 플라이 아웃 관련 명령을 표시 하도록 선택한 텍스트에 적응 합니다.
...
| 방지 | IsSpellCheckEnabled **true** 이 고 틀렸습니다 텍스트 선택 됩니다. |
...
TextCommandBarFlyout 수 없는 사용자 지정할 수 및 각 텍스트 컨트롤에 의해 자동으로 관리 됩니다. 그러나 사용자 지정 명령을 사용 하 여 기본 TextCommandBarFlyout 바꿀 수 있습니다.

- 기본 텍스트 선택에 표시 되는 TextCommandBarFlyout로 사용자 지정 CommandBarFlyout (또는 다른 플라이 아웃 형식)을 만들 하 고 **SelectionFlyout** 속성을 지정할 수 있습니다. SelectionFlyout **null**로 설정 하면 명령이 없는 선택에 표시 됩니다.
- 상황에 맞는 메뉴로 표시 되는 TextCommandBarFlyout 기본으로 사용자 지정 CommandBarFlyout (또는 다른 플라이 아웃 유형) **ContextFlyout** 속성 텍스트 컨트롤에 할당 합니다. ContextFlyout을 **null**로 설정 하면 이전 버전의 텍스트 컨트롤에 표시 된 메뉴 플라이 아웃은 TextCommandBarFlyout 대신 표시 됩니다.
