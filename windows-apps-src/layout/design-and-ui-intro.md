...
이러한 다양한 디바이스에 잘 어울리는 앱을 디자인하는 것은 아주 어려운 일일 수 있습니다. 다행히도 UWP(유니버설 Windows 플랫폼)는 다양한 장치, 화면 및 입력 방법과 호환되는 사용자 환경을 만드는 데 도움이 되는 일련의 기본 기능 및 범용 구성 요소를 제공합니다. 이 문서에서는 UI 기능 및 UWP 앱의 이점을 설명하고 첫 UWP 앱 만들기에 대한 몇 가지 고급 디자인 참조 자료를 제공합니다. 
...
## <a name="uwp-features"></a>UWP 기능

UWP 앱을 만들 때 얻게 되는 몇 가지 기능부터 살펴보겠습니다.

### <a name="effective-pixels-and-scaling"></a>유효 픽셀 및 크기 조정

UWP 앱은 모든 디바이스에서 쉽게 읽고 상호 작용할 수 있도록 컨트롤, 글꼴 및 기타 UI 요소의 크기를 조정합니다.
...
-   시스템은 UI 크기를 확장할 때 4의 배수씩 확장합니다. 명확하게 나타내기 위해 디자인을 4x4 픽셀 그리드에 맞추고, UI 요소의 여백, 크기 및 위치를 4 유효 픽셀의 배수로 설정합니다. 텍스트에는 요구 사항이 없습니다. 텍스트의 크기와 위치는 자유롭게 설정할 수 있습니다. 
...
> [!TIP]
> 이미지 편집 프로그램에서 화면 모형을 만드는 경우 DPI를 72로 설정하고 이미지 크기를 목표로 하는 크기 클래스의 유효 해상도로 설정하세요. 크기 클래스 및 유효 해상도 목록은 [화면 크기 및 중단점 문서](screen-sizes-and-breakpoints-for-responsive-design.md)를 참조하세요.
...
## <a name="uwp-and-the-fluent-design-system"></a>UWP 및 Fluent 디자인 시스템

 Fluent 디자인 시스템을 사용하면 조명, 깊이, 움직임, 재질 및 배율이 통합된 깔끔한 현대식 UI를 만들 수 있습니다. 아름답고 매력적이고 직관적인 경험을 만드는 Fluent 디자인이 Windows 10 장치 및 앱 전반에 적용되고 있습니다. 
 
 Fluent 디자인을 어떻게 앱에 통합할 수 있을까요? 간편하게 통합할 수 있도록 지속적으로 새 컨트롤과 기능이 추가되고 있습니다. 다음은 UWP에 제공되는 현재 Fluent 디자인 목록입니다.  

* [아크릴](../style/acrylic.md)은 반투명 표면을 만드는 브러시 유형입니다.
* [시차](../style/parallax.md)는 목록 같은 스크롤 콘텐츠에 3차원 원근감, 깊이 및 움직임을 추가합니다.
* [표시](../style/reveal.md)는 빛을 사용하여 대화형 UI 요소를 비추는 가리키기 효과를 만듭니다. 
* [연결된 애니메이션](../style/connected-animation.md)은 컨텍스트를 유지하고 연속성을 제공하여 우아한 장면 전환을 연출합니다. 

여러분이 지금 읽고 계신 [설계 지침](https://developer.microsoft.com/windows/apps/design)도 업데이트되었으며, 이제는 Fluent 디자인 원칙을 기반으로 합니다.
...
<p>탐색 요소는 사용자가 표시하려는 콘텐츠를 선택하는 데 도움을 줍니다. 탐색 요소의 예로는 [탭 및 피벗](../controls-and-patterns/tabs-pivot.md), [하이퍼링크](../controls-and-patterns/hyperlinks.md) 및 [탐색 창](../controls-and-patterns/navigationview.md)이 있습니다.</p>
...
표시 또는 숨기기 기술에는 언제 더 많은 메타데이터를 표시할 것인지 선택하는 기능도 포함되어 있습니다. 휴대폰이나 패블릿과 같이 공간이 중요한 경우에는 최소한의 메타데이터를 표시하는 것이 가장 좋습니다. 노트북 또는 데스크톱 PC를 사용할 때는 많은 양의 메타데이터를 표시할 수 있습니다. 메타데이터 표시 또는 숨기기를 처리하는 방법의 몇 가지 예는 다음과 같습니다.
...
UWP 앱을 디자인하는 다양한 도구가 제공됩니다. XD, Illustrator, Photoshop, Framer, Sketch와 추가 디자인 도구 및 글꼴 다운로드는 [디자인 도구 키트 페이지](../design-downloads/index.md)를 참조하세요. 
