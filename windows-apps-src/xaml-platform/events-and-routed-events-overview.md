...
**참고**헤더를 개인 클래스 멤버로 선언 된 partial 클래스에서 c + +에 대 한 이벤트 처리기 정의 가져오기 하지 않습니다. C++ 프로젝트에 대한 빌드 작업에는 C++에 대한 XAML 형식 시스템 및 코드 숨김 모델을 지원하는 코드를 생성하는 작업이 포함됩니다.
