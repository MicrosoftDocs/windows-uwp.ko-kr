...
반환 된 데이터에서 **listCount** 필드는 서비스에서-으로 유지 총 목록에 있는 항목 수를 나타냅니다, 그리고 목록의 끝 인 및이 얼마나 많은 특정 항목이 retu 된 다른 숫자는 잠재적으로 위치를 확인 하기 위해 사용할 수 있습니다. 요청으로 rned 합니다.
...
| filterContentType| string| 선택 사항입니다. 쉼표로 구분 된 목록을 반환할 콘텐츠 형식 지정 (목록에 없는 종류를 반환 합니다). 목록에서 특정 콘텐츠 형식에만 다운로드할 하는 데 사용 합니다. 이 필터에 대 한 쉼표로 구분 된 콘텐츠 형식 목록이 사용 됩니다. (여러 콘텐츠 유형에 한 번 호출에서 쿼리할 수 있습니다.) 지원 되는 콘텐츠 유형은 엔터테인먼트 검색 서비스 (EDS)으로 정의 된 모든 미디어 형식이 포함 합니다. 기본값: null (모든 콘텐츠 형식).| 
