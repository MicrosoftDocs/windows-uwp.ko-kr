...
? 포함 = valuemetadata 쿼리 매개 변수는 순위표 값과 관련 된 모든 메타 데이터를 포함 하도록 응답 수 있습니다. 지정 된 클라이언트를 포함 하는 값 메타 데이터 모델 등 경합 트랙에 가장 좋은 시간을 얻는 데 자동차의 색 값과 관련 된 데이터.
 
값 메타 데이터 하 고 순위표 기반으로, 순위표 자체에 없는 사용자 상태에서 정의 됩니다.
 
순위표 Api 모두 읽기 전용 이며 따라서 GET 동사만 지원 합니다. 순위 되 고 정렬 된 "페이지" 데이터 플랫폼을 통해 작성 된 개별 사용자 통계에서 파생 되는 인덱싱된 플레이어 통계을 반영 합니다. 전체 순위표 인덱스 상당히 크며 수 및 호출자 읽으세요에서 하나 묻지 않습니다, 그리고 따라서이 URI는 호출자가 해당 순위표 으로부터 보려는 보기의 종류에 대 한 특정 되도록 허용 하는 몇 가지 쿼리 문자열 인수를 지원 합니다.
...
| 매개 변수| 유형| 설명| 
| --- | --- | --- | --- | --- | --- | 
| 포함 = valuemetadata| string| 응답에는 순위표 값과 관련 된 모든 값 메타 데이터를 나타냅니다.| 
...
| continuationToken| 문자열| 이전에는 continuationToken 반환 되 면 다음 호출자에 게 전달할 수 다시 "있는 그대로" 토큰 결과의 다음 페이지를 가져오는 쿼리 문자열.| 
...
콘텐츠 격리 및 액세스 제어 시나리오에 구현 된 권한 부여 논리가 있습니다.
   ...
   * 순위표와 사용자 통계 호출자에 게 요청을 사용 하 여 유효한 XSTS 토큰 제출 된 모든 플랫폼에서 클라이언트에서 읽을 수 있습니다. 쓰기는 데이터 플랫폼에서 지원 되는 클라이언트 분명히 제한 됩니다.
...
| X Xbl-계약 버전| 문자열입니다. 사용 하는 API의 어떤 버전을 나타냅니다. 이 값은 응답에서 값 메타 데이터를 포함 하려면 "3"으로 설정 되어야 합니다.| 
...
| 헤더| 설명| 
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | 
| If-None-Match| 문자열입니다. 엔터티 태그에 클라이언트 캐싱 지 원하는 경우 사용 합니다. 예제 값: <b>"686897696a7c876b7e"</b>|  | 
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | 
| continuationToken| 64 비트 부호 없는 정수| 필수. 원하는 경우 해당 URI에 대 한 결과의 다음 페이지를 가져오려면 <b>skipItems</b> 쿼리 매개 변수를 다시 피드에 값을 지정 합니다. <b>PagingInfo</b> 하지 않으면 다음 없는 경우 얻을 수 있는 데이터의 다음 페이지 없습니다.| 
| totalItems| 64 비트 부호 없는 정수| 필수. 순위표 항목의 총 수입니다. 예제 값: 1234567890| 
 
leaderboardInfo | leaderboardInfo| 섹션| 필수. 항상 반환 됩니다. 요청 된 순위표에 대 한 메타 데이터를 포함 합니다.| 
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | 
| displayName| string| 사용 되지 않습니다.| 
| totalCount| 문자열 (64 비트 부호 없는 정수)| 필수. 순위표 항목의 총 수입니다. 예제 값: 1234567890| 
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | 
| 게이머 태그| string| 필수. 순위표 항목에서 사용자의 게이머 합니다.| 
...
다음 요청 URI는 글로벌 순위표에 순위를 건너뛰는 보여 줍니다.
