...
네트워크를 통한 멀티 플레이어 시나리오 (여기서 개인 정보 통신 검사 수행 해야 Xbox 사용자 ID (XUID)이 있는 사용자와 하지 않는 네트워크 외부 사용자 간의)에서는 사용자가 입력할 [PermissionCheckBatchRequest (JSON)을](../../json/json-permissioncheckbatchrequest.md) 참조 하십시오.
