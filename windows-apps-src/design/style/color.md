...
앱에서 자체 색을 정의 하는 것 외에도 **ColorSchemeResources** 태그를 사용 하 여 앱 전체에서 원하는 영역 우리의 systematized 색을도 범위 수 있습니다. 이 API 뿐만 아니라 색을 지정 하 고 테마 많은 컨트롤 그룹을 한 번에 몇 가지 속성, 뿐만 아니라 제공 하면 다른 많은 시스템 혜택을 설정 하 여 사용자 지정 색을 수동으로 정의 게 일반적으로 허용 합니다.
...
- **ColorSchemeResources** 에서 설정한 색도 해당 시스템 색을 사용 하는 모든 유사한 컨트롤이 나타날 때까지 아래로 계단식 배열 됩니다.
...
ColorSchemeResources는 리소스 되는 시스템 범위 위치를 알려 주는 API입니다. ColorSchemeResources는 [X:key](https://docs.microsoft.com/windows/uwp/xaml-platform/x-key-attribute)세 가지 중 하나일 수 있는 수행 해야 합니다.
- Default
...
X:key 설정 하면 시스템 또는 앱 테마 색 적절 하 게 변경 보려는 경우 두 테마에서 다른 사용자 지정 모양을 합니다.

### <a name="how-to-apply-scoped-colors"></a>범위가 지정 된 색을 적용 하는 방법

범위는 **ColorSchemeResources** API xaml에서을 통해 리소스 모든 시스템 색 또는 [테마 리소스](https://docs.microsoft.com/windows/uwp/design/controls-and-patterns/xaml-theme-resources) 라이브러리가 이며 페이지 또는 컨테이너의 범위 내에서를 재정의 하는 브러시를 수행할 수 있습니다.
...
시스템 색 변경에 *하는 단일 컨트롤만* 범위를 지정 하려는 경우 해당 컨트롤의 리소스 내에서 **ColorSchemeResources** 를 정의 하 여이 수행할 수 있습니다.
...
```
기본적으로, 앞으로 정확히 동일한 기능 했지만 이제 눈금 목록에 추가 하는 다른 모든 컨트롤 색 변경 내용이 반영 되지 않습니다. 이러한 시스템 색 범위 **Button_A** 만 때문입니다.

### <a name="nesting-scoped-resources"></a>범위가 지정 된 중첩 리소스

시스템 색 중첩 가능한 이기도 및 **ColorSchemeResources** 앱 레이아웃의 태그 내는 중첩 된 요소의 리소스에 배치 하 여 수행 됩니다.
...
임의 개수의 색 정의 리소스가 중첩 된 요소에 대해 작동 합니다.

### <a name="scoping-with-a-resourcedictionary"></a>ResourceDictionary를 사용 하 여 범위 지정

컨테이너 또는 페이지의 리소스에 제한 되지 하 고 병합할 수 있는 모든 범위에서 일반적으로 사전 병합는 방식으로 ResourceDictionary도 이러한 시스템 색을 정의할 수 있습니다.

#### <a name="mycustomthemexaml"></a>MyCustomTheme.xaml

먼저 ResourceDictionary 만들게 됩니다. 다음은 ThemeDictionaries 내 **ColorSchemeResources** 배치 하 고 원하는 시스템 색을 재정의:
