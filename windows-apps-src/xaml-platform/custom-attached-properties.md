...
연결 된 속성을 선언 하 여 종속성 속성으로 정의 된 **공용** **정적** **readonly** 형식의 속성 [  **DependencyProperty**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.DependencyProperty)합니다. 이 속성은 [**RegisterAttached**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.dependencyproperty.registerattached) 메서드의 반환 값을 사용하여 정의합니다. 속성 이름으로 지정 하면 연결 된 속성 이름과 일치 해야 합니다 **RegisterAttached** *이름* "Property" 끝에 추가 하는 문자열을 사용 하 여 매개 변수입니다. 표시하는 속성과 관련하여 종속성 속성 식별자를 명명하기 위해 설정된 규칙입니다.
...
> [!NOTE]
> 속성 요소 구문을 통해 원하는 사용 되는 연결 된 속성을 정의할 수 이기도 합니다. 이 경우 값에 형식 변환은 필요하지 않지만 의도한 값을 XAML에서 생성할 수 있는지 확인해야 합니다. [**VisualStateManager.VisualStateGroups** ](https://docs.microsoft.com/dotnet/api/system.windows.visualstatemanager?view=netframework-4.8) 은 예제만 속성 요소 사용을 지 원하는 기존 연결 된 속성입니다.
...
> [!NOTE]
> 사용 중인 경우 C++/WinRT, 후 다음 섹션을 건너뜁니다 ([명령적으로 사용 하 여 사용자 지정 연결 된 속성 설정 C++/WinRT](#setting-your-custom-attached-property-imperatively-with-cwinrt)).
...
## <a name="setting-your-custom-attached-property-imperatively-with-cwinrt"></a>명령적으로 사용 하 여 사용자 지정 연결 된 속성을 설정 C++/WinRT

사용 중인 경우 C++/WinRT을 XAML 태그에서가 아니라 하지만 명령형 코드에서 사용자 지정 연결 된 속성을를 액세스할 수 있습니다. 아래 코드 하는 방법입니다.
...
> [!NOTE]
> 패널의 작동 방식에 대 한 자세한 내용은 참조 하세요. [XAML 사용자 지정 panel 개요](https://docs.microsoft.com/windows/uwp/layout/custom-panels-overview)합니다.
