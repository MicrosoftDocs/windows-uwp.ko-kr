...
반환 되는 데이터의 **listCount** 필드 서비스에서-따라서 유지 총 목록에 얼마나 많은 항목이, 위치를 확인 목록의 끝 이면이 잠재적으로 얼마나 많은 특정 항목 retu 된 다른 숫자를 사용할 수 있습니다. 요청으로 rned 합니다.
...
| 매개 변수| 유형| 설명| 
| --- | --- | --- | --- | --- | --- | 
| skipItems| 32 비트의 부호 있는 정수| 선택 사항입니다. 결과 반환 하기 전에 열거형에서 건너뛸 항목 수입니다. 기본값: 0입니다.| 
| maxItems| 32 비트의 부호 있는 정수| 선택 사항입니다. 최대 반환할 항목 수입니다. 기본 25 개 항목 이면 무제한 요청에 지정 됩니다. 서비스에이 값은 최대 두지 않습니다. 값을 목록의 항목 수가 보다 큰 경우 모든 항목 오류가 없는 반환 됩니다.| 
| filterItemId| string| 선택 사항입니다. 목록에서 항목을 지정 합니다. 목록에서 항목의 모든 인스턴스를 반환합니다. 신속 하 게 확인 하 고 목록의 항목은 클라이언트 수 있습니다. 큰 목록 전체 목록을 반복 하지 않고 항목의 인스턴스를 모두 확인 하는 데 편리 합니다. 기본값: null입니다.| 
| filterContentType| string| 선택 사항입니다. 쉼표로 구분 된 목록을 반환할 콘텐츠 형식 지정 (목록에 없는 종류를 반환 합니다). 목록에서 특정 콘텐츠 형식에만 다운로드할 하는 데 사용 합니다. 이 필터에 대 한 콘텐츠 형식의 쉼표로 구분 된 목록을 사용 됩니다. (한 번만 호출에 여러 콘텐츠 형식의 쿼리할 수 있습니다.) 지원 되는 콘텐츠 유형은 엔터테인먼트 검색 서비스 (EDS) 하 여 정의 된 모든 미디어 형식이 포함 합니다. 기본값: null (모든 콘텐츠 형식).| 
| filterDeviceType| string| 선택 사항입니다. 반환할 장치 종류의 쉼표로 구분 된 목록 지정 (목록에 없는 종류를 반환 합니다). 특정 장치 유형 집합에서 삽입 된 항목을 반환할만 반환 집합을 필터링 합니다. 장치 종류의 쉼표로 구분 된 목록 (여러 디바이스 유형에 한 번만 호출에서 쿼리할 수 있습니다)이이 필터에 사용 됩니다. 가능한 값: XboxOne, MCapensis, WindowsPhone, WindowsPhone7, 웹, PC, MoLive 합니다. 기본값: null (모든 콘텐츠 형식).| 
