...
[**XamlLight**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.media.xamllight) 개체는 [**CompositionLights**](https://docs.microsoft.com/uwp/api/Windows.UI.Composition.CompositionLight)에 적용하여 동적으로 XAML UIElements를 조명하는 데 사용됩니다. XamlLight UIElements 또는 전기 UIElements의 트리를 적용 하는 XAML 브러시를 대상으로 하는 것에 대 한 메서드를 제공 하 고 현재에 있는지 여부에 따라 리소스 사용 CompositionLight 수명 관리를 지원 합니다.
...
이 예제에서는 다양 한 색상된 스포트라이트 대상 UIElements 및 브러시에 적용 되는 사용자 지정 XamlLight에 대 한 정의 표시 합니다.
...
다음 XAML ui 요소 또는 브러시 하 게 하 빛이 빛을 적용할 수 있습니다. 이 예제에서는 서로 다른 잠재적인 사용법을 보여줍니다.

> [!Important]
> 에 대 한 [C + + / WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt), 두 개 제거 `local:OrangeSpotLight.IsTarget="True"` 아래 태그에서 합니다. 연결 된 속성은 이미 코드 숨김에서 설정 됩니다.
...
![Xaml 간단 하 여 조명을 요소의 예](images/orange-spot-light.png)
