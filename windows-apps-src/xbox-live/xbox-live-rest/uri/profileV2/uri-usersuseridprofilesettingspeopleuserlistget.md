...
**userList** 및 **userIds** 상호 배타적인 매개 변수가 있습니다. 하나 또는 둘 다를 지정 하면 **BadRequest** 을 다시 가져오게 됩니다. **userList** 은 여러 명명 된 목록을 요청에 유용한 시나리오에서 future 검사에 대 한 배열입니다. **userIds** XUIDs에 대해 10 진수 문자열의 구성 된-JSON 64 비트 부호 없는 정수를 직렬화 하는 작업에 잘못 되었습니다. 마지막으로, 일반 읽을 이름 대신 64 비트 부호 없는 정수로 또는 **XONLINE_PROFILE_ASDF**같은 모호한 상수를 사용 하 여 설정 Xbox One의 설정 라는 됩니다.
...
| 매개 변수| 유형| 설명| 
| --- | --- | --- | --- | --- | --- | 
| 설정| string| 쉼표로 구분 된 목록의 이름을 설정 합니다.| 
...
| 헤더| 유형| 설명| 
| --- | --- | --- | --- | --- | --- | --- | --- | --- | 
| x xbl-계약 버전| 32 비트 부호 있는 정수| 값 = 2| 
