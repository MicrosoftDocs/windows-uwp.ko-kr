...
5.  사용하려는 **게시자 표시 이름**을 입력합니다. 다른 사용자가 이미 선택한 이름을 입력하거나 다른 사용자에게 해당 이름을 사용할 권한이 있는 것처럼 보이는 경우 해당 이름의 선택이 허용되지 않습니다. 회사 계정의 경우 게시자 표시 이름은 조직의 등록된 회사 이름 또는 거래 명이어야 합니다. 고객이 이 이름을 검색할 때 보고 이 이름으로 앱을 알게 되므로 신중하게 이름을 선택합니다.
