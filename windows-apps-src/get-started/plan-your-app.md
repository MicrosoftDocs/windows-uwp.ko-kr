...
좋은 IA는 사용자 시나리오를 가능하게 할 뿐만 아니라 시작할 주요 화면을 구상하는 데 도움이 됩니다. 예를 들어 [들리는](https://go.microsoft.com/fwlink/p/?LinkID=268089) 앱 사용자의 라이브러리, 저장소, 뉴스 및 통계에 대 한 액세스를 제공 하는 허브로 바로 시작 합니다. 경험에 포커스가 하므로 사용자가 가져오고 오디오북을 빠르게 즐길 수 있습니다. 앱의 심층 수준에서는 보다 구체적인 작업에 중점을 둡니다.
...
-   **파일 선택기 및 파일 선택기 확장** 로드 하 고 로컬 파일 시스템, 연결 된 저장 장치, 홈 그룹 또는 기타 앱에서 자신의 파일을 저장할 수 있도록 합니다. 다른 앱에서 사용자 앱의 콘텐츠를 로드할 수 있도록 파일 선택기 확장을 제공할 수도 있습니다.

자세한 내용은 [앱 계약 및 확장](https://msdn.microsoft.com/library/windows/apps/hh464906)을 참조하세요.
<!-- Win 8 page. Should have replacement. -->

**여러 가지 보기, 폼 팩터 및 하드웨어 구성**Windows 충전 및 중점에서 앱에 사용자를 배치 합니다. 사용자가 해당 앱을 사용하기로 결정한 어떤 디바이스에서든, 어떤 입력 모드를 사용하든, 어떤 방향으로든, 어떤 하드웨어 구성에서든, 어떤 환경에서든 앱 UI가 두드러지게 하고 싶을 것입니다.
...
-   **알림**사용자가 알림 메시지를 통해 시간이 중요 하거나 개인적으로 관련 콘텐츠 하 고 앱이 닫힌 경우에 앱으로 다시 초대 수 있도록 합니다. [타일, 배지 및 알림 메시지](../design/shell/tiles-and-notifications/index.md)에 대해 자세히 알아보세요.
...
-   **로밍**사용자가 위치 작업 한 선택할 수 있도록 하 고 중요 가장, 사용 중인 장치와 관계 없이 UX를 유지 하는 데이터를 로밍 하 여 장치에서 지속적인 환경을 만듭니다. 로밍을 통해 설정 및 상태를 유지 관리하여 부엌에 있는 가족용 PC, 회사 PC, 개인용 태블릿, 기타 폼 팩터 등 어디에서든 앱을 쉽게 사용할 수 있게 합니다. [응용 프로그램 데이터 관리](../design/app-settings/store-and-retrieve-app-data.md)에 대해 살펴보고, [로밍 중인 응용 프로그램 데이터에 대한 지침](https://msdn.microsoft.com/library/windows/apps/hh465094)을 참조하세요.
...
-   **카메라 및 외장형 저장 장치**사용자가 자신의에서 연결 또는 기본 제공 카메라 채팅 및 회의 프로필 pics, 세계, 문서화 라인 vlogs 기록에 대 한 연결 또는 어떠한 활동 앱은 특성입니다. [이동식 저장소의 콘텐츠 액세스](https://msdn.microsoft.com/library/windows/apps/hh465189)에 대해 자세히 알아보세요.
...
-   **지리적 위치**사용자가 해결, 지도, 위치 또는 근처에 있는 사람, 활동, 목적지에 대 한 알림을 받기 위해 표준 웹 데이터 나 지리적 위치 센서의 지리적 위치 정보를 사용 합니다. [지리적 위치](https://msdn.microsoft.com/library/windows/apps/hh465139)에 대해 자세히 알아보세요.
...
**명령 구성 방법?** 흐름 단계 개요를 사용 하 여 디자인 해야 하는 잠재적 명령을 식별할 수 있습니다. 그런 후 앱에서 해당 명령을 사용할 위치를 고려합니다.

-   **항상 콘텐츠를 사용 하려고 시도 합니다.** 가능 하도록된 사용자가 앱의 캔버스에서 콘텐츠를 직접 조작할 보다는 내용에 대해 실행 되는 명령 추가 합니다. 예를 들어 사용자가 여행 앱에서 활동을 선택하고 위쪽 또는 아래쪽 명령 단추를 사용하기보다 캔버스의 목록에 활동을 끌어다 놓아 여행 일정표를 다시 짤 수 있도록 합니다.
...
**앱의 각 보기에 배치 하는 방법을 결정 합니다.** Windows에서는 가로 및 세로 방향을 지원 하며 최소 너비를 전체 화면에서 모든 너비로 앱 크기 조정을 지원 합니다. 모든 크기의 모든 화면에서 두 방향으로 모두 제대로 표시 및 작동하는 앱을 만들려고 합니다. 즉, 각 크기와 보기에 대한 UI 요소의 레이아웃을 계획해야 합니다. 이렇게 하면 사용자의 요구 및 기본 설정에 맞게 앱 UI가 유연하게 바뀝니다.
...
**시작 화면**시작 화면, 가능한 한 빠르게 로드 되어야 하 고 화면에 남아만으로 응용 프로그램 상태를 초기화 해야 합니다. 시작 화면에는 앱의 개성을 잘 나타내는 내용을 표시해야 합니다.

**처음 시작**하기 전에 사용자가 서비스에 대 한 등록 하 고를 계정에 로그인 하거나 자체 콘텐츠를 추가, 어떤 보게 될까요? 사용자에게 정보를 요청하기 전에 앱의 가치를 입증해 보세요. 사람들에게 사용 확정을 요청하기 전에 살펴보고 앱의 기능을 이해할 수 있도록 샘플 콘텐츠를 보여 주는 것이 좋습니다.
