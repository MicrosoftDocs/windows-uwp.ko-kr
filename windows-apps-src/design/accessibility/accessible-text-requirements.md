...
여기에서 설명하는 텍스트 대비에 대한 권장 사항은 웹 접근성 표준인 [G18: Ensuring that a contrast ratio of at least 4.5:1 exists between text (and images of text) and background behind the text](https://go.microsoft.com/fwlink/p/?linkid=221823)(영문)를 기반으로 합니다. 이 지침은 *WCAG 2.0에 대한 W3C 기술* 사양에 있습니다.
...
* 글로벌 크기와 해상도 설정 **>System >Display >Scale 설정 및 레이아웃**입니다. 어떤 크기 조정 옵션을 더 정확 하 게으로 디스플레이 디바이스의 기능에 따라 달라질 수 있습니다.
* **설정->Ease 액세스 >Display의**텍스트 크기 설정입니다. 모든 응용 프로그램 및 화면 (모든 UWP 텍스트 컨트롤을 모든 사용자 지정 또는 템플릿 없이도 환경을 배율 텍스트 지원) 컨트롤을 지원 하기 위한만 텍스트의 크기를 지정 하려면 **더 큰 텍스트** 설정을 조정 합니다. 
> [!NOTE]
