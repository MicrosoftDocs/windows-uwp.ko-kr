---
author: stevewhims
description: C + + / WinRT 함수 및 구현 및/또는 컬렉션을 전달 하려는 경우 시간과 노력을 많이 저장 하는 기본 클래스를 제공 합니다.
title: 모음을 C + + / WinRT
ms.author: stwhi
ms.date: 08/24/2018
ms.topic: article
...
내부적으로 Windows 런타임 컬렉션에 복잡 한 이동 부분을 많이 있습니다. 하지만 컬렉션 개체를 전달 하는 Windows 런타임 함수를 하거나 고유의 컬렉션의 속성 및 컬렉션 형식을 구현 하는 것이 하려는 때 함수 및 기본 클래스에서 C + + / 하기를 지원 하기 위해 WinRT 합니다. 이러한 기능 손으로 활용 하는 복잡성을 수행 하 고 시간과 노력에 많은 오버 헤드를 저장 합니다.

> [!IMPORTANT]
> 이 항목에 설명 된 기능은 이상 [Windows 10 SDK 미리 보기 빌드 17661](https://www.microsoft.com/software-download/windowsinsiderpreviewSDK)설치한 경우에 사용할 수 있습니다.

[**IVector**](/uwp/api/windows.foundation.collections.ivector_t_) 는 모든 임의 액세스 컬렉션의 요소에 의해 구현 Windows 런타임 인터페이스입니다. 사용자가 직접 **IVector** 를 구현 하는 경우 [**IIterable**](/uwp/api/windows.foundation.collections.iiterable_t_), [**IVectorView**](/uwp/api/windows.foundation.collections.ivectorview_t_)및 [**IIterator**](/uwp/api/windows.foundation.collections.iiterator_t_)를 구현 해야 합니다. *필요한* 사용자 지정 컬렉션을 입력 하는 경우에 많은 작업입니다. 하지만 **std:: vector** (또는 **std:: map**또는 **std::unordered_map**)의 데이터를 포함 하는 경우 Windows 런타임 API에 전달 하는 작업을 수행 하기 원하는 모든 다음 하려는 가능한 경우 해당 수준의 작업을 수행 하지 합니다. 때문에 *는* 가능한을 방지 하 고 C + + / WinRT를 사용 하면 효율적이 고 적은 노력으로 컬렉션을 만들 수 있습니다.
...
위의 코드 예제에서 볼 수 있는 **추가** 에 대 한 호출의 오버 헤드를 방지할 수 있습니다. 원본 데이터를 이미 있을 또는 Windows 런타임 컬렉션 개체를 만들어 전 채울 수도 있습니다. 다음은 작업을 수행 하는 방법입니다.
...
데이터 **winrt::single_threaded_vector**포함 하는 임시 개체를 전달할 수와 같이 `coll1`위의 합니다. (하면 없습니다 액세스할 것 다시 가정함) **std:: vector** 를 이동할 수 또는 함수에 있습니다. 두 경우 모두 함수에는 *rvalue* 전달 합니다. 이렇게 하면 효율성을 높이 및 데이터를 복사를 방지 하기 위해 컴파일러. *Rvalue*하는 방법에 대 한 상세 [값 범주 및 자신에 대 한 참조를](cpp-value-categories.md)참조 하십시오.

컬렉션에 XAML 항목 컨트롤에 연결 하려는 경우 다음을 할 수 있습니다. 하지만 [**ItemsControl.ItemsSource**](/uwp/api/windows.ui.xaml.controls.itemscontrol.itemssource) 속성을 올바르게 설정 하려면 **IVector** **IInspectable** 의 (또는 [**IBindableObservableVector**](/uwp/api/windows.ui.xaml.interop.ibindableobservablevector)등의 상호 운용성 형식의) 형식의 값으로 설정 해야 하면 유의 해야 합니다. 바인딩 (영문), 적합 한 형식의 컬렉션을 생성 하 고 그에 요소를 추가 하는 코드 예제는 다음과 같습니다.
...
*수* 위의 컬렉션 XAML 항목 제어;에 바인딩되어야 합니다. 하지만 컬렉션에는 관찰 가능 하지 않습니다.
...
모든 동시성을 제공 하지 않는 나타냅니다는 "단일 스레드" 이러한 함수의 이름에&mdash;즉, 달라도 스레드 안전 하 게 보호 합니다. 이러한 함수에서 반환 된 개체는 모두 민첩 한 스레드 언급 하는 것은 아파트, 상관이 없습니다 (참조 [Agile 개체에서 C + + / WinRT](agile-objects.md)). 단일 스레드에서는 개체를 바로 됩니다. 되며, 그 데이터는 한 가지 방법은 또는 다른 응용 프로그램 이진 인터페이스 (ABI)를 통해 전달 하려는 경우에 완전히 적합 합니다.

## <a name="base-classes-for-collections"></a>컬렉션에 대 한 기본 클래스

전체의 유연성을 위해 사용자 지정 컬렉션을 구현 하려는 경우에 복잡 한 과정을 수행 하를 방지 하기 위해 싶을 것입니다. 예,이 사용자 지정 벡터 보기는 다음과 같이 나타납니다 *없이 지원이 C + + / WinRT의 기본 클래스*합니다.
...
**Get_container** 에서 반환 되는 컨테이너 **시작** 및 **끝** 인터페이스는 **winrt::vector_view_base** 를 제공 해야 것으로 예상 됩니다. 위의 예제에서와 같이 있는 **std:: vector** 를 제공 합니다. 하지만 사용자 고유의 사용자 지정 컨테이너를 포함 하 여 동일한 계약을 fulfils 하는 컨테이너를 반환할 수 있습니다.
