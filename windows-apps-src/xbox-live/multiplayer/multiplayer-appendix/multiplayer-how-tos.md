...
| 참고                                                                                                                                                                                                                                                                                                                                    |
|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 세션에 대 한 변경 내용에 대 한 구독에 연결된 플레이어 세션에서 활성 필요 합니다. ConnectionRequiredForActiveMembers 필드 설정 해야 세션에 대 한 /constants/system/capabilities 개체의 true입니다. 이 필드는 세션 템플릿에서 일반적으로 설정 됩니다. [MPSD 세션 템플릿](multiplayer-session-directory.md)를 참조 하세요. |
...
여러 사용자가 동일한 세션에 대 한 알림을 구독 인 경우 해당 세션에 모든 변경 각 사용자에 대 한 어깨 탭을 트리거합니다. 다음 중 하나를 제외한 모든 중복 됩니다. 제목을; 알림을 이미 되는 모든 변경 내용을 무시 해야 여전히 것이 좋습니다 제목을 알림에 세션에서 모든 사용자를 구독 하는 동안 분기 및 ChangeNumber 속성을 사용 하 여 수행할 수 있습니다.
...
-   어깨 탭 해당 **RealTimeActivityMultiplayerSessionChangeEventArgs.Branch 속성**에 대 한 높은 **RealTimeActivityMultiplayerSessionChangeEventArgs.ChangeNumber 속성** 수 없는 경우이 건너뜁니다. 해당 변경 이미 처리 되었습니다.

| 참고                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 세션을 받지 **RealTimeActivityMultiplayerSessionChangeEventArgs.Branch 속성**에 의해 추적 되 **RealTimeActivityMultiplayerSessionChangeEventArgs.ChangeNumber 속성** 값이 필요 합니다. **RealTimeActivityMultiplayerSessionChangeEventArgs.Branch 속성** 은 세션의 수명 동안 변경 (및 되돌리려면 **RealTimeActivityMultiplayerSessionChangeEventArgs.ChangeNumber 속성** ) 값 . |
...
2.  MPSD는 새 세션을 만드는 데 필요한 모든 세션 데이터 새 **MultiplayerSession 클래스** 개체를 만듭니다.
3.  필요한 변경 사항에 MPSD 세션을 작성 하기 전에 확인 합니다. 예를 들어 구성원 **MultiplayerSession.Join 메서드**를 호출 하 여 세션에 참가 하면 클라이언트 MPSD 세션 업데이트에 대 한 호출에 참가할 수를 알려주는 숨겨진된 로컬 요청 데이터를 추가 합니다.
...
| 참고                                                                                                                                       |
|---------------------------------------------------------------------------------------------------------------------------------------------------------|
| 멤버 세션에 참가 있고 해당 보안 장치 주소 포함 될 때까지 (잠재적인 호스트) 멤버에 대 한 장치 토큰을 사용할 수 있습니다. |
...
4.  호스트 디바이스 토큰을 설정 하려고 할 때 HTTP/412 상태 코드를 받으면 세션 데이터를 쿼리하고 로컬 콘솔에 대 한 호스트 장치 토큰 있는지 확인 합니다. 로컬 콘솔에 대 한 없으면 중재자도 지정 된 다른 콘솔.
...
Xbox One 프로토콜 활성화 하는 동안 **CoreApplicationView.Activated 이벤트** 를 발생합니다. 멀티 플레이 API의 컨텍스트에서 사용자가 초대를 수락 하거나 다른 사용자를 가입 하는 경우이 이벤트는 발생 합니다. 이러한 작업 조인 사용자 대상 사용자를 사용 하 여 게임 플레이에 연결 하 여 제목에 반응 해야 하는 활성화를 트리거합니다.
...
2.  제목 정품 인증에서 세션을 시작 하 고 세션 변경 알림을 등록 합니다. 참조 **하는 방법: MPSD 세션 변경 알림을 등록**합니다.
...
4.  사용자 중재자 없는 경우 7 단계를 진행 합니다.
5.  중재자 인 경우 **MultiplayerSession.SetHostDeviceToken 메서드**를 호출 합니다.
...
3.  대기실 세션을 사용 하 여 **MultiplayerService.SendInvitesAsync 메서드**를 호출 하 여 초대를 보냅니다. 이 메서드는 게임에서 UI 명단 **SystemUI.ShowPeoplePickerAsync 메서드** 또는 **PartyChat.GetPartyChatViewAsync 메서드**를 사용 하 여 빌드를 할 수 있습니다.
...
Windows 10 장치에서 게임 플레이 세션 있어야 합니다 `userAuthorizationStyle` 큰 세션 하지 않은 접근 권한 값을 **true** 로 설정 합니다. 이 `joinRestriction` 속성을 "none"는 세션 수 없습니다. 있는 공개적으로 직접 즉 수 없습니다.

일반적인 시나리오는 플레이어를 수집 하는 게임 플레이 세션이 나 매치 메이 킹 세션으로 해당 플레이어가 이동 로비 세션을 만드는 것입니다. 게임 플레이 세션을 공개적으로 있는 없는 경우 게임 클라이언트 충족 하지 않는 한 게임 플레이 세션에 참가할 수 있지만 합니다 `joinRestriction` 이 시나리오에 대 한 대부분의 경우 너무 제한적인 설정 합니다.
...
3. 제목을 게임 세션에 로비 세션에서 멤버를 이동 하려는 경우 각 클라이언트 사용 전송 핸들 로비 세션에서 사용 하 여 게임 세션에 참가 하 고 `multiplayer_service::write_session_by_handle(multiplayerSession, multiplayerSessionWriteMode, handleId)` API입니다.
...
8.  호출 하 여 **MultiplayerService.WriteSessionByHandleAsync 메서드**3 단계에 설명 된 대로 획득 핸들을 사용 하 여 합니다. 이제 사용자 세션의 구성원을 게임에 연결 하려면 세션에서 데이터를 사용할 수 있습니다.

## <a name="set-the-users-current-activity"></a>사용자의 현재 동작을 설정 합니다.

사용자의 현재 활동 제목에 대 한 Xbox 대시보드 사용자 환경에 표시 됩니다. 세션을 통해 또는 제목 활성화를 통해 사용자에 대 한 작업을 설정할 수 있습니다. 후자의 경우, 사용자는 매치 메이 킹 통해 또는 게임을 시작 하 여 세션을 입력 합니다.
...
2.  모든 변경 될 때 이러한 방법 중 하나를 사용 하 여 MPSD를 로컬 변경 내용을 씁니다.
    ...
    쓰기 메서드는 서버에 가입을 작성 하 고 다른 세션 멤버와가 콘솔의 보안 장치 주소 (SDAs)를 검색 하는 최신 세션을 가져옵니다. 이러한 콘솔 간에 네트워크 연결을 설정 하는 방법에 대 한 자세한 내용은 **Xbox One에서 Winsock 소개**를 참조 하세요.

3.  이전 로컬 세션 개체를 무시 하 고 이후 작업은 최신 알려진된 세션 상태에 따라 새로 검색 된 세션 개체를 사용 합니다.
...
매치 메이 킹 정찰 일치 티켓을 만들려면 다음을 해야 합니다.

1.  호출 하 여 **MatchmakingService.CreateMatchTicketAsync 메서드**티켓 세션에 대 한 참조를 전달 합니다. 메서드는 티켓 세션 MPSD에서 읽고 매치 메이 킹 사용자 세션에서 시작 합니다. 내부적으로 메서드가 **POST (/serviceconfigs/ {서비스 안내} /hoppers/ {hoppername})를**호출 합니다.
2.  매치 메이 킹 서비스를 새 세션 세션이 나 다른 기존 세션의 멤버를 일치 하는 경우에 Never를 *preserveSession* 매개 변수를 설정 합니다. 제목을 게임 플레이 계속 하려면 티켓 세션으로 기존 게임 세션을 다시 사용할 수 있도록 항상 *preserveSession* 매개 변수를 설정 합니다. 제출 된 세션은 유지 하 고 일치 하는 모든 플레이어 해당 세션에 추가 되는 다른 서비스를 확인 한 다음 수 있습니다.
