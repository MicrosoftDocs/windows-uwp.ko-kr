...
[프로세스 모니터](https://docs.microsoft.com/en-us/sysinternals/downloads/procmon) 는 앱의 파일 및 레지스트리 작업의 결과 관찰 하기 위한 강력한 유틸리티입니다.  응용 프로그램 호환성 문제를 이해 하는 데 도움이 수 있습니다.  프로세스 모니터를 연 다음 필터를 추가 합니다 (필터 >... 필터) 응용 프로그램 실행 파일에서 이벤트에만 포함 하도록 합니다.
...
파일 시스템 액세스 오류가 같으면 System32/SysWOW64 또는 패키지의 파일 경로 아래에 있는 실패 한 이벤트를 검색 합니다. 도 필터 여기에 도움이 수도 있습니다. 이 목록의 맨 아래에서 시작 하 고 위쪽으로 이동 합니다. 이 목록의 맨 아래에 표시 되는 오류 발생 한 가장 최근에 합니다. "액세스 거부" 같은 문자열을 포함 하는 오류와 "경로/이름 찾을 수 없음", 대부분의 주의 기울이는 하 고 의심 스러운 보이지 않을 것을 무시 합니다. [PSFSample](https://github.com/Microsoft/MSIX-PackageSupportFramework/blob/master/samples/PSFSample/) 에 두 가지 문제가 있습니다. 다음 이미지에 표시 되는 목록에 이러한 문제를 확인할 수 있습니다.

![ProcMon Config.txt](images/desktop-to-uwp/procmon_config_txt.png)

이 이미지에 표시 되는 첫 번째 문제를 "C:\Windows\SysWOW64" 경로에 있는 "Config.txt" 파일에서 읽는 응용 프로그램이 실패 했습니다. 응용 프로그램은 해당 경로 직접 참조 하려고 가능성이 아닙니다. 상대 경로 사용 하 여 해당 파일에서 읽는 하려고 하는 대부분의 경우, 기본적으로 "System32/SysWOW64"은 응용 프로그램의 작업 디렉터리 및 합니다. 이 응용 프로그램 패키지의 다른 곳으로 설정 해야 현재 작업 디렉터리의 예상 제안 합니다. Appx 내에서 찾고, 실행 파일과 동일한 디렉터리에 파일이 있는지 볼 수 있습니다.
...
예를 들어, 응용 프로그램 실행 파일 응용 프로그램와 동일한 디렉터리에 있는 로그 파일에 쓰는, [파일 리디렉션 수정을](https://github.com/Microsoft/MSIX-PackageSupportFramework/tree/develop/FileRedirectionShim) 사용 하 여 로컬 앱 데이터 저장소 등의 다른 위치에 해당 로그 파일을 만들 수 있습니다.

### <a name="runtime-fixes-from-the-community"></a>커뮤니티에서 런타임 수정

커뮤니티 기여 하려면 [GitHub](https://github.com/Microsoft/MSIX-PackageSupportFramework/tree/develop) 페이지를 검토 해야 합니다. 다른 개발자가 유사한 문제 해결 및 런타임 수정 공유 가능 합니다.
...
.Msix (또는.appx) 파일을 이미 있는 경우 레이아웃 폴더 역할을 패키지에 대 한 준비 영역에 해당 콘텐츠 압축을 푸는 수 있습니다.  이를 수행할 수는 **x64 VS 2017 용 네이티브 도구 명령 프롬프트**, 또는 실행 검색 경로에 SDK bin 경로 사용 하 여 수동으로 합니다.
...
파일 이름을 만들기 ``config.json``, 해당 파일을 패키지의 루트 폴더에 저장 합니다. 방금 교체 하는 실행 파일을 가리키도록 config.json 파일의 선언 된 응용 프로그램 ID를 수정 합니다. 프로세스 모니터를 사용 하 여에서 얻은 정보를 사용 하 여 있습니다도 작업 디렉터리를 설정 뿐 아니라 수 파일 리디렉션 수정을 사용 하 여 패키지 상대 "PSFSampleApp" 디렉터리 아래에 있는.log 파일을 읽기/쓰기 리디렉션할 합니다.
...
| 응용 프로그램 | 실행 | 시작 하려는 실행 파일의 패키지 상대 경로입니다. 대부분의 경우, 수정 하기 전에이 값이 패키지 매니페스트 파일에서 가져올 수 있습니다. 값은 `Executable` 의 특성은 `Application` 요소. |
...
| 프로세스 | 실행 | 대부분에서의 이름이 됩니다 합니다 `executable` 제거 경로 및 파일 확장명을 가진 위에 구성 합니다. |
| 수정 | dll | .Msix/.appx 로드를 수정 하기 위해 패키지 상대 경로입니다. |
| 수정 | 구성 | (선택 사항) 수정 메일 그룹 동작 하는 방법을 제어 합니다. 이 값의 정확한 형식으로 원하는 각 수정이 "blob" 해석할 수 수정 하 여 수정 별로 달라 집니다. |

`applications`, `processes`, 및 `fixups` 키가 배열입니다. 즉,는 둘 이상의 응용 프로그램, 프로세스 및 수정을 DLL 지정 하려면 config.json 파일을 사용할 수 있습니다.
...
자세한 내용은 참조 [signtool을 사용 하 여 패키지에 서명 하는 방법](https://docs.microsoft.com/en-us/windows/desktop/appxpkg/how-to-sign-a-package-using-signtool) 및 [패키지 서명 인증서를 만드는 방법](https://docs.microsoft.com/en-us/windows/desktop/appxpkg/how-to-create-a-package-signing-certificate)
...
패키지 된 응용 프로그램 호환성 문제를 진단 하는 대체 기술을 추적 수정을 사용 하는 것입니다. 이 DLL의 PSF에 포함 된 하 고 앱의 동작을 유사한 프로세스 모니터의 진단 세부 보기를 제공 합니다.  응용 프로그램 호환성 문제를 표시 하도록 특별히 설계 되었습니다.  추적 수정을 사용 하 여를 추가 하 DLL 패키지에 다음 조각에 config.json를 추가 하 고 다음 패키지 응용 프로그램을 설치 합니다.
...
기본적으로 수정 추적 하기 위해 "예상" 것으로 간주 될 수 있는 오류를 필터링 합니다.  예를 들어, 응용 프로그램 무조건적으로 확인 하는 경우 이미 있는 결과 무시 하지 않고 파일을 삭제 하려고 할 수 있습니다. 이것은 파일 시스템 기능에서 모든 오류를 수신 하도록 선택 하는 위의 예제에서는 되므로 일부 예기치 않은 오류 필터링 얻을 수 있는 맬웨어 결과 있습니다. 그 전에 Config.txt 파일에서 읽는 시도 실패 하는 메시지 "파일 찾을 수 없음"에서 알고 있으므로이 야기 합니다. 이 자주 관찰 되 고 하지 일반적으로 예상 된 것으로 간주 되는 실패 합니다. 실제로 예기치 않은 오류를 필터링 하 고 다음 대체 모든 오류 문제 여전히 식별할 수 없는 경우 시작 가능성이 가장 됩니다.

기본적으로 수정 추적 하기 위해 출력 디버거에 연결 된 전송 가져옵니다. 이 예제에서는 디버거를 첨부할 생각 하 고 출력을 보려면 SysInternals에서 [DebugView](https://docs.microsoft.com/en-us/sysinternals/downloads/debugview) 프로그램을 대신 사용 됩니다. 앱을 실행 한 후 알 수 있습니다 동일한 오류 이전과 하는 것 가리킵니다 우리는 동일한 런타임 수정 합니다.
...
| PSFLauncher | C + + 빈 프로젝트입니다. 이 프로젝트는 패키지 지원 프레임 워크의 런타임 배포 파일을 수집 하는 위치입니다. 실행 파일을 출력 합니다. 해당 실행 파일은 먼저 솔루션을 시작할 때 실행 됩니다. |
...
만들고 솔루션에 이들 각이 프로젝트를 구성 하는 단계를 살펴보겠습니다.
...
**솔루션 탐색기**패키징 프로젝트를 마우스 오른쪽 단추로 클릭 **편집**, 선택한 다음 프로젝트 파일의 맨 아래에이 추가 합니다.
...
다음 **속성**을 선택 하 고 프로젝트를 마우스 오른쪽 단추로 합니다.
...
디버그 하거나 기존 런타임 수정 프로그램을 확장 하려는 경우이 가이드의 [런타임 수정 찾기](#find) 섹션에 설명 된 지침을 사용 하 여 얻은 런타임 수정 파일을 추가 합니다.
...
**열기 **일반** 설정 페이지에는 프로젝트에 대 한 속성 페이지의 대상 이름** 속성을 설정 ``PSFLauncher32`` 또는 ``PSFLauncher64`` 응용 프로그램의 아키텍처에 따라 합니다.
...
Visual Studio에서 f5 키를 눌러 디버거를 시작 합니다.  먼저 시작 되는 대상 데스크톱 응용 프로그램을 시작 하는 PSF 시작 관리자 응용 프로그램입니다.  대상 데스크톱 응용 프로그램을 디버깅 하려면 수동으로 **디버그**선택 하 여 데스크톱 응용 프로그램 프로세스에 연결 해야 합니다->**프로세스에 연결**하 고 다음 응용 프로그램 프로세스를 선택 합니다. .NET 네이티브 런타임 수정 DLL 사용 하 여 응용 프로그램의 디버깅을 허용 하려면 관리 코드와 네이티브 코드 유형 (디버깅 혼합된 모드)를 선택 합니다.  

했으므로이 설정한 후 데스크톱 응용 프로그램 코드 및 런타임 수정 프로젝트에서 줄의 코드로 옆에 중단점을 설정할 수 있습니다. 응용 프로그램에 소스 코드가 없는, 런타임 수정 프로젝트에서 줄의 코드로 옆는 중단점을 설정할 수 있습니다.
...
동일한 서명을 함수를 사용 하는 함수를 만듭니다 사용자가 수정 하려는 동작입니다. 다음은 대체 하는 함수 예제는 `MessageBoxW` 함수 합니다.
...
에 대 한 대체 함수를 생성할 수 있습니다 예를 들어 합니다 `CreateFile` 함수입니다. 구현을 호출할 수도 있습니다 합니다 `CopyFile` 함수를 하지만 구현은 `CopyFile` 함수를 호출할 수는 `CreateFile` 함수 합니다. 따라서에 대 한 호출에서 무한 반복 주기 발생할 수 있습니다 합니다 `CreateFile` 함수입니다.
...
런타임 수정 구성 데이터를 추가 하려는 경우 추가할 것을 고려 하는 ``config.json``. 사용할 수 이런 식으로는 `FixupQueryCurrentDllConfig` 를 쉽게 해당 데이터를 구문 분석 합니다. 이 예제에서는 해당 구성 파일에서 문자열 및 부울 값을 구문 분석합니다.
...
Visual Studio를 사용 하 여 다른 문제 디버거를 통해 시작 된 모든 자식 프로세스에 연결 하는 것에 대 한 기본 지원을 없는입니다.   이렇게 하면 시작 된 후 Visual Studio에서 수동으로 연결 되어 있어야 합니다 대상 응용 프로그램의 시작 경로 논리를 디버그 하기가 어렵습니다.

하위 프로세스를 지 원하는 디버거를 사용 하 여가이 문제를 해결 하려면 다음을 첨부 하세요.  참고 아닌지 일반적으로 대상 응용 프로그램에 적시에 (JIT) 디버거를 첨부할 수 있습니다.  대부분의 JIT 기술 관련 ImageFileExecutionOptions 레지스트리 키를 통해 대상 앱 대신 디버거 실행 때문입니다.  이 방법을 detouring 메커니즘 PSFLauncher.exe FixupRuntime.dll 대상 앱에 삽입 하는 데 사용 합니다.  WinDbg, [Windows 용 디버깅 도구](https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/index)포함 되어 있으며 [Windows SDK](https://developer.microsoft.com/en-US/windows/downloads/windows-10-sdk)가져온 지원 하위 프로세스를 연결 합니다.  또한 지원 직접 [시작 하 고 UWP 앱을 디버깅](https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/debugging-a-uwp-app-using-windbg#span-idlaunchinganddebuggingauwpappspanspan-idlaunchinganddebuggingauwpappspanspan-idlaunchinganddebuggingauwpappspanlaunching-and-debugging-a-uwp-app)합니다.
...
>[!NOTE]
> [PLMDebug](https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/plmdebug) 디버거를 시작 시 응용 프로그램 연결할도 사용할 수 있으며 [Windows 용 디버깅 도구에](https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/index)에서 포함 되어 있습니다.  그러나 이제 WinDbg에서 제공 하는 직접 지원 보다 사용 하 여 더 복잡 한 됩니다.
