...
버퍼 요소는 1 ~ 4 개 구성 요소 이루어져 있습니다. 버퍼 요소에는 압축된 데이터 값(R8G8B8A8 표면 값), 8비트 정수 하나 또는 32비트 부동 소수점 값 네 개가 포함될 수 있습니다.
...
꼭 짓 점 버퍼의 간단한 예는만 위치 데이터를 포함 하는 하나. 다음 그림과 같이 표현할 수 있습니다.
...
이 꼭 짓 점 버퍼는 꼭 짓 점별 데이터를 포함 각 꼭 짓 점 세 개의 요소 (위치, 일반 및 텍스처 좌표)를 저장합니다. 위치 및 일반 좌표는 일반적으로 32비트 부동 소수점 3개를 사용하여 지정되고 텍스처 좌표는 32비트 부동 소수점 2개를 사용하여 지정됩니다.
...
상수 버퍼를 사용 하면 파이프라인에 셰이더 상수 데이터를 효율적으로 제공할 수 있습니다. 스트림 출력 단계의 결과를 저장 하는 상수 버퍼를 사용할 수 있습니다. 개념적으로 상수 버퍼는 다음 그림과에서 같이 단일 요소 꼭 짓 점 버퍼 처럼 보입니다.
...
셰이더는 셰이더 상수 버퍼를 읽으려면는 HLSL 부하 함수를 사용 합니다. 각 셰이더 단계는 최대 15개의 셰이더 상수 버퍼를 허용하며 각 버퍼는 최대 4,096개 상수를 보관할 수 있습니다.
