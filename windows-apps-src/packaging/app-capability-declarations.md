...
| **완전 신뢰 권한 수준이** | **RunFullTrust** 제한 된 접근 권한 값을 완전 신뢰 권한 수준에서 사용자의 컴퓨터에서 실행할 수 있습니다. 이 접근 권한이 값은 [FullTrustProcessLauncher](https://docs.microsoft.com/uwp/api/windows.applicationmodel.fulltrustprocesslauncher) API 사용 해야 합니다.<br /><br />이 접근 권한이 값을 appx로 전달 되는 모든 데스크톱 응용 프로그램에 대 한 필수 이기도 (와 마찬가지로 [데스크톱 브리지](https://developer.microsoft.com/windows/bridges/desktop)) 패키지를 Desktop App Converter (DAC) 또는 시각을 사용 하 여 이러한 앱을 패키징할 때 자동으로 매니페스트의 표시 됩니다 Studio 합니다. |
