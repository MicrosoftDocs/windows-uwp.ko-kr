...
바이너리를 미리 컴파일하여.NET Framework 응용 프로그램의 시작 시간을 향상 시킬 수 있습니다. 패키지 및 Windows 스토어를 통해 배포 하는 큰 응용 프로그램에서이 기술을 사용할 수 있습니다. 경우에 따라 20% 성능이 향상을 관찰 되었습니다. [기술 개요](https://github.com/dotnet/coreclr/blob/master/Documentation/botr/readytorun-overview.md)에서이 기술에 대해 자세히 알아볼 수 있습니다.
...
대상.NET Framework 4.6.2 하도록 프로젝트를 구성 하는.NET Framework 4.6.2 개발 도구 해야 이상. .NET 데스크톱 개발 워크 로드에서 선택적 구성 요소로 이러한 도구는 Visual Studio 설치 관리자를 통해 사용할 수 있습니다.
...
네이티브 이미지 컴파일러는 지정 된 플랫폼에 대 한 코드를 최적화합니다. 이 기능을 사용 하려면 하나의 특정 플랫폼 예: x86 또는 x64 대상 응용 프로그램을 구성 해야 합니다.

솔루션에 여러 프로젝트에 있는 경우만 입력 지점 프로젝트 (대개 실행 파일을 생성 하는 프로젝트)에 x86 또는 x64 컴파일됩니다. 기본 프로젝트에서 참조 추가 바이너리 AnyCPU로 컴파일되는 경우에 기본 프로젝트에서 지정 된 아키텍처를 사용 하 여 처리 됩니다.
...
3. **새 프로젝트 플랫폼** 대화 상자에는 **설정 복사** 드롭다운 목록에서 **모든 CPU**를 설정 되어 있는지 확인 합니다.

![X86 구성](images/desktop-to-uwp/configure-x86.png)

이 단계를 반복 `Release/x64` x64를 생성 하려면 이진 파일.
...
네이티브 이미지 컴파일러는 실행 파일을 생성 하는 Visual Studio 프로젝트에 추가 해야 하는 NuGet 패키지로 제공 됩니다. 이것이 일반적으로 Windows Forms 또는 WPF 프로젝트입니다. 이렇게 하려면 다음 PowerShell 명령을 사용 합니다.
...
> [!NOTE]
> 미리 보기 패키지 NuGet.org 목록으로 게시 됩니다. 검색 NuGet.org 하거나 Visual Studio에서 패키지 관리자 UI를 사용 하 여 찾아서 없습니다. 그러나 패키지 관리자 콘솔에서 설치할 수 및 때 다른 컴퓨터에서 복원 있습니다. 만들 것 패키지 완벽 하 게 액세스할 수 있는 첫 번째 비-미리 보기 버전을 게시 했습니다.
...
도구 바이너리에서 처리 하 고 있는지 확인 하려면 빌드 출력 이와 같은 메시지를 포함 하는지 검토할 수 있습니다.
...
A. 이 버전 현재 사용할 수 있는 Live 이동 라이선스에 포함 됩니다.
