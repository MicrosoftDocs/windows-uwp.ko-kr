...
일반적으로 게임에 스레드를 통해 **비동기 작업** 을 실행 제어 거의 및 스레드 **완료 콜백에서**사용 하는 경우 결과 반환 합니다.  일부 게임은 힙의 섹션에는 단일 스레드 스레드 동기화에 대 한 모든 필요성을 방지 하기 위해 접촉만 되도록 설계 되었습니다. **모든 완료 콜백이** 스레드에서 호출 되지 않습니다는 **비동기 작업** 의 결과 사용 하 여 공유 상태를 업데이트 하 고 게임 컨트롤, 스레드 동기화를 해야 합니다.
...
* **AsyncQueue** **비동기 작업** 을 실행 스레드를 확인할 수 있는 스레드 AsyncBlock의 **모든 완료 콜백이**호출 있습니다.
...
* *큐* 에 대 한 핸들을 되는 async_queue_handle_t는 **AsyncQueue**를 지정 합니다. 이 설정 되지 않은 경우 기본 큐 사용 됩니다.

각 비동기 API 호출에 대 한 힙에서 새 AsyncBlock를 만들어야 합니다.  AsyncBlock AsyncBlock의 모든 완료 콜백이 호출 되 고 삭제 한 다음 될 때까지 live 해야 합니다.
...
**비동기 작업이** 완료 되었음을 알 수 다양 한 가지가 있습니다.

* AsyncBlock의 **모든 완료 콜백이** 호출 되어
* 완료 될 때까지 기다릴 true로 **GetAsyncStatus** 를 호출 합니다.
* waitEvent **AsyncBlock** 에서 설정 하 고 신호 이벤트를 대기

**하지만 GetAsyncStatus** 와 waitEvent AsyncBlock의 **완료 콜백에서** AsyncBlock의 **모든 완료 콜백이** 선택 사항 실행 한 후에 **비동기 작업** 을 완료 간주 됩니다.

**비동기 작업이** 완료 되 면 결과 얻을 수 있습니다.

### <a name="getting-the-result-of-the-asynchronous-task"></a>**비동기 작업** 의 결과 가져오기

결과 얻기 위해 대부분의 **비동기 API** 함수는 Function\의 해당 \[Name] 결과 함수는 비동기 호출의 결과 받을 수 있습니다. 이 예제 코드에서 **XblProfileGetUserProfileAsync** 는 해당 **XblProfileGetUserProfileResult** 기능을 갖습니다. 이 함수는 함수의 결과 반환 하 고 그에 따라 역할을 사용할 수 있습니다.  검색 결과에 전체 세부 정보에 대 한 각 **비동기 API** 함수의 documention를 참조 하세요.
...
* *수동* -수동 큐는 자동으로 발송 하지 않습니다.  개발자가 원하는 모든 스레드에서 디스패치합니다 수입니다. 특정 스레드에 비동기 호출의 작업 또는 콜백 쪽 할당할 사용할 수 있습니다.  이 아래에 자세히 설명 합니다.

* *스레드 풀* -디스패치 스레드 풀을 사용 합니다.  스레드 풀 큐에서 차례 대로 threadpool 스레드를 사용할 수 있게 하는 대로 실행에 대 한 호출을 수행 하는 동시에 호출을 호출 합니다.  사용 하 여 easist 이지만 최소한의 스레드 사용 되는 컨트롤을 제공 합니다.

* *고정 스레드* -QueueUserAPC를 사용 하 여 비동기 큐에서 만든 스레드에서 디스패치. 사용자 모드 APC, 대기 중인 경우 스레드 하지 APC 함수를 호출 하 여 경고 상태가 아니면로 리디렉션됩니다. **SleepEx**, **SignalObjectAndWait**, **WaitForSingleObjectEx**, **WaitForMultipleObjectsEx**또는 **MsgWaitForMultipleObjectsEx** 경고 대기 작업을 수행 하 여 상태가 경고 스레드
...
큐의 ID를 지정 하 여 동일한 큐 형식과는 **AsyncQueue** 만드는 **CreateSharedAsyncQueue** 호출할 수도 있습니다.
...
에 **AsyncQueue** 를 간단 하 게 만든 후 작업 및 완료 기능에서 스레딩 제어 하려면 **AsyncBlock** 에 추가 합니다.
...
**AsyncQueueDispatchMode_Manual** 할당 된 작업을 발송 하기 위해 **DispatchAsyncQueue** 함수를 사용 하 여 디스패치 해야 합니다.
...
* *상황에 맞* 는-제출 콜백을 전달 되는 데이터에 대 한 포인터입니다.
* *콜백* -새 콜백을 큐에 제출 되 면 호출 될 하는 함수입니다.
...
그런 다음 백그라운드 스레드에서 수신 대기할 수 있습니다이 세마포의 절전 모드 및 **DispatchAsyncQueue**호출 합니다.
...
것이 가장 좋습니다 Win32 세마포의 개체를 사용 하 여 구현 합니다.  구현 하는 경우 대신 Win32 이벤트 개체를 사용 하 여 확인 해야 하는 다음 잊지 코드를 사용 하 여 모든 이벤트와 같은:
