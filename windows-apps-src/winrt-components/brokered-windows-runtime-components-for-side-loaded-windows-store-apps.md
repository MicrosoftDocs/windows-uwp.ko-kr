...
description: 이 백서는 업무에 중요 한 핵심 작업을 담당 하는 기존 코드를 사용 하 여 터치 친화적인.NET 앱 엔드가 Windows10에서 지원 하는 기업 대상 기능을 설명 합니다.
ms.author: misatran
ms.date: 02/08/2017
ms.topic: article
...
ms.locfileid: "5982246"
---
# <a name="brokered-windows-runtime-components-for-a-side-loaded-uwp-app"></a>테스트용으로 로드하는 UWP 앱의 조정된 Windows 런타임 구성 요소

이 문서에서는 업무에 중요 한 핵심 작업을 담당 하는 기존 코드를 사용 하 여 터치 친화적인.NET 앱 엔드가 Windows10에서 지원 하는 기업 대상 기능에 설명 합니다.
...
>**참고**이 기능은 용 으로만.NET을 사용 합니다. 클라이언트 앱과 데스크톱 구성 요소는 둘 다 .NET을 사용해서 작성해야 합니다.
...
한 번 참조**winmd**만들어집니다 (프로젝트의 대상 폴더 아래 "참조" 폴더)는 손으로 (복사) 각 사용 하는 응용 프로그램 테스트용으로 로드 프로젝트를 수행 하 고 참조 합니다. 이 내용은 다음 섹션에서 자세히 설명하겠습니다. 위의 빌드 규칙에 포함 된 프로젝트 구조 되도록 구현 및 참조**winmd**혼동을 피하기 위해 빌드 계층 구조에서 명확 하 게 분리 된 디렉터리에 있습니다.
...
이 응용 프로그램 구성에 적용되지 않는 outOfProcessServer 범주에 항목이 여러 개 있기 때문에, 범주는 inProcessServer입니다. 하지만 유의 합니다 <Path> 구성 요소 항상 clrhost.dll을 포함 해야 합니다 (이것은**하지**적용 하 고 다른 값을 지정 하면 정의 되지 않은 방식으로 실패).
...
"계약 정의" 섹션의 설명대로, 데스크톱 구성 요소의 참조 winmd를 프로젝트에서 참조해야 합니다. Visual Studio 프로젝트 시스템은 일반적으로 이름이 같은 2개 수준의 디렉터리 구조를 만듭니다. 샘플에서는 이 구조가 EnterpriseIPCApplication\\EnterpriseIPCApplication입니다. 참조 **winmd**이 두 번째 수준 디렉터리 및 다음 대화 상자를 사용 하 여 프로젝트 참조를 수동으로 복사한 (클릭를**찾습니다.** 단추)를 찾아이 **winmd**를 참조 합니다. 그러면 데스크톱 구성 요소의 최상위 수준 네임스페이스(예제: Fabrikam)가 프로젝트의 참조 부분에서 최상위 수준 노드로 나타납니다.

>**참고** 것이 매우 중요**참조 winmd**를 사용 하 여응용 프로그램 테스트용으로 로드 합니다. **구현 winmd**를 통해 실수로 수행 하는 경우를 테스트용으로 로드 하는 앱 디렉터리로 참조, 가능성이 받게 됩니다 "IStringable을 찾을 수 없습니다"와 관련 된 오류가 발생 합니다. 이것은 확실 하는 잘못 된**winmd**참조 되었습니다. IPC 서버 앱의 (다음 섹션에서 자세히 설명) 신중 하 게 사후 빌드 규칙 분리 이러한 두**winmd**별도 디렉터리에 있습니다.
...
참조가 제대로 구성되면 다음 작업은 서버의 기능을 구현하는 것입니다. [Windows 런타임 구성 요소 (C\ # /vb/c + + 및 XAML을 사용 하는 UWP 앱)와 상호 운용성 모범 사례](https://msdn.microsoft.com/library/windows/apps/hh750311.aspx)MSDN 항목을 참조 하세요.
...
-   반환*목록<T>* 여기서*T*개체는 비동기 작업 또는 속성 반입에서 이면 크로스 프로세스 활성도 줄어듭니다 반입 됩니다. 예를 들어, 돌아가면는*목록&lt;사용자&gt;* 개체입니다. 각 반복 단계가 크로스 프로세스 호출이 됩니다. 각*사용자*반환 되는 개체는 프록시 및 메서드를 호출할 때마다 개체로 또는 해당 개별 개체에 속성 하면 크로스 프로세스 호출이 발생 합니다. 따라서 "순수"*목록&lt;사용자&gt;* 개체를*개수*큰 많은 느린 호출 하면. 배열의 구조체를 대량으로 전송하면 더 높은 성능을 보입니다. 예:
...
모든 성능 고려 사항의 경우처럼, 측정과 테스트가 중요합니다. 원칙적으로, 걸리는 시간을 결정하기 위해 다양한 작업에 원격 분석을 삽입해야 합니다. 범위 전체에서 측정을 고려해 야: 예를 들어는 실제로 걸리는 모든*사용자*를 사용 하도록개체는 테스트용으로 로드 된 응용 프로그램의 특정 쿼리가?
...
브로커 서버에 대 한 모듈 목록의*clrhost.dll*을 나열 해야에서 로드 된 모듈 목록.
