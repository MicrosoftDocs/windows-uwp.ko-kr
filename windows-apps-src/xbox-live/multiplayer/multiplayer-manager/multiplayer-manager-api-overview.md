...
| JoinGame() | join_game() | 제 3 자 매치 메이 킹 서비스를 통해 일반적으로 발견 된 고유한 세션 이름이 지정 된 기존 게임 세션을 결합 합니다. Xbox 사용자 Id 게임의 일부가 될 목록에 전달할 수 있습니다.|
...
호출 하는 경우 `do_work()`, 마지막으로 호출 되므로 변경 내용을 세션을 나타내는 이벤트는 목록을 반환 하는 멀티 플레이어 관리자 `do_work()`. 구성원 세션에 참가 등 이러한 이벤트에는 변경 포함, 구성원 세션이 떠났음을 멤버 속성이 변경, 호스트 클라이언트 변경 등.

모든 가능한 이벤트 형식 목록은 참조는 `multiplayer_event_type` 열거 합니다.

반환 된 각 `multiplayer_event` 포함 된 `event_args`, 이벤트 형식에 대 한 적절 한 event_args 클래스에 캐스팅 해야 하는 합니다. 예를 들어 경우는 `event_type` 는 `member_joined`를 캐스팅 하는 다음는 `event_args` 에 대 한 참조는 `member_joined_event_args` 클래스.
...
영구 세션 중 하나로이 장치와 함께 재생 하고자 하는 친구 초대에 있는 사용자를 관리에 사용 합니다. 대기실 세션 멀티 플레이어 관리자 멀티 플레이 작업을 수행할 멤버를 하나 이상 포함 해야 합니다. 호출 하 여 새 로비 세션을 처음 만들 수는 `add_local_user()` 메서드.
