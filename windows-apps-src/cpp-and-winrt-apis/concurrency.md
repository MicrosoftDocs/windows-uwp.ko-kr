...
> [!NOTE]
> **Get** 함수가 존재에서 C + + /winrt 프로젝션 모든 C + 내에서 함수를 호출할 수 있도록 **winrt::Windows::Foundation::IAsyncAction**입력 + WinRT 프로젝트. **가져오기** 는 실제 Windows 런타임 형식의 **IAsyncAction**응용 프로그램 이진 인터페이스 (ABI) 표면의 포함 되지 않으므로 [**IAsyncAction**](/uwp/api/windows.foundation.iasyncaction) 인터페이스의 구성원으로 나열 된 함수를 찾지 않습니다.
...
비동기 프로그래밍에 대 한 Windows 런타임 기능 인플라이트 비동기 작업을 취소할 수 있도록 합니다. 간단한 예로부터 시작 해 보겠습니다.
...
코 루틴이 일시 중단 된 동안에 물론 취소 발생할 수 있습니다. 코 루틴에 다시 시작 될 때에 다른 부딪치는 또는 `co_await`, 취소에 대 한 확인 됩니다. 문제 취소에 대 한 응답 잠재적으로 너무 거친-세분화 된 대기 시간 중 하나입니다.

따라서 두 번째 방법은 사용자 코 루틴이 내에서 취소에 대 한 명시적으로 폴링합니다. 위 예제는 아래 목록에 코드를 사용 하 여 업데이트 합니다. 새이 예제에서는 **ExplicitCancellationAsync** [**winrt::get_cancellation_token**](/uwp/cpp-ref-for-winrt/get-cancellation-token) 함수에 의해 반환 되는 개체를 검색 하 고 사용 하 여 주기적으로 코 루틴이 취소 되었는지 여부를 확인 합니다. 취소 되지 않은으로 코 루틴이 무한; 취소 되 면 루프와 함수 정상적으로 종료 합니다. 이전 예제에서는 하지만 여기 종료 명시적으로 발생 하는 대로 및 컨트롤에서 결과 동일 합니다.
...
**CancellationPropagatorAsync** 취소 콜백 자체에 대 한 람다 함수를 등록 하 고 대기 합니다 (이 일시 중단) 중첩 된 작업이 완료 될 때까지 합니다. 하거나 **CancellationPropagatorAsync** 취소 될 때에 중첩 된 코 루틴을 취소를 전파 됩니다. 취소; 폴링 필요가 없음 나는 취소 무기한 차단 합니다. 이 메커니즘은 C + 아무것도 알고 있는 코 루틴 또는 동시성 라이브러리를 사용 하 여 interop를 사용할 수 있을 만큼 유연 + WinRT 합니다.
...
완료 처리기에 대 한 자세한 내용은 [비동기 작업을 위한 대리자 유형](handle-events.md#delegate-types-for-asynchronous-actions-and-operations)참조 하세요.

## <a name="fire-and-forget"></a>발생 한 것을 잊지합니다

경우에 따라 다른 작업을 동시에 수행할 수 있는 작업이 하 고 해당 작업이 완료 되기를 기다리는 필요가 없습니다 (없는 다른 작업에 따라 달라), 필요도 없습니다 값을 반환 합니다. 이 경우 작업을 실행 하 고 기억 수 있습니다. 반환 형식이 [**winrt::fire_and_forget**](/uwp/cpp-ref-for-winrt/fire-and-forget) (대신 **concurrency:: task**또는 Windows 런타임 비동기 작업 형식 중 하나)는 코 루틴을 작성 하 여 수행할 수 있습니다.
