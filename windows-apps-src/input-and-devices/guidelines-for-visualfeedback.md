...
다음은 Windows 8에 포함된 몇 가지 기본 제공 접촉 시각화의 예입니다.
...
활성화
사용
스크러빙 및 작은 대상을 위한 폐색 도구 설명
0ms
예
빠른 동작 설명을 위해 제공됩니다. 일반적으로 명령에 사용됩니다.
동작에 대한 폐색 도구 설명
200ms
예
풍부한 도구 설명
~2000ms
아니요
더 천천히 신중하게 생각해야 하는 탐색 및 학습을 위해 제공됩니다. 일반적으로 컬렉션 항목과 함께 사용됩니다.
자체 노출 조작
~2000ms
아니요
상황에 맞는 메뉴
...
자체 노출 조작은 대상 개체에 대해 동작을 수행하는 방법을 보여 주고 해당 동작의 결과를 미리 보여 주는 정보용 시각 신호 또는 애니메이션입니다.

다음 몇 개의 그림은 시작 화면에서 크로스 밀기 선택을 위한 자체 노출 조작을 보여 줍니다. 사용자가 앱 타일을 끌지 않고 터치하면 마치 끈 것처럼 타일이 아래로 내려가면서 앱을 실제로 선택할 경우 표시되는 선택 확인 표시가 나타납니다.

![선택되지 않은 상태를 보여 주는 스크린샷](images/crossslide-selfreveal1.png)

*항목을 손가락으로 눌러 선택을 위한 자체 노출 조작을 시작합니다. 자체 노출 조작은 항목에 대해 수행될 동작을 보여 줍니다.*

![선택 애니메이션을 보여 주는 스크린샷](images/crossslide-selfreveal2.png)

*손가락을 떼지 않고 살짝 밀어 항목을 실제로 선택합니다.*

![끌어서 놓기 애니메이션을 보여 주는 스크린샷](images/crossslide-selfreveal3.png)

*사용자가 계속해서 손가락을 밀면 자체 노출 시각화가 변경되어 이제 개체를 이동할 수 있음을 보여 줍니다.*

자체 노출 조작이 표시되면 사용자가 손가락을 떼도 개체가 더 이상 활성화되지 않습니다.
