...
앱 아이콘 자산은 비트맵 비트맵 원활 하 게 조정 되므로 각 배율 인수에 대 한 각 아이콘 자산 버전을 제공 하는 것이 좋습니다: 100%, 125%, 150%, 200% 및 400%입니다. 아이콘의이! Fortunatly, Visual Studio는 간편 하 게 생성 하 고이 아이콘을 업데이트 하는 도구를 제공 합니다. 

## <a name="microsoft-store-listing-image"></a>Microsoft Store 목록 이미지

"지정 하는 방법 이미지 내 앱 목록에 대 한 Microsoft Store에서"?

기본적으로 사용 하 여 패키지에서 이미지의 일부가 스토어에서 다른 [제출 프로세스 중에 제공 된 이미지](https://docs.microsoft.com/en-us/windows/uwp/publish/app-screenshots-and-images)) (함께이 페이지의 맨 표에 설명 된 대로 합니다. 그러나 스토어 Windows 10 (Xbox 포함) 고객에 게 목록을 표시할 때 앱 패키지의 로고 이미지를 사용 하는 것을 방지 하 고 대신 업로드 한 이미지만 사용 스토어가 있습니다. 이렇게 하면 앱의 모양 보다 잘 제어 Store 전반에서 다양 한 디스플레이에 있습니다. (단 제품 이전 OS 버전을 지 원하는 경우 해당 고객 수 여전히 이미지에서 패키지에이 옵션을 사용 하는 경우에.) 제출 프로세스의 **스토어 목록** 단계 **스토어 로고** 섹션에서이 수행할 수 있습니다.

![앱 제출 프로세스 동안 스토어 로고를 지정합니다.](images/app-icons/storelogodisplay.png)

이 확인란을 선택 하면 **이미지를 표시 하는 저장소** 라는 섹션이 새로 나타납니다. 여기에서 스토어 앱 패키지의 로고 이미지 대신 사용 하는 3 가지 이미지 크기를 업로드할 수 있습니다: 300 x 300, 150 x 150 71 x 71 픽셀입니다. 300 x 300 크기에만 필요 하지만 3 크기를 제공 하는 것이 좋습니다.
...
Visual Studio가 기본적으로 배지 로고를 생성 하지 않는 것입니다. 배지 로고는를 아마도 다른 응용 프로그램 아이콘 일치 하지 않아야 하기 때문입니다. 자세한 내용은 [배지 알림은 UWP 앱 문서](/windows/uwp/design/shell/tiles-and-notifications/badges)를 참조 하세요. 
...
왜 그렇지 기본적으로 배지 로고를 생성 하지 않는 이유는 자산 생성기를 사용 하 여 필요한 모든 자산을 생성 하는 경우: 다른 응용 프로그램 자산와에서 매우 다릅니다. 배지 로고 알림 및 앱의 타일에 표시 되는 상태 이미지입니다. 
...
타일 크기를 4 가지: (71 x 71) 작은 크기 (150 x 150), 전체 (150 x 310) 및 큰 (310 x 310). 
