...
Visual Studio에서는 XAML UI를 쉽게 만들 수 있는 여러 가지 도구를 제공합니다.

+ 도구 상자에서 XAML 디자이너 디자인 화면으로 컨트롤을 끌어와서 앱을 실행하기 전에 어떤 모습인지 확인합니다. 
+ 속성 패널을 사용하면 디자이너에서 활성화된 컨트롤의 모든 속성을 보고 설정할 수 있습니다.
+ 문서 개요는 UI에 대한 XAML 시각적 트리의 부모/자식 구조를 보여 줍니다.
+ XAML 편집기를 사용하면 XAML 태그를 직접 입력하고 수정할 수 있습니다.
...
3. 도구 상자에서 **공용 XAML 컨트롤** 을 확장하고 [TextBlock](/uwp/api/windows.ui.xaml.controls.textblock) 컨트롤을 찾습니다. `TextBlock` 컨트롤을 디자인 화면으로 끌어서 페이지의 왼쪽 위 모서리 근처에 놓습니다.
...
4. **속성** 패널에서, `TextBlock` 컨트롤의 **이름** 값을 **TitleTextBlock** 으로 변경합니다. (`TextBlock` 컨트롤이 여전히 활성 개체인지 확인하세요.)
...
9. **속성** 패널의 검색 상자에서 **textwrapping** 을 입력하여 `TextWrapping` 속성을 찾습니다. `TextWrapping` 속성에 대한 _속성 마커_ 를 선택하여 해당 메뉴를 엽니다. (속성 마커는 각 속성 값의 오른쪽에 있는 작은 상자 모양 기호입니다. 속성 마커가 검은색이면 해당 속성이 기본값이 아닌 값으로 설정되었음을 나타냅니다.) **속성** 메뉴에서 **다시 설정** 을 선택하여 `TextWrapping` 속성을 다시 설정합니다.
...
먼저 루트 [Grid](/uwp/api/windows.ui.xaml.controls.grid) 레이아웃을 [RelativePanel](/uwp/api/windows.ui.xaml.controls.relativepanel)로 바꿉니다. `RelativePanel`을 사용하면 패널 또는 다른 UI 요소를 기준으로 UI 청크를 쉽게 다시 정렬할 수 있습니다. 이에 대한 유용성은 [XAML 적응형 레이아웃](xaml-basics-adaptive-layout.md) 자습서에서 확인할 수 있습니다.

다음으로, [GridView](/uwp/api/windows.ui.xaml.controls.gridview) 컨트롤을 추가하여 데이터를 표시합니다.
...
2. **ImageGridView** 라는 `GridView` 컨트롤을 `TextBlock` 요소 아래에 추가합니다. `RelativePanel` _연결된 속성_ 을 설정하여 컨트롤을 제목 텍스트 아래에 배치하고 화면의 전체 너비에 맞게 늘립니다.
...
3. `GridView` 컨트롤에서 항목을 표시하려면 표시할 데이터 컬렉션을 제공해야 합니다. **MainPage.xaml.cs** 를 열고 `GetItemsAsync` 메서드를 찾습니다. 이 메서드는 **MainPage** 에 추가한 속성인 **Images** 라는 컬렉션을 채웁니다.
    ...
    `GetItemsAsync` 메서드의 끝에 다음 코드 줄을 추가합니다.

    ```csharp
    ImageGridView.ItemsSource = Images;
    ```

    그러면 `GridView` 컨트롤의 [ItemsSource](/uwp/api/windows.ui.xaml.controls.itemscontrol.itemssource) 속성이 앱의 **Images** 컬렉션으로 설정됩니다. 또한 표시할 항목을 `GridView` 컨트롤에 제공합니다.
...
이제 데이터를 표시하는 방법을 `GridView`에 지시하는 [DataTemplate](/uwp/api/windows.ui.xaml.datatemplate) 개체를 만듭니다. 데이터 템플릿의 전체 설명은 [항목 컨테이너 및 템플릿](../controls-and-patterns/item-containers-templates.md)을 참조하세요.
...
1. **MainPage.xaml** 을 엽니다.

2. 평점을 표시하려면 WinUI([Windows UI 라이브러리](/windows/apps/winui/)) NuGet 패키지에서 [RatingControl](/uwp/api/microsoft.ui.xaml.controls.ratingcontrol)을 사용합니다. WinUI 컨트롤의 네임스페이스를 지정하는 XAML 네임스페이스 참조를 추가합니다. 이 참조를 다른 `xmlns:` 항목 바로 뒤의 여는 `Page`태그에 배치합니다.
...
4. 대화 상자에서 **Name (key)** 값을 **ImageGridView_DefaultItemTemplate** 으로 변경한 다음, **확인** 을 선택합니다.
    ...
    **확인** 을 선택하면 다음과 같은 몇 가지 작업이 수행됩니다.
    ...
    + `DataTemplate` 개체가 **MainPage.xaml** 의 `Page.Resources` 섹션에 추가됩니다.
    ...
    + `GridView` 컨트롤의 [ItemTemplate](/uwp/api/windows.ui.xaml.controls.itemscontrol.itemtemplate) 속성은 `DataTemplate` 리소스로 설정됩니다.
...
6. 컨트롤을 `Grid` 레이아웃에 추가합니다.
    ...
    a. 그리드에 [이미지](/uwp/api/windows.ui.xaml.controls.image) 컨트롤을 추가합니다. 기본적으로 첫 번째 그리드 행(0행)에 배치됩니다. 이는 이미지가 표시되는 위치입니다. 그러나 지금은 앱의 스토어 로고를 자리 표시자로 사용합니다.
...
이제 앱을 실행하여 방금 만든 항목 템플릿이 있는 `GridView` 컨트롤을 확인합니다. 다음에는 배경색을 변경하고 그리드 항목 사이에 공간을 추가합니다.

![항목 템플릿을 보여주는 실행 중인 컬렉션 앱의 스크린샷.](images/xaml-basics/layout-1.png)
...
2. 대화 상자에서 **Name (key)** 값을 **ImageGridView_DefaultItemContainerStyle** 로 변경한 다음, **확인** 을 선택합니다.
    ...
    `GridViewItem` 기본 스타일은 많은 속성을 설정합니다. 하지만, 템플릿 복사본에서 수정할 속성만 유지하면 됩니다.
...
3. `Background`와 `Margin`을 제외한 `Setter` 요소를 모두 삭제합니다.

4. `Background` 속성의 값을 `Gray`로 변경합니다.
...
![앱 창의 오른쪽에 추가 공간이 있는 항목 템플릿을 보여주는 실행 중인 컬렉션 앱의 스크린샷.](images/xaml-basics/layout-2.png)
...
1. `GridView`에서 [HorizontalAlignment](/uwp/api/windows.ui.xaml.frameworkelement.horizontalalignment) 속성을 `Center`로 설정합니다.
...
5. 대화 상자에서 **Name (key)** 값을 **ImageGridView_ItemsPanelTemplate** 으로 변경한 다음, **확인** 을 선택합니다.
    ...
    다양한 패널을 사용하여 앱의 컨트롤에 대한 레이아웃을 처리하는 것처럼 `GridView`에는 항목의 레이아웃을 관리하는 내부 패널이 있습니다. 이제 이 패널([ItemsWrapGrid](/uwp/api/windows.ui.xaml.controls.itemswrapgrid))에 액세스할 수 있으므로 해당 속성을 수정하여 `GridView` 컨트롤 내의 항목 레이아웃을 변경할 수 있습니다.
