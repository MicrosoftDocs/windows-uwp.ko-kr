...
앱은 예약 이름을 하나 이상 포함해야 합니다. 대시보드에서 앱을 완전히 제거(앱용으로 예약한 모든 이름도 해제)하려면 **개요** 페이지에서 **이 앱 삭제**를 클릭합니다. 진행 중인 앱 제출이 있을 경우, 먼저 제출을 삭제해야 합니다. 앱을 스토어에 이미 게시한 경우 대시보드에서 삭제할 수 없습니다.

## <a name="rename-an-app-that-has-already-been-published"></a>이미 게시된 앱의 이름 바꾸기

앱이 이미 Windows 스토어에 있는데 이름을 바꾸려면 위에 설명된 단계에 따라 앱용으로 새 이름을 예약하고 앱에 대한 새 제출을 만듭니다. 앱을 스토어에서 새 이름으로 표시하려면 패키지를 업데이트하여 새 이름을 포함해야 합니다. 앱 매니페스트의 [**Package/Properties/DisplayName**](https://msdn.microsoft.com/en-us/library/windows/apps/dn934748.aspx) 요소에서 새 이름을 사용하고 앱 이름을 포함하는 그래픽이나 텍스트를 업데이트해야 합니다. 앱 설명을 검토하고 앱 설명에서 이름을 언급하면 해당 이름을 변경할 수도 있습니다.
