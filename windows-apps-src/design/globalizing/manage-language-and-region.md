---
Description: 이 항목에서는 조건 사용자 프로필 언어 목록, 앱 매니페스트 언어 목록 및 앱 런타임 언어 목록을 정의합니다. 이 항목 및 이 기능 영역의 다른 항목에서 이러한 용어를 사용할 예정이므로 이 용어의 의미를 알아 두는 것이 중요합니다.
...
언어 리소스 한정자를 사용하여 리소스 파일 또는 폴더의 이름을 지정합니다. 리소스 한정자에 대한 자세한 내용은 [언어, 배율, 고대비 및 기타 한정자에 맞게 리소스 조정](../../app-resources/tailor-resources-lang-scale-contrast.md)을 참조하세요. 이미지 (또는 다른 자산) 리소스 파일 수 또는 같은 리소스 컨테이너 파일을 수 있습니다는 *.resw* 텍스트 문자열이 포함 된 합니다.

**참고** 앱의 기본 언어의 리소스도 언어 한정자를 지정 해야 합니다. 예를 들어, 앱의 기본 언어는 영어 (미국) 인 경우 한정으로 자산 `\Assets\Images\en-US\logo.png`합니다.

- Windows는 복잡 한 일치, EN-GB 및 EN-US와 같은 국가별 변형에서 포함 하 여 수행 합니다. 따라서 적절 하 게 지역 하위 태그를 포함 합니다. [리소스 관리 시스템이 언어 태그를 일치하는 방법](../../app-resources/how-rms-matches-lang-tags.md)을 참조하세요.
- 값이 없는 보류 스크립트 언어에 대해 정의 된 경우에 한정자에서 스크립트 언어 하위 태그를 지정 합니다. 예를 들어, ZH-CN, zh-tw로 제공 하는 대신 Zh-hant, Hant-ZH-TW, 또는 Zh-hans 사용 (자세한 내용을 참조 하세요. 합니다 [IANA 언어 하위 태그 레지스트리](https://go.microsoft.com/fwlink/p/?linkid=227303)).
- 단일 표준 언어에 있는 언어에 대 한 지역 한정자를 포함할 필요가 없습니다 있습니다. 예를 들어, JA-JP 대신 일본을 사용 합니다.
...
지역화는 모든 리소스에 대 한 필요 하지 않을 수 있습니다.

- 여기에 최소한 기본 언어에 있는 모든 리소스를 확인 합니다.
- 일부 리소스의 하위 집합 (일부만 지역화) 밀접 한 관련이 언어용 충분히 사용할 수 있습니다. 예를 들어 앱에 스페인어로 된 전체 리소스 집합이 있을 경우 앱의 일부 UI는 카탈로니아어로 지역화되지 않을 수 있습니다. 카탈로니아어 및 스페인어는 사용자에 대 한 카탈로니아어에 사용할 수 없는 리소스를 스페인어로 나타납니다.
- 일부 리소스는 대부분의 공통 리소스에 다른 리소스 지도 하는 동안 특정 언어에 대 한 예외를 필요할 수 있습니다. 이 경우 'und' 결정 되지 않은 언어 태그를 사용 하 여 모든 언어에 사용할 리소스를 표시 합니다. Windows 와일드 카드 'und' 언어 태그로 해석 (비슷합니다 '\*')는 다른 특정 일치 항목 뒤의 상위 앱 언어와 일치 합니다. 예를 들어 몇 개의 리소스가 핀란드어에 대해 서로 다르지만 나머지 리소스가 모든 언어에 대해 동일할 경우 핀란드어 리소스는 핀란드어 태그로 표시하고 나머지는 'und'로 표시해야 합니다.
- 글꼴 또는 텍스트의 높이 같은 언어 스크립트를 기반으로 하는 리소스 결정 되지 않은 언어 태그를 사용 하 여 지정된 된 스크립트를 사용 하 여: ' und-&lt;스크립트&gt;'. 예를 들어, 라틴 글꼴은 `und-Latn\\fonts.css`를 사용하고 키릴 글꼴은 `und-Cryl\\fonts.css`를 사용합니다.
...
일반적으로 [**Windows.Globalization**](/uwp/api/windows.globalization?branch=live) 네임스페이스의 API는 앱 런타임 언어 목록을 사용하여 언어를 결정합니다. 일치하는 형식을 가진 언어가 없으면 사용자 로캘이 사용됩니다. 이 로캘은 시스템 시계에 사용된 것과 같습니다. 제공 하는 사용자 로캘이 **설정을** > **시간 및 언어** > **언어 및 지역**  >  **추가 날짜, 시간 및 지역 설정** > **지역: 날짜, 시간 또는 숫자 형식 변경**합니다. 또한 **Windows.Globalization** API는 응용 앱 런타임 언어 목록 대신 사용할 언어 목록을 지정하는 재정의를 수락합니다.
...
<td align="left">없음</td>
<td align="left">영어(GB)</td>
<td align="left">UI: 영어(GB)<br>숫자/날짜/시간: 영어(GB)</td>
</tr>
<tr>
...
<td align="left">없음</td>
<td align="left">프랑스어(오스트리아)</td>
<td align="left">UI: 프랑스어 (프랑스) (프랑스어 (오스트리아)에서 대체 (fallback))<br>숫자/날짜/시간: 프랑스어(오스트리아)</td>
</tr>
<tr>
...
<td align="left">없음</td>
<td align="left">영어(캐나다); 프랑스어(캐나다)</td>
<td align="left">UI: 영어 (미국) (영어 (캐나다)에서 대체 (fallback))<br>숫자/날짜/시간: 영어(캐나다)</td>
</tr>
<tr>
...
<td align="left">UI: 스페인어 (스페인) (영어에 사용 가능한 대체 없음 이후 기본값을 사용)<br>날짜/시간/숫자 스페인어(스페인)</td>
</tr>
<tr>
...
<td align="left">없음</td>
<td align="left">카탈로니아어; 프랑스어(프랑스)</td>
<td align="left">UI: 대부분의 카탈로니아어 및 일부 프랑스어 (프랑스) 일부 문자열 카탈로니아어에 있기 때문에<br>숫자/날짜/시간: 카탈로니아어</td>
</tr>
<tr>
