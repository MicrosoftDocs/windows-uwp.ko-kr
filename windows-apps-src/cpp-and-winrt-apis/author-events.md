...
파일 탐색기에서 스텁 파일을 복사 `BankAccount.h` 및 `BankAccount.cpp` 폴더에서 `\BankAccountWRC\BankAccountWRC\Generated Files\sources\` 는 프로젝트 파일이 있는 폴더로 `\BankAccountWRC\BankAccountWRC\`, 대상에 파일을 바꿉니다. 이제 `BankAccount.h`와 `BankAccount.cpp`를 열고 런타임 클래스를 구현합니다. `BankAccount.h`에서 전용 멤버 2개를 BankAccount 구현체(팩터리 구현체 *아님*)에 추가합니다.
...
참조를 추가 하 고 이동 `\BankAccountWRC\Debug\BankAccountWRC\BankAccountWRC.winmd` (또는 두 프로젝트가 동일한 솔루션의 경우 프로젝트-프로젝트 참조를 추가). **추가**와 **확인**을 차례대로 클릭합니다. 이제 BankAccountCoreApp을 빌드합니다. 예기치 않은 오류 메시지가 표시 하는 이벤트가 있는 페이로드 파일인 `readme.txt` 하지 존재, Windows 런타임 구성 요소 프로젝트에서 해당 파일을 제외, 후 BankAccountCoreApp 리 빌드합니다.
...
이러한 두 대리자 형식에 대 한 형식 매개 변수 형식 매개 변수가 Windows 런타임 형식 이어야 너무 하므로, ABI 교차 해야 합니다. 등의 숫자 및 문자열 기본 형식 뿐만 아니라 첫 번째 및 제 3 자 런타임 클래스를 포함 합니다. 컴파일러에 도움이 되는 "*WinRT 형식 이어야 합니다.*" 오류 제약 조건의 잊을 경우.

모든 매개 변수 또는 이벤트 인수를 전달 해야 하지 고유의 간단한 Windows 런타임 대리자 형식을 정의할 수 있습니다. 아래 예제에서는 간단한 버전의 **BankAccount** 런타임 클래스를 보여 줍니다. **SignalDelegate** 이라는 대리자 형식을 선언 하 고를 사용 하는 매개 변수를 사용 하 여 이벤트 대신 신호 유형을 이벤트를 발생 시킵니다.
...
이벤트 내부적 으로만 사용 되는 경우 내에서 C + + WinRT [**winrt::event**](/uwp/cpp-ref-for-winrt/event) 구조체 템플릿을 계속 사용할 수 있지만 C + 매개 변수화 합니다 (전체에서 아니라 바이너리) 프로젝트 + /winrt의 비 Windows 런타임 [**winrt:: delegate&lt;... T&gt; **](/uwp/cpp-ref-for-winrt/delegate) 구조체 템플릿은 효율적이 고 참조 계산 대리자입니다. 임의 개수의 매개 변수를 지원 하 고 Windows 런타임 형식에 국한 되지는 않습니다.
...
이벤트 및 대리자 하지 함수 매개 변수로 전달 하는 것이 좋습니다. **추가** 기능 [**winrt::event**](/uwp/cpp-ref-for-winrt/event) 중 경우 대리자를 전달 해야 하기 때문에 한 가지 예외를입니다. 이 지침에 대 한 이유 대리자 (측면에서 지원 여부 하나의 클라이언트 등록 또는 다중) 여러 Windows 런타임 언어에서 다른 폼을 수행할 수 있는 때문입니다. 이벤트는 여러 구독자 모델을 사용 하 여 훨씬 더 예측할 수이 고 일관 된 옵션을 구성합니다.
