...
3. **[Visual Studio](https://www.visualstudio.com/)** 는 **유니버설 Windows 앱 개발 도구**입니다. UWP 앱에 대 한 버전은 Visual Studio 2015 업데이트 3 최소입니다. 개발자 및 보안 업데이트에 대 한 Visual Studio의 최신 릴리스를 사용 하는 것이 좋습니다. 
...
성공적으로 수 있도록 사용자의 장치에 필요한 다음 사전 설정 단계는 Xbox Live 및 다양 한 Xbox Live 서비스 호출을 사용 하 여 로그인 합니다.

### <a name="set-your-sandbox"></a>에 샌드박스를 설정 합니다.

샌드박스 [Xbox Live 서비스 구성](../xbox-live-service-configuration.md) 타이틀을 해제할 준비가 될 때까지 정품에서 격리 됨 상태로 유지 하는 방법을 제공 합니다. 축적 일부 데이터는 샌드박스 관련 됩니다. 예를 들어 타이틀 라는 *헤드샷*는 통계를 정의 하 고 타이틀을 테스트 하는 동안 일부 수가 헤드샷 사용자 계정에 축적 합니다. 이 값은 타이틀의 개발 샌드박스를 특정 되며는 헤드샷을 통해 수행 하지는 타이틀의 일반 정품 버전으로 전환 하는 경우.
...
에 로그인 하 여 개발 샌드박스를 하려면 테스트 계정 만들기 하거나에 샌드박스를는 일반 MSA (Microsoft 계정)에 대 한 액세스를 제공 해야 합니다. 이 일부 기타 혜택 뿐만 아니라 개발, 제목에 향상 된 보안을 제공합니다.
...
Xbox 서비스 API 및 c + + 및 WinRT UWP 및 XDK, 모두에 대 한에서 미러링 유형을 제공 있고 해당 네임 스페이스 **Microsoft.Xbox.Live.SDK.*로 구성 됩니다. UWP** 및 **Microsoft.Xbox.Live.SDK.* 합니다. XboxOneXDK**.
...
1. **솔루션 탐색기**에서 **참조** 노드를 마우스 오른쪽 단추로 클릭 하 고 **참조 추가** 선택 합니다.
...
Xbox Live 플랫폼 확장 SDK를 설치한 후에 Visual Studio에서 참조를 추가 해야 합니다. 하 여이 수행할 수 있습니다.
...
5.  타이틀에 적용 되는 값과 **TitleId** 및 **PrimaryServiceConfigId** 바꿔 다음 템플릿으로 구성 파일을 편집 합니다. Microsoft 개발자 센터에서 루트 Xbox Live 페이지에서 올바른 값을 가져올 수 있습니다. Microsoft 개발자 센터에서 **서비스 안내**으로 **PrimaryServiceConfigId** 나타납니다.
