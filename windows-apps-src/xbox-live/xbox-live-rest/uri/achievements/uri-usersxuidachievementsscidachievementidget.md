...
| --- | --- | --- | --- | --- | --- | --- | 
| 사용자| 예| Xbox LIVE 누구를 위해 요청 이루어지고에 유효한 사용자 합니다.| 403 사용할 수 없음| 
| 제목| 아니요| 호출 제목입니다.| 인증에 따라 다릅니다. 2013, 월 1 일 기준 인증 누락 된 경우 공용으로 표시 되지 않은 모든 SCIDs에 대 한 액세스를 거부 따라서는 클레임을 제공 하지 않습니다.| 
| 샌드박스| 아니요| 결과 검색할 샌드박스입니다.| 인증에 따라 다릅니다. 2013, 월 1 일 기준 인증 누락 된 기본 클레임을 제공 하지 않습니다.| 
...
| 헤더| 유형| 설명| 
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | 
| 권한 부여| 문자열| HTTP 인증에 대 한 인증 자격 증명 합니다. 예제 값: "XBL3.0 x =&lt;userhash>; &lt;token> ".| 
...
| Accept Language| string| 원하는 로캘 및 대체 (예: FR-FR, fr, EN-US ~ EN-GB, en 전세계) 목록입니다. 지역화 된 문자열을 일치를 찾을 때까지 도전 과제 서비스 목록에서 작동 합니다. 발견 되 면 사용자의 IP 주소에서 제공 되는 사용자 토큰에 정의 된 위치와 일치 하도록 시도 합니다. 없는 일치 하는 지역화 된 문자열을 여전히 발견 되 면 제목 개발자/게시자가 제공 하는 기본 문자열을 사용 합니다. | 
