...
ms.locfileid: "3423123"
---
# <a name="working-with-symbologies"></a>기호 처리
[바코드 기호](https://docs.microsoft.com/uwp/api/windows.devices.pointofservice.barcodesymbologies)는 특정한 바코드 형식으로 데이터를 매핑한 것입니다. 일반적인 기호로 UPC, Code 128, QR 코드 및 등을 포함합니다.  유니버설 Windows 플랫폼 바코드 스캐너 Api 사용 하면 응용 프로그램이 스캐너를 수동으로 구성 하지 않고도 스캐너가 이러한 기호 처리 하는 방법을 제어할 수 있습니다. 
...
스캐너에서 특정 기호가 지원 하는지 확인할 [IsSymbologySupportedAsync](https://docs.microsoft.com/uwp/api/windows.devices.pointofservice.barcodescanner.issymbologysupportedasync#Windows_Devices_PointOfService_BarcodeScanner_IsSymbologySupportedAsync_System_UInt32_)호출할 수 있습니다.
...
완료 된 특성을 수정 하 고 설정할 준비가 되 면 [SetSymbologyAttributesAsync](https://docs.microsoft.com/uwp/api/windows.devices.pointofservice.claimedbarcodescanner.setsymbologyattributesasync)호출할 수 있습니다. 이 메서드는 특성 성공적으로 설정 하는 경우 **true** **부울**을 반환 합니다.
...
* **범위**: **DecodeLength1** 및 **DecodeLength2** 싱글바이트 문자 사이의 길이 디코딩합니다. 순서 **DecodeLength1** 와 **DecodeLength2** do (하나 사용 가능 다른 보다 높거나 낮은) 하는 중요 하지 않습니다.
...
다음 코드 조각은 설정을 확인 숫자 전송 하는 방법을 보여 줍니다.
