...
좋은 IA는 사용자 시나리오를 가능하게 할 뿐만 아니라 시작할 주요 화면을 구상하는 데 도움이 됩니다. 예를 들어 [들리는](http://go.microsoft.com/fwlink/p/?LinkID=268089) 앱 사용자의 라이브러리, 저장소, 뉴스 및 통계에 대 한 액세스를 제공 하는 허브로 바로 시작 합니다. 사용자가 가져오고 오디오북을 빠르게 즐길 수 있도록 경험 포커스가 합니다. 앱의 심층 수준에서는 보다 구체적인 작업에 중점을 둡니다.
...
**다른 보기, 폼 팩터 및 하드웨어 구성을**Windows 충전 및 중점에서 앱에 사용자를 배치 합니다. 사용자가 해당 앱을 사용하기로 결정한 어떤 디바이스에서든, 어떤 입력 모드를 사용하든, 어떤 방향으로든, 어떤 하드웨어 구성에서든, 어떤 환경에서든 앱 UI가 두드러지게 하고 싶을 것입니다.

**터치 중심**Windows에서는 마우스 기능을 에뮬레이트하는 보다 더 간단 하 게 하는 고유한 터치 환경을 제공 합니다.
...
**매력적이 고 신선한**앱에 신선한 느낌을 사용자가 이러한 표준 환경 해야 합니다.
...
-   **로밍**작업 한 위치를 선택할 수 있도록 하는 UX 가장, 사용 중인 장치와 관계 없이 유지 하는 데이터를 로밍 하 여 장치에서 지속적인 환경을 만듭니다. 로밍을 통해 설정 및 상태를 유지 관리하여 부엌에 있는 가족용 PC, 회사 PC, 개인용 태블릿, 기타 폼 팩터 등 어디에서든 앱을 쉽게 사용할 수 있게 합니다. [응용 프로그램 데이터 관리](../design/app-settings/store-and-retrieve-app-data.md)에 대해 살펴보고, [로밍 중인 응용 프로그램 데이터에 대한 지침](https://msdn.microsoft.com/library/windows/apps/hh465094)을 참조하세요.
-   **사용자 타일**  확인가 사용자 타일 이미지를 로드 하 여 사용자에 게 앱을 개인 또는 앱에서 콘텐츠를 Windows 전체의 개인 타일로 설정할 수 있도록 합니다.

**장치 기능**앱이 최신 오늘날의 디바이스의 기능 활용 해야 합니다.

-   **근접 연결 제스처**는 물리적으로 가까이에서 실제로 디바이스를 함께 탭 하 여 (멀티 플레이어 게임)을 가진 다른 사용자와 장치를 연결할 수 있게 합니다. [근접 연결 및 탭하기](https://msdn.microsoft.com/library/windows/apps/hh465229)에 대해 자세히 알아보세요.
-   **카메라 및 외장형 저장 장치**사용자에 게 채팅 및 회의 기록 vlogs, 프로필 pics, 세계를 문서화 하는 기본 제공 또는에서 연결 카메라를 연결 하거나 어떤 활동 앱은 특성입니다. [이동식 저장소의 콘텐츠 액세스](https://msdn.microsoft.com/library/windows/apps/hh465189)에 대해 자세히 알아보세요.
...
-   **지리적 위치**사용자가 문제를 해결할 지도, 위치 또는 근처에 있는 사람, 활동, 목적지에 대 한 알림을 받기 위해 표준 웹 데이터에서 나 지리적 위치 센서의 지리적 위치 정보를 사용 합니다. [지리적 위치](https://msdn.microsoft.com/library/windows/apps/hh465139)에 대해 자세히 알아보세요.
...
**처음 시작**사용자 서비스에 등록 하 고 해당 계정에 로그인 하거나 자체 콘텐츠를 추가, 전에 어떤 화면 보게 될까요? 사용자에게 정보를 요청하기 전에 앱의 가치를 입증해 보세요. 사람들에게 사용 확정을 요청하기 전에 살펴보고 앱의 기능을 이해할 수 있도록 샘플 콘텐츠를 보여 주는 것이 좋습니다.

**홈페이지**홈 페이지는 사용자를 표시는 앱을 시작할 때마다입니다. 여기에 표시되는 콘텐츠는 앱의 주요 특징을 소개하고 적절한 용도를 바로 보여 줍니다. 이 페이지에 한 가지 유용한 특성을 제공하여 사람들이 앱의 나머지 부분을 탐색하도록 유도합니다. 방문 페이지에서는 검색 기능보다 사용자의 주의를 분산시키는 요소 제거에 중점을 둡니다.
