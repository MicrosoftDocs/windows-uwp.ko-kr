...
강력한 이미 응용 프로그램에서 제공 하는 경우 사용자 지정 인식 기능을 사용할 수 있습니다 [AreCandidatesEnabled](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview.arecandidatesenabled) 속성에 기본 제공 제안을 사용 하지 않으려면 다음 예와 같이.
...
1. 먼저 처리 Loaded 이벤트 응용 프로그램의 시각적 트리 통과 시작 하려면 FindInnerTextBox 함수를 호출 하는 위치 합니다.
...
경우에 따라 [HandwritingView](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.handwritingview) 그렇지 수 없는 있습니다. UI 요소에 설명 있는지 확인 해야 할 수 있습니다.
...
정보는 팝업 같은 텍스트 입력에 대 한 응답에 표시 되는 사용자 지정 UI 경우 필기 보기를 안 보일 하지 되므로 해당 UI의 위치를 변경 해야 합니다.
