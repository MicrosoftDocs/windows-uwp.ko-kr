...
먼저 응용 프로그램에 대 한 패키지를 만듭니다. 그런 다음 설치 하 고 실행 동작을 관찰 합니다. 호환성 문제를 식별 하는 데 도움이 되는 오류 메시지가 나타날 수 있습니다. 또한 문제를 식별 [프로세스 모니터](https://docs.microsoft.com/en-us/sysinternals/downloads/procmon) 를 사용할 수 있습니다.  일반적인 문제는 프로그램 경로 권한과 작업 디렉터리에 대 한 응용 프로그램 가정 관련이 있습니다.
...
이 문제에 해당 패키지 경로.log 파일을 작성 하는 응용 프로그램 실패 했습니다. 이 것이 좋습니다 파일 리디렉션 수정 도움이 될 수 있습니다.
...
MSIX 컨테이너에서 실행 되는 응용 프로그램에서 액세스할 수 없는 디렉터리의 데이터를 읽거나 쓸 하려고 리디렉션할 수 있으며 [파일 리디렉션 수정](https://github.com/Microsoft/MSIX-PackageSupportFramework/tree/master/fixups/FileRedirectionFixup) 을 사용할 수 있습니다.
...
커뮤니티 기여 하려면 [GitHub](https://github.com/Microsoft/MSIX-PackageSupportFramework) 페이지를 검토 해야 합니다. 다른 개발자가 유사한 문제 해결을 공유한 런타임 수정 가능 합니다.
...
.Msix (또는.appx) 파일을 이미 있는 경우 패키지에 대 한 준비 영역으로 지원할 수 있는 레이아웃 폴더에 해당 콘텐츠 압축을 풉니다 수 있습니다. Sdk 설치 경로에 따라 makemsix 도구를 사용 하 여 명령 프롬프트에서 이렇게 하려면,이 여기서 것 makemsix.exe 도구를 Windows 10 PC에서: x86: C:\Program Files (x86) \Windows Kits\10\bin\x86\makemsix.exe x64: C:\Program Files ( x86) \Windows Kits\10\bin\x64\makemsix.exe
...
파일 이름을 만들기 ``config.json``, 해당 파일을 패키지의 루트 폴더에 저장 합니다. 방금 교체 하는 실행 파일을 가리키도록 config.json 파일의 선언 된 응용 프로그램 ID를 수정 합니다. 프로세스 모니터를 사용 하 여에서 얻은 정보를 사용 하도 작업 디렉터리를 설정 뿐 아니라 수 파일 리디렉션 수정을 사용 하 여 패키지 상대 "PSFSampleApp" 디렉터리 아래에서.log 파일을 읽기/쓰기를 리디렉션하 합니다.
...
| 응용 프로그램 | workingDirectory | (선택 사항) 시작 하는 응용 프로그램의 작업 디렉터리를 사용 하 여 패키지 상대 경로입니다. 이 값을 설정 하지 않으면, 운영 체제를 사용 합니다 `System32` 응용 프로그램의 작업 디렉터리로 디렉터리 합니다. |
| 프로세스 | 실행 | 대부분에서의 이름이 됩니다는 `executable` 제거 경로 파일 확장명을 가진 위에 구성 합니다. |
...
패키지 된 응용 프로그램 호환성 문제를 진단 하는 대체 기술을 추적 수정을 사용 하는 것입니다. 이 DLL의 PSF 포함 이며 앱의 동작을 프로세스 모니터와 유사한의 진단 세부 보기를 제공 합니다.  응용 프로그램 호환성 문제를 표시 하도록 특별히 설계 되었습니다.  추적 수정을 사용 하 여를 추가 하 DLL 패키지에 다음 조각에 config.json를 추가 하 고 다음 패키지 응용 프로그램을 설치 합니다.
...
기본적으로 추적 수정을 "예상"으로 간주할 수도 있는 오류를 필터링 합니다.  예를 들어, 응용 프로그램 무조건적으로 확인 하는 경우 이미 있는 결과 무시 하 고 않고 파일을 삭제 하려고 할 수 있습니다. 이것은 파일 시스템 기능에서 모든 오류를 수신 하도록 선택 하는 위의 예제에서는 되므로 일부 예기치 않은 오류 필터링 얻을 수 있는 맬웨어 결과가 있습니다. 그 전에 Config.txt 파일에서 읽는 시도 실패 하는 메시지 "파일 찾을 수 없음"에서 알고 있으므로이 수행 합니다. 이 자주 관찰 되 고 하지 일반적으로 예상 된 것으로 간주 되는 실패 합니다. 실제로 것 가능성이 가장 예기치 않은 오류를 에게만 필터링 하 고 다음 폴백 모든 오류를 여전히 식별할 수 없는 문제가 있으면 시작 합니다.

기본적으로 연결 된 디버거에 추적 수정을에서 출력 전송 합니다. 이 예제에서는 디버거를 연결 하려고 하지 하 고 출력을 보려면 SysInternals에서 [DebugView](https://docs.microsoft.com/en-us/sysinternals/downloads/debugview) 프로그램을 대신 사용 됩니다. 앱을 실행 한 후 알 수 있습니다 동일한 오류 이전과 하는 것 가리킵니다 우리는 동일한 런타임 수정 합니다.
...
| Project | 목적 |
|-------|-----------|
| DesktopApplicationPackage | 이 프로젝트는 [Windows 응용 프로그램 패키징 프로젝트](desktop-to-uwp-packaging-dot-net.md) 를 기반으로 하며 MSIX 패키지 출력. |
| Runtimefix | 런타임 수정으로 역할을 하는 하나 이상의 대체 함수를 포함 하는 c + + Dynamic-Linked 라이브러리 프로젝트입니다. |
...
디버그 또는 확장 된 기존 런타임 수정 하려는 경우이 가이드의 [런타임 수정 사항 찾기](#find) 섹션에 설명 된 지침을 사용 하 여 얻은 런타임 수정 파일을 추가 합니다.
...
| 수정 | 구성 | (선택 사항) DLL 수정을 동작 하는 방법을 제어 합니다. 이 값의 정확한 형식으로 원하는 각 수정이 "blob" 해석할 수 수정 하 여 수정 별로 달라 집니다. |
...
Visual Studio에서 f5 키를 눌러 디버거를 시작 합니다.  먼저 시작 하는 구름을 대상 데스크톱 응용 프로그램을 시작 하면 PSF 시작 관리자 응용 프로그램에 합니다.  대상 데스크톱 응용 프로그램을 디버깅 하려면 수동으로 **디버그**선택 하 여 데스크톱 응용 프로그램 프로세스에 연결 해야 합니다->**프로세스에 연결**하 고 다음 응용 프로그램 프로세스를 선택 합니다. 기본 런타임 수정 DLL 사용 하 여.NET 응용 프로그램의 디버깅을 허용 하려면 관리 코드와 네이티브 코드 유형 (혼합된 모드 디버깅)를 선택 합니다.  
...
>[!NOTE]
> Visual Studio는 가장 간단한 개발 제공 되며, 디버깅, 일부의 제한이,이 가이드의 뒷부분에서 적용할 수 있는 다른 디버깅 기술을 설명 하겠습니다.

## <a name="create-a-runtime-fix"></a>런타임 수정 만들기

런타임을 해결 하려면를 대체 함수를 작성 하 고 모든 구성 데이터를 포함 하 여 새로운 런타임 수정을 만들 수 있습니다 문제에 대 한 해결 아직 없는 경우는 타당 합니다. 각 부분에 대해 살펴보겠습니다.

### <a name="replacement-functions"></a>대체 기능

먼저, 응용 프로그램 MSIX 컨테이너에서 실행 되 면 호출이 실패 하는 함수를 식별 합니다. 그런 다음 런타임 관리자를 대신 호출 하 고 싶은 대체 함수를 만들 수 있습니다. 이렇게 하면 최신 런타임 환경 규칙에 맞는 동작을 함수의 구현 체 바꿉니다 수 있습니다.

Visual Studio에서이 가이드 앞부분에서 만든 런타임 수정 프로젝트를 엽니다.

선언 합니다 ``FIXUP_DEFINE_EXPORTS`` 매크로 대 한 include 문을 추가 하 고는 `fixup_framework.h` 각각의 맨 합니다. 런타임 수정 사항을의 기능을 추가 하려는 CPP 파일입니다.
...
#### <a name="protect-against-recursive-calls-to-functions-in-runtime-fixes"></a>런타임 수정 함수를 중첩할 호출 으로부터 보호

선택적으로 적용할 수 있습니다 합니다 `reentrancy_guard` 런타임 수정 함수를 중첩할 호출 으로부터 보호 하는 함수를 입력 합니다.

에 대 한 대체 함수를 생성할 수 있습니다 예를 들어 합니다 `CreateFile` 함수입니다. 구현을 호출할 수는 `CopyFile` 함수 하지만 구현의 합니다 `CopyFile` 함수를 호출할 수는 `CreateFile` 함수. 이에 대 한 호출에서 무한 반복 주기 시킬 수는 `CreateFile` 함수.
...
런타임 수정에 구성 데이터를 추가 하려는 경우 추가할 것을 고려 하 고 ``config.json``. 사용할 수 이런 식으로는 `FixupQueryCurrentDllConfig` 를 쉽게 해당 데이터를 구문 분석 합니다. 이 예제에서는 해당 구성 파일에서 문자열 및 부울 값을 구문 분석합니다.
...
첫째,는.msix에서 설치 하는 것이 아니라 패키지 레이아웃 폴더 경로에서 느슨한 파일 배포 하 여 응용 프로그램을 실행 F5 디버깅 /.appx 패키지입니다.  일반적으로 레이아웃 폴더가 설치 패키지 폴더와 동일한 보안 제한을가지고 있지 않습니다. 결과적으로, 런타임 수정 적용 하기 전에 패키지 경로 액세스 거부 오류를 재현 불가능할 수도 있습니다 것입니다.
...
또 다른 문제가 Visual Studio 디버거를 통해 시작 된 모든 자식 프로세스에 연결 하는 것에 대 한 기본 제공 지원이 없기는입니다.   이렇게 하면 시작 된 후 Visual Studio에서 수동으로 연결 되어 있어야 합니다 대상 응용 프로그램의 시작 경로에서 논리를 디버그 하기가 어렵습니다.

하위 프로세스를 지 원하는 디버거를 사용 하 여가이 문제를 해결 하려면 다음을 첨부 하세요.  참고 아닌지 일반적으로 대상 응용 프로그램에 적시에 (JIT) 디버거를 첨부할 수 있습니다.  대부분의 JIT 기술 관련 ImageFileExecutionOptions 레지스트리 키를 통해 대상 앱 대신 디버거를 시작 하는 때문입니다.  이 방법을 detouring 메커니즘 PSFLauncher.exe FixupRuntime.dll 대상 앱에 삽입 하는 데 사용 합니다.  WinDbg, [Windows 용 디버깅 도구](https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/index)포함 되어 있으며 [Windows SDK](https://developer.microsoft.com/en-US/windows/downloads/windows-10-sdk)에서 가져온 지원 하위 프로세스를 연결 합니다.  또한 지원 직접 [시작 하 고 UWP 앱을 디버깅](https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/debugging-a-uwp-app-using-windbg#span-idlaunchinganddebuggingauwpappspanspan-idlaunchinganddebuggingauwpappspanspan-idlaunchinganddebuggingauwpappspanlaunching-and-debugging-a-uwp-app)합니다.

하위 프로세스도 대상 응용 프로그램 시작을 디버그 하려면 시작 ``WinDbg``.

```
windbg.exe -plmPackage PSFSampleWithFixup_1.0.59.0_x86__7s220nvg1hg3m -plmApp PSFSample
```

에 ``WinDbg`` , 묻는 메시지를 표시 하 고, 디버깅 자식을 활성화 하 고, 적절 한 중단점을 설정 합니다.
...
>[!NOTE]
> [PLMDebug](https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/plmdebug) 디버거를 시작 시 앱 연결할도 사용할 수 있으며 [Windows 용 디버깅 도구에](https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/index)에서 포함 되어 있습니다.  하지만 이제 WinDbg에서 제공 하는 직접 지원 보다 사용 하기 더 복잡 한 것입니다.
