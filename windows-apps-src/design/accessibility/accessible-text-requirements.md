...
여기서는 설명 텍스트 대비에 권장 되는 표준 웹 액세스 가능성을 기반한 [G18: 확인의 대비 비율은 최소 배경의 사이 있는 텍스트 (텍스트의 이미지) 및 텍스트 뒤에 나타나는 배경의](https://go.microsoft.com/fwlink/p/?linkid=221823)합니다. 이 지침은 *WCAG 2.0에 대한 W3C 기술* 사양에 있습니다.
...
## <a name="text-font-size-and-scale"></a>텍스트 글꼴 크기 조정 및 규모
...
다양한 텍스트 요소와 컨트롤에 [**IsTextScaleFactorEnabled**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.textblock.istextscalefactorenabled) 속성이 있습니다. 이 속성은 기본적으로 **true** 값으로 설정됩니다. 때 **true**, 해당 요소에서 텍스트의 크기를 확장할 수 있습니다. 에 작은 텍스트에 영향을 줍니다 크기 조정은 **FontSize** 보다 큰 있는 텍스트에 영향을 보다 심층적 **FontSize**합니다. 요소를 설정 하 여 자동 크기 조정을 해제할 수 있습니다 **IsTextScaleFactorEnabled** 속성을 **false**합니다. 

참조 [텍스트 크기 조정](https://docs.microsoft.com/windows/uwp/design/input/text-scaling) 대 한 자세한 내용은 합니다.

앱에 다음 태그를 추가 하 고 실행 합니다. 조정 된 **텍스트 크기** 설정과 각각 어떻게 **TextBlock**합니다.
...
모든 앱에서 보편적으로 크기 조정 UI 텍스트는 사용자에 대 한 중요 한 내게 필요한 옵션 환경에 따라 텍스트 확장을 해제 하는 권장 하지 않습니다.
