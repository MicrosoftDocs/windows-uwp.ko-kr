...
클라이언트가 오프 라인 상태인 경우에는 [캐시 정책](https://docs.microsoft.com/previous-versions/windows/apps/hh465435(v=win.10)) 헤더가 알림에 포함 된 경우에만 기본 알림이 WNS에 의해 캐시 됩니다. 그러나 장치가 다시 온라인 상태가 되 면 하나의 원시 알림만 캐시 되 고 배달 됩니다.
