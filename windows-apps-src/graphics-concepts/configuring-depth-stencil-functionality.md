...
-   모든 렌더링 대상 모든 차원에서 같은 크기를 생성 해야 합니다 (너비 및 높이 대 한 3D 또는 배열 크기에 대 한 깊이 \*배열 형식).
...
-   [혼합](#compositing)
-   [Decaling](#decaling)
-   [윤곽선과 실루엣](#outlines-and-silhouettes)
-   [양면 스텐실](#two-sided-stencil)
-   [깊이 스텐실 버퍼 질감으로 읽기](#reading-the-depth-stencil-buffer-as-a-texture)
...
일반적으로 모든 증가 및 감소된 값은 상계됩니다. 그러나 장면은 이미 정상 기하 도형으로 렌더링되었기 때문에 섀도 볼륨이 렌더링될 때 일부 픽셀이 z 버퍼 테스트에서 실패합니다. 스텐실 버퍼에 남은 값이 그림자의 픽셀에 해당합니다. 이 나머지 스텐실 버퍼 내용은 마스크로 사용되어 전부를 포함하는 큰 검은색 사각형을 장면에 알파 혼합합니다. 스텐실 버퍼가 마스크로 작용하므로 결과는 그림자 안의 픽셀이 어두워지는 것입니다.
...
[출력 병합기 (OM) 단계](output-merger-stage--om-.md)
