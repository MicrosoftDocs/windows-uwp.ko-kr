...
동일한 버전 번호를 사용하는 여러 Windows 10 패키지를 제공하는 경우 지정된 장치에 제공할 패키지를 고려할 때 순위를 지정하는 데 아키텍처가 사용됩니다(x64, x86, ARM, 중립 순). 동일한 버전 번호를 사용하는 앱 번들의 등급을 매길 때 번들 내 가장 높은 아키텍처 등급이 고려됩니다. x64 패키지를 포함하는 앱 번들은 x86 패키지만 포함하는 앱 번들보다 등급이 높습니다.
...
> [!NOTE]
> 또한 Windows 10 패키지가 있는 경우 Windows 10 패키지의 버전 번호는 게시할 예정이거나 게시한 모든 Windows 8, Windows 8.1 및/또는 Windows Phone 8.1 패키지의 버전 번호보다 높아야 합니다. 자세한 내용은 [이전에 게시된 앱에 Windows 10용 패키지 추가](guidance-for-app-package-management.md#adding-packages-for-windows-10-to-a-previously-published-app)를 참조하세요.
