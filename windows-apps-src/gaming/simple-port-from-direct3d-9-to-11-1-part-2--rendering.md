...
> **참고**  픽셀 셰이더에 꼭 짓 점 셰이더 바인딩 위한 Direct3D 9의 규칙은 Direct3D 11의 규칙 보다 더 완화 된 되었습니다. Direct3D 9 정렬은 유연하지만 비효율적이었습니다.
...
> **참고**꼭 짓 점 셰이더는 SV\_POSITION 시스템 값 의미 체계를 출력 해야 합니다. 이 의미 체계는 꼭짓점 위치 데이터를 결정하여 값을 조정합니다. 여기서 x는 -1과 1 사이입니다. y는 -1과 1 사이입니다. z는 원래 같은 유형의 좌표 w 값으로 나눕니다(z/w). w는 원래 w 값으로 1을 나눈 값입니다(1/w).
...
> **참고**  HLSL 컴파일러에 대 한 올바른 대상 기능 수준을 설정 해야 합니다: Visual Studio에서 HLSL 소스 파일을 마우스 오른쪽 단추로 클릭 하 고 속성을 선택 하는 **셰이더 모델** 설정 변경 **HLSL 컴파일러-&gt; 일반**. 앱이 Direct3D 셰이더 자원을 만들 때 Direct3D는 하드웨어 기능에 대해 이 속성을 확인합니다.
