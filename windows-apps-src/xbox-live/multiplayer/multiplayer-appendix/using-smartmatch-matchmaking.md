...
매치 메이 킹을 시작 하기 전에 매치 메이 킹을 함께 입력 하려면 사용자 그룹을 나타낼 매치 메이 킹 "정찰" 일치 티켓 세션을 설정 합니다. 이 그룹의 모든 사용자 **(문자열, 부울, 부울) MultiplayerSession.Join 메서드**를 사용 하 여 세션에 참가 합니다.
...
응답을 일치 티켓 생성 메서드는 **CreateMatchTicketResponse 클래스** 개체입니다. 응답 일치 티켓 ID가 포함 되어 사용할 수 있는 GUID를 사용 하 여 매치 메이 킹 티켓을 삭제 하 여 취소할 수 있습니다. 응답은 사용자 설정에 사용할 수 있는 hopper에 대 한 평균 대기 시간을도 포함 되어 있습니다.
...
이제 제목 해야 다음 충분 한 플레이어, 표시에 있는지 확인 하기 위해 대상 세션을 초기화 하는 단계를 수행 및 품질의 QoS (서비스) 검사를 성공적으로 연결할 수 있는를 수행 합니다. 초기화 및/또는 QoS 실패 하면 다른 그룹을 찾을 수 있도록 제목 매치를 다시 제출 하기 위해 티켓 세션을 표시 합니다. 프로세스에 대 한 자세한 내용은 [대상 세션 초기화 단계 및 QoS를](smartmatch-matchmaking.md)참조 하세요.
...
매치 메이 킹 서비스 세션에 대 한 일치 티켓 생성 될 때 한 번에 티켓 세션의 스냅숏을 사용 합니다. 따라서 모든 플레이어에 가입 하거나 티켓 세션 않으면 제목은 삭제 하 고 다시 일치 티켓 매치 메이 킹 서비스를 사용 해야 합니다.
...
| 중요                                                                                                                                                                                                                       |
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| *PreserveSession* 를 사용 하 여 두 개의 세션을 항상 설정 수 없음을 결합 될 수 있으므로 서로 일치 시킬 수 있다는 것을 것입니다. 제목에서 사용 되는 매치 메이 킹 흐름 고려이 취해야 합니다. |
...
| 참고                                                                                                                                                                                                         |
|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 티켓 세션 개체에 일치 하지 않는 상수 있으면 만들기 티켓 메서드를 완료할 수 없습니다. 필수인을 추가 하 여이 방지할 수 있습니다 규칙 상수를 일치 하는 플레이어와 일치 하는 것을 방지 하는 hopper입니다. |

**MatchTicketDetailsResponse.PreserveSession 속성** 을 사용 안 함 설정 매치 메이 킹 서비스는 티켓의 내부 표현으로 서버 측정 각 구성원에서 복사 합니다. "특수" 티켓 특성으로 티켓의 내부 표현에 저장 된 티켓을 단일 서버 측정 컬렉션으로 티켓의 구성원 서버 측정을 병합 합니다.

**MatchTicketDetailsResponse.PreserveSession** 은 항상 설정, 서버 측정 사용 되지 않습니다. 대신, 매치 메이 킹 서비스 티켓 내부 표현에 대기 시간이 0 1 크기의 serverMeasurements 컬렉션으로에 세션에 대 한 /properties/system/matchmaking/serverConnectionString 값을 복사 합니다.

5.  매치 메이 킹 서비스는 티켓 세션 측정 컬렉션 고려 하 여 서버를 수행 하는 다른 그룹을 나타내는 다른 사용자와 일치 합니다. 기본으로 매우 동일한 데이터 센터에 있는 다른 그룹을 사용 하 여 그룹 일치 하려고 합니다.
...
8.  일부 또는 모든 클라이언트 **GameServerPlatformService.AllocateClusterAsync 메서드**를 호출합니다. 첫 번째 승인을 다른 수신 하는 동안 할당을 트리거합니다. 메서드는 MPSD에서 대상 세션을 가져오고 대상 세션 뿐만 아니라 부하 및 기타 Xbox Live 계산 관련 기술 datacenter 기본 설정에 따라 데이터 센터를 선택 합니다.
9.  모든 클라이언트를 재생합니다.
