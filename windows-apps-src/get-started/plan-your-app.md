...
좋은 IA는 사용자 시나리오를 가능하게 할 뿐만 아니라 시작할 주요 화면을 구상하는 데 도움이 됩니다. [들리는](http://go.microsoft.com/fwlink/p/?LinkID=268089) 앱 예를 들어 사용자의 라이브러리, 저장소, 뉴스 및 통계에 대 한 액세스를 제공 하는 허브로 바로 시작 합니다. 사용자가 가져오고 오디오북을 빠르게 즐길 수 있도록 경험 포커스가 합니다. 앱의 심층 수준에서는 보다 구체적인 작업에 중점을 둡니다.
...
**앱 계약**앱은 광범위 한, 앱 간, 기능 간 사용자 흐름을 사용 하는 앱 계약에 참여할 수 있습니다.

-   **공유**사용자가 다른 앱을 통해 다른 사람들과 앱의 콘텐츠를 공유 하 고 다른 사람 및 앱 으로부터 공유 가능한 콘텐츠를 너무 받을 수 있도록 합니다.
-   **원격 재생**사용자가 오디오, 비디오 또는 홈 네트워크의 다른 장치에 앱에서 이미지를 즐길 수 있도록 합니다.
-   **파일 선택기 및 파일 선택기 확장** 사용자가 로드 하 고 로컬 파일 시스템, 연결 된 저장 장치, 홈 그룹 또는 기타 앱에서 해당 파일을 저장할 수 있도록 합니다. 다른 앱에서 사용자 앱의 콘텐츠를 로드할 수 있도록 파일 선택기 확장을 제공할 수도 있습니다.
...
**장치 접근 권한 값**앱이 최신 오늘날의 디바이스의 기능을 최대한 활용 합니다.
...
-   **카메라 및 외장형 저장 장치**사용자가 자신의에서 연결 또는 기본 제공 카메라 채팅 및 회의 기록, 세계를 문서화 하는 프로필 pics 라인 vlogs 연결할 또는 어떠한 활동 앱은 특성입니다. [이동식 저장소의 콘텐츠 액세스](https://msdn.microsoft.com/library/windows/apps/hh465189)에 대해 자세히 알아보세요.
...
-   **지리적 위치**표준 웹 데이터 나 지리적 위치 센서의 지리적 위치 정보를 사용 하 여 사용자의 해결, 지도, 위치 또는 근처에 있는 사람, 활동, 목적지에 대 한 알림 메시지를 가져옵니다. [지리적 위치](https://msdn.microsoft.com/library/windows/apps/hh465139)에 대해 자세히 알아보세요.
...
**명령 구성 방법?** 단계 흐름의 개요를 사용 하 여 디자인 해야 하는 잠재적 명령을 식별할 수 있습니다. 그런 후 앱에서 해당 명령을 사용할 위치를 고려합니다.

-   **항상 콘텐츠를 사용 하려고 시도 합니다.** 가능 하도록된 사용자가 앱의 캔버스에서 콘텐츠를 직접 조작할 아닌 콘텐츠에 역할 명령을 추가 합니다. 예를 들어 사용자가 여행 앱에서 활동을 선택하고 위쪽 또는 아래쪽 명령 단추를 사용하기보다 캔버스의 목록에 활동을 끌어다 놓아 여행 일정표를 다시 짤 수 있도록 합니다.
...
**시작 화면**시작 화면, 가능한 한 빠르게 로드 되어야 하 고 남아 화면에만 앱 상태를 초기화 해야 합니다. 시작 화면에는 앱의 개성을 잘 나타내는 내용을 표시해야 합니다.

**처음 시작**하기 전에 사용자가 서비스에 대 한 등록 하 고 계정에 로그인 하거나 자체 콘텐츠를 추가, 어떤 보게 될까요? 사용자에게 정보를 요청하기 전에 앱의 가치를 입증해 보세요. 사람들에게 사용 확정을 요청하기 전에 살펴보고 앱의 기능을 이해할 수 있도록 샘플 콘텐츠를 보여 주는 것이 좋습니다.

**홈 페이지**는 사용자를 표시 있는 앱을 시작할 때마다 홈 페이지입니다. 여기에 표시되는 콘텐츠는 앱의 주요 특징을 소개하고 적절한 용도를 바로 보여 줍니다. 이 페이지에 한 가지 유용한 특성을 제공하여 사람들이 앱의 나머지 부분을 탐색하도록 유도합니다. 방문 페이지에서는 검색 기능보다 사용자의 주의를 분산시키는 요소 제거에 중점을 둡니다.

## <a name="validate-your-design"></a>디자인 유효성 검사

앱 개발을 많이 진행하기 전에 지침, 사용자 인상 및 요구 사항을 기준으로 디자인 또는 프로토타입의 유효성을 검사하여 나중에 다시 작업하는 일이 없도록 합니다. 각 기능에 UX 지침 집합과 앱 꾸미는 데 Microsoft Store에 앱을 게시 하려면 충족 해야 하는 스토어 요구 사항 집합이 있습니다. [Windows 앱 인증 키트](https://developer.microsoft.com/windows/develop/app-certification-kit)를 사용하여 기술 사양이 스토어 요구 사항에 맞는지 테스트할 수 있습니다. Microsoft Visual Studio의 성능 도구를 사용하여 모든 시나리오에서 최상의 환경을 사용자에게 제공하도록 설정할 수도 있습니다.
