...
description: 이 항목에서는 Windows 10 유니버설 Windows 플랫폼 (UWP) 앱을 매우 간단한 유니버설 8.1 앱을 포팅 사례 연구를 제공 합니다.
ms.date: 02/08/2017
ms.topic: article
...
**참고**    Bookstore1Universal 열면\_10 메시지가 "Visual Studio 업데이트 필요"를 표시 하는 경우 Visual Studio에서 다음의 단계에 따라 [TargetPlatformVersion](w8x-to-uwp-troubleshooting.md)합니다.

## <a name="downloads"></a>다운로드

[다운로드는 Bookstore1\_81 유니버설 8.1 앱](https://go.microsoft.com/fwlink/?linkid=532946)합니다.

[다운로드는 Bookstore1Universal\_10 Windows 10 앱](https://go.microsoft.com/fwlink/?linkid=532950)합니다.

## <a name="the-universal-81-app"></a>유니버설 8.1 앱

어떤 Bookstore1 같습니다\_81-포트에는 앱-것 같습니다. 앱의 이름과 페이지 제목 아래에는 세로로 스크롤되는 책의 목록 상자가 있습니다.

![어떻게 bookstore1\-81 windows 표시](images/w8x-to-uwp-case-studies/c01-01-win81-how-the-app-looks.png)

Bookstore1\_Windows에서 81

![어떻게 bookstore1\-81 windows phone에서 찾습니다](images/w8x-to-uwp-case-studies/c01-02-wp81-how-the-app-looks.png)

Bookstore1\_Windows Phone 81
...
프로젝트를 만들려면 새 Visual studio에서 파일 복사를 통해 되도록 Bookstore1에서 매우 빠른 작업은\_81, 새 프로젝트에서 복사 된 파일을 포함 합니다. 비어 있는 응용 프로그램(Windows 유니버설) 프로젝트를 새로 만들어 시작합니다. Bookstore1Universal 이름을\_10입니다. 이러한 파일은 Bookstore1에서 복사할 파일\_Bookstore1Universal 81\_10입니다.
...
방금 복사한 소스 코드와 태그 파일을 편집 하 고는 Bookstore1에 대 한 참조를 변경\_Bookstore1Universal 81 네임 스페이스\_10입니다. **파일에서 바꾸기** 기능을 사용하면 이 작업을 빠르게 수행할 수 있습니다. 보기 모델과 다른 명령적 코드에서 코드를 변경할 필요가 없습니다. 반환 된 값을 변경 하지만 실행 중인 앱의 버전을 보기 쉽게 확인 하기 위해 합니다 **Bookstore1Universal\_10.BookstoreViewModel.AppName** 속성을 "BOOKSTORE1\_81"를 " BOOKSTORE1UNIVERSAL\_10"입니다.

이제 앱을 빌드 및 실행할 수 있습니다. Windows 10으로 포트를 아직 명시적 작업 없이 완료 후 새 UWP 앱의 모양을 다음과 같습니다.

![초기 소스 코드가 변경된 Windows 10 앱](images/w8x-to-uwp-case-studies/c01-03-desk10-initial-source-code-changes.png)

데스크톱 장치에서 실행 되는 초기 소스 코드를 사용 하 여 Windows 10 앱 변경

![초기 소스 코드가 변경된 Windows 10 앱](images/w8x-to-uwp-case-studies/c01-04-mob10-initial-source-code-changes.png)

모바일 장치에서 실행 되는 초기 소스 코드를 사용 하 여 Windows 10 앱 변경
...
Bookstore1\_81 앱 사용 되는 두 개의 다른 리소스 사전이 (BookstoreStyles.xaml) Windows 8.1 및 Windows Phone 8.1 운영 체제의 스타일에 맞게 합니다. 이러한 두 BookstoreStyles.xaml 파일이 모두 Windows 10 앱에 필요한 스타일을 정확 하 게 포함 되어 있습니다. 하지만 다행히도 우리가 원하는 결과는 실제로 이러한 두 파일의 스타일보다 훨씬 더 간단합니다. 따라서 다음 단계에서는 프로젝트 파일 및 태그를 제거하고 단순화하는 작업을 주로 진행할 것입니다. 단계는 다음과 같습니다. 이 항목 맨 위에 있는 링크를 사용하여 프로젝트를 다운로드하고, 여기서부터 사례 연구 끝까지 포함된 모든 변경의 결과를 확인할 수 있습니다.

-   항목 사이의 간격을 촘촘히 하려면 MainPage.xaml에서 `BookTemplate` 데이터 템플릿을 찾고 **Grid** 루트에서 `Margin="0,0,0,8"`을 삭제합니다.
-   또한 `BookTemplate`에는 `BookTemplateTitleTextBlockStyle` 및 `BookTemplateAuthorTextBlockStyle`에 대한 참조가 있습니다. Bookstore1\_81 단일 키에 게 두 앱의 다른 구현을 있도록 간접 참조로 해당 키를 사용 합니다. 해당 간접 참조는 더 이상 필요하지 않습니다. 시스템 스타일은 직접 참조할 수만 있습니다. 따라서 해당 참조를 `TitleTextBlockStyle` 및 `SubtitleTextBlockStyle`로 각각 바꿉니다.
...
-   `TitlePanel`에서 `TitleTextBlockStyle`(현재 너무 큼)에 대한 참조를 `CaptionTextBlockStyle`에 대한 참조로 변경합니다. `PageTitleTextBlockStyle` 다른 Bookstore1\_더 이상 필요 하지 않은 81 간접 참조 합니다. 대신 참조 `HeaderTextBlockStyle`로 변경합니다.
...
모바일 장치에서 실행 되는 가져오는 Windows 10 앱
