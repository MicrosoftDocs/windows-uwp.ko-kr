...
? 포함 valuemetadata = 쿼리 매개 변수 모델 등 경주 트랙의 시간을 달성 하는 데 사용 하는 자동차의 색 사용자 상태 값과 연결 된 모든 메타 데이터를 포함 하도록 응답을 허용 합니다.

값 메타 데이터 응답에 포함 하려면 요청 호출 3 X Xbl-계약 버전 헤더 값도 설정 해야 합니다.
...
| 매개 변수| 형식| 설명|
| --- | --- | --- | --- | --- | --- |
| statNames| 문자열| 쉼표로 구분한 목록 사용자 통계 이름입니다. 예를 들어, 다음 URI는 URI에 지정 된 사용자 id를 대신 하 여 요청 된 4 개의 통계가 알립니다. {0}:: nomakrdown}<br/><br/>`https://userstats.xboxlive.com/users/xuid({xuid})/scids/{scid}/stats/wins,kills,kdratio,headshots?include=valuemetadata`| 
| include=valuemetadata| 문자열| 응답 들면 다음과 같습니다 상태 값과 연결 된 모든 값 메타 데이터를 포함 함을 나타냅니다.|
...
콘텐츠 격리 및 액세스 제어 시나리오에 대해 구현 하는 권한 부여 논리가 있습니다.
   ...
   * 순위표 및 사용자 통계는 호출자에 게 유효한 XSTS 토큰 요청과 함께 전송 된 모든 플랫폼에서 클라이언트에서 읽을 수 있습니다. 쓰기는 데이터 플랫폼에서 지 원하는 클라이언트를 제한 합니다.
...
| X-Xbl-Contract-Version| 문자열| 사용 하는 API의 버전을 나타냅니다. 이 값은 응답에서 메타 데이터 값을 포함 하기 위해 "3"에 설정 되어야 합니다.|
